...
有时文章的标题不需要转换，如維基詞典里的单字，或如《计-{}-算机世界报》之类的专有名词。在这种情况下，可以在文章中加一`__NOTC__` 或`__NOTITLECONVERT__`（注意前后都是两道下划线）来禁止对该文章标题的转换。然而，在中文维基百科，由于可能的简繁不同体问题（如“-{计算机世界报}-”和“-{計算機世界報}-”），我们不推荐使用这两个标签，而推荐使用下文中的“`-{T|}-`”转换。

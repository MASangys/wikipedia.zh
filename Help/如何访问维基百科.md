...
维基媒体基金会使用下列IP地址提供服务入口，您可以使用下面的IP地址替换后续教程（如[\#修正域名解析](https://zh.wikipedia.org/wiki/#修正域名解析 "wikilink")）中提供的IP地址。`text-lb`和`upload-lb`之间的数据不互通，通常应该使用`text-lb`中的IP地址，但是对于图片服务器（`upload.wikimedia.org`）及地图服务（`maps.wikimedia.org`）则应该使用`upload-lb`中的IP地址。对中国大陆使用IPv4的用户而言，使用位于**新加坡**（国际出口为[中国联通的用户除外](https://zh.wikipedia.org/wiki/中国联通 "wikilink")）或**旧金山**的服务器体验比较好；使用IPv6访问尚无明显差异。国际出口是指与中国大陆境外网络直接连接的中国运营商。

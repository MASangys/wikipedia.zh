...
维基媒体基金会旗下项目均支持[IPv6](../Page/IPv6.md "wikilink")连接。教育网和部分移动数据连接会使用IPv6，因此在不使用任何代理的情况下有可能可以直接访问维基百科。 然而，防火长城对一些语言的DNS污染可能会影响到IPv6地址的解析。如果有必要，仍然需要手动调整IPv6地址（参见[修正域名解析](https://zh.wikipedia.org/wiki/#修正域名解析 "wikilink")）。
...
因维基百科的ip可以连接，所以可以通过本地反代的方式规避防火长城的SNI检测。使用[Nginx](../Page/Nginx.md "wikilink")进行本地反向代理，详见**[此处](https://zh.wikipedia.org/wiki/Help_Talk:如何访问维基百科#使用Nginx本地反代无需代理服务器直连维基百科 "wikilink")**。

如果您不懂得如何配置，可以使用[Accesser](https://github.com/URenko/Accesser)（[Python](../Page/Python.md "wikilink")编写）。

### 修正域名解析

截至2019年4月24日，[防火长城](../Page/防火长城.md "wikilink")会对所有语言版本的维基百科（所有以`.wikipedia.org`结尾的[域名](../Page/域名.md "wikilink")）进行[DNS污染和基于](../Page/域名服务器缓存污染.md "wikilink")[SNI检测的](../Page/服务器名称指示.md "wikilink")[TCP连接重置](../Page/TCP重置攻击.md "wikilink")。对此您可以在设置修正域名解析（具体方法参见下文）后先访问未被封锁的维基媒体基金会旗下的其他项目（例如[元维基或](https://zh.wikipedia.org/wiki/元维基 "wikilink")[维基数据](../Page/维基数据.md "wikilink")）再切换至维基百科，即可在接下来的一段时间内正常访问。如果上述操作无效，可以等待几分钟后重试，通常即可成功连接。您也可以[使用代理服务访问维基百科以获得更加稳定的体验](https://zh.wikipedia.org/wiki/#代理服务器 "wikilink")。
...
：由于部分DNS服务商的出口服务器全部位于中国大陆以外，或者出口服务器部署有限无法针对所有地区或运营商进行优化，可能会导致用户在访问中国大陆境内的网站时被解析到不合适的IP地址（例如中国网站在海外架设的[CDN](https://zh.wikipedia.org/wiki/CDN "wikilink")），进而产生延迟增大、加载缓慢甚至失败等情况。如您遇到这类现象，可以试着查询当前使用的DNS出口的有关信息，若结果显示当前DNS出口与您的运营商不一致、与您所处的省份不一致或位于中国大陆以外且DNS服务器未使用[ECS功能](https://tools.ietf.org/html/rfc7871)（Edns-Client-Subnet），则可以考虑暂时停用上述DNS服务改用运营商的默认DNS或中国大陆主流厂商的智能DNS服务。
  ...
  -
    ...
    **注释：**
    ...
    :\* **ECS功能**允许DNS服务器向权威DNS服务器传达您的IP地址从而可以让权威DNS服务器返回合适的IP地址。因此，如果DNS服务商使用了该功能 ，那么不论DNS出口位于何处，返回给用户的结果都是合适的。

### 域前置

可以使用域前置的方法避开SNI封锁，但方法较为复杂，详见[相关讨论](https://zh.wikipedia.org/wiki/Help_talk:如何访问维基百科#修改火狐浏览器关于SNI的部分 "wikilink")（不建议使用）。
...
[翻墙软件是一种專用代理软件](https://zh.wikipedia.org/wiki/翻墙软件 "wikilink")，通常由代理服务提供商制作，该软件会自动配置电脑的代理设置，从而使您的电脑和目标服务器之间的流量都通过其指定的代理服务器中转。常见的翻墙软件有[赛风](https://zh.wikipedia.org/wiki/赛风 "wikilink")、[蓝灯](../Page/蓝灯.md "wikilink")、[自由门](../Page/自由门.md "wikilink")等。不同翻墙软件的使用方法都不尽相同，请閱讀相應软件官方下載網頁的使用说明。
...
鉴于一些[网络论坛](../Page/网络论坛.md "wikilink")中存在的網址过滤和发言字数限制，用户可以使用短网址服务压缩链接长度并绕开过滤规则，不过这些服务**并不能**用来绕过网络封锁。另外维基百科为了避免用户绕过[垃圾链接过滤器](https://zh.wikipedia.org/wiki/mw:Extension:SpamBlacklist "wikilink")，禁止用户添加主要短网址服务的链接，所以这些链接（除第一个及第二个外）都不能添加到维基百科中，建议各位用户在编辑时使用完整地址作为外部链接。
  ...
  - https://t.cn/：[新浪微博](../Page/新浪微博.md "wikilink")的短网址服务，通过第三方提供的短链接生成器可以压缩任意网址。该服务在中国大陆境内较为稳定。
  - https://dwz.cn/：[百度](../Page/百度.md "wikilink")的短網址服務，需要登录才能使用，可自訂短網址。該服務在大陸境內較穩定。

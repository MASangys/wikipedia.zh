**模板**（）是维基百科的系统中的一项功能，类似于[计算机语言中的](https://zh.wikipedia.org/wiki/计算机语言 "wikilink")[子程序](../Page/子程序.md "wikilink")。本文提供在[MediaWiki创建模板的技术指南](../Page/MediaWiki.md "wikilink")；维基百科使用[MediaWiki并对模板有一定的规范](../Page/MediaWiki.md "wikilink")
。另外也可参考：
...
[`一二``六七`](https://zh.wikipedia.org/wiki/一二{{{1}}}六七 "wikilink")
...
模板标签不仅可以调用模板，也可以调用（嵌入）其他页面，就是像模板一样把被调页面的内容加入到调用方页面。
  ...
  - 如果调用[主命名空间的条目](https://zh.wikipedia.org/wiki/WP:命名空间 "wikilink")，为了避免和调用同名模板产生歧义，需要在条目名前加冒号。如`{{:福寿岭站}}`会将[福寿岭站中的内容加入到该页来](../Page/福寿岭站.md "wikilink")，而不是调用{{福寿岭站}}（）。
...
talk:维基百科模板中的相关内容](https://zh.wikipedia.org/wiki/:Category_talk:维基百科模板 "wikilink")。
...
...`，参见[Help:名字空间](https://zh.wikipedia.org/wiki/Help:名字空间 "wikilink")、[Special:所有消息和](https://zh.wikipedia.org/wiki/Special:所有消息 "wikilink")[MediaWiki:Blockedtext](https://zh.wikipedia.org/wiki/MediaWiki:Blockedtext "wikilink")。

### 参数及模板命名

参数命名可以调用其他参数，如[Template:XYZ内容为](https://zh.wikipedia.org/wiki/Template:XYZ "wikilink"){{{{{{XYZ}}}}}}，则调用{{XYZ|XYZ=SDF|SDF=789}}会把参数XYZ的值SDF作为参数名，显示参数SDF的值，即。MediaWiki会把{{{{{{XYZ}}}}}}解析为{{{
{{{XYZ}}} }}}而不是{{ {{ {{XYZ}} }} }}（结果是{{ {{  }}
}}）。这里空格是敏感的。另外，调用其他参数仅可调用1层。如[Template:XYZ2内容为](https://zh.wikipedia.org/wiki/Template:XYZ2 "wikilink"){{{{{{{{{XYZ}}}}}}}}}，则{{XYZ2|XYZ=SDF|SDF=ABC|ABC=789}}结果为。

又如，[Template:1a2b内容为](https://zh.wikipedia.org/wiki/Template:1a2b "wikilink"){{-{}-{第一个|第一个未定}}-{}-}-{{-{}-{第二个|第二个未定}}-{}-}，[Template:nanb内容为](https://zh.wikipedia.org/wiki/Template:nanb "wikilink"){{1a2b|{{{1|第一个}}}
...
另外，如果模板名为参数，如[Template:Sample2内容为](https://zh.wikipedia.org/wiki/Template:Sample2 "wikilink"){{{{{tc}}}}}，则调用{{Sample2|tc=\!}}相当于{{\!}}，显示为。这里，{{{{{tc}}}}}解析为{{
{{{tc}}} }}而不是{{{ {{tc}}
}}}。如果[Template:Sample3内容为](https://zh.wikipedia.org/wiki/Template:Sample3 "wikilink"){{{
...
#### subst与preload过程中的noinclude、includeonly和IncludeOnly
  ...
  -
    *以下部分行为是错误的，是**旧版本软件的缺陷**，并将**随MediaWiki
    1.17的部署被修正**（[bugzilla:5210](https://zh.wikipedia.org/wiki/bugzilla:5210 "wikilink")）。*
...
模板的一般用法有：
  ...
  - [导航模板](https://zh.wikipedia.org/wiki/Wikipedia:导航模板 "wikilink")，用于给读者提供类似的条目，例如：[Template:洲](https://zh.wikipedia.org/wiki/Template:洲 "wikilink")。
  - 信息框模板，用于同一类条目中提供统一格式的信息，例如：[Template:Infobox
    网球在条目右边绘制信息框](https://zh.wikipedia.org/wiki/Template:Infobox_网球 "wikilink")，提供网球运动员的相关信息。
  - 提供跨语言文本。
...
调用[解析器函数的语法是](https://zh.wikipedia.org/wiki/WP:解析器函数 "wikilink"){{\#xxx:}}。如{{\#ifeq:{{{lang}}}|zh|我使用中文|I\<ins\>

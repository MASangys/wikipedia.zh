...
`   tagOpen    : "`[`",``   ``sampleText``   ``:``   ``"链接标题",``
 ``tagClose``   ``:``
 ``"`](https://zh.wikipedia.org/wiki/",_sampleText_:_"链接标题",_tagClose_:_" "wikilink")`",`
...
`   tagOpen    : "#REDIRECT `[`",``   ``sampleText``   ``:``
 ...
 ``"`](https://zh.wikipedia.org/wiki/",_sampleText_:_wgULS\("目标条目名","目標條目名"\),_tagClose_:_" "wikilink")`",`
`   speedTip   : "重定向"`

...
remarkStr; site.loadPage(site.DYKCPage, function(revision) { var curDate
...
'很抱歉因为无法识别新条目推荐候选页的格式，故未能自动推荐该条目。\\n'
...
(k.startsWith('preview')) { dialog.$\[k\].html(dialog.config\[k\]); }
...
end) return null; var pre = qus.substring(0, start), tar =

**A\*搜尋演算法**，俗稱**A星演算法**。這是一種在圖形平面上，有多個[節點的](../Page/節點.md "wikilink")[路徑](../Page/路径.md "wikilink")，求出最低通過[成本的](../Page/成本.md "wikilink")[演算法](../Page/算法.md "wikilink")。常用於遊戲中的NPC的移動計算，或[网络游戏的BOT的移動計算上](../Page/网络游戏.md "wikilink")。

该算法综合了和[Dijkstra算法的优点](../Page/Dijkstra算法.md "wikilink")：在进行启发式搜索提高算法效率的同时，可以保证找到一条最优路径（基于评估函数）。
...
这个公式遵循以下特性：
  ...
  - 如果\(h(n)\)不大于顶点\(n\)到目標頂點的實際距離，则一定可以求出最优解，而且\(h(n)\)越小，需要计算的节点越多，算法效率越低，常见的评估函数有——[欧几里得距离](../Page/欧几里得距离.md "wikilink")、[曼哈顿距离](../Page/曼哈顿距离.md "wikilink")、[切比雪夫距离](../Page/切比雪夫距离.md "wikilink")；
  - 如果\(h(n)\)为0，即只需求出起点到任意顶点\(n\)的最短路径\(g(n)\)，而不计算任何评估函数\(h(n)\)，则转化为[单源最短路径问题](../Page/单源最短路径.md "wikilink")，即[Dijkstra算法](../Page/Dijkstra算法.md "wikilink")，此时需要计算最多的顶点；

...
action）；如果是零值，用[宽字符打印诊断消息](https://zh.wikipedia.org/wiki/宽字符 "wikilink")，然后调用`abort()`。诊断消息包括：
...
诊断信息的显示目标依赖于被调用程序的类型。如果是控制台程序，诊断信息显示在[stderr设备](https://zh.wikipedia.org/wiki/stderr "wikilink")；如果是基于窗口的程序，`assert()`产生一个Windows
MessageBox来显示诊断信息。

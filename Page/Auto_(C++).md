...
auto可以在声明变量时根据变量初始值的类型自动为此变量选择匹配的类型。C++语言类似的关键字还有[decltype](https://zh.wikipedia.org/wiki/decltype "wikilink")。
...
或者保存[lambda表达式类型的变量声明](https://zh.wikipedia.org/wiki/匿名函数#C.2B.2B "wikilink")：

`  auto ptr = [](double x){return x*x;};//类型为std::function<double(double)>函数对象`

在[模板函数定义时](../Page/模板_\(C++\).md "wikilink")，如果变量的类型依赖于模板参数，使用auto关键字使得在编译期确定这些类型，如：
...
这一组规则同于[模板函数的](../Page/模板_\(C++\).md "wikilink")[模板参数推导](https://zh.wikipedia.org/wiki/模板参数推导 "wikilink")（template
argument
...
var=initValue;`“auto&&”并不意味着这一定是[右值引用类型的变量](../Page/右值引用.md "wikilink")，而是类似于[模板函数参数的类型推导](https://zh.wikipedia.org/wiki/模板参数推导 "wikilink")，既可能是左值引用，也可能是右值引用。其目的是把初始化表达式的[值分类情况](https://zh.wikipedia.org/wiki/值_\(计算机科学\) "wikilink")，完美转发给由auto声明的变量。也即：

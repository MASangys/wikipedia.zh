...
[BET_Multilayer_Adsorption.svg](https://zh.wikipedia.org/wiki/File:BET_Multilayer_Adsorption.svg "fig:BET_Multilayer_Adsorption.svg")
BET
理论在[朗缪尔理论的单分子吸附模型的基础上](../Page/朗缪尔方程.md "wikilink")，基于以下三个假设拓展到多层吸附的情况：
...
其中 \(E_1\) 是第一层的吸附热，\(E_L\) 为其余各层的单层吸附热，数值上等于气体的液化焓。
[BET-1.jpg](https://zh.wikipedia.org/wiki/File:BET-1.jpg "fig:BET-1.jpg")

根据 (1) 式，在温度恒定的情况下，以 \({1}/{v [ ({P_0}/{P}) -1 ]}\) 对 \(\phi={P}/{P_0}\)
作图应得一直线，该图称为**BET图**，实际上，只有在 \(0.05 < {P}/{P_0} < 0.35\)
的范围内，BET图表现出较良好的线性。根据直线的斜率 \(A\) 和截距 \(I\)
可以求出单层吸附量和 BET 常数，如下两式所示。
...
用 BET
理论可以求算出硬化[水泥浆的内表面积](../Page/水泥.md "wikilink")。在不同的环境湿度下测定达到平衡时水泥吸收的水蒸气量，便可以得到
BET 图。当水温为97°C时，实验得到 BET 图的斜率为 24.20 克/克（水泥），截距为 0.33 克/克，由 (3)(4) 两式得到
\(v_m = 0.0408 \text{g/g}, c=73.6\)，又水的吸附界面为0.114平方纳米，于是由 (5)(6) 两式可以得到
\(S_{BET} = 156 \text{m}^2/\text{g}\) ，表示每克水泥硬化后内表面积为 156 平方米。
...
1.  S. Brunauer, P. H. Emmett and E. Teller, *J. Am. Chem. Soc.*, 1938,
    **60**, 309.
    [<doi:10.1021/ja01269a023>](http://dx.doi.org/10.1021/ja01269a023)
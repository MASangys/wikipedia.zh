在[計算複雜度理論領域內](../Page/計算複雜度理論.md "wikilink")，**BPL**（有限錯誤機率對數空間，Bounded-error
Probabilistic
...
雖然雙邊錯誤比起單邊錯誤（回答特定答案時絕不會出錯，只有在另一個答案時會）更加一般化，**[RL](../Page/RL_\(複雜度\).md "wikilink")**和它的[補集](../Page/補集_\(複雜度\).md "wikilink")**co-RL**包含在**BPL**裡面。**BPL**也包含在**[PL](../Page/PL_\(複雜度\).md "wikilink")**（一個相類似的複雜度類，不過其錯誤率恰為1/2而非小於1/2）裡面；就像**[PP](../Page/PP_\(複雜度\).md "wikilink")**一樣，**PL**可能需要花費很多次的計算來降低錯誤的機率，因此比較不實用。

使用了一個弱的去隨機化結果證明**[BPL](../Page/BPL.md "wikilink")**包含在**[SC](../Page/SC_\(複雜度\).md "wikilink")**裡面。\[3\]這裡的SC是一個複雜度類，包含可以使用決定型圖靈機在多項式時間和多項式對數（polylogarithmic）空間解決的問題；換句話說，這個結論證明了

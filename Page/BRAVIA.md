**BRAVIA**是[日本索尼](../Page/日本.md "wikilink")（[SONY](https://zh.wikipedia.org/wiki/SONY "wikilink")）的新一代[平板](https://zh.wikipedia.org/wiki/平板电视 "wikilink")[液晶電視品牌](https://zh.wikipedia.org/wiki/液晶電視 "wikilink")。BRAVIA是「Best
Resolution Audio Visual Integrated Architecture」的縮寫。

## 關於

[Taipei101_by_Sony.JPG](https://zh.wikipedia.org/wiki/File:Taipei101_by_Sony.JPG "fig:Taipei101_by_Sony.JPG")
索尼過去因錯失[液晶面板技術](../Page/液晶.md "wikilink")，使第一品牌陸續被[夏普](https://zh.wikipedia.org/wiki/夏普 "wikilink")、[松下](../Page/松下電器.md "wikilink")、[三星](../Page/三星電子.md "wikilink")、[樂金超越](https://zh.wikipedia.org/wiki/樂金 "wikilink")，電視部門的業績不佳。2005財年第一季度財報顯示，索尼電視部門淨虧損72.6億[日元](https://zh.wikipedia.org/wiki/日元 "wikilink")（約合6,500萬[美元](../Page/美元.md "wikilink")），拖垮了索尼整體的獲利。索尼希望捨棄從[映像管電視時代就使用的](https://zh.wikipedia.org/wiki/映像管 "wikilink")[WEGA品牌](https://zh.wikipedia.org/wiki/WEGA "wikilink")，會象徵其電視事業的躍進。索尼第三號主管伊原直言：「電視是索尼事業核心中的核心，索尼能否翻身，就看索尼能否在電視機市場殺出重圍，全面搶攻成長快速的液晶電視市場，以奪回全球第一大的液晶電視品牌為目標。」

BRAVIA電視品牌最開始是在2005年8月24日發表於中國大陸，2005年9月陸續在美國、日本、台灣發表其最新電視品牌，並將在全球砸下1.4億美元作為BRAVIA行銷。

BRAVIA品牌部份機種採用與三星合資的[S-LCD所研發的第七代液晶面板](https://zh.wikipedia.org/wiki/S-LCD "wikilink")，目前將用在所有平面電視，包括[背投影電視](https://zh.wikipedia.org/wiki/背投电视 "wikilink")、[液晶電視](https://zh.wikipedia.org/wiki/液晶電視 "wikilink")。映像管電視則繼續沿用WEGA品牌。之後索尼將以BRAVIA品牌全面取代使用8年之久的WEGA。

BRAVIA系列液晶電視熱賣之賜，Sony在美國數位平面電視市場的佔有率在4個月的時間內由個位數竄升至30%。根據科技市調機構iSuppli的統計，Sony為2005年第3季液晶電視市場的大贏家，市佔率由前一季的第6名躍升至第3名。在台灣，BRAVIA，以32吋的S32A10定位戰鬥機種，以[新台幣](https://zh.wikipedia.org/wiki/新台幣 "wikilink")59,900元強勢價格，使剛上市的800台在3天內銷售一空，除了對台灣二線品牌造成衝擊外，也對國際一線品牌夏普、松下、三星、樂金造成極大的降價壓力。也因BRAVIA的全球熱賣，使得原本拖垮索尼獲利的電視業務在2005年第三季財報扭轉為高度成長的獲利支柱。

BRAVIA全球行銷口號是「Color」-色彩；在[中國大陆](https://zh.wikipedia.org/wiki/中國大陆 "wikilink")，-{索尼}-（中國）的主要行銷口號是「博大晶深」；在台灣，台灣-{新力}-國際(2009年改名為台灣-{索尼}-)的2005年主要行銷口號是「感動的色彩
永遠值得」、「液晶電視再進化」，2006年主要行銷口號是「映在心中的色彩 感動最大」、「液晶電視再進化」，2007年主要行銷口號是「最美的承諾
最感動的色彩」、「液晶電視再進化」，2008年主要行銷口號是「美麗的色彩 永存人心」、「完美色彩
永不妥協」，2009年主要行銷口號是「用眼睛思考色彩」，2010年主要行銷口號是「進化完美
超越當代」，2011年主要行銷口號是「未來 重新定義」，2012年主要行銷口號是「無比熱情 創新視界」，2013年主要行銷口號是「Feel
the Beauty」，2014年主要行銷口號是「鍾於不凡」，2015年主要行銷口號是「色彩
絕非表面」，2016年主要行銷口號是「明暗世界．全收眼界」，2017年主要行銷口號是「嶄新世界
全感新境界」，2018年主要行銷口號是「嶄新體驗
全面覺醒」；在香港則利用「色彩·真感動」作為宣傳口號。2008年，-{索尼}-（中國）开始在大陆用“看心中的红，看BRAVIA”作为营销口号贯穿于广告中。

## BRAVIA市场

BRAVIA由于其特殊的品牌归属，在中国大陆的定位一向是较高的，但凭借索尼之前强大的1080i全高清产品线、Cyber-Shot和Handycam的产品号召力以及之前[WEGA](https://zh.wikipedia.org/wiki/WEGA "wikilink")(贵翔)[平面特丽珑产品的过硬质量](../Page/特丽珑.md "wikilink")，众多中国用户都选择了BRAVIA作为他们的终端显示设备。同时，凭藉索尼一向的高端工艺设计，BRAVIA也吸引了许多希望电器外观家具化的用户购买其产品。

## BRAVIA相關技術

[Sony_kdl_46x2000_bravia_display_dreambox_dm7000_receiver_by_hdtvtotal_dot_com.jpg](https://zh.wikipedia.org/wiki/File:Sony_kdl_46x2000_bravia_display_dreambox_dm7000_receiver_by_hdtvtotal_dot_com.jpg "fig:Sony_kdl_46x2000_bravia_display_dreambox_dm7000_receiver_by_hdtvtotal_dot_com.jpg")

  - TRILUMINOS RGB Dynamic
    [LED](https://zh.wikipedia.org/wiki/LED "wikilink") 背光

Sony獨創的RGB LED動態[背光技術是當今顯像技術的重大突破](../Page/背光.md "wikilink")，不但延續
Sony特麗霓虹色彩的光學技術，最重要的是以三原色LED燈管取代過去日光燈作為光源，大幅提升紅藍綠三原色的純淨度，達到世界唯一的
1,000,000：１動態對比，使用動態背光控制技術，調校出最適化的亮度變化，更能呈現出豐富的深邃層次。Sony在2008年將這一技術用在民用消費品牌BRAVIA旗艦型號XBR8（美國）、XR1（日本）、X4500（歐洲、亞洲）上。

  - Triluminos LED 背光

Sony研發的Triluminos
LED背光燈技術，以特別方式排列LED的紅、藍及綠[三原色](https://zh.wikipedia.org/wiki/三原色 "wikilink")，創造更廣濶的色域，顯著提升至近乎原色。Sony最早在2004年將這一技術用在消費品牌[QUALIA型號KDX](../Page/QUALIA.md "wikilink")-46Q005、KDX-40Q005（日本），2007年將這一技術用在民用消費品牌BRAVIA旗艦型號70X7000（日本）、70X3500（亞洲）上。

  - Backlight Master Drive 極致精準直下式背光技術

擁有超高密度 LED 結構，每一顆 LED 都能獨立運作，並由全新研發的演算系統控制，能夠創造出準確且令人驚豔的畫面對比。獨特的聚焦式 LED
設計可將光源聚集在一點上，減少光擴散避免光暈問題，呈現生動的亮與深邃的黑，展現最細膩出色的光影效果。

  - Live Color Creation

Sony 開創出「Live Color Creation
自然色再生科技」最新一代背光模組，擴大色彩在數位化顯像還原的效果，經日本[專業機構證實](https://zh.wikipedia.org/wiki/專業機構 "wikilink")，紅、藍、綠三原色彩度全面大幅提升。

  - WCG-CCFL

Sony
獨家的全新廣色階背光模組，能呈現有層次的自然原色，讓眼睛看見最真實的影像，相較於傳統的背光模組，色彩表現能力最高可提升30%（相較於過去
NTSC 的液晶電視只能呈現約70%，WCCFL 可使色彩顯像還原範圍擴大到約91%）。

  - HCFL

Sony 特有的科技成功創造出世界上第一個高效能微熱陰極燈管〈HCFL〉背光科技。HCFL 背光科技可節省百分之四十%的能源，並同時維持與
CCFL 背光科技相同的明亮度。

  - S-LCD 面板

Sony第七代百萬級畫素液晶面板，無論[對比](https://zh.wikipedia.org/wiki/對比 "wikilink")、亮度、色數、層次、反應速度均領先群倫，並創造出全世界最廣178°超廣視角，有效防止室內其他光源產生反射現象。

  - BRAVIA ENGINE

BRAVIA擁有Sony獨創「BRAVIA ENGINE」影像CPU，具備Digital Reality
Creation畫質精細高倍密技術，能將不良的影像訊號，轉換成精細而層次分明的畫質，同時過濾雜訊，加倍提升銳利度，呈現出高解析度、豐富的立體層次。

  - BRAVIA ENGINE EX

BRAVIA Engine EX 影像 CPU，採用最新一代 BRAVIA Engine Picture Enchancement
進階版本技術，展現最佳化的 Full HD 高畫質面板效能。並搭戴「DRC-MF
v1」全新高畫質倍頻處理，將家用[第四台的類比訊號](../Page/第四台.md "wikilink")、[DVD](https://zh.wikipedia.org/wiki/DVD "wikilink")
影片...等標準訊號來源，同步倍頻強化為高於原先影像 4
倍精密度，呈現高精密、高[解晰度的最高畫質](https://zh.wikipedia.org/wiki/解晰度 "wikilink")。

  - BRAVIA ENGINE PRO

BRAVIA ENGINE PRO 影像 CPU 進階技術，是日本 Sony 針對未來數位高畫質開創最新技術，內建「DRC-MF
v2.5」全新高畫質倍頻處理。所有過程同步在千分之一秒將所有影像自動調校達到真實的 1080P 高畫質，能將 BRAVIA X
系列液晶電視的超高顯像解析度發揮到極致。呈現真實的質感、色彩、立體層次的極致影像．並且已達到未來全球數位電視化的最高顯像規格，能支援不同影像訊號輸入，提高彩度、強化銳利度、加強對比...等，將所有影像加倍成清晰而精細畫面，達到未來數位影像最高規格
Full HD 1080P 無可取代的高畫質。

  - BRAVIA ENGINE 2

BRAVIA Engine 2 影像 [CPU](https://zh.wikipedia.org/wiki/CPU "wikilink")，為
Sony 第二代全方位影像處理器，色彩方面不僅大幅增豔，並搭配 Advanced Contrast
Enhancer（ACE）對比增強器，能強化色彩鮮明立體的層次，展現亮部到暗部間的細微光影，讓畫面栩栩如生。在影像表現上，更能有效抑制畫面噪訊，將第四台原有的不良訊號強化為高精密畫質，重現真實精彩的節目內容。

  - BRAVIA ENGINE 2 EX

BRAVIA ENGINE 2 EX 影像處理器，是BRAVIA Engine Picture Enhancement
進階版本技術及「DRC-MF
v1.0」倍頻處理，將家用[第四台](../Page/第四台.md "wikilink")[類比訊號](../Page/類比.md "wikilink")、DVD影片…等標準
SD 訊號來源，同步倍頻強化為HD高等級影像。

  - BRAVIA ENGINE 2 PRO

BRAVIA ENGINE 2 PRO 影像處理器，內建「[DRC-MF](../Page/DRC-MF.md "wikilink")
v3.0」最新高畫質倍頻處理技術，不僅將所有訊號來源自動昇級為 1080p
影像外，更強化動態影像清晰度、抑制鏡頭中細微處的雜訊、提高彩度、加強色彩對比及銳利度、柔化字幕邊緣…等全新功能，讓新一代
BRAVIA X4500 系列超高顯像解析度發揮至極致，傳遞最完整的 Full HD 1080p
精細高畫質，成就高畫質世界中前所未有的最高規格。

  - BRAVIA ENGINE 3

BRAVIA ENGINE 3 影像處理器，為 BRAVIA
創新的第三代全方位影像處理器，從影像的三大方向著手進行全面性的完美調校：首先增強畫面清晰度，任何一處細節都不放過；接著更強化立體層次的色彩對比，使畫面栩栩如生，令人驚嘆；最後再將細節做進化處理，創造出鮮明顏色的畫面，大幅增進色彩鮮豔度，讓您擁有
BRAVIA ENGINE 3，就能享受世界頂級的高畫質饗宴。

  - X-Reality 極真影像晶片

Sony 劃世代的 X-Reality
極真影像晶片都能夠針對影像加以分析、特別處理，提供清晰生動且幾近真實的畫面。在處理器大幅提升影像解析度前，會先經過「智慧型噪訊抑制技術」(Intelligent
MPEG Noise Reduction)處理，同時 Sony 的「智慧型影像強化技術」(Intelligent Image
Enhancer)還針對畫面訊號的四大構面進行分析-紋理、輪廓、對比及色彩，並依照特性逐一優化後結合，呈現最貼近真實的逼真畫面。

  - X-Reality PRO 超極真影像晶片

Sony 劃世代的超極真影像晶片能夠以畫素作為單位分析每一個畫面，並採用精密的配色模式，呈現前所未有的視覺體驗！

  - 4K X-Reality PRO

Sony 最高階的影像晶片 4K X-Reality PRO
能進行影像樣式分析並搭配真實畫面建立資料庫，經由分析、比對並優化影像，諸如畫面中的臉部表情、茂盛的風景、建築物等，能讓每個畫素最佳化。無論觀賞藍光光碟、電視節目甚至網路影片等，皆能提升至細部更自然、色彩更豐富且高對比的完美畫面，同時更可將
Full HD 畫質提升至媲美 4K\* 的畫質表現，這就是 4K X-Reality PRO 超極真影像晶片的驚奇魅力。

  - 4K Processor X1

新推出 4K 超極真影像處理器 X1，整合 4K 超極真影像技術、原色顯示技術及高光感精準背光技術，能針對電視、藍光光碟、DVD，甚至 4K\*
網路影像等來源進行分析，提升至最細緻畫質、最精準對比及最飽滿鮮豔色彩

  - 4K HDR Processor X1 Extreme

全新研發的 4K HDR 超極真影像處理器 X1 進階版，是專為 4K HDR 影像量身訂做的新版處理器，大幅提升 40%
即時影像處理效能。能有效提升各點畫質，讓世界真實地呈現在眼前；當中的 HDR
分區精準強化技術可針對畫面中各別物體分析顏色、調整對比，呈現更豐富的深度與細節；雙影像數據資料庫同時處理影像，可大幅提升解析度，創造超凡清晰的動人畫面;而
4K HDR 平滑漸層技術可有效撫平色階差異，呈現高於傳統電視 64 倍的色彩層次。

  - DRC-MF

（Digital Reality Creation-Multi
Function）多媒體數位真實解像技術，是新力在標準畫質時代為了使普通電視發揮出高畫質訊號水準而研發的一種影像強化技術，可藉由在影像中加入更多的像素與掃瞄線來有效提升影片的影像解析度。DRC與傳統影像處理系統不同，DRC可以即時處理影像訊號，以產生更柔和無雜訊的影像。

  - TRILUMINOS™ Display 原色顯示技術

Sony 獨家最新 TRILUMINOS™ Display
原色顯示技術，打破傳統液晶電視的色彩範圍限制，可顯示更寬廣的色域表現，呈現最豐富絢爛的真實原色。尤其一般液晶電視最難表現的紅色及翡翠綠，透過此技術，能生動地呈現更豐富、更細膩、更立體、更有層次的彩色世界，完美呈現嬌艷的紅、清透的湛藍、純粹晶瑩的翡翠綠，甚至膚色的光潤質感等，細膩的色彩層次將一一重現眼前。

  - X.V. Color

“x.v.Color.” 此一革新的技術符合
“[xvYCC](https://zh.wikipedia.org/wiki/xvYCC "wikilink")”（eXtended
Video YCC ，動態影像廣泛色彩空間）的國際標準，可創造接近百分之百人類肉眼可接收的色彩程度。

  - Mortion Flow

Sony 搭配獨有的 Algorithm 運算技術，創造出 Motion Flow 動態影像流暢科技，能將影像由原先一秒 60 格的畫面變成
120 格，靜止影像格數增加了一倍，影像自然更加流暢，清晰呈現。

  - Mortion Flow PRO

Motion Flow PRO 動態影像流暢技術，完整保留 Motion Flow 在播放極速影像上的優異表現，流暢呈現每秒 120
張畫面，影像銳利度增強外，還可選擇「清晰」模式，透過動態
[LED](https://zh.wikipedia.org/wiki/LED "wikilink")
集中光源，精確控光源的調校，減去不必要的亮源。

  - Mortion Flow 240Hz

將電視畫面每秒所能展現的畫面數量增加至四倍，令高速動態畫面影像更穩定、精確及流暢。
這項精密的嶄新技術能為每一格影像創製並加插三格全新影像，把影像輸出率由原來的60Hz提升至240Hz。

  - Mortion Flow XR240

極瞬流線影像科技 240 使用 「影像模糊抑制」 (Image Blur Reduction) 以及 「背光閃爍控制」 (Backlight
Blinking) 等技術，透過 Sony 創新的演算法，呈現 240Hz
能夠產生的流暢影像，減少畫面播放過程中產生的殘影，呈現精采的動態影像。

  - Mortion Flow XR480

極瞬流線影像科技使用 240Hz 的面板，在 1/240 秒的畫面中，結合「影像模糊抑制」(Image Blur
Reduction)、「畫面內插」(Frame Interpolation)、「畫面背光閃爍控制」(Backlight
Frame Blinking) 以及「背光掃描」 (Backlight
Scanning)等技術，使動態影像表現都如親臨現場般完美，清晰的速度感以及精準的細膩感，讓影像流暢度再進化\!

  - Mortion Flow XR960

極瞬流線影像科技 960 採用先進的 240Hz 面板結合了「影像模糊抑制」(Image Blur
Reduction)、「畫面內插」(Frame
Interpolation)、「線性背光閃爍控制」(Backlight Line
Blinking)以及「背光掃描」(Backlight Scanning)等技術，將畫面增幅，能提供相當於 960Hz
的效能，加倍的演算技術讓分格更細緻，達到超順暢、超精準的動態影像。

  - Mortion Flow XR1440

能將畫面增幅，加倍的演算技術讓分格更細緻，呈現精彩動態影像。XR1440 採用先進的 「背光掃描」
技術，更以領先的「線性背光閃爍控制」技術達到超順暢、超精準的動態影像。

  - SimulView

藉由 SimulViewTM 雙人共享遊戲介面，當兩位玩家同時遊戲，只要戴上輕便的專屬眼鏡，即可在同一螢幕上觀看到專屬自己的遊戲進行畫面

  - Xross Media Bar（[XMB](../Page/XMB.md "wikilink")）

是一種由索尼開發的圖形用戶界面，最初用作[PSX媒體中心的人機界面](../Page/PSX.md "wikilink")。該界面擁有橫越螢幕水平展開的圖示，可透過移動該些圖示而非游標來導航瀏覽。該些圖示用來組織可供使用者選擇的選項，當選擇水平條的圖示後會有更多的圖示在上或下方垂直顯示（可透過上下方向鍵選擇）。

  - RF Remote

與以往電視機和遙控器之間採用紅外線信號通信不同，新BRAVIA
X4500遙控器與電視機之間是採用2.4GHz波段的無線訊號，遙控器無需指向電視機就可遙控電視機。

  - DLNA（數位生活網路聯盟）

透過家中網路共享數位影像、影片及音樂，將儲存在[DLNA相容裝置](https://zh.wikipedia.org/wiki/DLNA "wikilink")（例如：電腦、相機）上的數位影像、影片及音樂透過家中網路在DLNA相容的[電視上播放](https://zh.wikipedia.org/wiki/電視 "wikilink")。

  - BRAVIA Sync

利用 [HDMI](../Page/HDMI.md "wikilink") 線輕鬆連接 BRAVIA 液晶電視與 BRAVIA
家庭劇院組，搭配最新的 BRAVIA Sync 進階影音同步操控功能，能讓您使用一支遙控器，隨指 one
touch，操縱開啟、關閉及音效切換，通通搞定，Sony HD
[Handycam高畫質攝錄放影機](../Page/Handycam.md "wikilink")、Sony[DVD與](https://zh.wikipedia.org/wiki/DVD "wikilink")[藍光播放機](../Page/藍光光碟.md "wikilink")、[PS3
slim遊戲機也可對應](https://zh.wikipedia.org/wiki/PS3_slim "wikilink") BRAVIA
Sync 功能。

  - SXRD

全名則為Silicon X-tal Reflective
Display（矽液晶反射顯示器）技術則為Sony開創，皆為[LCOS技術的進一步延伸](https://zh.wikipedia.org/wiki/LCOS "wikilink")，具有高對比與壽命長等特性，主要用於投影機和內投影電視。

## 外部链接

  - [BRAVIA-日本網站](http://www.sony.jp/products/Consumer/bravia/)
  - [BRAVIA-歐洲網站](https://web.archive.org/web/20070430190709/http://www.bravia-advert.com/)
  - [BRAVIA-中國網站](http://www.sonystyle.com.cn/products/bravia/index.htm)
  - [BRAVIA-台灣網站](http://www.sonystyle.com.tw/braviatheatre/)
  - [BRAVIA-香港網站](https://web.archive.org/web/20051001005638/http://www.sonystyle.com.hk/campaign/bravia/main.html)

[Category:高清晰度电视](https://zh.wikipedia.org/wiki/Category:高清晰度电视 "wikilink")
[Category:品牌](https://zh.wikipedia.org/wiki/Category:品牌 "wikilink")
[Category:索尼](https://zh.wikipedia.org/wiki/Category:索尼 "wikilink")
**Banshee**是[GNOME](../Page/GNOME.md "wikilink")的一個[跨平台的](https://zh.wikipedia.org/wiki/跨平台 "wikilink")[開放原始碼](https://zh.wikipedia.org/wiki/開放原始碼 "wikilink")[媒体播放器](https://zh.wikipedia.org/wiki/媒体播放器 "wikilink")。Banshee建基於[Mono](../Page/Mono.md "wikilink")與[Gtk\#之上](https://zh.wikipedia.org/wiki/Gtk_Sharp "wikilink")，使用[GStreamer](../Page/GStreamer.md "wikilink")多媒體平台進行編碼，並可對多種音樂格式進行解碼，包括了[Ogg Vorbis](https://zh.wikipedia.org/wiki/Ogg_Vorbis "wikilink")、[MP3](../Page/MP3.md "wikilink")與[FLAC](../Page/FLAC.md "wikilink")。Banshee可以播放、匯入與燒錄音頻[光碟](../Page/光碟.md "wikilink")以及與數款便攜媒體播放器同步音樂，包括蘋果公司的[iPod與](https://zh.wikipedia.org/wiki/iPod "wikilink")[Creative Zen播放器](https://zh.wikipedia.org/wiki/Creative_Zen "wikilink")。儲存在iPod上的音樂可直接播放而不需經過同步，而儲存在Banshee資料庫的唱片封面會傳送到iPod。對[MTP協定](https://zh.wikipedia.org/wiki/Media_Transfer_Protocol "wikilink")、[PlaysForSure裝置以及](../Page/Microsoft_PlaysForSure.md "wikilink")[Rio Karma播放器的支援預定會在不久的將來加入](https://zh.wikipedia.org/wiki/Rio_Karma "wikilink")。

Banshee在[Ubuntu](../Page/Ubuntu.md "wikilink") 11.04 Alpha 3中取代[Rhythmbox](../Page/Rhythmbox.md "wikilink")，成為系統的預設媒體播放器\[1\]。

## 插件

Banshee採用一個可使用插件的架構，因此可高度的擴展與用戶化。現時穩定的插件包括：

  - Audioscrobbler：加入將已播放歌曲報告到用戶的[Last.fm](../Page/Last.fm.md "wikilink")播放清單的能力。
  - Podcasting：允許Banshee訂閱Podcast饋送，會定期更新。另有一個利用[Podcast Alley](http://www.podcastalley.com)的"Find new podcasts"功能。
  - [DAAP音樂共享](https://zh.wikipedia.org/wiki/Digital_Audio_Access_Protocol "wikilink")：允許與[iTunes及其他兼容DAAP的音樂軟體共享音樂資料庫](https://zh.wikipedia.org/wiki/iTunes "wikilink")，當前版本的Banshee只部份地與iTunes 7兼容。iTunes 7可開啓Banshee的資料庫，但Banshee無法開啓iTunes 7的資料庫。
  - 使用[Musicbrainz的元數據搜尋器](https://zh.wikipedia.org/wiki/Musicbrainz "wikilink")：為庫料庫中的項目自動擷取缺少與增補的元數據，包括唱片封面。
  - 使用[Last.fm](../Page/Last.fm.md "wikilink")的音樂推介：基於現時播放的歌曲來推介音樂
  - 微型模式插件：提供一個包括最低限度播放控制器與歌曲資訊的小型視窗
  - 於[GNOME](../Page/GNOME.md "wikilink")支援多媒體按鍵：在透過GNOME配置時Banshee可經由多媒體按鍵控制
  - 廣播：提供對互聯網串流廣播電台的支援
  - 通知區域圖示：在GNOME的通知區域加入圖示

## Helix Banshee

Helix Banshee是一個Banshee的修改版，包含在某些Linux分發版如[openSUSE當中](https://zh.wikipedia.org/wiki/openSUSE "wikilink")。Helix Banshee包含了Banshee核心，經過修改以使用[Helix框架](https://zh.wikipedia.org/wiki/Helix "wikilink")，與Banshee不同，Helix Banshee使用Helix引擎進行播放。

## 相關條目

  - [Amarok](../Page/Amarok.md "wikilink")
  - [Rhythmbox](../Page/Rhythmbox.md "wikilink")
  - [Songbird](../Page/Songbird.md "wikilink")
  - [媒體播放器比較](https://zh.wikipedia.org/wiki/媒體播放器比較 "wikilink")
  - [媒體播放器列表](https://zh.wikipedia.org/wiki/媒體播放器列表 "wikilink")

## 參考資料

## 外部連結

  - [官方網站](https://web.archive.org/web/20140102111027/http://banshee.fm/)
  - [Helix Banshee](https://helix-banshee.helixcommunity.org/)

[Category:媒體播放器](https://zh.wikipedia.org/wiki/Category:媒體播放器 "wikilink") [Category:自由跨平台软件](https://zh.wikipedia.org/wiki/Category:自由跨平台软件 "wikilink") [Category:自由媒体播放器](https://zh.wikipedia.org/wiki/Category:自由媒体播放器 "wikilink") [Category:自由音讯软件](https://zh.wikipedia.org/wiki/Category:自由音讯软件 "wikilink")

1.
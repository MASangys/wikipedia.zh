...
雖然這個系統的作者聲稱有關技術乃係利用法律容許用戶本身為系統所作出的備份而製成有關光碟，微軟仍然堅持有關系統的製作侵犯了微軟本身商品的版權。因此，若用戶有意使用，必須先行理解使用時可能引起的法律問題。

## BartPE與Windows預先安裝環境（WinPE）比較

BartPE雖然與[Windows預先安裝環境](../Page/Windows預先安裝環境.md "wikilink")（WinPE）相似，但兩者在以下幾方面不同：

  - WinPE是微軟的正式產品，但BartPE並未經過微軟公司驗證，也沒有得到微軟公司准許使用。
  - WinPE由微軟提供，只供應予[OEM生產商](https://zh.wikipedia.org/wiki/OEM "wikilink")，但BartPE除了使用了微軟的作業系統的光碟以外，其他附加的額外功能都由[自由軟件提供](https://zh.wikipedia.org/wiki/自由軟件 "wikilink")。
  - WinPE可選取的插件有限，但BartPE理論上能採用及提供無限的插件。
...
用BartPE在中文Windows環境下製作出來的LiveCD不能正確顯示中文字。

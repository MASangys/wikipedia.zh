...
可以证明这个算法是正确的，我们要用到[高德纳](../Page/高德纳.md "wikilink")（Donald Ervin Knuth）的[0-1原理](https://zh.wikipedia.org/wiki/0-1原理 "wikilink")，我们发现，对于输入的任意两个有序的0,1序列，奇序列与偶序列正好相差0个，1个或2个0。由于奇序列的第一个元素不参与最后的洗牌比较，所以参与比较的0,1数偶只有0个或1个，所以对0,1序列一定能够得到正确的排序。故而对任意的序列，奇偶归并网络可以产生正确的排序。

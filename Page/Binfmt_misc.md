**binfmt_misc**是[Linux内核](../Page/Linux内核.md "wikilink")的一项功能，其使得内核可识别任意类型的[可执行文件格式并传递至特定的](../Page/執行檔.md "wikilink")[用户空间应用程序](https://zh.wikipedia.org/wiki/使用者空間 "wikilink")，如[模拟器和](https://zh.wikipedia.org/wiki/仿真器 "wikilink")[虚拟机](../Page/虛擬機器.md "wikilink")\[1\]。它是内核中准备用户空间程序运行的诸多二进制格式文件处理程序之一
\[2\].。
...
除此之外，用户也可通过[Wine](../Page/Wine.md "wikilink")直接执行[可移植可执行](../Page/可移植可执行.md "wikilink")文件（为[MS-DOS](../Page/MS-DOS.md "wikilink")或[Microsoft
Windows编译](https://zh.wikipedia.org/wiki/Microsoft_Windows "wikilink")）。举个例子，下列配置将使用[Wine](../Page/Wine.md "wikilink")运行DOS和Windows
[EXE文件](../Page/EXE格式.md "wikilink")（根据“MZ”识别）：

`:DOSWin:M::MZ::/usr/bin/wine:`

要使用[Mono](../Page/Mono.md "wikilink")运行EXE（.NET）文件：\[7\]\[8\]
`:CLR:M::MZ::/usr/bin/mono:`

另外，用户可通过此功能运行为其他架构打造的二进制码（需要QEMU）。若二

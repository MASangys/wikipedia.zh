[BlobWarsMBS.png](https://zh.wikipedia.org/wiki/File:BlobWarsMBS.png "fig:BlobWarsMBS.png")
**Blob Wars**是[Parallel
Realities制作的开源](https://zh.wikipedia.org/wiki/Parallel_Realities "wikilink")[动作冒险游戏系列](https://zh.wikipedia.org/wiki/动作冒险游戏 "wikilink")。

## Blob Wars: Metal Blob Solid

**Blob Wars: Metal Blob Solid**很像*[New Zealand
Story](https://zh.wikipedia.org/wiki/New_Zealand_Story "wikilink")*又借鉴了*[Ape
Escape](https://zh.wikipedia.org/wiki/Ape_Escape "wikilink")*。主要是Blob，名为Bob。

游戏中Blob必须救援对抗Galdov (Metal Blob)入侵战争中失散的伙伴。*Metal Blob Solid*大约需要九小时完成。
游戏用[C++](../Page/C++.md "wikilink")写成，使用[SDL调配声音视频和输入](https://zh.wikipedia.org/wiki/Simple_DirectMedia_Layer "wikilink")。

[Linux](../Page/Linux.md "wikilink"), [Microsoft
Windows](https://zh.wikipedia.org/wiki/Microsoft_Windows "wikilink"),
[Mac OS X](https://zh.wikipedia.org/wiki/Mac_OS_X "wikilink"),
[Amiga](../Page/Amiga.md "wikilink"),
[BeOS](../Page/BeOS.md "wikilink"),
[GP2X](https://zh.wikipedia.org/wiki/GP2X "wikilink"), [PlayStation
Portable](../Page/PlayStation_Portable.md "wikilink"),
[Xbox](../Page/Xbox_\(遊戲機\).md "wikilink")
和[Dreamcast](../Page/Dreamcast.md "wikilink")都可以玩。

### 主线

几个朋友莫名消失之后，Bob行动了，独自前往敌营拯救朋友。发现藏在古墓中水晶的魔法力量。战胜Galdov消灭所有的水晶。

## Blob Wars : Blob And Conquer

**Blob Wars : Blob And Conquer** 是是Blob Wars : Metal Blob
Solid的续篇，以3D世界表现。

故事开始于Metal Blob
Solid结束的8个月之后。另外，这次Bob不用过多的解救同胞了，反而有个战斗的同伴，就是Teeka这一个伙计会陪你闯到底，他可以掉队的。

游戏开发大约用了2年时间\[1\]，使用[C++](../Page/C++.md "wikilink")和[SDL](https://zh.wikipedia.org/wiki/Simple_DirectMedia_Layer "wikilink")（输入和音频控制），OpenGL视频渲染。

## 参考

## 外部链接

  - [*Blob Wars: Metal Blob Solid*
    Homepage](http://www.parallelrealities.co.uk/projects/blobWars.php)
  - [*Making of Metal Blob Solid*
    Homepage](https://web.archive.org/web/20101003102426/http://www.parallelrealities.co.uk/articles/makingOfMBS.php)
  - [Parallel Realities Homepage](http://www.parallelrealities.co.uk/)
  - [Game tips and cheats menu on the
    [PSP](../Page/PlayStation_Portable.md "wikilink")
    version](https://web.archive.org/web/20100325134023/http://homepage.bluewin.ch/Chainsaw/index4.html)
  - [GP2X
    port](https://web.archive.org/web/20071023015553/http://gp2x.de/cgi-bin/cfiles.cgi?0,0,0,0,29,1262)
  - [Newsforge Article and
    Interview](http://www.newsforge.com/article.pl?sid=04/12/04/0156233)
  - [*Blob Wars : Blob And Conquer*
    Homepage](http://www.parallelrealities.co.uk/projects/blobAndConquer.php)
  - [*Making of Blob And Conquer*
    Homepage](https://web.archive.org/web/20100110215156/http://www.parallelrealities.co.uk/articles/makingOfBAC.php)

[Category:自由软件](https://zh.wikipedia.org/wiki/Category:自由软件 "wikilink")
[Category:开源游戏](https://zh.wikipedia.org/wiki/Category:开源游戏 "wikilink")
[Category:2003年电子游戏](https://zh.wikipedia.org/wiki/Category:2003年电子游戏 "wikilink")
[Category:平台游戏](https://zh.wikipedia.org/wiki/Category:平台游戏 "wikilink")
[Category:Linux游戏](https://zh.wikipedia.org/wiki/Category:Linux游戏 "wikilink")
[Category:Windows游戏](https://zh.wikipedia.org/wiki/Category:Windows游戏 "wikilink")
[Category:免费游戏](https://zh.wikipedia.org/wiki/Category:免费游戏 "wikilink")
[Category:2008年电子游戏](https://zh.wikipedia.org/wiki/Category:2008年电子游戏 "wikilink")

1.  <http://www.parallelrealities.co.uk/makingOfBAC.php>
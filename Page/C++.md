...
一個常見的混淆其實只是一個微妙的術語問題：由於它的演化來自C，在C++中的術語**物件**和C語言一樣是意味著**記憶體區域**，而不是**-{zh:類別;zh-tw:類別;zh-cn:类}-的實體**，在其它絕大多數的[物件導向語言也是如此](https://zh.wikipedia.org/wiki/物件導向 "wikilink")。舉例來說，在C和C++中，語句`int i;`定義一個`int`型別的物件，這就是變數的值`i`將在指派時，所存入的記憶體區域。

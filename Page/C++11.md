...
14882:1998公開於1998年，第二版於2003年更新，分别通稱C++98以及C++03，两者差异很小），且已被[C++14](../Page/C++14.md "wikilink")取代。相比于C++03，C++11標準包含[核心語言的新機能](https://zh.wikipedia.org/wiki/核心語言 "wikilink")，而且擴展[C++標準程式庫](https://zh.wikipedia.org/wiki/C++標準程式庫 "wikilink")，併入了大部分的[C++
Technical Report
...
早在1972年，C語言誕生的初期，常數[0](../Page/0.md "wikilink")帶有常數及空指標的雙重身分。
...
在C++03中，列舉型別不是型別安全的。列舉型別被視為整數，这使得两种不同的列舉型別之間可以進行比較。C++03唯一提供的安全機制是一个整数或一个枚举型值不能隱式轉換到另一个列舉別型。此外，列舉所使用整數型別及其大小都由實作方法定義，皆無法明確指定。最後，列舉的名稱全數暴露於枚举类型的[作用域](../Page/作用域.md "wikilink")中，因此兩個不同的列舉，不可以有相同的列舉名。（好比`enum

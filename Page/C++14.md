**C++14**是[C++](../Page/C++.md "wikilink")的现行标准的非正式名称，正式名称为"**International
Standard ISO/IEC 14882:2014(E) Programming Language
C++**"。C++14旨在作为[C++11](../Page/C++11.md "wikilink")的一个小扩展，主要提供漏洞修复和小的改进。C++14标准的委员会草案（）N3690于2013年5月15日发表。\[1\]工作草案（）N3936已于2014年3月2日完成。最终的投票期结束于2014年8月15日，结果（一致通过）已于8月18日公布。\[2\]
...
在[C++](../Page/C++.md "wikilink")之前的版本中，模板可以是函数模板或类模板（C++11引入了[类型别名模板](https://zh.wikipedia.org/wiki/C++11#模板的別名 "wikilink")）。C++14现在也可以创建变量模板。在提案中给出的示例是变量`pi`，其可以被读取以获得各种类型的`pi`的值（例如，当被读取为整数类型时为3;当被读取为`float`,`double`,`long
...
[C++11](../Page/C++11.md "wikilink")增加了default member

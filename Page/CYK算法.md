**CYK算法**（，縮寫為CYK
algorithm）是由[約翰·科克](../Page/約翰·科克.md "wikilink")，Younger和共同研究出来大约发表于1965年的一个算法，它是一个用来判定任意给定的字符串\(~w \in \Sigma^*\)
是否属于一个[上下文无关文法的算法](../Page/上下文无关文法.md "wikilink")。普通的[回溯法](../Page/回溯法.md "wikilink")（backtracking）在最坏的情况下需要[指数时间才能解决这样的问题](../Page/指数时间.md "wikilink")，而CYK算法只需要[多项式时间就够了](../Page/多项式时间.md "wikilink")（\(~O(n^3)\)

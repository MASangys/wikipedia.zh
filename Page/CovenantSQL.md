**CovenantSQL** 是一个开源的关系型数据库，具备去中心化、不可篡改、共享经济等特点，CovenantSQL 不仅仅为 ĐApp
提供支持，并且同时支持传统 App。在兼容传统 SQL
的同时，实现了高性能的可拓展性、安全性及保密性。用户可以获得数据的控制权，不需要担心数据安全及隐私问题。简单的迁移步骤及兼容全类型
SQL 可以降低用户的迁移成本。\[1\]。CovenantSQL 从[比特币的论文](../Page/比特币.md "wikilink")
\[2\] 和 vSQL \[3\] 等论文中汲取了最初的设计灵感。

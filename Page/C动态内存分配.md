**C动态内存分配**是在[C语言中为了实现](https://zh.wikipedia.org/wiki/C语言 "wikilink")[动态内存分配而进行的](../Page/动态内存分配.md "wikilink")。这种管理是通过[C標準函式庫中的](../Page/C標準函式庫.md "wikilink")
`malloc`、`realloc`、`calloc`、`free` 等函数进行的。\[1\]\[2\]

[C++](../Page/C++.md "wikilink") 为了兼容 C 语言也提供这些函数，但是更推荐使用 `new`、`delete`
操作符来完成类似的操作。\[3\]
...
`malloc` 所执行的内存分配基于字节数而不是类型，其返回类型为 void 指针（`void
*`），表示该指针所指向区域的[数据类型未知](https://zh.wikipedia.org/wiki/数据类型 "wikilink")。C++
由于其强类型系统，实际使用该指针时需要进行强制类型转换，而 C 语言中则不必进行。

[Degasperis-Procesi_equation_plot.png](https://zh.wikipedia.org/wiki/File:Degasperis-Procesi_equation_plot.png "fig:Degasperis-Procesi_equation_plot.png")
[Degasperis-Procesi_equation_animation.gif](https://zh.wikipedia.org/wiki/File:Degasperis-Procesi_equation_animation.gif "fig:Degasperis-Procesi_equation_animation.gif")
...
\(A(x) := b^3+3*b^3*\frac{(\frac{(1-(1/4)*p^2)}{(1-p^2)}-1)*(\frac{(4*(1-(1/4)*p^2)}{(1-p^2)}-1)}{((2*\frac{(1-(1/4)*p^2))}{(1-p^2)}-1+\sqrt{\frac{(1-(1/4)*p^2}{(1-p^2))}}*cosh(x))}\)

\(B(x,t) := \frac{x}{p}+4*a^2*t+log(\frac{(\alpha+1+exp(x*(\alpha-1)))}{(alpha-1+(e)^(x* (\alpha+1)))})\)

其中

\(alpha := \frac{((2*a-1)*(a+1)}{((2*a+1)*(a-1)))^(1/2)}\)
...
\(B(x,t)=3.33*x+4.2967*t+ln(\frac{(5.418+exp(3.418*x))}{(3.418+exp(5.4181*x))})\)
...
1.  [Degasperis-Procesi
    equation](http://www.scholarpedia.org/article/Degasperis-Procesi_equation)
...
[Sepang_drs.png](https://zh.wikipedia.org/wiki/File:Sepang_drs.png "fig:Sepang_drs.png")

DRS在以下情况下可以使用：

  - 后车和前车的时间差为一秒以内时后车可以启用（包括头车追赶慢车）
  - 后车只能在赛道的规定的超车区域（DRS区域）使用
  - 排位赛可在规定区域内使用DRS

此外，为保证安全：

  - 比赛前两圈不能使用
  - [安全车离开的后两圈不能使用](https://zh.wikipedia.org/wiki/安全车 "wikilink")
  - 前车不得利用DRS来进行防守
  - 在危险的比赛条件时不能使用（例如雨）

出于安全的考虑，从2013赛季起，车手不能在练习赛和排位赛中随意使用DRS，只允许在指定的DRS区域内使用\[3\]。作为补偿，[国际汽联宣布在每个赛道都会设立两个DRS区域](https://zh.wikipedia.org/wiki/国际汽联 "wikilink")。\[4\]

## 评价

有车手和车迷对一级方程式引入DRS反应不一。有人认为这使近年来F1赛车超车缺乏的问题得以解决，使比赛的观赏性大大增强；但另有人认为这使得超车太容易\[5\]，前车的车手没有一个平等的机会保持自己的位置，因为规则不允许前车利用DRS进行防守\[6\]。

## 参见

  - [动能回收系统](https://zh.wikipedia.org/wiki/动能回收系统 "wikilink")
  - [F1赛事规则](https://zh.wikipedia.org/wiki/F1赛事规则 "wikilink")

...
这被转译为“把4和5压入栈顶，通过乘法算符，从栈中弹出两个元素，将二者相乘并把结果压回栈顶”。接着使用`p`命令打印栈顶的元素。`q`命令退出此次调用的dc实例。注意数值相互间必须以空白分隔，但某些算符可以不必如此。
还可以用如下命令得到这个结果：
...
使用命令`k`来变更[算术精度](../Page/有效数字.md "wikilink")，它设置算术运算的小数位数。因为缺省精度是`0`，如下命令序列：
...
通过使用命令`k`调整精度，可以产生任意数目的小数数位。如下命令序列：
...
dc还支持控制输入和输出的[基数](../Page/进位制.md "wikilink")。`i`命令弹出栈顶元素并将它用作输入基数。十六进制数字必须大写以避免和dc命令冲突，输入基数必须在2和16之间，输出基数必须大于等于2。`o`命令设置输出基数，要记住输入基数将影响对后面的所有数值的分析，所以通常建议先设置输出基数。要读取设置的这些数值，`K`、`I`和`O`命令将压入当前精度、输入基数和输出基数到栈顶。

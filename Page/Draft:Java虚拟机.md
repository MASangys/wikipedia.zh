...
验证器仅允许有效程序中的部分字节码序列执行（如[跳跃（分支）指令仅允许对相同](https://zh.wikipedia.org/wiki/分支_\(計算機科學\) "wikilink")[方法内的指令使用](../Page/方法_\(電腦科學\).md "wikilink")）。另外，验证器确保每个指令均处于固定的堆位置，\[10\]即时编译编译器则能转化堆访问至固定的寄存器访问。由于这点，Java虚拟机自身的堆架构并不意味着使用即时编译编译器模拟[寄存器机](../Page/寄存器机.md "wikilink")会影响程序速度。与经代码验证的Java虚拟机架构相比，两者In the face of the code-verified JVM architecture, it makes no difference to a JIT compiler whether it gets named imaginary registers or imaginary stack positions that must be allocated to the target architecture's registers. In fact, code verification makes the JVM different from a classic stack architecture, of which efficient emulation with a JIT compiler is more complicated and typically carried out by a slower interpreter.

**神龍播放器**（Dragon Player）是一個精簡的媒體播放器。這是[KDE
3的Codeine影片播放器改名後繼續進行開發](../Page/KDE_3.md "wikilink")，由於神龍播放器使用[Phonon](../Page/Phonon_\(KDE\).md "wikilink")
- 一個多媒體的API本身可以連接任何的多媒體框架，它將可播放於任何支持的多媒體框架。

## 功能

  - 精簡的界面
  - 恢復播放影片
  - 支援字幕
  - 影片顯示設置（亮度，對比度）
  - 由於使用[Solid和](../Page/Solid_\(KDE\).md "wikilink")[Phonon](../Page/Phonon_\(KDE\).md "wikilink")，神龍播放器是獨立於任何多媒體框架或硬件抽象層
  - 支持播放CD和DVD \[1\]

## 參見

  - [Kaffeine](../Page/Kaffeine.md "wikilink")

## 参考文献

## 外部連結

  - [(zh_TW)| 神龍播放器介紹](http://userbase.kde.org/Dragon_Player)
  - [(zh_CN)| 神龍播放器介紹](http://userbase.kde.org/Dragon_Player)
  - [KDE-Apps.org](http://kde-apps.org/content/show.php?content=17161)

[en:Kdemultimedia\#Dragon
Player](../Page/en:Kdemultimedia#Dragon_Player.md "wikilink")

[Category:KDE](https://zh.wikipedia.org/wiki/Category:KDE "wikilink")

1.  [KDE Commit-Digest for 27th
    January 2008](http://dot.kde.org/1201910126/)
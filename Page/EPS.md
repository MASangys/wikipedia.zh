...
PostScript：電腦要指揮印表機做事，就得說印表機懂得的語言，這類語言主要有PostScript及PCL（Printer Command
Language）兩種。PostScript是Adobe公司所提出的一種列印語言，它是一種複雜的向量式印表機描述語言，所以需要在印表機上加裝昂貴的處理卡與字型，而且印表機因使用PostScript語言還要付費給Adobe公司，所以比較不普遍。不過PostScript語言可以用來放大、縮小文件，也可以保證在不同的環境中，會有相同的列印結果，可是專業印刷的唯一選擇。[PCL則是HP](../Page/PCL.md "wikilink")（惠普公司）在1970年提出的一種印表機語言，因為免費授權給印表機廠商使用，因此成為PC印表機的主流語言。

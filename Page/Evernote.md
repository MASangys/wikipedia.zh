...
2012年5月10日，Evernote宣布正式进入中国，取名“印象笔记”\[7\]。

## 数据存储与访问

在支持的操作系统中，Evernote将笔记存储在本地。

有互联网接入和Evernote帐户的用户可以将笔记远程同步至Evernote的服务器。这种方法允许用户在多台计算机或不同操作系统平台上访问和编辑他们的笔记；即使互联网连接不可用，用户也可以查看，输入和编辑笔记。但是，存储在Evernote的服务器上的笔记未加密。

不使用Evernote客户端软件时，用户也可以直接使用浏览器或者多媒体设备来访问他们的帐户。

## 数据录入

除了键盘输入，在支持的设备上Evernote可以直接用照相机拍照，语音录音来记录笔记。在某些情况下，拍摄图像显示的文字可以被[OCR识别并索引](https://zh.wikipedia.org/wiki/OCR "wikilink")。Evernote也支持触摸屏的手写识别。Evernote网页编辑擴充功能支持大多数网络浏览器，可以方便地捕获裁剪网页，并存储到Evernote。如果没有选定网页的特殊区域，Evernote可以将整个页面剪辑下来。Evernote还支持电子邮件服务功能，允许用户通过电子邮件将笔记添加到自己的Evernote帐户\[8\]。
...
<!-- end list -->
  ...
  - 价格：
    中國版：标准帐户人民幣12元/月或者人民幣98元/年；高级帐户人民幣18元/月或者人民幣148元/年。
    國際版：較中國版價格高。各幣種價格不同，此處不列出。
...
私有邮箱区别：
  ...
  - 印象笔记：<username>.xxxxxx@m.yinxiang.com ；

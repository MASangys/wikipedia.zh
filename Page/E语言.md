...
在E语言中，所有值都是[对象](../Page/面向对象程序设计.md "wikilink")，计算是通过向对象发送消息来执行的。每个物体都属于一个*vat*（类似[行程](../Page/行程.md "wikilink")）。每个vat都有一个执行线程、一个堆栈帧和一个事件队列。[分布式编程只是向远程对象](../Page/分布式计算.md "wikilink")（其他vat中的对象）发送消息的问题。与远程方的所有通信都由E运行时[加密](https://zh.wikipedia.org/wiki/加密 "wikilink")。到达的消息被放置到vat的事件队列中; vat的事件循环按到达的次序逐个处理传入的消息。

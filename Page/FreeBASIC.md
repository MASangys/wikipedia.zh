...
语言的[编译器](../Page/编译器.md "wikilink")\[1\]，可运行在[Microsoft
Windows](../Page/Microsoft_Windows.md "wikilink")、保护模式的[DOS](../Page/DOS.md "wikilink")（[DOS扩展](../Page/DOS扩展.md "wikilink")）、[Linux以及](../Page/Linux.md "wikilink")[Xbox上](../Page/Xbox.md "wikilink")。

FreeBASIC通过使用"QB"关键字可以在高级别上支持用[QBASIC编写的](../Page/QBASIC.md "wikilink")[程序](../Page/程序.md "wikilink")。许多用QBASIC写的程序可以在不需任何修改的情况下编译和运行在这个模式下。但是如果使用默认的参数来编译的话，大部分的程序还是要进行一些修改的。
...
命令行选项），[空指针检查](../Page/空指针.md "wikilink")（同上）。
...
它使用[GNU
Binutils](../Page/GNU_Binutils.md "wikilink")[软件开发工具作为](../Page/软件开发工具.md "wikilink")[后端](../Page/后端.md "wikilink")，除了动态和静态[库](../Page/库.md "wikilink")，还可以制作控制台和图形／[GUI可执行文件](../Page/GUI.md "wikilink")。FreeBASIC完全支持[C库和部分](../Page/C语言.md "wikilink")[C++库](../Page/C++.md "wikilink")。这使得程序员可以使用和创建C和许多其他语言的库。

FreeBASIC还不是一个[优化编译器](../Page/优化编译器.md "wikilink")，但是它的速度被认为接近主流工具，如[GCC](../Page/GCC.md "wikilink")\[2\]。

## 语法

FreeBASIC语法试图尽量保持接近[BASIC语法](../Page/BASIC.md "wikilink")，特别是[QuickBASIC](../Page/QuickBASIC.md "wikilink")。虽然语法上试图保持与其前辈一致，FreeBASIC还是遵循了一些现代的标准和编码实现。具有标准的过程特性以及[面向对象特性](../Page/面向对象.md "wikilink")，如[对象](../Page/对象_\(计算机科学\).md "wikilink")、[运算符重载](../Page/运算符重载.md "wikilink")、[函数重载和](../Page/函数重载.md "wikilink")[命名空间等等](../Page/命名空间.md "wikilink")，已经被加入到FreeBASIC中。

FreeBASIC的行在遇到[行尾字符或](../Page/行尾.md "wikilink")[冒号时结束](../Page/冒号.md "wikilink")。因此不需要使用特殊字符（如[C语言中的](../Page/C语言.md "wikilink")[分号](../Page/分号.md "wikilink")）来通知编译器此行结束。多条语句可以通过冒号`:`分割写在一行里。

FreeBASIC支持块[注释和行尾注释](../Page/注释.md "wikilink")。整行注释使用一个[撇号](../Page/撇号.md "wikilink")`'`，块注释从`/'`开始，到`'/`结束。
...
\- 当选择FB语言设置时（[命令行参数为](../Page/命令行参数.md "wikilink")*-lang
fb*），将开启所有的FreeBASIC新特性，并且不允许使用QuickBASIC的"hackish"特性（该特性不符合现代编程实践）。

\- FB-Lite语言设置（*-lang
fblite*）提供了访问大多数FreeBASIC中新的非面向对象的特性，但是允许使用与老版本BASIC类似的代码风格。语法规则，如允许隐变量、后缀、[`GOSUB`](../Page/GOSUB.md "wikilink")
/ `RETURN`、[行号等等](../Page/行号.md "wikilink")。
...
FreeBASIC拥有一个内置的2D软件[图形库](../Page/图形库.md "wikilink")，与QuickBASIC兼容，为用户提供了简单的图形元素（如矩形，线和圆）、[位块传输以及一些QuickBASIC的图形库中没有的附加特性](../Page/位块传输.md "wikilink")。图形库本身并不依赖于操作系统，因此可以跨平台使用。

虽然图形库是内置的，但是只有在选择使用时才会被包含，这可以简单地通过调用FBgfx
*Screen*命令来实现。通用库（如[OpenGL](../Page/OpenGL.md "wikilink")+）为了[硬件加速使用](../Page/硬件加速.md "wikilink")[API](../Page/API.md "wikilink")（Windows、Linux等）来创建一个[窗口可以不包含FreeBASIC的图形库](../Page/窗口.md "wikilink")。

## 未来的开发

FreeBASIC仍然继续开发，目标是成为一个[GCC前端](../Page/GCC.md "wikilink")\[4\]，这将带来许多[C++和其他](../Page/C++.md "wikilink")[面向对象编程语言中的新特性](../Page/面向对象编程语言.md "wikilink")，高级[优化技术](../Page/优化.md "wikilink")，以及可以移植到任何现代系统上的能力。

0.17版中已经加入了面向对象程序设计的[类](../Page/类.md "wikilink")，扩展了基本的[数据类型](../Page/数据类型.md "wikilink")。还有更多的新特性值得期待。

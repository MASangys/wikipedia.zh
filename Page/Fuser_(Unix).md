[Unix命令](https://zh.wikipedia.org/wiki/Unix "wikilink")**fuser**用于显示哪些[进程正在使用给定的](https://zh.wikipedia.org/wiki/进程 "wikilink")[文件](https://zh.wikipedia.org/wiki/文件 "wikilink")、[文件系统](../Page/文件系统.md "wikilink")或[unix套接字](https://zh.wikipedia.org/wiki/unix套接字 "wikilink")。例如，要查看访问USB驱动器的进程ID及其用户：
...
## 选项
  ...
  - \-k : 杀死所有访问文件的进程。例如，`fuser -k /path/to/your/filename`会杀死所有访问此目录的进程而无需确认。使用-i在操作前进行确认
    \-i : 交互模式。杀死进程前提示
    \-v : 详细模式
    \-u : 显示用户名
    \-a : 显示所有文件
    \-m : `name`指定已挂载的文件系统或块设备上的文件。列出所有访问该文件系统上文件的进程。如果指定了目录文件，将自动更改为`name`/.，即是可能挂载在此目录中的任何文件系统。
...
## 相关命令
  ...
  - [BSD](../Page/BSD.md "wikilink")操作系统上的等价命令是`fstat(1)`

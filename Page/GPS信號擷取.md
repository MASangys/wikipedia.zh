...
MHz，從特定衛星得到的頻率並不一定是如規格所給予，而是因為有相對運動的關係，在頻率上產生[都卜勒效應](../Page/都卜勒效應.md "wikilink")，因為衛星的移動，假設GPS接收機靜止接收GPS信號，都卜勒效應所影響的範圍大約為5
kHz，如果GPS接收機也處於高速移動的狀態下，都卜勒效應所影響的頻率可能會到達10 kHz。　　　

  - 碼相位

碼相位代表[C/A 碼開始的正確資料區塊](../Page/C/A_碼.md "wikilink")

目前來說，做GPS信號擷取的方法都是根據GPS信號的特質來做處理，衛星信號是由32個不同的偽隨機雜訊（Pseudorandom
Noise,PRN）序列區隔，而上述所說的特質就是指這些偽隨機雜訊序列的彼此之間的幾乎沒有[互相關](../Page/互相關.md "wikilink")，只有自己本身和自己只有在沒有延遲的情況下，[自相關函數才會有極值](../Page/自相關函數.md "wikilink")。假設接收到的信號\(\mathit{s}\)是由\(\mathit{n}\)個可得知的衛星信號組合而成。

`     `\(s(t)=s^1(t)+s^2(t)+...+s^n(t)\)

會運用接收機本身的偽隨機雜訊序列產生器產生\(k\)衛星的偽隨機雜訊序列，再透過改變碼相位和[振盪器所產生的載波頻率不同](../Page/振盪器.md "wikilink")，來與接收到的信號做處理比對，而這幾種方法分別為：
...
接下來將所得到的信號I、Q對時間1毫秒做[積分](../Page/積分.md "wikilink")，最後再平方相加得到信號的能量，若所得到的能量超過所設定的標準，則頻率和碼相位參數就是正確的，接著就會把這些參數傳遞到[GPS信號追蹤](../Page/GPS信號追蹤.md "wikilink")（GPS
...
如圖2所示，輸入信號首先和接收機本身所產生的特定衛星的偽隨機雜訊序列相乘，碼相位分別為0\~1022總共1023種，所得到的信號再利用[傅立葉變換從時域轉換到頻域](../Page/傅立葉變換.md "wikilink")，這裡的傅立葉變換可用[離散傅立葉變換或是](../Page/離散傅立葉變換.md "wikilink")[快速傅立葉變換](../Page/快速傅立葉變換.md "wikilink")，當輸入信號與接收機本身所產生的信號完美的相關之時（意即同樣的偽隨機雜訊序列而且沒有延遲。），傅立葉變換後得到的極值落點的頻率，代表的就是載波信號的頻率。

所得到頻率的精確度決定于離散傅立葉變換的長度，長度代表了在分析資料裡的[取樣數](../Page/取樣.md "wikilink")，要是1毫秒的資料被分析，則取樣數與取樣頻率的關係為1比1000，也就是說，當[取樣頻率](../Page/取樣頻率.md "wikilink")\(\mathit{f_s}=10 MHz\)，則取樣數\(\mathit{N}=10,000\)。

在離散傅立葉變換的長度為10000，前 N/2 個輸出取樣代表了從 0 到
\(\frac{f_s}{N}\)Hz，意即輸出頻率的[解析度為](../Page/解析度.md "wikilink")：
...
信號擷取的目標就是要將輸入信號和接收機本身產生的偽隨機雜訊序列做相關運算，於是我們用[圓形相關運算](../Page/圓形相關運算.md "wikilink")（circular
...
\(X^{*}(k)\)是指\(X(k)\)的[共軛複數](../Page/共軛.md "wikilink")（Complex
Conjugate），整個演算法的流程如右圖3所示，輸入信號先與接收機本身震盪器產生的載波頻率相乘，得到了\(I\)
信號和與其相位差\(90^o\)的信號\(Q\) 這兩個信號組成了複數的信號

\(x(n)=I(n)+jQ(n)\)

接著再做離散傅立葉變換，而接收機本身所產生的偽隨機雜訊序列去做傅立葉變換後得到的信號取共軛，這兩個信號做相乘，再做[反傅立葉變換](../Page/反傅立葉變換.md "wikilink")，得到的信號是在時域的，而再取絕對值平方項，若有極值，極值所在的點即為碼相位。跟之前演算法做比較，此演算法只搜尋了41種頻率，效率更是大大的提升，但是由於要做傅立葉變換和反傅立葉變換，在硬體實現上亦有困難處所在。

## 參見

  - [全球定位系統](../Page/全球定位系統.md "wikilink")
  - [GPS信號](../Page/GPS信號.md "wikilink")
  - [C/A 碼](../Page/C/A_碼.md "wikilink")
...
code](../Page/en:GPS_signals#Coarse/Acquisition_code.md "wikilink")

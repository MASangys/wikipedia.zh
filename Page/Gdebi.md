**Gdebi**
是一个安装[.deb软件包的工具](../Page/.deb.md "wikilink")。它提供了图形化的使用界面，但也有命令行选项。
在[Ubuntu中会默认安装Gdebi](../Page/Ubuntu.md "wikilink")。

## 技术

Gdebi 安装本地的.deb软件包，并自动解决依赖问题(它可以自动下载并安装所需的软件包)。

## 参见

  - [Synaptic](../Page/Synaptic.md "wikilink")
  - [RPM](../Page/RPM.md "wikilink")
  - [Yum](../Page/Yum.md "wikilink")

## 外部链接

  - [Gdebi项目主页](https://launchpad.net/gdebi)

[Category:Linux软件包管理相关软件](https://zh.wikipedia.org/wiki/Category:Linux软件包管理相关软件 "wikilink")
[Category:开放源代码](https://zh.wikipedia.org/wiki/Category:开放源代码 "wikilink")
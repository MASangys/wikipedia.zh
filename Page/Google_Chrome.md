...
Vista系统的支持](../Page/Windows_Vista.md "wikilink")\[14\]，这两个系統上的最後版本為49.0.2623.112\[15\]。
...
Play上发布](../Page/Google_Play.md "wikilink")。6月29日，Chrome的iOS版本在[App
Store上架](../Page/App_Store_\(iOS\).md "wikilink")，可免費安裝，標誌著Google
Chrome全面進軍行動平台。

2013年1月11日，Chrome Beta for Android在[Google
Play上架](../Page/Google_Play.md "wikilink")\[36\]。
...
目前，「Google Chrome」存在四个版本分支：
  ...
  - \-{zh-hk:[Stable
    Channel（穩定版／正式版）](https://www.google.com/intl/zh-HK/chrome/);
    zh-cn:[Stable
    Channel（稳定版／正式版）](https://www.google.com/intl/zh-CN/chrome/browser/);
    zh-tw:[Stable
    Channel（穩定版／正式版）](http://www.google.com/intl/zh-TW/chrome/browser/)}-，为追求稳定的普通用户使用。
  - \-{zh-hk:[Beta
    Channel（測試版）](https://www.google.com/intl/zh-HK/chrome/browser/index.html?extra=betachannel);
    zh-cn:[Beta
    Channel（测试版）](https://www.google.com/intl/zh-CN/chrome/browser/index.html?extra=betachannel)}-，正式版發布前的公開測試所用，版本上的新功能不會作太多修改，主要為安全上的測試，可能會存在不穩定情況，適用喜歡較穩定又可嘗鮮的用户下載使用。
  - \-{zh-hk:[Dev
    Channel（開發版）](https://www.google.com/intl/zh-HK/chrome/dev/);
    zh-cn:[Dev
    Channel（开发版）](https://www.google.com/intl/zh-CN/chrome/browser/index.html?extra=devchannel)}-，主要为功能上的测试，可能存在稳定性问题，更新速度通常为一周一次，适合开发人员使用。\[37\]
  - [Canary
    Build（構造版）](https://www.google.com/intl/zh-CN/chrome/canary/)，2010年7月加入\[38\]，比开发版更新更不穩定，但相對的也會加入更多測試性的新功能，仅适合开发人员或追求紧贴最新版本的用户使用。Canary
    Build有一個特色是它可以**獨立安裝**及**免安裝**，不會覆蓋原本的「Google
    Chrome」版本，讓系統中可以同時擁有两套安裝版「Google
    Chrome」，但Canary build的用户文件不能與其他分支共用。
...
<!-- end list -->
  ...
  - 詐騙和惡意程式保護
    當「Google
    Chrome」偵測到瀏覽網站可能有害時，便會發出即時的警告。瀏覽器會將造訪網址的一部分傳送給Google，以瞭解該網址是否列在已知的詐騙與惡意網站清單中。[（網路釣魚及惡意軟體的偵測功能）](http://www.google.com/support/chrome/bin/answer.py?answer=99020)

<!-- end list -->

  - 自動更新（Auto-updates）
    為確保受到最新版安全性更新的保護，「Google
    Chrome」會定期檢查更新，以確保更新永遠為最新版本。更新檢查可確保會使用最新版的安全性功能與修正程式，瀏覽器自動更新新的版本，不需要使用者採取任何動作，更新後會在下一次啟動中自動使用。
...
<!-- end list -->
  ...
  - 針對瀏覽錯誤提供建議
    出現無法解析網址或未能成功連線的狀況時，「Google
    Chrome」會嘗試判斷想要前往的網頁，並提供建議。瀏覽器會將嘗試瀏覽的網頁網址傳送到Google，以便建議替代或類似的網頁。\[54\]

<!-- end list -->

  - DNS預先擷取
    DNS預先擷取是指「網域名稱系統」預先擷取。當瀏覽網頁時，「Google
    Chrome」可查詢或預先擷取網頁上所有連結的IP位址。瀏覽器使用IP位址來載入網頁，因此只要事先查詢這些資料，當按下網頁上的任何連結時，便可更快速地載入目標網頁。
...
<!-- end list -->
  ...
  - 表單自動填入
    ...
    **「Google
    Chrome」可儲存您的地址及信用卡資訊。**首次填入表單時，瀏覽器會自動儲存所輸入的聯絡資訊，作為「自動填入」的輸入項目，例如名稱、地址、電話號碼或電子郵件地址。
...
<!-- end list -->
  ...
  -
    **按一下即可自動填入表單。**填入表單時，「Google
    Chrome」會偵測正在輸入的文字是否符合「自動填入」項目中儲存的資訊。如果有符合項目的話，則在正填寫的欄位下方會出現下拉式選單，其中顯示與輸入內容相符的項目。
...
<!-- end list -->
  ...
  - 網址列（Omnibox）
    在「Google
    Chrome」的搜尋與位址列組合（又稱為「網址列」）中，可輸入搜尋內容或網址。「網址列」會在您輸入搜尋內容與網址時提供建議，還提供自動完成功能，協助您輸入幾個字就可以找到所需的內容。此外還可以搜尋瀏覽記錄和已加入書籤的網站。在Windows
    XP系统的电脑上安装最新版的Chrome后，还可能会出现将Google搜索框与Omnibox结合的界面。
      - 查看網頁警示
    <!-- end list -->
      - 安全警示
      - 彈出式視窗警示
...
### 擴充功能与應用程式
  ...
  - 使用者自訂擴充功能
    2009年9月6日，「Google
    Chrome」首次預設啟用擴充功能，並提供數款擴充功能\[62\]。在官方擴充功能中心中當使用者找到喜歡的擴充功能時，按一下網頁上的「安裝」，擴充功能便會安裝到使用者的「Google
    Chrome」。整個過程無需重新啟動瀏覽器。
    ...
    **從9.0版本起，使用者可以隱藏「Google
    Chrome」上已安裝並顯示在網址列右方的擴充功能**，此外「設定」圖示左上方會顯示新安裝並在背景運行的擴充功能/應用程式，同時在功能表內加入顯示在背景運行的擴充功能/應用程式的數量一行。
    從10.0版本起，「Google Chrome」默認啟用語音輸入的API
    從25.0版本起，「Google
    Chrome」開始支援語音辨識輸入API，並且在某些網站或程式要自動安裝擴充功能時，改為讓使用者手動確認是否安裝。
...
<!-- end list -->
  ...
  - [Chrome Web
    ...
    **Chrome Web Store**，前稱「Google Chrome擴充功能中心」。
    ...
    Store」上線一年後，宣布擁有超過8500款擴充套件及超過1500款主題，超過三分之一使用者安裝至少一個擴充功能，同時共有超過7000萬擴充功能和主題被安裝\[70\]。
...
從7.0版本的開發版本起，「Google
Chrome」（除正式版本外）加入研究室功能，將未完成或未完善的功能收集供使用者測試，此前所有測試功能都需要使用指令碼啟用。
由7.0至8.0（62681）版本，使用者可經「about:labs」進入研究室。從8.0（62682）版本起易名為「about:flags」或「chrome://flags」\[72\]。
...
## 外部連結
  ...
  - [Google
    Chrome漫畫](http://tech.sina.com.cn/i/2008-09-02/21522430443.shtml)
...
13.  StatCounter Global
    Stats|last=Statcounter|website=gs.statcounter.com|language=en|access-date=February
    9, 2018}}
...
36. [Chrome beta for Android登錄Google Play，使用Chrome
    V25内核](http://www.guao.hk/posts/chrome-beta-for-android-released-on-google-play.html)
...
66. [Google Docs
    Blog](http://googledocs.blogspot.com/2010/06/chrome-extensions-for-google-docs.html)
...
77. [THE HTML5
    TEST官方統計](https://beta.html5test.com/results/desktop.html)
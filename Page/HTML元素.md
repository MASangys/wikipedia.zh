...
在HTML語法中，大多數元素編寫一個開始標記和結束標記，與其之間的內容。**HTML標記**係由元素名稱，包圍於兩側之小於號與大於號之[括號所組成](https://zh.wikipedia.org/wiki/括號 "wikilink")。其結束標記擁有一斜線位在小於號之括號後，有別於其起始標記。例如：一個段落係由`p`元素所代表，其被寫成
...
HTML元素是在自1995年以來發布的一系列免費開放標準中定義的，最初由[IETF以及隨後由](https://zh.wikipedia.org/wiki/IETF "wikilink")[W3C制订](https://zh.wikipedia.org/wiki/W3C "wikilink")。
...
<!-- end list -->
  ...
  -
    指定文件的[CSS樣式](https://zh.wikipedia.org/wiki/層疊樣式表 "wikilink")，其形式通常為：
      -
        <code>
        <style type="text/css">
        ...
    ...
    既可作為一個容器之樣式說明，亦可連結至外部CSS樣式表，例如，在[CSS中](https://zh.wikipedia.org/wiki/層疊樣式表 "wikilink")，與表單中的`@import`指令：
      -
        <code>
        <style>
        @import *url*;
...
區塊元素的矩形結構經常稱之為[盒模型](https://zh.wikipedia.org/wiki/:File:W3C_and_Internet_Explorer_box_models.svg "wikilink")，並由幾個部分所組成。每個元素包含以下內容：
  ...
  - 一個元素的**邊距（margin）**是該元素周圍之空白。任何其他元素之content、padding及border將不允許輸入該區域，除非迫於一些進階的[CSS布局](https://zh.wikipedia.org/wiki/CSS "wikilink")。使用多數的標準[DTD](https://zh.wikipedia.org/wiki/文件類型描述 "wikilink")，margin在不同元素的左邊和右邊將互相推離。另一方面，margin在元素的上方及下方，將不堆疊或相互交融。這意味著位於該元素之間的空白將與較大的margin一樣大。

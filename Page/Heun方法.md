...
\[y'(t) = f(t,y(t)), \qquad y(t_0)=y_0,\] 根據Heun法，首先计算中间值\(\tilde{y}_{i+1}\)，然后計算在下一个積分点的最終近似值\(y_{i+1}\)。
...
[Heun's_Method_Diagram.jpg](https://zh.wikipedia.org/wiki/File:Heun's_Method_Diagram.jpg "fig:Heun's_Method_Diagram.jpg") 欧拉方法是Heun法的基础。欧拉方法利用区间開端端点的函數切线，来估计函数在此区间内的斜率，並假设著如果步长很小，误差就很小。然而，即使在步长非常小的情况下，由于大量步骤的积累误差使估计偏离实际函数的值。

**HootSuite**是一家网站和在线品牌管理服务提供商，支持诸如[Facebook](../Page/Facebook.md "wikilink")、[Twitter](../Page/Twitter.md "wikilink")、[LinkedIn](https://zh.wikipedia.org/wiki/LinkedIn "wikilink")、[Foursquare](../Page/Foursquare.md "wikilink")、[Ping.fm](https://zh.wikipedia.org/wiki/Ping.fm "wikilink")、[Ning](../Page/Ning.md "wikilink")、[mixi](https://zh.wikipedia.org/wiki/mixi "wikilink")、[MySpace以及](https://zh.wikipedia.org/wiki/MySpace "wikilink")[WordPress.com](../Page/WordPress.com.md "wikilink")。它同时还是一款Twitter客户端。\[1\]

## 服务内容

该服务通常被用来管理网上品牌，并向Twitter等社群網站发送信息。使用HootSuite的公司和组织包括[Facebook](../Page/Facebook.md "wikilink")、[奥巴马政府等](https://zh.wikipedia.org/wiki/巴拉克·奥巴马 "wikilink")。\[2\]\[3\]HootSuite提供基于浏览器的界面，用户可以时刻了解Twitter账户的最新动态。\[4\]HootSuite还使用[縮略網址服務ow](../Page/縮略網址服務.md "wikilink").ly来缩短发布的URL地址。\[5\]

## 参考资料

## 外部链接

  - [官方网站](http://hootsuite.com/)

[Category:Twitter](https://zh.wikipedia.org/wiki/Category:Twitter "wikilink")
[Category:网站](https://zh.wikipedia.org/wiki/Category:网站 "wikilink")
[Category:Web_2.0](https://zh.wikipedia.org/wiki/Category:Web_2.0 "wikilink")
[Category:被防火长城封锁的网站](https://zh.wikipedia.org/wiki/Category:被防火长城封锁的网站 "wikilink")

1.  [tabs on your
    brand"](http://www.nationalpost.com/rss/Keeping+tabs+your+brand/3036293/story.html%22Keeping).
    *The National Post*. Retrieved July 4, 2010.

2.  ["HootSuite helps you manage your brand on Twitter (hey, it works
    for the White
    House)"](http://venturebeat.com/2009/07/30/hootsuite-the-white-houses-twitter-client-opens-to-public/).
    VentureBeat. Retrieved July 4, 2010.

3.
4.
5.  ["Ow.ly and Ht.ly \~ Choose Bar or Non-bar URL Shortener for
    HootSuite"](http://blog.hootsuite.com/htly-owly/). HootSuite.
    Retrieved July 4, 2010.
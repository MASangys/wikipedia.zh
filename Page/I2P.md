...
I2PTunnel是I2P的内置程序，它可以通过[\#隧道将远程计算机上的端口映射为本地主机端口](https://zh.wikipedia.org/wiki/#隧道 "wikilink")，从而允许任意TCP/IP程序通过I2P进行通信。
...
I2P免费的假名Email服务，由'Postman'维护。邮件传输服务器为*pop.mail.i2p*（[POP3](https://zh.wikipedia.org/wiki/Post_Office_Protocol "wikilink")）及*smtp.mail.i2p*（[SMTP](https://zh.wikipedia.org/wiki/Simple_Mail_Transfer_Protocol "wikilink")）。可以通过I2PTunnel利用普通的Email客户端访问。
...
每10分钟，用户的I2P路由就会与新的节点建立连接，将本机数据包与其他I2P节点中转的数据包一并通过这些隧道中转传递，令数据包最终到达最终的[\#目标](https://zh.wikipedia.org/wiki/#目标 "wikilink")。

> 本文内容由[IEEE 802.11b-1999](https://zh.wikipedia.org/wiki/IEEE 802.11b-1999)转换而来。


**802.11b标准**，全称为**IEEE 802.11b-1999**是对[802.11原始标准](../Page/IEEE_802.11_\(原始标准\).md "wikilink")[物理层](../Page/物理层.md "wikilink")上的一个修订。802.11b工作在2.4G[信道](../Page/信道.md "wikilink")上，物理层速率提升至11Mbps。该规范在世界上得到了较广泛的应用。相关的修改已经集成进IEEE 802.11-2007，成为[802.11协议的一部分](https://zh.wikipedia.org/wiki/802.11 "wikilink")。

## 描述

802.11b工作在2.4G的[ISM频段](../Page/ISM频段.md "wikilink")上，物理层上使用了的（）调制方式，不同于802.11原始标准，使得最大速率达到11Mbps。在[介质访问控制](../Page/介质访问控制.md "wikilink")上，标准采用[载波侦听多路访问／冲突避免](https://zh.wikipedia.org/wiki/载波侦听多路访问#CSMA.2FCA "wikilink")（CSMA/CA）的方式，这与原始标准相同。考虑到CSMA/CA协议的开销，802.11b设备间的最大吞吐量，[TCP可以达到](https://zh.wikipedia.org/wiki/TCP "wikilink")5.9Mbps，[UDP达到](https://zh.wikipedia.org/wiki/UDP "wikilink")7.1Mbps。由于[微波炉](../Page/微波炉.md "wikilink")，[蓝牙设备](https://zh.wikipedia.org/wiki/蓝牙 "wikilink")，[ZigBee](../Page/ZigBee.md "wikilink")等产品都工作在ISM频段上，802.11b设备可能会受到其他设备的干扰。

802.11b设备在1999年中就开始在市场上出现。[苹果公司的](https://zh.wikipedia.org/wiki/苹果公司 "wikilink")[iBook是第一个配置](https://zh.wikipedia.org/wiki/iBook "wikilink")802.11b网络（可选）的主流电脑。

## 信道和频率

[2.4_GHz_Wi-Fi_channels_(802.11b,g_WLAN).svg](https://zh.wikipedia.org/wiki/File:2.4_GHz_Wi-Fi_channels_\(802.11b,g_WLAN\).svg "fig:2.4_GHz_Wi-Fi_channels_(802.11b,g_WLAN).svg")

| 信道 | 中心频率      | 相邻信道的频率差 | 频宽              | 重叠的信道      |
| -- | --------- | -------- | --------------- | ---------- |
| 1  | 2.412 GHz | 5 MHz    | 2.401–2.423 GHz | 2-5        |
| 2  | 2.417 GHz | 5 MHz    | 2.406–2.428 GHz | 1,3-6      |
| 3  | 2.422 GHz | 5 MHz    | 2.411–2.433 GHz | 1-2,4-7    |
| 4  | 2.427 GHz | 5 MHz    | 2.416–2.438 GHz | 1-3,5-8    |
| 5  | 2.432 GHz | 5 MHz    | 2.421–2.443 GHz | 1-4,6-9    |
| 6  | 2.437 GHz | 5 MHz    | 2.426–2.448 GHz | 2-5,7-10   |
| 7  | 2.442 GHz | 5 MHz    | 2.431–2.453 GHz | 3-6,8-11   |
| 8  | 2.447 GHz | 5 MHz    | 2.436–2.458 GHz | 4-7,9-12   |
| 9  | 2.452 GHz | 5 MHz    | 2.441–2.463 GHz | 5-8,10-13  |
| 10 | 2.457 GHz | 5 MHz    | 2.446–2.468 GHz | 6-9,11-13  |
| 11 | 2.462 GHz | 5 MHz    | 2.451–2.473 GHz | 7-10,12-13 |
| 12 | 2.467 GHz | 5 MHz    | 2.456–2.478 GHz | 8-11,13-14 |
| 13 | 2.472 GHz | 5 MHz    | 2.461–2.483 GHz | 9-12, 14   |
| 14 | 2.484 GHz | 12 MHz   | 2.473–2.495 GHz | 12-13      |

[802.11b](../Page/IEEE_802.11.md "wikilink") 的信道和频率 \[1\]

  -
    *注：14信道仅在日本使用；信道12-13在美国等一些国家不允许使用。*

## 参见

  - [IEEE 802.11](../Page/IEEE_802.11.md "wikilink")
  - [IEEE 802.11g-2003](../Page/IEEE_802.11g-2003.md "wikilink")
  - [Wi-Fi](../Page/Wi-Fi.md "wikilink")
  - [WLAN信道列表](../Page/WLAN信道列表.md "wikilink")

## 参考

  -
  -
[Category:IEEE_802.11](https://zh.wikipedia.org/wiki/Category:IEEE_802.11 "wikilink")

1.  <http://download.wcvirtual.com/reference/802%20Channel%20Freq%20Mappings.pdf>
...
此處**grandfather.h**第一次被引入時會定義巨集**H_GRANDFATHER**。當**father.h**再次引入**grandfather.h**時，`#ifndef`測試失敗，[編譯器會直接跳到](../Page/編譯器.md "wikilink")`#endif`的部分，也避免了第二次定義`foo`結構。程式也就能夠正常編譯。

...
和其他一些發行版採用 [Upstart](../Page/Upstart.md "wikilink")\[3\] 來代替\[4\] 傳統的
init
處理程序。至2015年，大部分[Linux發行版都已採用新的](https://zh.wikipedia.org/wiki/Linux發行版 "wikilink")[systemd替代](https://zh.wikipedia.org/wiki/systemd "wikilink")[System
V和](../Page/UNIX_System_V.md "wikilink")[Upstart](../Page/Upstart.md "wikilink")，但[systemd向下相容](https://zh.wikipedia.org/wiki/systemd "wikilink")[System
V](../Page/UNIX_System_V.md "wikilink")。

## BSD风格

BSD init 运行存放于'/etc/rc'的初始化 [shell](../Page/Unix_shell.md "wikilink")
[脚本](https://zh.wikipedia.org/wiki/脚本 "wikilink")，然后启动基于文本模式的终端([getty](https://zh.wikipedia.org/wiki/getty_\(Unix\) "wikilink"))或者基于图形界面的终端([窗口系统](https://zh.wikipedia.org/wiki/窗口系统 "wikilink")，如
[X](https://zh.wikipedia.org/wiki/X_Window_System "wikilink"))。
这里没有[运行模式的问题](https://zh.wikipedia.org/wiki/运行模式 "wikilink")，因为文件
'rc' 决定了 init 如何执行。
...
collection](https://zh.wikipedia.org/wiki/ports_collection "wikilink")/[pkgsrc](https://zh.wikipedia.org/wiki/pkgsrc "wikilink")
...
系统是否有一个默认[运行模式](https://zh.wikipedia.org/wiki/运行模式 "wikilink")。如果没有默认的运行模式，那么用户将进入系统控制台，手动决定进入何种运行模式。
...
[System
V中](../Page/UNIX_System_V.md "wikilink")[运行模式描述了系统各种可能的状态](https://zh.wikipedia.org/wiki/运行模式 "wikilink")。通常会有
...
目前絕大多數[Linux發行版已經基於新的](https://zh.wikipedia.org/wiki/Linux發行版 "wikilink")[systemd](https://zh.wikipedia.org/wiki/systemd "wikilink")，systemd一般不再使用`/etc/inittab`檔案，也使用新的target取代System
...
Linux系统中，现代的[bootloader](https://zh.wikipedia.org/wiki/bootloader "wikilink")(如
[LILO](../Page/LILO.md "wikilink") 或者
[GRUB](https://zh.wikipedia.org/wiki/GRUB "wikilink"))，用户可以在初始化过程中最后启动的进程来取代默认的
`/sbin/init`。

通常是在 bootloader 环境中通过执行
`init=`[`/foo/bar`](https://zh.wikipedia.org/wiki/foobar "wikilink")
命令。例如，如果执行 `init=/bin/bash`，启动单用户 root 的 shell 环境，无需用户密码。
...
如果内核加载后， init 不能被正常启动， 这将导致
[panic](https://zh.wikipedia.org/wiki/Kernel_panic "wikilink")
错误，此时系统将不可使用。想要通过 init 自身来改变 init
的路径，不同的版本情况不太一样(NetBSD中可执行 `boot -a` ;
...
很多人一直努力地从某些方面改进传统的 init 守护进程，使它变得更完善。下面列出的是一些改进（排序不分先后）：
  ...
  - [SystemStarter](../Page/SystemStarter.md "wikilink")：用来替代
    [launchd](https://zh.wikipedia.org/wiki/launchd "wikilink") —
    [Apple](https://zh.wikipedia.org/wiki/苹果公司 "wikilink") [Mac OS
    X开启进程](https://zh.wikipedia.org/wiki/Mac_OS_X "wikilink")
  ...
  - Mudur：用 [Python](../Page/Python.md "wikilink") 写成的 init 替代品，
    可以异步开启进程，[Pardus](https://zh.wikipedia.org/wiki/Pardus_\(操作系统\) "wikilink")
    Linux 发行版
  - [systemd](https://zh.wikipedia.org/wiki/systemd "wikilink")：完全替代init，可并行启动服务，并能减少在shell上的系统开销，相比传统的[System
    V是一大革新](../Page/UNIX_System_V.md "wikilink")，已被大多数Linux发行版所使用。

下面列出的项目还没有大范围的使用：

  - [eINIT](https://zh.wikipedia.org/wiki/Einit "wikilink"), 完全代替 init
    ，可以异步开启进程，但是完成这个过程可以不使用 shell 脚本
  - *svscan* 来自
    [daemontools](https://zh.wikipedia.org/wiki/daemontools "wikilink")
    被用作 1 号进程 - 似乎将被 runit 替代
  - cinit
...
## 参见
  ...
  - [pidof](https://zh.wikipedia.org/wiki/pidof "wikilink") 或者 killall5,
    从 System V 开始在很多发行版中使用的另一种程序

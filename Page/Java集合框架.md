...
framework**)是一個包含一系列實作可重複使用集合的[資料結構的](../Page/資料結構.md "wikilink")[類別和](../Page/類別_\(電腦科學\).md "wikilink")[介面集合](../Page/介面_\(程式設計\).md "wikilink")。
...
當時若想使用可重用集合[資料結構](../Page/資料結構.md "wikilink")，也是有數種第三方框架可供選擇。最廣為使用的例如的*集合包*(Collections
package)\[4\]的*集合包*\[5\]
...
Java的java.util.Set定義了集。集不能包含任何重複的元素，另外集也沒有順序。也因為如此，在集內的元素無法以索引存取。在集底下實作了**雜湊集**(java.util.HashSet)、**連結雜湊集**(java.util.LinkedHashSet)和**樹狀集**(java.util.TreeSet)。雜湊集使用**雜湊映射表**(java.util.HashMap)來儲存元素和其雜湊值來防止重複。連結雜湊集藉由建立雙向連結列表來按照各個元素的加入順序進行聯繫，如此可以保持這個集的順序。樹狀集使用[紅黑樹來確保沒有重複的元素](../Page/紅黑樹.md "wikilink")。另外，如此的實作容許樹狀集能實作**已排序集**(java.util.SortedSet)。\[13\]
...
Java的java.util.Map定義了映射表。映射表是一種以索引和元素掛鉤的簡單資料結構。如果在映射表內以雜湊值代表某元素的索引，則這個映射表實質上是一個集。如果以一個遞增號碼來做索引，則實質上為一個列表。在映射表底下實作了**雜湊表**(java.util.HashMap)、**連結雜湊表**(java.util.LinkedHashMap)、和**樹狀表**(java.util.TreeMap)。雜湊表會儲存索引值的雜湊值來做為比對並提出該索引連接的元素之用。連結雜湊表進一步拓展了上述架構，它增加了一個[雙向連結串列來連結當中的元素](../Page/雙向連結串列.md "wikilink")，使其能保存元素加入時的順序。樹狀表使用紅黑樹來比對索引值。\[16\]

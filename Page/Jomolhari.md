是一款由Christopher
Fynn创造的[藏文](../Page/藏文.md "wikilink")[有头体电脑字体](https://zh.wikipedia.org/wiki/有头体 "wikilink")，以[SIL开源字体授权自由发布](../Page/SIL开源字体授权.md "wikilink")。支持使用[Unicode标准和中华人民共和国国家标准GB](https://zh.wikipedia.org/wiki/Unicode标准 "wikilink")/T20524-2006《信息交换用藏文编码字符集》的编码显示藏文。该字体的设计基于不丹人手写的范文，\[1\]并且可用于[藏语](https://zh.wikipedia.org/wiki/藏语 "wikilink")、[宗喀语和其他使用藏文的语言](https://zh.wikipedia.org/wiki/宗喀语 "wikilink")。

## 格式和授权

Jomolhari字体可以在[OpenType格式使用](../Page/OpenType.md "wikilink")[TrueType轮廓](../Page/TrueType.md "wikilink")。根据[SIL开源字体授权的条款发行](../Page/SIL开源字体授权.md "wikilink")。

该字体还可从[Savannah上的](https://zh.wikipedia.org/wiki/GNU_Savannah "wikilink")[Free
Tibetan Fonts
Project](http://savannah.nongnu.org/projects/free-tibetan)及其他站点下载。[VOLT计划的文件包含的OpenType方式查看的该字体的源代码也能从Savannah下载](https://zh.wikipedia.org/wiki/Visual_OpenType_Layout_Tool "wikilink")。

该字体还可以通过[deb和](https://zh.wikipedia.org/wiki/deb "wikilink")[rpm安装包用在Linux上](https://zh.wikipedia.org/wiki/rpm "wikilink")。

## 使用

  - Jomolhari内置于多个[Linux发行版](../Page/Linux发行版列表.md "wikilink")：
      - [Fedora](../Page/Fedora.md "wikilink")\[2\]
      - [Debian](../Page/Debian.md "wikilink")\[3\]
      - [CentOS](../Page/CentOS.md "wikilink")
      - [Ubuntu](../Page/Ubuntu.md "wikilink")
      - Dzongkha Linux

<!-- end list -->

  - Jomolhari被不丹报刊作为主要的字体用在其宗喀语版或章节的内容：
      - [不丹观察者](https://zh.wikipedia.org/wiki/不丹观察者 "wikilink")
      - [不丹时报](https://zh.wikipedia.org/wiki/不丹时报 "wikilink")
      - [今日不丹](https://zh.wikipedia.org/wiki/今日不丹 "wikilink")
      - [不丹商务](https://zh.wikipedia.org/wiki/不丹商务 "wikilink")
      - [Druk
        Yoedzer](https://zh.wikipedia.org/wiki/Druk_Yoedzer "wikilink")
  - Jomolhari被[MediaWiki用作显示藏文的默认字体](../Page/MediaWiki.md "wikilink")，\[4\]包括了维基百科。
  - Jomolhari还被[不丹国家图书馆网站用在宗喀语页面](https://zh.wikipedia.org/wiki/不丹国家图书馆 "wikilink")。\[5\]

## 派生作品

[西藏与喜马拉雅图书馆复刻了Jomolhari字体](https://zh.wikipedia.org/wiki/西藏与喜马拉雅图书馆 "wikilink")，为[Adobe
InDesign](../Page/Adobe_InDesign.md "wikilink") CS3及CS4创造出Jomolhari
ID字体。该字体添加了附加的lookup以及通用OpenType特性以使文字能在这些版本中正确显示。\[6\]\[7\]

[魏安也创造了该字体的复刻版BabelStone](../Page/魏安.md "wikilink")
Tibetan以提供扩展支持。该字体添加后来的Unicode版本中收录的藏文字符，添加了更多的字符组合，尤其是用于藏文速记缩写和转写西夏语的组合。\[8\]

## 参考资料

## 外部链接

  - [Jomolhari - A Free Tibetan Script
    Font](https://sites.google.com/site/chrisfynn2/home/fonts/jomolhari)
  - [Free Tibetan Fonts
    Project](http://savannah.nongnu.org/projects/free-tibetan)
  - [用于三星设备的Jomolhari
    FlipFont安装包](http://code.google.com/p/bho/downloads/detail?name=JomolhariFlipFont.apk)
  - [Digital Tibetan Wiki](http://digitaltibetan.org)上的[Tibetan
    Fonts](http://digitaltibetan.org/index.php/Tibetan_Fonts#Jomolhari)

[Category:藏语](https://zh.wikipedia.org/wiki/Category:藏语 "wikilink")
[Category:字体](https://zh.wikipedia.org/wiki/Category:字体 "wikilink")

1.
2.
3.
4.
5.
6.
7.
8.
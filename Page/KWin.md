在[Unix系統中](../Page/Unix.md "wikilink")， KWin是[X
Window系統中的一個](../Page/X_Window系統.md "wikilink")[窗口管理器](../Page/X窗口管理器.md "wikilink")。
構成[KDE Software
Compilation的一部分](../Page/KDE_Software_Compilation.md "wikilink")
，雖然它可以單獨使用或用於其他的桌面環境。

KWin可由基于[ECMAScript的](../Page/ECMAScript.md "wikilink")[脚本](../Page/脚本语言.md "wikilink")（如[QML](../Page/QML.md "wikilink")、[QtScript](../Page/QtScript.md "wikilink")）来进行配置。\[1\]

## 歷史

| 名稱   | 版本  | KDE 版本 | 細節                              |
| ---- | --- | ------ | ------------------------------- |
| KWM  | 1.0 | 1.0    |                                 |
| KWin | 2.0 | 2.0    | 擴展對主題和視窗效果的支援                   |
| KWin | 3.0 | 3.2    | 改進 ICCCM 延伸標準的支援                |
| KWin | 4.0 | 4.0    | 加入了組合模式                         |
| KWin | 4.4 | 4.4    | 碰到螢幕邊緣時最大化與平鋪、组合和分頁。\[2\]       |
| KWin | 4.5 | 4.5    | Tiling\[3\]（在4.10中移除）           |
| KWin | 4.9 | 4.9    | 變更部份的API\[4\] ，變更的部份可能會與之前的不相容。 |

## 外觀和感覺

## 組合模式

[Kwin-coverswitch4.1_beta_1.png](https://zh.wikipedia.org/wiki/File:Kwin-coverswitch4.1_beta_1.png "fig:Kwin-coverswitch4.1_beta_1.png")

### 輔助

| 名稱     | 描述            |
| ------ | ------------- |
| 反轉     | 反轉桌面和窗口顏色     |
| 放大鏡    | 放大游標附近屏幕      |
| 讓輪廓更分明 | 讓你的桌面看起來更輪廓分明 |
| 跟踪鼠標   | 顯示鼠標定位效果      |
| 縮放     | 放大整個桌面        |

### 外觀

| 名稱   | 描述               |
| ---- | ---------------- |
| 縮小動畫 | 最小化窗口的動畫         |
| 半透明  | 不同條件下讓視窗變半透明     |
| 神燈   | 窗口最小化時模擬神燈將精靈收進去 |
| 爆炸   | 使視窗關閉時讓視窗發生爆炸    |

## 參考文獻

## 外部連結

  - [KWin release notes for
    KDE4.0](http://techbase.kde.org/Projects/KWin/4.0-release-notes)
  - [KDE UserBase 上的 KWin](http://userbase.kde.org/KWin_\(zh_TW\))
  - [KDE UserBase 上的
    KWin](https://web.archive.org/web/20100819132835/http://userbase.kde.org/KWin_\(zh_CN\))
  - [給 KWin 4
    用的外框](https://web.archive.org/web/20100826191559/http://kde-look.org/index.php?xcontentmode=75)
  - [KWin Decorations
    HOWTO](http://www.usermode.org/docs/kwintheme.html)

[Category:KDE_Software_Compilation](https://zh.wikipedia.org/wiki/Category:KDE_Software_Compilation "wikilink")
[Category:X_视窗管理器](https://zh.wikipedia.org/wiki/Category:X_视窗管理器 "wikilink")

1.
2.
3.
4.
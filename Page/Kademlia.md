...
尽管异或标准对于理解**Kademlia**并不是必要，但是对于协议的分析却至关重要。[异或运算形成了允许闭合分析的循环群](https://zh.wikipedia.org/wiki/异或运算 "wikilink")，为了能够预见网络的行为和正确性，其他的一些[分散式雜湊表协议和算法都要求模拟或复杂的形式分析](../Page/分散式雜湊表.md "wikilink")，而**Kademlia**并不需要，另外，把位组作为路由信息也简化了**Kademlia**算法。

...
2.  利用[深度优先搜索](../Page/深度优先搜索.md "wikilink")求出\(G^R\)的逆后排序
...
当图是使用[邻接表](../Page/邻接表.md "wikilink")形式组建的，Kosaraju算法需要对整张图进行了两次的完整的访问，每次访问与顶点数\(V\)和边数\(E\)之和\(V+E\)成正比，所以可以在线性时间\(O(V+E)\)内访问完成。该算法在实际操作中要比[Tarjan算法](../Page/Tarjan算法.md "wikilink")和要慢，这两种算法都只需要对图进行一次完整的访问。

当图是使用[邻接矩阵](../Page/邻接矩阵.md "wikilink")形式组建的，算法的时间复杂度为\(O(V^2)\)。

## 参考

## 文献及链接

  - [Micha Sharir.A strong connectivity algorithm and its applications to data flow analysis. *Computers and Mathematics with Applications* 7(1):67–72, 1981](http://www.sciencedirect.com/science/article/pii/0898122181900080)\]

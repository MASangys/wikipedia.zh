[最优控制理論主要探討的是讓](https://zh.wikipedia.org/wiki/最优控制 "wikilink")[动力系统](../Page/动力系统.md "wikilink")以在最小成本來運作，若系統動態可以用一組[线性微分方程](../Page/线性微分方程.md "wikilink")表示，而其成本為[二次](../Page/二次函数.md "wikilink")[泛函](../Page/泛函.md "wikilink")，這類的問題稱為線性二次（LQ）問題。此類問題的解即為**線性二次調節器**（），簡稱**LQR**。

LQR是回授控制器，方程式在後面會提到。LQR是[LQG（線性二次高斯）問題解當中重要的一部份](https://zh.wikipedia.org/wiki/線性二次高斯控制 "wikilink")。而LQG問題和LQR問題都是[控制理论](../Page/控制理论.md "wikilink")中最基礎的問題之一。
...
LQR演算法減少了工程師為了讓控制器最佳化，而需付出的心力。不過工程師仍然要列出成本函數的相關參數，並且將結果和理想的設計目標比較。因此控制器的建構常會是[迭代](../Page/迭代.md "wikilink")的，工程師在[模擬過程中決定最佳控制器](https://zh.wikipedia.org/wiki/模擬 "wikilink")，再去調整參數讓結果更接近設計目標。

在本質上，LQR演算法是找尋合適[狀態回授控制器的](../Page/状态空间.md "wikilink")[自動化方式](https://zh.wikipedia.org/wiki/自動化 "wikilink")。因此也常會有[控制工程](../Page/控制工程.md "wikilink")師用其他替代方式，例如[全狀態回授](../Page/全狀態回授.md "wikilink")（也稱為極點安置）的作法，此作法對控制器參數和控制器性能之間的關係比較明確。而LQR演算法的困難之處在找合適的[加權因子](https://zh.wikipedia.org/wiki/加權 "wikilink")，這也限制了以LQR控制器合成的相關應用。
...
而\(P\)是連續時間[Riccati方程](../Page/Riccati方程.md "wikilink")的解：
...
從終端條件\(P_N = Q\)開始計算。注意\(u_N\)沒有定義，因為 \(x\) 是由\(A x_{N-1} + B u_{N-1}\)推導到其最終狀態 \(x_N\)。
...
## 外部連結
  ...
  - [MATLAB function for Linear Quadratic Regulator design](http://www.mathworks.com/help/toolbox/control/ref/lqr.html)
  - [Mathematica function for Linear Quadratic Regulator design](http://reference.wolfram.com/mathematica/ref/LQRegulatorGains.html)

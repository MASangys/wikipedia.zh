...
Containers）的縮寫，一種[作業系統層虛擬化](https://zh.wikipedia.org/wiki/作業系統層虛擬化 "wikilink")（Operating
system–level
...
在Linux內核中，提供了[cgroups功能](../Page/cgroups.md "wikilink")，來達成資源的區隔化。它同時也提供了名稱空間區隔化的功能，使應用程式看到的作業系統環境被區隔成獨立區間，包括行程樹，網路，使用者id，以及掛載的檔案系統。但是cgroups並不一定需要啟動任何虛擬機器。

LXC利用cgroups與名稱空間的功能，提供應用軟體一個獨立的作業系統環境。LXC不需要[Hypervisor這個軟體層](https://zh.wikipedia.org/wiki/Hypervisor "wikilink")，軟體容器（Container）本身極為輕量化，提升了建立[虛擬機器的速度](../Page/虛擬機器.md "wikilink")。軟體[Docker被用來管理LXC的環境](https://zh.wikipedia.org/wiki/Docker_\(軟體\) "wikilink")。
...
目前的LXC使用下列内核功能来控制进程：
  ...
  - [AppArmor和](https://zh.wikipedia.org/wiki/AppArmor "wikilink")[SELinux配置](https://zh.wikipedia.org/wiki/SELinux "wikilink")
  - Seccomp策略
  - [chroot](../Page/chroot.md "wikilink")（使用pivot_root）
  - [Kernel
    Capibilities](https://zh.wikipedia.org/wiki/Kernel_Capibilities "wikilink")
  - [控制组](https://zh.wikipedia.org/wiki/控制组 "wikilink")（[cgroups](../Page/cgroups.md "wikilink")）
...
## 使用
  ...
  - **[Proxmox
    VE](https://zh.wikipedia.org/wiki/Proxmox_VE "wikilink")**:
    它直到4.0版才使用LXC技術，在此之前的版本都是使用OpenVZ技術。

<!-- end list -->

  - **[Docker](https://zh.wikipedia.org/wiki/Docker "wikilink")**：它在0.9版之前都是使用LXC技術，但在0.9版之後，已不再是唯一且預設的執行環境。

...
自初始狀態開始迭代套入L-system的文法規則，和[正規文法所產生的語言不同處在於](https://zh.wikipedia.org/wiki/正規文法 "wikilink")，L-system在一次迭代中可同時套用許多不同的文法規則。如果在一次迭代中只能夠套用一個文法規則，產生出來的結果被稱為語言而不是L-system。由此可知，L-system為[正規文法所產生出的語言的子集合](https://zh.wikipedia.org/wiki/正規文法 "wikilink")。

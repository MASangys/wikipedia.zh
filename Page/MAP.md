**[地图](../Page/地图.md "wikilink")**（Map）為將某區域以視覺效果呈現的圖樣。

**Map**或**MAP**可能是以下事物：

## 電腦

  - [移动应用部分](../Page/移动应用部分.md "wikilink")（Mobile Application Part）,
    一种手机网络协议

  - Multiple Address and Port, a feature of  products

  - , a set of computer network communication protocols

  -
  - [藍牙規範](../Page/藍牙規範.md "wikilink")（Message Access Profile）：a
    Bluetooth profile for exchange of messages between devices

  - , an IPv6 transition technology

## 數學及程式

  - [映射](../Page/映射.md "wikilink")（Map）

  - , used to apply a function to a list of values and return another
    list with the results

  - [关联数组](../Page/关联数组.md "wikilink")（Map）：or associative array, a data
    type composed of a collection of key/value pairs

      - , an implementation in the C++ language

## 醫學或生物學

  - , a member of proteins that interact with the microtubules of the
    cellular cytoskeleton

  - [平均動脈壓](../Page/平均動脈壓.md "wikilink")（Mean Arterial Pressure）：the
    driving force of blood flow

  - , a mediator of intracellular signaling

  - **, a strain of pathogenic bacteria

## 組織

  - , the official Moroccan news agency

  - , a UK charity

  - , a network to promote peace in the Middle East

  - , a Peruvian art museum

  - , a political party in the Gold Coast (now Ghana)

  - , a motorists' safety training program

  - , a communications policy law firm and advocacy organization

  - , a Puerto Rican art museum

## 科學及科技

  - [疟疾地图计划](../Page/疟疾地图计划.md "wikilink")（Malaria Atlas Project）

  - , in an internal combustion engine

  - , in queueing theory

  - , in applied probability

  - [最大后验概率](../Page/最大后验概率.md "wikilink")（Maximum a posteriori
    estimation）：統計學術語。

  - [信息檢索](../Page/信息檢索.md "wikilink")（Mean average precision）

  - , an enzyme

  - [威尔金森微波各向异性探测器](../Page/威尔金森微波各向异性探测器.md "wikilink")（Microwave
    Anisotropy Probe）：an unmanned space mission

  - , in aviation

  -
  - **, a bacterium associated with Crohn's disease

  - , commonly referred to simply as "map", mental representations of
    physical locations

## 大眾文化

  - 《[愛探險的朵拉](../Page/愛探險的朵拉.md "wikilink")》中的其中一集。

  - ，一個位於[加州的獨立樂團](../Page/加州.md "wikilink")。

## 其他用法

  -
  - [關卡](../Page/關卡_\(電子遊戲\).md "wikilink")（電子遊戲）

  -
  - [南島語系的](../Page/南島語系.md "wikilink")[ISO
    639-2語言編碼](../Page/ISO_639-2.md "wikilink")。

  - [北約擴張](../Page/北約擴張.md "wikilink")（Membership action plan）：a stage
    for countries that are on their way to joining NATO

  - [Ministry of Aviation
    Industry](../Page/Ministry_of_Aviation_Industry.md "wikilink"), a
    former

  -
  -
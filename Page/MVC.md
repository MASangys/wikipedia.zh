...
MVC模式最早由[Trygve
Reenskaug在](https://zh.wikipedia.org/wiki/Trygve_Reenskaug "wikilink")1978年提出\[1\]，是[施乐帕罗奥多研究中心](https://zh.wikipedia.org/wiki/帕罗奥多研究中心 "wikilink")（Xerox
...
将应用程序划分为三种组件，模型 - 视图 - 控制器（MVC）设计定义它们之间的相互作用。\[2\]
  ...
  - **模型（Model）** 用于封装与应用程序的业务逻辑相关的数据以及对数据的处理方法。“ Model
    ...
    上发生的改变。（比如：[观察者模式](../Page/观察者模式.md "wikilink")（[软件设计模式](https://zh.wikipedia.org/wiki/软件设计模式 "wikilink")））
...
。如今，同一个Web应用程序会提供多种用户界面，例如用户希望既能够通过浏览器来收发[电子邮件](../Page/电子邮件.md "wikilink")，还希望通过手机来访问[电子邮箱](https://zh.wikipedia.org/wiki/电子邮箱 "wikilink")，这就要求Web网站同时能提供[Internet界面和](https://zh.wikipedia.org/wiki/Internet "wikilink")[WAP界面](https://zh.wikipedia.org/wiki/WAP "wikilink")。在MVC设计模式中，
...
保持相对独立，所以可以方便的改变应用程序的数据层和业务规则。例如，把数据库从[MySQL移植到](../Page/MySQL.md "wikilink")[Oracle](https://zh.wikipedia.org/wiki/Oracle "wikilink")，或者把[RDBMS数据源改变成](https://zh.wikipedia.org/wiki/RDBMS "wikilink")[LDAP数据源](https://zh.wikipedia.org/wiki/LDAP "wikilink")，只需改变
Model
即可。一旦正确地实现了控制器，不管数据来自数据库还是[LDAP服务器](https://zh.wikipedia.org/wiki/LDAP "wikilink")，View
...
Patterns](https://zh.wikipedia.org/wiki/Design_Patterns "wikilink"))
都是有助於應用程式長遠的發展，雖然大雜燴式的程式也可以用來發展長生命週期的應用程式，但是相較於
MVC，大雜燴式的程式在可擴充性和可維護性 (尤其是可測試性) 上會遠比 MVC
複雜很多，相反的，MVC
...
[微软所推出的](../Page/微软.md "wikilink")[MFC](https://zh.wikipedia.org/wiki/MFC "wikilink")
Document/View架构是早期对于MVC模式的实现，[MFC將程式分成CView以及CDocument兩大類別](https://zh.wikipedia.org/wiki/MFC "wikilink")，其中的Document对应MVC中的
Model ，View 相当于MVC中的
View＋Controller，再加上CWinApp類別，合成三大項。但是基本上[MFC是一個失敗的MVC模式作品](https://zh.wikipedia.org/wiki/MFC "wikilink")。

由於[MFC之下的Document](https://zh.wikipedia.org/wiki/MFC "wikilink")/View
定義過於模糊，未將Controller（MessageMap）部份取出，因此 Controller 可以置入 View
或Document，但不管置入哪一方面，都會與View或Document綁死，沒有彈性。
...
和其他的各种框架不一样，J2EE为模型对象（Model Objects）定义了一个规范。
  ...
  - 视图(View)
    ...
    的代码则可能是一个[servlet的一部分](https://zh.wikipedia.org/wiki/servlet "wikilink")，特别是在客户端服务端交互的时候。

<!-- end list -->

  - 控制器（Controller）
    [J2EE应用中](https://zh.wikipedia.org/wiki/J2EE "wikilink")，Controller
    可能是一个[servlet](https://zh.wikipedia.org/wiki/servlet "wikilink")。
    除了可直接以J2EE來撰寫外，亦可用其他框架來撰寫，常見的有[Struts2](https://zh.wikipedia.org/wiki/Struts2 "wikilink")、[Spring
...
<!-- end list -->
  ...
  - 模型（Model）
    Model
    则是由一个[实体Bean来实现](https://zh.wikipedia.org/wiki/JavaBean "wikilink")。
...
SQL類別](https://zh.wikipedia.org/wiki/語言集成查詢 "wikilink")（使用O/R
Designer工具所製作而成的DBML檔）或ADO.NET實體資料模型（Entity Data Model，使用[ADO.NET Entity
Framework製作出的EDMX檔](https://zh.wikipedia.org/wiki/ADO.NET_Entity_Framework "wikilink")）來實作。
...
[Catalyst和](../Page/Catalyst.md "wikilink")[Jifty是透過](https://zh.wikipedia.org/wiki/Jifty "wikilink")[Perl語言所開發出來的Web](../Page/Perl.md "wikilink")

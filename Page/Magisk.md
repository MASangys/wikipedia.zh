...
Manager（图形化管理界面）、[Root](../Page/Root_\(Android\).md "wikilink")、[启动脚本](../Page/Initrd.md "wikilink")、[SElinux补丁和](https://zh.wikipedia.org/wiki/SELinux "wikilink")[启动时认证](../Page/Android软件开发.md "wikilink")/dm-verity/强制加密移除功能。Magisk同时提供了在无需修改系统文件的情况下更改/system或/vendor分区内容的接口，利用其与[Xposed类似的模块系统](../Page/Xposed框架.md "wikilink")，开发者可以利用其对系统进行美化（如更改字体）或修改软件功能（如去广告）等。

...
[MFoundation.svg](https://zh.wikipedia.org/wiki/File:MFoundation.svg "fig:MFoundation.svg") Media Foundation（以下簡稱MF）架構分為控制層（Control layer），核心層（Core layer）與平台層（Platform layer）。大部分MF功能均由核心層提供，開發者則由控制層來控制行為。一般開發者僅須了解控制層與核心層，幾乎不須接觸平台層。

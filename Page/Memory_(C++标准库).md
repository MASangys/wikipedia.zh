...
[智能指针](../Page/智能指针.md "wikilink")是指当指针对象的生命期结束析构时，同时把指针所指的对象也delete或者[引用计数](../Page/引用计数.md "wikilink")减1。
...
enable_shared_from_this是一个helper类模板，用于作为数据类的基类。当数据类的对象已经被shared_ptr所拥有，如果需要从该数据类对象获得其shared_ptr指针，就需要调用作为基类的enable_shared_from_this的成员函数`shared_from_this`。例如：

...
相对于Windows内核的封闭，Linux内核源代码的开放让漏洞很容易被发现。而且Linux进程一旦获得Root权限，就可以无限制的访问资源，相对而言有风险。

## 學習和使用

| 比較 | Windows                                                      | Linux                                                             |
| -- | ------------------------------------------------------------ | ----------------------------------------------------------------- |
| 界面 | 界面統一，外殼程式固定,所有Windows程式選單幾乎一致，快捷鍵也幾乎相同                       | 圖形界面風格依發行版不同而不同，可能互不相容。GNU/Linux的終端機是從UNIX傳承下來，基本命令和操作方法也幾乎一致。 |- |
| 使用 | 使用比較簡單，容易入門。圖形化界面對沒有電腦背景知識的用戶使用十分有利。                         | 圖形界面使用簡單，容易入門。命令行界面，需要学习才能掌握。 |-                                  |
| 軟體 | 每一種特定功能可能都需要[商業軟體的支持](../Page/专有软件.md "wikilink")，需要購買相應的授權。 | 大部分軟體都可以[自由獲取](../Page/开源软件.md "wikilink")，同樣功能的軟體選擇较少。           |

帶有研究學習性質的個人和團體，因為授權協議的不同，Windows的學習和引用的門檻更高，但是现有资源支援更为广泛；Linux则是基于网络，轻易获取最新资源。

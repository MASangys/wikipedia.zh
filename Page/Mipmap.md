在三维计算机图形的贴图渲染中有一个常用的技术被称为**Mipmapping**。为了加快渲染速度和减少图像锯齿，贴图被处理成由一系列被预先计算和优化过的图片组成的文件,这样的贴图被称为 MIP map 或者 mipmap。这个技术在三维游戏中被非常广泛的使用。“MIP”来自于拉丁语 *multum in parvo* 的首字母，意思是“放置很多东西的小空间”。Mipmap 需要占用一定的内存空间，同时也遵循[小波压缩规则](https://zh.wikipedia.org/wiki/小波压缩规则 "wikilink") （wavelet compression）。
...
[Category:计算机图形数据结构](https://zh.wikipedia.org/wiki/Category:计算机图形数据结构 "wikilink") [Category:贴图过滤](https://zh.wikipedia.org/wiki/Category:贴图过滤 "wikilink")

...
如果一个项目中存在`package.json`文件，那么用户可以直接使用`npm
install`命令自动安装和维护当前项目所需的所有模块\[12\]。在`package.json`文件中，开发者可以指定每个依赖项的[版本范围](../Page/軟件版本號.md "wikilink")，这样既可以保证模块自动更新，又不会因为所需模块功能大幅变化导致项目出现问题\[13\]。开发者也可以选择将模块固定在某个版本之上\[14\]。

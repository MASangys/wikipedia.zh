**OPPO Find
7**是一款由[OPPO公司研製的](../Page/OPPO.md "wikilink")[Android](../Page/Android.md "wikilink")[智慧型手機](https://zh.wikipedia.org/wiki/智慧型手機 "wikilink")，為第一支採用了2K解析度螢幕的智慧型手機。

在中國，共有標準版(內部代號為**X9077**)、輕裝版(內部代號為**X9007**)。在海外，共有兩種版本，分別是 OPPO Find
7(內部代號為**X9076**、OPPO Find 7a(內部代號為**X9006**)。

## 規格

OPPO Find 7 共計有兩種版本，分別為標準版(Find 7)、輕裝版(Find 7a)

|       | Find 7                              | Find 7a                         |
| ----- | ----------------------------------- | ------------------------------- |
| 螢幕尺寸  | 5.5 吋 1440 x 2560 pixel             | 5.5 吋 1080 x 1920 pixel         |
| 螢幕類型  | IPS TFT                             | IPS TFT                         |
| 尺寸    | 152.6 × 75 × 9.2 (mm)               | 152.6 × 75 × 9.2 (mm)           |
| 重量    | 171 (g)                             | 170 (g)                         |
| 標準電池  | 3000 mAh (可換式)                      | 2800 mAh (可換式)                  |
| 指紋辨識  | N/A                                 | N/A                             |
| 顏色    | 黑/白                                 | 黑/白                             |
| 網路系統  | GSM 四頻 / WCDMA 850 + 900 + 2100     | GSM 四頻 / WCDMA 850 + 900 + 2100 |
| EDGE  | 有                                   | 有                               |
| HSDPA | 有                                   | 有                               |
| LTE   | LTE 1800 + FDD 2600                 | LTE 1800 + FDD 2600             |
| 雙卡雙待  | N/A                                 | N/A                             |
| Wi-Fi | 802.11ac                            | 802.11ac                        |
| 藍牙    | v4.0                                | v4.0                            |
| GPS   | 有                                   | 有                               |
| USB   | 3.0                                 | 2.0                             |
| NFC   | 有                                   | 有                               |
| 作業系統  | Android 4.3                         | Android 4.3                     |
| CPU   | Qualcomm S801 2.5GHz 四核 (MSM8974AC) | Qualcomm S801 2.3GHz 四核         |
| RAM   | 3GB                                 | 2GB                             |
| ROM   | 32GB                                | 16GB                            |
| 記憶卡   | microSD (最大128GB)                   | microSD (最大128GB)               |
| 主相機   | 1300 萬畫素 CMOS                       | 1300 萬畫素 CMOS                   |
| 次相機   | 500 萬畫素 CMOS                        | 500 萬畫素 CMOS                    |
| 補光燈   | 有                                   | 有                               |
| 錄影解析度 | 4096x2160 /30fps                    | 1920x1080 /30fps                |

## 参考文献

[OPPO Find 7
標準版](https://web.archive.org/web/20170616143825/http://www.opposhop.cn/products/106#tab-params)
[OPPO Find 7
輕裝版](https://web.archive.org/web/20170101200914/http://www.opposhop.cn/products/104.html)

## 参见

  - [Android](../Page/Android.md "wikilink")

[Category:Android設備](https://zh.wikipedia.org/wiki/Category:Android設備 "wikilink")
[Category:OPPO](https://zh.wikipedia.org/wiki/Category:OPPO "wikilink")
[Category:觸控手機](https://zh.wikipedia.org/wiki/Category:觸控手機 "wikilink")
...
<!-- end list -->
  ...
  - **格式规格** —
    保存和交换数据是本地化过程中重要的一环。对众多格式尽可能使用开放的标准可以增加互操作性。在可行时，Okapi框架则利用现有的标准，例如[XLIFF](https://zh.wikipedia.org/wiki/XLIFF "wikilink")、[SRX](https://zh.wikipedia.org/wiki/SRX_Segmentation_Rules_eXchage_LISA_OSCAR_XML_based_Standard "wikilink")、[TMX等](https://zh.wikipedia.org/wiki/Translation_Memory_eXchange "wikilink")。
...
有两种主要类型的组件：
  ...
  - **过滤器** — 实现了多种过滤器组件，包括用于：HTML, OpenOffice.org、Microsoft
    ...
    PO文件](../Page/Gettext.md "wikilink")、[XLIFF](https://zh.wikipedia.org/wiki/XLIFF "wikilink")、[TMX](https://zh.wikipedia.org/wiki/Translation_Memory_eXchange "wikilink")、[Qt

**OmniROM于2013年起创立，基于[AOSP](https://zh.wikipedia.org/wiki/AOSP "wikilink")**\[1\]打造，其开发团队不仅包含离开CyanogenMod的开发人员，亦有数名来自其它项目的杰出开发者共同打造\[2\]。

## 项目内容

Om基于niROM旨在：

  - 开创由社区引领的开发过程（用户决定了开发的方向）
  - 打造一流的Nightly（术语，意为每日/夜更新）支持，从而迅速解决Bug报告
  - 着重于开放源代码
      - 补丁将会预设遵循GPL协议
      - 将聚合来自其它ROM的一切功能
      - 为不同项目作出优化
      - 无Android专用的代码（所以QT应用也得以实现）

<!-- end list -->

  -
## 源起

该项目的产生是对CyanogenMod项目之商业化的直接反应。\[3\]

## 支持设备

OmniRom支持以下机型（仅列出更新中的机型，逢星期日更新）

小米：[小米手機5](../Page/小米手機5.md "wikilink")、[小米手機6](../Page/小米手機6.md "wikilink")、小米MIX2、[紅米Note
4](../Page/紅米Note_4.md "wikilink")

一加：[一加手机5](../Page/一加手机5.md "wikilink")、[一加手机5T](../Page/一加手机5T.md "wikilink")、[一加手机6](../Page/一加手机6.md "wikilink")

索尼：[Sony Xperia XZ](../Page/Sony_Xperia_XZ.md "wikilink")、[Sony Xperia
XZs](../Page/Sony_Xperia_XZs.md "wikilink")

摩托罗拉：Moto G5 Plus、Moto G4 Plus

## 延伸阅读

  - [CyanogenMod](../Page/CyanogenMod.md "wikilink")
  - [AOKP](../Page/AOKP.md "wikilink")
  - [AOSP](https://zh.wikipedia.org/wiki/AOSP "wikilink")

## 参考资料

## 外部链接

  - [OmniROM官方网站](http://www.omnirom.org)
  - [OmniROM下载](http://dl.omnirom.org/)

[Category:定制Android固件](https://zh.wikipedia.org/wiki/Category:定制Android固件 "wikilink")

1.
2.
3.
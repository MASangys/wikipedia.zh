**OpenIndiana**项目的目标是继续开发和发行[OpenSolaris](../Page/OpenSolaris.md "wikilink")。\[1\]该项目隶属于[Illumos基金会](../Page/Illumos.md "wikilink")。\[2\]由于[Oracle宣布不再发行OpenSolaris](https://zh.wikipedia.org/wiki/Oracle "wikilink")，该项目被建立以为OpenSolaris继续其更新。\[3\]

## 参见

  - [Solaris](../Page/Solaris.md "wikilink")
  - [OpenSolaris](../Page/OpenSolaris.md "wikilink")
  - [Oracle](https://zh.wikipedia.org/wiki/Oracle "wikilink")

## 参考

[Category:类Unix系统](https://zh.wikipedia.org/wiki/Category:类Unix系统 "wikilink")
[Category:OpenSolaris](https://zh.wikipedia.org/wiki/Category:OpenSolaris "wikilink")

1.

2.
3.  [Frequently Asked
    Questions](http://wiki.openindiana.org:8080/display/oi/Frequently+Asked+Questions)
     OpenIndiana Wiki（英文）
**Openswan**，一個自由軟體專案，提供[IPsec](../Page/IPsec.md "wikilink")功能，可在[Linux](../Page/Linux.md "wikilink")與[FreeBSD](../Page/FreeBSD.md "wikilink")作業系統上運作，採用[GPL釋出](https://zh.wikipedia.org/wiki/GPL "wikilink")。可支援[Linux核心](https://zh.wikipedia.org/wiki/Linux核心 "wikilink")
2.0，2.2，2.4 與 2.6版本。

## 歷史

自[FreeS/WAN專案的](https://zh.wikipedia.org/wiki/FreeS/WAN "wikilink")2.04版[分支出來](https://zh.wikipedia.org/wiki/复刻_\(软件工程\) "wikilink")。2012年，自Openswan專案中，又分支出[Libreswan](https://zh.wikipedia.org/wiki/Libreswan "wikilink")。

## 註釋

## 外部連結

  - [Openswan website](http://www.openswan.org/)
  - [Old Openswan
    wiki](https://web.archive.org/web/20120204201016/http://wiki.openswan.org/)
  - [Openswan bugtracker and new
    wiki](https://github.com/xelerance/Openswan/issues)

[Category:虛擬私人網路](https://zh.wikipedia.org/wiki/Category:虛擬私人網路 "wikilink")
[Category:Linux軟件](https://zh.wikipedia.org/wiki/Category:Linux軟件 "wikilink")
[Category:自由安全软件](https://zh.wikipedia.org/wiki/Category:自由安全软件 "wikilink")
[Category:加密软件](https://zh.wikipedia.org/wiki/Category:加密软件 "wikilink")
[Category:网络安全](https://zh.wikipedia.org/wiki/Category:网络安全 "wikilink")
...
用[游標處理譜面上的不同物件](https://zh.wikipedia.org/wiki/游標 "wikilink")，其中主要的物件為以下三者。
...
外觀：轉盤出現時會出現一個占用整個遊戲視窗的大圓圈、漸近環，以及兩旁垂直的計量條（Spinner Metre，不預裝在新版遊戲程式）。
...
(旋轉轉盤最高轉速為477RPM。)旋轉轉盤是在同遊戲修改器與準確度、連擊數中，最可以改變分數及排名的物件。
...
用鍵盤上設定的四個按鍵作為鼓棒敲擊鼓面的四個動作，並且在適當的時機敲擊以獲得分數。從系統預設的鍵盤按鍵位置（ＺＸＣＶ）可以發現，排列的方式也一定程度的模仿了敲鼓的位置。

##### 「咚（Don）」以及「咔（Katsu）」

外觀：「咚」預設的圖示為紅色的圓圈，「咔」則是藍色，敲擊完成後會發出如同其名的聲音。

遊玩：每一個圓圈所代表的動作都是請玩家敲鼓，「咚」是代表敲擊鼓面（預設為ＸＣ），「咔」是代表敲擊鼓邊（預設為ＺＶ），如果出現大型的圓圈則要同時敲旗鼓面或是鼓邊。
...
其實就相當於[太鼓之達人的氣球連打](https://zh.wikipedia.org/wiki/太鼓之達人 "wikilink")，在《**osu\!**》中替換成轉盤

外觀：是一個和osu\!中的[旋轉轉盤](https://zh.wikipedia.org/wiki/旋轉 "wikilink")（Spinner）相同的縮小版

遊玩：雖然外觀和osu的旋轉轉盤相似，但並不是要玩家用游標繞著圓心轉，而是將指定的敲擊次數盡快打完，和「連打」不同的是，「咚」和「咔」必須要交互敲擊。

### CatchTheBeat

简称CtB模式，在中文玩家群中也被称作接水果模式。屏幕顶端會掉下許多水果，玩家需要操作角色接住從天而降的水果。通過鍵盤的左右方向鍵或者鼠標可移動角色，按住右鍵或Shift可加速移動。當兩個水果跨度過大時系統會自動加速且背景會有紅色的閃光效果。

### osu\!mania

由osu\!玩家[Tom94](https://osu.ppy.sh/u/1857058)研发，借鉴《[O2Jam](../Page/劲乐团.md "wikilink")》和《[DJMax](../Page/DJMax.md "wikilink")》的游戏模式，于2012年10月中旬上线。此模式游玩时会有音符或滑条从屏幕上方不同位置落下，玩家需要在特定时间按下或按住对应键位。
...
osu\!stream的遊玩方式中，增加了新的圈圈類型和模式。\[7\]

圆圈类型：
**長按**：一个白色边框的紫色圆圈，上面带有“HOLD”标记。玩家需要按节奏按住该圆圈，直到圆圈周围的半透明扇形计时器走满一圈。
...
| --------------------------------------- | --- | ------ | -------- | ---- |
| [Cookiezi](https://osu.ppy.sh/u/124493) | 652 | 99.86% | 10677813 | 583  |
| [rrtyui](https://osu.ppy.sh/u/352328)   | 651 | 98.33% | 10520461 | 516  |
| [hvick225](https://osu.ppy.sh/u/50265)  | 649 | 98.47% | 10509037 | 520  |
...
另外也有圖譜的說法。《**osu\!**》的根本就是譜面來進行，每張譜面都是由一首曲目開始的，也因此選擇了哪一首音樂就相當於選擇了怎麼樣的節奏。

《**osu\!**》的創立中強調過互動性，其中絕大多數的譜面都是由玩家、愛好者製作分享給其他人遊玩。

如果一張譜面通過了官方的審核，那麼該譜面就可以上傳到官方網站，讓其他玩家可以透過官方網站下載，此外該譜面也會列入表現排名中，列表會顯示前五十名，而前四十名的玩家的遊玩過程可以在線上重播。

### 譜面狀態

分為**Ranked**、**Approved**、**Qualified**、**Pending**、**Graveyard**和**Loved**。

#### Ranked

作者將譜面上傳，且通過了官方的審核後，該譜面就能被稱作Ranked。這樣的譜面通常完成度高而且節奏與音樂非常接近，玩家可以遊玩該譜面的同時獲得與音樂上的互動。
成功通過一張Ranked的譜面後，玩家的遊玩成績將會被上傳至網路上，並且計算出表現分數，同時也會提升帳號的經驗值/等級。

#### Approved

與Ranked相像，但大多譜面難度比較高，較適合進階玩家遊玩。因為一些因素如：長度太長（擔心入門者覺得冗長而無聊），会使某张图变为**Approved**状态，但因為精緻或完成度高而有自己的歌曲獨立排行榜（與Ranked同樣），通過後也會影響達成率、等級和成就。

#### Qualified

準備好進入Ranked的譜面可被提名進入Qualified並維持此狀態一週，在這一週期間譜面基本上與Ranked相同，唯一的差別為若是於此期間譜面被發現有問題將會降回Pending狀態，成績記录亦會被消除。

#### Pending

代表製作者上傳後還在審核的未完成譜面，玩家依然可以下載遊玩，只是無法獲得排行和經驗值，同時也可能沒辦法藉遊玩此類譜面獲得與音樂上的互動。這類譜面審核通過後會成為Ranked譜面或Approved譜面，但製作者無後續更新後會成為Graveyard譜面，將不再允許修改和上傳，之後也會被官方刪除。

#### Graveyard

超过4周没有更新的谱面自动挪入该类别，放置过久的Graveyard谱面将会被删除。原譜面作者可以自行将其恢复为Pending状态。
...
來源大多為玩家[捐贈資助](http://osu.ppy.sh/p/support)，另外一部分來自[osu\!store](http://osu.ppy.sh/store/listing)。

#### 贊助者（Supporter）

捐贈一定金額後對應一定的期限，並獲得一些福利，例如聊天視窗中，贊助者的名字會顯示為亮黃色，也能得到更改使用者名稱（僅免費第一次），並可以在遊戲程式中使用osu\!direct下載譜面（非贊助會員不能使用）等。

#### osu\!store

販售周邊商品如衣服、馬克杯、osu\!tablet等，另外有販售更改使用者名稱或是個人資料檔旗幟（舉辦比賽期間）等虛擬商品。

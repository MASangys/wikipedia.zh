...
算法使用两个控制变量*flag*与*turn*. 其中*flag\[n\]*的值为真，表示ID号为n的进程希望进入该[临界区](https://zh.wikipedia.org/wiki/临界区 "wikilink"). 变量*turn*保存有权访问共享资源的进程的ID号.
...
有限等待(Bounded waiting)意味着一个进程在提出进入临界区请求后，只需要等待临界区被使用有上限的次数后，该进程就可以进入临界区。\[5\]即进程不论其优先级多低，不应该饿死(starvation)在该临界区入口处。Peterson算法显然让进程等待不超过1次的临界区使用，即可获得权限进入临界区。

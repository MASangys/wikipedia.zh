**Poedit**（舊稱**poEdit**）是一個[免費增值软件及](https://zh.wikipedia.org/wiki/Freemium "wikilink")[跨平台的](https://zh.wikipedia.org/wiki/跨平台 "wikilink")[gettext類](../Page/gettext.md "wikilink")（.po
格式，應用程式裡採用了gettext來作程式的[國際化](https://zh.wikipedia.org/wiki/國際化 "wikilink")）國際化翻譯編輯器，也是同類型軟件中最廣泛使用的一個。現時它不論在
Unix+GTK 或 Windows平台 配合 [wxWidgets](../Page/wxWidgets.md "wikilink")
均有相關版本。

## 參看

  - [wxWidgets](../Page/wxWidgets.md "wikilink")
  - [Gettext](../Page/Gettext.md "wikilink")
  - [Translate Toolkit](../Page/Translate_Toolkit.md "wikilink")
  - [国际化与本地化](../Page/国际化与本地化.md "wikilink") -
    [中文化](../Page/中文化.md "wikilink")
  - 其他類似軟體：[Virtaal](https://zh.wikipedia.org/wiki/Virtaal "wikilink")

## 外部連結

  - [Poedit](http://www.poedit.net)

  -
  - [Using poEdit to make
    translations](https://web.archive.org/web/20100504182946/http://wiki.knowledgetree.com/Using_poEdit_to_make_translations)

  - [Translation with POEdit – Internationalize / Localize WP Themes
    Guide –
    Part 3](http://gettingeek.com/translation-with-poedit-internationalize-localize-wp-themes-guide-part-3-74.html)

  - [poedit](https://help.ubuntu.com/community/poedit) at [Ubuntu
    Community Documentation](https://help.ubuntu.com/community/)

  - [Issues using poEdit](http://drupal.org/node/17564) (regarding
    plural forms)

  - [Poedit website](http://www.poedit.net)

  -
  - [Online PO file editor](http://localise.biz/free/poedit) A
    browser-based Poedit clone

[Category:共享軟體](https://zh.wikipedia.org/wiki/Category:共享軟體 "wikilink")
[Category:軟體在地化工具](https://zh.wikipedia.org/wiki/Category:軟體在地化工具 "wikilink")
[Category:使用MIT许可证的软件](https://zh.wikipedia.org/wiki/Category:使用MIT许可证的软件 "wikilink")
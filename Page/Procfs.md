在许多类 [Unix](../Page/Unix.md "wikilink") 计算机系统中， **procfs** 是
[进程](../Page/进程.md "wikilink") [文件系统](../Page/文件系统.md "wikilink")
(file system)
...
[Tom J. Killian](../Page/Tom_J._Killian.md "wikilink") 实现了 [UNIX 8th
Edition](../Page/Version_8_Unix.md "wikilink") 版本的 `/proc`：他1984年6月向
[USENIX](../Page/USENIX.md "wikilink") 提交了一份题为 "Processes as Files"
的论文。procfs 的设计目标是用来替代进行进程跟踪的
*[ptrace](../Page/ptrace.md "wikilink")* 系统调用。 详细文档请参考 [proc(4) manual
page](http://man.cat-v.org/unix_8th/4/proc)。

### SVR4

[Roger Faulkner](../Page/Roger_Faulkner.md "wikilink") 和 [Ron
Gomes](../Page/Ron_Gomes.md "wikilink") 将 V8 的 `/proc` 移植到了
[SVR4](../Page/System_V_Release_4.md "wikilink")，并在1991年1月的
[USENIX](../Page/USENIX.md "wikilink") 上发表了一篇题为 "The Process File System
and Process Model in UNIX System V" 的论文。此类 procfs 支持
[`ps`](../Page/ps_\(Unix\).md "wikilink") 的创建，但是文件只能由如下函数访问： `read()`,
`write()`, [`ioctl`](../Page/ioctl.md "wikilink")`()`.
...
[4.4BSD](../Page/4.4BSD.md "wikilink") 基于九號計畫 克隆得到了其 `/proc` 系统。注意，在
FreeBSD 中 procfs 已被放弃\[1\]。默认安装的 base 系统不依赖 procfs，FreeBSD 原生 ports
也不再依赖 procfs。
...
每个正在运行的进程对应于`/proc`下的一个目录，目录名就是进程的PID，每个目录包含:
  ...
  - ,
    当前[工作目录的](../Page/工作目录.md "wikilink")[符号链接](../Page/符号链接.md "wikilink").

  - 影响进程的环境变量的名字和值.

  - , 最初的[可执行文件的符号链接](../Page/可执行文件.md "wikilink"), 如果它还存在的话。
  ...
  - ,
    该进程所能看到的根路径的符号链接。如果没有[chroot监狱](../Page/chroot.md "wikilink")，那么进程的根路径是/.

  - 包含了进程的基本信息，包括运行状态、内存使用。

  - , 一个目录包含了到该进程启动的任何任务

(用户可以获得[PID使用工具如](../Page/进程ID.md "wikilink")[pgrep](../Page/pgrep.md "wikilink"),
[pidof或](../Page/pidof.md "wikilink")[ps](../Page/ps_\(Unix\).md "wikilink"):
...
[Linux](../Page/Linux_kernel.md "wikilink") 2.6把 `/proc`
下大量的非進程相關的系统信息移動到一個专门的偽文件系統，稱為
[sysfs](../Page/sysfs.md "wikilink")（該文件系統是掛載到 `/sys` 上面）:
  ...
  - , 包含对应于计算机上各种总线的目录,
    如input/[PCI](../Page/PCI.md "wikilink")/[USB](../Page/USB.md "wikilink").
    在/sys/bus下包含更丰富的信息。
  ...
  - , 包含[CPU信息](../Page/CPU.md "wikilink"), 诸如厂商（vendor），型号 (family,
    model，model names), 速度, 缓存大小, 逻辑核数 , 物理核数, [CPU
    flags](../Page/CPU_flags.md "wikilink")，以及[BogoMips](../Page/BogoMips.md "wikilink").对于多核CPU，/proc/cpuinfo的逻辑核数"siblings"与物理核数"cpu
    cores"分别表示:\[3\]
...
CPU
package是指单独封装的一颗CPU。这可以区分[超线程与双核](../Page/超线程.md "wikilink")，例如每颗CPU超线程数量为*siblings
/ CPU cores*. 如果二者的值相等，则CPU不支持超线程.\[4\]
  ...
  - ,
    一个目录包含了当前网络栈的信息，特别是列出了存在的网络连接(对跟踪路由特别有用，因为[iptables转发被用于重定向网络连接](../Page/iptables.md "wikilink"))

  - , 一个设备号、尺寸与名的列表，内核用于辨别已存在的[硬盘分区](../Page/硬盘分区.md "wikilink")

  - ,
    给出任何通过[SCSI或](../Page/SCSI.md "wikilink")[RAID控制器挂接的设备的信息](../Page/RAID.md "wikilink")
  ...
  - , 包括共享内存与[进程间通信](../Page/进程间通信.md "wikilink") (IPC)信息

  - , 包含当前终端信息;
    是可利用的[tty类型列表](../Page/终端.md "wikilink")，其中的每一个是该类型的可用设备列表。

  - , 内核启动后经过的秒数与idle模式的秒数

  - , 包含Linux内核版本，发布号（distribution number）,
    编译内核的[gcc版本](../Page/gcc.md "wikilink")，其他相关的版本

  - 其他文件依赖于不同的硬件，模块配置与内核改变

Linux下使用 `/proc` 的基本工具是 [procps](../Page/procps.md "wikilink") (`/proc`
processes) 中的程序，這個程序只對 procfs 具有意義。procfs
對部分功能從[核心態移到](../Page/核心態.md "wikilink")[用戶態的過程中產生重大的意義](../Page/用戶態.md "wikilink")。像是
GNU 版本的 ps 只需在[用戶態底下運作透過](../Page/用戶態.md "wikilink") procfs
取得資料便可以完成所有的工作。

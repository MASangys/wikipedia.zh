...
再以\(v = y q_2\)代入：
  ...
  -
    \(v' = v^2 + P(x)v + Q(x)\)；其中令 \(Q(x) = q_0q_2;P(x)=q_1+\frac{q_2'}{q_2}\) 。

再以\(v = -\frac{u'}{u}\)代入上式。

:\# \(v'=-\left(\frac{u'}{u}\right)'=-\frac{u''}{u} +\left(\frac{u'}{u}\right)^2=-\frac{u''}{u}+v^2\!\) 则

\[\frac{u''}{u}= v^2 -v'=-Q -Pv= -Q+P\frac{u'}{u}\] 因此

\[u'' -Pu' +Qu=u'' -(q_1+\frac{q_2'}{q_2})u' +q_0q_2u=0\] 最终 \(y=-\frac{u'}{q_2u}\).
...
已知 \(y = y_1\) 是一特定解，可設通解\(y = y_1 + \frac{1}{z}\)，代入整理得[一階線性常微分方程](https://zh.wikipedia.org/wiki/一階線性常微分方程 "wikilink")：

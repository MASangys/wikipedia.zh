**Savage**是由[S3
Graphics推出的一个](../Page/S3_Graphics.md "wikilink")[圖形處理器產品線](../Page/圖形處理器.md "wikilink")。
...
Pro的。它們可以被配備與8-32
的MIB內存。集成電視編碼器下降的同時，DVD加速是值得稱道的，該芯片支持DVI接口的液晶顯示器的一個早期版本。
A“LT”後綴部分降低了功耗為特色，如ATI的RAGE
LT系列，用於筆記本電腦。不過，這結束了在幾個AGP卡如九的S3的Savage4的8MB的一部分（如圖）。
的Savage4板供應商，贏得了眾多的設計贏得了包括鑽石多媒體（隱形III S540）和Creative
Labs的。的Savage4系列的單週期三線性過濾和S3TC紋理壓縮技術創建了一個三維的卡具有出色的圖像質量。然而，持續64位內存總線的帶寬限制，S3保證這款顯卡來說從來都不是32位色下的性能部分。驅動程序又是一個問題與S3的產品整體性能，使軟件和硬件的兼容性問題。
...
eat）功能，在執行Quak Ⅱ、Unreal及Half-Life等高階遊戲時可達到更好的效 果。
Savage4繪圖晶片擁有S3第二代材質壓縮技術S3TC（S3 Texture Compressi on），S3在前一代的Savage
3D繪圖晶片中已開發出材質壓縮技術，並且授權Mic rosoft使用於DirectX
6.0中。Savage4所使用的材質壓縮技術較第一代先進，S3
亦是目前唯一將材質壓縮技術使用於繪圖晶片的廠商。，兩個流行的遊戲，附帶內置支持S3TC。壓縮紋理是一個巨大的進步，在標準紋理上使用的所有其他卡。不僅如此，但是S3TC這些更高質量的紋理渲染性能的影響可以忽略不計的

S3將推出Savage4 GT及Savage4 Pro兩種版本繪圖晶片。Savage4 Pro針對中
階主流電腦設計，支援四倍頻AGP、143MHz的顯示記憶體及最大32MB顯示記憶體；Savage4
GT則是針對低價電腦使用，支援二倍頻AGP、125MHz的顯示記憶體及最大16MB顯示記憶體
；因此不想更換主機板的使用者，亦能使用配備Savage4
GT繪圖晶片顯示卡。 針對99年使用者對3D繪圖的需求，Savage4 支援完整Direct X 6.0
...
6，因而只得空有的硬件光影转换，驱动一直未能編寫好，最終没有完整支援硬件光影转换OpenGL和Direct3D已經停止開發。局限當年遊戲纹理层不多，即使理論上填充速率相當，Savage2000
2X2架構比不上GeForce 256的4X1架構實際。 Savage 2000最終不能为S3扭转败局，不久S3便被VIA收购。

## "Savage IGP"

VIA 主板上ProSavage IGP 代號有桌面電腦的ProSavage(4/8/DDR)和手提電腦的SuperSavage MX &
IX，他們是混合 Savage4 3D 和 Savage2000 2D 的技術。桌面電腦晶片組包括有，支援 AMD 的KM133,
KM266，支持intel 的PL133T, PM133T, PL133T 和 P4M266.

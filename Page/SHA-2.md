...
<span style="color:green;">*`Initialize``   ``table``   ``of``
 ``round``   ``constants`*</span>
<span style="color:green;">`(first 32 bits of the `*`fractional``
 ``parts`*` of the cube roots of the first 64 primes 2..311):`</span>
`k[0..63] :=`
...
<span style="color:green;">*`Process``   ``the``   ``message``   ``in``
 ``successive``   ``512-bit``   ``chunks:`*</span>
`break message into 512-bit chunks`
**`for`**` each chunk`
`    break chunk into sixteen 32-bit big-endian words w[0..15]`

`    `<span style="color:green;">*`Extend``   ``the``   ``sixteen``
 ``32-bit``   ``words``   ``into``   ``sixty-four``   ``32-bit``
 ``words:`*</span>
`    `**`for`**` i `**`from`**` 16 to 63`

**SQL**（\[1\]或\[2\]，**结构化查询语言**\[3\]\[4\]\[5\]\[6\]）是一种[特定目的程式语言](../Page/特定目的程式语言.md "wikilink")，用于管理[关系数据库管理系统](../Page/关系数据库管理系统.md "wikilink")（RDBMS），或在[关系流数据管理系统](../Page/关系流数据管理系统.md "wikilink")（RDSMS）中进行流处理。

SQL基于[关系代数和](../Page/关系代数_\(数据库\).md "wikilink")[元组关系演算](../Page/元组关系演算.md "wikilink")，包括一个[数据定义语言和](../Page/数据定义语言.md "wikilink")[数据操纵语言](../Page/数据操纵语言.md "wikilink")。SQL的范围包括数据插入、查询、更新和删除，创建和修改，以及数据访问控制。尽管SQL经常被描述为，而且很大程度上是一种[声明式编程](../Page/声明式编程.md "wikilink")（[4GL](../Page/第四代程式语言.md "wikilink")），但是其也含有[过程式编程的元素](../Page/过程式编程.md "wikilink")。
...
SQL在1986年成为[美国国家标准学会](../Page/美国国家标准学会.md "wikilink")（ANSI）的一项[标准](../Page/标准.md "wikilink")，在1987年成为[国际标准化组织](../Page/国际标准化组织.md "wikilink")（ISO）标准\[10\]</ref>。此后，这一标准经过了一系列的增订，加入了大量新特性。虽然有这一标准的存在，但大部分的SQL代码在不同的数据库系统中并不具有完全的跨平台性。
...
(SQL)表达式](../Page/Case_\(SQL\).md "wikilink")。支持CHECK约束。创建临时表。支持cursor。支持[事务隔离](../Page/事务隔离.md "wikilink")。
...
SQL语言分成了几种要素，包括：
  ...
  - 表达式，可以产生任何[标量值](../Page/变量_\(程序设计\).md "wikilink")，或由和的[数据库表](../Page/数据库表.md "wikilink")
  - 谓词，给需要评估的SQL[三值逻辑（3VL）](../Page/三值逻辑.md "wikilink")（true/false/unknown）或[布尔](../Page/逻辑代数.md "wikilink")[真值指定条件](../Page/真值.md "wikilink")，并限制语句和查询的效果，或改变程序流程。
  ...
  - [无意义的空白在SQL语句和查询中一般会被忽略](../Page/空白.md "wikilink")，更容易格式化SQL代码便于阅读。

## 语言特点

SQL是高级的[非過程化編程語言](../Page/非過程化編程.md "wikilink")，它允许用户在高层数据结构上工作。它不要求用户指定对数据的存放方法，也不需要用户了解其具体的数据存放方式。而它的界面，能使具有底层结构完全不同的数据库系统和不同数据库之间，使用相同的SQL作为数据的输入与管理。它以记录项目〔records〕的合集（set）〔项集，record
...
SQL同時也是数据库[文件格式的](../Page/文件格式.md "wikilink")[扩展名](../Page/扩展名.md "wikilink")。
...
SQL包含四个部分：
  ...
  - [事务控制語言](../Page/事务控制語言.md "wikilink")
...
SQL努力遵从标准](../Page/Mimer_SQL.md "wikilink")。Oracle数据库的`DATE`行为如同`DATETIME`，缺少`TIME`
type)。
...
<td><p>1989</p></td>
<td><p>SQL-89</p></td>
<td><p><a href="../Page/聯邦資料處理標準.md" title="wikilink">FIPS</a> 127-1</p></td>
...
<td></td>
<td><p>SQL3</p></td>
<td><p>增加了正则表达式匹配、递归查询（传递闭包）、数据库触发器、过程式与控制流语句、非标量类型(arrays)、面向对象特性。在Java中嵌入SQL(<a href="../Page/SQL/OLB.md" title="wikilink">SQL/OLB</a>)及其逆(<a href="../Page/SQL/JRT.md" title="wikilink">SQL/JRT</a>)</p></td>
</tr>
<tr class="odd">
<td><p>2003</p></td>
<td></td>
<td></td>
<td><p>增加<a href="../Page/XML.md" title="wikilink">XML相关特性</a>(<a href="../Page/SQL/XML.md" title="wikilink">SQL/XML</a>)、<em>window functions</em>、标准化sequences、自动产生值的列。对SQL:1999的新特性重新描述其内涵。</p></td>
</tr>
<tr class="even">
<td><p>2006</p></td>
<td></td>
<td></td>
<td><p>导入/导出XML数据与SQL数据库。<a href="../Page/XQuery.md" title="wikilink">XQuery</a></p></td>
</tr>
<tr class="odd">
<td><p>2008</p></td>
<td></td>
...
SQL标准包含9部分：
  ...
  - ISO/IEC 9075-3:2016 Part 3: *Call-Level Interface*
    ([SQL/CLI](../Page/SQL/CLI.md "wikilink")). 定义了接口成分(structures,
    procedures, variable bindings) 用于编写能执行SQL的应用程序的语言：Ada, C/C++, COBOL,
  ...
  - ISO/IEC 9075-4:2016 Part 4: *Persistent stored modules*
    ([SQL/PSM](../Page/SQL/PSM.md "wikilink"))。SQL的过程式扩展，包括控制流、条件处理、语句条件signals与resignals、cursors、本地变量、表达式赋值到变量与参数。此外，SQL/PSM形式化声明与维护了持续性（persistent）数据库语言例程（例如存储过程）。这部分标准主要包含可选的特性。
  ...
  - ISO/IEC 9075-9:2016 Part 9: *Management of External Data*
    ([SQL/MED](../Page/SQL/MED.md "wikilink")). 这部分标准主要包含可选的特性。
  - ISO/IEC 9075-10:2016 Part 10: *Object language bindings*
    ([SQL/OLB](../Page/SQL/OLB.md "wikilink"))。定义了[SQLJ](../Page/SQLJ.md "wikilink")，SQL嵌入到Java，保证了SQLJ应用程序二进制可移植。这部分标准主要包含可选的特性。
  - ISO/IEC 9075-11:2016 Part 11: *Information and definition schemas*
    ([SQL/Schemata](../Page/SQL/Schemata.md "wikilink")). 定义了Information
    Schema与Definition Schema，提供了常用工具集使得SQL数据库与对象自描述。这些工具包括SQL object
    identifier、structure与integrity constraints、security与authorization
    specifications, features与packages。这部分标准主要包含强制与可选的特性。
  - ISO/IEC 9075-13:2016 Part 13: *SQL Routines and types using the Java
    TM programming language* ([SQL/JRT](../Page/SQL/JRT.md "wikilink")).
  ...
  - ISO/IEC 9075-14:2016 Part 14: *XML-Related Specifications*
    ([SQL/XML](../Page/SQL/XML.md "wikilink")). 这部分标准主要包含可选的特性。
...
<!-- end list -->
  ...
  -
    微軟[MS SQL-Server](../Page/MS_SQL-Server.md "wikilink")，以及系列資料庫所用的SQL
...
<!-- end list -->
  ...
  -
    ...
    [資料庫所使用的SQL](../Page/Oracle資料庫.md "wikilink")

...
NAT**，NAT会话穿越应用程序）是一种[网络协议](../Page/网络协议.md "wikilink")，它允许位于[NAT](../Page/网络地址转换.md "wikilink")（或多重NAT）后的客户端找出自己的公网地址，查出自己位于哪种类型的NAT之后以及NAT为某一个本地端口所绑定的Internet端端口。这些信息被用来在两个同时处于NAT路由器之后的主机之间建立UDP通信。该协议由RFC
5389定义。
...
STUN是一个[客户机－服务器协议](../Page/客户机－服务器.md "wikilink")。一个VoIP电话或软件包可能会包括一个STUN客户端。这个客户端会向STUN服务器发送请求，之后，服务器就会向STUN客户端报告NAT路由器的公网IP地址以及NAT为允许传入流量传回内网而开通的端口。

以上的响应同时还使得STUN客户端能够确定正在使用的[NAT类型](../Page/网络地址转换.md "wikilink")——因为不同的NAT类型处理传入的UDP分组的方式是不同的。四种主要类型中有三种是可以使用的：[完全圆锥型NAT](../Page/完全圆锥型NAT.md "wikilink")、[受限圆锥型NAT和](../Page/受限圆锥型NAT.md "wikilink")[端口受限圆锥型NAT](../Page/端口受限圆锥型NAT.md "wikilink")——但大型公司网络中经常采用的对称型NAT（又称为双向NAT）则不能使用。

## 演算法

STUN使用下列的[演算法](../Page/演算法.md "wikilink")（取自RFC 3489）來發現NAT
gateways以及防火牆（firewalls）:

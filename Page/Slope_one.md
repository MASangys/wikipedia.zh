**Slope One**
是一系列应用于[协同过滤的算法的统称](https://zh.wikipedia.org/wiki/协同过滤 "wikilink")。由
Daniel Lemire和Anna Maclachlan于2005年发表的论文中提出。
\[1\]有争议的是，该算法堪称基于项目评价的non-trivial
...
以及多达2,000,000个回归量。除非我们只选择某些用户共同评价过的项目对，否则协同过滤会遇到[过适](https://zh.wikipedia.org/wiki/过适 "wikilink")\[7\](过拟合)
问题。
...
Item-based 协同过滤只是
[协同过滤的一种形式](https://zh.wikipedia.org/wiki/协同过滤 "wikilink").其它还有像
...
为了大大减少[过适](https://zh.wikipedia.org/wiki/过适 "wikilink")(过拟合)的发生，提升算法简化实现，
**Slope One**
系列易实现的Item-based[协同过滤算法被提了出来](https://zh.wikipedia.org/wiki/协同过滤 "wikilink")。本质上，该方法运用更简单形式的回归表达式(\(f(x)=x+b\))

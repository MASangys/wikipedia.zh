...
尽管在引入Trim前已有将某些驱动器“重置”为新状态的工具，但它们也将删除该驱动器上的所有数据，从而使这些工具和技术不能用于持续性优化。\[4\]在2014年，许多SSD有内部的后台垃圾收集机制，它们与修整工作相独立。尽管这在不支持修整的操作系统上也成功地保持了性能，它存在增加[写入放大和闪存单元磨损的相关缺点](../Page/写入放大.md "wikilink")。\[5\]

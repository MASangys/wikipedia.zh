**Turbo BASIC**是一個[BASIC](../Page/BASIC.md "wikilink")
[編譯器與編輯器](../Page/編譯器.md "wikilink")，最初由Robert 'Bob'
Zale所開發，後來被[Borland買下](../Page/Borland.md "wikilink")。當Borland決定不再販售之後，Zale從Borland手中買回它，並改名為[PowerBASIC並創立PowerBASIC公司](../Page/PowerBASIC.md "wikilink")，以繼續支援與研發。今天仍然存在市面上。
...
Basic的整合化開發環境能在其中執行一個BASIC程式（參見下面的例子）以為傳統BASIC測試，或是產生完全獨立的MS-DOS可執行檔（.EXE），能在其它沒有安裝Turbo
BASIC或是其執行函式庫的電腦上執行。當然編譯出來的可執行程式檔會比原來僅含程式碼的.bas檔案大得多。

Turbo Basic聲稱其乃以x86組合語言撰寫，所編譯出來的程式比其他程式所編譯的執行速度較快。另外，因為早期對於中文系統的缺陷，Turbo
Basic在中文環境下的使用存在一些問題。例如，在倚天中文環境底下雖然可以顯示中文文字，但是IDE有亂碼，且使用其INPUT命令並不能直接輸入中文字。后来的UCDOS，天汇等汉字系统已经能够很好地支持Turbo
BASIC。在Turbo Basic的IDE编辑环境中，甚至在Turbo Basic所编译出的软件中，都可以顺利地输入中文字。

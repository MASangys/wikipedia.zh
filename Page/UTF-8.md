...
Posix系统明确不建议使用字节序掩码`EF BB BF`。\[12\]因为很多文本文件期望以 “\#\!”（[Shebang](../Page/Shebang.md "wikilink")）开头指示要运行的程序。Linux系统选择使用Unicode规范形式Normalization Form C（NFC），即优先使用预组装字符（precomposed character）而非组合字符序列（combining character sequence）。
...
字符串处理时，使用UTF-8或locale依赖的多字节编码情形，比使用C语言wchar_t的宽字符固定宽度编码，要慢1至2个数量级。\[13\]
...
11.
12. [UTF-8 and Unicode FAQ for Unix/Linux by Markus Kuhn](http://www.cl.cam.ac.uk/~mgk25/unicode.html)
13.
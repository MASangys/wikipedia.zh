> 本文内容由[VRML](https://zh.wikipedia.org/wiki/VRML)转换而来。


**VRML**是一种專為[WWW](https://zh.wikipedia.org/wiki/WWW "wikilink")（萬維網）而設計的三維圖像[置标语言](../Page/置标语言.md "wikilink")。全称是虚拟现实建模语言，是由VRML协会设计的。VRML标准中既定义了描述三维模型的编码格式，也定义了描述交互或脚本的编码及行为模式。VRML协会现已更名为[Web3D联盟](http://www.web3d.org)，VRML标准现在也已经升级为[X3D](../Page/X3D.md "wikilink")标准。

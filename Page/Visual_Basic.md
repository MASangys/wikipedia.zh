...
1997年2月，VB5.0发布。程序员可以用32位的版本导入由4.0版本创建的16位程序，并且能顺利编译。同时还包含了对用户自建控件的支持。自从Visual
Basic 5.0以后，VB支持编译成本机代码，但必须在VB运行库的支持下工作。
...
微软开发了一系列有关VB的脚本语言：
  ...
  - [Visual Basic for
    Applications](../Page/Visual_Basic_for_Applications.md "wikilink")，即**VBA**，包含在微软的应用程序中（比如[Microsoft
    Office](../Page/Microsoft_Office.md "wikilink")），以及类似[WordPerfect
    Office这样第三方的产品里面](../Page/WordPerfect_Office.md "wikilink")。VBA这样嵌入在各种应用程序中看起来有些矛盾，但是它的功能和VB一样强大。
...
但是很多VB的支持者解释说VB的易用性就是它最大的优势，可以让经验丰富的VB程序员或是刚刚懂得皮毛的人都能用自己的方式快速开发程序。而且VB的程序可以非常简单的和[数据库连接](../Page/数据库.md "wikilink")。比如利用控件可以绑定数据库，这样一来用VB写出的程序就可以掌握数据库的所有信息而不用写一行代码。

很多语言的特性比如GoSub、On
Error和用變數名字的最后一个字符判断宣告类型（比如字符串型str$）都是从[BASIC中来的](../Page/BASIC.md "wikilink")。VB的语法和绝大部分语言都不同，可能会让新的程序员混淆。比如宣告“Dim
a, b, c As
Integer”宣告了c为整數變數，但是a,b依然是Variant（.NET中为Object）类型的。（如果想宣告3个Integer，必须写Dim
a As Integer, b As Integer, c as Integer；而直接写Dim
a，后面不写类型，则自动变成Variant类型——占用大量资源。這個問題在VB.NET以後的版本已经解決，Dim
a, b, c As Integer的类型都将为整數）
...
## 外部連結
  ...
  - [Studio
    Express首頁（下載）](http://www.visualstudio.com/zh-tw/products/visual-studio-express-vs)

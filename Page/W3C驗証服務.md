...
（X）HTML驗證工具將網頁與W3C標準比較。標準則根據文件宣告的版本而驗證工具會讀取[DOCTYPE宣告而決定使用的標準](https://zh.wikipedia.org/wiki/DOCTYPE "wikilink")。\[5\]當驗證工具讀取到網頁及決定使用的標準後，驗證工具將會檢查錯誤，例如元素開關的遺漏，引號的遺漏及其他代碼錯誤。\[6\]驗證工具隨後會建立報告，指出文件是否正確。錯誤則會以列表方式顯示。某些錯誤，例如遺漏元素關閉，可以造成數個，甚至過百個錯誤。但是[網頁設計師則只需要處理第一個出現的錯誤並排除餘下連串的錯誤](https://zh.wikipedia.org/wiki/網頁設計師 "wikilink")。\[7\]
...
當W3C及其他HTML/XHTML驗證工具檢查網頁的結構時，同時亦需要一個獨立的驗證工具來檢查是否正確[CSS](https://zh.wikipedia.org/wiki/CSS "wikilink")。CSS驗證工具使用與HTML/XHTML驗證工具類似的驗證方式，即是將CSS文件與標準比較。\[8\]

...
**\*安装Apache，PHP，MySQL（最好不要启用InnoDB）phpMyAdmin**
...
**\*配置webERP(1)：建立数据库，GB2312编码方式，并执行导入web-new.sql**
  ...
  -
    进入http://localhost/phpMyAdmin管理界面，简便方式可以直接用root进入
...
**\*进入webERP：**
  ...
  -
    进入http://localhost/weberp/index.php， 出现如下界面，就可以用登陆界面的用户名和密码登录了

**\*中文设置** 1.打开/includes/ConnectDB_mysqli.inc找到

`   if ( !$db ) {`
`       echo '`
`' . _('The configuration in the file config.php for the database user name and password do not provide the information required to            connect to the database server');`
`       exit;`
`   }`
...
## 外部链接
  ...
  - [WebERP專案的官方網站](http://www.weberp.org)

...
Tools），针对[面向服务的体系结构的作业计划工具](../Page/面向服务的体系结构.md "wikilink")（SOA
Job Scheduler），以及基于开源接口的消息传递接口\[2\]，和通过“超算基础配置文件（High Performance
Computing Basic Profile，HPCBP）”这个基于（Open Grid
Forum，OGF）标准特性来实现集群互通性\[3\]。
...
**Windows HPC Server 2008 R2**是基于[Windows Server 2008
R2的操作系统](../Page/Windows_Server_2008_R2.md "wikilink")，于2010年9月20日发布\[8\]
。

## Windows HPC Pack

在Windows HPC Server 2008 R2之后，微软发布了**HPC
Pack**以针对四种用途：普遍（Express）、企业（Enterprise）、工作站（Workstation）和计算时间获取（Cycle
Harvesting）。之后的**HPC Pack 2012**的发布本身就包含了这四种能力导致需求被简化了。HPC Pack
2012可以被安装在任何基于[Windows Server
2012标准版](../Page/Windows_Server_2012.md "wikilink")（Standard）和数据中心版（Datacenter）的计算机上。\[9\]
...
## 外部链接
  ...
  -
  - [Windows HPC Server 2008 R2 Technical
    Library](http://technet.microsoft.com/en-us/library/ee783547%28WS.10%29.aspx)

  - [Windows HPC Server 2008 Resource
    Kit](https://web.archive.org/web/20110724131253/http://resourcekit.windowshpc.net/Default.htm)

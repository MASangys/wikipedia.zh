...
在執行XBAP之前，必須先設定：
  ...
  - Visual Studio 2005 extensions for .NET Framework 3.0（November 2006
    CTP）
...
## 特點
  ...
  - XBAPS可以使用WPF（[Windows Presentation
    Foundation](../Page/Windows_Presentation_Foundation.md "wikilink")）的98.2%的功能。
  - XBAPS必須執行於iFrame之下。
...
XAML与HTML一样是flow
layout，Grid類似HTML中的Table，使整個頁面的顯示方式變成網格式区域。微軟還推荐使用StackPanel，DockPanel等继承自Panel的規劃方式，Panel在使用上的類似HTML的{{\<}}DIV{{\>}}{{\<}}\\DIV{{\>}}。
...
## 外部連結
  ...
  - [XBAP = next-generation
    ActiveX?](http://rrelyea.spaces.live.com/blog/cns!167AD7A5AB58D5FE!1553.entry)

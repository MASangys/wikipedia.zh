...
XFS的开发始于1993年，在1994年被首次部署在IRIX
5.3上。2000年5月，XFS在[GNU通用公共许可证下发布](../Page/GNU通用公共许可证.md "wikilink")，并被移植到Linux上。2001年XFS首次被[Linux发行版所支持](../Page/Linux发行版.md "wikilink")，现在所有的Linux发行版上都可以使用XFS。

XFS最初被合并到[Linux](../Page/Linux内核.md "wikilink")
...
[日志文件系统是一种即使在断电或者是操作系统崩溃的情况下保证文件系统一致性的途径](../Page/日志文件系统.md "wikilink")。XFS对文件系统[元数据提供了日志支持](../Page/元数据.md "wikilink")。当文件系统更新时，元数据会在实际的磁盘块被更新之前顺序写入日志。XFS的日志被保存在磁盘块的循环缓冲区上，不会被正常的文件系统操作影响。XFS日志大小的上限是64k个块和128MB中的较大值，下限取决于已存在的文件系统和目录的块的大小。在外置设备上部署日志会浪费超过最大日志大小的空间。XFS日志也可以被存在文件系统的数据区（称为内置日志），或者一个额外的设备上（以减少磁盘操作）。
...
XFS通过实现[扩展文件属性给文件提供了多个数据流](../Page/扩展文件属性.md "wikilink")，使文件可以被附加多个名/值对。文件名是一个最大长度为256字节的、以NULL字符结尾的可打印字符串，其它的关联值则可包含多达64KB的二进制数据。这些数据被进一步分入两个名字空间中，`root`和`user`。保存在root名字空间中的扩展属性只能被超级用户修改，user名字空间中的可以被任何对该文件拥有写权限的用户修改。扩展属性可以被添加到任意一种
XFS
...
XFS的磁盘配额在文件系统被初次挂载时启用。这解决了一个在其它大多数文件系统中存在的一个竞争问题：要求先挂载文件系统，但直到调用quotaon
(8)之前配额不会生效。
...
XFS文件系统创建时默认使用内置日志，把日志和文件系统数据放置在同一个块设备上。由于在所有的文件系统写入发生前都要更新日志中的元数据，内置日志可能导致磁盘竞争。在大多数负载下，这种等级的竞争非常低以至于对性能没有影响。但对于沉重的随机写入负载，比如在忙碌的数据块服务器上，XFS可能因为这种
I/O竞争无法获得最佳性能。另一个可能提高这个问题的严重性的因素是，日志写入被要求以同步方式提交——它们必须被完全写入，之后对应实际数据的写入操作才能开始。
...
## 参考文献
  ...
  - [XFS for Linux Administration（SGI Document
    Number: 007-4273-003）](https://web.archive.org/web/20160120053200/http://techpubs.sgi.com/library/tpl/cgi-bin/getdoc.cgi?coll=linux&db=bks&srch=&fname=%2FSGI_Admin%2FLX_XFS_AG%2Fsgi_html%2Fch01.html)
...
2.  [Red Hat Linux Bugzilla entry on XFS
    implementation](https://bugzilla.redhat.com/show_bug.cgi?id=521173)
3.  [　Release Notes for Red Hat Enterprise
    Linux 7.0](https://access.redhat.com/site/documentation/en-US/Red_Hat_Enterprise_Linux/7/html/7.0_Release_Notes/chap-Red_Hat_Enterprise_Linux-7.0_Release_Notes-File_Systems.html)
4.  [XFS mailing list post regarding state of DMAPI
    support](http://oss.sgi.com/archives/xfs/2010-10/msg00025.html)
5.  [Linux questions about freezing
    Ext3](http://www.linuxquestions.org/questions/linuxquestions-org-member-intro-24/how-to-freeze-ext3-file-system-229914/)
6.  [Freeze Feature Commit to Linux
    kernel](http://git.kernel.org/?p=linux/kernel/git/torvalds/linux-2.6.git;a=commit;h=c4be0c1dc4cdc37b175579be1460f15ac6495e9a)
7.  [Bitubique.com](http://bitubique.com/content/defragment-xfs-file-system)

8.  [XFS.org](http://xfs.org/index.php/XFS_FAQ#Q:_Is_there_a_way_to_make_a_XFS_filesystem_larger_or_smaller.3F),
    FAQ
9.  [SGI.com](http://oss.sgi.com/archives/xfs/2002-01/msg02379.html)
10. [and
    <http://oss.sgi.com/archives/xfs/2010-05/msg00329.html>](http://xfs.org/index.php/Improving_Metadata_Performance_By_Reducing_Journal_Overhead)
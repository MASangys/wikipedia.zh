...
最初有传言指Xubuntu会随Ubuntu
5.10版发布，但结果被证实为办不到，虽然在软件库中已经有了提供xfce桌面环境的xubuntu-desktop软件包。第一个官方发布的版本为2006年6月1日发布的Xubuntu
6.06即"Dapper Drake"。
...
用户可以使用Desktop CD（有图形界面）或Alternate CD（仅有字符界面）安装Xubuntu。使用Desktop
CD安装需要192 MB記憶體，使用Alternate CD安装仅需要64
MB記憶體。\[2\]安装完成后，Xubuntu需要192
MB的記憶體来运行，但强烈建议使用至少256
MB記憶體（对于8.04以上版本，建议至少用512记忆体运行）。

## 相似计划

另一個追求輕巧的分支計劃[Lubuntu正在進行](../Page/Lubuntu.md "wikilink")，并在2010年5月2日推出第一个正式版(10.04)。\[3\]
...
1.
2.  {{ cite web|url=<http://www.xubuntu.org/get#requirements>
    |title=Xubuntu 8.04 Hardware Requirements |accessdate=2008-04-10
    |deadurl=yes
    |archiveurl=<https://web.archive.org/web/20100622083826/http://www.xubuntu.org/get>
    |archivedate=2010-06-22 }}

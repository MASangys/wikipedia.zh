**Β函数**，又称为**贝塔函数**或**第一类[欧拉积分](../Page/欧拉积分.md "wikilink")**，是一个[特殊函数](../Page/特殊函数.md "wikilink")，由下式定义：

\[\mathrm{\Beta}(x,y) = \int_0^1t^{x-1}(1-t)^{y-1}\,dt
\!\]

其中\(\textrm{Re}(x), \textrm{Re}(y) > 0\,\)。

## 性质

Β函数具有以下[對稱性質](../Page/對稱.md "wikilink")：

\[\Beta(x,y) = \Beta(y,x).
\!\] 当x,y是正整数的时候，我们可以从伽马函数定义得到如下式子：

\[\Beta(x,y)=\dfrac{(x-1)!\,(y-1)!}{(x+y-1)!}
\!\]

它有许多其它的形式，包括：

\[\Beta(x,y)=\dfrac{\Gamma(x)\,\Gamma(y)}{\Gamma(x+y)}
\!\]

\[\Beta(x,y) =
  2\int_0^{\frac{\pi}{2}}(\sin\theta)^{2x-1}(\cos\theta)^{2y-1}\,d\theta,
  \qquad \textrm{Re}(x)>0,\ \textrm{Re}(y)>0
\!\]

\[\Beta(x,y) =
  \int_0^\infty\dfrac{t^{x-1}}{(1+t)^{x+y}}\,dt,
  \qquad \textrm{Re}(x)>0,\ \textrm{Re}(y)>0
\!\]

\[\Beta(x,y) =
  \sum_{n=0}^\infty \dfrac{{n-y \choose n}} {x+n},
\!\]

\[\Beta(x,y) = \prod_{n=0}^\infty \left( 1+ \dfrac{x y}{n (x+y+n)}\right)^{-1},
\!\]

\[\Beta(x,y) \cdot \Beta(x+y,1-y) =
  \dfrac{\pi}{x \sin(\pi y)},
\!\]

\[\Beta(x,y) =
  \dfrac{1}{y}\sum_{n=0}^\infty(-1)^n\dfrac{y^{n+1}}{n!(x+n)}
\!\]

其中\(\Gamma\,\)是[伽玛函数](../Page/伽玛函数.md "wikilink")。

就像伽玛函数描述了[阶乘一样](../Page/阶乘.md "wikilink")，我们也可以用贝塔函数来定义[二项式系数](../Page/二项式系数.md "wikilink")：

\[{n \choose k} = \frac1{(n+1) \Beta(n-k+1, k+1)}\]

## 伽玛函数与贝塔函数之间的关系

为了推出两种函数之间的关系，我们把两个阶乘的乘积写为：

\[\Gamma(x)\Gamma(y) =
  \int_0^\infty\ e^{-u} u^{x-1}\,du \int_0^\infty\ e^{-v} v^{y-1}\,dv.
\!\]

现在，设\(u \equiv a^2\), \(v \equiv b^2\)，因此：

\[\begin{align}
 \Gamma(x)\Gamma(y) & {} =
  4\int_0^\infty\ e^{-a^2} a^{2x-1}\mathrm{d}a \int_0^\infty\ e^{-b^2} b^{2y-1}\,db \\
& {} = \int_{-\infty}^\infty\ \int_{-\infty}^\infty\ e^{-(a^2+b^2)} |a|^{2x-1} |b|^{2y-1} \,da \,db.
\end{align}
\!\]

利用变量代换\(a = r\cos\theta\)和\(b = r\sin\theta\)，可得：

\[\begin{align}
 \Gamma(x)\Gamma(y) & {} =
  \int_0^{2\pi}\ \int_0^\infty\ e^{-r^2} |r\cos\theta|^{2x-1} |r\sin\theta|^{2y-1} r \, dr \,d\theta \\
& {} = \int_0^\infty\ e^{-r^2} r^{2x+2y-2} r\, dr \int_0^{2\pi}\ |(\cos\theta)^{2x-1} (\sin\theta)^{2y-1}| \, d\theta \\
& {} = \frac{1}{2}\int_0^\infty\ e^{-r^2} r^{2(x+y-1)} \, d(r^2) 4\int_0^{\frac{\pi}{2}}\ (\cos\theta)^{2x-1} (\sin\theta)^{2y-1} \,d\theta \\
& {} = \Gamma(x+y) 2\int_0^{\frac{\pi}{2}}\ (\cos\theta)^{2x-1} (\sin\theta)^{2y-1} \, d\theta \\
& {} = \Gamma(x+y) \Beta(x,y).
\end{align}\]

因此，有：

\[\Beta(x,y) = \frac{\Gamma(x)\,\Gamma(y)}{\Gamma(x+y)}.\]

## 导数

贝塔函数的导数是：

\[{\partial \over \partial x} \mathrm{B}(x, y) = \mathrm{B}(x, y) \left( {\Gamma'(x) \over \Gamma(x)} - {\Gamma'(x + y) \over \Gamma(x + y)} \right) = \mathrm{B}(x, y) (\psi(x) - \psi(x + y))\]

其中\(\psi(x)\)是[双伽玛函数](../Page/双伽玛函数.md "wikilink")。

## 估计

[斯特灵公式给出了一个用来近似计算贝塔函数的公式](../Page/斯特灵公式.md "wikilink")：

\[\Beta(x,y)\approx\sqrt {2\pi } \frac{{x^{x - \frac{1}{2}} y^{y - \frac{1}{2}} }}{{\left( {x + y} \right)^{x + y - \frac{1}{2}} }}.\]

## 不完全贝塔函数

**不完全贝塔函数**是贝塔函数的一个推广，把贝塔函数中的[定积分用](../Page/定积分.md "wikilink")[不定积分来代替](../Page/不定积分.md "wikilink")，就像[不完全伽玛函数是伽玛函数的推广一样](../Page/不完全伽玛函数.md "wikilink")。

不完全贝塔函数定义为：

\[\Beta(x;\,a,b) = \int_0^x t^{a-1}\,(1-t)^{b-1}\,dt. \!\]

当*x* = 1，上式即化为贝塔函数。

**正则不完全贝塔函数**（或简称**正则贝塔函数**）由贝塔函数和不完全贝塔函数来定义：

\[I_x(a,b) = \dfrac{\Beta(x;\,a,b)}{\Beta(a,b)}. \!\]

当*a*和*b*是整数时，计算以上的积分（可以用[分部积分法](../Page/分部积分法.md "wikilink")），可得：

\[I_x(a,b) = \sum_{j=a}^{a+b-1} {(a+b-1)! \over j!(a+b-1-j)!} x^j (1-x)^{a+b-1-j}.\]

正则不完全贝塔函数是[Β分布的](../Page/Β分布.md "wikilink")[累積分布函數](../Page/累積分布函數.md "wikilink")，可由[二項式分布描述一個實](../Page/二項式分布.md "wikilink")[隨機變量X的機率分布](../Page/隨機變量.md "wikilink")：

\(F(k;n,p) = \Pr(X \le k) = I_{1-p}(n-k, k+1) = 1 - I_p(k+1,n-k)\)

其中p為[試驗成功機率](../Page/試驗成功.md "wikilink")，n為樣本數。

### 性质

\[I_0(a,b) = 0 \,\]

\[I_1(a,b) = 1 \,\]

\[I_x(a,b) = 1 - I_{1-x}(b,a) \,\]

## 参见

  - [Β分布](../Page/Β分布.md "wikilink")
  - [二项分布](../Page/二项分布.md "wikilink")
  - [伽玛函数](../Page/伽玛函数.md "wikilink")

## 参考文献

  - M. Zelen and N. C. Severo. in Milton Abramowitz and Irene A. Stegun,
    eds. *[Handbook of Mathematical
    Functions](../Page/Handbook_of_Mathematical_Functions.md "wikilink")
    with Formulas, Graphs, and Mathematical Tables.* New York: Dover,
    1972. *[（See §6.2, 6.6,
    and 26.5）](http://www.math.sfu.ca/~cbm/aands/page_258.htm)*

  - W. H. Press, B. P. Flannery, S. A. Teukolsky, W. T. Vetterling.
    *[Numerical Recipes](http://nr.com/) in C*. Cambridge, UK: Cambridge
    University Press, 1992. Second edition. *(See section 6.4)*

  -
## 外部链接

  - [贝塔函数计算器](https://web.archive.org/web/20080711054908/http://www.danielsoper.com/statcalc/calc35.aspx)
  - [不完全贝塔函数计算器](https://web.archive.org/web/20070120151547/http://www.danielsoper.com/statcalc/calc36.aspx)
  - [正则不完全贝塔函数计算器](https://web.archive.org/web/20070120151557/http://www.danielsoper.com/statcalc/calc37.aspx)

[Category:特殊超几何函数](https://zh.wikipedia.org/wiki/Category:特殊超几何函数 "wikilink")
[Category:伽玛及相关函数](https://zh.wikipedia.org/wiki/Category:伽玛及相关函数 "wikilink")
...
将*x* = 1
代入，便得莱布尼兹公式(1的反正切是π ⁄ 4)。这种推理产生的一个问题是1不在幂级数的[收敛半径](../Page/收敛半径.md "wikilink")以内。因此，需要额外论证当*x* = 1时级数收敛到tan<sup>−1</sup>(1)。一种方法是利用[交替级数判别法](https://zh.wikipedia.org/wiki/交替级数判别法 "wikilink")，然后使用[阿贝尔定理](../Page/阿贝尔定理.md "wikilink")证明级数收敛到tan<sup>−1</sup>(1)。然而，也可以用一个完全初等的证明。

...
给定\((\Omega, \mathcal{A})\)，其上的任何有限测度\(\mu\)都[等价于一个](../Page/绝对连续.md "wikilink")\((\Omega, \mathcal{A})\)的概率测度。具体的构造方法是：令\(\left(B_n\right)_{n\in\mathbb{N}}\)为全集\(\Omega\)的一个不交覆盖（[划分](../Page/集合划分.md "wikilink")），并且每个\(B_n\)在\(\mu\)下的测度都是有限的；再令\(\left(\omega_n\right)_{n\in\mathbb{N}}\)为一个由正实数构成的[数列](../Page/数列.md "wikilink")，并且[级数和](../Page/级数.md "wikilink")

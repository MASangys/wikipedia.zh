**一致连续性**描述定义在一定[度量空间](../Page/度量空间.md "wikilink")上的[函数](../Page/函数.md "wikilink")的性质。与[连续性刻画函数在局部的性质不同](https://zh.wikipedia.org/wiki/连续 "wikilink")，一致连续刻画的是函数的整体性质。一致连续是比连续更苛刻的条件。一个函数在某[度量空间](../Page/度量空间.md "wikilink")上一致连续，则其在此[度量空间](../Page/度量空间.md "wikilink")上必然连续，但反之未必成立。直观上，一致连续可以理解为，当自变量*x*在足够小的范围内变动时，函数值*y*的变动也会被限制在足够小的范围内。

## *ε-δ*定义

设\((X, d_1)\)和\((Y, d_2)\)是两个[度量空间](../Page/度量空间.md "wikilink")，\(M \subseteq X\)并且\(N \subseteq Y\)，则一个函数\(f : M\to N\) **一致连续**当且仅当对任意的\(\epsilon > 0\)，存在\(\delta >0\)，使得任意的\(x,y \in M\)只要\(d_1(x,y)< \delta\)，就有\(d_2(f(x),f(y)) < \epsilon\)。

**一致连续性**描述定义在一定[度量空间上的](../Page/度量空间.md "wikilink")[函数的性质](../Page/函数.md "wikilink")。与[连续性刻画函数在局部的性质不同](https://zh.wikipedia.org/wiki/连续 "wikilink")，一致连续刻画的是函数的整体性质。一致连续是比连续更苛刻的条件。一个函数在某[度量空间上一致连续](../Page/度量空间.md "wikilink")，则其在此[度量空间上必然连续](../Page/度量空间.md "wikilink")，但反之未必成立。直观上，一致连续可以理解为，当自变量*x*在足够小的范围内变动时，函数值*y*的变动也会被限制在足够小的范围内。
...
当\(X\)和\(Y\)都是[实数集的子集](https://zh.wikipedia.org/wiki/实数集 "wikilink")，\(d_1\)和\(d_2\)为[欧几里得度量所定义的距离](https://zh.wikipedia.org/wiki/欧几里得度量 "wikilink")\(|\cdot|\)时，一致连续的定义可表述为：如果对任意的\(\epsilon > 0\)，存在\(\delta>0\)，使得对任意的\(|x-y|< \delta\)，都有\(|f(x)-f(y)|<\epsilon\)，则\(f(x)\)在\(X\)上一致连续。

**一階保持**（Frst-order
hold、FOH）是一種的數學模型，可以透過傳統的[數位類比轉換器](../Page/數位類比轉換器.md "wikilink")（DAC）及稱為[積分器的](../Page/積分器.md "wikilink")[模拟电路完成](../Page/模拟电路.md "wikilink")。一階保持可以將訊號重建為近似原始訊號的函數。在實務上，像一階保持或[零階保持之類的保持電路有其必要性](../Page/零階保持.md "wikilink")。根據[采样定理](../Page/采样定理.md "wikilink")，可以將離散後的訊號用[狄拉克δ脈衝序列](../Page/狄拉克δ函数.md "wikilink")*x*<sub>s</sub>(*t*)表示，再經過[低通滤波器即可還原到原始的訊號](../Page/低通滤波器.md "wikilink")。不過實務上無法輸出狄拉克δ脈衝序列。利用傳統的數位類比轉換器以及一些線性類比電路就可以重建預測型或是延遲型的一階保持電路。
...
*z*<sup>−1</sup>的[数字滤波器](../Page/数字滤波器.md "wikilink")，將数字滤波器（*x*\[*n*\]−*x*\[*n*−1\]）的輸出送到傳統的[數位類比轉換器](../Page/數位類比轉換器.md "wikilink")（本質上是[零階保持](../Page/零階保持.md "wikilink")），再用*H*(*s*)
= 1/(*sT*)來積分DAC的輸出。

**三维投影**是将三维空间中的点映射到二维平面上的方法。由于目前绝大多数图形数据的显示方式仍是二维的，因此三维投影的应用相当广泛，尤其是在计算机图形学，工程学和[工程制图中](../Page/工程制图.md "wikilink")。
...
当视平面的法向（即摄像机的朝向）平行于[笛卡尔坐标系三根坐标轴中的一根](../Page/笛卡尔坐标.md "wikilink")，数学变换定义如下：
...
\[b_y = s_z a_z + c_z\]
其中向量**s**是一个任意的缩放因子，而**c**是一个任意的偏移量。这些常量可自由选择，通常用于将[视口调整到一个合适的位置](../Page/视口.md "wikilink")。该投影变换同样可以使用矩阵表示（为清晰起见引入临时向量**d**）
...
[透视投影的定义更为复杂](../Page/透视投影.md "wikilink")。可以将其理解为透过摄像机取景器对于被投影物体进行观察。摄像机的位置、朝向和[视野都将影响投影变换的结果](../Page/视野.md "wikilink")。我们定义以下变量来对这一变换进行描述：
...
首先我们定义点\(\mathbf{d}_{x,y,z}\)作为点\(\mathbf{a}\)向摄像机[坐标系所作的](../Page/坐标系.md "wikilink")[变换](../Page/变换.md "wikilink")，其中摄像机[坐标系由摄像机的位置](../Page/坐标系.md "wikilink")\(\mathbf{c}\)和旋转\(\mathbf{\theta}_{x,y,z}\)所决定。该过程为：先用\(\mathbf{a}\)[减去](../Page/矩阵加法.md "wikilink")\(\mathbf{c}\)，然后使用由\(-\mathbf{\theta}\)产生的[旋转矩阵乘上该结果](../Page/旋转矩阵.md "wikilink")。该变换通常称为摄像机变换(注意该计算过程假设使用[左手法则](../Page/笛卡尔坐标系#取向.md "wikilink"))：
\[2\] \[3\]
...
或在[齐次坐标系下可以表示为](../Page/齐次坐标系.md "wikilink")：

**不动点组合子**（，或**不动点算子**）是计算其他函数的一个[不动点的](../Page/不动点.md "wikilink")[高阶函数](https://zh.wikipedia.org/wiki/高阶函数 "wikilink")。
...
在[无类型lambda演算中众所周知的](https://zh.wikipedia.org/wiki/无类型lambda演算 "wikilink")（可能是最简单的）不动点组合子叫做**Y**组合子。它是[Haskell
B. Curry发现的](https://zh.wikipedia.org/wiki/Haskell_Curry "wikilink")，定义为
...
例如，在[Standard
ML中](https://zh.wikipedia.org/wiki/Standard_ML "wikilink")**Y**组合子的[传值调用变体有类型](https://zh.wikipedia.org/wiki/传值调用 "wikilink")∀a.∀b.((a→b)→(a→b))→(a→b)，而[传名调用变体有类型](https://zh.wikipedia.org/wiki/传名调用 "wikilink")∀a.(a→a)→a。传名调用（正则序）变体在应用于传值调用的语言的时候将永远循环下去
--
...
**Y**组合子用[SKI-演算表达为](../Page/SKI组合子演算.md "wikilink")
  ...
  -
    **Y** = S (K (S I I)) (S (S (K S) K) (K (S I
    ...
    Tromp发现](https://zh.wikipedia.org/wiki/John_Tromp "wikilink")，它是

[Incomplete_lower_gamma_3d.gif](https://zh.wikipedia.org/wiki/File:Incomplete_lower_gamma_3d.gif "fig:Incomplete_lower_gamma_3d.gif")
[Incomplete_lower_gamma_4d.gif](https://zh.wikipedia.org/wiki/File:Incomplete_lower_gamma_4d.gif "fig:Incomplete_lower_gamma_4d.gif")
[Incomplete_lower_gamma_5d.gif](https://zh.wikipedia.org/wiki/File:Incomplete_lower_gamma_5d.gif "fig:Incomplete_lower_gamma_5d.gif")
在[数学中](../Page/数学.md "wikilink")，**上不完全Γ函数**和**下不完全Γ函数**是[\(\Gamma\)函数的推广](https://zh.wikipedia.org/wiki/Gamma函数 "wikilink")。它们的定义分别如下：

\[\Gamma(s,x) = \int_x^{\infty} t^{s-1}\,e^{-t}\,{\rm d}t .\,\! \qquad \gamma(s,x) = \int_0^x t^{s-1}\,e^{-t}\,{\rm d}t .\,
\quad \Re(s)>0, x\in\mathbb R_0^+\]
通过[解析延拓可以将定义域拓展到](https://zh.wikipedia.org/wiki/解析延拓 "wikilink")
**C**×**C** （除去可数个奇点外），详见下文。
...
在上面定义的约束条件下，通过[分部积分](https://zh.wikipedia.org/wiki/分部积分 "wikilink")，可以计算得[递归关系](../Page/递归.md "wikilink")：

\[\Gamma(s,x)= (s-1)\Gamma(s-1,x) + x^{s-1} e^{-x}\] 以及反过来：

\[\gamma(s,x) =(s-1)\gamma(s-1,x) - x^{s-1} e^{-x}\]
因为正常的[\(\Gamma\)函数定义为](https://zh.wikipedia.org/wiki/Gamma函数 "wikilink")：
...
由[比值审敛法可知](../Page/比值审敛法.md "wikilink")，右边的级数的[收敛半径是无穷大](../Page/收敛半径.md "wikilink")。

由[魏尔斯特拉斯原理](https://zh.wikipedia.org/wiki/魏尔斯特拉斯原理 "wikilink")\[1\]，下式中的函数，有时记作\(\gamma^*\)，是关于
 和  的[整函数](../Page/整函数.md "wikilink")\[2\]。
...
的确给出了下不完全伽玛函数的一个解析延拓。其中前两个因子给出了下不完全伽玛函数的奇点（即=0 或
为非正整数），而后面的因子则给出了下不完全伽玛函数的零点。
...
在选定了 <sup></sup> 的单值分支之后，下不完全伽玛函数的积分定义式可以自然地拓展到
为任意复数的情形，只是此时该积分应该理解为复平面上的路径积分，且积分路径需避开单值分支间的割线。需注意的是此时仍然要求
 的实部大于 0，否则积分不收敛。
...
根据上面的讨论，下不完全伽玛函数有下列性质：
  ...
  - 当  不是整数时是  的多值[全纯函数](https://zh.wikipedia.org/wiki/全纯 "wikilink")，=0
    是其枝点；
  - 对于确定的 ，指定主分支后，下不完全伽玛函数是
    的[亚纯函数](../Page/亚纯函数.md "wikilink")，非正整数是其一阶极点\[4\]。
...
\[\Gamma(s,z)=\Gamma(s)-\gamma(s,z)\] 由于伽玛函数和下不完全伽玛函数关于 ,
都至少是亚纯函数，上式可以自然地作解析延拓，并以此作为上不完全伽玛函数的定义。下不完全伽玛函数的多值性自然地导致上不完全伽玛函数的多值性，下面的讨论基于主分支。

进一步地，由黎曼可去奇点原理\[5\]，由于等号右边在  取非正整数时的邻域内有界，故作为
的函数，非正整数是上不完全伽玛函数的可去奇点，可以通过对等号右边取极限来定义非正整数时上不完全伽玛函数的值。下面以
=0 为例来说明这种极限过程，其它情况可以类推得到。
...
上不完全伽玛函数和下不完全伽玛函数都可以用合流超几何函数表示，详见[合流超几何函数一文](https://zh.wikipedia.org/wiki/合流超几何函数 "wikilink")。
...
另一方面，不完全伽玛函数对  的偏导数是 \[\[Meijer_G-函数|Meijer {e^t} {\\rm d}t=
\\frac{\\partial^m}{\\partial s^m} \\Gamma (s,x) </math>

利用上式和 [Mellin
变换的性质](https://zh.wikipedia.org/wiki/Mellin_变换 "wikilink")，并作解析延拓，就可以得到上不完全伽玛函数对参变量的高阶偏导数的表达式。
...
<references />
  ...
  - G. Arfken and H. Weber. *Mathematical Methods for Physicists*.
    Harcourt/Academic Press, 2000. *(See Chapter 10.)*

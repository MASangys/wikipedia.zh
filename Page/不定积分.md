在[微积分中](../Page/微积分.md "wikilink")，一个[函数](../Page/函数.md "wikilink")\(f = f\)的**不定积分**，也称为**原函数**或**反导数**，是一个[导数等于](../Page/导数.md "wikilink")\(f= f\)的函数\(f = F\)，即\(f= F'\)。不定积分和[定积分间的关系由](../Page/定积分.md "wikilink")[微积分基本定理确定](../Page/微积分基本定理.md "wikilink")。

\[\int f(x)dx = F(x)+C\]

其中\(F\)是\(f\)的不定积分。这样，许多函数的[定积分的计算就可以简便地通过求不定积分来进行](../Page/定积分.md "wikilink")。
...
称为函数\(f= k\!\)的**原函数族**，也就是\(f= k(x)=2^x\!\)的所有可能的原函数的[集合](../Page/集合.md "wikilink")，其中\(f= C\!\)叫做**积分常数**。从图像上来看，这是\(f= K(x)=\frac{2^x}{\ln2}\!\)垂直平移后得到的一组函数，几何上的解释就是它们在\(f= x\!\)轴同一点上的斜率都是一样的。
...
**微积分基本定理**：如果\(f= f\)是闭区间\(f= [a,b]\)上的[连续函数](../Page/连续.md "wikilink")，\(f=F\)是\(f=f\)在\(f=[a,b]\)上的一个原函数，那么
...
因此，\(f= f\)的**原函数族**中的每个函数都可以叫做\(f= f\)的不定积分，简写作\(f= \int f(x)\, \mathrm{d}x.\)，因为在计算定积分时，积分常数在相减时消掉了。如果\(f= F\)定义在几个不同的[区间上](../Page/区间.md "wikilink")，那么每个区间上的积分常数可以互不相同。例如
...
什么样的函数具有原函数是微积分理论中的基本问题。首先，每个[连续函数都有原函数](../Page/连续.md "wikilink")，并且由上面可知，原函数的个数是无限个。其次，对于一个有原函数的函数，它的原函数族中在某点取某个特殊值的只有一个。特别来说，对某个点\(a\)，\(f\)恰有一个在\(a\)上取值为零的原函数，它可以表示为如下的**积分上限函数**：
...
许多看似很“简单”的函数的原函数是无法用[初等函数](../Page/初等函数.md "wikilink")（[指数函数](../Page/指数函数.md "wikilink")、[对数函数](../Page/对数函数.md "wikilink")、[代数函数](../Page/代数函数.md "wikilink")、[三角函数](../Page/三角函数.md "wikilink")、[反三角函数以及它们的有限次加減乘除開根號组合](../Page/反三角函数.md "wikilink")）来表达的（但它们一样存在！），比如说如下几个不定积分：
...
求初等函数的不定积分比求它们的导数要困难得多。如上面所看到的，有些初等函数的原函数无法用初等函数来表达。以下是求不定积分的一些技巧。
  ...
  - [分部积分法](../Page/分部积分法.md "wikilink")，用于函数乘积的积分。
  - 对于实值[分式函数的积分](../Page/分式.md "wikilink")，可以先将函数展开成若干一次分式函数以及二次分式函数的幂的和，再进行积分。
  - [Risch算法](../Page/Risch算法.md "wikilink")。
  - 对于常见的不定积分，可以查看**[积分表](../Page/积分表.md "wikilink")**
  - 当函数的不定积分不能用初等函数表达时，可以采用其他办法计算函数的定积分，比如[数值积分](../Page/数值积分.md "wikilink")。

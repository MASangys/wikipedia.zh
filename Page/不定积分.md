在[微积分中](https://zh.wikipedia.org/wiki/微积分 "wikilink")，[函数](../Page/函数.md "wikilink")
\(f\) 的**不定积分**（或稱**反導函數**或**原函数**)
，是一个[可微函數](https://zh.wikipedia.org/wiki/可微 "wikilink")
\(F\) 且其[微分等于原來的函數](../Page/微分.md "wikilink") \(f\)，即
\(F' = f\)。不定积分和[定积分间的关系由](https://zh.wikipedia.org/wiki/定积分 "wikilink")[微积分基本定理聯繫起來](../Page/微积分基本定理.md "wikilink")。
透過[微積分基本定理](https://zh.wikipedia.org/wiki/微積分基本定理 "wikilink")，函数的[定积分的计算就可以簡單地通过求不定积分来进行](https://zh.wikipedia.org/wiki/定积分 "wikilink")。

## 例子

函数 \(K(x)=\tfrac{2^x}{\ln2}\) 是函数 \(k(x)=2^x\!\) 的一个反導函數，但实际上 \(k\)
的反導函數有无穷多个。与\(K\) 相差一个常数的函数都是 \(k\)
的反導函數，這是因为常数函数的导数为零，例如：\(\tfrac{2^x}{\ln2}+\ln2,\ \tfrac{2^x}{\ln2}-e^\sqrt{\pi}\)
都為函數 \(k(x)\) 的反導函數。函数族 \(\{\tfrac{2^x}{\ln2}+c\;| \;c\in\mathbb{R} \}\)
是 \(k(x)=2^x\)
的所有可能的反導函數的[集合](https://zh.wikipedia.org/wiki/集合 "wikilink")，其中
\(c\) 叫做**积分常数**。从图像上来看，这是 \(K(x)=\tfrac{2^x}{\ln2}\)
向上或向下平移后得到的一组函数，由定義可知它们在 \(x\)
轴同一点的斜率都是一样的。
...
**微积分基本定理**：如果函數 \(f\)是闭区间 \([a,b]\)
上的[连续函数](https://zh.wikipedia.org/wiki/连续 "wikilink")，\(F\)
是 \(f\) 在 \([a,b]\) 上的一个反導函数，那么有

\[\int_{a}^{b} f(x)\,\mathrm{d}x = F(b) - F(a)\]

证明：取区间\([a,b]\)的一个分割：\(a = x_0 < x_1 < x_2 < \cdots < x_n = b\)，又设\(\Delta x_{i} = x_{i+1} - x_{i}\)，根据[均值定理有](https://zh.wikipedia.org/wiki/均值定理 "wikilink")
\(\xi_i\in (a,b)\)， 使得
...
\(f\) 在闭区间 \([a,b]\) 上连续，故[黎曼可积](../Page/黎曼积分.md "wikilink")，讓
\(\sup_{0\le i\le n-1} \{\Delta x_i\} \le\lambda\) 于是當
\(\lambda\to 0\)，也就是分割越來越細時有
...
\(f\) 的每个反導函数都可以叫做 \(f\)
的不定积分，简写作\(\int f(x)\, \mathrm{d}x.\)，因为在计算定积分时，积分常数在相减时消掉了。如果
\(F\)
定义在几个不同的[区间上](https://zh.wikipedia.org/wiki/区间 "wikilink")，那么每个区间上的积分常数可以互不相同。例如

\[F(x)=\begin{cases}-\frac{1}{x}+C_1\qquad x<0\\-\frac{1}{x}+C_2\qquad x>0\end{cases}\]

就是函數 \(f(x)=\tfrac{1}{x^2}\) 的不定积分的一般形式。其定義域為
\((-\infty,0)\cup(0,\infty)\)。

### 由積分定義的函數

什么样的函数具有反導函数是微积分基本定理中的基本问题。首先，每个[连续函数都有反導函数](https://zh.wikipedia.org/wiki/连续 "wikilink")，并且由上面可知，任一函數的反導函数如果存在的話會有无限多个。其次，由微分基本性質可知，对于一个有反導函数的函数，其反導函数在某点取某特定值的只有一个。要證明存在性，假設函數
\(f\) 的反導函數在 \(a\) 點为零，則它可以表示为如下的由积分定義的函数：
...
進一步可知：\(f\) 的反導函数中在点 \(a\)上取值为 \(A\)
的只有一个，就是\(\int_a^x f(t)\,\mathrm{d}t + A\)。
...
这个函数在0上不连续，但可以验证函数：\(F(x)=x^2\sin\tfrac{1}{x}\)（\(x\ne0\)时），\(\displaystyle F(0)=0\)
是 \(f\) 的反導函數。

许多看似很“简单”的函数的反導函數是无法用[初等函数](../Page/初等函数.md "wikilink")（[指数函数](../Page/指数函数.md "wikilink")、[对数函数](https://zh.wikipedia.org/wiki/对数函数 "wikilink")、[代数函数](https://zh.wikipedia.org/wiki/代数函数 "wikilink")、[三角函数](../Page/三角函数.md "wikilink")、[反三角函数以及它们的有限次加減乘除開根號组合](../Page/反三角函数.md "wikilink")）来表达的（但它们一样存在！），比如说如下几个不定积分：

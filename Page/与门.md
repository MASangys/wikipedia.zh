...
如果没有现成的与门，则可用[与非门](../Page/与非门.md "wikilink")或[或非门](../Page/或非门.md "wikilink")来实现，因为与非门和或非门被认为是“通用的逻辑门电路”\[1\]，因为通过这两种门电路可以实现其他所有门电路的功能。若用与非门实现与门，则将一个与非门的两个输入结在一起，然後在其输入端接入一个双输入的与非门；若用或非门实现与门，则将两个输入接在一起的或非门分别作另外一个或非门的两个输入，然後在後者的输入接一个输入接在一起的或非门。[异或门](../Page/异或门.md "wikilink")也可用来实现与门，但很少使用。

[BPMN-AProcesswithNormalFlow.svg](https://zh.wikipedia.org/wiki/File:BPMN-AProcesswithNormalFlow.svg "fig:BPMN-AProcesswithNormalFlow.svg")
**业务流程模型和标记法**（**BPMN**, **Business Process Model and
Notation**）是一套图形化表示法，用于以[业务流程模型详细说明各种](../Page/业务流程模型.md "wikilink")[业务流程](../Page/业务流程.md "wikilink")。

它最初由业务流程管理倡议组织（BPMI, Business Process Management
Initiative）开发，名称为"Business Process Modeling
Notation"，即“业务流程建模标记法”。BPMI于2005年与[对象管理组织](../Page/对象管理组织.md "wikilink")（OMG,
Object Management Group）合并。2011年1月OMG发布2.0版本，同时改为现在的名称。\[1\]

## 概述

业务流程模型和标记法（BPMN, Business Process Model and
Notation）是[对象管理组织](../Page/对象管理组织.md "wikilink")（OMG,
Object Management
Group）维护的关于的行业性标准。它建立在与[UML的](../Page/UML.md "wikilink")[活动图非常相似的](../Page/活动图.md "wikilink")[流程图法](../Page/流程图.md "wikilink")（flowcharting）基础上\[2\]，为“业务流程图”（BPD,
Business Process
Diagram）\[3\]中的特定[业务流程提供一套图形化标记法](../Page/业务流程.md "wikilink")。BPMN的目标是，通过提供一套既符合业务人员直观又能表现复杂流程语义的标记法，同时为技术人员和业务人员从事[业务流程管理提供支持](../Page/业务流程管理.md "wikilink")。BPMN规范还提供从标记法的图到执行语言基础构造的映射，尤其是（BPEL）\[4\]。

BPMN的首要目的是提供全体业务相关者易于理解的标准标记法。业务相关者包括创造与梳理流程的、负责实施流程的技术开发者、以及管理和监督流程的经理人。BPMN旨在充当公共语言，跨越业务流程设计和实施之间常见的鸿沟。

当前有多种竞争的业务流程建模语言标准供建模过程和工具选用\[5\]。广泛采用BPMN将有助于统一基本的业务流程概念的表达（例如：公共或私有的流程、编排），就像一些高级的业务概念一样（例如：[例外处理](../Page/例外处理.md "wikilink")、[事务补偿](../Page/事务补偿.md "wikilink")）。

## 历史

BPMN最初由业务流程管理倡议组织（BPMI, Business Process Management
Initiative）开发，该组织于2005年与[对象管理组织](../Page/对象管理组织.md "wikilink")（OMG,
Object Management Group）合并，从那时起，由OMG维护。BPMN最初的名称为"Business Process
Modeling Notation"，即“业务流程建模标记法”，2011年1月OMG发布2.0版本，同时改为现在的名称。\[1\]
...
此外，虽然BPMN会显示数据的流（消息）以及活动与数据器物的关联，但它并非[数据流图](../Page/数据流图.md "wikilink")（data
flow diagram）。
...
这四大类对象令我们有机会做出简单的业务流程图（BPD, business process
diagram）。同时，BPMN也允许在BPD中创建你自己的流对象、器物类型，使图更好理解。

### 流对象与连接对象

<center>

Image:BPMN-Event.jpg|事件 Event Image:BPMN-Activity.jpg|活动 Activity
Image:BPMN-Gateway.jpg|关口 Gateway Image:BPMN-Connecting.png|连接
Connections
...
“流对象”通过“**连接对象**”（Connecting objects）互相连接。连接对象包括三个类型（顺序、消息、关联）：
  ...
  - 顺序流（Sequence Flow）
    “顺序流”用实心线和箭头表示，显示活动进行的顺序。“顺序流”还可以在开始端带有符号，以小菱形标明其中一些发自活动的“条件流”（conditional
    flow），同时以对角斜线标明发自活动或决定，带条件流的“默认流”（default flow）。
  - 消息流（Message Flow）
...
Image:BPMN-Swimlanes.jpg|泳道 Swimlanes Image:BPMN-Data.jpg|数据对象 Data
objects Image:BPMN-Group.jpg|组 Groups Image:BPMN-Annotation.png|注释
Annotation
...
Image:BPMN-AProcesswithNormalFlow.jpg|带正常流的流程
Image:BPMN-DiscussionCycle.jpg|讨论循环
Image:BPMN-E-MailVotingProcess.jpg|电子邮件投票流程
Image:BPMN-CollectVotes.jpg|采集投票
...
BPMN
2.0为新的业务流程模型和标记法建立单一规范，对标记法、和交换格式做出界定。2.0版的名称有所修改，但仍会维持“BPMN”这个标志。特征包括：

  - 结合BPMN和（BPDM），形成单纯一致的语言
  - 使能在建模工具间交换业务流程模型及其布局，以保持语义完整性
  - 扩充BPMN以允许将模型配置与编排成独立或集成的模型
  - 支持模型上不同透视法的显示和交替，令用户可以聚焦于特定的关注点
  - 串行化BPMN，为模型转换提供[XML方案](../Page/XML.md "wikilink")（schemes），向执行方向扩展业务模型。
...
1.  [BPMN Information Home Page](http://www.bpmn.org/) OMG information
    page for BPMN.
2.  [BPMN 1.1
    Poster](http://bpt.hpi.uni-potsdam.de/pub/Public/BPMNCorner/BPMN1_1_Poster_EN.pdf)
    - Poster showing all BPMN constructs
...
2.  [*Process Modeling Notations and Workflow
    Patterns*](http://www.bpmn.org/Documents/Notations%20and%20Workflow%20Patterns.pdf)
    , paper by Stephen A. White of IBM Corporation (2006)

3.  [*An XML Representation for Crew
    Procedures*](http://ntrs.nasa.gov/archive/nasa/casi.ntrs.nasa.gov/20050202022_2005202152.pdf),
    Richard C. Simpson (2004), Final Report NASA Faculty Fellowship
    Program (Johnson Space Center)

4.  [*Business Process Modeling
    Notation*](http://www.bpmn.org/Documents/BPMN%20V1-0%20May%203%202004.pdf)
    , specification of BPMN v1.0 by Stephen A. White (3 May 2004), for

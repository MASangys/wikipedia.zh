...
**中心极限定理**是[概率论中的一组定理](../Page/概率论.md "wikilink")。中心极限定理说明，在适当的条件下，大量相互独立[随机变量的均值经适当标准化后](../Page/随机变量.md "wikilink")[依分布收敛于](../Page/依分布收敛.md "wikilink")[正态分布](../Page/正态分布.md "wikilink")。这组定理是[数理统计学和误差分析的理论基础](../Page/数理统计学.md "wikilink")，指出了大量随机变量之和近似服从正态分布的条件。
...
棣莫佛－拉普拉斯（de Moivre -
Laplace）定理是中央极限定理的最初版本，讨论了服从[二项分布的随机变量序列](../Page/二项分布.md "wikilink")。它指出，参数为*n*,
*p*的二项分布以*np*为均值、*np(1-p)*为方差的正态分布为极限。

### 内容

若 \(X \sim B(n,p)\) 是 \(n\) 次[伯努利实验中事件](../Page/伯努利試驗.md "wikilink") *A*
出现的次数，每次試驗成功的機率為 \(p\)，且 \(q=1-p\)，则对任意有限区间 \([a,b]\)：

令\(x_k \equiv \frac{k-np}{\sqrt{npq}}\)，當\(n\to{\infty}\)时

(i)
\(P(X=k) \to \frac{1}{\sqrt{npq}} \cdot \frac{1}{\sqrt{2\pi}} e^{-\frac{1}{2}x^{2}_{\mu_n}}\)

(ii)
\(P(a \le{\frac{X-np}{\sqrt{npq}}} \le{b})\to\int_a^b\varphi(x) dx\)，其中\(\varphi(x)=\frac{1}{\sqrt{2\pi}}e^{-\frac{x^2}{2}}(-\infty<x<\infty).\)
...
[Clt_in_action.gif](https://zh.wikipedia.org/wiki/File:Clt_in_action.gif "fig:Clt_in_action.gif")
[林德伯格](../Page/林德伯格.md "wikilink")－[列维](../Page/列维.md "wikilink")（Lindeberg-Levy）定理，是棣莫佛－拉普拉斯定理的扩展，讨论独立同分布随机变量序列的中央极限定理。它表明，独立同分布(iid
independent and indentically
distributed)、且数学期望和方差有限的随机变量序列的标准化和以标准正态分布为极限：

### 内容

设随机变量\(X_1,X_2,\cdots,X_n\)独立同分布，
且具有有限的[数学期望和](../Page/数学期望.md "wikilink")[方差](../Page/方差.md "wikilink")\(E(X_i) = \mu\)，\(D(X_i) = \sigma^2 \neq 0  (i=1,2,\cdots,n)\)。记

\(\bar{X}=\frac{1}{n}\sum_{i=1}^{n}X_{i}\)，\(\zeta_n=\frac{\bar{X} -\mu}{\sigma/\sqrt{n}}\)，则
...
由于\(e^{-t^2/2}\)是连续函数，它对应的分布函数为\(\Phi(Z)\)，因此由[逆极限定理知](../Page/逆极限定理.md "wikilink")
...
令\(k_n={\rm arg}\max_{1\leq k\leq n}s_k^2/\sigma_n^2\)，则由[李雅普诺夫不等式](../Page/李雅普诺夫不等式.md "wikilink")，

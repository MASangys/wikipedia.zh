...
[1D_MITMNEW.png](https://zh.wikipedia.org/wiki/File:1D_MITMNEW.png "fig:1D_MITMNEW.png")
假設\(ENC\)是加密函式，\(DEC\)是解密函式，也就是\(ENC^{-1}\)，而\(k_1\)與\(k_2\)為兩次加密用的秘鑰，則可以推導出：
...
這也是為什麼[三重資料加密演算法](https://zh.wikipedia.org/wiki/三重資料加密演算法 "wikilink")（3DES）使用了三把56
bits的秘鑰（168 bits），卻只有兩把秘鑰的強度（112 bits）。

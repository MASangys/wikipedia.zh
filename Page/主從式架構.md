**主從式架構** ()
也称**客户端-服务器（Client/Server）架构**、**C/S架构**，是一种[网络架构](https://zh.wikipedia.org/wiki/网络架构 "wikilink")，它把[客户端](https://zh.wikipedia.org/wiki/客户端 "wikilink")
(Client) （通常是一个采用[图形用户界面的程序](../Page/图形用户界面.md "wikilink")）与服务器 (Server)
区分开来。每一个客户端软件的实例都可以向一个服务器或[应用程序服务器发出请求](https://zh.wikipedia.org/wiki/应用程序服务器 "wikilink")。有很多不同类型的服务器，例如[文件服务器](https://zh.wikipedia.org/wiki/文件服务器 "wikilink")、[游戏服务器等](https://zh.wikipedia.org/wiki/游戏服务器 "wikilink")。
...
[UML](https://zh.wikipedia.org/wiki/UML "wikilink") 中的一個標準。

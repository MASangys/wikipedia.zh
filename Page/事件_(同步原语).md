**事件**作为一种[同步原语](https://zh.wikipedia.org/wiki/同步 "wikilink")，是[计算机科学中的一种同步机制](../Page/计算机科学.md "wikilink")，用来指示等待中的进程特定条件已经变为真。
...
Windows
API还有一个函数**PulseEvent**，会把一个正在等待该事件对象的线程变成可调度状态，事件对象随即保持未触发状态；如果函数调用时没有等待该事件对象的线程那么等效于什么都不做。可见，PulseEvent并不等价于触发自动重置事件对象。[MSDN已经指出不要使用PulseEvent](https://zh.wikipedia.org/wiki/MSDN "wikilink")，\[1\]因为无法保证一个正在等待该事件对象的线程一定会被唤醒，其根源是挂起的线程可被系统调度执行内核级[异步过程调用](https://zh.wikipedia.org/wiki/异步过程调用 "wikilink")，这种事无法预测也无法察觉，此时发生PulseEvent就无法唤醒这个正在执行内核级[异步过程调用的线程](https://zh.wikipedia.org/wiki/异步过程调用 "wikilink")。

## C++语言标准

C++语言标准已经实现了这种并发编程的同步事件。在[C++11的](../Page/C++11.md "wikilink")[STL中增加了](https://zh.wikipedia.org/wiki/STL "wikilink")[future与](https://zh.wikipedia.org/wiki/b:C++/STL/Future "wikilink")[promise两种类模板](https://zh.wikipedia.org/wiki/b:C++/STL/Future "wikilink")，分别表示同步事件的消费者与生产者。

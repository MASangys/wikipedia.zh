**事务隔离**（）定义了[数据库](../Page/数据库.md "wikilink")系统中一个操作的结果在何时以何种方式对其他[并发操作可见](https://zh.wikipedia.org/wiki/并发 "wikilink")。隔离是事务[ACID](../Page/ACID.md "wikilink")（原子性、一致性性、隔离性、持久性）四大属性之一。

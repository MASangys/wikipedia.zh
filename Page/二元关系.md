...
设\(A = \{x_1, x_2, \ldots, x_n\}\)，\(R\)是\(A\)上的关系，令[图](https://zh.wikipedia.org/wiki/图 "wikilink")\(G=(V, E)\)，其中顶点集合\(V = A\)，边集合为\(E\)，且对于任意的\(x_i, x_j \in V\)，满足\((x_i, x_j) \in E\)当且仅当\((x_i, x_j) \in R\)。则称图\(G\)是关系\(R\)的**关系图**，记作\(G_R\)。

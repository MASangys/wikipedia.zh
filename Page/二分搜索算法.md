在[计算机科学中](../Page/计算机科学.md "wikilink")，**二分搜索**（），也称**折半搜索**（）\[1\]、**对数搜索**（），是一种在[有序数组中查找某一特定元素的搜索](https://zh.wikipedia.org/wiki/有序数对 "wikilink")[演算法](https://zh.wikipedia.org/wiki/演算法 "wikilink")。搜索过程从数组的中间元素开始，如果中间元素正好是要查找的元素，则搜索过程结束；如果某一特定元素大于或者小于中间元素，则在数组大于或小于中间元素的那一半中查找，而且跟开始一样从中间元素开始比较。如果在某一步骤数组为空，则代表找不到。这种搜索算法每一次比较都使搜索范围缩小一半。

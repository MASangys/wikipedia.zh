[binary_tree.svg](https://zh.wikipedia.org/wiki/File:binary_tree.svg "fig:binary_tree.svg")
[Binary_tree.png](https://zh.wikipedia.org/wiki/File:Binary_tree.png "fig:Binary_tree.png")
在[電腦科學中](https://zh.wikipedia.org/wiki/電腦科學 "wikilink")，**二元樹**（）是每個節點最多只有兩個分支（即不存在分支度大於2的節點）的[樹結構](https://zh.wikipedia.org/wiki/樹結構 "wikilink")。通常分支被稱作“左子樹”或“右子樹”。二元樹的分支具有左右次序，不能随意顛倒。

二元樹的第\(i\)層至多擁有\(2^{i-1}\)個節點；深度為\(k\)的二元樹至多總共有\(2^\begin{align} k \end{align}-1\)個節點（定义根节点所在深度
\(k_0=0\)），而總計擁有節點數符合的，稱為「滿二元樹」；深度為\(k\)有\(n\)個節點的二元樹，當且僅當其中的每一節點，都可以和同樣深度\(k\)的滿二元樹，序號為1到\(n\)的節點一對一對應時，稱為[完全二元樹](https://zh.wikipedia.org/wiki/完全二元樹 "wikilink")。對任何一棵非空的二元樹\(T\)，如果其葉片（終端節點）數為\(n_0\)，分支度為2的節點數為\(n_2\)，則\(n_0 = n_2 + 1\)。

與普通樹不同，普通樹的節點個數至少為1，而二元樹的節點個數可以為0；普通樹節點的最大分支度沒有限制，而二元樹節點的最大分支度為2；普通樹的節點無左、右次序之分，而二元樹的節點有左、右次序之分。

二元樹通常作為資料結構應用，典型用法是對節點定義一個標記函數，將一些值與每個節點相關聯。這樣標記的二元樹就可以實現[二元搜尋樹和](../Page/二元搜尋樹.md "wikilink")[二元堆積](https://zh.wikipedia.org/wiki/二元堆積 "wikilink")，並應用於高效率的搜索和排序。

## [圖論中的定義](https://zh.wikipedia.org/wiki/圖論 "wikilink")

[二元樹是一個連通的無環圖](https://zh.wikipedia.org/wiki/二元樹 "wikilink")，並且每一個頂點的度不大於3。有根[二元樹還要滿足根節點的度不大於](https://zh.wikipedia.org/wiki/二元樹 "wikilink")2。有了根節點之後，每個頂點定義了唯一的父節點，和最多2個子節點。然而，沒有足夠的資訊來區分左節點和右節點。如果不考慮[連通性](https://zh.wikipedia.org/wiki/連通性 "wikilink")，允許圖中有多個[連通分量](https://zh.wikipedia.org/wiki/連通分量 "wikilink")，這樣的結構叫做[森林](../Page/森林.md "wikilink")。

## 二元樹(Binary Tree)的類型

[二元樹是一個有根](https://zh.wikipedia.org/wiki/二元樹 "wikilink")[樹](https://zh.wikipedia.org/wiki/樹_\(資料結構\) "wikilink")，並且每個節點最多有2個子節點。非空的[二叉樹](https://zh.wikipedia.org/wiki/二叉樹 "wikilink")，若樹葉總數為
n0，分支度為2的總數為 n2，則 n0 = n2 + 1。

[BinaryTree_leaf.jpg](https://zh.wikipedia.org/wiki/File:BinaryTree_leaf.jpg "fig:BinaryTree_leaf.jpg")

一棵深度為k，且有\(2^\begin{align} k+1 \end{align}-1\)個節點的[二元樹](https://zh.wikipedia.org/wiki/二元樹 "wikilink")，稱為[滿二元樹](https://zh.wikipedia.org/wiki/滿二元樹 "wikilink")（Full
Binary
Tree）。這種樹的特點是每一層上的節點數都是最大節點數。而在一棵[二元樹中](https://zh.wikipedia.org/wiki/二元樹 "wikilink")，除最後一層外，若其餘層都是滿的，並且最後一層或者是滿的，或者是在右邊缺少連續若干節點，則此[二元樹為](https://zh.wikipedia.org/wiki/二元樹 "wikilink")[完全二元樹](https://zh.wikipedia.org/wiki/完全二元樹 "wikilink")（Complete
Binary
Tree）。具有n個節點的[完全二元樹的深度為](https://zh.wikipedia.org/wiki/完全二元樹 "wikilink")\(log_2n+1\)。深度為k的[完全二元樹](https://zh.wikipedia.org/wiki/完全二元樹 "wikilink")，至少有\(2^\begin{align} k-1 \end{align}\)個節點，至多有\(2^\begin{align} k \end{align}-1\)個節點。
...
[eclb.jpg](https://zh.wikipedia.org/wiki/File:eclb.jpg "fig:eclb.jpg")
在使用[記錄或](https://zh.wikipedia.org/wiki/記錄 "wikilink")[記憶體位址指標的程式設計語言中](https://zh.wikipedia.org/wiki/指標_\(資訊學\) "wikilink")，二叉樹通常用樹結點結構來存儲。有時也包含指向唯一的父節點的指標。如果一個結點的子結點個數小於2，一些子結點指針可能為空值，或者為特殊的[哨兵結點](https://zh.wikipedia.org/wiki/哨兵 "wikilink")。
...
[3clb.jpg](https://zh.wikipedia.org/wiki/File:3clb.jpg "fig:3clb.jpg")
...
以上的[遞迴演算法使用與樹的高度成比例的棧空間](https://zh.wikipedia.org/wiki/遞迴 "wikilink")。如果我們在每個結點中存儲指向父結點的指標，那樣可以使用[反覆運算演算法](https://zh.wikipedia.org/wiki/反覆運算 "wikilink")，只使用常數空間實現所有這些遍歷。然而，指向父結點的指標佔用更多的空間。這只在需要指向父節點的指標或棧空間有限時才使用。例如，
這是一個中序遍歷的反覆運算演算法：
...
和深度優先遍歷不同，廣度優先遍歷會先訪問離根節點最近的節點。參見[廣度優先搜索](https://zh.wikipedia.org/wiki/廣度優先搜索 "wikilink")。
二叉樹的廣度優先遍歷又稱按層次遍歷。演算法借助佇列實現。
...
線索二叉樹（保留遍歷時結點在任一序列的前驅和後繼的資訊）：若結點有左子樹，則其lchild域指示其左孩子，否則令lchild域指示其前驅；若結點有右子樹，則其rchild域指示其右孩子，否則令rchild指示其後繼。還需在結點結構中增加兩個標誌域LTag和RTag。LTag=0時，lchild域指示結點的左孩子，LTag=1時，lchild域指示結點的前驅；RTag=0時，rchild域指示結點的右孩子，RTag=1時，rchild域指示結點的後繼。以這種結點結構構成的二叉鏈表作為二叉樹的存儲結構，叫做線索鏈表，其中指向結點前驅和後繼的指標叫做線索，加上線索的二叉樹稱為線索二叉樹。對二叉樹以某種次序遍歷使其變為線索二叉樹的過程叫做線索化。若對二叉樹進行中序遍歷，則所得的線索二叉樹稱為中序線索二叉樹，線索鏈表稱為為中序線索鏈表。線索二叉樹是一種物理結構。
[<File:tbt1.jpg>](https://zh.wikipedia.org/wiki/File:tbt1.jpg "fig:File:tbt1.jpg")
...
## 外部链接
  ...
  - [binary trees](http://www.findstat.org/BinaryTrees) entry in the
    [FindStat](http://www.findstat.org/)数据库
  - [Gamedev.net introduction on binary
    trees](http://www.gamedev.net/page/resources/_/technical/general-programming/trees-part-2-binary-trees-r1433)
  - [二叉树归纳证明](http://www.brpreiss.com/books/opus4/html/page355.html)
  - [Balanced binary search tree on array How to create bottom-up an
    Ahnentafel list, or a balanced binary search tree on
    array](http://piergiu.wordpress.com/2010/02/21/balanced-binary-search-tree-on-array/)
  - [Binary trees and Implementation of the same with working code
    examples](http://www.cpphub.com/search/label/Binary%20trees)

...
\[q(x,y,z) = ax^2 + by^2 + cz^2 + dxy + exz + fyz\] 其中*a*, ..., *f*是系数。\[1\] 注意一般的[二次函数](../Page/二次函数.md "wikilink")和[二次方程](../Page/二次方程.md "wikilink")不是二次形式的例子，因为它们不总是[齐次的](https://zh.wikipedia.org/wiki/齐次多项式 "wikilink")。
...
1.  对于在[整数](../Page/整数.md "wikilink")环上定义的二次型的系数的要求，有两大传统。其中一个传统要求任何在[整数](../Page/整数.md "wikilink")环上定义的二次型中，任何项的系数都是整数（可以是任何整数）。另外，一个自[高斯以来的另一个传统要求任何在](https://zh.wikipedia.org/wiki/高斯 "wikilink")[整数](../Page/整数.md "wikilink")环上定义的二次型中，除了任何项的系数都必须是整数外，还要求任何涉及两个不同变量相乘的项的系数都必须是偶数。换句话说，按照后一传统，二元二次型中*xy*的系数*b*被替换为2*b*（其中新系数2*b*中的*b*是任何整数），而三元二次型中*xy*的系数*d*，*xz*的系数*e*以及*yz*的系数*f*分别被替换为2*d*、2*e*以及2*f*（其中新系数2*d*、2*e*以及2*f*中的*d*、*e*及*f*是任何整数）。换句话说，按照后一传统，对于任何在[整数](../Page/整数.md "wikilink")环上定义的二次型，与该二次型相对应的对称线性型的矩阵中的所有元素都必须是整数。两种传统都能在文献中找到。
...
由于二维游戏的图像都是[平面](../Page/平面.md "wikilink")结构，所以图像引擎在显示图像之前也往往只对图像本身进行处理，而不像[三维游戏那样还要处理图像周围的环境](https://zh.wikipedia.org/wiki/三维图像引擎 "wikilink")。这种处理一般是将图像伸缩，变形，色彩处理，图像合成等。图像引擎还可分为以下几个部分。

...
游戏引擎的程序基础是建立在使用图形[API](https://zh.wikipedia.org/wiki/API "wikilink")[函数基础之上的](../Page/函数.md "wikilink")，而不是直接控制[显卡](https://zh.wikipedia.org/wiki/显卡 "wikilink")，主要适用于2D图像的API是[DirectX](../Page/DirectX.md "wikilink")，而纯2D图形API往往不能提供更丰富的图像操作支持，一些引擎也使用用于3D图像的API如[OpenGL](../Page/OpenGL.md "wikilink")，[Direct3D等来模拟](../Page/Direct3D.md "wikilink")2D。

### 引擎组成

由于二维游戏的图像都是[平面结构](../Page/平面.md "wikilink")，所以图像引擎在显示图像之前也往往只对图像本身进行处理，而不像[三维游戏那样还要处理图像周围的环境](https://zh.wikipedia.org/wiki/三维图像引擎 "wikilink")。这种处理一般是将图像伸缩，变形，色彩处理，图像合成等。图像引擎还可分为以下几个部分。

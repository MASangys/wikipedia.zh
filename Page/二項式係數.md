...
二項式係數亦有不同的[符號表達方式](https://zh.wikipedia.org/wiki/排列組合符號 "wikilink")，包括：C(*n*, *k*)、<sub>*n*</sub>*C*<sub>*k*</sub>、<sup>*n*</sup>*C*<sub>*k*</sub>、\(C^{k}_{n}\)、\(C^{n}_{k}\)\[3\]，其中的 C 代表組合（combinations）或選擇（choices）。很多計算機使用含有 C 的變種記號，使得算式只佔一行的空間，相同理由也發生在[置換](../Page/置換.md "wikilink")數 \(P_k^n\)，例如寫作 P(*n*, *k*)。
...
若將*n*換成任意數值（負數、實數或複數）*α*，甚至是在任何能為正整數給出[逆元素](../Page/逆元素.md "wikilink")的[交換環中的一元素](https://zh.wikipedia.org/wiki/交換環 "wikilink")，則二項式係數可籍乘數公式擴展\[4\]：
...
[帕斯卡法則](../Page/帕斯卡法則.md "wikilink")是一重要的[遞歸等式](https://zh.wikipedia.org/wiki/遞歸 "wikilink")：  此式可以用於[數學歸納法](https://zh.wikipedia.org/wiki/數學歸納法 "wikilink")，以証明\(\tbinom n k\)對於所有*n*和*k*均為自然數（等同於証明k\!為所有k個連續整數之積的因數），此特性並不易從[公式(1)中得出](https://zh.wikipedia.org/wiki/#定義及概念 "wikilink")。
...
第*n*橫行列出\(\tbinom n k\)的*k* = 0,…,*n*項，其建構方法為在外邊填上1，然後將上一行中每兩個相鄰數相加的和填在其下，此方法可快速地計算二項式係數而不涉及乘法或分數，例如從第5橫行可馬上得出
...
就任意*k*，多項式\(\tbinom{t}{k}\)可看成是惟一的*k*次多項式*p*(*t*)滿足*p*(0) = *p*(1) = ... = *p*(*k* − 1) = 0 及 *p*(*k*) = 1.
...
在任何包含[**Q**的](https://zh.wikipedia.org/wiki/有理數 "wikilink")[域中](../Page/域_\(數學\).md "wikilink")，最多*d*階的多項式有惟一的線性組合\(\sum_{k=0}^d a_k \binom{t}{k}\)。係數*a*<sub>*k*</sub>是數列*p*(0), *p*(1), …, *p*(*k*)的[第*k*差分](../Page/差分.md "wikilink")，亦即： \[6\]
...
从t=1,2,3时 3*t*(3*t* + 1)/2 =6,21,45用[帕斯卡矩阵](../Page/帕斯卡矩阵.md "wikilink")的[逆可算出](../Page/逆矩阵.md "wikilink")：

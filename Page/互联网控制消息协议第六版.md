...
ICMPv6协议定义在[OSI模型中的](../Page/OSI模型.md "wikilink")[网络层](../Page/网络层.md "wikilink")（），用于报告IPv6节点数据包处理过程中的错误消息和完成一些网络诊断功能（如[ping和](../Page/ping.md "wikilink")[traceroute等](../Page/traceroute.md "wikilink")）。它是IPv6体系总体的的一个组成部分，其基本的协议\[1\]必须被所有IPv6实现和IPv6节点所完整支持。

除了ICMPv4原有的功能，ICMPv6还承担了IPv4其他协议所实现的功能或IPv6所需的新功能，例如[邻居发现协议](../Page/邻居发现协议.md "wikilink")（同时取代了[ARP](../Page/地址解析协议.md "wikilink")/[RARP](../Page/逆地址解析协议.md "wikilink")，[SEND作为其安全扩展](../Page/安全邻居发现.md "wikilink")）、多播管理（取代了[IGMP](../Page/因特网组管理协议.md "wikilink")）。

## 数据包格式

[IANA](../Page/IANA.md "wikilink")（因特网地址授权委员会）定义ICMPv6的协议号为58。
...
ICMPv6包有类型（Type），代码（Code），[校验和](../Page/校验和.md "wikilink")（Checksum）和消息体（Message
Body）等几个字段。**类型**域为8位，用来表示消息的类型，如果[最高有效位是](../Page/最高有效位.md "wikilink")0（即值的范围是0-127），则表示一个错误消息；如果是1，则该ICMPv6包是一个信息消息。**代码**域同样为8位，其内容取决于消息类型，用来区分某一类型的多条消息。**校验和**用来检测ICMP消息和IPv6包的错误。**消息体**的内容取决于消息类型。
...
ICMPv6 包的校验和是一个 16
位的[二进制](../Page/二进制.md "wikilink")[补码](../Page/补码.md "wikilink")，计算消息类型域开始的整个
...
1.  如果消息是对发送到其中一个[单播](../Page/单播.md "wikilink")（unicast）地址消息的回应，则使用该地址作为源地址。
2.  如果消息是对发送到其他地址消息的回应，如：
      - [多播](../Page/多播.md "wikilink")（multcast）组地址
      - 该节点实现的[选播](../Page/选播.md "wikilink")（anycast）地址
      - 其他节点的单播地址

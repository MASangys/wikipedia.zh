...
我们保持轮数为1轮，由证明者发起，而将上面的**PTM**换作概率[多项式时间图灵机](https://zh.wikipedia.org/wiki/多项式时间 "wikilink")，那么我们得到了复杂性类**MA**:验证者是一个在概率[多项式时间的图灵机](https://zh.wikipedia.org/wiki/多项式时间 "wikilink")**(亚瑟)**，证明者**（梅林）**给出对于问题的解之后验证者必须在1/3的错误率以内决定*n*是否在这个语言之内。
更正式的说：
...
在[计算复杂性理论内](https://zh.wikipedia.org/wiki/计算复杂性理论 "wikilink")，**IP**是由以下特定**交互式证明系统**所能解决的一类问题：验证者是一个在概率[多项式时间的图灵机](https://zh.wikipedia.org/wiki/多项式时间 "wikilink")，双方可以交换多项式\(p(n)\)次信息，最后验证者必须在1/3的错误率以内决定*n*是否在这个语言之内。（所以在**[BPP](https://zh.wikipedia.org/wiki/BPP "wikilink")**内的语言一定在**IP**之内，因为我们可以让验证者直接忽略证明者然后自己对这问题作决定。）
更正式的说：
...
\=== IP = PSPACE ===
要证明**IP**与**[PSPACE](../Page/PSPACE.md "wikilink")**相等，我们先证明出**IP**是**PSPACE**的子集，然后证明**PSPACE**也是**IP**的子集，因此之故两个集合相等。要证明出\(\text{IP} \subseteq \text{PSPACE}\)，我们展现出一个用多项式空间的机器可以怎样模拟一个交互式证明系统。要证明\(\text{PSPACE} \subseteq \text{IP}\)这一件事，我们推导一个**PSPACE**-完全语言[TQBF是在](https://zh.wikipedia.org/wiki/TQBF "wikilink")**IP**里面。这两个部份的证明均是由Sipser提出的。
...
**NEXPTIME**，这类问题是在*指数时间*之内以[非决定性解的出来的问题](https://zh.wikipedia.org/wiki/非决定性图灵机 "wikilink")，这是一个非常大的类别。此外，在**MIP**系统内，即使不做任何多余的假设，所有的**NP**语言均有[零知识证明](https://zh.wikipedia.org/wiki/零知识证明 "wikilink")；在IP里面唯有假设存在单向函式才可能成立。

## IPP

**IPP**（*unbounded IP*）是
**IP**的一种变体，将原来的**[BPP](https://zh.wikipedia.org/wiki/BPP "wikilink")**检验者换成**[PP](https://zh.wikipedia.org/wiki/PP_\(complexity\) "wikilink")**检验者。更精确的说，我们将完备性跟可靠性的条件修改如下：
...
虽然**IPP**仍旧与**PSPACE**相等，但是**IPP**协议系统在涉及[启示图灵机的情况下与](https://zh.wikipedia.org/wiki/启示图灵机 "wikilink")**IP**的状况差异颇大：对所有的启示图灵机**IPP**=**PSPACE**，但是几乎对所有的启示图灵机，**IP**
≠ **PSPACE**。\[1\]

## QIP

**QIP**是将**IP**的**[BPP](https://zh.wikipedia.org/wiki/BPP "wikilink")**检验者换成一个**[BQP](https://zh.wikipedia.org/wiki/BQP "wikilink")**检验者所产生的变体，说更明白些，**BQP**是可以用[量子计算机](../Page/量子计算机.md "wikilink")在多项式时间内解决的问题类别。并且，这一些讯息是用[量子位所表示的](https://zh.wikipedia.org/wiki/量子位 "wikilink")。\[2\]在2009年,
Jain, Ji,
Upadhyay，和Watrous证明了**QIP**也与**PSPACE**相等，\[3\]总结起以上Kitaev和Watrous的理论，我们得到**QIP**包含在**[EXPTIME](https://zh.wikipedia.org/wiki/EXPTIME_\(复杂度\) "wikilink")**内的结论，因为**QIP**

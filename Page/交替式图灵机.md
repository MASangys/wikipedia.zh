**交替式图灵机**（,
ATM）是[计算复杂度理论中定义的一种](https://zh.wikipedia.org/wiki/计算复杂度理论 "wikilink")[非确定型图灵机](../Page/非确定型图灵机.md "wikilink")（NTM）。与一般非确定型图灵机不同，交替式图灵机将接受语言的规则一般化到[NP和](https://zh.wikipedia.org/wiki/NP_\(复杂度\) "wikilink")[反NP](../Page/反NP.md "wikilink")。交替式图灵机的概念由Chandra和于1976年提出。
...
例如，考虑以下[电路最小化问题](https://zh.wikipedia.org/wiki/电路最小化问题 "wikilink")：给定一个能够计算[布尔函数](../Page/布尔函数.md "wikilink")
\(f\) 的电路 \(A\) 和一个整数
\(n\)，确定是否存在一个最多包含\(n\)个[门的电路](https://zh.wikipedia.org/wiki/逻辑门 "wikilink")\(B\)可以计算\(f\)。一台2次交替图灵机从一个存在状态出发可以在多项式时间内解决这个问题（在存在状态通过猜测电路
...
也许交替式图灵机解决的最简单的问题是[量词布尔公式问题](https://zh.wikipedia.org/wiki/量词布尔公式问题 "wikilink")。这一问题是[布尔可满足性问题的扩充](../Page/布尔可满足性问题.md "wikilink")，即每个变量可以被一个[全称量词或](https://zh.wikipedia.org/wiki/全称量词 "wikilink")[存在量词所约束](https://zh.wikipedia.org/wiki/存在量词 "wikilink")。交替式图灵机依照约束的次序对每一个存在量词寻找一个可能的赋值，对每一个全称量词寻找所有的赋值。在对所有约束变量确定值之后，交替式图灵机根据剩余的布尔公式确定接受还是拒绝。因此，接受一个存在量词意味着存在一个赋值，使得赋值后的量词布尔公式可满足。类似地，接受一个全称量词意味着对任何一个赋值，赋值后的量词布尔公式可满足。
...
这些定义与确定性图灵机给定的[P](https://zh.wikipedia.org/wiki/P_\(复杂度\) "wikilink")、[PSPACE和](../Page/PSPACE.md "wikilink")[EXPTIME在空间上类似](../Page/EXPTIME.md "wikilink")。对于两种计算模型导出的两类复杂度类，Chandra、Kozen和Stockmeyer证明了以下定理：

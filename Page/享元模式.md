**享元模式**（英語：Flyweight
Pattern）是一種軟體[設計模式](../Page/設計模式_\(計算機\).md "wikilink")。它使用共享物件，用來儘可能減少記憶體使用量以及分享資訊給儘可能多的相似物件；它適合用於當大量物件只是重複因而導致無法令人接受的使用大量記憶體。通常物件中的部分狀態是可以分享。常見做法是把它們放在外部資料結構，當需要使用時再將它們傳遞給享元。

典型的享元模式的例子為[文書處理器中以圖形結構來表示字符](../Page/文書處理器.md "wikilink")。一個做法是，每個[字形有其字型外觀](../Page/字形.md "wikilink"),
字模 metrics,

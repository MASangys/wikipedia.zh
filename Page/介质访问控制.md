**-{zh-cn:媒体接入控制;zh-tw:媒體存取控制;}-**（，縮寫：**MAC**）子層，是[局域网](../Page/局域网.md "wikilink")中[数据链路层](../Page/数据链路层.md "wikilink")的下层部分，提供定址及媒體存取的控制方式，使得不同設備或網路上的節點可以在多點的網路上通訊，而不會互相衝突，上述的特性在[局域网](../Page/局域网.md "wikilink")或者[城域网中格外重要](https://zh.wikipedia.org/wiki/城域网 "wikilink")。早期網路發展時以MAC判別個網路介面之位置，但後來網際網路發展後，才有IP之制定與使用。若只是兩台設備之間[全雙工的通訊](https://zh.wikipedia.org/wiki/全雙工 "wikilink")，因為兩台設備可以同時傳送及接收資料，不會衝突，因此不需要用到MAC協定。

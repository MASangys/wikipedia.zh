作为[数学的一个分支](../Page/数学.md "wikilink")，在[泛函分析中](../Page/泛函分析.md "wikilink")，[向量空间子集的](../Page/向量空间.md "wikilink")**代数内部**()或**径向核**()是对[内部概念的细化](../Page/内部.md "wikilink")。
它是给定集合相对于该点是[吸收的的点构成的子集](../Page/吸收集.md "wikilink")，即集合的[径向点构成的集合](../Page/径向集.md "wikilink")。\[1\]代数内部的元素通常被称为**内点**()。 \[2\]\[3\]
...
令\(A,B \subset X\)则：
  ...
  - \(A\)是[吸收的当且仅当](../Page/吸收集.md "wikilink")\(0 \in \operatorname{core}(A)\)
    \[5\]
...
令\(X\)是[拓扑向量空间](https://zh.wikipedia.org/wiki/拓扑向量空间 "wikilink")，\(\operatorname{int}\)表示内部算子，且\(A \subset X\)，则有：
  ...
  - 如果\(A\)是非空凸集且\(X\)
    有限维的，则有\(\operatorname{int}A = \operatorname{core}A\)\[8\]

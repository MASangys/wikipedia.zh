在[數學裡](https://zh.wikipedia.org/wiki/數學 "wikilink")，**代數整數**（**algebraic
integer**）是[複數中的一类](https://zh.wikipedia.org/wiki/複數 "wikilink")。一个複数α是代数整数当且仅当它是某个個[整](../Page/整数.md "wikilink")[系數的首一](https://zh.wikipedia.org/wiki/系數 "wikilink")[多項式](../Page/多項式.md "wikilink")\(P(x)\)的根。其中首一（英文：monic）意謂最高[冪次項的系數是](../Page/冪.md "wikilink")1。
...
\(P(x) = a_0 + a_1 x + \cdots + a_m x^m\)
  ...
  -
    有一个根是有理数：\(\scriptstyle r = \frac{p}{q}\)，其中*p*、*q*是[互素的整数](https://zh.wikipedia.org/wiki/互素 "wikilink")，那么必然有：分母*q*
    [整除](https://zh.wikipedia.org/wiki/整除 "wikilink")\(a_m\)，以及分子*p*
    [整除](https://zh.wikipedia.org/wiki/整除 "wikilink")\(a_0\)。因此，由于代数整数是某个首一多项式的根，如果它是有理数，那么它的分母整除多项式的最高[冪次項](../Page/冪.md "wikilink")，也就是说整除1。所以这个有理数的分母是1，即是说它是整数。反过来，所有的整数*n*都是整系数首一多项式\(\displaystyle  x - n\)的根，所以是代数整数。
...
## 性质
  ...
  - 兩個代數[整數的和是一個代數整數](https://zh.wikipedia.org/wiki/整數 "wikilink")，他們的差及積也是。這時它們滿足的首一多項式可以用[結式表達](../Page/結式.md "wikilink")；但他們的商就不一定是代數整數。
  - 一個以代數整數為系數的首一多項式的根也是代數整數。換句話說，代數整數構成一個[環](https://zh.wikipedia.org/wiki/環 "wikilink")，並且在任何[代數擴張下是](../Page/代數擴張.md "wikilink")[整閉的](https://zh.wikipedia.org/wiki/整閉 "wikilink")。

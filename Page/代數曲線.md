...
以複數域\(\mathbb{C}\)為例，我們可以定義複係數[有理函數域](../Page/有理函數.md "wikilink")\(\mathbb{C}(x)\)。變元\(x, y\)對代數關係\(y^2 =x^3-x-1\)生成的域\(\mathbb{C}(x,y)\)是一個[橢圓函數域](https://zh.wikipedia.org/wiki/橢圓函數 "wikilink")，代數曲線\(\{ (x,y) \in \mathbb{C}^2 : y^2 = x^3 - x - 1 \}\)
給出它的一個幾何模型。
...
複射影曲線可以嵌入\(n\)維複射影空間\(\mathbb{C}P^n\)。複射影曲線在拓撲上為二維的對象，當曲線光滑時，它是個緊[黎曼曲面](../Page/黎曼曲面.md "wikilink")，即一維的緊[複流形](https://zh.wikipedia.org/wiki/複流形 "wikilink")，因而是可定向的二維緊[流形](https://zh.wikipedia.org/wiki/光滑流形 "wikilink")。這時該曲面的拓撲[虧格](https://zh.wikipedia.org/wiki/虧格 "wikilink")（直觀說就是曲面有幾個洞或把手）等同於曲線上由代數幾何學定義的**虧格**。視這類曲線為[黎曼曲面](../Page/黎曼曲面.md "wikilink")，則可以採[複分析手法加以研究](../Page/複分析.md "wikilink")。另一方面，[黎曼則證明了任何緊黎曼曲面都同構於一條複射影曲線](https://zh.wikipedia.org/wiki/黎曼 "wikilink")。
...
在特徵非零的域上，一條代數曲線僅有有限個奇點；無奇點的曲線即**平滑曲線**。奇點在雙有理映射下可能映為光滑點；事實上，奇點總是可藉著平面的[拉開映射或](../Page/拉開.md "wikilink")[正規化解消](https://zh.wikipedia.org/wiki/正規概形 "wikilink")，由此得到的新平滑曲線仍雙有理等價於原曲線；然而對[代數封閉域上的射影曲線](https://zh.wikipedia.org/wiki/代數封閉域 "wikilink")，其奇點總數則關係到曲線的[幾何虧格](https://zh.wikipedia.org/wiki/幾何虧格 "wikilink")，後者是個雙有理不變量。

### 奇點分類

[Cusp.png](https://zh.wikipedia.org/wiki/File:Cusp.png "fig:Cusp.png")
曲線的[奇點包括](../Page/奇点_\(几何\).md "wikilink")[多重點](https://zh.wikipedia.org/wiki/叉點 "wikilink")（這是曲線的自交點）及[尖點](../Page/尖點.md "wikilink")（如仿射曲線\(x^3=y^2\)之於原點\((0,0)\)，見右圖）等等。一般來說，仿射平面曲線\(f(x,y)=0\)在一點\(P\)的奇點性質可以透過下述方式理解：
...
換言之，曲線在原點附近將近似於\(m\)條（含重複）直線\(a_i x - b_i y = 0\)的聯集。上式中相異的直線數\(r\)稱作**分支數**，正整數\(m\)稱作平面曲線在該點的**重數**，此外還有一個內在的不變量\(\delta_P := \dim \mathcal{O}_{\tilde{C},P}/\mathcal{O}_{C,P}\)，其中\(\tilde{C} \rightarrow C\)是該曲線的[正規化態射](https://zh.wikipedia.org/wiki/正規概形 "wikilink")。資料\[m,
δ,
...
若取\(t\)為整數，對應的\(X,Y,Z\)是[不定方程](https://zh.wikipedia.org/wiki/不定方程 "wikilink")\(X^2+XY+Y^2=Z^2\)的整數解；若將\(X\)代以\(-X\)，則此方程詮釋為θ=60°時的[餘弦定理](../Page/餘弦定理.md "wikilink")，藉此能描述所有一角為

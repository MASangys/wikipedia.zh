...
在Windows XP
中，工作管理員提供了「关机」菜单，可以待机、休眠、关机、重新启动、注销、切换用户，但是如果不使用「歡迎畫面」，則工作管理員中不出現此功能表，且按下Ctrl+Alt+Delete後顯示的是「Windows
安全」對話方塊。
...
<!-- end list -->
  ...
  - 显示[資料執行防止状态和虚拟化状态的列](https://zh.wikipedia.org/wiki/資料執行防止 "wikilink")。虚拟化状态是指UAC虚拟化，启用这个虚拟化会对某些系统位置的文件和注册表引用静默地重定向到用户特定的区域。
...
<!-- end list -->
  ...
  -   - CPU选项卡默认不再显示系统中每个逻辑处理器的单个图形。现在可以显示每个[NUMA节点的数据](https://zh.wikipedia.org/wiki/NUMA "wikilink")。

<!-- end list -->

  -   - “CPU”选项卡会在安装了64个到640个处理器上的电脑的图表中显示简单的百分率。\[5\]这些图表是蓝色的，而较暗的颜色表示较高的使用率。

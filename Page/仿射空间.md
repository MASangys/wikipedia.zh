...
下面的非正式描述可能比正式的定义容易理解一些：仿射空间是没有起点只有方向大小的向量所构成的[向量空间](../Page/向量空间.md "wikilink")。假设有甲乙两人，其中甲知道一个空间中真正的原点，但是乙认为另一个点***p***才是原点。现在求两个向量***a***和***b***的和。乙画出***p***到***a***和***p***到***b***的箭头，然后用平行四边形找到他认为的向量***a*** + ***b***。但是甲认为乙画出的是向量***p**'' +(***a**'' − ***p***) +(***b*** − ***p***)。同样的，甲和乙可以计算向量***a***和***b***的[线性组合](https://zh.wikipedia.org/wiki/线性组合 "wikilink")，通常情况下他们会得到不同的结果。然而，请注意：

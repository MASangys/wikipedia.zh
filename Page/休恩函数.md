...
[HeunG.gif](https://zh.wikipedia.org/wiki/File:HeunG.gif "fig:HeunG.gif")
...
[HeunB.gif](https://zh.wikipedia.org/wiki/File:HeunB.gif "fig:HeunB.gif")
HeunB(a,b,c,d,z)是下列双合流方程的解
\(y_zz-\frac{(\alpha+2*z+z^2*\alpha-2*z^3)}{(z+1)^2/(z-1)^2}*y_z+\frac{(\delta+(2*\alpha+\gamma)*z+\beta*z^2)}{(z-1)^3/(z+1)^3*y(z)};\)

## HeunC函数

[HeunC.gif](https://zh.wikipedia.org/wiki/File:HeunC.gif "fig:HeunC.gif")
HeunC(a,b,c,d,z)是下列微分方程的解
\(y_zz-\frac{(-z^2*a+(-2-b-c+a)*z+b+1)*y_z)}{(z*(z-1))}-\frac{(1/2)*(((-b-c-2)*a-2*d)*z+(b+1)*a+(-c-1)*b-c-2*e)*y}{(z*(z-1))}\)
...
[HeunD.gif](https://zh.wikipedia.org/wiki/File:HeunD.gif "fig:HeunD.gif")
HeunD(a,b,c,d,z)函数是下列微分方程的解
\(y_zz-\frac{(-2*z^5+4*z^3+z^4*a-2*z-a)*y_z)}{((z-1)^3*(z+1)^3)}-\frac{(-z^2*b+(-c-2*a)*z-d)*y}{((z-1)^3*(z+1)^3)}\)
...
[HeunT.gif](https://zh.wikipedia.org/wiki/File:HeunT.gif "fig:HeunT.gif")
...
\([HeunT(a, b, c, z) = HeunT(j*a, b, j^2*c, j*z), And(j^3 = 1)]\)

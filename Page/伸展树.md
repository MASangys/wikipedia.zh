**伸展树**（）是一种能够自我平衡的[二叉查找树](../Page/二叉查找树.md "wikilink")，它能在均摊\(O(\log n)\)的时间内完成基于伸展（Splay）操作的插入、查找、修改和删除操作。它是由[丹尼尔·斯立特](../Page/丹尼尔·斯立特.md "wikilink")（）和[羅伯特·塔揚在](../Page/羅伯特·塔揚.md "wikilink")1985年发明的\[1\]。
...
伸展树最显著的缺点是它有可能会变成一条[链](../Page/链表.md "wikilink")。例如，在以非递减顺序访问全部n个之后就会出现这种情况。此时树的高度对应于最坏情况的时间效率，操作的实际时间效率可能很低。然而[均摊的最坏情况是对数级的](../Page/均摊分析.md "wikilink")——\(O(\log n)\)。
...
以下是伸展树的C++实现（用[指针实现](../Page/指针.md "wikilink")）
...
m次伸展操作的均摊时间效率 \(T_\mathrm{amortized}(m) = O(m \log n)\)\[3\]

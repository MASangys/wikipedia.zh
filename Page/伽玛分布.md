...
假设X<sub>1</sub>, X<sub>2</sub>, ... X<sub>n</sub> 为连续发生事件的等候时间，且这n次等候时间为独立的，那么这n此等候时间之和Y (Y=X<sub>1</sub>+X<sub>2</sub>+...+X<sub>n</sub>)服从伽玛分布，即 Y\~Gamma(α , β)，其中α = n, β = λ。这里的 λ 是连续发生事件的平均发生频率。 [指数分布](../Page/指数分布.md "wikilink")是伽玛分布α = 1的特殊情况。

...
**-{zh-tw:點陣圖;zh-cn:位图;}-**（**Bitmap**），又称**栅格图**（）或**-{zh-tw:位圖;zh-cn:点阵图;}-**，是使用[像素](../Page/像素.md "wikilink")[阵列](https://zh.wikipedia.org/wiki/阵列 "wikilink")(Pixel-array/Dot-matrix[點陣](https://zh.wikipedia.org/wiki/點陣 "wikilink"))来表示的[图像](https://zh.wikipedia.org/wiki/图像 "wikilink")。
...
根据[位深度](https://zh.wikipedia.org/wiki/位深度 "wikilink"),可将位图分为1、4、8、16、24及32[位](https://zh.wikipedia.org/wiki/位深度 "wikilink")-{}-图像等。每个像素使用的信息位数越多，可用的颜色就越多，颜色表现就越逼真，相应的数据量越大。例如，位深度为
1 的像素位图只有两个可能的值（黑色和白色），所以又称为二值位图。位深度为 8 的图像有 2<sup>8</sup>（即
256）个可能的值。位深度为 8 的灰度模式图像有 256 个可能的灰色值。\[1\]

[RGB图像由三个颜色通道组成](https://zh.wikipedia.org/wiki/RGB "wikilink")。8 位/通道的
RGB 图像中的每个通道有 256 个可能的值，这意味着该图像有 1600 万个以上可能的颜色值。有时将带有 8 位/通道 (bpc) 的
RGB 图像称作 24 位-{}-图像（8 位 x 3 通道 = 24
位数据/像素）。\[2\]通常将使用24位RGB组合数据位表示的的位图称为[真彩色位图](../Page/真彩色.md "wikilink")。

[BMP](https://zh.wikipedia.org/wiki/BMP "wikilink")[文件是](../Page/電腦檔案.md "wikilink")[微软公司所开发的一种交换和存储数据的方法](../Page/微软.md "wikilink")，各个版本的[Windows都支持BMP格式的文件](https://zh.wikipedia.org/wiki/Microsoft_Windows "wikilink")。Windows提供了快速、方便的存储和[压缩BMP文件的方法](../Page/数据压缩.md "wikilink")。BMP格式的缺点是，要占用较大的存储空间，文件尺寸太大。

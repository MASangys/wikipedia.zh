**位置空间**与**动量空间**是[物理学中一对联系紧密的](../Page/物理学.md "wikilink")[矢量空间](https://zh.wikipedia.org/wiki/矢量空间 "wikilink")。
...
**动量空间**是空间中所有物体的[动量向量的集合](../Page/动量.md "wikilink")。这个空间通常也是三维的。一个物体的动量可以反映它的运动情况。无论在经典力学还是在量子力学中，动量都是非常重要的一个概念。然而，依据量子力学的[德布罗意关系](https://zh.wikipedia.org/wiki/物质波 "wikilink")，**p**
=
...
位矢**r**的[量纲为](../Page/量纲.md "wikilink")\[L\]，动量**p**的量纲为\[M\]\[L\]\[T\]<sup>−1</sup>，波矢**k**的量纲为\[L\]<sup>−1</sup>，因而类比于角频率ω之于时间*t*，**k**可以视为系统空间上的频率。一个系统的物理现象既可以用位矢描述，也可以用动量描述。两种描述方式所提供的系统信息是等价的。通常利用**r**描述更为直观，但在[固体物理学中](https://zh.wikipedia.org/wiki/固体物理学 "wikilink")，**k**更为常用。

## 经典力学中的位置空间与动量空间

### 拉格朗日力学

在[拉格朗日力学中](../Page/拉格朗日力学.md "wikilink")，[拉格朗日量](../Page/拉格朗日量.md "wikilink")\(\mathcal{L}(\mathbf{q},\ \dot{\mathbf{q}},\ t)\,\!\)通常是在[位形空间中给出的](https://zh.wikipedia.org/wiki/位形空间 "wikilink")。其中，\(\mathbf{q} = \left( q_{1}, q_{2}, \ldots, q_{n} \right)\,\!\)是一个*n*[元](../Page/多元组.md "wikilink")[广义坐标](https://zh.wikipedia.org/wiki/广义坐标 "wikilink")。描述物体运动的[拉格朗日方程为](https://zh.wikipedia.org/wiki/拉格朗日方程 "wikilink")：
...
拉格朗日量还可以在动量空间中给出，\(\mathcal{L}'(\mathbf{p},\ \dot{\mathbf{p}},\ t)\,\!\)\[2\]。其中，\(\mathbf{p} = \left( p_{1}, p_{2}, \ldots, p_{n} \right)\,\!\)是一个*n*元的广义动量。通过[勒让德变换改变广义坐标空间中拉格朗日量的](https://zh.wikipedia.org/wiki/勒让德变换 "wikilink")[全微分的变量](../Page/全微分.md "wikilink")：
...
与拉格朗日力学中必须全部使用坐标或是全部使用动量的情况不同，在[哈密顿力学中](../Page/哈密顿力学.md "wikilink")，[哈密顿方程将坐标与动量放在了对等的地位](https://zh.wikipedia.org/wiki/哈密顿方程 "wikilink")。对于一个哈密顿量为\(H\left(\mathbf{q},\mathbf{p},t\right)\)的系统，其运动方程为：

\[\dot{q}_i = \frac{\partial H}{\partial p_i} \,,\quad \dot{p}_i = - \frac{\partial H}{\partial q_i} \,.\]

## 量子力学中的位置空间与动量空间

在[量子力学中](../Page/量子力学.md "wikilink")，系统是通过其[量子态进行描述的](https://zh.wikipedia.org/wiki/量子态 "wikilink")。量子态可以表述为基态依[权重的](../Page/权重.md "wikilink")[线性叠加](../Page/态叠加原理.md "wikilink")。理论上，只要所有基态都是正交的，其就可以用来表述量子态。如果选择[位置算符的](../Page/位置算符.md "wikilink")[本征矢作为基矢的话](https://zh.wikipedia.org/wiki/特征向量 "wikilink")，那么就是在位置表象下表征一个系统的[波函数](../Page/波函数.md "wikilink")\(\psi\left(\mathbf{r}\right)\)。[薛定谔方程较为常见的以位矢](../Page/薛定谔方程.md "wikilink")**r**表述的形式就是位置表象下表述系统状态的一个例子。如果选定不同算子的本征矢作为基矢，那么对应地，用来描述系统的状态的表象也将不同。如果选定[动量算符的本征矢作为基矢的话](https://zh.wikipedia.org/wiki/动量算符 "wikilink")，那么波函数\(\phi\left(\mathbf{k}\right)\)也将在动量表象下进行描述。\[3\]

动量表象下的波函数与[傅里叶变换以及](../Page/傅里叶变换.md "wikilink")[频域这一概念密切相关](https://zh.wikipedia.org/wiki/频域 "wikilink")。由于对于一个自由粒子，其空间频率与动量成正比。因而，在以动量本征值之和描述一个系统时，我们也就是在用频率分量之和来描述一个系统，也就是说，我们对于这个系统进行了傅里叶变换。\[4\]这一点将在下面进行的表象变换过程中得以体现。
...
当通过傅里叶变换给出一个系统的[幺正算符时](../Page/幺正算符.md "wikilink")，**r**与**p**可以证明是，也就是说它们有相同的[谱性](https://zh.wikipedia.org/wiki/谱_\(泛函分析\) "wikilink")。用物理语言来说就是，动量算符在动量表象中对于波函数的作用等价于位置算符在位置表象中对于波函数的作用。

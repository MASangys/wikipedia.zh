在[量子力學裏](https://zh.wikipedia.org/wiki/量子力學 "wikilink")，**位置算符**（）是一種[量子算符](../Page/算符.md "wikilink")。對應於位置算符的[可觀察量是粒子的位置](../Page/可觀察量.md "wikilink")。位置算符的本徵值是[位置向量](../Page/位置向量.md "wikilink")。採用[狄拉克標記](https://zh.wikipedia.org/wiki/狄拉克標記 "wikilink")，位置算符
\(\hat{x}\) 的本徵態 \(|x\rang\) 滿足方程式
...
設定量子態 \(|\Psi\rang=\hat{x}|\psi \rang\) 。量子態 \(|\Psi\rang\)
、\(|\psi\rang\)
的位置空間表現，即[波函數](https://zh.wikipedia.org/wiki/波函數 "wikilink")，分別定義為
...
在位置空間裡，使用連續本徵態 \(|x'\rang\) 所組成的基底，任意[量子態](../Page/量子態.md "wikilink")
\(|\psi\rang\) 展開為
...
應用[狄拉克正交歸一性](https://zh.wikipedia.org/wiki/正交歸一性 "wikilink")，這方程式與左矢
\(\lang x|\) 的內積為
...
總結，位置算符 \(\hat{x}\) 作用於量子態 \(|\psi\rang\) 的結果 \(|\Psi\rang\)
，表現於位置空間，等價於波函數 \(\psi(x)\) 與 \(x\) 的乘積
\(\Psi(x)\) 。位置算符 \(\hat{x}\) 的位置空間表現是位算符 \(\hat{\mathfrak{x}}\) ，可以稱算符
\(\hat{\mathfrak{x}}\) 為位置算符。

## 本徵函數

假設，在位置空間裡，位置算符 \(\hat{\mathfrak{x}}\)
的[本徵值為](https://zh.wikipedia.org/wiki/本徵值 "wikilink") \(q\)
的[本徵函數是](../Page/本徵函數.md "wikilink") \(g_q(x)\) 。用方程式表達，\[1\]
...
這性質不是普通的[正交歸一性](https://zh.wikipedia.org/wiki/正交歸一性 "wikilink")，這性質稱為[狄拉克正交歸一性](https://zh.wikipedia.org/wiki/正交歸一性 "wikilink")。因為這性質，位置算符的本徵函數具有[完備性](https://zh.wikipedia.org/wiki/完備性 "wikilink")，也就是說，任意波函數
\(\psi(x)\)
都可以表達為本徵函數的[線性組合](https://zh.wikipedia.org/wiki/線性組合 "wikilink")：

\[\psi(x)=\int_{ - \infty}^{\infty}\ \psi(q)g_{q}(x)\ dq\] 。

雖然本徵函數 \(g_q(x)\)
所代表的量子態是無法實際體現的，並且嚴格而論，不是一個函數，它可以視為代表一種理想量子態，這種理想量子態具有準確的位置
\(q\)
，因此，根據[不確定性原理](https://zh.wikipedia.org/wiki/不確定性原理 "wikilink")，這種理想量子態的[動量呈](https://zh.wikipedia.org/wiki/動量 "wikilink")[均勻分佈](https://zh.wikipedia.org/wiki/均勻分佈 "wikilink")。

## 期望值

採用位置空間表現，設想一個移動於一維空間的量子粒子。在這裏，希爾伯特空間是 \(\mathcal{H} = L^2(\mathbb{R})\)
，是實值[定義域的](https://zh.wikipedia.org/wiki/定義域 "wikilink")[平方可積函數的空間](../Page/平方可積函數.md "wikilink")。\[2\]兩個態向量的內積是
...
位置算符 \(\hat{x}\) 作用於量子態 \(|\psi\rang\) 的結果，表現於位置空間，等價於波函數 \(\psi(x)\) 與
\(x\) 的乘積，所以，
...
推廣至三維空間相當直截了當，參數為三維位置 \(\mathbf{r}\) 的波函數為 \(\psi(\mathbf{r})\)
，位置的[期望值為](../Page/期望值.md "wikilink")\[3\]
...
。這關係稱為位置算符與動量算符的[對易關係](https://zh.wikipedia.org/wiki/對易關係 "wikilink")。由於兩者的對易關係不等於
0
，位置與動量彼此是[不相容可觀察量](https://zh.wikipedia.org/wiki/可觀察量#不相容可觀察量 "wikilink")。\(\hat{x}\)
與 \(\hat{p}\) 絕對不會擁有共同的基底量子態。一般而言，\(\hat{x}\) 的本徵態與 \(\hat{p}\) 的本徵態不同。
...
。所以，\(x\) 的不確定性與 \(p\) 的不確定性的乘積 \(\Delta x\ \Delta p\) ，必定大於或等於
\(\hbar/2\) 。

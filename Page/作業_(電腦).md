在計算機科學中，**作業**（），又譯為**工作**、**任務**，是在[記憶體中的一組](https://zh.wikipedia.org/wiki/記憶體 "wikilink")[程式指令](https://zh.wikipedia.org/wiki/程式 "wikilink")。它包括了一段虛擬[定址空間分頁](https://zh.wikipedia.org/wiki/定址空間 "wikilink")，與[作業系統資源](https://zh.wikipedia.org/wiki/作業系統 "wikilink")，提供[執行緒運行的空間](https://zh.wikipedia.org/wiki/執行緒 "wikilink")。有時候，它與[行程被認為是同義的](../Page/行程.md "wikilink")；但也有人認為，在即時系統（Real-time）中運行的[行程才被稱為作業](../Page/行程.md "wikilink")。

## Windows作业对象

Windows
2000操作系统开始，Job是一组进程的集合，用于限制其使用的资源、性能信息统计维护、自动关闭子进程等。允许将进程组合在一起并创建一个进程的"容器"来限制进程能够做什么。无法将当前进程或它的任何子进程从作业中去除，这个安全特性可以确保进程无法摆脱对它施加的限制。Win32关于作业的API：

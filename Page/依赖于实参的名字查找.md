**依赖于实参的名字查找**是[C++](../Page/C++.md "wikilink")[程序设计语言中的](https://zh.wikipedia.org/wiki/程序设计语言 "wikilink")[名字查找机制之一](https://zh.wikipedia.org/wiki/名字查找 "wikilink")。英文为“argument-dependent lookup”，因此缩写为**ADL**。\[1\]ADL依据[函数调用中的](../Page/子程序.md "wikilink")的[数据类型查找未限定](https://zh.wikipedia.org/wiki/數據類型 "wikilink")（unqualified）的函数名（或者函数模板名）。这也被称作“克尼格查找”（Koenig lookup），虽然并不是它的发明者。\[2\]
...
3.  如果实参是类模板特化后得到的类型，除了应用类类型规则，还要包括下述规则提及的类型的相关命名空间与类的集合：
    1.  类型模板形参（type template parameter）所对应的模板实参的类型，不包括非类型的模板形参、[模板的模板形参](../Page/模板的模板参数.md "wikilink")；
    2.  [模板的模板实参](../Page/模板的模板参数.md "wikilink")（template template argument）所在的命名空间；
...
ADL能查询到的函数被认为是类的接口之一。[标准模板库](../Page/标准模板库.md "wikilink")的几个`std`命名空间的算法使用未限定的`swap`调用。使用时，如果名字查找没找到其它结果，则使用[`std::swap`](https://zh.wikipedia.org/wiki/:b:C++/Utility "wikilink")函数；如果这些算法使用第三方类，如`Foo`，在另一个命名空间中发现包含了`swap(Foo&, Foo&)`, 则重载版的`swap`被优先使用。
...
运算符`<<`等价于调用函数`operator<<`，但没有给出`std`限定符。通过ADL查找到函数`std::ostream& std::operator<<(std::ostream&, const char*)`。
...
（其中`a`与`b`的类型是`N::A`）。因为如果`N::swap(N::A&, N::A&)`存在，上述第二段代码将调用它，而上述第一段代码将不调用它。
...
## 外部链接
  ...
  - [*What's In a Class? - The Interface Principle*](http://www.gotw.ca/publications/mill02.htm) by [Herb Sutter](https://zh.wikipedia.org/wiki/Herb_Sutter "wikilink")
  - [*Namespaces and the Interface Principle*](http://www.gotw.ca/publications/mill08.htm) by [Herb Sutter](https://zh.wikipedia.org/wiki/Herb_Sutter "wikilink")
  - [Why I Hate Namespaces](http://www.informit.com/guides/content.aspx?g=cplusplus&seqNum=286) by Danny Kalev
  - ["A Modest Proposal: Fixing ADL (revision 2)"](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2006/n2103.pdf) by [Herb Sutter](https://zh.wikipedia.org/wiki/Herb_Sutter "wikilink")

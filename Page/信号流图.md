...
[Modified_SFG_for_feedback_amplifier.PNG](https://zh.wikipedia.org/wiki/File:Modified_SFG_for_feedback_amplifier.PNG "fig:Modified_SFG_for_feedback_amplifier.PNG") [Control_parameter.PNG](https://zh.wikipedia.org/wiki/File:Control_parameter.PNG "fig:Control_parameter.PNG") [Signal_flow_graph_for_feedback_amplifier.png](https://zh.wikipedia.org/wiki/File:Signal_flow_graph_for_feedback_amplifier.png "fig:Signal_flow_graph_for_feedback_amplifier.png")信号流图，其中有一個控制變數*P*，和二個中間變數*x<sub>j</sub>=Px<sub>i</sub>*，此圖是由D.Amico等人所製\[27\]\]\] 圖3是由表示[负反馈放大器](../Page/负反馈放大器.md "wikilink")的一種可能的信号流图，可以得到放大器增益的方程式為

\[G = \frac {y_2}{x_1}\] \(= G_{\infty} \left( \frac{T}{T + 1} \right) + G_0 \left( \frac{1}{T + 1} \right) \ .\)

其參數的說明如下：*T* = ，*G<sub>∞</sub>* = 直變大器增益，direct amplifier gain, *G<sub>0</sub>* = 前饋（表示回授可能有的雙向特性，也可能是刻意的前饋[補償](../Page/超前-滞后补偿器.md "wikilink")）。

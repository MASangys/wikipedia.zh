**傅里叶变换**（、）是一种線性[积分变换](../Page/积分变换.md "wikilink")，用于信号在[时域](https://zh.wikipedia.org/wiki/时域 "wikilink")（或空域）和[频域之间的变换](https://zh.wikipedia.org/wiki/频域 "wikilink")，在[物理学和](../Page/物理学.md "wikilink")[工程学中有许多应用](../Page/工程学.md "wikilink")。因其基本思想首先由[法国学者](https://zh.wikipedia.org/wiki/法国 "wikilink")[约瑟夫·傅里叶系统地提出](../Page/约瑟夫·傅里叶.md "wikilink")，所以以其名字来命名以示纪念。实际上傅里叶变换就像化学分析，确定物质的基本成分；信号来自自然界，也可对其进行分析，确定其基本成分。\[1\]
...
\(\hat f\)
则是[复数函数](https://zh.wikipedia.org/wiki/复数 "wikilink")，用一个复数来表示[振幅和](https://zh.wikipedia.org/wiki/振幅 "wikilink")[相位](https://zh.wikipedia.org/wiki/相位 "wikilink")。
...
自变量*x*表示*时间*（以秒为单位），变换变量*ξ*表示[频率](https://zh.wikipedia.org/wiki/频率 "wikilink")（以[赫兹为单位](../Page/赫兹.md "wikilink")）。在适当条件下，\(\hat f\)可由**逆变换**（inverse
Fourier transform）由下式确定\(f\)：

\[f(x) = \int_{-\infty}^\infty \hat f(\xi)\ e^{2 \pi i \xi x}\,d\xi\]，*x*为任意实数。

[傅里叶逆定理提出](https://zh.wikipedia.org/wiki/傅里叶逆定理 "wikilink")\(f\)可由\(\hat f\)确定，[傅里叶在其](../Page/约瑟夫·傅里叶.md "wikilink")1822年出版的著作《热分析理论》（）中首次引入这个定理。虽然现在标准下的证明直到很久以后才出现。\(f\)和\(\hat{f}\)常常被称为*傅里叶积分对*
或*傅里叶变换对*。
...
傅里叶变换源自对[傅里叶级数的研究](../Page/傅里叶级数.md "wikilink")。在对傅里叶级数的研究中，复杂的[周期函数可以用一系列简单的](../Page/周期函数.md "wikilink")[正弦](../Page/正弦.md "wikilink")、[余弦波之和表示](https://zh.wikipedia.org/wiki/余弦 "wikilink")。傅里叶变换是对傅里叶级数的扩展，由它表示的函数的周期趋近于无穷。
...
若函数\(f \left( x\right )\)存在傅里叶变换，则对任意[实数](../Page/实数.md "wikilink")\(\omega_{0}\)，函数\(f(x) e^{i \omega_{0} x}\)也存在傅里叶变换，且有\(\mathcal{F}[f(x)e^{i \omega_{0} x}]=F(\omega - \omega _0 )\)。式中花体\(\mathcal{F}\)是傅里叶变换的作用[算子](../Page/算子.md "wikilink")，平体\(F\)表示变换的结果（复函数），\(e\)为[自然对数的底](https://zh.wikipedia.org/wiki/自然对数 "wikilink")，\(i\)为[虚数单位](../Page/虚数.md "wikilink")\(\sqrt{-1}\)。
...
\(*\)代表[複共軛](https://zh.wikipedia.org/wiki/複共軛 "wikilink")。

## 傅里叶变换的不同变种

傅里叶变换也可以写成[角频率形式](https://zh.wikipedia.org/wiki/角频率 "wikilink")：
2*πξ*}}其单位是[弧度每秒](https://zh.wikipedia.org/wiki/弧度 "wikilink")。
...
[傅里叶分析最初是研究](../Page/傅里叶分析.md "wikilink")[周期性现象](https://zh.wikipedia.org/wiki/週期 "wikilink")，即傅里叶级数的，后来通过傅里叶变换将其推广到了非周期性现象。理解这种推广过程的一种方式是将非周期性现象视为周期性现象的一个特例，即其[周期为无限长](https://zh.wikipedia.org/wiki/周期 "wikilink")。
...
为了在科学计算和[数字信号处理等领域使用计算机进行傅里叶变换](../Page/数字信号处理.md "wikilink")，必须将函数*x*<sub>*n*</sub>定义在*离散*点而非连续域内，且须满足[有限性或](https://zh.wikipedia.org/wiki/有限性 "wikilink")[周期性条件](https://zh.wikipedia.org/wiki/周期性 "wikilink")。这种情况下，使用离散傅里叶变换，将函数*x*<sub>*n*</sub>表示为下面的求和形式：
...
以上各种傅里叶变换可以被更统一的表述成任意[局部紧致的](https://zh.wikipedia.org/wiki/紧性 "wikilink")[阿贝尔群上的傅里叶变换](../Page/阿贝尔群.md "wikilink")。这一问题属于[调和分析的范畴](https://zh.wikipedia.org/wiki/调和分析 "wikilink")。在调和分析中，一个变换从一个群变换到它的[对偶群](https://zh.wikipedia.org/wiki/对偶群 "wikilink")（dual
group）。此外，将傅里叶变换与卷积相联系的卷积定理在调和分析中也有类似的结论。傅里叶变换的广义理论基础参见[龐特里亞金對偶性](../Page/龐特里亞金對偶性.md "wikilink")（Pontryagin
duality）中的介绍。

### 时频分析变换

[小波变换](https://zh.wikipedia.org/wiki/小波变换 "wikilink")，[chirplet轉換和](https://zh.wikipedia.org/wiki/chirplet轉換 "wikilink")[分数傅里叶变换试图得到时间信号的频率信息](https://zh.wikipedia.org/wiki/分数傅里叶变换 "wikilink")。同时解析频率和时间的能力在数学上受[不确定性原理的限制](../Page/不确定性原理.md "wikilink")。
...
<td><p><a href="https://zh.wikipedia.org/wiki/高斯函数" title="wikilink">高斯函数</a><span class="math inline">exp ( − <em>α</em><em>t</em><sup>2</sup>)</span>的傅里叶变换是他本身.只有当<span class="math inline"><em>R</em><em>e</em>(<em>α</em>) &gt; 0</span>时，这是可积的。</p></td>
</tr>
<tr class="odd">
<td><p>15</p></td>
...
Airy分布用J<sub>1</sub>（1阶[第一类贝塞尔函数](https://zh.wikipedia.org/wiki/贝塞尔函数#第一类贝塞尔函数 "wikilink")）表达。
...
## 参见
  ...
  - [正交变换](https://zh.wikipedia.org/wiki/正交变换 "wikilink")

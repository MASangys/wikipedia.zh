...
**傑恩斯-卡明斯模型**（Jaynes–Cummings model
(JCM)）是一個[量子光學的理论模型](https://zh.wikipedia.org/wiki/量子光學 "wikilink")。
這是一個描述[雙態系統和量化](../Page/雙態系統.md "wikilink")[光腔](https://zh.wikipedia.org/wiki/光腔 "wikilink")(optical
cavity)交互作用的模型，這種交互作用和光子的存在與否無關(在电磁辐射能造成光子自發性的[放射與](https://zh.wikipedia.org/wiki/放射 "wikilink")[吸收](https://zh.wikipedia.org/wiki/吸收 "wikilink"))。它主要被運用在[原子物理學](https://zh.wikipedia.org/wiki/原子物理學 "wikilink")，量子光學，固態量子信息電路的理論與實驗上。
...
其中：
  ...
  - \(\begin{smallmatrix}\hat{E} = \hat{a} +\hat{a}^{\dagger}\end{smallmatrix}\)[場運算符](https://zh.wikipedia.org/wiki/場運算符 "wikilink")，目的是把量化[輻射場转化為](https://zh.wikipedia.org/wiki/輻射場 "wikilink")[玻色子的模型](../Page/玻色子.md "wikilink")，另外[雙態原子是能被三維](https://zh.wikipedia.org/wiki/雙態原子 "wikilink")[布洛赫球面所描述的](../Page/布洛赫球面.md "wikilink")[半自旋粒子](https://zh.wikipedia.org/wiki/半自旋 "wikilink")
      - \(\begin{smallmatrix}\hat{a}^{\dagger}\end{smallmatrix}\)是玻色子的[創生算符](https://zh.wikipedia.org/wiki/創生及湮滅算符 "wikilink")
      - \(\begin{smallmatrix}\hat{a}\end{smallmatrix}\)
        是玻色子的[湮滅算符](https://zh.wikipedia.org/wiki/創生及湮滅算符 "wikilink")

<!-- end list -->

  - \(\begin{smallmatrix}\hat{S} = \hat{\sigma}_+ +\hat{\sigma}_-\end{smallmatrix}\)是原子耦合區的[偏振運算符](../Page/偏振.md "wikilink")
  - \(\begin{smallmatrix}\hat{\sigma}_+ = |e \rangle \langle g |\end{smallmatrix}\)與\(\begin{smallmatrix}\hat{\sigma}_- = |g \rangle \langle e |\end{smallmatrix}\)
    是原子的[階梯算符](../Page/階梯算符.md "wikilink")
  - \(\begin{smallmatrix}\hat{\sigma}_z = |e \rangle \langle e | - |g \rangle \langle g |\end{smallmatrix}\)
    是原子[反轉運算符](https://zh.wikipedia.org/wiki/反轉運算 "wikilink")
  - \(\begin{smallmatrix}\omega_a\end{smallmatrix}\)是原子的躍遷頻率
  - \(\begin{smallmatrix}\omega_c\end{smallmatrix}\) 是模型的角頻率

### JCM哈密頓量

通過把[薛丁格繪景轉換為](../Page/薛丁格繪景.md "wikilink")[相互作用繪景](../Page/相互作用繪景.md "wikilink")(又名旋轉框架(rotating
frame))
，使得\(\begin{smallmatrix}H_0 = \hat{H}_{\text{field}} +\hat{H}_{\text{atom}}\end{smallmatrix}\)，可以得到：
...
為了求解這個方程，簡化模型是再所難免的。注意到，當
\(\begin{smallmatrix}|\omega_c - \omega_a| \ll \omega_c+\omega_a\end{smallmatrix}\)
的時候，快速振盪
“反向旋轉”項（也就是慢速震蕩項）可被忽略，這被稱為[旋波近似](https://zh.wikipedia.org/wiki/旋波近似 "wikilink")。再將之轉換回薛丁格繪景，JCM哈密頓量就變成了：
...
> \(\begin{smallmatrix}\delta = \omega_a - \omega_c\end{smallmatrix}\)
> 稱之為場與雙態系統的失諧量（頻率）。

為了更好地求解哈密頓量，把\(\begin{smallmatrix}\begin{smallmatrix}\hat{H}_{I}\end{smallmatrix}\end{smallmatrix}\)的[本徵態轉換成](https://zh.wikipedia.org/wiki/本徵態 "wikilink")[張量積](https://zh.wikipedia.org/wiki/張量積 "wikilink")
\(\begin{smallmatrix}|n,g\rangle, |n,e\rangle\end{smallmatrix}\)（\(\begin{smallmatrix}n \in \mathbb{N}\end{smallmatrix}\)，表示模型中輻射量子的數量。）

對位任意正整數n，狀態\(\begin{smallmatrix}|\psi_{1n}\rangle := |n,e\rangle\end{smallmatrix}\)
與狀態\(\begin{smallmatrix}|\psi_{2n}\rangle := |n+1,g\rangle\end{smallmatrix}\)
會退化為\(\begin{smallmatrix}\hat{H}_{I}\end{smallmatrix}\)
，\(\begin{smallmatrix}\hat{H}_{\text{JC}}\end{smallmatrix}\)
足以在子空間\(\begin{smallmatrix}\text{span} \{ |\psi_{1n}\rangle ,|\psi_{2n}\rangle\}\end{smallmatrix}\)對角化。
\(\begin{smallmatrix}\hat{H}_{\text{JC}}\end{smallmatrix}\)的元素屬於\(\begin{smallmatrix}{H}^{(n)}_{ij} := \langle\psi_{in}|\hat{H}_{\text{JC}}|\psi_{jn}\rangle\end{smallmatrix}\)的子空間，表示為：
...
其中，\(\begin{smallmatrix}\Omega_n(\delta) = \sqrt{\delta^2 +\Omega^2(n+1)}\end{smallmatrix}\)
是[拉比頻率特殊的](https://zh.wikipedia.org/wiki/拉比頻率 "wikilink")[失諧參數](https://zh.wikipedia.org/wiki/失諧 "wikilink")。

含能量本徵態
\(\begin{smallmatrix}|n,\pm\rangle~\end{smallmatrix}\)的[特徵值是](https://zh.wikipedia.org/wiki/特徵值 "wikilink")：
...
為了得到動量的一般情況。 首先考慮一個場疊加態的初態
\(\begin{smallmatrix}~|\psi_\text{field}(0)\rangle=\sum_n{C_n|n\rangle}~\end{smallmatrix}\)，若置一激發態原子于場內，則系統初態為：

\[|\psi_\text{tot}(0)\rangle= \sum_n C_n \left[ \cos \left(\frac{\alpha_n}{2}\right)|n,+\rangle-\sin \left(\frac{\alpha_n}{2}\right)|n,-\rangle\right].\]

其中 \(\begin{smallmatrix}~|n,\pm\rangle~\end{smallmatrix}\) 是該系統的定態,
含時狀態向量是：
...
可以直接通過[海森堡記法](../Page/海森堡模型.md "wikilink")（Heisenberg notation）來確定么正演化算符
（unitary evolution operator） :\[1\]
...
原子反轉的量子震盪圖像（二次反比失諧參數
\(\begin{smallmatrix}a = (\delta/(2g))^2 = 40\end{smallmatrix}\)，
其中\(\delta\)是失諧參數），基於 [A.A.
Karatsuba](https://zh.wikipedia.org/wiki/Anatolii_Alexeevitch_Karatsuba "wikilink")
和 E.A. Karatsuba 取得的基本公式\[2\]。

## 參考資料

## 參考文獻

  - [Serge Haroche](../Page/塞尔日·阿罗什.md "wikilink"), Jean-Michel Raimond:
    Exploring the Quantum: Atoms, Cavities, and Photons. Oxford
    University Press 2006, ISBN 978-0198509141

## 延伸閱讀

  - C.C. Gerry and P.L. Knight (2005). *Introductory Quantum Optics*,
    Cambridge: Cambridge University Press.
  - M. O. Scully and M. S. Zubairy (1997), *Quantum Optics*, Cambridge:
    Cambridge University Press.
  - D. F. Walls and G. J. Milburn (1995), *Quantum Optics*,
    Springer-Verlag.

[Category:量子光學](https://zh.wikipedia.org/wiki/Category:量子光學 "wikilink")

1.  [S.
    Stenholm](https://zh.wikipedia.org/wiki/Stig_Stenholm "wikilink"),
    "Quantum theory of electromagnetic fields interacting with atoms and
    molecules", *Physics Reports*, **6**(1), 1-121 (1973).
2.
...
TLS协议的优势是与高层的[应用层协议](https://zh.wikipedia.org/wiki/应用层 "wikilink")（如[HTTP](../Page/超文本传输协议.md "wikilink")、[FTP](../Page/文件传输协议.md "wikilink")、[Telnet](../Page/Telnet.md "wikilink")等）无耦合。应用层协议能透明地运行在TLS协议之上，由TLS协议进行建立加密通道需要的协商和认证。应用层协议传送的数据在通过TLS协议时都会被加密，从而保证通信的私密性。

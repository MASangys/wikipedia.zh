**元对象机制**（**MOF**，Meta-Object
Facility）起源于[统一建模语言](../Page/统一建模语言.md "wikilink")（UML）。[對象管理機構](https://zh.wikipedia.org/wiki/對象管理機構 "wikilink")（Object
Management Group|OMG）需要一种元模型结构来定义UML。
...
除了M3模型，MOF还提供创建、操作模型、元模型的方法，通过定义CORBA接口描述这些操作。由于MOF M3模型和UML结构模型相似，MOF
M3元模型通常建模为UML类图。MOF的支持标准是XMI，它定义一种基于XML的M3、M2、M1层模型交换格式。
...
MOF使用MOF::Classes
(不要与UML::Classes混淆)在元层上定义概念（模型元素），如同众所周知的面向对象。MOF可用于定义[面向对象元模型](https://zh.wikipedia.org/wiki/面向对象元模型 "wikilink")（如UML）以及[非面向对象元模型](https://zh.wikipedia.org/wiki/非面向对象元模型 "wikilink")（如Petri网或Web服务元模型）
。
...
MOF现在是一个国际标准：ISO/IEC 19502:2005 Information technology—Meta Object
Facility (MOF)
...
OMG的MOF不能与托管对象格式（Managed Object
Format）混为一谈，托管对象格式由分布式管理任务组（DMTF）在公共信息模型（CIM）基础规范\[1\]
的第3节中定义。
...
1.  [Common Interface Model (CIM) Interface
    Structure](http://www.dmtf.org/standards/published_documents/DSP0004V2.3_final.pdf)
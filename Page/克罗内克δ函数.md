在数学中，**克罗内克函数**（又称克罗内克δ函数、克罗内克δ）\(\delta_{ij}\,\!\)
是一个[二元函数](https://zh.wikipedia.org/wiki/函數#多元函數 "wikilink")，得名于德国数学家[利奥波德·克罗内克](../Page/利奥波德·克罗内克.md "wikilink")。克罗内克函数的自变量（输入值）一般是两个[整数](../Page/整数.md "wikilink")，如果两者相等，则其输出值为1，否则为0。
...
克罗内克函数和[狄拉克δ函数都使用δ作为符号](../Page/狄拉克δ函数.md "wikilink")，但是克罗内克δ用的时候带两个下标，而狄拉克δ函数则只有一个变量。
...
在[线性代数中](../Page/线性代数.md "wikilink")，克罗内克函数可以被看做一个[张量](https://zh.wikipedia.org/wiki/张量 "wikilink")，写作
\(\delta^i_j\,\!\) 。

## 数字信号处理

[unit_impulse.gif](https://zh.wikipedia.org/wiki/File:unit_impulse.gif "fig:unit_impulse.gif")
类似的，在[数字信号处理中](../Page/数字信号处理.md "wikilink")，与克罗内克函数等价的概念是变量为
\(\mathbb{Z}\,\!\) ([整数](../Page/整数.md "wikilink"))的函数：
...
如果将整数看做一个装备了[计数测度的](https://zh.wikipedia.org/wiki/计数测度 "wikilink")[测度空间](https://zh.wikipedia.org/wiki/测度空间 "wikilink")，那么这个性质和[狄拉克δ函数的定义是一样的](../Page/狄拉克δ函数.md "wikilink")：

\[\int_{-\infty}^\infty \delta(x-y)f(x) dx=f(y)\,\!\] 。

实际上，狄拉克δ函数是根据克罗内克函数而得名的。在信号处理中，两者是同一个概念在不同的上下文中的表现。一般设定
\(\delta(t)\,\,\!\) 为连续的情况（狄拉克函数） ，而使用*i*, *j*, *k*, *l*, *m*, and *n*
等变量一般是在 离散的情况下（克罗内克函数）。

### 线性代数中的应用

在[线性代数中](../Page/线性代数.md "wikilink")，[单位矩阵可以写作](https://zh.wikipedia.org/wiki/单位矩阵 "wikilink")
\((\delta_{ij})_{i,j=1}^n\,\!\) 。

在看做是[张量时](https://zh.wikipedia.org/wiki/张量 "wikilink")（克罗内克张量），可以写作
\(\delta^i_j\,\!\) 。

这个(1,1)向量表示:

  - 作为[线性映射的单位矩阵](../Page/线性映射.md "wikilink")。
  - [迹数](https://zh.wikipedia.org/wiki/迹数 "wikilink")。
  - [内积](https://zh.wikipedia.org/wiki/内积 "wikilink")
    \(V^* \otimes V \to K\,\!\) 。
  - 映射 \(K \to V^* \otimes V\,\!\)
    ，将数量乘积表示为[外积的形式](../Page/外积.md "wikilink")。

## 廣義克羅內克函數

定義**廣義克羅內克函數**為 \(n\times n\,\!\)
矩陣的[行列式](../Page/行列式.md "wikilink")，以方程式表達為\[1\]
...
其中，\(\delta^{i}_{j}\,\!\) 是個[張量函數](../Page/張量.md "wikilink")，定義為
\(\delta^{i}_{j}\ \stackrel{def}{=}\ \delta_{ij}\,\!\) 。
...
<!-- end list -->
  ...
  -
    其中，\(\epsilon^{ijk}\,\!\) 和 \(\epsilon_{lmn}\,\!\)
    是[列維-奇維塔符號](../Page/列維-奇維塔符號.md "wikilink")。
...
对任意的整数 \(n\,\!\)
，运用标准的[留数计算](../Page/留数.md "wikilink")，可以将克罗内克函数表示成积分的形式：

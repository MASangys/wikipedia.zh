数学上，**克罗内克积**（）是两个任意大小的矩阵间的运算，表示为\(\otimes\)。克罗内克积是[外积](../Page/外积.md "wikilink")从向量到矩阵的推广，也是[张量积](../Page/张量积.md "wikilink")在标准基下的矩阵表示。
...
**克罗内克积**不符合[交换律](https://zh.wikipedia.org/wiki/交换律 "wikilink")：通常，*A* \(\otimes\) *B* 不同于 *B* \(\otimes\) *A*。

*A* \(\otimes\) *B*和*B* \(\otimes\) *A*是排列等价的，也就是说，存在[排列矩阵](https://zh.wikipedia.org/wiki/排列矩阵 "wikilink")*P*和*Q*，使得

\[A \otimes B = P \, (B \otimes A) \, Q.\] 如果*A*和*B*是方块矩阵，则*A* \(\otimes\) *B*和*B* \(\otimes\) *A*甚至是排列[相似的](https://zh.wikipedia.org/wiki/相似矩阵 "wikilink")，也就是说，我们可以取*P* = *Q*<sup>T</sup>。

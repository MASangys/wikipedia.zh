**Kruskal演算法**是一種用來尋找[最小生成樹的演算法](https://zh.wikipedia.org/wiki/最小生成樹 "wikilink")，由Joseph
Kruskal在1956年發表。用來解決同樣問題的還有[Prim演算法和](https://zh.wikipedia.org/wiki/Prim演算法 "wikilink")等。三種演算法都是[贪心算法的應用](https://zh.wikipedia.org/wiki/贪心法 "wikilink")。和Boruvka演算法不同的地方是，Kruskal演算法在圖中存在相同權值的邊時也有效。

在[概率论中](https://zh.wikipedia.org/wiki/概率论 "wikilink")，**全变差距离**（total variation distance）是概率测度的一种距离。它也是一种[统计距离度量](https://zh.wikipedia.org/wiki/统计距离 "wikilink")，有时也称为**统计距离**（statistical distance）或**变差距离**（variational distance）。

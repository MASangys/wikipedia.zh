在[程序设计中](../Page/程序设计.md "wikilink")，**全局变量**是在所有[作用域都可访问的变量](../Page/作用域.md "wikilink")，与之对应的是[局部变量](https://zh.wikipedia.org/wiki/局部变量 "wikilink")。

通常，使用不必要的全局变量被认为是坏习惯，这正是由于全局变量的非局部性：全局变量可能被从任何地方修改（除非位于[保护内存中](https://zh.wikipedia.org/wiki/保护内存 "wikilink")），也可能被任何地方所依赖。于是全局变量便拥有了建立相互依存关系的无限可能，而互相依存关系的建立会使得复杂度增加，参见[远隔作用](https://zh.wikipedia.org/wiki/远隔作用_\(计算机科学\) "wikilink")（Action
...
需要指出的是，[C语言不存在真正意义上的](https://zh.wikipedia.org/wiki/C语言 "wikilink")“全局变量”。被习惯性误称为“全局变量”的，一般是文件作用域对象。[ANSI
C](../Page/ANSI_C.md "wikilink")/ISO C也没有这种提法。
[C语言中所谓](https://zh.wikipedia.org/wiki/C语言 "wikilink")“全局变量”的例子：
...
Blockstruktur](https://zh.wikipedia.org/wiki/de:Variable_\(Programmierung\)#Variablen_in_einer_Blockstruktur "wikilink")

...
RDSMS中的连续 [SQL](../Page/SQL.md "wikilink") 查询符合 [ANSI](https://zh.wikipedia.org/wiki/美國國家標準協會 "wikilink") [SQL](../Page/SQL.md "wikilink") 标准。最常见的RDSMS SQL 查询通过声明 `SELECT` 语句实现。对数据进行的连续 [SQL](../Page/SQL.md "wikilink") `SELECT` 操作跨越了一个或者多个数据流，可以使用（可选的） `FROM` 和（可选的） `JOIN` 子句来指定多数据流的聚合规则，用 `WHERE` 语句和比较谓词来限制查询返回的记录，通过 `GROUP BY` 将具有相同值的数据流映射到同一个更小的数据集，通过 `HAVING` 来过滤从 `GROUP BY` 产生的结果记录，并通过 `ORDER BY` 对接过进行排序。

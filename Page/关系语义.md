**Kripke 语义**(也叫做**关系语义**或**框架语义**，并经常混淆于[可能世界语义](https://zh.wikipedia.org/wiki/可能世界语义 "wikilink"))是[模态逻辑](../Page/模态逻辑.md "wikilink")系统的形式[语义](https://zh.wikipedia.org/wiki/语义 "wikilink")，于 1950 年代晚期和 1960 年代早期由[索尔·阿伦·克里普克](../Page/索尔·阿伦·克里普克.md "wikilink")建立。它后来为另一个非经典逻辑，最重要的[直觉逻辑所接受](https://zh.wikipedia.org/wiki/直觉逻辑 "wikilink")。Kripke 语义的发现是非经典逻辑开发中重大突破，因为这种逻辑的[模型论](../Page/模型论.md "wikilink")在 Kripke 之前实际上是不存在的。
...
**Kripke 框架**或**模态框架**是 \<*W*,*R*\> 对，这里的 *W* 是非空集合，*R* 是在 *W* 上的[二元关系](../Page/二元关系.md "wikilink")。*W* 的元素叫做**节点**或**世界**，而 *R* 叫做**[可及关系](../Page/可及关系.md "wikilink")**。

**Kripke 模型**是 \(<W,R,\Vdash>\) 三元组，这里的 \(<W,R>\) 是 Kripke 框架，而 \(\Vdash\) 是在 *W* 的节点和模态公式之间的如下关系:
...
语义对于逻辑(就是推理系统)研究是有用的，条件是在语义*蕴涵*关系忠实的反映语法对应物 -- *推论*关系 (*可推导性*)。所以知道哪个模态逻辑关于哪类 Kripke 框架是可靠的和完备的，并为它们确定这种类是关键性的。

对于 Kripke 框架的任何类 *C*，*Thm(C)* 是[正规模态逻辑](../Page/正规模态逻辑.md "wikilink")；特别是，最小化正规模态逻辑 *K* 的定理，在所有 Kripke 模型中都是有效的。不幸的是，逆命题不是一般性成立的: 有 Kripke 不完备的正规模态逻辑。事实上这不是问题，因为实际中研究的多数模态系统关于由简单条件所描述的框架类是完备的。

正规模态逻辑 *L* **对应**于框架类 *C*，条件是 *C*=*Mod(L)*。换句话说，*C* 是 *L* 关于 *C* 是可靠的最大的框架类；随后 *L* 是 Kripke 完备的当且仅当它关于它所对应的类是完备的。

作为一个例子，考虑模式 *T* : \(\Box\)*A* → *A*。*T* 在任何[自反的框架](../Page/自反关系.md "wikilink") \<*W*,*R*\> 中是有效的: 如果 *w* \(\Vdash \Box\)*A*，则 *w* \(\Vdash\)*A*，因为 *w* *R* *w*。在另一方面，使 *T* 有效的框架必须是自反的: 固定 *w* ∈*W*，并定义命题变量 *p* 的满足为如下: *u* \(\Vdash\)*p* 当且仅当 *w* *R* *u*。那么 *w* \(\Vdash \Box\)*p*，所以 *w* \(\Vdash\)*p* 于 *T*，这意味着 *w* *R* *w* 使用了 \(\Vdash\) 的定义。我们见到 *T* 对应于自反的 Kripke 框架的类。

特征化 *L* 的对应类经常比证明它的完备性要容易许多，所以对应性充当完备性证明的指导。对应性还用于证实模态逻辑的*不完备性*: 假定 *L*<sub>1</sub>⊆*L*<sub>2</sub> 是对应于同一个框架类的正规模态逻辑，*L*<sub>1</sub> 不证明 *L*<sub>2</sub> 的所有定理。那么 *L*<sub>1</sub> 是 Kripke 不完备的。例如，模式 \(\Box(A\equiv\Box A)\to\Box A\) 生成一个不完备的逻辑，因为它对应于同 *GL* 一样的框架类(viz. 传递性和逆良基的框架)，但是它不证明 \(\Box A\to\Box\Box A\)。
...
对于任何正规模态逻辑 *L*，我们可以构造一个 Kripke 模型(称为**典范模型**)，它且只有它使 *L* 的定理有效，通过接纳使用[极大一致集合作为模型的标准技术](https://zh.wikipedia.org/wiki/极大一致集合 "wikilink")。典范 Kripke 模型扮演的角色类似于在代数语义中的 [Lindenbaum–Tarski代数构造](https://zh.wikipedia.org/wiki/Lindenbaum–Tarski代数 "wikilink")。

公式集合 *L*是*一致的*，如果从它们、*L* 的公理和肯定前件中不能推导出矛盾。*极大 L一致的集合*(简写为 *L*-*MCS*)是没有真*L*一致的超集的 *L*一致的集合。

*L* 的**典范模型**是 Kripke 模型 \<*W*,*R*,\(\Vdash\)\>，这里的 *W* 是所有*L*-*MCS*，而关系 *R* 和 \(\Vdash\) 为如下:
...
明显的，公式的典范集合的并集自身是典范的。服从前面的讨论，由公式的典范集合公理化的任何逻辑是 Kripke 完备的和[紧致的](../Page/紧致性定理.md "wikilink")。
...
在某些情况下，我们可以使用 FMP 来证明逻辑的 Kripke 完备性: 所有正规模态逻辑关于[模態代數的类都是完备的](https://zh.wikipedia.org/wiki/模態代數 "wikilink")，而*有限的*模态代数可以变换成 Kripke 框架。作为例子，Robert Bull 使用这个方法证明了 *S4.3* 的所有普通扩展都有 FMP，并且是 Kripke 完备的。

### 多模态逻辑

Kripke 语义对有多于一个模态的逻辑有直接的推广。带有 \(\{\Box_i;\,i\in I\}\) 作为必然性算子的集合的语言的 Kripke 框架，由对每个 *i* ∈*I* 装备上二元关系 *R<sub>i</sub>* 一个非空集合 *W*构成。满足关系的定义修改为如下:
...
[直觉逻辑的](https://zh.wikipedia.org/wiki/直觉逻辑 "wikilink") Kripke 语义服从和模态逻辑的语义同样的原理，但是它使用了满足的不同的定义。

**直觉 Kripke 模型**是一个三元组 \(<W,\le,\Vdash>\)，这里的 \(<W,\le>\) 是[偏序](https://zh.wikipedia.org/wiki/偏序 "wikilink")(也有说是[预序](https://zh.wikipedia.org/wiki/预序 "wikilink")) Kripke 框架，而 \(\Vdash\) 满足下列条件:
...
设 *L* 是[一阶语言](../Page/一阶逻辑.md "wikilink")。*L* 的 Kripke 模型是三元组 \<*W*,≤,{*M<sub>w</sub>*}<sub>*w*∈*W*</sub>\>，这里的 \<*W*,≤\> 是直觉 Kripke 框架，*M<sub>w</sub>* 是 *w* ∈*W* 每个节点的(经典) *L*-结构，而下列相容性条件只要在 *u* ≤ *v* 时都是成立的:
...
作为独立开发的[层论的一部分](../Page/层_\(数学\).md "wikilink")，在 1965 年左右认识到 Kripke 语义密切相关于在 [topos论中对](https://zh.wikipedia.org/wiki/topos论 "wikilink")[存在量化](../Page/存在量化.md "wikilink")的处理。就是对一个层的截面的存在性的'局部'示象是一种'可能性'的逻辑。因为这种开发是很多人的工作，比之于理论更合于概念上洞察的天性，归与荣誉不是很容易的。**Kripke-Joyal 语义**这个名称经常用做这种联系。

## 模型构造

同在经典的[模型论](../Page/模型论.md "wikilink")中一样，有从其他模型构造一个新的 Kripke 模型的方法。

在 Kripke 语义中天然的[同态](../Page/同态.md "wikilink")叫做**p-态射**(它是*伪满射*的简写，但这个术语很少用)。Kripke 框架 \<*W*,*R*\> 和 \<*W’*,*R’*\> 的 p-态射是一个映射 *f*:*W* → *W’* 满足

  - *f* 保留可及关系，就是说 *u R v* 蕴涵 *f(u) R’ f(v)*，
  - 在 *f(u) R’ v’* 的时候，有一个 *v* ∈ *W* 使得 *f(v)=v’*。

Kripke 模型 \<*W*,*R*,\(\Vdash\)\> 和 \<*W’*,*R’*,\(\Vdash\)’\> 的 p-态射是它们的底层框架的 p-态射 *f*:*W* → *W’*，它满足
...
我可以使用**拆分**(unravelling)把 Kripke 模型变换成[树](../Page/树_\(图论\).md "wikilink")。给出一个模型 \<*W*,*R*,\(\Vdash\)\> 和固定的节点 *w*<sub>0</sub> ∈ *W*，我们定义一个模型 \<*W’*,*R’*,\(\Vdash\)’\>，这里的 *W’* 是所有有限序列 *s*=\<*w*<sub>0</sub>,*w*<sub>1</sub>,...,*w<sub>n</sub>*\> 的集合，使得对于所有 *i*\<*n* 和 *s* \(\Vdash\)*p*，*w<sub>i</sub>* *R* *w<sub>i+1</sub>* 当且仅当对于所有变量 *p*，*w<sub>n</sub>* \(\Vdash\)*p*。定义可及关系 *R’* 变化；在最简单的情况下我们置
...
Kripke 语义的主要缺陷是存在 Kripke 不完备逻辑和完备但不紧致的逻辑。可以使用来代数语义的想法，通过对 Kripke 装备限制可能求值的集合的额外结构来修正。这引发了[一般框架语义](https://zh.wikipedia.org/wiki/一般框架 "wikilink")。
...
Kripke 语义不是 Kripke 首创的，以上述方式给出的基于使求值相对于节点的语义早于 Kripke 的工作许久:
  ...
  - Jónsson 和 [Tarski](https://zh.wikipedia.org/wiki/Alfred_Tarski "wikilink") 给出了仍然影响着当代模态逻辑研究的表达语义的方式，就是代数方法，这包含了 Kripke 语义的很多关键想法。他们把这个想法应用于直觉逻辑的语义研究，但没有见到与模态逻辑的联系；
  - Kanger 对模态逻辑的释义给出了更加复杂的方式，但是包含了 Kripke 方式的很多关键想法。他首先注意到在关于可及关系的条件和 Lewis-风格的模态逻辑公理之间的联系。但是 Kanger 没能给出对他的系统的完备性证明；
  - [Jaakko Hintikka](https://zh.wikipedia.org/wiki/Jaakko_Hintikka "wikilink") 在他的论文中介入了是 Kripke 语义的简单变体的认识逻辑，等同于通过最大化一致集合的方式构造求值的塑造。他没能为认识逻辑给出推理规则，所以没能给出完备性证明；
  - [Richard Montague](https://zh.wikipedia.org/wiki/Richard_Montague "wikilink") 有了包含在 Kripke 工作中的很多关键想法，但是他没有把它们当作是重要的，所以一直没有发表直到 Kripke 的论文出版在逻辑学社区中造成了轰动之后；
  - [Evert W. Beth](https://zh.wikipedia.org/wiki/E._W._Beth "wikilink") 为直觉逻辑提出了一种基于树的语义，它极其类似于 Kripke 语义，除了使用了更加麻烦的满足定义之外。

尽管**Kripke 语义**的根本思想在 Kripke 首次发表之前就广为流传了，[Saul Kripke](https://zh.wikipedia.org/wiki/Saul_Kripke "wikilink") 关于模态逻辑的工作仍可恰当的当作是开拓性的。最重要的是，Kripke 是第一个为模态逻辑证明了完备性定理的人，并且 Kripke 识别了最弱的正规模态逻辑。

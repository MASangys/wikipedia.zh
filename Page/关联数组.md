在[计算机科学中](../Page/计算机科学.md "wikilink")，**关联数组**（），又称**映射**（）、**字典**（）是一个抽象的[数据结构](../Page/数据结构.md "wikilink")，它包含着类似于（键，值）的有序对。一个关联数组中的有序对可以重复（如C++中的multimap）也可以不重复（如C++中的map）。
...
STL 提供了 8
个关联数组[容器模板](https://zh.wikipedia.org/wiki/容器_\(抽象数据类型\) "wikilink")：
...
[C++/CLI](https://zh.wikipedia.org/wiki/C++/CLI "wikilink") 中另有 .Net
所提供的托管实现，见下。
...
<td><p>字典的接口<br />
System.Collections <a href="../Page/命名空间.md" title="wikilink">命名空间中为非泛型版本</a>，即其内容类型为 System.Object 类型</p></td>
</tr>
<tr class="even">
...
## 外部链接
  ...
  - [NIST's Dictionary of Algorithms and Data Structures: Associative
    Array](http://www.nist.gov/dads/HTML/assocarray.html)

[Bubblesort-edited-color.svg](https://zh.wikipedia.org/wiki/File:Bubblesort-edited-color.svg "fig:Bubblesort-edited-color.svg") **冒泡排序**（）-{zh-tw:又稱為**泡沫排序**，; zh-cn:;}-是一種簡單的[排序算法](../Page/排序算法.md "wikilink")。它重複地走訪過要排序的[數列](https://zh.wikipedia.org/wiki/數列 "wikilink")，一次比較兩個元素，如果他們的順序錯誤就把他們交換過來。走訪數列的工作是重複地進行直到沒有再需要交換，也就是說該數列已經排序完成。這個算法的名字由來是因為越小的元素會經由交換慢慢「浮」到數列的頂端。

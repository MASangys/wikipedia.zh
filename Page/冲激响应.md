在[信号处理中](https://zh.wikipedia.org/wiki/信号处理 "wikilink")，**脈衝響應**（）一般是指[系统在输入为](https://zh.wikipedia.org/wiki/系统 "wikilink")[单位脈衝函数时的输出](https://zh.wikipedia.org/wiki/单位脈衝函数 "wikilink")（响应），是[暫態響應中的一種](../Page/暫態響應.md "wikilink")。对于[连续时间系统来说](https://zh.wikipedia.org/wiki/连续时间系统 "wikilink")，脈衝响应一般用函数\(h(t;\tau)\)来表示，相对应的输入信号，也就是单位脈衝函数满足[狄拉克δ函数的形式](../Page/狄拉克δ函数.md "wikilink")，其函数定义如下：

...
如果一個可微函數\(f\)它的導數\(f'\)在某區間是單調下跌的，\(f\)就是凹的：一個[凹函數擁有一個下跌的](https://zh.wikipedia.org/wiki/凹函數 "wikilink")[斜率](../Page/斜率.md "wikilink")（當中下跌只是代表非上升而不是嚴謹的下跌，也代表這容許零[斜率的存在](../Page/斜率.md "wikilink")。）
...
如果\(f(x)\)是二次可微的，那麼\(f(x)\)就是凹的[若且唯若](https://zh.wikipedia.org/wiki/若且唯若 "wikilink")\(f''(x)\)是非正值。如果二階導數是負值的話它就是嚴謹凹函數，但相反而言又不一定正確，例如當\(f(x) = -x^4\)。
如果\(f\)是凹的也是[可微的](https://zh.wikipedia.org/wiki/可微 "wikilink")，那麼

\[f(y) \leq f(x) + f'(x)[y-x]\]
一個在\(\mathbb C\)的[連續函數是凹的若且唯若对于任意属于](https://zh.wikipedia.org/wiki/連續函數 "wikilink")\(\mathbb C\)的*x*和*y*，有
...
## 例子
  ...
  - 函數\(f(x)=-x^2\) 和
    \(f(x)=\sqrt{x}\)都是凹函數因為它們的[二階導數永遠都是一個負值](https://zh.wikipedia.org/wiki/二階導數 "wikilink")。

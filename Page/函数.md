...
**函数**在[数学](../Page/数学.md "wikilink")中为两不为空集的[集合间的一种对应关系](https://zh.wikipedia.org/wiki/集合 "wikilink")：输入值集合中的每项元素皆能对应​​**唯一**一项输出值集合中的元素。例如[实数](../Page/实数.md "wikilink")x对应到其[平方](../Page/平方.md "wikilink")x<sup>2</sup>的关系就是一个函数，若以3作为此函数的输入值，所得的输出值便是9。

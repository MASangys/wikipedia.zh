...
作为一个直观的理解，我们可以把上面三种分化分别看成 \((-\infty, d]\cup(d, +\infty)\)、\((-\infty, d)\cap[d, +\infty)\) 和 \((-\infty, d)\cup(d, +\infty)\)，而“\(A\)中有最大数、\(A'\)中有最小数”的情况就是 \((-\infty, d]\cap[d, +\infty)\)，中间的分划点 d 同时（不合法地）属于两边集合。

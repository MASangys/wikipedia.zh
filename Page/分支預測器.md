...
分支预测器猜测[條件運算式](../Page/條件運算式.md "wikilink")两路分支中哪一路最可能发生，然后[推测执行](../Page/推测执行.md "wikilink")这一路的指令，来避免流水线停顿造成的时间浪费。如果后来发现分支预测错误，那么流水线中推测执行的那些中间结果全部放弃，重新取得正确的分支路线上的指令开始执行，这招致了程序执行的延迟。

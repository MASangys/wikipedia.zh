...
分配器的应用中比较特别的一点是，容器的实现过程中可能会假定分配器对指针与相关[整型的](../Page/整数_\(计算机科学\).md "wikilink")[类型定义与默认分配器所提供的等价](../Page/typedef.md "wikilink")，因而给定分配器类型的所有[实例在比较时常会得出](https://zh.wikipedia.org/wiki/对象_\(计算机科学\) "wikilink")“相等”的结果，而这一效果实际上恰与设计分配器的初衷背道而驰，并使带状态分配器的可用性大大受限\[9\]，斯特潘诺夫后来对此评论道：“（分配器）理论上说是不差的主意（……）但不幸的是在实践中无法发挥其功效。“他洞察到若要令分配器更加实用，就有必要针对核心语言的部分进行修改\[10\]。
...
定义自定义分配器的主要原因之一是提升性能。利用专用的自定义分配器可以提高程序的效能，又或提高内存使用效率，亦或两者兼而有之\[16\]\[17\]。默认分配器使用[`new`操作符分配存储空间](https://zh.wikipedia.org/wiki/New_\(C++\) "wikilink")，而这常利用[C语言堆分配函数](https://zh.wikipedia.org/wiki/C语言 "wikilink")（malloc()）实现\[18\]。由于堆分配函数常针对偶发的内存大量分配作优化，因此在为需要一次分配大量内存的容器（如[向量](https://zh.wikipedia.org/wiki/向量_\(C++\) "wikilink")、[双端队列](../Page/双端队列.md "wikilink")）分配内存时，默认分配器一般效率良好\[19\]。但是，对于与[双向链表这](../Page/双向链表.md "wikilink")-{zh-hans:类;zh-hant:類別;}-需要频繁分配少量内存的容器来说，若采用默认分配器分配内存，则通常效率很低\[20\]\[21\]。除此之外，基于malloc()的默认分配器还存在许多问题，诸如较差的引用局部性\[22\]，以及可能造成\[23\]\[24\]。

...
為了解決這個問題，使用了[多文件介面的原理設計出了分頁瀏覽器](https://zh.wikipedia.org/wiki/多文件介面 "wikilink")。僅使用一個視窗就可以用分頁來呈現多個網頁，另外因為多個網頁已經集中於一個視窗，因此不需要特別去調整視窗，記憶體也相對用量較少。如果碰到會開啟多個視窗的Script也只需要關掉一個視窗即可，同時為了解決前述這個問題，瀏覽器也相對提升了功能，例如提供了阻擋[彈出式廣告的特色](https://zh.wikipedia.org/wiki/彈出式廣告 "wikilink")。

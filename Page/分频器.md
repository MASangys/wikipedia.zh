...
再生分频器又称米勒分频器，是由罗杰·勒鲁瓦·米勒（Roger Leroy Miller）发明\[1\]，
可以输入信号和来自混频器的反馈信号混合。
[RFD_block_diagram_cn.svg](https://zh.wikipedia.org/wiki/File:RFD_block_diagram_cn.svg "fig:RFD_block_diagram_cn.svg")
其中反馈信号为\(f_{in}/2\)。这种分频器的混频器输出端会包含不同频率\(f_{in}/2\)、\(3f_{in}/2\)的和，而低通滤波器（LPF）
会滤除较高频率，然後将频率\(f_{in}/2\)放大并作为反馈输入混频器。

稳态检测似乎非常容易，不过启动电路是较为复杂的。为了建立平稳的1/2频率反馈，半频时的放大器增益必须大於单位频率时，而相移也必须是\(2{pi}\)的整数倍。

### 注入锁定分频器

有小量高频信号输入的自激[振荡器与其输入信号保持一致](https://zh.wikipedia.org/wiki/振荡器 "wikilink")。这种分频器在[电视的发展中是非常重要的](../Page/电视.md "wikilink")，被称为注入锁定分频器（injection
locked frequency
divider，**ILFD**）是一种工作方式与[注入锁定振荡器类似的分频器](https://zh.wikipedia.org/wiki/注入锁定振荡器 "wikilink")。注入锁定分频器的输入信号频率是自激振荡器的自激振荡频率的倍数或分数。当这种分频器的功率低於带宽静态分频器或基於触发器的分频器时，缺点是其同步范围小。
...
[Freq_Divider_mod_8.svg](https://zh.wikipedia.org/wiki/File:Freq_Divider_mod_8.svg "fig:Freq_Divider_mod_8.svg")实现的除8分频器\]\]
[Frequencydivider.png](https://zh.wikipedia.org/wiki/File:Frequencydivider.png "fig:Frequencydivider.png")实现的除2分频器\]\]
将数字信号进行整数倍分频可利用[扭环形计数器](https://zh.wikipedia.org/wiki/扭环形计数器 "wikilink")。扭环形计数器是一种[移位寄存器网络](../Page/移位寄存器.md "wikilink")，由输入信号作为时钟信号，最後一个移位寄存器的输出接回到第一个移位寄存器的输入，而输出信号出自於移位寄存器输出的组合。例如，一个除3分频器可以由一个3位扭环形计数器实现。每个移位寄存器的有效值是000、100、110、111、011以及001，这一模式不断重复，循环开始时由输入信号作为时钟信号进行同步。如果使用更多的移位寄存器构成扭环形计数器，就可以实现更大的除数。

实现2<sup>n</sup>整数分频，我们可以使用简单的二进制计数器，并将输入信号作为时钟信号。最低有效输出位轮换的速率是输入速率的1/2，紧接其後的第二位的速率是其速率的1/4，第三位的速率是其速率的1/8，以此类推。[触发器方案是实现整数分频的经典方法](../Page/触发器.md "wikilink")。这种分频是通过使频率和相位与来自温度等环境变化的信号源保持一致的方式实现的。最简单的配置是串接触发器，每个触发器可以实现除2。例如，3个串接触发器的电路可以实现除8。通过在触发器串接电路上附加逻辑门，可以实现其他的分频比。利用集成电路逻辑家族中的单片可以实现一些常用的分频比。
...
一般的分频器能在100 MHz（10<sup>8</sup> Hz）的频率下工作，最快的电路能工作在10<sup>11</sup>
Hz的频率，而飞秒激光[频率梳可得到数量级为](https://zh.wikipedia.org/wiki/频率梳 "wikilink")10<sup>15</sup>
Hz的频率。

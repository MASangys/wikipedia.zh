...
容易证明，刘维尔数一定是[无理数](https://zh.wikipedia.org/wiki/无理数 "wikilink")。若不然，则\(x = \frac{c}{d}, (c, d \in \mathbb{Z}, d > 0)\)。
取足够大的\(n\)使\({2^{n-1}} > d\)，在\(\frac{c}{d} \ne \frac{p}{q}\)时有
...
所有刘维尔数都是超越数，但反过来并不对。例如，著名的[e和](../Page/e_\(数学常数\).md "wikilink")[\(\pi\)就不是刘维尔数](https://zh.wikipedia.org/wiki/圆周率 "wikilink")。实际上，有[不可数多的超越数都不是刘维尔数](https://zh.wikipedia.org/wiki/不可数集 "wikilink")。
...
**证明**：令\(M = \max \left\{ \left|f'(x)\right| | x \in \left[ \alpha - 1, \alpha + 1 \right] \right\}\)，记\(f\)的其它的不重复的根为
\(\alpha_1, \alpha_2, ..., \alpha_m\)，取这样的A
...
取满足\(\frac{1}{2^r} \le A\)的正整数\(r\)，并令\(m = r + n\)，存在整数\(a, b\)其中
\(b > 1\)有
...
## 外部链接
  ...
  - [The Beginning of Transcendental
    Numbers](http://www.math.sc.edu/~filaseta/gradcourses/Math785/Math785Notes5.pdf)

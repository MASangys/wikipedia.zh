...
equation）是指一个[微分方程](../Page/微分方程.md "wikilink")，其[數值分析的解只有在時間間隔很小時才會](../Page/數值分析.md "wikilink")[穩定](../Page/數值穩定性.md "wikilink")，只要時間間隔略大，其解就會不穩定。目前很難去精确地去定義哪些微分方程是刚性方程，然而粗略而言，若此方程式中包含使其快速變動的項，則其為剛性方程。
...
考虑下面的[初值问题](../Page/初值问题.md "wikilink")：

\[\,y'(t)=-15y(t),\quad t \ge 0,\, y(0)=1\] 其精确解是

\[y(t)=e^{-15t}\]，并且显然当\(t\to\infty\)时\(y(t)\to 0\)。
會希望[数值解能够具有相同的特性](../Page/数值解.md "wikilink")。

若以[歐拉方法來求數值解](../Page/歐拉方法.md "wikilink")，則使用不同的步长（step
size）將會得到不同的結果。第一种，步长\(h=1/4\)的[欧拉法强烈的震荡并且很快离开了图的边界](../Page/欧拉法.md "wikilink")。当将步长减半为\(h=1/8\)时，得到的结果在图的范围以内。但是它依然在0附近震荡，并且不可能表示精确的解。

而[梯形法](../Page/梯形法.md "wikilink")，即两阶段，表达为

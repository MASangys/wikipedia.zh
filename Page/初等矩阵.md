[线性代数中](../Page/线性代数.md "wikilink")，**初等矩阵**（又稱為**基本矩陣**\[1\]）是一个与[单位矩阵只有微小区别的](https://zh.wikipedia.org/wiki/单位矩阵 "wikilink")[矩阵](../Page/矩阵.md "wikilink")。具体来说，一个*n*阶单位矩阵*E*经过一次初等行变换或一次初等列变换所得矩阵称为*n*阶初等矩阵。\[2\]
...
初等行变换不影响[线性方程组的解](../Page/线性方程组.md "wikilink")，也可用于[高斯消元法](https://zh.wikipedia.org/wiki/高斯消元法 "wikilink")，用于逐渐将系数矩阵化为标准形。初等行变换不改变矩阵的[核](https://zh.wikipedia.org/wiki/核_\(矩阵\) "wikilink")（故不改变解集），但改变了矩阵的[像](https://zh.wikipedia.org/wiki/像_\(數學\) "wikilink")。反过来，初等列变换没有改变像却改变了核。

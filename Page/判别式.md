...
当多项式的系数不是实数或复数[域时](https://zh.wikipedia.org/wiki/域 "wikilink")，同样有判别式的概念。判别式总是系数域中的元素。这时，判别式为零当且仅当多项式在它的[分裂域中有重根](../Page/分裂域.md "wikilink")。判别式的通常形式为：

\[a_n^{2n-2}\prod_{i<j}{(r_i-r_j)^2}\]

其中的\(a_n\)是多项式的最高次项系数，\(r_1, ..., r_n\)是多项式在某个分裂域中的根（如有重根的按重数重复排列）。

判别式的概念也被推广到了多项式以外的其它[代数结构](../Page/代数结构.md "wikilink")，比如说[圆锥曲线](../Page/圆锥曲线.md "wikilink")、[二次型和](../Page/二次型.md "wikilink")[代数数域中](../Page/代数数域.md "wikilink")。在[代数数论中](https://zh.wikipedia.org/wiki/代数数论 "wikilink")，判别式与所谓的“[分歧](https://zh.wikipedia.org/wiki/分歧_\(代数\) "wikilink")”的概念紧密相关。实际上，愈为几何的分歧类型对应着愈为抽象的判别式类型，因此在许多方面判别式都是一个中心概念。判别式在本质上表现为相应[行列式的计算](../Page/行列式.md "wikilink")。
...
多项式的判别式可以在任意的[域中定义](https://zh.wikipedia.org/wiki/域 "wikilink")，定义方式一样。带有根\(r_i\,\)的表达式仍然有效，只是根要在系数域的某个[分裂域中取](../Page/分裂域.md "wikilink")。
...
它决定了圆锥曲线的[形状](https://zh.wikipedia.org/wiki/形状 "wikilink")。如果判别式小于0，则是[椭圆或](../Page/椭圆.md "wikilink")[圆](../Page/圆.md "wikilink")。如果判别式等于0，则是一条[抛物线](../Page/抛物线.md "wikilink")。如果大于0，则是[双曲线](../Page/双曲线.md "wikilink")。这个公式不适用于退化的情形（当这个多项式可以因式分解时）。

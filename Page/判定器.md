...
有附标 *e* 的图灵机是否在所有输入上停机的[判定问题是不可判定的](https://zh.wikipedia.org/wiki/判定问题 "wikilink")。事实上，这个问题位于[算术层次的](https://zh.wikipedia.org/wiki/算术层次 "wikilink") \(\Pi^0_2\) 级别。因此这个问题严格的比[停机问题](../Page/停机问题.md "wikilink")更困难，它问有附标 *e* 的机器是否停机在输入 *0* 上。直觉的说，在不可解决性上的这个困难在于每个“全机器”问题的实例提出无限多个停机问题的实例。

...
其中\(E\)代表統計[平均](https://zh.wikipedia.org/wiki/平均 "wikilink")。於是由[維納-辛欽定理](https://zh.wikipedia.org/wiki/維納-辛欽定理 "wikilink")(Wiener–Khinchin
theorem)，平穩[隨機過程的功率譜是](https://zh.wikipedia.org/wiki/隨機過程 "wikilink")[自相關函數的](https://zh.wikipedia.org/wiki/自相關函數 "wikilink")[傅立葉轉換](https://zh.wikipedia.org/wiki/傅立葉轉換 "wikilink")，即:
...
其中\(r_{xx}(m)\)是\(x(n)\)的真實(統計的)自相關序列。因此\(r_{xx}^'(m)\)是[自相關函數](https://zh.wikipedia.org/wiki/自相關函數 "wikilink")\(r_{xx}(m)\)的無偏差估計。估計\(r_{xx}^'(m)\)的[方差近似為](../Page/方差.md "wikilink"):
...
然而一般來說當\(N \rightarrow \infty\)時估計\(P_{xx}(f)\)的[方差不會衰減到零](../Page/方差.md "wikilink")。例如當數據序列是一個[隨機過程時](https://zh.wikipedia.org/wiki/隨機過程 "wikilink")，方差是\(var[P_{xx}(f)] = \Gamma_{xx}^2(f)[1+(\frac{sin2\pi fN}{Nsin2\pi f})^2]\),當\(N \rightarrow \infty\)時，極限為\(\lim_{N \to \infty}var[P_{xx}(f)] = \Gamma_{xx}^2(f)\)。
...
從\(E[P_{xx}^{(i)}(f)]\)的公式注意到，現在真實譜與巴特利特窗的[頻率特性](https://zh.wikipedia.org/wiki/頻率 "wikilink")\(W_B(f)\)有關。數據長度從\(N\)點減小到\(M=N/K\)，導致窗口的譜寬度增加\(K\)因子。結果[頻率分辨率得到減小因子](https://zh.wikipedia.org/wiki/頻率 "wikilink")\(K\)。分辨率降低的結果使得[方差減小](../Page/方差.md "wikilink")。Bartlett估計的[方差是](../Page/方差.md "wikilink"):

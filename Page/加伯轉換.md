...
將[短時距傅立葉變換中的](../Page/短時距傅立葉變換.md "wikilink")[窗函數代入](https://zh.wikipedia.org/wiki/窗函數 "wikilink")[高斯函數](https://zh.wikipedia.org/wiki/高斯函數 "wikilink")，即可得下面的定義。

\[G_x(t,f)=\int_{-\infty}^{\infty}e^{-\pi (\tau-t)^2} e^{-j2 \pi f \tau}x(\tau)\, d\tau\]
根據[高斯函數會從兩側遞減的性質](https://zh.wikipedia.org/wiki/高斯函數 "wikilink")，我們可以將上式進一步化簡：
...
1.  其他[窗函數的](https://zh.wikipedia.org/wiki/窗函數 "wikilink")[短時距傅立葉變換](../Page/短時距傅立葉變換.md "wikilink")，如[短時距傅立葉變換提到的方形窗函數](../Page/短時距傅立葉變換.md "wikilink")，無法同時兼顧時間軸和頻率軸的解析度；一者解析度提升，另一者解析度必定下降。但高斯函數由[海森堡測不準原理可得知](https://zh.wikipedia.org/wiki/海森堡測不準原理 "wikilink")，是最能同時讓兩軸兼顧解析度的窗函數(將於下個章節詳述)。
...
由於高斯[窗函數的寬度可以由其變異數做調整](https://zh.wikipedia.org/wiki/窗函數 "wikilink")，因此我們將這個參數加入加伯轉換的數學式子中，讓轉換更加彈性。

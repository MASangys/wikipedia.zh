...
**动态主机設定協定**（，缩写：**DHCP**）是一個[局域网的](../Page/局域网.md "wikilink")[网络协议](https://zh.wikipedia.org/wiki/网络协议 "wikilink")，使用[UDP协议工作](../Page/用户数据报协议.md "wikilink")，主要有兩個用途：
...
<p>客户在物理子网上发送广播来寻找可用的服务器。网络管理员可以配置一个本地路由来转发DHCP包给另一个子网上的DHCP服务器。该客户实现生成一个目的地址为255.255.255.255或者一个子网广播地址的<a href="https://zh.wikipedia.org/wiki/UDP" title="wikilink">UDP包</a>。</p>

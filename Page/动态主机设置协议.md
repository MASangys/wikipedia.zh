...
**动态主机設定協定**（，缩写：**DHCP**）是一個[局域网的](../Page/局域网.md "wikilink")[网络协议](../Page/网络协议.md "wikilink")，使用[UDP协议工作](../Page/用户数据报协议.md "wikilink")，主要有兩個用途：
...
2071）的时候，就可以在尽可能少的地方去做这些改动。对于一些设备，如[路由器和](../Page/路由器.md "wikilink")[防火墙](../Page/防火墙.md "wikilink")，则不应使用DHCP。把[TFTP或](../Page/TFTP.md "wikilink")[SSH服务器放在同一台运行DHCP的机器上也是有用的](../Page/Secure_Shell.md "wikilink")，目的是为了集中管理。

DHCP也可用于直接为服务器和桌面计算机分配地址，并且通过一个[PPP代理](../Page/PPP.md "wikilink")，也可为拨号及宽带主机，以及住宅[NAT](../Page/NAT.md "wikilink")[网关和路由器分配地址](../Page/网关.md "wikilink")。DHCP一般不适用于使用在无边际路由器和[DNS服务器上](../Page/DNS.md "wikilink")。

## 歷史

DHCP於1993年10月成為標準協議，其前身是[BOOTP協議](../Page/BOOTP.md "wikilink")。當前的DHCP定義可以在RFC
...
DHCP统一使用两个[IANA分配的端口作为](../Page/IANA.md "wikilink")[BOOTP](../Page/BOOTP.md "wikilink")：[服务器端使用](../Page/服务器端.md "wikilink")67/udp，[客户端使用](../Page/客户端.md "wikilink")68/udp。

DHCP运行分为四个基本过程，分别为请求IP租约、提供IP租约、选择IP租约和确认IP租约。

客户在获得了一个IP地址以后，就可以发送一个[ARP请求来避免由于DHCP服务器地址池重叠而引发的IP冲突](../Page/ARP.md "wikilink")。
...
<tbody>
<tr class="odd">
<td><p><a href="../Page/UDP.md" title="wikilink">UDP</a> Src=0.0.0.0<br />
...
<tr class="odd">
<td><p>192 octets of 0's. <a href="../Page/BOOTP.md" title="wikilink">BOOTP</a> legacy</p></td>
</tr>
<tr class="even">
...
<p>客户在物理子网上发送广播来寻找可用的服务器。网络管理员可以配置一个本地路由来转发DHCP包给另一个子网上的DHCP服务器。该客户实现生成一个目的地址为255.255.255.255或者一个子网广播地址的<a href="../Page/UDP.md" title="wikilink">UDP包</a>。</p>
...
<p>当客户PC收到一个IP租约提供时，它必须告诉所有其他的DHCP服务器它已经接受了一个租约提供。因此，该客户会发送一个DHCPREQUEST消息，其中包含提供租约的服务器的IP。当其他DHCP服务器收到了该消息后，它们会收回所有可能已提供给客户的租约。然后它们把曾经给客户保留的那个地址重新放回到可用地址池中，这样，它们就可以为其他计算机分配这个地址。任意数量的DHCP服务器都可以响应同一个IP租约请求，但是每一个客户网卡只能接受一个租约提供。</p></td>

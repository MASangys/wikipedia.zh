[Crystal_128_yast_dhcp.png](https://zh.wikipedia.org/wiki/File:Crystal_128_yast_dhcp.png "fig:Crystal_128_yast_dhcp.png")  **动态主机設定協定**（，缩写：**DHCP**）是一個用于[局域网](../Page/局域网.md "wikilink")的[网络协议](https://zh.wikipedia.org/wiki/网络协议 "wikilink")，位于[OSI模型](../Page/OSI模型.md "wikilink")的[应用层](https://zh.wikipedia.org/wiki/应用层 "wikilink")，使用[UDP协议工作](../Page/用户数据报协议.md "wikilink")，主要有兩個用途：
...
甚至对于那些很少改变地址的服务器来说，DHCP仍然被建议用来设置它们的地址。如果[服务器](../Page/服务器.md "wikilink")需要被重新分配地址（RFC 2071）的时候，就可以在尽可能少的地方去做这些改动。对于一些设备，如[路由器](../Page/路由器.md "wikilink")和[防火墙](../Page/防火墙.md "wikilink")，则不应使用DHCP。把[TFTP或](https://zh.wikipedia.org/wiki/TFTP "wikilink")[SSH服务器放在同一台运行DHCP的机器上也是有用的](../Page/Secure_Shell.md "wikilink")，目的是为了集中管理。

DHCP也可用于直接为服务器和桌面计算机分配地址，并且通过一个[PPP](../Page/PPP.md "wikilink")代理，也可为拨号及宽带主机，以及住宅[NAT](https://zh.wikipedia.org/wiki/NAT "wikilink")[网关](../Page/网关.md "wikilink")和路由器分配地址。DHCP一般不适用于使用在无边际路由器和[DNS服务器上](https://zh.wikipedia.org/wiki/DNS "wikilink")。

...
16位Windows，所有进程共用同一个内存地址空间。如果一个DLL被多个进程使用，它只被加载到内存中一次，只有一份数据节（data segment）。也就是说，DLL是系统全局而不是进程内的；进程不能得到DLL的一份独立的拷贝。每个DLL在内存中只有一份实例（instance）。\[1\]
...
如果是遵从C命名规范（naming convention）的外部名字，它们必须在C++代码中声明为`extern "C"`以避免它们使用C++命名规范。如果使用dll的语言（如[Fortran](../Page/Fortran.md "wikilink")、[Visual Basic 6](https://zh.wikipedia.org/wiki/Visual_Basic_6 "wikilink")）不能识别C命名规范，那么需要采取办法指出在DLL中输出函数的名字。也可以通过DEF文件来定义输出函数的名字与序号。\[3\]或者使用如下的链接指令来制定dll输出函数名字：
...
1.  [Raymond Chen：“What is the difference between HINSTANCE and HMODULE?”，in 《old new thing》，MSDN，，June 14, 2004](https://blogs.msdn.microsoft.com/oldnewthing/20040614-00/?p=38903/)

2.  \[<https://msdn.microsoft.com/en-us/library/d91k01sh(v=vs.140>).aspx MSDN:Exporting from a DLL Using DEF Files\]

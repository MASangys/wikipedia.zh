...
Memory**，**DRAM**）是一种[半导体](../Page/半导体.md "wikilink")[記憶體](https://zh.wikipedia.org/wiki/記憶體 "wikilink")，主要的作用原理是利用[電容內儲存](../Page/電容.md "wikilink")[電荷的多寡來代表一個](../Page/電荷.md "wikilink")[二进制](https://zh.wikipedia.org/wiki/二进制 "wikilink")[位元](../Page/位元.md "wikilink")（bit）是1還是0。由於在現實中電晶體會有漏電電流的現象，導致電容上所儲存的電荷數量並不足以正確的判別資料，而導致資料毀損。因此對於DRAM來說，周期性地充電是一個無可避免的要件。由於這種需要定時刷新的特性，因此被稱為「動態」記憶體。相對來說，靜態記憶體（[SRAM](../Page/静态随机存取存储器.md "wikilink")）只要存入資料後，縱使不刷新也不會遺失記憶。
...
與大部分的[隨機存取記憶體](https://zh.wikipedia.org/wiki/隨機存取記憶體 "wikilink")（RAM）一樣，由於存在DRAM中的資料會在電力切斷以後很快消失，因此它屬於一種[揮發性記憶體](../Page/揮發性記憶體.md "wikilink")（volatile
memory）設備。
...
DRAM通常以一个[电容和一个](https://zh.wikipedia.org/wiki/电容 "wikilink")[電晶體为一个单元排成二维矩阵](https://zh.wikipedia.org/wiki/電晶體 "wikilink")，左图所示是一个4×4的矩阵。基本的操作機制分為讀(Read)和寫(Write)，讀的時候先讓Bitline(BL)先充電到操作電壓的一半，然後在把電晶體打開讓BL和電容產生電荷共享的現象，若內部儲存的值為1，則BL的電壓會被電荷共享抬高到高於操作電壓的一半，反之，若內部儲存的值為0，則會把BL的電壓拉低到低於操作電壓的一半，得到了BL的電壓後，在經過放大器來判別出內部的值為0和1。寫的時候會把電晶體打開，若要寫1時則把BL電壓抬高到操作電壓使電容上儲存著操作電壓，若要寫0時則把BL降低到0伏特使電容內部沒有電荷。

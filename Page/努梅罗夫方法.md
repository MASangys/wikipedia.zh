**努梅罗夫方法**属于四阶线性多步法，用于求解不出现一阶微分项的二阶[常微分方程](../Page/常微分方程.md "wikilink")。努梅罗夫方法属于[隐式方法](https://zh.wikipedia.org/wiki/隐式方法 "wikilink")，但如果微分方程线性，则可转化为[显式方法](https://zh.wikipedia.org/wiki/显式方法 "wikilink")。该方法由
俄国天文学家[Boris Vasil'evich
Numerov提出](https://zh.wikipedia.org/wiki/Boris_Vasil'evich_Numerov "wikilink")。
...
求出函数 \(y(x)\) 在区间 \([a, b]\) 上等距格点上的值，从连续的两个格点上的函数值 \(x_{n-1}\) 和
\(x_n\) 开始，其他的函数值可由

\[y_{n+1} = \frac {\left( 2-\frac{5 h^2}{6} f_n \right) y_n - \left( 1+\frac{h^2}{12}f_{n-1} \right)y_{n-1}}{1+\frac{h^2}{12}f_{n+1}}\]
算得。

其中， \(f_n=f(x_n)\) 和 \(y_n=y(x_n)\) 为在格点 \(x_n\)
上的函数值，\(h=x_n-x_{n-1}\)为格点间距。
...
该式为隐式的[线性多步方法](https://zh.wikipedia.org/wiki/线性多步方法 "wikilink")。当 \(f\)
是 \(y\) 的线性函数时，该式变为显式方法，精度为4阶。
...
从 \(y(x_n)\) 的[泰勒展开开始](https://zh.wikipedia.org/wiki/泰勒展开 "wikilink")，
我们可求 \(x_n\) 的相接邻点上的函数值
...
将其代入到四阶微分项中，并把二阶导
\(\frac{d^2}{d x^2} \left[ f(x) y(x) \right]\) 替换为 \(f_n y_n\) 的二阶差分公式
\(\frac{f_{n-1} y_{n-1} -2 f_{n} y_{n} + f_{n+1} y_{n+1}}{h^2}\)
...
## 外部链接
  ...
  - [Lecture notes: Computerphysik und
    Numerik](http://www.jkrieger.de/download/computerphysik.pdf) - by
    Jan Krieger
  - [Lecture notes of Werner
    Scholz](http://magnet.atp.tuwien.ac.at/scholz/projects/numerov/node2.html#SECTION00020000000000000000)
    - At Vienna University of Technology
  - [Lecture notes of Alexander
    Wagner](http://www.stellarcom.org/aw/Physik/cp/node48.html)

...
設\(A\)和\(B\)為集合。稱它們**等勢**，指的是存在*\(A\)*到*\(B\)*一個[雙射](https://zh.wikipedia.org/wiki/雙射 "wikilink")\(f\)，即*\(A\)*中的元素可以與*\(B\)*中的元素[一一對應起來](https://zh.wikipedia.org/wiki/一一對應 "wikilink")。例子：集合\(A=\{1,2,3\}\)與\(B=\{\)蘋果,馬,園丁\(\}\)等勢，這是因為「\(1\rightarrow\)蘋果,
\(2\rightarrow\)馬, \(3\rightarrow\)園丁」是兩個集合之間的一一對應。不過在這個例子中,
不用等勢的概念也知道它們的元素不多不少,
是3個。對於無窮集可舉一個例子如下：[正](https://zh.wikipedia.org/wiki/正數 "wikilink")[偶數集合](https://zh.wikipedia.org/wiki/偶數 "wikilink")\(E=\{2,4,6,\ldots\}\)和[自然數集合](https://zh.wikipedia.org/wiki/自然數 "wikilink")\(\mathbb{N}=\{1,2,3,\ldots\}\)等勢，這是因為由公式\(f(n)=2n\)所決定的函數\(f:\mathbb{N}\rightarrow E\)是一個由\(\mathbb{N}\)到\(E\)的雙射。
...
集合\(S\)的勢通常標記為\(|S|\)。其[冪集的勢則通常標記為](../Page/冪集.md "wikilink")\(2^{|S|}\)。
...
<!-- end list -->
  ...
  - 若對於兩個集合\(X\)和\(Y\)有\(|X|\) ≤
    \(|Y|\)，則存在一\(Y\)的[子集](../Page/子集.md "wikilink")\(Z\)使得\(|X| = |Z|\)。

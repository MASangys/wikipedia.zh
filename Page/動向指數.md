**動向指數**（Directional Movement
Index，**DMI**），又稱**動向指標**、**趨向指標**，由[美國威爾德](https://zh.wikipedia.org/wiki/美國 "wikilink")（[Welles
Wilder
JR.](https://zh.wikipedia.org/wiki/:en:J._Welles_Wilder "wikilink")）提出的技術分析方法，「基本原理，是股價在上漲及下跌過程中，藉創新高價或新低價的動能，研判多空買賣力道，藉以尋求多空雙方力道的均衡點，以及股價在多空雙方互動下，波動的趨劫循環過程。」\[1\]是用以研判市場[多](https://zh.wikipedia.org/wiki/多頭 "wikilink")[空趨勢的指標](https://zh.wikipedia.org/wiki/空頭 "wikilink")，能預告「頭部確認」、「底部完成」的訊號，是為判斷波段走勢相當實用的[技術分析方法](https://zh.wikipedia.org/wiki/技術分析 "wikilink")。
...
<!-- end list -->
  ...
  -
    「所謂DM值，今日股價波動幅度大於昨日股價波動幅部分的最大值，可能是創高價的部分或創低價的部份；如果今日股價波動幅度較前一日小，則DM
    = 0。」\[3\]
...
2.  若為**下跌趨勢**，記作
    -DM。-DM的負號（–）是表示**反向趨勢**（下跌），並非數值為[負數](https://zh.wikipedia.org/wiki/負數 "wikilink")。\[4\]
3.  其他狀況：DM = 0。
...
<!-- end list -->
  ...
  -
    先計算出
    +DM、–DM及TR的14日[算術平均數](https://zh.wikipedia.org/wiki/算術平均數 "wikilink")，得到
    +DM14、–DM14及TR14三組數據作為起始值，再計算各自的[移動平均值](https://zh.wikipedia.org/wiki/移動平均#指數移動平均 "wikilink")（EMA）。
...
<!-- end list -->
  ...
  - 二、計算「**趨向指數**」（Directional Movement
    Index，**DX**）及「**平均趨向指數**」（Average
...
<!-- end list -->
  ...
  -
    「所謂 DM 值，今日股價波動幅度大於昨日股價波動幅部分的最大值，可能是創高價的部分或創低價的部份；如果今日股價波動幅度較前一日小，則
    DM = 0。」\[5\]

<!-- end list -->

1.  若\(H_t - H_{t-1} > 0\) 且
    \(H_t - H_{t-1} > L_{t-1} - L_t\)，則表示股價高點持續走高，為**上漲趨勢**，記作
    \(+DM\)。
2.  若\(L_{t-1} - L_t > 0\) 且
    \(H_t - H_{t-1} < L_{t-1} - L_t\)，則為**下跌趨勢**，記作
    \(-DM\)。\(-DM\)的負號（–）是表示**反向趨勢**（下跌），並非數值為[負數](https://zh.wikipedia.org/wiki/負數 "wikilink")。\[6\]
...
<!-- end list -->
  ...
  -
    所謂真實波幅（TR）是以\(H_t\) ，\(L_t\)
    ，及\(C_{t-1}\)三個價格做比較，求出當日股價波動的最大幅度。當日最高價\(H_t\)永遠大於最低價\(L_t\)，因此有下例三種狀況，其真實波幅
    TR 值計算如下：
...
<!-- end list -->
  ...
  -
    先計算出 +DM、–DM 及 TR
    的14日[算術平均數](https://zh.wikipedia.org/wiki/算術平均數 "wikilink")，得到
    +DM14、–DM14 及 TR14
    三組數據作為起始值，再計算各自的[移動平均值](https://zh.wikipedia.org/wiki/移動平均#指數移動平均 "wikilink")：
...
\[-DI14 = \frac{-DM14}{TR14} \times 100\]
  ...
  -
    真實波幅（TR）為股價波動的最大波幅，趨向指標（DM）不出其外，因此方向指標（DI）的值將介於 0\~100
    之間。例如　+DI14=45，–DI14=23，這代表過去 14 日中向上的趨勢佔 45%，向下的趨勢佔
    23%，其他的則沒有方向性。
...
\[DX = \frac{| (+DI14) - (-DI14) |}{| (+DI14) + (-DI14) |}\]
  ...
  -
    其中 |　|
    為[絕對值符號](https://zh.wikipedia.org/wiki/絕對值 "wikilink")，表示將運算後結果取其數值，而不論正負。再將
    DX 作[移動平均](../Page/移動平均.md "wikilink")，得到 ADX。

## 指標應用

  - \+DI與–DI表示多空相反的二個動向，當據此繪出的兩條曲線彼此糾結相纏時，代表上漲力道與下跌力道相當，多空勢均力敵。當
    +DI與–DI彼此穿越時，由下往上的一方其力道開始壓過由上往下的另一方，此時出現買賣訊號。
  - ADX可作為趨勢行情的判斷依據，當行情明顯朝多空任一方向進行時，ADX數值都會顯著上升，趨勢走強。若行情呈現盤整格局時，ADX會低於
    +DI與–DI二條線。若ADX數值低於20，則不論DI表現如何，均顯示市場沒有明顯趨勢。
...
3.
4.  \-DM的負號（–）是表示趨勢動向為**反向**（下跌），有別於上漲趨勢的
    +DM，為股價最低價跌幅大過最高價漲幅，並非數值為[負數](https://zh.wikipedia.org/wiki/負數 "wikilink")。此觀念亦應用在DMI其他指標中（如：+DI、-DI），表示二個不同的趨勢動向。

...
預先運算生成的電腦圖像中，如[電腦合成影像的電影](https://zh.wikipedia.org/wiki/電腦合成影像 "wikilink")，可以繪出逼真的動態模糊，因為渲染器（renderer）有更多時間來繪製每個畫面。時間上避免失真需依照許多瞬間時間的組合來製造畫面。
...
<File:City> lights in
motion.jpg|這些城市燈光是固定的而相機的方向在曝光期間改變。虛線是由於相機來回移動造成的每盞燈的個別軌跡。
<File:Truck> with motion
blur.jpg|卡車和相機在曝光期間皆移動。因為移動相機或搖動鏡頭來固定指向卡車駕駛座旁的門，所以地面看起像往卡車後方移動。
<File:Tj> pullingout.jpg|相對於車輛的快速移動相機快門速度較慢，街道上的車流留下亮眼的光線。
<File:Nightly> Rotation above San Jose International
Airport.jpg|夜間搭上在聖荷西國際機場上方剛起飛的飛機。城市燈光形成同心線條。
...
假設有一清晰平面圖片\(y(x)\)，我們只能觀察到其模糊後的圖片\((y \star psf)(x)\)
,\(psf(x)\)為已知的[點擴散函數PSF](https://zh.wikipedia.org/wiki/點擴散函數 "wikilink")(Point
Spread Function), \(\star\)表示捲積（摺積）（convolution）。
...
\(\epsilon(x)\)為雜訊。
\(x\)為分佈在整齊的\(n_1\)×\(n_2\)格子X中，\(X=\{k_1,k_2: k_1=1,2,...,n_1, k_2=1,2,...,n_2\}\)。
...
\[R= \frac{MTF^{*}}{|MTF|^2+\frac{1}{SNR}}B\]
其他「非遞迴式」方法，例如：調整濾波器（Regularized
filter），引入回復圖像的限制條件。

...
**包络检波器**（）是以高频信号为输入信号并提供原始信号的的一种电子线路。电路中的[电容器会在上升沿充电](../Page/电容器.md "wikilink")，并在信号下降时通过[電阻器缓慢释放电荷](https://zh.wikipedia.org/wiki/電阻器 "wikilink")。串联的二极管将输入信号[整流](../Page/整流器.md "wikilink")，只在正输入端比负输入端电位高的时候允许电流流过。
...
在[音乐中有时包络检波器也被称为](../Page/音乐.md "wikilink")**包络跟随器**。它还用来检测输入信号的[振幅变化](https://zh.wikipedia.org/wiki/振幅 "wikilink")，以产生类似于这些变化的控制信号。
但在此情况下的输入信号是由可听频率。

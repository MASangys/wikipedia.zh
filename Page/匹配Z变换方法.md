...
**匹配Z变换方法**（matched Z-transform method）也稱為**極點-零點映射**（pole–zero mapping）\[1\]\[2\]或**極點－零點匹配法**（pole–zero matching method）\[3\]，簡稱**MPZ**或**MZT**\[4\]，是將濾波器轉換到離散時間濾波器（[数字滤波器](../Page/数字滤波器.md "wikilink")）設計的技巧。
...
一般情形下，類比濾波器的極點會比零點多，在\(s=\infty\)處的零點可以移到奈奎斯特頻率，作法是放在\(z=-1\)的位置 \[9\]\[10\]\[11\]\[12\]。

此轉換方式可以保持[有界輸入有界輸出穩定性](../Page/有界輸入有界輸出穩定性.md "wikilink")以及[最小相位](../Page/最小相位.md "wikilink")，但不會保持時域或是頻域的響應，因此不常使用\[13\]\[14\]。較常使用的方式有[雙線性轉換](../Page/雙線性轉換.md "wikilink")及[冲激不变法](../Page/冲激不变法.md "wikilink")\[15\]。匹配Z变换方法的高頻響應誤差比雙線性轉換要小，因此比較容易透過加入額外的零點來修正其特性，此方式稱為MZTi（i表示改良版improved）\[16\]。

在數位控制中，匹配Z变换方法有一個特別的應用，就是，可以調整[可控制性](../Page/可控制性.md "wikilink")系統的極點，一般會將不穩定（或接近不穩定）的極點調整到穩定的位置。

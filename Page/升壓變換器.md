...
[開關電源為了要有高效率](https://zh.wikipedia.org/wiki/開關電源 "wikilink")，其開關需要快速的打開及關閉，而且損失要低。1950年代商用[半導體開關的發明對開關電源非常重要](https://zh.wikipedia.org/wiki/半導體 "wikilink")，因此像升壓變換器之類的開關電源才得以進行。主要的DC-DC轉換器技術是在1960年代初期，可以購得半導體開關時發展的。[航天產業需要體積小](../Page/航天.md "wikilink")、輕量化而且高效率的電源轉換器，因此開關電源快速發展。

像開關電源之類的切換式系統在設計上是一大挑戰，因為其型態和開關何時導通、何時斷路有關。[加利福尼亞理工學院的](../Page/加利福尼亞理工學院.md "wikilink")在1977年出版了現今使用的DC-DC轉換器模型。Middlebrook將每一個開關狀庇下的電流組態用一種叫做狀態空間平均（state-space
averaging）的方式加以平均。因此將二個（開關導通時、開關斷路時）不同的系統方程式變成一個。新的模型帶來有见地的设计方程，也帶動了開關電源的成長。

## 應用

[BoostConverter_TI.jpg](https://zh.wikipedia.org/wiki/File:BoostConverter_TI.jpg "fig:BoostConverter_TI.jpg")[計算機上的升壓變換器](https://zh.wikipedia.org/wiki/計算機 "wikilink")，可以從二顆[AA电池提供的](https://zh.wikipedia.org/wiki/五号电池 "wikilink")2.4V電源提昇到9V\]\]
電池供電的系統一般會將電池串聯來提較高的電壓，不過在許多高電壓的應用中，因為空間限制，無法用足夠多個的電池串聯到所需的電壓。升壓變換器可以提昇電壓，減少所需的電池數量。像[電動車及照明系統就是利用電池再配合升壓變換器供電的系統](https://zh.wikipedia.org/wiki/電動車 "wikilink")。

NHW20的[豐田普銳斯](../Page/豐田普銳斯.md "wikilink")[混合動力車使用](https://zh.wikipedia.org/wiki/混合動力車 "wikilink")500V的電動機，若沒有升壓變換器，需要使用將近417個電池來驅動電動機，不過豐田普銳斯只用了168個電池，再利用升壓變換器將電池總電壓從202
V提昇到500
V。升壓變換器也可以用來作一些較小型設備的供電，例如可攜式照明系統，像[白光LED一般需要](https://zh.wikipedia.org/wiki/白光LED "wikilink")3.3V才能發光，配合升壓變換器可以用鹼式電池提供的1.5V電壓，升壓後再供電。
...
升壓變換器的基本原理就是利用[電抗器在電流變化時會產生或消除磁場](https://zh.wikipedia.org/wiki/電抗器 "wikilink")，來抵抗電流的變化。
...
[Boost_chronogram.svg](https://zh.wikipedia.org/wiki/File:Boost_chronogram.svg "fig:Boost_chronogram.svg")
升壓變換器的連續模式是指在運作時電感器電流（\(I_L\)）不會降到零，圖3為此模式下的電壓及電流。
輸出電壓可以用以下方計算，假設使用的是理想元件，在穩態下操作\[1\]：
...
## 外部連結
  ...
  - [Video tutorial explaining boost converters with practical circuit
    design
    example](http://afrotechmods.com/tutorials/2014/12/21/dc-dc-boost-converter-tutorial/)
  - [Explanation of nonlinear behavior, modeling, and linearization of
    the boost dc/dc
    converter](https://www.researchgate.net/publication/307936847_Invited_Lecture_-_Boost_DCDC_Converter_Nonlinearity_and_RHP-Zero_Survey_of_the_Control-to-Output_Transfer_Function_Linearization_Methods)
  - [Linearization of boost power stage, small-signal
    analysis](http://www.postreh.com/vmichal/papers/modulated_ramp_pwm_generator_boost.pdf)

**单例模式**，也叫**单子模式**，是一种常用的[软件设计模式](https://zh.wikipedia.org/wiki/软件设计模式 "wikilink")。在应用这个模式时，单例对象的[类必须保证只有一个实例存在](../Page/类_\(计算机科学\).md "wikilink")。许多时候整个系统只需要拥有一个的全局[对象](https://zh.wikipedia.org/wiki/对象 "wikilink")，这样有利于我们协调系统整体的行为。比如在某个[服务器程序中](../Page/服务器.md "wikilink")，该服务器的配置信息存放在一个[文件中](https://zh.wikipedia.org/wiki/文件 "wikilink")，这些配置数据由一个单例对象统一读取，然后服务[进程中的其他对象再通过这个单例对象获取这些配置信息](https://zh.wikipedia.org/wiki/进程 "wikilink")。这种方式简化了在复杂环境下的配置管理。
...
在[Java编程语言中](https://zh.wikipedia.org/wiki/Java编程语言 "wikilink")，单例模式(懒汉模式)应用的例子如下述代码所示

...
单元测试的目标是隔离程序部件并证明这些单个部件是正确的。\[2\]一个单元测试提供了代码片断需要满足的严密的书面规约。因此，单元测试带来了一些益处。
單元測試在[軟體開發過程的早期就能發現問題](https://zh.wikipedia.org/wiki/軟體開發過程 "wikilink")。

### 适应变更

单元测试允许程序员在未来[重构代码](https://zh.wikipedia.org/wiki/重构 "wikilink")，并且确保模块依然工作正确（[复合测试](https://zh.wikipedia.org/wiki/复合测试 "wikilink")）。这个过程就是为所有[函数和](../Page/函数.md "wikilink")[方法编写单元测试](https://zh.wikipedia.org/wiki/方法 "wikilink")，一旦变更导致错误发生，借助于单元测试可以快速定位并修复错误。
...
在测试驱动开发的软件实践中，单元测试可以取代正式的设计。每一个单元测试案例均可以视为一项类、方法和待观察行为等设计元素。下面的[Java例可以帮助说明这一点](../Page/Java.md "wikilink")。
...
因为很多类会引用其它[类](https://zh.wikipedia.org/wiki/类 "wikilink")，对这个类的测试经常会要求测试其它的类。一个最普遍的例子是依赖于[数据库的类](../Page/数据库.md "wikilink")：为了测试它，测试人员通常编写代码去操作数据库。这是不对的，因为单元测试不应超出待测试的类边界。
...
测试不可能發現所有的程序错误，单元测试也不例外。按定义，单元测试只测试程序单元自身的功能。因此，它不能發現集成错误、性能问题、或者其他系统级别的问题。单元测试结合其他软件测试活动更为有效。与其它形式的[软件测试类似](../Page/软件测试.md "wikilink")，单元测试只能表明测到的问题，不能表明不存在未测试到的错误。

软件测试是一个组合问题。例如，每一个布尔型的决断语句需要至少两种测试：一个返回真，一个返回假。因此，针对每行书写的代码，程序员通常需要写3至5行的测试代码。\[3\]這很明顯地很花時間而且對此的投入可能並不值得。也有些問題是根本不能簡單地檢測出來的——例如具不確定性的或牽扯到多[線程的問題](https://zh.wikipedia.org/wiki/線程 "wikilink")。此外，替單元測試寫的程式碼可能就像要測試的程式碼一樣有程序錯誤。[佛瑞德·布魯克斯在](../Page/佛瑞德·布魯克斯.md "wikilink")[人月神話一書中舉例說明](https://zh.wikipedia.org/wiki/人月神話 "wikilink")：「絕對不要帶兩個計時器去海邊。最好總是帶一或三個」。意味著，如果兩個[計時器互相衝突的話](https://zh.wikipedia.org/wiki/精密航海記時器 "wikilink")，你該怎麼知道哪個是對的？为了获得单元测试的好处，在软件开发过程中应形成一套严格纪律意识。仔细保留记录是必要的，不仅仅只保留执行的测试，也包括保留对应的源码和其它软件单元的变更历史。即，使用[版本控制系统是必要的](https://zh.wikipedia.org/wiki/版本控制系统 "wikilink")。如果后续版本不能通过一个以前测试通过的单元测试，版本控制系统可以提供对应时间段对源代码所做的变更清单。
...
单元测试是[极限编程的基础](../Page/极限编程.md "wikilink")，依赖于自动化的单元测试框架。自动化的单元测试框架可以来源于第三方，如[xUnit](https://zh.wikipedia.org/wiki/xUnit "wikilink")，也可以由开发组自己创建。
...
通常在沒有特定框架支援下，透過撰寫在測試中的執行單元，並使用[判定](../Page/斷言_\(程式\).md "wikilink")、[異常處理](https://zh.wikipedia.org/wiki/異常處理 "wikilink")、或其他[控制流程機制來表示失敗的用戶代碼](../Page/控制流程.md "wikilink")（client
code）執行單元測試是可行的。不透過框架的單元測試有用之處在於進行單元測試時會有一個[參進障礙](https://zh.wikipedia.org/wiki/參進障礙 "wikilink")（barrier
to
entry）；進行一點單元測試幾乎不比沒做好多少，但是一旦使用了框架，加入單元測試相對來說會簡單許多。\[6\]在某些框架中許多先進單元測試特徵遺失了或者必須是手工編寫的。
...
## 外部链接
  ...
  - [The evolution of Unit Testing Syntax and
    Semantics](http://weblogs.asp.net/rosherove/archive/2008/01/17/the-evolution-of-unit-testing-and-syntax.aspx)
  - [Unit Testing Guidelines from
    GeoSoft](http://geosoft.no/development/unittesting.html)
  - [Test Driven Development (Ward Cunningham's
    Wiki)](http://c2.com/cgi/wiki?TestDrivenDevelopment)
  - [Unit Testing 101 for the
    Non-Programmer](http://www.saravanansubramanian.com/Saravanan/Articles_On_Software/Entries/2010/1/19_Unit_Testing_101_For_Non-Programmers.html)
  - [Step-by-Step Guide to JPA-Enabled Unit Testing (Java
    EE)](https://web.archive.org/web/20100211214020/http://www.sizovpoint.com/2010/01/step-by-step-guide-to-jpa-enabled-unit.html)
...
3.
4.
5.  IEEE Standards Board, ["IEEE Standard for Software Unit Testing: An
    American National Standard, ANSI/IEEE
    Std 1008-1987"](http://iteso.mx/~pgutierrez/calidad/Estandares/IEEE%201008.pdf)
     in *IEEE Standards: Software Engineering, Volume Two: Process
    Standards; 1999 Edition; published by The Institute of Electrical
    and Electronics Engineers, Inc.* Software Engineering Technical

> 本文内容由[单向链表](https://zh.wikipedia.org/wiki/单向链表)转换而来。


**单向链表**（又名单链表、线性链表）是[链表](../Page/链表.md "wikilink")的一种，其特点是链表的链接方向是单向的，对链表的访问要通过从头部开始，依序往下讀取。

## 数据结构

一个单向链表的节点被分成两个部分。第一个部分保存或者显示关于节点的信息，第二个部分存储下一个节点的地址。单向链表只可向一个方向遍历。

[<File:Link> zh.png](https://zh.wikipedia.org/wiki/File:Link_zh.png "fig:File:Link zh.png")

## 動態單鏈表（C语言）

單向鏈表的[數據结構可以分為兩部分](https://zh.wikipedia.org/wiki/數據结構 "wikilink")：數據域和[指针域](../Page/指標_\(電腦科學\).md "wikilink")，數据域存儲數據，指针域指向下一個儲存節點的地址。

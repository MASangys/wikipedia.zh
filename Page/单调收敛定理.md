...
如果*a<sub>k</sub>*是一个单调的[实数](../Page/实数.md "wikilink")[序列](../Page/序列.md "wikilink")（例如*a<sub>k</sub>* ≤
*a*<sub>*k*+1</sub>），则这个序列具有极限（如果我们把正无穷大和负无穷大也算作极限的话）。这个极限是[有限的](../Page/有限.md "wikilink")，当且仅当序列是[有界的](../Page/有界.md "wikilink")。

### 证明

我们证明如果递增序列\(\langle a_n \rangle\)有上界，则它是收敛的，且它的极限为\(\sup_n \{a_n\}\)。

由于\(\{ a_n \}\)非空且有上界，因此根据实数的[最小上界公理](../Page/最小上界公理.md "wikilink")，\(c = \sup_n \{a_n\}\)存在，且是有限的。现在，对于每一个\(\varepsilon > 0\)，都存在一个\(a_N\)，使得\(a_N > c - \varepsilon\)，否则\(c - \varepsilon\)是\(\{ a_n \}\)的一个上界，这与\(c\)为最小上界\(\sup_n \{a_n\}\)的事实矛盾。于是，由于\(\langle a_n \rangle\)是递增的，对于所有的n
\>
N，都有\(|c - a_n| = c - a_n \leq c - a_N < \varepsilon\)，因此根据定义，\(\langle a_n \rangle\)的极限为\(\sup_n \{a_n\}\)。证毕。

类似地，如果一个实数序列是递减且有下界，则它的[最大下界就是它的极限](../Page/最大下界.md "wikilink")。

## 单调[级数的收敛性](../Page/级数.md "wikilink")

### 定理

如果对于所有的自然数*j*和*k*，*a*<sub>*j*,*k*</sub>都是非负实数，且*a*<sub>*j*,*k*</sub> ≤
*a*<sub>*j*+1,*k*</sub>，则（参见\[1\]第168页）：
...
设( X, A, \(\mu\)
)为一个[测度空间](../Page/测度.md "wikilink")。设\(f_1, f_2, \ldots\)为\(\mu\)-[可测的](../Page/可测函数.md "wikilink")\([0,\infty]\)值单调递增函数。也就是说：
...
我们首先证明f是\(\mu\)-可测函数。为此，只需证明区间\[0,t\]在f下的原像是X上的[σ代数A的一个元素](../Page/σ代数.md "wikilink")。设I为\([0,\infty]\)的一个子区间。那么：
...
根据[勒贝格积分的定义](../Page/勒贝格积分.md "wikilink")，

\[\int f d \mu = sup \{\int g d \mu | g \in SF, g\leq f \}\]，
其中SF是X上的\(\mu\)-可测[简单函数的交集](../Page/简单函数.md "wikilink")。由于在每一个\(x\in X\)，都有\(f_k(x)\leq f(x)\)，我们便有：
...
我们现在证明另一个方向的不等式（也可从[法图引理推出](../Page/法图引理.md "wikilink")），也就是说，我们来证明：
...
[it:Passaggio al limite sotto segno di integrale\#Integrale di
Lebesgue](../Page/it:Passaggio_al_limite_sotto_segno_di_integrale#Integrale_di_Lebesgue.md "wikilink")

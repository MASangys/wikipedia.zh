...
设( X, A, \(\mu\)
)为一个[测度](../Page/测度.md "wikilink")空间。设\(f_1, f_2, \ldots\)为\(\mu\)-[可测的](../Page/可测函数.md "wikilink")\([0,\infty]\)值单调递增函数。也就是说：
...
其中SF是X上的\(\mu\)-可测[简单函数](../Page/简单函数.md "wikilink")的交集。由于在每一个\(x\in X\)，都有\(f_k(x)\leq f(x)\)，我们便有：

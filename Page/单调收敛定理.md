...
如果*a<sub>k</sub>*是一个单调的[实数](../Page/实数.md "wikilink")[序列](../Page/序列.md "wikilink")（例如*a<sub>k</sub>* ≤ *a*<sub>*k*+1</sub>），则这个序列具有极限（如果我们把正无穷大和负无穷大也算作极限的话）。这个极限是[有限的](https://zh.wikipedia.org/wiki/有限 "wikilink")，当且仅当序列是[有界的](https://zh.wikipedia.org/wiki/有界 "wikilink")。
...
设( X, A, \(\mu\) )为一个[测度](../Page/测度.md "wikilink")空间。设\(f_1, f_2, \ldots\)为\(\mu\)-[可测的](../Page/可测函数.md "wikilink")\([0,\infty]\)值单调递增函数。也就是说：

\[\forall x\in X, k\in \mathbb{N}, f_k(x) \leq f_{k+1}(x)\]。 接着，设序列\(f_{n}\)的逐点极限为f。也就是说：

\[\forall x\in X, f(x):= \lim_{k\to\infty} f_k(x)\]。 那么，f是\(\mu\)-可测的，且（参见\[2\]第21.38节）：

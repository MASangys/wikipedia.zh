...
**卡尔曼滤波**（Kalman
filter）是一种高效率的[递归滤波器](../Page/递归滤波器.md "wikilink")（[自回归滤波器](../Page/自迴歸模型.md "wikilink")），它能够从一系列的不完全及包含[雜訊的](../Page/雜訊_\(通訊學\).md "wikilink")[测量中](../Page/测量.md "wikilink")，估计[动态系统的状态](../Page/动态系统.md "wikilink")。卡尔曼滤波會根據各測量量在不同時間下的值，考慮各時間下的[联合分布](../Page/联合分布.md "wikilink")，再產生對未知變數的估計，因此會比只以單一測量量為基礎的估計方式要準。卡尔曼濾波得名自主要貢獻者之一的[鲁道夫·卡尔曼](../Page/鲁道夫·卡尔曼.md "wikilink")。

卡尔曼滤波在技術領域有許多的應用。常見的有飛機及太空船的\[1\]。卡尔曼滤波也廣為使用在[時間序列的分析中](../Page/時間序列.md "wikilink")，例如[信号处理及](../Page/信号处理.md "wikilink")[计量经济学中](../Page/计量经济学.md "wikilink")。卡尔曼滤波也是[機器人運動規劃及控制的重要主題之一](../Page/機器人.md "wikilink")，有時也包括在。卡尔曼滤波也用在中軸神經系統運動控制的建模中。因為從給與運動命令到收到感覺神經的回授之間有時間差，使用卡尔曼滤波有助於建立符合實際的系統，估計運動系統的目前狀態，並且更新命令\[2\]。

卡尔曼滤波的演算法是二步驟的程序。在估計步驟中，卡尔曼滤波會產生有關目前狀態的估計，其中也包括不確定性。只要觀察到下一個量測（其中一定含有某種程度的誤差，包括隨機雜訊）。會透過[加權平均來更新估計值](../Page/加權平均.md "wikilink")，而確定性越高的量測加權比重也越高。演算法是迭代的，可以在中執行，只需要目前的輸入量測、以往的計算值以及其不確定性矩陣，不需要其他以往的資訊。
...
卡尔曼滤波的一个典型实例是从一组有限的，包含噪声的，通过对物体位置的观察序列（可能有偏差）预测出物体的位置的[坐标及](../Page/坐标.md "wikilink")[速度](../Page/速度.md "wikilink")。在很多工程应用（如[雷达](../Page/雷达.md "wikilink")、[電腦视觉](../Page/電腦视觉.md "wikilink")）中都可以找到它的身影。同时，卡尔曼滤波也是[控制理论以及控制](../Page/控制理论.md "wikilink")[系统工程中的一个重要课题](../Page/系统工程.md "wikilink")。

例如，对于雷达来说，人们感兴趣的是其能够跟踪目标。但目标的位置、速度、加速度的测量值往往在任何时候都有噪声。卡尔曼滤波利用目标的动态信息，设法去掉噪声的影响，得到一个关于目标位置的好的[估计](../Page/估计.md "wikilink")。这个估计可以是对当前目标位置的估计（滤波），也可以是对于将来位置的估计（预测），也可以是对过去位置的估计（插值或平滑）。
...
[斯坦利·施密特](../Page/斯坦利·施密特.md "wikilink")（Stanley
...
以下的讨论需要[线性代数以及](../Page/线性代数.md "wikilink")[概率论的一般知识](../Page/概率论.md "wikilink")。
...
Markov
model）上。其基本动态系统可以用一个[马尔可夫链表示](../Page/马尔可夫链.md "wikilink")，该马尔可夫链建立在一个被高斯[噪声](../Page/噪声.md "wikilink")（即正态分布的噪声）干扰的[线性算子上的](../Page/线性算子.md "wikilink")。系统的[状态可以用一个元素为实数的](../Page/状态空间.md "wikilink")[向量表示](../Page/向量.md "wikilink")。随着[离散时间的每一个增加](../Page/离散时间信号.md "wikilink")，这个[线性算子就会作用在当前](../Page/线性算子.md "wikilink")[状态上](../Page/状态.md "wikilink")，产生一个新的状态，并也会带入一些噪声，同时系统的一些已知的控制器的控制信息也会被加入。同时，另一个受噪声干扰的线性算子产生出这些隐含状态的可见输出。
...
[Kalman_filter_model.png](https://zh.wikipedia.org/wiki/File:Kalman_filter_model.png "fig:Kalman_filter_model.png")，方块代表[矩阵](../Page/矩阵.md "wikilink")，星号代表[高斯噪声](../Page/高斯噪声.md "wikilink")，其[协方差矩阵在右下方标出](../Page/协方差矩阵.md "wikilink")。\]\]
...
**v**<sub>*k*</sub>}都认为是互相[独立的](../Page/统计学独立.md "wikilink")。

实际上，很多真实世界的动态系统都并不确切的符合这个模型；但是由于卡尔曼滤波器被设计在有噪声的情况下工作，一个近似的符合已经可以使这个滤波器非常有用了。更多其它更复杂的卡尔曼滤波器的变种，在下邊討論中有描述。

## 卡尔曼滤波器

卡尔曼滤波是一种[递归的估计](../Page/递归.md "wikilink")，即只要获知上一时刻状态的估计值以及当前状态的观测值就可以计算出当前状态的估计值，因此不需要记录观测或者估计的历史信息。卡尔曼滤波器与大多数滤波器不同之處，在於它是一种纯粹的[时域滤波器](../Page/时域.md "wikilink")，它不需要像[低通滤波器等](../Page/低通滤波器.md "wikilink")[频域滤波器那样](../Page/频域.md "wikilink")，需要在频域设计再转换到时域实现。
...
使用上述公式计算\(\textbf{P}_{k|k}\)仅在最优卡尔曼增益的时候有效。使用其他增益的话，公式要复杂一些，请参见[推导](../Page/卡尔曼滤波#推导.md "wikilink")。
...
卡尔曼滤波器是一个[最小均方误差估计器](../Page/最小均方误差.md "wikilink")，后验状态误差估计（英文：*a
posteriori* state estimate）是

\[\textbf{x}_{k} - \hat{\textbf{x}}_{k|k}\]

最小化这个矢量幅度平方的期望值，\(\textrm{E}[|\textbf{x}_{k} - \hat{\textbf{x}}_{k|k}|^2]\)，这等同于最小化后验估计协方差矩阵**P**<sub>*k*|*k*</sub>的[迹](../Page/迹数.md "wikilink")（trace）。将上面方程中的项展开、抵消，得到：
...
当[矩阵导数是](../Page/矩阵导数.md "wikilink")0的时候得到**P**<sub>*k*|*k*</sub>的[迹](../Page/迹数.md "wikilink")（trace）的最小值：
...
这个增益称为*最優卡尔曼增益*，在使用时得到[最小均方误差](../Page/最小均方误差.md "wikilink")。
...
这个公式的计算比较简单，所以实际中总是使用这个公式，但是需注意這公式仅在使用最优卡尔曼增益的时候它才成立。如果算术精度总是很低而导致[數值穩定性出现问题](../Page/數值穩定性.md "wikilink")，或者特意使用非最优卡尔曼增益，那么就不能使用这个简化；必须使用上面导出的后验误差协方差公式。

## 与递归贝叶斯估计之间的关系

假设真正的状态是无法观察的[马尔可夫过程](../Page/马尔可夫过程.md "wikilink")，测量结果是從隐性马尔可夫模型观察到的状态。
...
## 应用
  ...
  - [自动驾驶仪](../Page/自动驾驶仪.md "wikilink")

  -
  - [经济学](../Page/经济学.md "wikilink")，特别是[宏观经济学](../Page/宏观经济学.md "wikilink")，[时间序列模型](../Page/时间序列.md "wikilink")，以及[计量经济学](../Page/计量经济学.md "wikilink")
...
## 参见
  ...
  -
  - 比较：[维纳滤波及](../Page/维纳滤波.md "wikilink")[粒子濾波器](../Page/粒子濾波器.md "wikilink")

  - [施密特–卡爾曼濾波器](../Page/施密特–卡爾曼濾波器.md "wikilink")

...
生成“黑线勾边”效果和轮廓线条可以用多种方法。常用方法之一是先渲染出比物体略大的黑色的边线，之后关闭**背面探测**（[Backface culling](https://zh.wikipedia.org/wiki/:en:Hidden_surface_determination "wikilink")，用于剔除物件背面不需渲染部分的功能），将朝后的面被用黑色绘制出来。为了扩大轮廓，这些背面会多次以[线框模型](../Page/线框模型.md "wikilink")绘制，且每次都有微小的改变。另一种方法是，背面先以固实方式渲染，并且其顶点均由[顶点着色器沿着其顶点法线进行转译](https://zh.wikipedia.org/wiki/顶点着色器 "wikilink")。在绘制边线后，重启背面探测来绘制物体的明暗和附属材质。最后，由于场景中的背面比正面所处位置更深远，图像会由[深度缓冲](../Page/深度缓冲.md "wikilink")组合。最终我们看到的就是，物件拥有黑色的勾边以及内部的轮廓线条。普遍的观点认为，这种用在动画和游戏上使用的“勾边”效果就是卡通渲染，但是实际上，卡通渲染指的是这种渲染方式，而无关是否拥有勾边效果。

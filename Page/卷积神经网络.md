...
## 可用包
  ...
  - Caffe: Caffe包含了CNN使用最广泛的库。它由伯克利视觉和学习中心（BVLC）研发，拥有比一般实现更好的结构和更快的速度。同时支持[CPU和](https://zh.wikipedia.org/wiki/CPU "wikilink")[GPU计算](https://zh.wikipedia.org/wiki/GPU "wikilink")，底层由[C++](../Page/C++.md "wikilink")实现，并封装了Python和[MATLAB](../Page/MATLAB.md "wikilink")的接口。
  - Torch7（www.torch.ch）
  - OverFeat
  - Cuda-convnet
  - MatConvnet

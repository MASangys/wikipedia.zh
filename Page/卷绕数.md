...
[平面上的闭](../Page/平面.md "wikilink")[曲线关于某个点的](../Page/曲线.md "wikilink")**卷绕数**，是一个整数，它表示了曲线绕过该点的总次数。卷绕数与曲线的[定向有关](../Page/定向.md "wikilink")，如果曲线依顺时针方向绕过某个点，则卷绕数是负数。

卷绕数在[代数拓扑中是基本的概念](../Page/代数拓扑.md "wikilink")，在[向量分析](../Page/向量分析.md "wikilink")、[复分析](../Page/复分析.md "wikilink")、[几何拓扑](../Page/几何拓扑.md "wikilink")、[微分几何和](../Page/微分几何.md "wikilink")[物理学中也扮演了重要的角色](../Page/物理学.md "wikilink")。
...
*x-y*平面上的曲线可以用[参数方程来定义](../Page/参数方程.md "wikilink")：

\[x = x(t)\quad , \quad y=y(t)\qquad (0 \leq t \leq 1).\]

如果我们把参数*t*视为时间，那么这个方程就描述了物体在和期间在平面上的运动。只要[函数](../Page/函数.md "wikilink")*x*(*t*)和*y*(*t*)是[连续的](../Page/连续.md "wikilink")，运动的轨迹就是一条曲线。只要物体的位置于和时相同，这条曲线就是闭曲线。

我们可以用[极坐标系来定义这种曲线的卷绕数](../Page/极坐标系.md "wikilink")。假设曲线不经过原点，我们可以把参数方程写成极坐标的形式：
...
在[微分几何中](../Page/微分几何.md "wikilink")，通常假设参数方程是可微的（或至少分段可微的）。在这种情况下，极坐标系*θ*与直角坐标系*x*和*y*有以下的关系：
...
根据[微积分基本定理](../Page/微积分基本定理.md "wikilink")，*θ*的总变化等于*dθ*的[积分](../Page/积分.md "wikilink")。因此，我们可以把可微曲线的卷绕数表示为一个[曲线积分](../Page/曲线积分.md "wikilink")：
  ...
  -
    卷绕数<math> = \\frac{1}{2\\pi} \\oint_C \\,\\frac{x}{r^2}\\,dy -
    \\frac{y}{r^2}
...
在[复分析中](../Page/复分析.md "wikilink")，闭曲线*C*的卷绕数可以表示为复数坐标。特别地，如果我们记*z* = *re*<sup>*iθ*</sup>，那么：
...
这是[柯西积分公式的一个特例](../Page/柯西积分公式.md "wikilink")。卷绕数在复分析中扮演了一个十分重要的角色（例如在[留数定理的表述中](../Page/留数定理.md "wikilink")）。

## 回转数

我们也可以考虑曲线关于它本身的卷绕数（又称为回转数
），也就是曲线的切向量旋转的次数。在右面的图中，曲线的回转数是4（或−4），那个小的回路也计算在内。这只对可微且光滑的曲线才有定义。参见：[回转切线定理](../Page/回转切线定理.md "wikilink")。
...
## 参考文献
  ...
  - Krantz, S. G. "The Index or Winding Number of a Curve about a
    Point." §4.4.4 in Handbook of Complex Variables. Boston, MA:
    Birkhäuser, pp. 49-50, 1999.

**双电子积分**就是涉及两个电子坐标的积分，是[量子化学计算中出现频率最高的一类积分](../Page/量子化学.md "wikilink")，也是进行[Hartree-Fock方程自洽场计算和其他高级量子化学计算过程中计算量最大的一个部分](https://zh.wikipedia.org/wiki/Hartree-Fock方程 "wikilink")。构成一个双电子积分的，是二至四个不同的轨道波函数、一个涉及两个电子坐标的算子（即双电子算子）和两套电子坐标。在量子化学计算中，出现频率最高的双电子算子是\(\frac{1}{r_{12}}\)，即在[原子单位下表征两电子间](https://zh.wikipedia.org/wiki/原子单位 "wikilink")[庫侖排斥力的算子](https://zh.wikipedia.org/wiki/庫侖定律 "wikilink")。
...
也可以用[狄拉克符号来简写上述双电子积分](../Page/狄拉克符号.md "wikilink")：

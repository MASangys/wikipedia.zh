**双端队列**（deque，全名double-ended queue）是一种具有[-{zh-hans:队列;
zh-hant:佇列;}-和](../Page/队列.md "wikilink")[-{zh-hans:栈;
zh-hant:堆疊;}-性质的](https://zh.wikipedia.org/wiki/栈 "wikilink")[抽象数据类型](../Page/抽象資料型別.md "wikilink")。双端队列中的元素可以从两端弹出，插入和删除操作限定在-{zh-hans:队列;
zh-hant:佇列;}-的两邊进行。

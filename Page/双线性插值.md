[Bilininterp.png](https://zh.wikipedia.org/wiki/File:Bilininterp.png "fig:Bilininterp.png")
**雙線性插值**，又稱為**雙線性內插**。在[数学上](../Page/数学.md "wikilink")，**双线性插值**是对[线性插值在二维](../Page/线性插值.md "wikilink")[直角网格上的扩展](../Page/直角坐标系.md "wikilink")，用于对双变量函数（例如
*x* 和 *y*）进行[插值](../Page/插值.md "wikilink")。其核心思想是在两个方向分别进行一次线性插值。

## 算法

[Bilinear_interpolation.png](https://zh.wikipedia.org/wiki/File:Bilinear_interpolation.png "fig:Bilinear_interpolation.png")

假如我们想得到未知函数 *f* 在点 \(P=\left( x, y\right)\) 的值，假设我们已知函数 *f* 在
\(Q_{11} = \left( x_1, y_1 \right)\),
\(Q_{12} = \left( x_1, y_2 \right)\),
\(Q_{21} = \left( x_2, y_1 \right)\), 及
\(Q_{22} = \left( x_2, y_2 \right)\) 四个点的值。
...
或者用[矩阵运算表示为](../Page/矩阵.md "wikilink")
...
顾名思义，双线性插值的结果不是线性的，它是两个[线性函数的积](../Page/线性函数.md "wikilink")。在单位正方形上，双线性插值可以记作
...
在[计算机视觉及](../Page/计算机视觉.md "wikilink")[图像处理领域](../Page/图像处理.md "wikilink")，双线性插值是一种基本的[重采样技术](../Page/图像缩放.md "wikilink")。

[材质贴图中](../Page/材质贴图.md "wikilink")，双线性插值也叫[双线性过滤或者](../Page/双线性过滤.md "wikilink")**双线性材质贴图**。图像的双线性插值放大算法中，目标图像中新创造的象素值，是由源图像位置在它附近的2\*2区域4个邻近象素的值通过加权平均计算得出的。双线性内插值算法放大后的图像质量较高，不会出现像素值不连续的的情况。然而此算法具有低通滤波器的性质，使高频分量受损，所以可能会使图像轮廓在一定程度上变得模糊。

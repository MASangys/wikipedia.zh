**双边带抑制载波传输**（**DSB-SC**）是通过[振幅調變](../Page/振幅調變.md "wikilink")（AM）使得频率关于对称分布，且将载波电平降低到最低程度（理想情况下完全抑制）的[传输方式](../Page/传输.md "wikilink")。
...
DSB-SC信号频谱图： [Image:Spectrum
DSBSC.svg](https://zh.wikipedia.org/wiki/File:Spectrum_DSBSC.svg "fig:Image:Spectrum DSBSC.svg")

## 产生

DSB-SC由混频器产生的。这是由载波信号乘以消息信号构成的。该过程的数学表示如下所示，其中用到了[积化和差公式](../Page/积化和差.md "wikilink")。
...
[Image:DSBSC
Modulation.svg](https://zh.wikipedia.org/wiki/File:DSBSC_Modulation.svg "fig:Image:DSBSC Modulation.svg")
...
\(\omega_c \gg \omega_m\)，第二项的频率要比原始消息信号高得多。一旦此信号通过低通滤波器，高频率分量被去除，只留下原始消息。
...
\(\cos\left(\Delta\omega\cdot t+\theta\right)\)
项会导致消息信号的失真与衰减。特别地，若频率正确，但相位错误，\(\theta\)
的贡献是恒定衰减因子，而 \(\Delta\omega\cdot t\) 表示复原信号的循环反转，是严重的失真。

[<File:Demodulation> distortion
diagram.png](https://zh.wikipedia.org/wiki/File:Demodulation_distortion_diagram.png "fig:File:Demodulation distortion diagram.png")
...
[Image:DSBSC Message
Signal.png](https://zh.wikipedia.org/wiki/File:DSBSC_Message_Signal.png "fig:Image:DSBSC Message Signal.png")

这个消息信号的方程是
\(s(t) = \frac{1}{2}\cos\left(2\pi 800 t\right) - \frac{1}{2}\cos\left( 2\pi 1200 t\right)\).

此例中载波为 5 kHz（\(c(t) = \cos\left( 2\pi 5000 t \right)\)）正弦波—作图如下。

[Image:DSBSC Carrier
Signal.png](https://zh.wikipedia.org/wiki/File:DSBSC_Carrier_Signal.png "fig:Image:DSBSC Carrier Signal.png")

调制是通过时域乘法实现的，产生一个 5 kHz 载波信号，其振幅变化与消息信号相同。

[Image:DSBSC Modulated
Output.png](https://zh.wikipedia.org/wiki/File:DSBSC_Modulated_Output.png "fig:Image:DSBSC Modulated Output.png")
...
[Image:DSBSC
Spectrum.png](https://zh.wikipedia.org/wiki/File:DSBSC_Spectrum.png "fig:Image:DSBSC Spectrum.png")

## 参考文献

## 外部链接

  - A DSBSC generation and demodulation instrument is described as side
    application of a commercial lock-in amplifier in [Double-sideband
    Suppressed-carrier
    Modulation](http://www.zhinst.com/blogs/sadik/2013/08/dsb-sc/).

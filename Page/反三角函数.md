...
在[数学中](../Page/数学.md "wikilink")，**反三角函数**是[三角函数的](../Page/三角函数.md "wikilink")[反函数](../Page/反函数.md "wikilink")。

## 數學符號

符号\(\sin^{-1}, \cos^{-1}\)等常用于\(\arcsin, \arccos\)等。但是这种符号有时在\(\sin^{-1} x\)和\(\frac{1}{\sin x}\)之间易造成混淆。

在编程中，函数\(\arcsin\), \(\arccos\), \(\arctan\)通常叫做\(\mathrm{asin}\),
\(\mathrm{acos}\),
\(\mathrm{atan}\)。很多编程语言提供两自变量[atan2函数](../Page/atan2.md "wikilink")，它计算给定\(y\)和\(x\)的\(\frac{y}{x}\)的反正切，但是值域为\([-\pi, \pi]\)。

<File:Asin> acos
plot.svg|在笛卡尔平面上\(f(x)=\arcsin x\)(紅)和\(f(x)=\arccos x\)(綠)函数的常用主值的图像。
<File:Atan> acot
plot.svg|在笛卡尔平面上\(f(x)=\arctan x\)(紅)和\(f(x)=\arccot x\)(綠)函数的常用主值的图像。
<File:Asec> acsc
plot.svg|在笛卡尔平面上\(f(x)=\arcsec x\)(紅)和\(f(x)=\arccsc x\)(綠)函数的常用主值的图像。
...
| [反余弦](../Page/反余弦.md "wikilink") | \(y=\arccos x\) | \(x=\cos y\) | \([-1,1]\)                      | \([0,\pi]\)                                  |
| [反正切](../Page/反正切.md "wikilink") | \(y=\arctan x\) | \(x=\tan y\) | \(\mathbb{R}\)                  | \((-\frac{\pi}{2},\frac{\pi}{2})\)           |
| [反余切](../Page/反余切.md "wikilink") | \(y=\arccot x\) | \(x=\cot y\) | \(\mathbb{R}\)                  | \((0,\pi)\)                                  |
| [反正割](../Page/反正割.md "wikilink") | \(y=\arcsec x\) | \(x=\sec y\) | \((-\infty,-1]\cup[1,+\infty)\) | \([0,\frac{\pi}{2})\cup(\frac{\pi}{2},\pi]\) |
| [反余割](../Page/反余割.md "wikilink") | \(y=\arccsc x\) | \(x=\csc y\) | \((-\infty,-1]\cup[1,+\infty)\) | \([-\frac{\pi}{2},0)\cup(0,\frac{\pi}{2}]\)  |
|                                  |                 |              |                                 |                                              |
...
如果\(x\)允许是[复数](../Page/复数.md "wikilink")，则\(y\)的值域只适用它的实部。
...
\[\arctan \frac{1}{x} = \frac{\pi}{2} - \arctan x =\arccot x, \\]
\(\ x > 0\)

\[\arctan \frac{1}{x} = -\frac{\pi}{2} - \arctan x = -\pi + \arccot x, \\]
\(\ x < 0\)

\[\arccot \frac{1}{x} = \frac{\pi}{2} - \arccot x =\arctan x, \\]
\(\ x > 0\)
...
| ------------- | --------------------------------------------- | --------------------------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------- |
| \(\arcsin x\) | \(\sin (\arcsin x) = x\)                      | \(\cos (\arcsin x) = \sqrt{1-x^2}\)           | \(\tan (\arcsin x) = \frac{x}{\sqrt{1-x^2}}\) | [150px](../Page/file:Trigonometric_functions_and_inverse3.svg.md "wikilink") |
| \(\arccos x\) | \(\sin (\arccos x) = \sqrt{1-x^2}\)           | \(\cos (\arccos x) = x\)                      | \(\tan (\arccos x) = \frac{\sqrt{1-x^2}}{x}\) | [150px](../Page/file:Trigonometric_functions_and_inverse.svg.md "wikilink")  |
| \(\arctan x\) | \(\sin (\arctan x) = \frac{x}{\sqrt{1+x^2}}\) | \(\cos (\arctan x) = \frac{1}{\sqrt{1+x^2}}\) | \(\tan (\arctan x) = x\)                      | [150px](../Page/file:Trigonometric_functions_and_inverse2.svg.md "wikilink") |
| \(\arccot x\) | \(\sin (\arccot x) = \frac{1}{\sqrt{1+x^2}}\) | \(\cos (\arccot x) = \frac{x}{\sqrt{1+x^2}}\) | \(\tan (\arccot x) = \frac{1}{x}\)            | [150px](../Page/file:Trigonometric_functions_and_inverse4.svg.md "wikilink") |
| \(\arcsec x\) | \(\sin (\arcsec x) = \frac{\sqrt{x^2-1}}{x}\) | \(\cos (\arcsec x) = \frac{1}{x}\)            | \(\tan (\arcsec x) = \sqrt{x^2-1}\)           | [150px](../Page/file:Trigonometric_functions_and_inverse6.svg.md "wikilink") |
| \(\arccsc x\) | \(\sin (\arccsc x) = \frac{1}{x}\)            | \(\cos (\arccsc x) = \frac{\sqrt{x^2-1}}{x}\) | \(\tan (\arccsc x) = \frac{1}{\sqrt{x^2-1}}\) | [150px](../Page/file:Trigonometric_functions_and_inverse5.svg.md "wikilink") |
|               |                                               |                                               |                                               |                                                                              |
...
对于实数\(x\)的反三角函數的[导函数如下](../Page/导函数.md "wikilink")：
...
因為要使根號內部恆為正，所以在條件加上\(|x|<1\)，其他導數公式同理可證<ref>
...
因為要使根號內部恆為正，所以在條件加上\(|x|>1\)，比較容易被忽略是\(\sec \theta\)產生的絕對值
\(\sec ^{-1}\theta\)的定義域是\(0\le \theta \le \pi\)，其所產生的反函數皆為正，所以需要加上絕對值。
设\(\theta =\arccsc x\)，得到：

\[\frac{d\operatorname{arc}\csc x}{dx}=\frac{d\theta }{d\csc \theta }=\frac{-1}{\csc \theta \cot \theta }=\frac{-1}{\left| x \right|\sqrt{x^{2}-1}}\]
因為要使根號內部恆為正，所以在條件加上\(|x|>1\)，比較容易被忽略是\(\csc \theta\)產生的絕對值
\(\csc ^{-1}\theta\)的定義域是\(-\frac{\pi}{2}\le \theta \le \frac{\pi}{2},\theta\ne0\)，其所產生的反函數皆為負，所以需要加上絕對值。</ref>。
...
\end{align}\]
当\(x\)等于1时，在有极限的域上的积分是[瑕积分](../Page/瑕积分.md "wikilink")，但仍是良好定义的。

## 无穷级数

如同正弦和余弦函数，反三角函数可以使用[无穷级数计算如下](../Page/无穷级数.md "wikilink")：
...
[欧拉发现了反正切的更有效的级数](../Page/欧拉.md "wikilink")：

\[\arctan x = \infty{x}{1+x^2} \sum_{n=0}^\infty \prod_{k=1}^n \frac{2k x^2}{(2k+1)(1+x^2)}\]。

（注意对*x*= 0在和中的项是[空积](../Page/乘法单位元.md "wikilink")1。）
...
使用[分部积分法和上面的简单导数很容易得出它们](../Page/分部积分法.md "wikilink")。

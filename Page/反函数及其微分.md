[右](https://zh.wikipedia.org/wiki/File:Umkehrregel_2.png "fig:右")
在数学中，对[函数](../Page/函数.md "wikilink")
\(y = f(x)\)进行逆运算的功能，在某种程度上，"抵消" \(f\)的作用
(准确定义请参阅[反函数](../Page/反函數.md "wikilink"))。\(f\)的反函数记作\(f^{-1}\)。
和 所表示含义相同。
...
其中 \(\mathcal{D}\) 表示一元[微分算子](../Page/微分算子.md "wikilink") (在函数的空间上)，
\(\circ\) 表示二元复合算子。

几何上，函数和反函数有关于直线 .镜像的图像，
这种映射将任何线的[斜率变成其](../Page/斜率.md "wikilink")[倒数](../Page/倒数.md "wikilink")。
...
</math>\(\frac{dy}{dx}\,\cdot\,\frac{dx}{dy}  =  2x \cdot\frac{1}{2x}  =  1.\)
...
<!-- end list -->
  ...
  -
    因此，具有连续导数的函数在其导数非零的每一点的[邻域内都有反函数](../Page/邻域.md "wikilink")。如果导数不连续的，则不成立。

## 高阶导数

上面给出的[链式法则是通过对等式](../Page/链式法则.md "wikilink")\(x= f^{-1}(f(x))\)关于\(x\)微分得到的。对于更高阶的导数，我们可以继续同样的过程。对恒等式对\(x\)求导两次，得到
...
对三阶导数类似:
  ...
  -
    <math>\\frac{d^3y}{dx^3} = -
    \\frac{d^3x}{dy^3}\\,\\cdot\\,\\left(\\frac{dy}{dx}\\right)^4 -
...
\\frac{d^2x}{dy^2}\\,\\cdot\\,y^3 + y = 0 \\mbox{ }\\mbox{ }\\mbox{
}\\mbox{ }; \\mbox{ }\\mbox{ }\\mbox{ }\\mbox{ } \\frac{d^2x}{dy^2} =
-\\frac{1}{y^2} </math>,

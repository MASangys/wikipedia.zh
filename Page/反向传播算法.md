...
最初在训练之前，会随机分配权重。之后神经元根据进行学习，在此情况下包含元组 (\(x_{1}\), \(x_{2}\), \(t\)) 的集合，其中 \(x_{1}\) 与 \(x_{2}\) 是网络的输入，\(t\) 为正确输出（在给定相同的输入时网络最终应当产生的输出）。网络在给定 \(x_{1}\) 和 \(x_{2}\) 时，会计算一个输出 \(y\)，很可能与 \(t\) 不同（因为权重最初是随机的）。衡量期望输出 \(t\) 与实际输出 \(y\) 之间的差异的一个常见方法是采用平方误差测度：

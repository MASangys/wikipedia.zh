...
举例来说，\(\lfloor 3.633 \rfloor = 3\)，\(\lfloor 56 \rfloor = 56\)，\(\lfloor -2 \rfloor = -2\)，\(\lfloor -2.263 \rfloor = -3\)。对于非负的实数，其下取整函数的值一般叫做它的**整数部分**或**取整部分**。而\(x -\lfloor x\rfloor\)叫做*x*的[小数部分](../Page/小数.md "wikilink")。每个[分数都可以表示成其整数部分与一个](https://zh.wikipedia.org/wiki/分数 "wikilink")[真分数的和](https://zh.wikipedia.org/wiki/真分数 "wikilink")，而实数的整数部分和小数部分是与此概念相应的拓延。
...
计算机中的上取整函数和下取整函数的命名来自于[英文的](https://zh.wikipedia.org/wiki/英文 "wikilink")*ceiling*（天花板）和*floor*（地板），相关的记法由[肯尼斯·艾佛森于](../Page/肯尼斯·艾佛森.md "wikilink")1962年引入。\[2\]
...
\[\lfloor {k+x} \rfloor = k + \lfloor x\rfloor.\]
  ...
  - 一般的[數值修約規則可以表述为将](https://zh.wikipedia.org/wiki/數值修約規則 "wikilink")*x*映射到floor(*x*
    + 0.5)；
...
\[\sum_{i=1}^{n-1} \lfloor im / n \rfloor = (m - 1) (n - 1) / 2\]
  ...
  - 最后，对于每个正整数*k*，其在 [p 进制下的表示有](../Page/进位制.md "wikilink")
    \(\lfloor \log_{p}(k) \rfloor + 1\)
    个[数位](https://zh.wikipedia.org/wiki/数位 "wikilink")。
...
1.  [Ronald
    ...
    "*Concrete Mathematics*". Addison-Wesley, 1999. Chapter 3, "Integer
    Functions".

在[程序设计中](../Page/程序设计.md "wikilink")，**句柄**（**handle**）是[Windows操作系统用来标识被应用程序所建立或使用的对象的整数](../Page/Windows操作系统.md "wikilink")。其本质相当于带有引用计数的[智能指针](../Page/智能指针.md "wikilink")。当一个[应用程序要引用其他系统](../Page/应用程序.md "wikilink")（如[数据库](../Page/数据库.md "wikilink")、[操作系统](../Page/操作系统.md "wikilink")）所管理的[内存块或](../Page/内存.md "wikilink")[对象时](../Page/对象_\(计算机科学\).md "wikilink")，可以使用句柄。

## 句柄与指针

句柄与普通[指针的区别在于](../Page/指针_\(信息学\).md "wikilink")，指针包含的是引用[对象的](../Page/对象_\(计算机科学\).md "wikilink")[内存地址](../Page/物理地址.md "wikilink")，而句柄则是由系统所管理的引用标识，该标识可以被系统重新定位到一个[内存地址上](../Page/物理地址.md "wikilink")。这种间接访问[对象的模式增强了系统对引用](../Page/对象_\(计算机科学\).md "wikilink")[对象的控制](../Page/对象_\(计算机科学\).md "wikilink")。（参见[封装](../Page/封装.md "wikilink")）。通俗的说就是我们调用句柄就是调用句柄所提供的服务，即句柄已经把它能做的操作都设定好了，我们只能在句柄所提供的操作范围内进行操作，但是普通指针的操作却多种多样，不受限制。
...
在上世纪80年代的操作系统（如[Mac
OS](../Page/Mac_OS.md "wikilink")\[1\]和[Windows](../Page/Windows.md "wikilink")）的[内存管理中](../Page/内存管理.md "wikilink")，句柄被广泛应用。[Unix系统的](../Page/Unix.md "wikilink")[文件描述符基本上也属于句柄](../Page/文件描述符.md "wikilink")。和其它[桌面环境一样](../Page/桌面环境.md "wikilink")，[Windows
API大量使用句柄来标识系统中的](../Page/Windows_API.md "wikilink")[对象](../Page/对象_\(计算机科学\).md "wikilink")，并建立操作系统与[用户空间之间的通信渠道](../Page/用户空间.md "wikilink")。例如，桌面上的一个窗体由一个`HWND`类型的句柄来标识。

如今，[内存容量的增大和](../Page/内存.md "wikilink")[虚拟内存算法使得更简单的](../Page/虚拟内存.md "wikilink")[指针愈加受到青睐](../Page/指针_\(信息学\).md "wikilink")，而指向另一指针的那类句柄受到冷淡。尽管如此，许多[操作系统仍然把指向私有](../Page/操作系统.md "wikilink")[对象的指针以及](../Page/对象_\(计算机科学\).md "wikilink")[进程传递给](../Page/进程.md "wikilink")[客户端的内部](../Page/客户端.md "wikilink")[数组下标称为句柄](../Page/数组.md "wikilink")。
...
在该书的中译本：
D.格里斯著,[仲萃豪等译](../Page/仲萃豪.md "wikilink")：《数字计算机的编译程序构造》，[科学出版社](../Page/科学出版社.md "wikilink"),
1976年版，给出了如下翻译：
...
[内核对象是内存中的数据结构](../Page/内核对象.md "wikilink")，由操作系统内核分配，并且只能由操作系统内核访问。内核对象的数据结构使用引用计数，计数变为0后操作系统内核就会销毁该内核对象。安全描述符（SECURITY_ATTRIBUTES结构）描述内核对象的安全性，指出内核对象的拥有者、哪些组或用户可以访问此对象。作为对照，用户对象或GDI对象（如菜单、窗口、鼠标光标等）在创建时不需要指出安全描述符。\[2\]
...
## 参见
  ...
  - [指针](../Page/指针_\(信息学\).md "wikilink")
  - [物理地址](../Page/物理地址.md "wikilink")

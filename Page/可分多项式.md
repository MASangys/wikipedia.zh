...
这里可以直接证明P(X)是不可约的，并且不可分。事实上这是为什么不可分性需要被强调的一个例子；用几何的语言来说，P代表了有限域上的一个射影直线，将坐标取p次幂。这样的[映射对有限域上的](https://zh.wikipedia.org/wiki/映射 "wikilink")[代数几何](../Page/代数几何.md "wikilink")是基础的。换言之，存在一些不能用伽罗华理论来描述的覆盖。（见[根态射](https://zh.wikipedia.org/wiki/根态射 "wikilink")（radical

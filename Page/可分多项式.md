[数学中](../Page/数学.md "wikilink")，**可分多项式**在不同的作者的书下有两个略微不同的定义。

最常见的一个定义是：当在一个给定[域K上的](../Page/域_\(數學\).md "wikilink")[多项式P](https://zh.wikipedia.org/wiki/多项式 "wikilink")(X)在K的[代数闭包中有不同的根时](https://zh.wikipedia.org/wiki/代数闭包 "wikilink")，称多项式为可分的。换言之它的互异根的数量需要等于多项式的次数\[1\]。在多项式[因式分解的观点下](../Page/因式分解.md "wikilink")，这样的多项式是[无平方多项式](https://zh.wikipedia.org/wiki/无平方多项式 "wikilink")。
...
两个定义对于K上[不可约多项式是等价的](https://zh.wikipedia.org/wiki/不可约多项式 "wikilink")，这个被用来定义域K的[可分扩张](../Page/可分扩张.md "wikilink")。
...
当中K是在有限域F<sub>p</sub>上的不定元T的[有理函数组成的域](https://zh.wikipedia.org/wiki/有理函数 "wikilink")。
这里可以直接证明P(X)是不可约的，并且不可分。事实上这是为什么不可分性需要被强调的一个例子；用几何的语言来说，P代表了有限域上的一个射影直线，将坐标取p次幂。这样的[映射对有限域上的](https://zh.wikipedia.org/wiki/映射 "wikilink")[代数几何是基础的](../Page/代数几何.md "wikilink")。换言之，存在一些不能用伽罗华理论来描述的覆盖。（见[根态射](https://zh.wikipedia.org/wiki/根态射 "wikilink")（radical
morphism）条目寻找更高层次的阐述）
...
可以证明L和它自己在K上的[张量积有非零的](../Page/张量积.md "wikilink")[幂零元](../Page/幂零元.md "wikilink")。这又一次证明了不可分性：这是说域上的张量积操作不需要形成一个域的积环（因此没有交换的[半单环](https://zh.wikipedia.org/wiki/半單模#半單環 "wikilink")）。
...
另一个例子：P同上，一个群G的预解式R是一个系数为p为系数的多项式的多项式，R提供了关于P的伽罗华群的信息。更准确的说，若R是可分的并且有有理根则P的伽罗华群包含于G。例如若D是P的[判别式](../Page/判别式.md "wikilink")，则X<sup>2</sup>-D是[交错群的预解式](../Page/交错群.md "wikilink")。当P是可约的这个预解式总是可分的，但是大多的预解式是不可分的。

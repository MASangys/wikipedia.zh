...
一个[函数f](../Page/函数.md "wikilink"):A→B，其定义域A和陪域B上分别定义了某种[加法](../Page/加法.md "wikilink")\(+_A\)和\(+_B\)。若该函数满足：∀x,y∈A，有\(f(x+_Ay)= f(x)+_Bf(y)\)。则称f对于\(+_A\)和\(+_B\)满足**可加性**。在上下文对于\(+_A\)和\(+_B\)都很明确的情况下，通常简称为
f 满足**可加性**，亦称f为**可加函数**。

...
严格来讲，一个*n*维流形的维数为 *p* 的叶状结构是一个有如下映射的[坐标卡](https://zh.wikipedia.org/wiki/坐标卡 "wikilink")\(U_i\)组成的一个覆盖:

\[\phi_i:U_i \to \R^n\]

使得在交集\(U_i \cap U_j\)上的[变换函数](https://zh.wikipedia.org/wiki/变换函数 "wikilink")\(\varphi_{ij}\) 定义为
...
其中\(x\)表示前\(n-p\)个坐标，而\(y\)表示后面*p*个坐标。在坐标卡\(U_i\)上,**条纹** \(x=\)[常数和其他坐标卡](https://zh.wikipedia.org/wiki/常数 "wikilink")\(U_j\)上的条纹吻合.

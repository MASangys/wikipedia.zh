...
homo，意為與符號含義表示相同）是某些编程语言的特殊屬性，它意味着一个程序的结构与其[句法](../Page/句法.md "wikilink")是相似的，因此易于通过阅读程序来推测程序的内在涵义。如果一门编程语言具备了同像性，说明该语言的文本表示（通常指源代码）與其[抽象語法樹](https://zh.wikipedia.org/wiki/抽象語法樹 "wikilink")（AST）具有相同的結構（即，AST
...
[Lisp編程语言是具有同像性質的典型範例](https://zh.wikipedia.org/wiki/Lisp "wikilink")\[1\]。它的設計很容易進行對列表的操作，而且其語法結構即採用嵌套列表形式的[S-表达式](../Page/S-表达式.md "wikilink")。LISP
程式以列表的形式來編寫;
...
語言）的新計算機。程序或過程資訊在任何時候的呈現，都應該相同於TRAC處理器執行期間對其作用的形式。我們期望內部代碼的字符表示，和外部代碼表示相同一致或非常相似。在本TRAC實作中內部字符立基於[ASCII](../Page/ASCII.md "wikilink")，因為TRAC程序和文本在處理器內部和外部都具有相同的表示，所以術語同像性一詞是適用的，從涵義相同於符號呈現的意義。\[...\]”

依照[道格拉斯·麥克羅伊](../Page/道格拉斯·麥克羅伊.md "wikilink")的提議，依據Peirce，C.S.McIlroy
...
同像性的典型演示是[元循環求值](../Page/自循環直譯器.md "wikilink")(meta-circular evaluator,

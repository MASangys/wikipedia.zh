...
（homology，在[希腊语中](https://zh.wikipedia.org/wiki/希腊语 "wikilink")*homos* =
同）是一类将一个[可换群或者](https://zh.wikipedia.org/wiki/可换群 "wikilink")[模的](https://zh.wikipedia.org/wiki/模_\(数学\) "wikilink")[序列和特定数学对象](../Page/序列.md "wikilink")（例如[拓扑空间或者](../Page/拓扑空间.md "wikilink")[群](../Page/群.md "wikilink")）联系起来的过程。背景知识请参看[同调论](../Page/同调论.md "wikilink")。

对于一个特定的拓扑空间，同调群通常比[同伦群要容易计算得多](https://zh.wikipedia.org/wiki/同伦群 "wikilink")，因此通常来讲用同调来辅助空间分类要容易处理一些。

## 同调群的构造

其过程如下：给定对象\(X\)，首先定义*[链复形](../Page/链复形.md "wikilink")*，它包含了\(X\)的信息。一个链复形是一个由[群同态联系起来的可换群或者模](https://zh.wikipedia.org/wiki/群同态 "wikilink")\(A_0,A_1,A_2,\dots\)的序列，群同态\(d_n : A_n \rightarrow A_{n-1}\)满足任何两个相连的同态的复合为0:
\(d_n \circ d_{n+1} = 0\)对于所有*n*成立。这意味着第*n*+1个映射的[像包含在第](https://zh.wikipedia.org/wiki/像_\(数学\) "wikilink")*n*个映射的[核中](../Page/核_\(代数\).md "wikilink")，我们定义***X*的第*n*阶同调群**为[因子群](https://zh.wikipedia.org/wiki/因子群 "wikilink")（因子模）
...
[环面被定义为两个圆](../Page/环面.md "wikilink")\(T = S^1 \times S^1\)的[笛卡尔积](https://zh.wikipedia.org/wiki/笛卡尔积 "wikilink")。环面具有一个路径连通的分支，两个独立的一维洞（在图中以红圈和蓝圈分别标出），以及一个二维洞（环面的内部）。其对应的同调群为\[4\]
...
[抽象代数中](../Page/抽象代数.md "wikilink")，同调用于定义[导出函子](https://zh.wikipedia.org/wiki/导出函子 "wikilink")，例如，[Tor函子](../Page/Tor函子.md "wikilink")。这里，我们可以从某个可加协变函子\(F\)和某个模\(X\)开始。\(X\)的链复形定义如下：首先找到一个自由模\(F_1\)和一个[满同态](../Page/满射.md "wikilink")\(p_1 : F_1 \rightarrow X\)。然后找到一个自由模\(F_2\)和一个满同态\(p_2 : F_2 \rightarrow \mathrm{ker}(p_1)\)。以该方式继续，得到一个自由模\(F_n\)和同态\(p_n\)的序列。将函子\(F\)应用于这个序列，得到一个链复形；这个复形的同调\(H_n\)仅依赖于\(F\)和\(X\)，并且按定义就是\(F\)作用于\(X\)的*n*阶导出函子。

## 同调函子

链复形构成一个[范畴](https://zh.wikipedia.org/wiki/范畴 "wikilink")：从链复形\((d_n : A_n \rightarrow A_{n-1})\)到链复形\((e_n : B_n \rightarrow B_{n-1})\)的态射是一个同态的序列\(f_n : A_n \rightarrow B_n\)，满足\(f_{n-1} \circ d_n = e_{n-1} \circ f_n\)对于所有*n*成立。*n*阶同调

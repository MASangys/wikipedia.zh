...
**向量空間**是现代[数学中的一个基本概念](../Page/数学.md "wikilink")。是[線性代數研究的基本对象](https://zh.wikipedia.org/wiki/線性代數 "wikilink")。

向量空间的一个直观模型是向量几何，[幾何上的](https://zh.wikipedia.org/wiki/幾何 "wikilink")[向量及相关的運算即向量加法](../Page/向量.md "wikilink")，標量乘法，以及对運算的一些限制如[封闭性](https://zh.wikipedia.org/wiki/封闭性 "wikilink")，[结合律](../Page/结合律.md "wikilink")，已大致地描述了“向量空間”这个數學概念的直观形象。
...
再考虑所有系数为实数的[多项式的集合](https://zh.wikipedia.org/wiki/多项式 "wikilink")\(\mathbb{R}[X]\)。对于通常意义上的多项式加法和标量乘法，\(\mathbb{R}[X]\)也构成一个向量空间。更广泛地，所有从实数域射到实数域的[连续函数的集合](../Page/连续函数.md "wikilink")\(\mathcal{C}(\mathbb{R},\mathbb{R})\)也是向量空间，因为两个连续函数的和或差以及连续函数的若干倍都还是连续函数。
...
可以证明，一个向量空間的所有基都擁有相同[基數](https://zh.wikipedia.org/wiki/基數 "wikilink")，稱為該空間的**[維度](../Page/維度.md "wikilink")**。当**V**是一个有限维空间时，**任何一组基中的元素个数都是定值，等于空间的维度**。例如，各种實數向量空間：ℝ⁰,
ℝ¹, ℝ², ℝ³,…, ℝ<sup>∞</sup>,…中，
...
如果两个向量空間V和W之间的一个線性映射是[一一映射](https://zh.wikipedia.org/wiki/一一映射 "wikilink")，那么这个线性映射称为（线性）**[同构](../Page/同构.md "wikilink")**，表示两个空间构造相同的意思。如果在V和W之間存在同構，那么稱這兩個空間為**同構的**。如果向量空間V和W之间存在同构\(f : \, V \rightarrow W\)，那么其逆映射\(g : \, W \rightarrow V\)也存在，并且对所有的\(x \in V, \, y \in W\)，都有：

...
在較底層語言（如[組合語言與](https://zh.wikipedia.org/wiki/組合語言 "wikilink")[C語言中](https://zh.wikipedia.org/wiki/C語言 "wikilink")），程式控制訊息與資料可能一同被存入呼叫堆疊中，因此造成安全隱患，可能允許惡意程式通過[栈缓冲区溢出](https://zh.wikipedia.org/wiki/栈缓冲区溢出 "wikilink")(stack
buffer overflow)來獲取程式的控制權。

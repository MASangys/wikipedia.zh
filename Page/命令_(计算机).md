在[计算机中](https://zh.wikipedia.org/wiki/電腦運算 "wikilink")，一个**命令**指的是为了完成某种特定任务，而向某种解释器程序发送的指示。 大部分情况下一个命令要么是发往某种[命令行界面程序](../Page/命令行界面.md "wikilink")（例如[殼層](../Page/殼層.md "wikilink")）的指示，要么就是在[图形用户界面中通过用户选择某项](../Page/图形用户界面.md "wikilink")[選單而引发的事件](../Page/選單.md "wikilink")。

具体来说，*命令*这个词一般用于[命令式](https://zh.wikipedia.org/wiki/命令式编程 "wikilink")[计算机语言](https://zh.wikipedia.org/wiki/计算机语言 "wikilink")。之所以这些语言被这么称呼，是因为这些语言内的[语句经常写成类似于](https://zh.wikipedia.org/wiki/陳述_\(程式\) "wikilink")[自然语言中的](https://zh.wikipedia.org/wiki/自然语言 "wikilink")[祈使語氣的样子](../Page/祈使語氣.md "wikilink")。如果说[指令式语言中的语句是自然语言中的句子的话](../Page/指令式編程.md "wikilink")，那么命令差不多就是动词了。
...
以下命令是等价的，都列出目录
`/bin `下的文件。 命令为 [`ls`](https://zh.wikipedia.org/wiki/ls "wikilink")，参数为 [`/bin`](../Page/文件系统层次结构标准.md "wikilink")，有`-l`、`-t`、`-r
三个选项。`
...
以下命令显示文件 `ch1.txt、``ch2.txt `的内容。 命令为 [`cat`](https://zh.wikipedia.org/wiki/Cat_\(Unix\) "wikilink")`，``ch1.txt`<font face="sans-serif">`、`</font>`ch2.txt`
均为参数。

在[物件導向程式設計的範疇中](../Page/物件導向程式設計.md "wikilink")，**命令模式**（英語：**Command
pattern**）是一種設計模式，它嘗試以物件來代表實際行動。命令物件可以把行動(action) 及其參數封裝起來，於是這些行動可以被：

  - 重複多次
  - 取消（如果該物件有[實作的話](../Page/實作.md "wikilink")）
  - 取消後又再重做
...
[<File:Command> Design Pattern Class
Diagram.png](https://zh.wikipedia.org/wiki/File:Command_Design_Pattern_Class_Diagram.png "fig:File:Command Design Pattern Class Diagram.png")

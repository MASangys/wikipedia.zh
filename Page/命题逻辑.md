...
公理的集合可能為空集、非空有限集、可數無限集或由公理模式所給定。[形式文法遞迴地定義了](../Page/形式文法.md "wikilink")[語言的表示式和合式公式](https://zh.wikipedia.org/wiki/形容語言 "wikilink")。之外，有時也可以給定一個[語義](https://zh.wikipedia.org/wiki/語義 "wikilink")，用以定義真值和[賦值](../Page/賦值.md "wikilink")（或[解釋](https://zh.wikipedia.org/wiki/解釋 "wikilink")）。
...
**命题演算**是一個[形式系统](https://zh.wikipedia.org/wiki/形式系统 "wikilink")\(\mathcal{L} = \mathcal{L}\ (\Alpha,\ \Omega,\ \Zeta,\ \Iota)\)，它的公式按如下方式构造：
  ...
  - \(\Alpha\!\)集合是由名為「命題符號」或「[命題變量](https://zh.wikipedia.org/wiki/命題變量 "wikilink")」之元素所組成的有限集合，一个「[命題變量](https://zh.wikipedia.org/wiki/命題變量 "wikilink")」可取值为集合里的「命題符號」。語法上來說，它們是形式語言\(\mathcal{L}\)最基本的元素，亦被稱之為「[原子公式](https://zh.wikipedia.org/wiki/原子公式 "wikilink")」或「終端元素」。在接著的例子中，\(\Alpha\!\)內的元素一般寫作字母p,
    q, r之類的形式。

<!-- end list -->

  - \(\Omega\!\)是名為「[算子符號](../Page/算子.md "wikilink")」或「[邏輯運算符](https://zh.wikipedia.org/wiki/邏輯運算符 "wikilink")」之元素所組成的有限集合。集合\(\Omega\!\)被[劃分成如下等不相交的子集](https://zh.wikipedia.org/wiki/集合劃分 "wikilink")：
...
在[合取](https://zh.wikipedia.org/wiki/合取 "wikilink")、[析取和](https://zh.wikipedia.org/wiki/析取 "wikilink")[蘊涵](https://zh.wikipedia.org/wiki/蘊涵 "wikilink")（∧、∨和→）這三個運算符之中，可以將其中一個拿來當做基本的，而另兩個則以其和否定（¬）來定義。實際上，所有的邏輯運算符都可以用[自足算子的方式來定義](https://zh.wikipedia.org/wiki/自足算子 "wikilink")。而雙條件（↔）當然可由合取和薀涵來定義，亦即a
↔ b可被定義為(a → b)∧(b → a)。
...
其推理規則為[肯定前件](https://zh.wikipedia.org/wiki/肯定前件 "wikilink")（即可由p和(p →
q)導出q）。而a ∨ b和a ∧ b則是分別被定義為¬a → b和¬(a → ¬b)。
...
在此命題演算的例子中，轉換規則被解釋為所謂的「[自然演繹系統](https://zh.wikipedia.org/wiki/自然演繹 "wikilink")」下之推理規則。这里表述的特定系统没有起始点，这意味着它對邏輯應用的解釋是從空公理集合中推導出其[定理的](../Page/定理.md "wikilink")。
...
此命题演算有十个[推理规则](../Page/推理规则.md "wikilink")。这些规则允许我们从给定的一组假定为真的公式中推导出其他为真的公式。前九个只是簡單地指我们可以从其他合式公式推论出特定的合式公式。但是最后一个规则使用了假言（hypothetical）推理，这意味着在规则的前提中，我们可以临时的假定一个（未证明的）假设作为推导出的公式集合的一部分，来查看我们是否能推导出一个特定的其他公式。因为前九个规则不是这样而通常被描述为“非假言”规则，而最后一个則被稱為“假言”规则。
  ...
  - ：從φ → ¬ ψ和φ → ψ中可推出¬ φ。
    [双重否定除去](https://zh.wikipedia.org/wiki/双重否定除去 "wikilink")：从¬ ¬
    φ中可推出φ。
    ：从φ和ψ中可推出(φ ∧ ψ)。
    ：从(φ ∧ ψ)中可推出φ和ψ。
    ：从φ中可推出(φ ∨ ψ)。
    ...
    [肯定前件](https://zh.wikipedia.org/wiki/肯定前件 "wikilink")（条件除去）：从φ和(φ →
    ψ)中可推出ψ。
    [条件证明](https://zh.wikipedia.org/wiki/条件证明 "wikilink")（条件介入）：若假定φ為真可證明出ψ，可推出(φ
    → ψ)。
...
上述的经典命题演算等价于[布尔代数](../Page/布尔代数.md "wikilink")，而[直觉命题演算等价于](https://zh.wikipedia.org/wiki/直觉逻辑 "wikilink")[海廷代数](../Page/海廷代数.md "wikilink")。等价性是通过在两个方向上转换各自系统的定理来证明的。经典命题演算或直觉命题演算的定理Φ被分别转换为布尔代数或Heyting代数的等式Φ
...
y或\(x\ \vdash\ y\)之间的区别在于，前者是内在于逻辑的，而后者是外在的。在两个项之间内在的实质条件是同类的另一个项。在两个项之间的外在的蕴涵表达了在逻辑语言之外的元真理，并被认为是[元语言的一部分](https://zh.wikipedia.org/wiki/元语言 "wikilink")。即使所研究的逻辑是直觉的，蕴涵都通常经典的理解为二值的：要么左侧蕴涵（或小于等于）右侧，要么不蕴涵之。
...
最直接的方式是开发一个更加复杂的逻辑演算，介入对所用于的句子的更精细的细节敏感的规则。在命题逻辑中的[原子句子被分解成](https://zh.wikipedia.org/wiki/原子句子 "wikilink")、[变量](https://zh.wikipedia.org/wiki/变量 "wikilink")、[谓词和](https://zh.wikipedia.org/wiki/谓词 "wikilink")[量词的时候](../Page/量词.md "wikilink")，它们就生成了[一阶逻辑](../Page/一阶逻辑.md "wikilink")，或者叫做一阶谓词逻辑，它保留命题逻辑的所有规则并增加了一些新规则。（例如，从“所有的狗都是动物”我们可以推出“如果Rover是狗，则Rover是动物”）。
...
## 引用
  ...
  - [Brown, Frank
    Markham](https://zh.wikipedia.org/wiki/Frank_Markham_Brown "wikilink")(2003),
  ...
  - [Chang, C.C.](https://zh.wikipedia.org/wiki/C.C._Chang "wikilink"),
    and [Keisler,
    H.J.](https://zh.wikipedia.org/wiki/Howard_Jerome_Keisler "wikilink")(1973),
    Model Theory, North-Holland, Amsterdam, Netherlands.
  - [Kohavi,
    Zvi](https://zh.wikipedia.org/wiki/Zvi_Kohavi "wikilink")(1978),
    Switching and Finite Automata Theory, 1st edition, McGraw–Hill,
    1970. 2nd edition, McGraw–Hill, 1978.
  - [Korfhage, Robert
    R.](https://zh.wikipedia.org/wiki/Robert_R._Korfhage "wikilink")(1974),
    Discrete Computational Structures, Academic Press, New York, NY.
  - [Lambek,
    J.](https://zh.wikipedia.org/wiki/Joachim_Lambek "wikilink") and
    [Scott,
    P.J.](https://zh.wikipedia.org/wiki/P.J._Scott "wikilink")(1986),
    Introduction to Higher Order Categorical Logic, Cambridge University
    Press, Cambridge, UK.
  - [Mendelson,
    Elliot](https://zh.wikipedia.org/wiki/Elliot_Mendelson "wikilink")(1964),
...
## 外部链接
  ...
  - [Klement, Kevin
    C.](https://zh.wikipedia.org/wiki/Kevin_C._Klement "wikilink")(2006),
  ...
  - [Propositional
    Logic](https://zh.wikipedia.org/wiki/PlanetMath:3475 "wikilink")([GFDLed](https://zh.wikipedia.org/wiki/GFDL "wikilink"))

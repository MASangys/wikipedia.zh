...
（*Mf*(*x*)可能是\(\infty\)。）
其中*m*是\(\mathbb R^n\)上的[勒貝格測度](https://zh.wikipedia.org/wiki/勒貝格測度 "wikilink")。
...
存在\(\delta >0\)使得\((r/(r+\delta))^n > c/c_1\)。
對任何\(x'\in B(x,\delta)\)，有\(B(x,r)\subset B(x',r+\delta)\) 所以
...
\end{align}\]
上式第四行的不等式使用了開球兩兩不交性質。從勒貝格測度的[內正則性](https://zh.wikipedia.org/wiki/內正則測度 "wikilink")，集合\(\{Mf > c\}\)的測度等於在其內的所有緊集的測度的上確界，故有

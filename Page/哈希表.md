...
<!-- end list -->
  ...
  - 对不同的关键字可能得到同一散列地址，即\(k_1 \neq k_2\)，而\(f(k_1)=f(k_2)\)，这种现象称为**冲突**（）。具有相同函数值的关键字对该散列函数来说称做**同义词**。综上所述，根据散列函数\(f(k)\)和处理冲突的方法将一组关键字映射到一个有限的连续的地址集（区间）上，并以关键字在地址集中的“[像](https://zh.wikipedia.org/wiki/像_\(数学\) "wikilink")”作为记录在表中的存储位置，这种表便称为**散列表**，这一映射过程称为[散列造表或](../Page/散列.md "wikilink")[散列](../Page/散列.md "wikilink")，所得的存储位置称[散列地址](https://zh.wikipedia.org/wiki/散列地址 "wikilink")。

<!-- end list -->

  - 若对于关键字集合中的任一个关键字，经散列函数映象到地址集合中任何一个地址的概率是相等的，则称此类散列函数为[均匀散列函数](https://zh.wikipedia.org/wiki/均匀散列函数 "wikilink")（Uniform
...
6.  [除留余数法](https://zh.wikipedia.org/wiki/除留余数法 "wikilink")：取关键字被某个不大于散列表表长m的数p除后所得的余数为散列地址。即\(hash(k)=k \,\bmod \,p\),
    \(p\le m\)。不仅可以对关键字直接取模，也可在[折叠法](https://zh.wikipedia.org/wiki/折叠法 "wikilink")、[平方取中法等运算之后取模](https://zh.wikipedia.org/wiki/平方取中法 "wikilink")。对p的选择很重要，一般取素数或m，若p选择不好，容易产生冲突。

## 处理冲突

为了知道冲突产生的相同散列函数地址所对应的关键字，必须选用另外的散列函数，或者对冲突结果进行处理。而不发生冲突的可能性是非常之小的，所以通常对冲突进行处理。常用方法有以下几种：

  - [开放定址法](https://zh.wikipedia.org/wiki/开放定址法 "wikilink")（open
...
[聚集](https://zh.wikipedia.org/wiki/聚集 "wikilink")（Cluster，也翻译做“堆积”）的意思是，在函数地址的表中，散列函数的结果不均匀地占据表的单元，形成区块，造成线性探测产生一次聚集（primary
...
<!-- end list -->
  ...
  - [再散列](https://zh.wikipedia.org/wiki/再散列 "wikilink")：\(hash_i = hash_i (key)\),
    \(i=1,2...k\)。\(hash_i\)是一些散列函数。即在上次散列计算发生冲突时，利用该次冲突的散列函数地址产生新的散列函数地址，直到冲突不再发生。这种方法不易产生“[聚集](https://zh.wikipedia.org/wiki/聚集 "wikilink")”（Cluster），但增加了计算时间。

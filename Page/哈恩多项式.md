[Hahn_polynomials.gif](https://zh.wikipedia.org/wiki/File:Hahn_polynomials.gif "fig:Hahn_polynomials.gif")
**哈恩多项式**是一个以德国数学家Wolfgang
Hahn命名的正交多项式，由下列[广义超几何函数定义](https://zh.wikipedia.org/wiki/广义超几何函数 "wikilink")：\[1\]
...
对于\(\alpha\) \> -1 和 \(\beta\) \> -1 以及 \(\alpha\) \< -N \(\beta\) \<
-N,下列正交关系成立\[2\]
...
哈恩多项式满足下列归递关系\[3\]
\(-x*Q_n(x)\)=\(A_{n}*Q_{n+1}(x)\)-\((A_{n}+C_{n})*Q_{n}(x)\)\*\(C_{n}*Q_{n-1}(x)\)

其中\(Q_{n}(x)=Q_{n}(x;\alpha,\beta,N)\)

## 极限关系

  - [拉卡多项式](https://zh.wikipedia.org/wiki/拉卡多项式 "wikilink")→**哈恩多项式**\[4\]
...
<references/>
  ...
  - Roelof Koekoek, Peter A.Lesky,ReneF.Swarttouw,Hypergeometric
    Orthogonal Polynomials ad Their q=Aalogues, Springer,2008.

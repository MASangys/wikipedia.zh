...
## 例子
  ...
  - 商環是構造[代數擴張的主要工具](../Page/代數擴張.md "wikilink")。例如取實係數多項式環\(R = \mathbb{R}[X]\)，\(I =(X^2+1)\mathbb{R}[X]\)，則商環\(\mathbb{R}[X]/(X^2+1)\)與複數域\(\mathbb{C}\)同構（考慮映射\(f (X) +(X^2+1) \mapsto f (i)\)）。一般而言，設\(F\)為一個[域](https://zh.wikipedia.org/wiki/域 "wikilink")，\(p (X) \in F[X]\)為\(F\)上的不可約多項式，則商環\(F[X]/p (X)\)的意義在於抽象地在\(F\)上加進\(p (X)\)的一個根。

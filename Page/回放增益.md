...
Vorbis等格式的数字化音频的可感知响度进行](https://zh.wikipedia.org/wiki/Ogg_Vorbis "wikilink")[标准化](../Page/标准化.md "wikilink")（Normalize）处理。

## 概要

[ReplayGainScanResults.PNG](https://zh.wikipedia.org/wiki/File:ReplayGainScanResults.PNG "fig:ReplayGainScanResults.PNG")
0.9进行回放增益扫描的结果\]\]
进行回放增益处理的步骤是，首先对整个音频文件进行[心理声学分析扫描](../Page/心理声学.md "wikilink")，以量度它的感知响度和峰值电平。音频文件的原始响度与目标响度（通常定为89分贝的[声压值](../Page/声压.md "wikilink")）的差别被计算出来，这个差别称为[增益值](../Page/增益.md "wikilink")。典型的做法是，将该音频文件的增益值和峰值电平值存储于文件的[元数据中](../Page/元数据.md "wikilink")，以使支持回放增益的音频播放器能够自动[衰减](https://zh.wikipedia.org/wiki/衰减 "wikilink")（或在一些情形下放大）输出，从而在回放不同的音频文件时得到相似的输出响度。这样就可以避免因制作时制定的标准不同而造成在播放不同专辑的音频文件时需要不断手动调整音量的问题。进行回放增益处理的另一个优势在于，音频文件的峰值电平信息也可用于避免响度较高的歌曲产生[削波](https://zh.wikipedia.org/wiki/削波 "wikilink")。

回放增益的的实现通常也包括向音频文件内加入元数据而不改变原始音频数据的过程。在回放增益标准指定了一个8字节文件头域的同时，许多流行的音频格式利用文件标记（tag）来记录回放增益信息。如[Flac和](https://zh.wikipedia.org/wiki/Flac "wikilink")[Ogg
Vorbis格式的文件利用](https://zh.wikipedia.org/wiki/Ogg_Vorbis "wikilink")`REPLAYGAIN_*`注释域；而[MP3文件通常使用](../Page/MP3.md "wikilink")[ID3v2或APEv](../Page/ID3.md "wikilink")2格式的文件标记来记录。
...
## 支持回放增益的音频播放器
  ...
  - [GNOME版本的](../Page/GNOME.md "wikilink")[Muine音乐播放器](https://zh.wikipedia.org/wiki/Muine "wikilink")。
  ...
  - [数字音频播放器的的](https://zh.wikipedia.org/wiki/数字音频播放器的 "wikilink")
    [Rockbox](https://zh.wikipedia.org/wiki/Rockbox "wikilink")。
  ...
  - [Xbox版本的](https://zh.wikipedia.org/wiki/Xbox "wikilink")[Xbox媒体中心](https://zh.wikipedia.org/wiki/Xbox_Media_Center "wikilink")。
...
## 回放增益扫描器的实现
  ...
  - [iVolume](https://zh.wikipedia.org/wiki/iVolume "wikilink"):替换`iTunNORM`元数据文件标记值（以每个专辑为基础可选），该值用于[iTunes和](https://zh.wikipedia.org/wiki/iTunes "wikilink")[iPod的声音测试音量标准化](https://zh.wikipedia.org/wiki/iPod "wikilink")。只用于Mac
    OS X上。
...
## 外部链接
  ...
  - [Replaygain](http://wiki.hydrogenaudio.org/index.php?title=Replaygain)位于[Hydrogenaudio](https://zh.wikipedia.org/wiki/Hydrogenaudio "wikilink")

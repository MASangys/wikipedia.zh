> 本文内容由[因特网信息访问协议](https://zh.wikipedia.org/wiki/因特网信息访问协议)转换而来。


**因特网信息访问协议**（，缩写：**IMAP**；以前称作**交互邮件访问协议**）是一个应用层协议，用来从本地邮件客户端（如[Microsoft](https://zh.wikipedia.org/wiki/Microsoft "wikilink") [Outlook](https://zh.wikipedia.org/wiki/Outlook "wikilink")、[Outlook Express](../Page/Outlook_Express.md "wikilink")、[Foxmail](../Page/Foxmail.md "wikilink")、[Mozilla Thunderbird](../Page/Mozilla_Thunderbird.md "wikilink")）访问远程服务器上的邮件。

## 概述

IMAP和[POP3](https://zh.wikipedia.org/wiki/POP3 "wikilink")（Post Office Protocol - Version 3，邮局协议第三版）是邮件访问最为普遍的Internet标准协议。事实上所有现代的邮件客户端和服务器都对两者给予支持。IMAP现在的版本是“IMAP第7版第3次修订版”（IMAP4rev1），在 RFC 3345 中定义。

IMAP由[Mark Crisp设计](https://zh.wikipedia.org/wiki/Mark_Crisp "wikilink")，对于邮件访问提供了相对于广泛使用的PP3邮件协议的另外一种选择。基本上，两者都允许一个邮件客户端访问邮件服务器上存储的信息。一些只在IMAP中存在的重要方面包括：

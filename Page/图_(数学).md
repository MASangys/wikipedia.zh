在[數學的分支](../Page/數學.md "wikilink")[图论中](../Page/图论.md "wikilink")，**图**（）用于表示物件與物件之間的關係，是[圖論的基本研究對象](../Page/圖論.md "wikilink")。一张圖由一些小圓點（稱為**頂點**或**結點**）和連結這些圓點的直線或曲線（稱為**[邊](../Page/邊.md "wikilink")**）組成。[西尔维斯特在](../Page/詹姆斯·约瑟夫·西尔维斯特.md "wikilink")1878年首次提出“图”这一名词。
...
一张**图** \(G\)
是一個[二元组](../Page/有序对.md "wikilink")\((V,E)\)，其中\(V\)称为顶點集，\(E\)称为边集。它們亦可寫成\(V(G)\)和\(E(G)\)。
\(E\)的元素是一個二元組數對，用\((x,y)\)表示，其中\(x,y \in V\)。

### 三元組的定義

一张**图** \(G\) 是一个三元组\((V,E,I)\)，其中\(V\)称为顶集（Vertices
set），\(E\)称为边集（Edges
set），\(E\)与\(V\)不相交；\(I\)称为关联函数，\(I\)将\(E\)中的每一个元素映射到\(V\times V\)。如果\(I(e)=(u,v) (e\in E, u,v \in V)\)那么称边\(e\)连接顶点\(u,v\)，而\(u,v\)则称作\(e\)的端点，\(u,v\)此时关于\(e\)相邻。同时，若两条边\(i,j\)有一个公共顶点\(u\)，则称\(i,j\)关于\(u\)相邻。
...
<!-- end list -->
  ...
  -
    则称为简单图（Simple
    graph）。簡單的有向圖和無向圖都可以使用以上的「二元組的定義」，但形如\((x,x)\)的序對不能屬於E。而無向圖的邊集必須是對稱的，即如果\((x,y)\in E\)，那么\((y,x)\in E\)。
...
[6n-graph2.svg](https://zh.wikipedia.org/wiki/File:6n-graph2.svg "fig:6n-graph2.svg")
  ...
  - **度**（Degree）：一个顶点的度是指与该頂點相关联的總边数，顶点\(v\)的度记作\(d(v)\)。度和邊有如下關係：\(\sum_{v\in V} d(v)=2\left|E\right|\)。
  - **出度**（Out-degree）和**入度**（In-degree）：對有向圖而言，頂點的度還可分為出度和入度。一個頂點的出度為\(d_o\)，是指有\(d_o\)條邊以該頂點為起點，或說與該點關聯的出邊共有\(d_o\)條。入度的概念也類似。
  - **[鄰接矩陣](../Page/鄰接矩陣.md "wikilink")**（Adjacency matrix）
  - **自环**（Loop）：若一条边的两个顶点相同，则此边称作自环。
  - **路径**（Path）：从頂點u到頂點v的一条路径是指一个序列\(v_0,e_1,v_1,e_2,v_2,...e_k,v_k\)，\(e_i\)的起點終點为\(v_{i-1}\)及\(v_i\)；\(k\)称作路径的长度；\(v_0=u\)，稱為路徑的起點；\(v_k=v\)，稱為路徑的終點。如果\(u=v\)，稱該路径是**闭**的，反之則稱為**開**的；如果\(v_1,...,v_k\)兩兩不等，則称之为**简单路径**（Simple
    path，注意，\(u=v\)是允許的）。
  ...
  - **距離**（Distance）：
    从頂點\(u\)出發到頂點\(v\)的最短路徑若存在，則此路徑的長度稱作從\(u\)到\(v\)的距離。若從\(u\)到\(v\)根本不存在路徑，則記該距離為無窮（\(\infty\)）。
  - **[距離矩陣](../Page/距離矩陣.md "wikilink")**（Distance matrix）
  - **橋**（Bridge）：若去掉一條邊，便會使得整個圖不連通，該邊稱為橋。

## 图的存储表示

  - [数组](../Page/数组.md "wikilink")（[邻接矩阵](../Page/邻接矩阵.md "wikilink")）存储表示（有向或无向）
  - [邻接表存储表示](../Page/邻接表.md "wikilink")
  - [前向星存储表示](../Page/前向星.md "wikilink")
  - [有向图的](../Page/有向图.md "wikilink")[十字链表存储表示](../Page/十字链表.md "wikilink")
  - [无向图的](../Page/无向图.md "wikilink")[邻接多重表存储表示](../Page/邻接多重表.md "wikilink")
...
图的遍历方法有[深度优先搜索法和](../Page/深度优先搜索法.md "wikilink")[广度（宽度）优先搜索法](../Page/广度优先搜索法.md "wikilink")。
...
图的**广度优先搜索**是树的按层次遍历的推广，它的基本思想是：首先访问初始点vi，并将其标记为已访问过，接着访问vi的所有未被访问过的邻接点vi1,vi2,…,
vi t，并均标记已访问过，然后再按照vi1,vi2,…, vi
t的次序，访问每一个顶点的所有未被访问过的邻接点，并均标记为已访问过，依次类推，直到图中所有和初始点vi有路径相通的顶点都被访问过为止。其非递归算法如下：
...
## 圖的重要類型
  ...
  - [补图](../Page/补图.md "wikilink")：与G拥有相同的点的完全图删除原图G的边所得到的图成为G的补图
  - [树状图](../Page/树状图.md "wikilink")
  - [平面图](../Page/平面图.md "wikilink")
  - [连通图](../Page/连通图.md "wikilink")
      - [强连通图](../Page/连通图#.E7.9B.B8.E5.85.B3.E6.A6.82.E5.BF.B5.md "wikilink")
      - [强连通分量](../Page/强连通分量.md "wikilink")
  - [有向无环图](../Page/有向无环图.md "wikilink")
      - [AOV网](../Page/AOV网.md "wikilink")
      - [AOE网](../Page/AOE网.md "wikilink")
  - [完全图](../Page/完全图.md "wikilink")：每一对不同顶点间都有边相连的的图，记作\(K_n\)。
  ...
  - [正则图](../Page/正则图.md "wikilink")：如果图中所有顶点的度皆相等，则此图称为正则图
...
## 外部链接
  ...
  - [Graph
    Theory](http://www.math.uni-hamburg.de/home/diestel/books/graph.theory/)（可下載的書籍，英語）

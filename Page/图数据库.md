...
图数据库的底层存储机制可能各有不同。有些依赖于关系引擎并将图数据“存储”到[表中](https://zh.wikipedia.org/wiki/数据库表 "wikilink")（虽然表是一个逻辑元素，但是这种方法在图数据库、图数据库管理系统和实际存储数据的物理设备之间施加了另一层抽象）。另一些则使用[键值存储或](https://zh.wikipedia.org/wiki/键值存储 "wikilink")[文件導向的數據庫进行存储](../Page/文件導向的數據庫.md "wikilink")，使它们具有固有的[NoSQL结构](../Page/NoSQL.md "wikilink")。大多数基于非关系存储引擎的图数据库还添加了**标记**或**属性**的概念，这些标记或属性本质上是具有指向另一个文档的指针的关系。这样就可以对数据元素进行分类，以便于集中检索。

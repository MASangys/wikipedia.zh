...
图数据库的底层存储机制可能各有不同。有些依赖于关系引擎并将图数据“存储”到[表中](https://zh.wikipedia.org/wiki/数据库表 "wikilink")（虽然表是一个逻辑元素，但是这种方法在图数据库、图数据库管理系统和实际存储数据的物理设备之间施加了另一层抽象）。另一些则使用[键值存储或](https://zh.wikipedia.org/wiki/key-value_store "wikilink")[文件導向的數據庫进行存储](../Page/文件導向的數據庫.md "wikilink")，使它们具有固有的[NoSQL结构](../Page/NoSQL.md "wikilink")。大多数基于非关系存储引擎的图数据库还添加了**标记**或**属性**的概念，这些标记或属性本质上是具有指向另一个文档的指针的关系。这样就可以对数据元素进行分类，以便于集中检索。

从图数据库中检索数据需要[SQL之外的](../Page/SQL.md "wikilink")[查詢語言](https://zh.wikipedia.org/wiki/查詢語言 "wikilink")，SQL是为了处理关系系统中的数据而设计的，因此无法“优雅地”处理遍历图。，没有一个像SQL那样通用的图查询语言，通常都是仅限与一个产品的。不过，已经有一些标准化的工作，使得[Gremlin](../Page/Gremlin.md "wikilink")、[SPARQL和](../Page/SPARQL.md "wikilink")[Cypher成为了多供应商查询语言](https://zh.wikipedia.org/wiki/Cypher_Query_Language "wikilink")。除了具有查询语言接口外，还可以通过[应用程序接口](../Page/应用程序接口.md "wikilink")（API）访问一些图数据库。
...
<td></td>
<td><p>未披露</p></td>
<td><p>Amazon Neptune 是<a href="https://zh.wikipedia.org/wiki/亚马逊公司" title="wikilink">亚马逊公司完全托管的图数据库</a>。它被用作<a href="https://zh.wikipedia.org/wiki/web服务" title="wikilink">web服务</a>，是<a href="../Page/亚马逊云计算服务.md" title="wikilink">亚马逊云计算服务的一部分</a>。支持流行的图形模型属性图和<a href="https://zh.wikipedia.org/wiki/W3C" title="wikilink">W3C的</a><a href="https://zh.wikipedia.org/wiki/资源描述框架" title="wikilink">RDF</a>，以及它们各自的<a href="https://zh.wikipedia.org/wiki/查询语言" title="wikilink">查询语言</a> Apache TinkerPop <a href="../Page/Gremlin.md" title="wikilink">Gremlin和</a><a href="../Page/SPARQL.md" title="wikilink">SPARQL</a>。</p></td>
</tr>
<tr class="odd">

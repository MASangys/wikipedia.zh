在[统计学中](../Page/统计学.md "wikilink")，**均方误差**（、**MSE**）是对于无法观察的参数\(\theta\)的一个估计函数*T*；其定义为：
...
也就是说，[偏差](../Page/偏差.md "wikilink")\(\operatorname{bias}(T)\)是估计函数的期望值与那个无法观察的参数的差。
...
第一个估计函数为[最大似然估计](../Page/最大似然估计.md "wikilink")，它是有偏的，即[偏差不为零](../Page/偏差.md "wikilink")，但是它的方差比第二个小。而第二个估计函数是无偏的。较大的方差某种程度上补偿了偏差，因此第二个估计函数的均方误差比第一个要大。

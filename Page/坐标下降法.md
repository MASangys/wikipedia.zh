...
坐标下降法基于的思想是多变量函数\(F(\mathbf{x})\)可以通过每次沿一个方向优化来获取最小值。与通过梯度获取最速下降的方向不同，在坐标下降法中，优化方向从算法一开始就予以固定。例如，可以选择[线性空间的一组](https://zh.wikipedia.org/wiki/线性空间 "wikilink")[基](../Page/基_\(線性代數\).md "wikilink")\(\mathbf{e}_1, \mathbf{e}_2, \dots, \mathbf{e}_n\)作为搜索方向。 在算法中，循环最小化各个坐标方向上的目标函数值。亦即，如果\(\mathbf{x}^k\)已给定，那么，\(\mathbf{x}^{k+1}\)的第\(i\)个维度为

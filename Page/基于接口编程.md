**基于接口编程**（**Interface-based programming**）也称**基于接口的架构**（**interface-based architecture**），它是一种在没有模块系统的[面向对象程序设计](../Page/面向对象程序设计.md "wikilink")语言中的[组件层面实现](https://zh.wikipedia.org/wiki/基于组件的软件工程 "wikilink")[模块化编程的](https://zh.wikipedia.org/wiki/模組_\(程式設計\) "wikilink")。符合此例的一个语言是[Java](../Page/Java.md "wikilink")，，它没有组件级别的模块系统。Java有一个包（package）系统，但Java软件组件通常由多个Java包组成——并且在任何情况下，接口编程可以提供优于仅使用Java包的优势，即使组件仅由单个Java包组成。

...
HOTP在2005年由[IETF发布在RFC](../Page/互联网工程任务组.md "wikilink")
4226标准文档中，定义了算法的同时附带有一例基于Java的实现。自此之后，世界上许多公司接纳了HOTP算法，它也成为了可以自由使用的[开源标准](../Page/开源标准.md "wikilink")。
...
任何有意愿使用HOTP的一方都必须定义一系列对于验证器而言已明确的参数，无论是否这些参数是否可以被验证器接受，都可能包含：
  ...
  - 服务器分发的[密钥](https://zh.wikipedia.org/wiki/密钥 "wikilink")，记为*K*，可以是任意长度的字符串，但必须保密。
...
专有的硬件密钥通常会提供六位验证码，也就是*d*的默认值。密钥截留最长长度为31位，由于公式中
\(\log_{10}\left(2^{31}\right)\) ≈
9.3，也就是*d*的最大值取10，因此第十位数值可能的取值也就更少，为0、1或者2（也就对应多出的0.3这个小数数值）。
...
**HOTP**本质上是[散列消息验证码](https://zh.wikipedia.org/wiki/金鑰雜湊訊息鑑別碼 "wikilink")（HMAC）的计数器截留，计数器数值记为*C*（服务器分发密钥记为*K*，散列函数记为*H*）。
...
截取首先是位于最小的四位*MAC*数值当中，截得之后用于作为回调数值，记为*i*。
  ...
  -
    truncate(*MAC*) = extract31(*MAC*, *MAC*\[(19 × 8) + 4:(19 × 8) +
    7\] × 8)
...
已有许多供应商（参考下面的列举）提供软件以及硬件密钥。实现开放验证提案的HOTP硬件密钥实现往往比使用专有算法的同类产品便宜许多。\[2\]
截止至2010年，这些硬件密钥的价格已经很低。\[3\] 有些产品也可以同时用于生成强密码。\[4\]

软件密钥在（几乎）所有主要移动平台或[智能手机均有提供](../Page/智能手机.md "wikilink")（如[J2ME](../Page/Java_ME.md "wikilink"),\[5\]
[Android](../Page/Android.md "wikilink"),\[6\]\[7\]
[iPhone](https://zh.wikipedia.org/wiki/iPhone "wikilink"),\[8\]
[BlackBerry](../Page/黑莓手機.md "wikilink"),\[9\]
[Maemo](../Page/Maemo.md "wikilink"),\[10\]
[macOS](https://zh.wikipedia.org/wiki/macOS "wikilink"),\[11\]
以及[Windows Mobile](../Page/Windows_Mobile.md "wikilink")\[12\]）。

## 接纳程度

尽管许多计算机媒体在2004-05年持负面态度，\[13\]\[14\]\[15\]自从IETF在2005年十二月将HOTP接纳为RFC
4226标准之后，许多提供商开始生产兼容HOTP的密钥，甚至是基于此的整套验证解决方案。

2010年（[高德纳咨询公司的下辖部门](../Page/高德纳咨询公司.md "wikilink")）发表文章，表示“[高德纳咨询公司预期基于硬件的](../Page/高德纳咨询公司.md "wikilink")[一次性密码](https://zh.wikipedia.org/wiki/一次性密码 "wikilink")的市场会有一定程度的增长，对于智能手机来说，一次性密码将会成为默认硬件平台的一部分”。\[16\]
...
## 外部链接
  ...
  - [RFC4226: HOTP: An HMAC-Based One-Time Password
    Algorithm](http://www.ietf.org/rfc/rfc4226.txt)
  - \[rfc:6287 RFC 6287: OCRA: An HMAC-Based OATH Challenge-Response
    Algorithm\]
  - [Initiative For Open
    Authentication](http://www.openauthentication.org/)
  - [Step by step Python implementation in a Jupyter
    Notebook](https://nbviewer.jupyter.org/github/algorithmic-space/cryptoy/blob/master/rfc4226.ipynb)

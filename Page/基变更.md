在[线性代数](../Page/线性代数.md "wikilink")中，*n* 维[向量空间](../Page/向量空间.md "wikilink")的[基是](https://zh.wikipedia.org/wiki/基_\(线性代数\) "wikilink") *n* 个向量 α<sub>1</sub>, ..., α<sub>n</sub> 的序列，带有所有这个空间中的向量可以唯一的表达为[基向量的](https://zh.wikipedia.org/wiki/基向量 "wikilink")[线性组合的性质](https://zh.wikipedia.org/wiki/线性组合 "wikilink")。因为经常需要处理一个向量空间的多于一个的基，在线性代数中能够轻易的变换向量的逐坐标表达，和变换关于一个基的[线性映射](../Page/线性映射.md "wikilink")到关于另一个基的等价表达是根本重要的。这种变换叫做**基变更**。
...
R<sup>n</sup> 的**平常基**是 \(\{ \vec{e}_1, \cdots, \vec{e}_n \}\)，这里的 \(\vec{e}_j = (0, \cdots, 1, 0, \cdots, 0)\) 是 R<sup>n</sup> 的元素，带有 1 在第 *j* 个位置上其他地方都是 0。

...
[Aleph0.svg](https://zh.wikipedia.org/wiki/File:Aleph0.svg "fig:Aleph0.svg")Aleph-0,最小的無限基數\]\]
[康托尔在](https://zh.wikipedia.org/wiki/康托尔 "wikilink")1874年－1884年引入最原始的[集合論](https://zh.wikipedia.org/wiki/集合論 "wikilink")（現稱[樸素集合論](https://zh.wikipedia.org/wiki/樸素集合論 "wikilink")）時，首次引入基數概念。
他最先考慮的是集合\(\{1,2,3\}\)和\(\{2,3,4\}\)，它們並非*相同*，但有*相同的基數*。驟眼看來，這是顯而易見，但究竟何謂兩個集合有相同數目的元素？
...
康托爾發現，原來[有理數集合與](https://zh.wikipedia.org/wiki/有理數 "wikilink")[代數數集合也是可數的](../Page/代數數.md "wikilink")。於是乎在1874年初，他嘗試證明是否所有無限集合均是可數，其後他得出了[實數集不可數的結論](https://zh.wikipedia.org/wiki/實數 "wikilink")。原先的證明用到了涉及區間套的複雜論證，而在他1891年的論文中，他以簡單而巧妙的[對角論證法重新證明了這一結果](../Page/對角論證法.md "wikilink")。實數集的基數，記作c，代表[連續統](https://zh.wikipedia.org/wiki/連續統假設 "wikilink")。
...
我们可以把这个概念扩展到一个類似於等式的关系。两个[集合X和](https://zh.wikipedia.org/wiki/集合 "wikilink")\(Y\)被称为有相同的'势'，如果存在\(X\)和\(Y\)之间的[双射](../Page/双射.md "wikilink")。通过[Schroeder-Bernstein定理](https://zh.wikipedia.org/wiki/康托尔－伯恩斯坦－施罗德定理 "wikilink")，这等价于有从\(X\)到\(Y\)和从\(Y\)到\(X\)的两个一一映射。我们接着記之为
\(| X | = | Y |\)。\(X\)的基数自身经常被定义为有着 \(| a | = | X |\)
的最小序数\(a\)。这叫做[冯·诺伊曼基数指派](../Page/冯·诺伊曼基数指派.md "wikilink")；为使这个定义有意义，必须证明所有集合都有同某个[序数一样的势](../Page/序数.md "wikilink")；这个陈述就是[良序原理](https://zh.wikipedia.org/wiki/良序定理 "wikilink")。然而即使不給集合的勢指派一個名字，討論集合之間[相對的勢還是可以的](https://zh.wikipedia.org/wiki/势的比较 "wikilink")。
...
当我们考虑这些大对象的时候，我们还想看看计数次序的概念是否符合上述为无限集合定义的基数。事實上是不一致的；通过考虑上面的例子，我们可以看到如果有“比无限大一”的某个对象存在，它必须跟起初的无限集合有一样的势。這時候可以使用另一種稱為[序数的形式概念](../Page/序数.md "wikilink")，它是建基于计数并依次考虑每个数的想法上。而我们會发现，势和序（ordinality）的概念对于无限的情況是有分歧的。

可以证明[实数的势大于刚才描述的自然数的势](../Page/实数.md "wikilink")。透過[对角论证法可以一目瞭然](https://zh.wikipedia.org/wiki/对角论证法 "wikilink")；跟势相關的经典问题（比如[连续统假设](../Page/连续统假设.md "wikilink")）主要关注在某一对无限基数之间是否有別的基数。現時数学家已经在描述更大更大基数的性质。

因为基数是数学中如此常用的概念，有各种各样的名字指涉它。势相同有时也叫做[等势](https://zh.wikipedia.org/wiki/等势 "wikilink")、均势或等多（equipotence,
equipollence, equinumerosity）。因此称有相同势的两个集合为等势的、均势的或等多的（equipotent,
equipollent, equinumerous）。

## 定義

首先，給出集合\(X\)和\(Y\)，我們稱\(X\)的勢小於等於\(Y\)，記作
\(| X | \leq  | Y |\)，當且僅當存在由\(X\)到\(Y\)的[單射](https://zh.wikipedia.org/wiki/單射 "wikilink")；稱\(X\)的勢與\(Y\)相等，記作
\(| X | = | Y |\)，
當且僅當存在由\(X\)到\(Y\)的[雙射](https://zh.wikipedia.org/wiki/雙射 "wikilink")（即一一對應）。

[康托尔-伯恩斯坦-施罗德定理指出如果](../Page/康托尔-伯恩斯坦-施罗德定理.md "wikilink")
\(| X | \leq  | Y |\) 及 \(| Y | \leq  | X |\) 則 \(| X | = | Y |\)。

假設[選擇公理](https://zh.wikipedia.org/wiki/選擇公理 "wikilink")，所有集合都可[良序](https://zh.wikipedia.org/wiki/良序 "wikilink")，且對於所有集合\(X\)與\(Y\)，有
\(| X | \leq  | Y |\) 或
\(| Y | \leq  | X |\)。因此，我們可以定義[序數](https://zh.wikipedia.org/wiki/序數 "wikilink")，而
集合\(X\)的**基數**則是與\(X\)等勢的最小[序數](https://zh.wikipedia.org/wiki/序數 "wikilink")\(\alpha\)。
...
給出集合\(X\)與\(Y\)，定義
\(X + Y = \{(x,0): x \in  X\} \cup  \{(y,1): y \in  Y\}\)，則基數和是
...
\[|X| |Y| = |X \times  Y|\]
其中\(X \times  Y\)是\(X\)和\(Y\)的[笛卡儿积](../Page/笛卡儿积.md "wikilink")。

基數指數是

\[|X|^{|Y|} = |X^{Y}|\]
其中\(X^{Y}\)是所有由\(Y\)到\(X\)的[函數的集合](https://zh.wikipedia.org/wiki/函數_\(數學\) "wikilink")。

在有限集時，這些運算與自然數無異。一般地，它們亦有普通算術運算的性質：

  - 加法和乘法是[可交換的](https://zh.wikipedia.org/wiki/交换律 "wikilink")，即
    \(|X|+|Y|=|Y|+|X|\) 及 \(|X||Y|=|Y||X|\)
  - 加法和乘法符合[結合律](https://zh.wikipedia.org/wiki/結合律 "wikilink")，\((|X|+|Y|)+|Z|=|X|+(|Y|+|Z|)\)及
    \((|X||Y|)|Z|=|X|(|Y||Z|)\)
...
## 參考文獻
  ...
  - Hahn, Hans, *Infinity*, Part IX, Chapter 2, Volume 3 of *The World
    of Mathematics*. New York: Simon and Schuster, 1956.
  - [Halmos,
    Paul](https://zh.wikipedia.org/wiki/Paul_Halmos "wikilink"), *[Naive
    set
    theory](https://zh.wikipedia.org/wiki/Naive_Set_Theory_\(book\) "wikilink")*.
...
[ru:Кардинальное
число](https://zh.wikipedia.org/wiki/ru:Кардинальное_число "wikilink")

**多协议标签交换**（，縮寫為**MPLS**）是一种在开放的通信网上利用[标签引导](https://zh.wikipedia.org/wiki/标签 "wikilink")[数据高速](https://zh.wikipedia.org/wiki/数据 "wikilink")、高效传输的新技术。多协议的含义是指MPLS不但可以支持多种网络层层面上的协议，还可以兼容第二层的多种[数据链路层技术](../Page/数据链路层.md "wikilink")。

它的价值在于能够在一个无连接的网络中引入连接模式的特性；其主要优点是减少了网络复杂性，兼容现有各种主流网络技术，能降低网络成本，在提供IP业务时能确保QoS和安全性，具有流量工程能力。此外，MPLS能解决VPN扩展问题和维护成本问题。

MPLS属于第三代网络架构，是新一代的IP高速骨干网络交换标准，由[IETF所提出](https://zh.wikipedia.org/wiki/IETF "wikilink")，由[Cisco](https://zh.wikipedia.org/wiki/Cisco "wikilink")、[ASCEND](https://zh.wikipedia.org/wiki/ASCEND "wikilink")、[3Com等网络设备大厂所主导](../Page/3Com.md "wikilink")。

采用MPLS的数据包只须在OSI第二层（数据链结层）执行硬件式交换（取代第三层（[网络层](../Page/网络层.md "wikilink")）软件式routing），它整合了IP选径与第二层标记交换为单一的系统，因此可以解决Internet路由的问题，使数据包传送的延迟时间减短，增加网络传输的速度，更适合多媒体讯息的传送。因此，MPLS最大技术特色为可以指定数据包传送的先后顺序。MPLS使用标记交换（Label
...
## 发展历史
  ...
  - 1996年，[Ipsilon公司提出的一种专门在ATM网上传送IP分组的技术](https://zh.wikipedia.org/wiki/Ipsilon公司 "wikilink")：IP

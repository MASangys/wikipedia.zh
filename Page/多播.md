...
链路本地多播是指将IP多播包发往处于同一物理的或虚拟的[数据链路层的若干主机组](../Page/数据链路层.md "wikilink")。由于这种多播不需要复杂的路由，因此其应用要广泛得多。在[IPv6中](../Page/IPv6.md "wikilink")，它被用于地址解析，而在[零配置网络中](https://zh.wikipedia.org/wiki/零配置 "wikilink")，它取代了低效的广播协议，完成服务发现、名字解析和地址冲突解析的功能。
...
多播[安全性是一个重要的问题](https://zh.wikipedia.org/wiki/安全性 "wikilink")。标准的、实用的通信安全解决方案一般采用的是对称[加密](https://zh.wikipedia.org/wiki/加密 "wikilink")。但是将其应用于IP多播流量可能会使任何一个接收方都拥有冒充发送方的能力。这显然是令人无法接受的。IETF的[MSEC](https://web.archive.org/web/20050729093021/http://www.ietf.org/html.charters/msec-charter.html)工作组正在开发用以解决这一问题的安全协议，这些协议大多都是在[IPsec协议集的体系框架内开发的](../Page/IPsec.md "wikilink")。

[IPsec不能被用于多播方案](../Page/IPsec.md "wikilink")，这是因为IPsec安全关联是被绑定到两个而非多个主机的。IETF提出了一个新的协议——[TESLA](https://zh.wikipedia.org/wiki/TESLA "wikilink")，就多播安全性而言，这个协议是灵活且令人信服的。

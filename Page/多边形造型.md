...
网格造型所用的基本对象是三维空间中的[顶点](https://zh.wikipedia.org/wiki/顶点 "wikilink")。将两个顶点连接起来的直线称为[边](../Page/边.md "wikilink")。三个顶点经三条边连接起来成为[三角形](../Page/三角形.md "wikilink")，三角形是[欧几里得空间](../Page/欧几里得空间.md "wikilink")空间中最简单的多边形。多个三角形可以组成更加复杂的多边形，或者生成多于三个顶点的单个物体。四边形和三角形是多边形造型中最常用的形状。通过共同的顶点连接在一起的一组多边形通常当作一个**元素**。组成元素的每一个多边形就是一个**表面**。

在[欧几里得几何](../Page/欧几里得几何.md "wikilink")中，任何三点都可以确定一个[平面](../Page/平面.md "wikilink")。因此，三角形总是位于一个平面，但是对于更加复杂的多边形来说可能并非如此。三角形的平面特性使得[曲面法线的确定变得很简单](https://zh.wikipedia.org/wiki/曲面法线 "wikilink")，曲面法线是垂直于三角形所有边的一个三维向量。曲面法线对于[光线跟踪中确定光线传输非常有用](https://zh.wikipedia.org/wiki/光线跟踪 "wikilink")，并且在流行的 [Phong shading](https://zh.wikipedia.org/wiki/Phong_shading "wikilink") 模型中它也是一个关键成分。有一些渲染系统使用[顶点法线取代曲面法线来获得效果更好的光照系统](https://zh.wikipedia.org/wiki/顶点法线 "wikilink")，这样做的代价就是计算量的增加。注意每个三角形都有两个方向相反的曲面法线。在许多系统中，只有一个法线是有效的，根据需求可以定义成可见或者不可见；另外一条法线称为 **背面**。

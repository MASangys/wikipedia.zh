**多項式時間**（）在[計算複雜度理論中](https://zh.wikipedia.org/wiki/計算複雜度理論 "wikilink")，指的是一個問題的計算時間\(m(n)\)不大於問題大小\(n\)的[多項式倍數](../Page/多項式.md "wikilink")。任何抽象機器都擁有一[複雜度類](https://zh.wikipedia.org/wiki/複雜度類 "wikilink")，此類包括可於此機器以多項式時間求解的問題。

以數學描述的話，則可說\(m(n) =\)
[O](https://zh.wikipedia.org/wiki/大O符號 "wikilink")\((n^k)\)，此\(k\)為一常數值（依問題而定）。

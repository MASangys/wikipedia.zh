[Image_processing_post_otsus_algorithm.jpg](https://zh.wikipedia.org/wiki/File:Image_processing_post_otsus_algorithm.jpg "fig:Image_processing_post_otsus_algorithm.jpg")
[Image_processing_pre_otsus_algorithm.jpg](https://zh.wikipedia.org/wiki/File:Image_processing_pre_otsus_algorithm.jpg "fig:Image_processing_pre_otsus_algorithm.jpg")
在[计算机视觉和](../Page/计算机视觉.md "wikilink")[图像处理中](../Page/图像处理.md "wikilink")，**大津二值化法**用来自动对基于聚类的图像进行[二值化](../Page/二值化.md "wikilink")，\[1\]
或者说，将一个灰度图像退化为二值图像。该算法以大津展之命名。算法假定该图像根据双模直方图（前景像素和背景像素）把包含两类像素，于是它要计算能将两类分开的最佳阈值，使得它们的类内[方差最小](../Page/方差.md "wikilink")；由于两两平方距离恒定，所以即它们的类间方差最大。\[2\]
因此，大津二值化法粗略的来说就是一维[Fisher判别分析的离散化模拟](../Page/線性判別分析.md "wikilink")。

原始方法的多级阈值扩展称为[多大津算法](../Page/多大津算法.md "wikilink")。\[3\]

## 方法

在大津算法中，我们穷举搜索能使类内方差最小的阈值，定义为两个类的方差的加权和：

\[\sigma^2_w(t)=\omega_1(t)\sigma^2_1(t)+\omega_2(t)\sigma^2_2(t)\] 权重
\(\omega_i\) 是被阈值 \(t\) 分开的两个类的概率，而 \(\sigma^2_ i\) 是这两个类的方差。
...
其中 \(x(i)\) 为第 \(i\) 个直方图面元中心的值。 同样的，你可以对大于 \(t\) 的面元求出右侧直方图的
\(\omega_2(t)\) 与 \(\mu_2\)。
...
5.  你可以计算两个最大值（和两个对应的）。\(\sigma^2_{b1}(t)\) 是最大值而 \(\sigma^2_{b2}(t)\)
    是更大的或相等的最大值
...
**total**为给定图像中的像素数。 **histogramCounts**为灰度图像不同灰度级（典型的8位图像）的256元直方图。
**level**为图像的阈值（double）。
...
该实现有一点点冗余的计算。但由于大津算法快速，这个实现版本是可以接受，并且易于理解的。因为在一些环境中，如果使用向量化的形式，可以更快地运算循环。大津二值化法使用架构最小的**堆－孩子标签**（heap-children
label）可以很容易地转变成多线程的方法。
...
## 外部链接
  ...
  - [Lecture notes on
    thresholding](http://homepages.inf.ed.ac.uk/rbf/CVonline/LOCAL_COPIES/MORSE/threshold.pdf)
    – covers the Otsu method.
  - [A plugin for
    ImageJ](http://rsb.info.nih.gov/ij/plugins/otsu-thresholding.html)
    using Otsu's method to do the threshold.
  - [A full explanation of Otsu's
    method](http://www.labbookpages.co.uk/software/imgProc/otsuThreshold.html)
    with a working example and Java implementation.
  - [Implementation of Otsu's
    method](http://www.itk.org/Doxygen/html/classitk_1_1OtsuThresholdImageFilter.html)
    in
    [ITK](../Page/Insight_Segmentation_and_Registration_Toolkit.md "wikilink")
  - [Otsu Thresholding in
    C\#](http://www.codeproject.com/KB/graphics/OtsuSharp.aspx) A
    straightforward C\# implementation with explanation.
  - [Otsu's method using
    MATLAB](http://www.mathworks.com/discovery/image-thresholding.html)

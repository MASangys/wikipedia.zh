...
2015年3月26日至31日的對[GitHub](../Page/GitHub.md "wikilink")發起的[旁觀者攻擊](https://zh.wikipedia.org/wiki/旁觀者攻擊 "wikilink")（），被认为是大炮的第一次重要应用。
...
[Traceroute-GFW.png](https://zh.wikipedia.org/wiki/File:Traceroute-GFW.png "fig:Traceroute-GFW.png")追踪[TTL来证明中国政府对GitHub发动攻击](https://zh.wikipedia.org/wiki/存活時間 "wikilink")\]\] 为了防止数据在网络中无限循环，名为[存活时间](https://zh.wikipedia.org/wiki/存活时间 "wikilink")（Time to live，TTL）的机制限定了数据包的寿命。从数据包发出开始，每经过一个路由，TTL就减去1，当TTL=0时数据包将会被丢弃。大多数系统发送数据包时都是从TTL=64开始，如果该数据包抵达时TTL=24，那么计算机和发送者之间经过了40跳（64-24=40）。在对GitHub发动的DDoS攻击中，攻击者劫持了[百度](../Page/百度.md "wikilink")的JS文件。如图所示，百度[服务器](../Page/服务器.md "wikilink")所发送数据包的TTL=64，第一次抵达用户[浏览器时TTL](https://zh.wikipedia.org/wiki/浏览器 "wikilink")=42，经过了22跳，用户发回的请求包的TTL值也是64，但接下来的响应包的TTL值却突然变成了227，显然有中间人设备注入了伪造包。一位研究人员用定制[Traceroute](../Page/Traceroute.md "wikilink")工具测试发现，注入设备位于第11跳和第12跳之间，通过查询第12跳设备的IP地址，作者发现它位于中国联通骨干网，因此得出了中国政府与此有关的结论。 \[22\]

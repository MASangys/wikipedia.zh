...
在大多数现代计算机编程语言中，[程序可以被分成如](https://zh.wikipedia.org/wiki/程序 "wikilink")[子程序](../Page/子程序.md "wikilink")的更小的组件，这些组件可以通过许多物理源[文件分发](https://zh.wikipedia.org/wiki/文件 "wikilink")，这些源文件被单独[编译](https://zh.wikipedia.org/wiki/编译 "wikilink")。当一个子程序在定义的位置以外的地方被使用时，就需要引入[前置声明和](https://zh.wikipedia.org/wiki/前置声明 "wikilink")[函数原型的概念](https://zh.wikipedia.org/wiki/函数原型 "wikilink")。例如，一个函数在一个源文件中有如下定义：

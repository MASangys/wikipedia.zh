...
[Andrei
Alexandrescu在](https://zh.wikipedia.org/wiki/Andrei_Alexandrescu "wikilink")\[5\]中称之为**静态多态**（static
polymorphism）。
...
这种技术获得了类似于[虚函数的效果](../Page/虚函数.md "wikilink")，并避免了[动态多态的代价](https://zh.wikipedia.org/wiki/动态多态 "wikilink")。也有人把CRTP称为“模拟的动态绑定”。\[6\]

这种模式广泛用于Windows
[ATL与](https://zh.wikipedia.org/wiki/Active_Template_Library "wikilink")[WTL库](https://zh.wikipedia.org/wiki/WTL "wikilink")，以及[Boost](../Page/Boost_C++_Libraries.md "wikilink").Iterator，Boost.Python或者Boost.Serialization等库中。
...
在[C++标准库头文件](https://zh.wikipedia.org/wiki/C++标准程式库 "wikilink")[<memory>](../Page/Memory_\(C++标准库\).md "wikilink")中，`std::shared_ptr`类封装了可被共享使用的指针或资源。一个被共享的对象不能直接把自身的原始指针（raw
...
Framework中](https://zh.wikipedia.org/wiki/.NET_Framework "wikilink")，常见把一个类作为[泛型超类或interface的类型参数](https://zh.wikipedia.org/wiki/泛型编程 "wikilink")。例如，下述Java类实现了标准库中的泛型界面`Comparable`:
...
这保证了[编译时不会比较](https://zh.wikipedia.org/wiki/编译时 "wikilink")`Item`与其它不是`Item`的对象如`String`。与C++不同，Java类不能使用不同的类型参数扩展一个超类两次，也不能用不同的类型参数实现同一个interface两次。这是由于Java实用了[类型擦除处理泛型](../Page/类型擦除.md "wikilink")。例如，上例的class

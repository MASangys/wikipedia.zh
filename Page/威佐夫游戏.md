...
[马丁·加德纳认为威佐夫游戏在中国称为](../Page/马丁·加德纳.md "wikilink")“捡石子”\[1\]。[荷兰数学家](../Page/荷兰.md "wikilink")于1907年发表过一篇论文，从数学角度分析了该游戏。
[Wythoff's_Nim.svg](https://zh.wikipedia.org/wiki/File:Wythoff's_Nim.svg "fig:Wythoff's_Nim.svg")
...
对于任一状态(n,m)，可用以下法则[递归地判断此状态是先手必胜还是后手必胜](../Page/递归.md "wikilink")：

1.  (0,0)为后手必胜状态。
2.  若一个状态的后续中存在后手必胜状态，则该状态为先手必胜。
3.  若一个状态的全部后续均为先手必胜，则该状态为后手必胜。

例如，由上述第一条、第二条可推出对所有的正整数m，(0,m)和(m,m)均为先手必胜状态。而(1,2)是后手必胜状态，因为它的后续(0,1)、(0,2)和(1,1)均为先手必胜状态。前几个后手必胜状态为(0,
0)、(1, 2)、(3, 5)、(4, 7)、(6,10)和(8, 13)。

## 后手必胜状态的通式

威佐夫发现后手必胜状态与[黄金分割率有关](../Page/黄金分割率.md "wikilink")。具体而言，若k为任意自然数且
...
1.  [Wythoff's game at
    Cut-the-knot](http://www.cut-the-knot.org/pythagoras/withoff.shtml),
    quoting [Martin Gardner](../Page/马丁·加德纳.md "wikilink")'s book

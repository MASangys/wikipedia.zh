...
假設*X*為一*n* ×
*p*矩陣，其各行（row）來自同一均值向量為\(\mathbf 0\)的\(p\)維[多變量常態分佈且彼此](https://zh.wikipedia.org/wiki/多變量常態分佈 "wikilink")[獨立](https://zh.wikipedia.org/wiki/統計獨立性 "wikilink")。
...
的[機率分佈](https://zh.wikipedia.org/wiki/機率分佈 "wikilink")。
...
統計學家[George
Seber曾論證威沙特分佈並非多變量卡方分佈](https://zh.wikipedia.org/wiki/:en:George_Seber "wikilink")，這是因為非對角元素的邊際分佈並非卡方分佈，Seber傾向於將*某某多變量*分佈此一遣詞用於所有元素的邊際分佈皆相同的情形。\[6\]

## 多變量常態分佈的估計

由於威沙特分佈可視為一多變量常態分佈其[共變異矩陣的](../Page/协方差矩阵.md "wikilink")[最大概似估計量](https://zh.wikipedia.org/wiki/最大概似估計量 "wikilink")（MLE）的的分佈，其衍自MLE的計算可為令人驚喜地簡約而優雅。\[7\]
基於[頻譜理論](https://zh.wikipedia.org/wiki/頻譜理論 "wikilink")，可將一純量視為一\(1\times 1\)矩陣的跡（trace）。請參考[共變異矩陣的估計](https://zh.wikipedia.org/wiki/共變異矩陣的估計 "wikilink")。
...
1.  生成一隨機\(p\times p\)下[三角矩陣](https://zh.wikipedia.org/wiki/三角矩陣 "wikilink")
    \({\textbf A}\)使得：
...
2.  計算\({\textbf V} = {\textbf L}{\textbf L}^T\)的[Cholesky分解](https://zh.wikipedia.org/wiki/Cholesky分解 "wikilink")。

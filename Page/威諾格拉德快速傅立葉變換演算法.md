...
當把[DFT的公式](../Page/DFT.md "wikilink")：\(y_j=\sum_{k=0}^{n-1} x_k e^{-j\begin{matrix} \frac{2\pi}{n} \end{matrix} ik} \qquad  j=0,1,\cdots,n-1.\)
...
故要將此矩陣\(\begin{bmatrix}  y_1-x_0 \\ y_2-x_0 \\ y_3-x_0 \\ y_4-x_0 \end{bmatrix}= \begin{bmatrix} w & w^2 & w^3 & w^4 \\ w^2  & w^4  & w  & w^3  \\ w^3 & w & w^4 & w^2 \\ w^4 & w^3 & w^2 & w\end{bmatrix}\begin{bmatrix}  x_1 \\ x_2 \\ x_3 \\ x_4 \end{bmatrix}\quad\)
...
如此第一行與第一列都跟所求得的順序：1,2,4,3一樣，此為[circular
correlation的形式](../Page/circular_correlation.md "wikilink")。

Step
3：為了要符合[迴旋摺積的定義](../Page/迴旋摺積.md "wikilink")(矩陣的對角線的項數相同)，故必須再將第二列與第四列交換，第二行與第四行交換，矩陣如下：
...
## 參考資料
  ...
  - S.Winograd, "On computing the discrete Fourier transform,"
    Mathematics of Computation, vol.32,no.141,pp.179-199,Jan.1978.

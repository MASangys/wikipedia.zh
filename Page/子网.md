**子网**（）这个词有两个相关的含义：其中一个较老的、一般的含义是[互联网中的一个物理网络](../Page/互联网.md "wikilink")；在[因特网协议](https://zh.wikipedia.org/wiki/因特网协议 "wikilink")（Internet
Protocol，IP）中，子网指的是从[分类网络中划分出来的一部分](../Page/分类网络.md "wikilink")。本文敘述其第二种含义。

具有相同的前半部分地址的一组IP地址，可以利用地址的前半部分划分组。在一个IP网络中划分子网使我们能将一个单一的大型网络——至少（逻辑上）看上去如此——分成若干个较小的网络。在最初引入这个概念的时候，[IPv4还未引入](../Page/IPv4.md "wikilink")[分類网络这个概念](../Page/分类网络.md "wikilink")。而引入划分子网这个概念的目的是为了允许一个单一的站点能拥有多个[局域网](../Page/局域网.md "wikilink")。即使在引入了分类网络号之后，这个概念仍然有它的用处，因为它减少了因特网[路由表中的表项数量](../Page/路由表.md "wikilink")（通过隐藏一个站点内部所有独立子网的相关信息）。此外它还带来了一个好处，那就是减少了网络开销，因为它将接收IP广播的区域划分成了若干部分。

## 网络掩码

「网络掩码」又叫「子-{}-网掩-{}-码」、「地址掩码」、「子網-{}-路遮-{}-罩」（subnet
mask），它是一种用来指明一个[IP地址的哪些位标识的是主机所在的网络地址以及哪些位标识的是主机地址的](../Page/IP地址.md "wikilink")[位掩码](https://zh.wikipedia.org/wiki/掩码 "wikilink")。
...
[IPv4地址被分为三部分](../Page/IPv4.md "wikilink")：网络部分（network）、子网部分（subnetwork，现在常被认为是网络部分的一部分——尽管它本来是“rest”部分的一部分）和主机（host）部分。共有三[类IP地址](../Page/分类网络.md "wikilink")，它们分别指定了各部分占多少位。
...
子网掩码并不局限于整数个八位位组的情况。例如，255.254.0.0（或“/15”）同样是一个有效的掩码。如果将它应用到A类地址上，就会产生128个间隔为2的子网（例如1.2.0.1～1.3.255.254，1.4.0.1～1.5.255.254等等）。這情況我們可以說是
Class A 的 network 向 host 借了 7個bits，也可以說是 host 向 Class B 的 network 借了
1個bits，後者又稱為。
...
## 外部链接
  ...
  - [IP Tutorial - Subnet Masks and
    Subnetting](http://compnetworking.about.com/library/weekly/aa043000b.htm)
  - [My IP
    Tool](https://web.archive.org/web/20051201064104/http://www.iptool.us/extensions/iptool.php)
    — Detect your subnet IP and external IP
  - [RFC 950 Internet Standard Subnetting
    Procedure](http://www.ietf.org/rfc/rfc950)
  - [RFC 1812 New Internet Subnetting
    Procedure](http://www.ietf.org/rfc/rfc1812)

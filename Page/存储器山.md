**存储器山**是一种综合研究[存储器层次结构的工具](https://zh.wikipedia.org/wiki/記憶體階層 "wikilink")。它反映了存储器层次结构中不同层次的[带宽](https://zh.wikipedia.org/wiki/:en:Bandwidth_\(computing\) "wikilink")。也反映了具有不同的[时间局部性与](https://zh.wikipedia.org/wiki/:en:Locality_of_reference "wikilink")[空间局部性](https://zh.wikipedia.org/wiki/:en:Locality_of_reference "wikilink")\[1\]的[程序的性能](https://zh.wikipedia.org/wiki/程序 "wikilink")。通过分析存储器山的数据，还可以看出存储器系统的部分[硬件参数](../Page/硬件.md "wikilink")。

T.
Stricker于1997年在其论文\[2\]中介绍了存储器山的思想，利用它对存储器系统进行全面描述，并在后来的工作中提出了术语“存储器山”\[3\]。[卡耐基梅隆大学教授](https://zh.wikipedia.org/wiki/卡耐基梅隆大学 "wikilink")[Randal
Bryant的著作](https://zh.wikipedia.org/wiki/:en:Randal_Bryant "wikilink")[《深入理解计算机系统》](http://csapp.cs.cmu.edu/)（Computer
Systems: A programmer's Perspective, [Randal
Bryant](https://zh.wikipedia.org/wiki/:en:Randal_Bryant "wikilink"),
David O'Hallaron）一书第6.6.1节亦提出了存储器山的概念，并进行了详细分析\[4\]。

理论上，每台[计算机都有一个唯一的存储器山](https://zh.wikipedia.org/wiki/计算机 "wikilink")\[5\]。通过运行一段测试[程序](https://zh.wikipedia.org/wiki/程序 "wikilink")，可以得到它的存储器山。了解存储器山，对[应用程序的优化能起到指导作用](https://zh.wikipedia.org/wiki/:en:Optimization_\(computer_science\) "wikilink")。下图展示了一个基于[Intel](https://zh.wikipedia.org/wiki/Intel "wikilink")
[Core 2](https://zh.wikipedia.org/wiki/Core_2 "wikilink")
Duo[处理器的存储器山](https://zh.wikipedia.org/wiki/CPU "wikilink")。
...
**[时间局部性（Temporal
locality）](https://zh.wikipedia.org/wiki/:en:Locality_of_reference#Locality_of_reference "wikilink")：**如果被访问过的[存储器地址在较短时间内被再次访问](https://zh.wikipedia.org/wiki/:en:Memory_address "wikilink")，则程序具有良好的时间局部性。在一定的时间内，重复访问同一个地址的次数越多，时间局部性越好。换句话说，对同一个地址的两次访问间隔时间越短，时间局部性越好。

**[空间局部性（Spatial
locality）](https://zh.wikipedia.org/wiki/:en:Locality_of_reference#Locality_of_reference "wikilink")：**如果程序访问某个存储器地址后，又在较短时间内访问临近的存储器地址，则程序具有良好的空间局部性。两次访问的地址越接近，空间局部性越好。
...
从[程序员的角度来看](../Page/程序员.md "wikilink")，要尽量编写具有良好的局部性的程序，以[适应层次结构](https://zh.wikipedia.org/wiki/:en:Locality_of_reference#Use_of_spatial_and_temporal_locality:_hierarchical_memory "wikilink")，提高程序运行速率。
...
1.  [Peter J.
    Denning](https://zh.wikipedia.org/wiki/Peter_J._Denning "wikilink")
    *[The locality

**-{zh-cn:存储过程;
zh-tw:預存程序;}-**，又稱**儲存程序**（），是在[資料庫儲存複雜程序](https://zh.wikipedia.org/wiki/資料庫 "wikilink")，以便外部程式呼叫的[資料庫物件](https://zh.wikipedia.org/wiki/資料庫物件 "wikilink")，可以視為資料庫的一種[函數或](https://zh.wikipedia.org/wiki/函數 "wikilink")[子程序](../Page/子程序.md "wikilink")。
...
預存程序具有下列的好處：
  ...
  - 預存程序可以回傳值，並可以接受參數。
  - 預存程序無法使用 SELECT
    指令執行，因為它是子程序，與[檢視表](https://zh.wikipedia.org/wiki/視圖 "wikilink")、[資料表或](https://zh.wikipedia.org/wiki/資料庫表 "wikilink")[使用者定義函數不同](https://zh.wikipedia.org/wiki/使用者定義函數 "wikilink")。

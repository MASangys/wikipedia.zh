...
在[类UNIX](https://zh.wikipedia.org/wiki/类UNIX "wikilink")[操作系统中](../Page/操作系统.md "wikilink")，为避免孤儿进程退出时无法释放所占用的资源而僵死，任何孤儿进程产生时都会立即为系统进程[init或](https://zh.wikipedia.org/wiki/init "wikilink")[systemd自动接收为子进程](https://zh.wikipedia.org/wiki/systemd "wikilink")，这一过程也被称为“收养”（）\[1\]。在此需注意，虽然事实上该进程已有`init`作为其父进程，但由于创建该进程的进程已不存在，所以仍应称之为“孤儿进程”。

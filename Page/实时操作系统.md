...
实时操作系统与一般的[操作系统相比](../Page/操作系统.md "wikilink")，最大的特色就是「[实时性](https://zh.wikipedia.org/wiki/實時計算 "wikilink")」\[1\]，如果有一个任务需要执行，实时操作系统会马上（在较短时间内）执行该任务，不会有较长的延时。这种特性保证了各个任务的及时执行。

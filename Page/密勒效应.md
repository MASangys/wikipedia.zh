**密勒效应**（）是在[电子学中](../Page/电子学.md "wikilink")，反相放大电路中，输入与输出之间的分布[电容或寄生电容由于](https://zh.wikipedia.org/wiki/电容 "wikilink")[放大器的放大作用](../Page/放大器.md "wikilink")，其等效到输入端的电容值会扩大1+K倍，其中K是该级放大电路电压放大倍数。虽然一般密勒效应指的是电容的放大，但是任何输入与其它高放大节之间的阻抗也能够通过密勒效应改变放大器的输入阻抗。
...
米勒效应是以[约翰·米尔顿·密勒命名的](https://zh.wikipedia.org/wiki/约翰·米尔顿·密勒 "wikilink")。1919年或1920年密勒在研究[真空管三极管时发现了这个效应](../Page/真空管.md "wikilink")，但是这个效应也适用于现代的半导体[三极管](https://zh.wikipedia.org/wiki/三极管 "wikilink")。
...
使用[共源共栅或者使用级联放大器来取代](https://zh.wikipedia.org/wiki/共源共栅 "wikilink")[共射电极可以减轻密勒效应](https://zh.wikipedia.org/wiki/共射电极 "wikilink")。在反馈放大器中密勒效应甚至有优点，因为否则的话需要用来稳定住放大器的电容器太大了，无法包含在电路中，一般在[集成电路中电容需要的面积最大](../Page/集成电路.md "wikilink")，因此大的电容往往很麻烦。

## 对频率响应的影响

[Miller_before_transform.PNG](https://zh.wikipedia.org/wiki/File:Miller_before_transform.PNG "fig:Miller_before_transform.PNG")
[Miller_after_transform.PNG](https://zh.wikipedia.org/wiki/File:Miller_after_transform.PNG "fig:Miller_after_transform.PNG")
图2（图示可能有错，应该为负反馈电路）显示了一个放大器电路，图1中联系输出和输入的阻抗在这里是电容\(C_C\)。一个[戴维寧电源](https://zh.wikipedia.org/wiki/戴维寧定理 "wikilink")\(V_A\)通过一个戴维寧电阻\(R_A\)驱动这个电路。在输出端一个\(RC\)电路作为负载（这个负载在这里的讨论中不重要，它仅仅被用来完整整个电路）。图中的电容向输出电路提供\(\  j\omega C_C ( v_i - v_O )\)的电流。
...
在频率足够高，\(\omega C_M R_A>1\)的情况下输出电压下降。因此整个电路是一个[低通滤波器](../Page/低通滤波器.md "wikilink")。在模拟放大器中密勒效应对电路的频率反响有非常大的影响。在这个例子中频率\(\omega_{3dB}\)在\(\omega_{3dB} C_MR_A=1\)时标志着低频反响的终点，局限着放大器的[带宽或者](../Page/带宽.md "wikilink")[截止頻率](../Page/截止頻率.md "wikilink")。
...
## 参考文献
  ...
  - John M. Miller, [Dependence of the input impedance of a
    three-electrode vacuum tube upon the load in the plate
    circuit](http://web.mit.edu/klund/www/papers/jmiller.pdf),
    Scientific Papers of the Bureau of Standards, 15(351):367-385, 1920.

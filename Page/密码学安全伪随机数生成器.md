...
由可忽略速度增长的函数是可忽略函数。可忽略函数的更准确的定义如下：当输入趋近于无穷大时，一个函数的增长速度小于任何多项式函数的反函数，则该函数是可忽略函数。换言之，\(\lim_{x \to \infty}{\mu(x) \over poly^{-1}(x)}=0\)。比如说，我们知道，在输入趋近于无穷大时，任何指数函数的增长速度大于任何多项式函数，因此，任何指数函数的反函数的增长速度一定小于任何多项式函数的反函数。指数函数的反函数是对数函数，因此，所有的对数函数都是可忽略函数。

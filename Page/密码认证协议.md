...
[点对点协议](../Page/点对点协议.md "wikilink")（PPP）使用的 PAP 允许用户访问服务器资源之前使用 PAP
进行验证。几乎所有的[网络操作系统远程服务器都支持PAP](https://zh.wikipedia.org/wiki/网络操作系统 "wikilink")。

PAP在网络上传送未加密的[ASCII密码](../Page/ASCII.md "wikilink")，因此被认为是不安全的。当远程服务器不支持诸如[CHAP或](https://zh.wikipedia.org/wiki/CHAP "wikilink")[EAP](../Page/扩展认证协议.md "wikilink")（后者实际上是一种[框架](https://zh.wikipedia.org/wiki/软件框架 "wikilink")）这样的更强的认证协议时，PAP被作为最后的对策来使用。

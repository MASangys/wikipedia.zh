...
}} 在[概率论与](https://zh.wikipedia.org/wiki/概率论 "wikilink")[统计学](../Page/统计学.md "wikilink")中，**对数正态分布**是[对数为](https://zh.wikipedia.org/wiki/对数 "wikilink")[正态分布](../Page/正态分布.md "wikilink")的任意[随机变量](../Page/随机变量.md "wikilink")的[概率分布](../Page/概率分布.md "wikilink"), 即一个随机变量的对数服从正态分布。如果 \(Y\) 是正态分布的随机变量，则 \(\exp(Y)\)（[指数函数](../Page/指数函数.md "wikilink")）为对数正态分布；同样，如果 \(X\) 是对数正态分布，则 \(\ln X\)为正态分布。 如果一个变量可以看作是许多很小独立因子的[乘积](https://zh.wikipedia.org/wiki/乘积 "wikilink")，则这个变量可以看作是对数正态分布。一个典型的例子是股票投资的长期收益率，它可以看作是每天收益率的乘积。 对于 \(x>0\)，对数正态分布的[概率密度函数为](https://zh.wikipedia.org/wiki/概率密度函数 "wikilink")
...
## 参考文献
  ...
  - *[对数正态分布特性](http://www.rotman.utoronto.ca/%7Ehull/Technical%20Notes/TechnicalNote2.pdf)*, [John Hull](https://zh.wikipedia.org/wiki/John_Hull "wikilink"), in *Options, Futures, and Other Derivatives* 6E (2005). ISBN 0-13-149908-4

...
一些版本可以自動執行-{}-長計算，透過[Windows記事本或其他的](https://zh.wikipedia.org/wiki/Windows記事本 "wikilink")[文字處理軟體建立一個文字檔](https://zh.wikipedia.org/wiki/文字處理 "wikilink")：文字檔的第一行是一個數字，跟著第二行是另一個數字。將這個結果粘贴到計算機的文字盒，小算盤就可以計算文字檔所描述的計算流程。但是不是所有的小算盤版本都可以正確讀取文字檔，這需要依靠精確的文字格式。且除了標準的算符外（**+**, **-** , **/**, **\***），小算盤在自動長計算過程中，無法接受其他的指令。這使小算盤只適合計算大量累計的數值，無論數據的來源是數據檔或是空白表格。
...
自從Windows 3.0開始，「sqrt」按鍵只在**標準**模式下出現。當轉到**工程**模式後，平方根的按鍵便會消失，此問題並沒有解決方法。微軟建議將數字經過0.5次方，即可不需「sqrt」按鍵的運算（e.g. 144 ^ .5），\[4\]选中「inv」選項，跟著按下「x^2」鍵，結果亦會是數值的開方。这个问题在[Windows 7中已经被修复](https://zh.wikipedia.org/wiki/Windows_7 "wikilink")。
...
[Powercalc.PNG](https://zh.wikipedia.org/wiki/File:Powercalc.PNG "fig:Powercalc.PNG") [calcplusconversion.jpg](https://zh.wikipedia.org/wiki/File:calcplusconversion.jpg "fig:calcplusconversion.jpg") [Microsoft_Powercalc.PNG](https://zh.wikipedia.org/wiki/File:Microsoft_Powercalc.PNG "fig:Microsoft_Powercalc.PNG") 微軟曾經在下載中心，發佈過兩款計算機程式。它們的功能都比Windows內置的小算盤強大。

*微軟計算器增強版* - 除了"標準"和"科學"模式，它亦擁有'換算法'模式。'換算法'模式支援[單位和](https://zh.wikipedia.org/wiki/單位 "wikilink")[汇率](../Page/汇率.md "wikilink")換算。計算器增強版支援從[欧洲中央银行](../Page/欧洲中央银行.md "wikilink")的網頁中，直接更新外幣汇率的資訊。它亦提供一個直接的介面。

*[Power Calculator](https://zh.wikipedia.org/wiki/Powertoy_Calculator "wikilink")* - 它是其中的一款的Windows XP [PowerToy](https://zh.wikipedia.org/wiki/PowerToy "wikilink")。Powertoy Calculator支援圖形計算。雖然這個程式容許使用者調整計算機的精確度，但有一些小問題。例如函數\(\sin(\pi x)\)，程式生就生產出一個錯誤的圖形。它對一個擁有大[弧度的](https://zh.wikipedia.org/wiki/弧度 "wikilink")[正弦](../Page/正弦.md "wikilink")函數，產生出一個錯誤的數值，例如\(\sin(10000)=-3.3326e+4265\)。

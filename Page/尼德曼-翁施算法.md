> 本文内容由[尼德曼-翁施算法](https://zh.wikipedia.org/wiki/尼德曼-翁施算法)转换而来。


**尼德曼-翁施算法**（）是基于生物信息学的知识来匹配蛋白序列或者DNA序列的算法。这是将动态算法应用于生物序列的比较的最早期的几个实例之一。该算法是由 Saul B. Needlman和 Christian D. Wunsch 两位科学家于1970年发明的。本算法高效地解决了如何将一个庞大的数学问题分解为一系列小问题，并且从一系列小问题的解决方法重建大问题的解决方法的过程。该算法也被称为优化匹配算法和整体序列比较法。时至今日尼德曼-翁施算法仍然被广泛应用于优化整体序列比较中。
...
Needleman和Wunsch的描述他们的算法为明确的情况下，当对准被匹配和不匹配仅仅惩罚，并差距没有惩罚（D =0）。从1970年最初发布提示递归。\(F_{ij} = \max_{h<i,k<j} \{ F_{h,j-1}+S(A_{i},B_{j}), F_{i-1,k}+S(A_i,B_j) \}\).
...
## 使用Needleman-Wunsch算法的工具
  ...
  - [EMBOSS Needle and EMBOSS Stretcher Global Alignment Tools](http://www.ebi.ac.uk/Tools/psa)
  - [Needleman-Wunsch alignment for two nucleotide sequences](https://blast.ncbi.nlm.nih.gov/Blast.cgi?PAGE_TYPE=BlastSearch&PROG_DEF=blastn&BLAST_PROG_DEF=blastn&BLAST_SPEC=GlobalAln&LINK_LOC=BlastHomeLink)
  - [MathWorks - Globally align two sequences using Needleman-Wunsch algorithm](http://www.mathworks.com.au/help/bioinfo/ref/nwalign.html)
...
## 外部链接
  ...
  - [NW-align: A protein sequence-to-sequence alignment program by Needleman-Wunsch algorithm (online server & source code)](http://zhanglab.ccmb.med.umich.edu/NW-align)
  - [Needleman-Wunsch Algorithm as Haskell Code](http://chneukirchen.org/blog/archive/2006/03/dynamic-programming-in-haskell.html)
  - [A live Javascript-based demo of Needleman-Wunsch](http://ds9a.nl/nwunsch)
  - [An interactive Javascript-based visual explanation of Needleman-Wunsch Algorithm](http://alfehrest.org/sub/nwa/index.html)
  - [B.A.B.A.](http://baba.sourceforge.net/) — an applet (with source) which visually explains the algorithm.
  - [A clear explanation of NW and its applications to sequence alignment](https://web.archive.org/web/20140729133238/http://www.science.marshall.edu/murraye/Clearer%20Matrix%20slide%20show.pdf)
  - [Sequence Alignment Techniques at Technology Blog](http://technology66.blogspot.com/2008/08/sequence-alignment-techniques.html)
  - [Biostrings](http://svitsrv25.epfl.ch/R-doc/library/Biostrings/html/00Index.html) R package implementing Needleman–Wunsch algorithm among others
  - [Parallel Needleman-Wunsch Algorithm for Grid](https://web.archive.org/web/20120113063118/http://www.cloudbus.org/~alchemi/files/Parallel%20Needleman%20Algo.pdf) Implementation by Tahir Naveed, Imitaz Saeed Siddiqui and Shaftab Ahmed - Bahria University
  - [Needleman-Wunsch Algorithm as Haxe Code](https://gist.github.com/jonasmalacofilho/5226596)

[Category:生物信息学算法](https://zh.wikipedia.org/wiki/Category:生物信息学算法 "wikilink") [Category:计算系统发生学](https://zh.wikipedia.org/wiki/Category:计算系统发生学 "wikilink") [Category:动态规划](https://zh.wikipedia.org/wiki/Category:动态规划 "wikilink") [Category:序列排比算法](https://zh.wikipedia.org/wiki/Category:序列排比算法 "wikilink")

1.
2.
3.
4.  Dieny R., Thevenon J., Martinez-del-Rincon J., Nebel J.-C. (2011) "Bioinformatics inspired algorithm for stereo correspondence".
5.  Madeo S., Pelliccia R., Salvadori C., Martinez-del-Rincon J., Nebel J.-C. (2014) "An optimized stereo vision implementation for embedded systems: application to RGB and Infra-Red images".
6.  Martinez-del-Rincon J., Thevenon J., Dieny R., Nebel J.-C. (2012) "Dense Pixel Matching Between Unrectified and Distorted Images Using Dynamic Programming".
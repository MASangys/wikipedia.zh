...
布尔逻辑还在[电子工程](../Page/电子工程.md "wikilink")中的电路设计中使用；这里的0和1表示在[数字电路](../Page/数字电路.md "wikilink")中某一个[位的不同状态](https://zh.wikipedia.org/wiki/位 "wikilink")，典型的是高和低[电压](https://zh.wikipedia.org/wiki/电压 "wikilink")。使用包含变量的表达式描述电路，并且对于这些变量的所有的值两个这种表达式是等价的，当且仅当对应的电路有相同的输入-输入行为。进一步的说，每种可能的输入-输出行为都被建模为适合的布尔表达式。

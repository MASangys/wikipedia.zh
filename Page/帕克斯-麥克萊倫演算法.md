...
此演算法的主要精神，在於利用疊代的方式最小化濾波器在[通帶](https://zh.wikipedia.org/wiki/通帶 "wikilink")(pass band)和[止帶](https://zh.wikipedia.org/wiki/止帶 "wikilink")(stop band)的最大誤差，因此有時也稱為最小化最大誤差演算法(Mini-max filter design)。由於帕克斯-麥克萊倫演算法也屬於Remez-exchange algorithm為了設計有限脈衝響應濾波器而產生的一種變形，因此也有人以Remez-exchange algorithm代稱。

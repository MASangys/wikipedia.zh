...
成立的任何实数\(\varphi\)。直观上来说，假设非零复数\(z\)在复平面\(O_{xy}\)中对应的向量是\(\overrightarrow{OP}\)（右图蓝色向量），那么它的幅角是所有能够描述正实数轴到\(\overrightarrow{OP}\)的转角的有向角。其中有向角的正方向规定为逆时针方向。图中可以看出，相差\(2 \pi\)的倍数的角都可以是幅角。这个性质也可以从[三角函数](../Page/三角函数.md "wikilink")\(\cos\)和\(\sin\)是以\(2 \pi\)为周期的[周期函数](../Page/周期函数.md "wikilink")中推导出来。

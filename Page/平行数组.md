**平行数组**是[程序设计](../Page/程序设计.md "wikilink")采用多个数组隐式表示一个以[记录](../Page/记录.md "wikilink")（record）为元素的数组。多个数组在同一下标的元素隐式对应于记录的各个域。

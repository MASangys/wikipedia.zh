...
在[圖論中](https://zh.wikipedia.org/wiki/圖論 "wikilink")，**平面圖**是可以画在[平面](../Page/平面.md "wikilink")上并且使得不同的邊可以互不交疊的[圖](https://zh.wikipedia.org/wiki/圖 "wikilink")\[1\]。而如果一个图无论怎样都无法画在平面上，并使得不同的边互不交叠，那么这样的图不是平面图，或者称为非平面图。完全图*K*<sub>5</sub>和完全[二分图](../Page/二分图.md "wikilink")*K*<sub>3,3</sub>（[湯瑪森圖](https://zh.wikipedia.org/wiki/湯瑪森圖 "wikilink")）是最“小”的非平面图。
...
說一個圖是平面圖，其實等價於說存在一個從空間到[平面](../Page/平面.md "wikilink")的[連續的](https://zh.wikipedia.org/wiki/連續 "wikilink")[單射](https://zh.wikipedia.org/wiki/單射 "wikilink")，能夠將這個圖投射到平面上。從這樣的角度看來，平面圖是空間的一部份在二維平面上的一個[嵌入](https://zh.wikipedia.org/wiki/嵌入 "wikilink")。然而，圖在二維平面的嵌入和在球面的嵌入是等價的：
...
依此準則，空間中的各種凸多面體對應的圖都是平面圖，因為只要在多面體的內部找一個合適的球，然後將多面體的頂點和邊都投影在這個球面上，就完成了相應的圖在球面的嵌入（由於多面體是凸的，投影得到的圖形的邊之間不會交疊）。比如說，所有的[正多面體](../Page/正多面體.md "wikilink")對應的圖都是平面圖\[4\]。
...
[缩略图](https://zh.wikipedia.org/wiki/File:Circle_packing_theorem_K5_minus_edge_example.svg "fig:缩略图") 一個**硬幣圖**的頂點是由許多圓的圓心組合而成，這些圓兩兩[外切或](https://zh.wikipedia.org/wiki/外切 "wikilink")[外離](https://zh.wikipedia.org/wiki/外離 "wikilink")，兩頂點之間連邊若且唯若以兩頂點唯圓心的兩圓外切。1936 年，首次證明：一個圖是平面圖若且唯若它是一個硬幣圖。
...
一個是可以被畫在平面上的[有向無環圖](https://zh.wikipedia.org/wiki/有向無環圖 "wikilink")，使得各個有向邊都是斜向上的。並不是所有平面有向無環圖都是向上平面圖，事實上，決定任意給定有向圖是否為向上平面圖的難度是 [NP完全](../Page/NP完全.md "wikilink")的。

## 平面圖的數量估計

若將頂點標號，有 n 個頂點的平面圖的數量的漸近式由\(g\cdot n^{-7/2}\cdot \gamma^n\cdot n!\)給出，其中常數\(\gamma\approx 27.22687\)\(\gamma\approx 27.22687\)\({\displaystyle \gamma \approx 27.22687}\)、\({\displaystyle g\approx 0.43\times 10^{-5}}\)\[7\]；若不將頂點標號，有 n 個頂點的平面圖的數量則界在 \(2.72^n\)和 \(30.06^n\)之間。

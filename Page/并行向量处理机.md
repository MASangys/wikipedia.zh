[Processor_board_cray-1_hg.jpg](https://zh.wikipedia.org/wiki/File:Processor_board_cray-1_hg.jpg "fig:Processor_board_cray-1_hg.jpg")
**向量处理器**，又称数组处理器，是一种实现了直接操作一维[数组](../Page/數組.md "wikilink")（向量）指令集的[中央处理器](../Page/中央处理器.md "wikilink")（CPU）。这与一次只能处理一个数据的[标量处理器正相反](../Page/标量处理器.md "wikilink")。向量处理器可以在特定工作环境中极大地提升性能，尤其是在数值模拟或者相似领域。向量处理器最早出现于20世纪70年代早期，并在70年代到90年代期间成为[超级计算机设计的主导方向](../Page/超级计算机.md "wikilink")，尤其是多个克雷（Cray）平台。由于90年代末标量处理器设计性能提升，而價格快速下降，基于向量处理器的超级计算机逐渐让出了主导地位。

现在，绝大多数商业化的CPU实现都能够提供某种形式的向量处理的指令，用来处理多个（向量化的）数据集，也就是所谓的SIMD（单一指令、多重数据）。常见的例子有
[VIS](../Page/VIS.md "wikilink"), [MMX](../Page/MMX.md "wikilink"),
[SSE](../Page/SSE.md "wikilink"),
[AltiVec](../Page/AltiVec.md "wikilink") 和
[AVX](../Page/AVX.md "wikilink")。向量处理技术也能在游戏主机硬件和图形加速硬件上看到。在2000年，[IBM](../Page/IBM.md "wikilink")，[东芝和](../Page/东芝.md "wikilink")[索尼合作开发了](../Page/索尼.md "wikilink")[Cell处理器](../Page/Cell_\(微處理器\).md "wikilink")，集成了一个标量处理器和八个向量处理器，应用在索尼的[PlayStation
3游戏机和其他一些产品中](../Page/PlayStation_3.md "wikilink")。

其他CPU设计还可能包括多重指令处理多重（向量化的）数据集的技术——也就是所谓的MIMD（多重指令、多重数据）——并实现了VLIM。此类设计通常用于特定应用场合，而不是面向通用计算机的市场化产品。在[富士通的](../Page/富士通.md "wikilink")
FR-V VLIW/vector 处理器中，组合使用了两种技术。

## 基本特点

平行向量处理机最大的特点是系统中的CPU是专门定制的[向量处理器](../Page/向量处理器.md "wikilink")（VP）。系统还提供共享存储器以及与VP相连的高速交叉开关。

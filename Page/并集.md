...
在[集合论](../Page/集合论.md "wikilink")和[数学](../Page/数学.md "wikilink")的其他分支中，一组[集合的](../Page/集合_\(数学\).md "wikilink")**并集**\[1\]（台湾叫做**聯-{}-集**、港澳叫做**-{併集}-**、大陆叫做**-{并集}-**）是这些集合的所有元素构成的集合，而不包含其他元素。
...
举例： 集合\(\{1, 2, 3\}\)和\(\{2, 3, 4\}\)的并集是\(\{1, 2, 3, 4\}\)。数\(9\) *不*属于[素数](../Page/素数.md "wikilink")集合\(\{2, 3, 5, 7, 11,\ldots\}\)和[偶数集合](https://zh.wikipedia.org/wiki/偶数 "wikilink")\(\{2, 4, 6, 8, 10,\ldots\}\)的并集，因为\(9\)既不是素数，也不是偶数。
...
同样，也可以写作"\(A_{1} \cup  A_{2} \cup  A_{3} \cup \ldots\)". （这是一个可数的集合的并集的例子，在[数学分析](../Page/数学分析.md "wikilink")中非常普遍；参见[\(\sigma\) -代数](https://zh.wikipedia.org/wiki/sigma代数 "wikilink")）。最后，要注意的是，当符号"\(\cup\)"放在其他符号*之前*，而不是*之间*的时候，要写的大一些。

> 本文内容由[广义表](https://zh.wikipedia.org/wiki/广义表)转换而来。


**广义表**（）是一种非线性的[数据结构](../Page/数据结构.md "wikilink")。但如果广义表的每个元素都是原子，它就变成了[线性表](https://zh.wikipedia.org/wiki/线性表 "wikilink")。广义表广泛地用于[人工智能](../Page/人工智能.md "wikilink")等领域的[LISP](../Page/LISP.md "wikilink")语言。

**广义表**一般记作 LS = (a1, a2, ···, an), n是它的长度，ai可以是单个元素(原子)，也可以是广义表(子表)，当广义表非空时，称第一个元素a1为LS的表头，称其余元素组成的表为LS的表尾。注意：表头是元素(可以是原子，也可以是广表)，表尾一定是广义表。\[1\]E=(a, E)是一个递归的表。D=(( ),(e),(a,(b,c,d)))是多层次的广义表，长度为3，深度为3。例：((a),a)的表头是(a)，表尾是(a)，((a))的表头是(a)，表尾是( )。

**广度优先搜索算法**（，縮寫為BFS），又譯作**寬度優先搜索**，或**橫向優先搜索**，是一種[圖形搜索演算法](../Page/圖形搜索演算法.md "wikilink")。簡單的說，BFS是從[根節點開始](../Page/根節點.md "wikilink")，沿着树的宽度遍历树的[节点](../Page/节点.md "wikilink")。如果所有节点均被访问，则算法中止。广度优先搜索的实现一般采用open-closed表。

## 作法

BFS是一種[盲目搜尋法](../Page/盲目搜尋法.md "wikilink")，目的是系統地展開並檢查[圖中的所有節點](../Page/圖.md "wikilink")，以找尋結果。換句話說，它並不考慮結果的可能位址，徹底地搜索整張圖，直到找到結果為止。BFS並不使用[經驗法則演算法](../Page/啟發式搜索.md "wikilink")。
...
zh-hant:連結串列;}-](../Page/連結串列.md "wikilink")），而被檢驗過的節點則被放置在被稱為
...
若所有邊的長度相等，廣度優先搜索演算法是最佳解——亦即它找到的第一個解，距離根節點的邊數目一定最少；但對一般的圖來說，BFS並不一定回傳最佳解。這是因為當圖形為加權圖（亦即各邊長度不同）時，BFS仍然回傳從根節點開始，經過邊數目最少的解；而這個解距離根節點的距離不一定最短。這個問題可以使用考慮各邊權值，BFS的改良演算法[成本一致搜尋法來解決](../Page/成本一致搜尋法.md "wikilink")。然而，若非加權圖形，則所有邊的長度相等，BFS就能找到最近的最佳解。
...
廣度優先搜索演算法能用來解決圖論中的許多問題，例如：
  ...
  - 尋找連接元件中的所有節點。
  - 尋找非加權圖中任兩點的最短路徑。
  - 測試一圖是否為[二分圖](../Page/二分圖.md "wikilink")。
  - [（Reverse）Cuthill–McKee演算法](../Page/Cuthill-McKee演算法.md "wikilink")
...
## 參見
  ...
  - [深度優先搜索](../Page/深度優先搜索.md "wikilink")

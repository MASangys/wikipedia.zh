...
第一個無限序數是 ω，第一個不能由 ω 有限引伸而成的序數是 ε<sub>0</sub>。對多數利用[超窮歸納法的證明](https://zh.wikipedia.org/wiki/超窮歸納法 "wikilink")，ε<sup>0</sub>已經足夠。要知道 \(\epsilon_0 = \omega^{\omega^{\omega^{\cdots}}}\) 且 \(\epsilon_0 = \omega^{\epsilon_0}\)。

**应用程序接口**（，缩写：；又称为**应用编程接口**）是[软件系统不同组成部分衔接的约定](../Page/软件.md "wikilink")。由於近年來软件的规模日益庞大，常常需要把复杂的系统划分成小的组成部分，编程接口的设计十分重要。程序设计的实践中，编程接口的设计首先要使软件系统的职责得到合理划分。良好的[接口设计可以降低系统各部分的相互依赖](https://zh.wikipedia.org/wiki/接口 "wikilink")，提高组成单元的[内聚性](https://zh.wikipedia.org/wiki/內聚力_\(計算機科學\) "wikilink")，降低组成单元间的[耦合程度](https://zh.wikipedia.org/wiki/耦合力_\(計算機科學\) "wikilink")，从而提高系统的可维护性和可扩展性。
...
应用程序接口是一組數量上千、極其複雜的函數和副程式，可讓程式設計師做很多工作，譬如「讀取文件」、「顯示選單」、「在視窗中顯示網頁」等等。[操作系统的API可用來分配](../Page/操作系统.md "wikilink")[記憶體或讀取](https://zh.wikipedia.org/wiki/記憶體 "wikilink")[檔案](../Page/檔案.md "wikilink")。許多系統應用程式藉由API接口來實現，像是圖形系統、資料庫、網路[Web服务](../Page/Web服务.md "wikilink")，甚至是線上遊戲。
...
### API与设备接口
  ...
  - [ASPI](https://zh.wikipedia.org/wiki/ASPI "wikilink") for
    [SCSI](https://zh.wikipedia.org/wiki/SCSI "wikilink") device
    interfacing

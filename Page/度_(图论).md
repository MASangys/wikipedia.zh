...
在[图论中](../Page/图论.md "wikilink")，一个[顶点在](https://zh.wikipedia.org/wiki/顶点 "wikilink")[图中的](../Page/图_\(数学\).md "wikilink")**度**
(**degree**)为与这个顶点相连接的[边的数目](../Page/边.md "wikilink")。在[多重图中](../Page/多重图.md "wikilink")，[自环被计数两次](../Page/自环.md "wikilink")。\[1\]
顶点 \(v\)
的度记作\(\deg(v)\)或\(\deg v\)。图*G*的**最大度**记作Δ(*G*)，**最小度**记作δ(*G*)，分别为图中所有顶点度的最大值和最小值。
在右边的[多重图中](../Page/多重图.md "wikilink")，最大度为5，最小度为0。
在[正则图中](https://zh.wikipedia.org/wiki/正则图 "wikilink")，所有度都是相同的，因为我们可以直接说该图的度是多少。
[完全图是正则图中的一种特殊情况](../Page/完全圖.md "wikilink")，其任意两个点均相连，若顶点数为p，则该图的度为p-1。
...
该公式表明，在任意无向图中，度为基数的顶点的个数为偶数，即为[握手定理](https://zh.wikipedia.org/wiki/握手定理 "wikilink")。该定理名称来自于一个热门的数学问题，即证明在一个团体中与他人握手奇数次的人的数量为偶数个。

## 度序列

[缩略图](https://zh.wikipedia.org/wiki/File:Conjugate-dessins.svg "fig:缩略图")
无向图的**度序列**是指包含其顶点度的非递增序列；右边的图其序列为(5,3,3,2,2,1,0)。\[2\]度序列是一个[图不变量](https://zh.wikipedia.org/wiki/图不变量 "wikilink")，所以[同构图具有相同的度序列](../Page/图同构.md "wikilink")。但是，度序列一般不能惟一地识别一个图；在某些情况下，异构图具有相同的程度序列。
...
[缩略图](https://zh.wikipedia.org/wiki/File:Depth-first-tree.png "fig:缩略图")
  ...
  - 度为1的顶点称为叶节点或端点，与该顶点相关联的边称为悬挂边。在右图中，{3,5}是一条悬挂边。这个术语在[数据结构与](../Page/数据结构.md "wikilink")[图论中对](../Page/图论.md "wikilink")[树的研究中很常见](../Page/树_\(数据结构\).md "wikilink")。
  - 有n个顶点的图中度为n-1的顶点称为[全连接顶点](https://zh.wikipedia.org/wiki/全连接顶点 "wikilink")。

## 全局属性

  - 如果图中每个顶点的度均为*k，*那么这个图被称作[k-正则图](https://zh.wikipedia.org/wiki/正則圖 "wikilink")，可称该图的度为*k*。类似地，在[二分图中](../Page/二分图.md "wikilink")，在同一侧顶点的度相同的图被称作[双正则图](https://zh.wikipedia.org/wiki/双正则图 "wikilink")。
...
## 参见
  ...
  - [二分图的](../Page/二分图.md "wikilink")[度序列](https://zh.wikipedia.org/wiki/二分图#度序列 "wikilink")

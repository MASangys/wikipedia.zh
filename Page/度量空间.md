...
## 度量空间的例子
  ...
  - 如果 \(G\) 是[无向连通图](../Page/图论.md "wikilink")，则 \(G\) 的顶点集合 \(V\) 可通过定义 \(d(x, y)\) 为连接 \(x\) 的 *\(y\)* 的最短路径的长度，变成度量空间。在[幾何群論裡](https://zh.wikipedia.org/wiki/幾何群論 "wikilink")，該度量可適用於一個群的[凱萊圖](../Page/凱萊圖.md "wikilink")上，並稱之為[字度量](../Page/字度量.md "wikilink")。

<!-- end list -->

  - 类似的，在 3D 中在[多面体](../Page/多面体.md "wikilink")的表面上的度量包括平常的度量，在表面上的距离；在多面体的边上第三个度量是路径为边的度量。例如，在[单位立方体相对顶点之间的距离分别是](https://zh.wikipedia.org/wiki/单位立方体 "wikilink") \(\sqrt 3\)、\(\sqrt 5\) 和 \(3\)。
...
度量空間 M 稱之為完備的，若每個[柯西序列](../Page/柯西序列.md "wikilink")均收斂於 M 內，亦即：若 \(d(x_n, x_m) \to 0\)，其中 n 與 m 個自趨近於無限大，則存在某個 \(y\in M\)，使得 \(d(x_n, y) \to 0\)。

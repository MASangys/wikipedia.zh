> 本文内容由[开放式最短路径优先](https://zh.wikipedia.org/wiki/开放式最短路径优先)转换而来。


**开放式最短路径优先**（，缩写为 OSPF）是一种基于[IP协议的](https://zh.wikipedia.org/wiki/IP协议 "wikilink")[路由协议](../Page/路由协议.md "wikilink")。它是大中型网络上使用较为广泛的[IGP协议](../Page/内部网关协议.md "wikilink")。OSPF是对的一种实现，运作于[自治系统](../Page/自治系统.md "wikilink")内部。OSPF分为OSPFv2和OSPFv3两个版本：OSPFv2定义于[RFC](../Page/RFC.md "wikilink") 2328，支持[IPv4](../Page/IPv4.md "wikilink")网络；而OSPFv3定义于RFC 5340，支持[IPv6](../Page/IPv6.md "wikilink")网络。

它采用[戴克斯特拉算法](../Page/戴克斯特拉算法.md "wikilink")来计算[最短路径树](https://zh.wikipedia.org/wiki/最短路径树 "wikilink")。它使用“代价（Cost）”作为路由度量。链路状态数据库（LSDB）用来保存当前[网络拓扑结构](https://zh.wikipedia.org/wiki/网络拓扑 "wikilink")，[路由器](../Page/路由器.md "wikilink")上属于同一区域的链路状态数据库是相同的（属于多个区域的路由器会为每个区域维护一份链路状态数据库）。
...
[OSPF_area.jpg](https://zh.wikipedia.org/wiki/File:OSPF_area.jpg "fig:OSPF_area.jpg") 一个OSPF网络分为多个区域。区域将网络中的路由器在逻辑上分组并以区域为单位向网络的其余部分发送汇总路由信息。区域编号由一个长度为32位的字段所定义，区域编号有两种表示方法，一种为[点分十进制](https://zh.wikipedia.org/wiki/点分十进制 "wikilink")（如Area 1.1.1.1，写法规则同[IPv4](../Page/IPv4.md "wikilink")地址）；另外一种为十进制数字格式（如Area 1，注意Area 1不等于Area 1.1.1.1，而是0.0.0.1）。根據 RFC2328 描述，區域編號通常使用 32 bits的點分十進位表示。区域编号不是IP地址，用于IPv6网络的OSPFv3也使用同样的（形似IPv4地址的）区域编号格式。

区域是以[接口](../Page/介面_\(資訊科技\).md "wikilink")（Interface）为单位来划分的，所以一台多接口路由器可能属于多个区域。相同区域内的所有路由器都维护一份相同的链路状态数据库（LSDB），如果一台路由器属于多个区域，那么它将为每一个区域维护一份LSDB。

  - 某一区域内的路由器只需要维护该区域的[链路状态数据库](https://zh.wikipedia.org/wiki/链路状态数据库 "wikilink")，而不用维护整个OSPF网络的链路状态数据库。
...
末梢区域（Stub Area）是一个不允许AS外部路由通告（AS External LSA，即类型5 LSA）在其内部进行洪泛扩散的区域。末梢区域的[ABR会通告一条默认路由](https://zh.wikipedia.org/wiki/#区域边界路由器 "wikilink")（default route）到该区域内所有路由器，任何发往AS外部网络的数据流都将依据[默认路由来转发](https://zh.wikipedia.org/wiki/默认路由 "wikilink")。
...
| 位        | 第1字节                                                       | 第2字节    | 第3字节    | 第4字节 |
| -------- | ---------------------------------------------------------- | ------- | ------- | ---- |
| 0\~191   | [OSPF包头](https://zh.wikipedia.org/wiki/#OSPF包头 "wikilink") |         |         |      |

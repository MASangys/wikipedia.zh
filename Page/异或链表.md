**异或链表**（）是[数据结构里面的一种链式存储结构](../Page/数据结构.md "wikilink"),可以在降低空间复杂度的情况下达到和[双向链表一样的目的](../Page/双向链表.md "wikilink")，使得在任何一个结点都能方便地访问它的前驱结点和后继结点。

## 概述

普通[双向链表的每个结点有三个域](../Page/双向链表.md "wikilink")

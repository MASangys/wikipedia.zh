**強弱型別**（Strong and weak
typing）表示在[電腦科學以及](https://zh.wikipedia.org/wiki/電腦科學 "wikilink")[程式設計中](https://zh.wikipedia.org/wiki/程式設計 "wikilink")，經常把[程式語言的](https://zh.wikipedia.org/wiki/程式語言 "wikilink")[类型系统分为](https://zh.wikipedia.org/wiki/类型系统 "wikilink")**強型別**（）和**弱型別**（）两种。這兩個術語並沒有非常明確的定義，但主要用以描述[程式語言對於混入不同](https://zh.wikipedia.org/wiki/程式語言 "wikilink")[資料型別的值進行運算時的處理方式](https://zh.wikipedia.org/wiki/資料型別 "wikilink")。強型別的語言遇到函式引數型別和實際叫用型別不符合的情況經常會直接出錯或者編譯失敗；而弱型別的語言常常會實行隐式转换，或者产生难以意料的结果。這對術語在短短的電腦歷史中，早已含括了更多的意義，而且時常很難知道寫的人究竟要表達哪個意思。

## 解釋

程式語言專家 [Benjamin C.
Pierce](https://zh.wikipedia.org/wiki/Benjamin_C._Pierce "wikilink")，《Types
and Programming Languages》和《Advanced Topics in Types and Programming
Languages》的作者，曾說：

  -
    「我花了幾個星期…試著弄清楚「強型別」、「靜態型別」、「安全」等術語，但我發現這異常的困難…這些術語的用法不盡相同，所以也就近乎無用。」\[1\]

大致而言，「強型別」隱含著程式語言對容許混合情況出現加上了嚴格的限制，以避免[程式碼以無效的資料使用方式](https://zh.wikipedia.org/wiki/程式碼 "wikilink")[編譯或執行](https://zh.wikipedia.org/wiki/編譯 "wikilink")。例如，整數[除法運算不可用於字串](../Page/除法.md "wikilink")；[鏈表上的運算步驟不可用於數字](https://zh.wikipedia.org/wiki/鏈表 "wikilink")。然而，這些限制的本質和效力是極易改變的。

## 強型別在文獻中的含義

已经被写作者认定为“强型別”的一些要素包括:

  - 靜態型別不同於動態型別。在靜態[型別系統中](https://zh.wikipedia.org/wiki/型別系統 "wikilink")，型別是與[變數相連繫的名稱](../Page/變數.md "wikilink")（通常是在声明時），而不是值（通常是在初始化時）。型別也可藉由編譯器來推斷。

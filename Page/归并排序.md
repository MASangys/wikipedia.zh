...
**归并排序**（，或），是建立在归并操作上的一种有效的[排序算法](../Page/排序算法.md "wikilink")，[效率為](https://zh.wikipedia.org/wiki/時間複雜度 "wikilink")\(O(n\log n)\)（[大O符号](../Page/大O符号.md "wikilink")）。1945年由[约翰·冯·诺伊曼首次提出](../Page/约翰·冯·诺伊曼.md "wikilink")。该算法是采用[分治法](../Page/分治法.md "wikilink")（Divide
...
1.  将序列每相邻两个数字进行[归并操作](https://zh.wikipedia.org/wiki/#归并操作 "wikilink")，形成\(ceil(n/2)\)个序列，排序后每个序列包含两/一个元素

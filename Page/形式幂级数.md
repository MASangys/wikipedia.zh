**形式幂级数**是一个[数学中的抽象概念](../Page/数学.md "wikilink")，是从[幂级数中抽离出来的](../Page/幂级数.md "wikilink")[代数对象](../Page/代数.md "wikilink")。形式幂级数和从[多项式中剥离出来的多项式环类似](https://zh.wikipedia.org/wiki/多项式 "wikilink")，不过允许（可数）[无穷多项因子相加](../Page/无穷.md "wikilink")，但不像[幂级数一般要求研究是否收敛和是否有确定的取值](../Page/幂级数.md "wikilink")。形式幂级数在代数和[组合理论中有广泛应用](https://zh.wikipedia.org/wiki/组合 "wikilink")。

## 简介

形式幂级数和多项式的形式定义有类似之处。对于熟悉幂级数的读者，也可以将其看作是不讨论幂级数敛散性，也就是将其中的不定元仅仅看作是一个代数对象，而不是任何具体数值的时候写出的幂级数。举例来说，以下的[级数式子](../Page/级数.md "wikilink")：

\[A = 1 - 3X + 5X^2 - 7X^3 + 9X^4 - 11X^5 + \cdots.\]

如果我们把它当成幂级数来研究的话，重点会放在它的[收敛半径等于](../Page/收敛半径.md "wikilink")1、其对应的幂级数函数是否满足某些性质等等。但作为形式幂级数来研究时，我们关注的是它本身的结构。我们甚至可以把它简写为：\([1,-3,5,-7,9, \cdots]\)这样，只关注它的系数。我们完全可以考虑各种系数的形式幂级数。比如说系数为[阶乘的形式幂级数](https://zh.wikipedia.org/wiki/阶乘 "wikilink")：\([1,1,2,6,24,120,, \cdots]\)，即使说它对应的幂级数：

\[A = 1 + X + 2X^2+6X^3+24X^4 + 120X^5 + \cdots.\]
在\(X\)取任何的非零实数值时都不收敛，我们仍然可以将其作为形式幂级数进行运算。
...
其中\(A + B\)里面\(X^3\)的系数就是\(A\)与\(B\)中\(X^3\)的系数的和；\(AB\)里面\(X^5\)的系数就是\(A\)与\(B\)中\(X\)的阶数相加等于5的项的系数乘积的和：
...
形式幂级数不仅能够定义乘法，也能定义乘法逆的运算。一个形式幂级数\(A\)的逆是指另一个形式幂级数\(C\)，使得\(AC = 1\).
如果这样的形式幂级数\(C\)存在，就是唯一的，将其记为\(A^{-1}\)。同时我们也可以定义形式幂级数的除法：当\(A\)的逆存在时，\(B/A = B\cdot A^{-1}.\)
比如说，可以很容易验证：
...
\[[X^5]A = -11\]
对以上定义的形式幂级数\(B\)，也有：\([X^3]B = 4\)。又比如：\([X^2] ( X + 3 X^2 Y^3 + 10 Y^6) = 3 Y^3\)，\([X^2 Y^3] ( X + 3 X^2 Y^3 + 10 Y^6) = 3\)。提取映射和多项式环中的对应映射一样，都可以看做是到一个子空间的投影映射。
...
这个定义使得\(\varphi\)是一个[同态](../Page/同态.md "wikilink")，所以\(R[[X|X]]\)也是一个交换环。
...
但需要注意的是这里的定义中\(\sum_{i=0}^\infty a_i X^i\)还是一个符号性的对象，因为我们并没有定义其中无限求和号的意义。为了更好地定义\(\sum_{i=0}^\infty a_i X^i\)本身，我们需要引入[拓扑的结构](https://zh.wikipedia.org/wiki/拓扑 "wikilink")，将其作为[极限来严格地说明](https://zh.wikipedia.org/wiki/极限 "wikilink")。需要注意的是，适合的拓扑结构不止一个。
  ...
  - 对不熟悉一般的[点集拓扑学的读者](../Page/点集拓扑学.md "wikilink")，也可以建立一个具体的[度量](../Page/度量空间.md "wikilink")（也就是定义“距离”）来定义拓扑。比如定义两个数列\(a = (a_n)_{n\in \mathbb{N} }\)和\(b = (b_n)_{n\in \mathbb{N} }\)的距离：
    :<math>d(a, b) =

\\begin{cases} 2^{-\\omega(a - b)} &\\quad a - b \\neq 0 \\\\ 0 & \\quad
a - b = 0 \\end{cases} </math>
...
那么部分和\(s_k\)和\(a\)的距离就会是\(2^{-(k+1)}\)，所以\(k\)趋于无穷大的时候，部分和数列和\(a\)的距离趋于0.
这样，在定义了有限项非零元的数列和多项式的关系以后，就可以将任意的数列定义为部分和数列的极限。
...
并且可以验证加法、乘法的交换律和结合律，以及乘法对加法的结合律。于是我们定义出了一个同构于\(R^{\mathbb{N}}\)的[拓扑环](https://zh.wikipedia.org/wiki/拓扑环 "wikilink")，将其称为\(R\)上的形式幂级数环\(R[[X|X]]\)。

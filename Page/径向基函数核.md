...
因为RBF核函数的值随距离减小，并介于0（极限）和1（当**x** = **x**'的时候）之间，所以它是一种现成的[相似性度量表示法](https://zh.wikipedia.org/wiki/相似性度量 "wikilink")。\[3\] 核的[特征空间有无穷多的维数](https://zh.wikipedia.org/wiki/特征空间 "wikilink")；对于\(\sigma = 1\)，它的展开式为：\[4\]

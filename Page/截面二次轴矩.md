...
area**），又称**面積慣性矩**，或面積对某一轴的**惯性矩**，通常是对受弯曲作用物体的横截面而言，是反映截面的形状与尺寸对弯曲变形影响的物理量。弯曲作用下的变形或[挠度不仅取决于荷载的大小](https://zh.wikipedia.org/wiki/挠度 "wikilink")，还与横截面的几何特性有关。如[工字梁的抗弯性能就比相同截面尺寸的矩形梁好](https://zh.wikipedia.org/wiki/工字梁 "wikilink")。它和反映截面抗扭转作用性能的[面積极惯性矩是相似的](https://zh.wikipedia.org/wiki/面積极惯性矩 "wikilink")。

面積二次轴矩虽然也称“惯性矩”，但它和用以计算旋转物体[角加速度的](https://zh.wikipedia.org/wiki/角加速度 "wikilink")[質量惯性矩](https://zh.wikipedia.org/wiki/質量惯性矩 "wikilink")（常称为转动惯量）是不同的两个概念。二者有相同的符号\(I\)（\(I\)是英文中惯性
inertia
的首字母），但依据上下文二者不致混淆。而且二者的[因次或](https://zh.wikipedia.org/wiki/因次 "wikilink")[单位不同](https://zh.wikipedia.org/wiki/单位_\(度量衡\) "wikilink")：面積二次轴矩的单位是长度的四次方，而后者的单位是长度的二次方乘以质量。
...
<!-- end list -->
  ...
  - *I<sub>xCG</sub>*
    ：对与*x*轴平行并且过截面形心的轴（与[中性轴重合](https://zh.wikipedia.org/wiki/中性轴 "wikilink")）的惯性矩
  - *A* ：截面面积
  - *d* ：两轴之间的距离

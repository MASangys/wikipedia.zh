...
最初的迪杰斯特拉算法不采用最小[优先级队列](https://zh.wikipedia.org/wiki/优先级队列 "wikilink")，时间复杂度是\(O(|V|^2)\)(其中\(|V|\)为图的顶点个数)。通过[斐波那契堆](../Page/斐波那契堆.md "wikilink")实现的戴克斯特拉算法时间复杂度是\(O(|E|+|V|\log|V|)\) (其中\(|E|\)是边数) 。对于不含负权的[有向图](https://zh.wikipedia.org/wiki/有向图 "wikilink")，这是目前已知的最快的单源[最短路径算法](https://zh.wikipedia.org/wiki/最短路径 "wikilink")。

...
由[鸽巢原理可知](https://zh.wikipedia.org/wiki/鸽巢原理 "wikilink")，任何可逆逻辑门，需要具有相同数量的输入端与输出端。对于一个输入端，存在有两个可能的可逆逻辑门。一为[非门](https://zh.wikipedia.org/wiki/非门 "wikilink")（NOT），另一种为 YES 门，即输入与输出相同。对于两个输入端，存在的可逆逻辑门为[受控反閘](../Page/受控反閘.md "wikilink")，它把第一个输入对第二个输入进行异或操作，并保持第一个输入不变。

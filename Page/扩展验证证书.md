**扩展验证证书**（**Extended Validation Certificate**，简称：**EV证书**）是一种根据一系列特定标准颁发的[X.509](../Page/X.509.md "wikilink")[电子证书](https://zh.wikipedia.org/wiki/电子证书 "wikilink")。根据要求，在颁发证书之前，[证书发行机构(CA)必须要验证](https://zh.wikipedia.org/wiki/数字证书认证中心 "wikilink")**申请者的身份**。不同机构根据证书标准发行的扩展验证证书并无太大差异，但是有的时候根据一些具体的要求，特定机构发行的证书可以被特定的软件所识别。
...
7.  [What browsers support Extended Validation (EV) and display an EV indicator? | Symantec](https://knowledge.symantec.com/support/ssl-certificates-support/index?page=content&id=SO10090&actp=search&viewlocale=en_US&searchid=1406590748892)

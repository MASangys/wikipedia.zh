...
其中 \(\, \phi(\vec{r},t)\)
是扩散中的物质在\(t\)时刻，位于\(\vec{r}\)处的[密度](../Page/密度.md "wikilink")；\(\, D(\phi,\vec{r})\)是密度\(\phi\)在\(\vec{r}\)处的[扩散系数](https://zh.wikipedia.org/wiki/扩散系数 "wikilink")。
...
更一般的，当*D*是对称正定[矩阵时](../Page/矩阵.md "wikilink")，方程描述的是[各向异性扩散](https://zh.wikipedia.org/wiki/各向异性 "wikilink")。此时方程的三维形式是：

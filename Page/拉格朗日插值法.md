...
在[数值分析中](https://zh.wikipedia.org/wiki/数值分析 "wikilink")，**拉格朗日插值法**是以[法国](https://zh.wikipedia.org/wiki/法国 "wikilink")18世纪[数学家](../Page/数学家.md "wikilink")[约瑟夫·拉格朗日命名的一种](../Page/约瑟夫·拉格朗日.md "wikilink")[多项式插值方法](../Page/多项式插值.md "wikilink")。许多实际问题中都用[函数来表示各結果之間某种内在联系或规律](../Page/函数.md "wikilink")，而不少函数都只能通过繁複实验和多次观测来了解。而，如果对实践中的某个[物理量进行观测](https://zh.wikipedia.org/wiki/物理 "wikilink")，在若干个不同的地方得到相应的观测值，拉格朗日插值法可以找到一个[多项式](https://zh.wikipedia.org/wiki/多项式 "wikilink")，其恰好在各个观测的点取到观测到的值。上面这样的多项式就称为**拉格朗日（插值）多项式**。[数学上来说](../Page/数学.md "wikilink")，拉格朗日插值法可以给出一个恰好穿过二维[平面上若干个已知点的多项式函数](../Page/平面.md "wikilink")。拉格朗日插值法最早被[英国数学家](https://zh.wikipedia.org/wiki/英国 "wikilink")[爱德华·华林于](../Page/爱德华·华林.md "wikilink")1779年发现\[1\]，不久后（1783年）由[莱昂哈德·欧拉再次发现](https://zh.wikipedia.org/wiki/莱昂哈德·欧拉 "wikilink")。1795年，拉格朗日在其著作《师范学校数学基础教程》中发表这个插值方法，从此他的名字就和这个方法联系在一起\[2\]。
...
它在点\(x_j\)取值为：\((x_j-x_0) \cdots (x_j-x_{j-1})(x_j-x_{j+1}) \cdots (x_j-x_{k})\)。由于已经假定\(x_i\)两两互不相同，因此上面的取值不等于**0**。于是，将多项式除以这个取值，就得到一个满足“在\(x_j\)取值为**1**，而在其他点取值都是**0**的多项式”：
...
这证明了\(\ell_0, \ell_1, \cdots , \ell_n\)是[线性无关的](https://zh.wikipedia.org/wiki/线性无关 "wikilink")。同时它一共包含**n+1**个多项式，恰好等于\(\mathbb{K}_n[X]\)的维数。所以\(\ell_0, \ell_1, \cdots , \ell_n\)构成了\(\mathbb{K}_n[X]\)的一组基底。
...
[Lagrange_polynomial_Divergence.jpg](https://zh.wikipedia.org/wiki/File:Lagrange_polynomial_Divergence.jpg "fig:Lagrange_polynomial_Divergence.jpg")
可以将拉格朗日基本多项式重新写为：
...
[he:אינטרפולציה\#צורת
לגראנז'](https://zh.wikipedia.org/wiki/he:אינטרפולציה#צורת_לגראנז' "wikilink")

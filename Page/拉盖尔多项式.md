在[数学中](../Page/数学.md "wikilink")，以[法国](../Page/法国.md "wikilink")[数学家](../Page/数学家.md "wikilink")命名的**拉盖尔多项式**定义为**拉盖尔方程**的标准解。

\[x\,y'' + (1 - x)\,y' + n\,y = 0\,\]
这是一个二阶[线性微分方程](../Page/线性微分方程.md "wikilink")。

这个方程只有当*n*非负时，才有非奇异解。拉盖尔多项式可用在[高斯积分法中](../Page/高斯求積.md "wikilink")，计算形如\(\int_0^\infty f(x) dx\)的积分。

这些多项式（通常用*L*<sub>0</sub>, *L*<sub>1</sub>等表示）构成一个。这个多项式序列可以用[罗德里格公式递推得到](../Page/罗德里格公式.md "wikilink")。

\[L_n(x)=\frac{e^x}{n!}\frac{d^n}{dx^n}\left(e^{-x} x^n\right).\]
在按照下式定义的内积构成的[内积空间中](../Page/内积空间.md "wikilink")，拉盖尔多项式是[正交多项式](../Page/正交多项式.md "wikilink")。
...
\[L_1(x) = 1 - x\,\]
然后运用下面的[递推关系得到更高阶的多项式](../Page/正交多项式.md "wikilink")。

\[L_{k + 1}(x) = \frac{1}{k + 1} \left( (2k + 1 - x)L_k(x) - k L_{k - 1}(x)\right).\]

## 广义拉盖尔多项式

上面提到的拉盖尔多项式的正交性，也可以用另外一种方式表达。即：如果*X*是一个服从[指数分布的](../Page/指数分布.md "wikilink")[随机变量](../Page/随机变量.md "wikilink")（即，[概率密度函数如下式](../Page/概率密度函数.md "wikilink")）：
...
相应的正交多项式为形如下式的**广义拉盖尔多项式**（可以通过[罗德里格公式得到](../Page/罗德里格公式.md "wikilink")）：
...
## 广义拉盖尔多项式的性质与应用
  ...
  - 拉盖尔多项式在x=0的取值（[常数项](../Page/常数项.md "wikilink")）为
    \(L_n^{(\alpha)}(0)= {n+\alpha\choose n} \approx \frac{n^\alpha}{\Gamma(\alpha+1)};\)
  - *L*<sub>*n*</sub><sup>(*α*)</sup>有*n*个[实的正](../Page/实数.md "wikilink")[根](../Page/根_\(数学\).md "wikilink")（应该注意到
    \(\left((-1)^{n-i} L_{n-i}^{(\alpha)}\right)_{i=0}^n\)构成以[施图姆序列](../Page/施图姆定理.md "wikilink")），且这些根全部位于[区间](../Page/区间.md "wikilink")\((0, n+\alpha+ (n-1) \sqrt{n+\alpha}]\)中。
...
雅可比[Theta 函数有下面的表示](../Page/Theta_函数.md "wikilink")：
...
低阶[不完全伽玛函数可展开为](../Page/不完全伽玛函数.md "wikilink")：
...
拉盖尔多项式可以用[围道积分表示](../Page/围道积分.md "wikilink")，如下式所示：
...
这里的*H*<sub>*n*</sub>表示乘上了exp(−*x*<sup>2</sup>)的[埃爾米特多項式](../Page/埃爾米特多項式.md "wikilink")（所谓的“物理学家形式”）。
正因为这样，广义拉盖尔多项式也在[量子谐振子的量子力学处理中出现](../Page/量子谐振子.md "wikilink")。
...
\((a)_n\)是[阶乘幂](../Page/阶乘幂.md "wikilink")，这里表示**升阶乘**。

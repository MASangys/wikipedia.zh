在[计算机科学领域](../Page/计算机科学.md "wikilink")，有向图的拓扑排序是其顶点的线性排序，使得对于从顶点\(u\)到顶点\(v\)的每个有向边\(uv\)，\(u\)在排序中都在\(v\)之前。
...
在[图论中](../Page/图论.md "wikilink")，由一个[有向无环图的顶点组成的序列](https://zh.wikipedia.org/wiki/有向无环图 "wikilink")，当且仅当满足下列条件时，称为该[图的一个](https://zh.wikipedia.org/wiki/图 "wikilink")**拓扑排序**（）：
...
`    将n加到L尾`
`    `**`for``
 ``each`**` node m with an edge `*`e`*` from n to m `**`do`**

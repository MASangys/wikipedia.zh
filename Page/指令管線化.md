...
[處理器的這種組織方式能使總體的處理時間顯著縮短](https://zh.wikipedia.org/wiki/處理器 "wikilink")。
...
管線化是假設程式執行時有一連串的指令要被執行（垂直座標i是指令集，水平座標表時間t）。絕大多數當代的CPU都是利用[時脈驅動](https://zh.wikipedia.org/wiki/時脈 "wikilink")。

而CPU是由內部的[邏輯閘與](../Page/邏輯閘.md "wikilink")[正反器組成](https://zh.wikipedia.org/wiki/正反器 "wikilink")。當受到時脈觸發時，[正反器得到新的數值](https://zh.wikipedia.org/wiki/正反器 "wikilink")，並且[邏輯閘需要一段時間來解析出新的數值](../Page/邏輯閘.md "wikilink")，而當受到下一個時脈觸發時[正反器又得到新的數值](https://zh.wikipedia.org/wiki/正反器 "wikilink")，以此類推。而藉由邏輯閘分散成很多小區塊，再讓正反器鏈接這些小區塊組，使邏輯閘輸出正確數值的時間延遲得以減少，這樣一來就可以減少指令執行所需要的周期。
...
並非在所有情況下管線技術都起作用。可能有一些缺點。如果一條指令管線能夠在每一個[時脈週期接納一條新的指令](https://zh.wikipedia.org/wiki/時脈週期 "wikilink")，被稱為**完整管線化**（fully
...
2.  一些[集成電路](https://zh.wikipedia.org/wiki/集成電路 "wikilink")（combinational
...
1.  非管線化的處理器每次（at a
    time）只執行一個指令。防止[分支](https://zh.wikipedia.org/wiki/分支 "wikilink")[延時](https://zh.wikipedia.org/wiki/延時 "wikilink")（事實上，每個分支都會產生延時）和[串行指令被](https://zh.wikipedia.org/wiki/串行 "wikilink")[并行執行產生的問題](https://zh.wikipedia.org/wiki/并行 "wikilink")。設計比較簡單和較低生產成本。
2.  在執行相同的指令時，非管線化處理器的[指令傳輸延遲時間](https://zh.wikipedia.org/wiki/指令傳輸延遲時間 "wikilink")（The
    instruction latency）比管線化處理器明顯較短。這是因為管線化的處理器必須在數據路徑（data
    path）中添加額外[正反器](https://zh.wikipedia.org/wiki/正反器 "wikilink")（flip-flops）。
3.  非管線化處理器有固定指令[位寬](https://zh.wikipedia.org/wiki/位寬 "wikilink")（a
    stable instruction bandwidth）。管線化處理器的性能更難以預測，並且不同的程序之間的變化（vary）可能更大。
...
很多處理器的管線深度到5層、7層、10層，甚至31層（像是[Intel](https://zh.wikipedia.org/wiki/Intel "wikilink")
[Pentium 4
Prescott](https://zh.wikipedia.org/wiki/Pentium_4#Prescott "wikilink")）。[Xelerator](https://zh.wikipedia.org/wiki/Xelerator "wikilink")
[X10q甚至有多于](https://zh.wikipedia.org/wiki/X10q "wikilink")1000層的管線深度[1](http://www.mdronline.com/watch/watch_Issue.asp?Volname=Issue+%23171&on=1#item13)。
...
C`，而[中央處理器](https://zh.wikipedia.org/wiki/中央處理器 "wikilink")（CPU）會將[記憶體](https://zh.wikipedia.org/wiki/記憶體 "wikilink")（Memory）內A位置與B位置的數值相加後放到C位置。在管線化處理器內，[管線控制器會將這個指令分拆成一連串微指令](https://zh.wikipedia.org/wiki/管線 "wikilink")：
...
*R1*,
*R2*和*R3*是CPU內的[暫存器](https://zh.wikipedia.org/wiki/暫存器 "wikilink")（register是CPU裡面能夠快速存取的暫存記憶體）。主存儲器內標註為A位置和B位置之存儲單元中的數值被載入（或稱複製）到暫存器R1和R2中，然後送到加法器中相加，結果輸出到暫存器R3中，R3中的數值再被存儲到主存儲器內標註為C位置的存儲單元。
...
注意\!
有時候，一個指令會依賴於其他指令的執行結構（例如以上的MOVE指令）。當一個指令因為操作數而需引用一個特定的位置，讀取（作為輸入）或者寫入（作為輸入），執行那些指令的循序不同於程序原本的執行循序能導致[冒險](https://zh.wikipedia.org/wiki/冒險_\(計算機體系結構\) "wikilink")（hazards）。現時有機種技術用於預防危害，或者繞過（working
around）它們。

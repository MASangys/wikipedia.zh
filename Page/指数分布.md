在[機率論和](../Page/機率論.md "wikilink")[統計學中](../Page/統計學.md "wikilink")，**指數分布**（）是一種連續[機率分佈](../Page/機率分佈.md "wikilink")。指數分布可以用来表示獨立隨機事件發生的時間間隔，比如旅客進入機場的時間間隔、打進客服中心電話的時間間隔、中文維基百科新條目出現的時間間隔等等。
...
其中λ \> 0是分布的一个参数，常被称为率参数（rate parameter）。即每单位时间发生该事件的次数。指数分布的区间是\[0,∞)。
如果一个[随机变量](../Page/随机变量.md "wikilink")*X* 呈指数分布，则可以写作：*X* \~
Exponential（λ）。

### 累积分布函数

[累积分布函数可以写成](../Page/累积分布函数.md "wikilink")：
...
[随机变量](../Page/随机变量.md "wikilink")*X* (*X* 的率参数是λ)
的[期望值是](../Page/期望值.md "wikilink")：

\[\mathbf{E}[X] = \frac{1}{\lambda}\]

比方说：如果你平均每个小时接到2次电话，那么你预期等待每一次电话的时间是半个小时。

*X* 的[方差是](../Page/方差.md "wikilink")：

\[\mathbf{D}[X] = \frac{1}{\lambda^2}\]

*X* 的[偏离系数是](../Page/偏离系数.md "wikilink")： **V**\[X\] = 1

### 无记忆性

指数函数的一个重要特征是无记忆性（**Memoryless
Property，又稱遺失記憶性**）。这表示如果一个[随机变量呈指数分布](../Page/随机变量.md "wikilink")，它的条件概率遵循：

\[P(T > s + t\; |\; T > t) = P(T > s) \;\; \hbox{for all}\ s, t \ge 0.\]

### 与泊松过程的关系

[泊松过程是一种重要的随机过程](../Page/泊松过程.md "wikilink")。泊松过程中，第k次随机事件与第k+1次随机事件出现的时间间隔服从指数分布。而根据泊松过程的定义，长度为t的时间段内没有随机事件出现的概率等于

\[\frac{e^{-\lambda t} (\lambda t)^0}{0!} = e^{-\lambda t}\],
长度为t的时间段内随机事件发生一次的概率等于
\(\frac{e^{-\lambda t} (\lambda t)^1}{1!}=e^{-\lambda t} \lambda t\),
所以第k次随机事件之后长度为t的时间段内，第k+n次 (n=1, 2,
3,...)随机事件出现的概率等于\(1-e^{-\lambda t}\)。这是指数分布。这还表明了泊松过程的无记忆性。

### 四分位数

率参数λ的[四分位数函数](../Page/四分位数.md "wikilink")（Quartile function）是：
...
给定[独立同分布样本](../Page/独立同分布.md "wikilink")*x* = (*x*<sub>1</sub>, ...,
*x*<sub>*n*</sub>)，λ的似然函数（Likelihood function）是：
...
似然函数[对数的](../Page/对数.md "wikilink")[导数是](../Page/导数.md "wikilink")：
...
1.  Donald E. Knuth (1998). The Art of Computer Programming, volume 2:
    Seminumerical Algorithms, 3rd edn. Boston: Addison-Wesley. ISBN
    0-201-89684-2. pp. 133
2.  Luc Devroye (1986). Non-Uniform Random Variate Generation. New York:
    Springer-Verlag. ISBN 0-387-96305-7. pp. 392–401

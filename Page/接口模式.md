在系统的设计时刻我们常常遇到这样一个问题：[类Client的实例instanceClient希望使用另一个对象instanceX提供的服务service](../Page/类.md "wikilink")，但在设计时刻我们并不能确定对象instanceX究竟属于哪个类。常见的解决办法是：将对象instanceX提供的服务service抽象为一个[接口ServiceProvider](../Page/接口_\(计算机科学\).md "wikilink")，然后让对象instanceClient通过持有接口ServiceProvider的实例来使用服务service。这种通过接口间接获得服务的解决方案就是**接口模式**。
...
接口模式有如下效果：
  ...
  - 类Client不直接依赖于任何实在（concrete）的服务提供者，而仅仅依赖于轻量的接口，这样就降低了系统的[耦合程度](../Page/耦合力_\(計算機科學\).md "wikilink")。
  - 因为接口将服务独立出来，服务的提供者就可以变化。
  - 因为引入了接口来间接提供服务，所以设计方案变得更加复杂。

## 评论

接口模式直接体现了[面向对象的系统设计的原则之一](../Page/面向对象的系统设计.md "wikilink")：“针对接口编程，而不是针对实现编程”。
...
| [抽象工厂模式](../Page/抽象工厂模式.md "wikilink") | 创建对象家族                |
| [生成器模式](../Page/生成器模式.md "wikilink")   | 创建多个对象的组合             |
| [工厂方法模式](../Page/工厂方法模式.md "wikilink") | 对象的实例化                |
...
| [状态模式](../Page/状态模式.md "wikilink")     | 状态与状态的变化              |
| [策略模式](../Page/策略模式.md "wikilink")     | 算法、策略                 |
| [访问者模式](../Page/访问者模式.md "wikilink")   | 作用于一组对象上的操作           |

[en:Interface pattern](../Page/en:Interface_pattern.md "wikilink")

...
連續系統一般會用[微分方程](../Page/微分方程.md "wikilink")來表示。若微分方程是線性常係數，可以將微分方程取[拉普拉斯轉換](https://zh.wikipedia.org/wiki/拉普拉斯轉換 "wikilink")，將其輸入和輸出之間的關係用[傳遞函數表示](https://zh.wikipedia.org/wiki/傳遞函數 "wikilink")。若微分方程為非線性，已找到其解，可以將非線性方程在此解附近進行線性化\[1\]。若所得的線性化微分方程是常係數的，也可以用拉普拉斯轉換得到傳遞函數。
...
在控制理論中的[穩定性是指控制系統的狀態在特定條件下](https://zh.wikipedia.org/wiki/穩定性 "wikilink")，可以維持在一定的範圍內，不會發散，而在什麼範圍內才算是穩定則依系統種類而不同。
  ...
  - 有輸入信號的[非線性系統](../Page/非線性系統.md "wikilink")，其穩定性是用[輸入-狀態穩定性](https://zh.wikipedia.org/wiki/輸入-狀態穩定性 "wikilink")（input-to-state stability），結合了李雅普諾夫穩定性及類似有界輸入有界輸出穩定性的表示方式。

...
hash
function」，若特別用在分支表中，則稱為。若要使用一維表格的控制表，數據的可能範圍需要很小，像[ASCII](../Page/ASCII.md "wikilink")或[EBCDIC](../Page/EBCDIC.md "wikilink")字元都只有255個可能資料，符合此條件，若數據是用ASCII字元表示，且可以確認其中有些數據不可能出現，其對應的控制表會更小。
...
\! 輸入1 \!\! **副程式編號** |- | **A** || **1** |- | **S** || **2** |- | **M**
|| **3** |- | **D** || **4** |- | **?** || **0** |}
此例可以在不使用查表法的情形下，很有效率的將[ASCII](../Page/ASCII.md "wikilink")輸入資料（A、S、M、D或其他）轉換為指標陣列的編號，不過為了和上例一致，仍使用陣列表示。

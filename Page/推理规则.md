在[逻辑中](../Page/逻辑.md "wikilink")，特别是[数理逻辑中](../Page/数理逻辑.md "wikilink")，**推理规则**（推论规则）是构造有效[推论的方案](https://zh.wikipedia.org/wiki/推论 "wikilink")。这些方案建立在一组叫做**前提**的公式和叫做**结论**的断言之间的[语法关系](../Page/语法.md "wikilink")。这些语法关系用于推理过程中，新的真的断言从其他已知的断言得出。规则也适用于[非形式逻辑和](https://zh.wikipedia.org/wiki/非形式逻辑 "wikilink")[逻辑论证](../Page/逻辑论证.md "wikilink")，但是形式化更加困难和有争议。
...
在规则的集合中，一个推理规则可能是多余的，在它是“可接纳的”或“可推导的”的意义上。一个可推导规则是可以使用其他规则从它的前提推导出它的结论的规则。可接纳规则是只要前提成立结论就成立的规则。所有可推导规则都是可接纳规则。要鉴别它们的区别，考虑定义[自然数的下列规则集合](../Page/自然数.md "wikilink")（[判断](../Page/自然演绎.md "wikilink")\(n\,\,\mathsf{nat}\)断言\(n\)是自然数的事实）:
...
这是自然数的事实，并可以通过[数学归纳法证明](../Page/数学归纳法.md "wikilink")。（要证明这个规则是可接纳的，你可以假定这个前提的一个推导，并在其上归纳出生成\(n \,\,\mathsf{nat}\)的推导）。但是，它不是可推导的，因为它依赖于前提的推导的结构。为此“可推导性”在增加到证明系统下是**稳定的**，而“可接纳性”不是。要看出区别，假设下列无意义规则被增加到证明系统：
...
可接纳规则可以被认为是一个证明系统的[定理](../Page/定理.md "wikilink")。例如，在[相继式演算中](../Page/相继式演算.md "wikilink")[切消成立](../Page/切消定理.md "wikilink")，“切”规则是可接纳的。
...
在证明论中，推理规则在[逻辑演算比如](https://zh.wikipedia.org/wiki/逻辑演算 "wikilink")[相继式演算和](../Page/相继式演算.md "wikilink")[自然演绎的规定中扮演了关键角色](../Page/自然演绎.md "wikilink")。

...
摄动理论计算出来的解答通常会表达为一个微小参数的[冪級數](../Page/冪級數.md "wikilink")。摄动理论解答与精确解之间的差别，可以用这微小参数来做数量比较。冪級數的第一个项目是精确解的解答。后面的项目描述解答的修正。这修正是因为精确解与原本问题的「完全解」之间的误差而产生的。更正式地，完全解\(A\,\!\)的近似可以表達为一个[級數](../Page/級數.md "wikilink")：
...
摄动理论的标准阐述主要是以微扰的阶数来分辨：一阶摄动理论或二阶摄动理论。再来就是以微扰的[简并度来分辨](../Page/简并度.md "wikilink")：无简并或有简并。有简并的摄动，又称为[奇异摄动](../Page/奇异摄动.md "wikilink")（），比较难解，必须用到更进阶的理论。
...
其中，\(D\,\!\)是某特定[微分算子](../Page/微分算子.md "wikilink")，\(\lambda\,\!\)是其[本征值](../Page/本征值.md "wikilink")。
...
又假设我们已知道\(D^{(0)}\,\!\)的解答的完备集\(\{f^{(0)}_i (x)\}\,\!\)；其中，解答\(f^{(0)}_i(x)\,\!\)是\(D^{(0)}\,\!\)的本征值为\(\lambda^{(0)}_i\,\!\)的[本征函数](../Page/本征函数.md "wikilink")。用方程表达，

\[D^{(0)} f^{(0)}_i (x)=\lambda^{(0)}_i f^{(0)}_i (x) \,\!\]。

还有，这一集合的解答\(\{f^{(0)}_i (x)\}\,\!\)形成一个[正交归一集](../Page/正交归一性.md "wikilink")：
...
其中，\(\mathcal{O}\,\!\)采用[大O符號来描述函数的渐近行为](../Page/大O符號.md "wikilink")。
...
## 参阅
  ...
  - [多體微擾理論](../Page/多體微擾理論.md "wikilink")

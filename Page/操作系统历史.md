...
早期的计算机没有操作系统。只有单独的机器，操作者会带着记录有程序和数据的卡片（punch card）或較後期的[打孔纸带去操作机器](https://zh.wikipedia.org/wiki/打孔纸带 "wikilink")。程序读入机器后，机器就开始工作直到程序停止。由于程序难免有误，所以机器通常都会中途崩溃。程序一般通过控制板的开关和状态灯来调试。据说[图灵能非常熟练地用这种方法操作](https://zh.wikipedia.org/wiki/图灵 "wikilink")[曼切斯特1型机器](https://zh.wikipedia.org/wiki/曼切斯特1型 "wikilink")。

...
**数字模拟转换器**（，英文缩写：）是一种将[数字信号转换为](https://zh.wikipedia.org/wiki/数字信号 "wikilink")[模拟信号](https://zh.wikipedia.org/wiki/模拟信号 "wikilink")（以[电流](../Page/电流.md "wikilink")、[电压或](https://zh.wikipedia.org/wiki/电压 "wikilink")[电荷的形式](https://zh.wikipedia.org/wiki/电荷 "wikilink")）的设备。[模拟数字转换器](https://zh.wikipedia.org/wiki/模拟数字转换器 "wikilink")（）则是以相反的方向工作。在很多数字系统中（例如计算机），信号以数字方式存储和传输，而数字模拟转换器可以将这样的信号转换为模拟信号，从而使得它们能够被外界（人或其他非数字系统）识别。

数字模拟转换器的常见用法是在音乐播放器中将数字形式存储的音频信号输出为模拟的声音。有的电视机的显像也有类似的过程。数字模拟转换器有时会降低原有模拟信号的精度，因此转换细节常常需要筛选，使得误差可以忽略。

由于成本的考虑以及对于模块化[电子元件的需求](https://zh.wikipedia.org/wiki/电子元件 "wikilink")，数字模拟转换器基本上是以[集成电路的形式制造](../Page/集成电路.md "wikilink")。数字模拟转换器有多重架构，它们各自都有各自的优缺点。在特定的应用中，数字模拟转换器的选用是否合适，取决于其一系列参数（包括转换速率以及分辨率）是否合适。
...
典型的数字模拟转换器将抽象数转换为具体的脉冲序列，然后利用[插值法输出近似连续的量](../Page/插值.md "wikilink")。其他的转换方法（例如基于[ΔΣ调变的方法](https://zh.wikipedia.org/wiki/ΔΣ调变 "wikilink")）则产生[脉冲密度调制](https://zh.wikipedia.org/wiki/脉冲密度调制 "wikilink")（）\[1\]进而产生平滑的连续信号。
...
大多数现代的音频信号都以数字信号的形式存储在诸如[数字音频播放器和](https://zh.wikipedia.org/wiki/数字音频播放器 "wikilink")[CD中](../Page/CD.md "wikilink")，为了使声音能够从音响设备上输出，数字信号必须重新转换为模拟信号。因此，数字模拟转换器被广泛应用于CD播放器、数字音频播放器以及个人计算机的声卡等设备中。
...
视频采样则以一种完全不同的规模工作，其原因是[阴极射线管的高度非线性响应特性](../Page/阴极射线管.md "wikilink")，电视机通过“伽玛曲线”扫描，在全部显示动态区上提供相对均匀的亮度。这样就需要在计算机的视频设备中使用具有高颜色分辨率的[随机存取存储数字模拟转换器](https://zh.wikipedia.org/wiki/RAMDAC "wikilink")（），为各个信号通道的每一种电平“硬编码”相应的模拟输出值（例如，[雅达利ST电脑和](https://zh.wikipedia.org/wiki/雅达利ST电脑 "wikilink")[世嘉公司的](https://zh.wikipedia.org/wiki/世嘉公司 "wikilink")[Mega
...
源于数字信号源（如计算机）的视频信号，在模拟监视器上显示之前，都必须转换到模拟信号。截至2007年，模拟输入比数字输入更普遍，不过这一情况在具有[DVI或](../Page/DVI.md "wikilink")[HDMI技术的](../Page/HDMI.md "wikilink")[平板显示器普及之后有所变化](https://zh.wikipedia.org/wiki/平板显示器 "wikilink")。\[3\]\[4\]然而，视频数字模拟转换器还是在所需要输出模拟信号的数字视频播放器中存在。数字模拟转换器通常与某些存储器一起构成集成电路芯片，并包含了[伽玛校正](https://zh.wikipedia.org/wiki/伽玛校正 "wikilink")、对比度、亮度设置所需的数据表。这样的集成电路即为随机存取存储数字模拟转换器。
...
系统中数字模拟转换器对于整体工作性能十分重要。以下是其主要工作性能指标：
  ...
  - 分辨率（）：是设计的转换输出可能值的个数。这也可以通过转换器使用的位数来表达（等于以2为底数，所有可能输出值个数的对数）。例如，一个1位的数字模拟转换器只能产生2种（2<sup>1</sup>）输出电平值，而8位的数字模拟转换器则可以产生256种（2<sup>8</sup>）输出电平值。数字模拟转换器的分辨率与其达到的[有效位](https://zh.wikipedia.org/wiki/有效位 "wikilink")（）有关。分辨率直接决定了视频设备的[色彩深度和音频设备的](../Page/色彩深度.md "wikilink")[音频位深度](https://zh.wikipedia.org/wiki/音频位深度 "wikilink")（）。
...
## 延伸阅读
  ...
  - [Behzad
    Razavi](https://zh.wikipedia.org/wiki/Behzad_Razavi "wikilink"),

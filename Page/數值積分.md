在[数值分析中](https://zh.wikipedia.org/wiki/数值分析 "wikilink")，**數值積分**是计算[定積分数值的方法和理论](https://zh.wikipedia.org/wiki/定積分 "wikilink")。在[数学分析中](../Page/数学分析.md "wikilink")，给定函数的[定積分的计算不总是可行的](https://zh.wikipedia.org/wiki/定積分 "wikilink")。许多定积分不能用已知的積分公式得到精确值。数值积分是利用黎曼积分等数学定义，用数值逼近的方法近似计算给定的定积分值。借助于电子计算设备，数值积分可以快速而有效地计算复杂的积分。

## 必要性

数值积分的必要性源自计算函数的[原函数的困难性](https://zh.wikipedia.org/wiki/原函数 "wikilink")。利用原函数计算定积分的方法建立在[牛顿-莱布尼兹公式之上](https://zh.wikipedia.org/wiki/牛顿-莱布尼兹公式 "wikilink")。然而，原函数可以用初等函数表示的函数为数不多，大部分的可积函数的积分无法用初等函数表示，甚至无法有解析表达式。例如常见的[正态分布函数](../Page/正态分布.md "wikilink")：
...
不仅如此，在很多实际应用中，只能知道积分函数在某些特定点的取值，比如天气测量中的气温、[湿度](../Page/湿度.md "wikilink")、气压等，医学测量中的[血压](https://zh.wikipedia.org/wiki/血压 "wikilink")、浓度等等。另外，积分函数有可能是某个[微分方程的解](../Page/微分方程.md "wikilink")。由于很多微分方程只能数值求解，因此只能知道函数在某些点上的取值。这时是无法用求原函数的方法计算函数的积分的。

另外，当积分区域是曲面、三维形体以至于高维流形时，[牛顿-莱布尼兹公式不再适用](https://zh.wikipedia.org/wiki/牛顿-莱布尼兹公式 "wikilink")，只能使用更广泛的[格林公式或](../Page/格林公式.md "wikilink")[斯托克斯公式](https://zh.wikipedia.org/wiki/斯托克斯公式 "wikilink")，以转化为较低维数上的积分，但只能用于少数情况。因此，只能使用数值积分计算函数的近似值。
...
\[\int_I f(t) dt \approx \mathbf{I}_{\mbox{appr}}(x_1, x_2 ,\cdots ,x_n) = \sum_{i=0}^{n} f(x_i) L_i\]
其中\(L_i\)是拉格朗日插值的基本多项式。

## 牛頓-柯特斯公式

[牛頓-柯特斯公式是一种插值型公式](https://zh.wikipedia.org/wiki/牛頓-柯特斯公式 "wikilink")。假设\(I = [a, b]\)中取\(x_k = a+k\frac{b-a}{n}\)
,可以写成：

\[\int_I f(t) dt \approx \mathbf{I}_{\mbox{appr}}(x_1, x_2 ,\cdots ,x_n) = (b -a) \sum_{k=0}^{n} C_{k}^{(n)}f(x_k)\]
其中的\(\ C_{k}^{(n)} = \frac{1}{n} \int_0^n \prod_{k \neq j} \frac{t - j}{k - j} dt = \frac{(-1)^{n-k}}{n \cdot k! (n - k)!} \int_0^n \prod_{k \neq j} (t - j) dt\)

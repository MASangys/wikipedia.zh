...
虽然[网络搜索引擎目前已不仅仅局限于对文本进行检索](https://zh.wikipedia.org/wiki/搜索引擎 "wikilink")，文本信息检索仍然是大部分[网络](../Page/互联网.md "wikilink")[搜索引擎的基础](https://zh.wikipedia.org/wiki/搜索引擎 "wikilink")。
...
Bush的论文《就像我们可能会想的……》\[1\]第一次提出了设计自动的，在大规模的存储数据中进行查找的机器的构想。这被认为是现在信息检索技术的开山之作。进入50年代后，研究者们开始为逐步的实现这些设想而努力。在50年代中期，在利用[电脑对文本数据进行检索的研究上](https://zh.wikipedia.org/wiki/电脑 "wikilink")，研究者取得了一些成果。其中最有代表性的是Luhn在[IBM公司的工作](../Page/IBM.md "wikilink")\[2\]，他提出了利用词对文档构建索引并利用检索与文档中词的匹配程度进行检索
的方法，这种方法就是目前常用的[倒排文档技术的雏形](https://zh.wikipedia.org/wiki/#倒排文檔索引技術 "wikilink")。
...
Salton提出的向量空间模型\[5\] \[6\]
\[7\]。至今该模型还是信息检索领域最为常用的模型之一。但是，检索的对象——文本集合的缺乏使得这些技术在海量文本上的可靠性无法得到验证。当时的研究大多针对数千篇的[文档组成的集合](https://zh.wikipedia.org/wiki/文件 "wikilink")。这时，美國國家標準技術研究所(NIST)组织的文本检索会议(Text
Retrieval Conference,
TREC)的召开改变了这一情况。TREC是一个评测性质的会议，为参评者提供了大规模的文本语料，从而大大推动了信息检索技术的快速发展。会议的第一次召开是1992年，不久后，互联网兴起为信息检索技术提供了一个巨大的实验场。从[Yahoo到](https://zh.wikipedia.org/wiki/Yahoo "wikilink")[Google](../Page/Google.md "wikilink")，大量实用的文本信息检索系统开始出现并得到广泛应用。这些系统从事实上改变了人类获取信息与知识的方式。
...
倒排文档索引的具体数据结构可以进行进一步的优化。在关键词查询上，往往采用B-Tree或[哈希表进行快速查询](../Page/哈希表.md "wikilink")。而文档列表的数据结构则可以采用简单的无序列表进行存储，但是此种无序列表存在一个问题，就是当多个关键词对应的文档集需要进行比较的时候，比较效率将比较低。因此，在实际应用中往往采用[二叉搜索树组织文档列表](https://zh.wikipedia.org/wiki/二叉搜索树 "wikilink")。
...
[ja:文書検索](https://zh.wikipedia.org/wiki/ja:文書検索 "wikilink") [en:Text
retrieval](https://zh.wikipedia.org/wiki/en:Text_retrieval "wikilink")

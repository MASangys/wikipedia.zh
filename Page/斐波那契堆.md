> 本文内容由[斐波那契堆](https://zh.wikipedia.org/wiki/斐波那契堆)转换而来。


**斐波那契堆**（**Fibonacci heap**）是[计算机科学](../Page/计算机科学.md "wikilink")中[树的集合](../Page/树_\(数据结构\).md "wikilink")。它比[二项堆](../Page/二项堆.md "wikilink")具有更好的[平摊分析](../Page/平摊分析.md "wikilink")性能，可用于实现合并[优先队列](https://zh.wikipedia.org/wiki/优先队列 "wikilink")。不涉及删除元素的操作有O(1)的平摊时间。 Extract-Min和Delete的数目和其它相比，较小时效率更佳。稠密图每次decrease key只要O(1)的平摊时间，和二项堆的O(lg n)相比是巨大的改进。

斐波纳契堆于1984年由[Michael L. Fredman与](https://zh.wikipedia.org/wiki/Michael_Fredman "wikilink")[Robert E. Tarjan提出](https://zh.wikipedia.org/wiki/罗伯特·塔扬 "wikilink")，1987年公开发表。\[1\]名字来源于运行时分析使用的[斐波那契数](https://zh.wikipedia.org/wiki/斐波那契数 "wikilink")。

## 结构

斐波那契堆是由一组最小堆有序树构成的。每个节点的度数为其子节点的数目。树的度数为其根节点的度数。

斐波那契堆中的[树都是有根的但是无序](../Page/树_\(图论\).md "wikilink")。每个节点*x*包含指向父节点的指针*p\[x\]*和指向任意一个子结点的*child\[x\]*。x的所有子节点都用双向循环链表链接起来，叫做*x*的子链表。子链表中的每一个节点*y*都有指向它的左兄弟的*left\[y\]*和右兄弟的*right\[y\]*。如果节点*y*是*x*仅有的子节点，则*left\[y\]=right\[y\]=y*。
...
[Category:数据结构](https://zh.wikipedia.org/wiki/Category:数据结构 "wikilink") [Category:堆](https://zh.wikipedia.org/wiki/Category:堆 "wikilink")

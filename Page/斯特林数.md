...
第一類Stirling數是有正負的，其絕對值是\(n\)個元素的項目分作\(k\)個環[排列的方法數目](https://zh.wikipedia.org/wiki/排列 "wikilink")。常用的表示方法有\(s(n,k) , \left[\begin{matrix} n \\ k \end{matrix}\right]\)。
...
這可以用[有向圖來表示](https://zh.wikipedia.org/wiki/有向圖 "wikilink")。

  - 給定\(s(n,0)=0,s(1,1)=1\)，有[遞歸關係](https://zh.wikipedia.org/wiki/遞歸 "wikilink")\(s(n,k)=s(n-1,k-1) + (n-1) s(n-1,k)\)
...
第二類Stirling數是\(n\)個元素的集定義k個[等價類的方法數目](https://zh.wikipedia.org/wiki/等價類 "wikilink")。常用的表示方法有\(S(n,k) , S_n^{(k)} ,  \left\{\begin{matrix} n \\ k \end{matrix}\right\}\)。
...
\(C(k,j)\)是二項式係數，\(B_n\)是[貝爾數](https://zh.wikipedia.org/wiki/貝爾數 "wikilink")。

## 兩者關係

\(\sum_{n=0}^{\max\{j,k \}} S(n,j) s(k,n) = \sum_{n=0}^{\max\{j,k \}} s(n,j) S(k,n) = \delta_{jk}\)

\(\delta_{jk}\)是[克羅內克爾δ](https://zh.wikipedia.org/wiki/克羅內克爾δ "wikilink")。

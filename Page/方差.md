**方差**（），[應用數學裡的專有名詞](https://zh.wikipedia.org/wiki/應用數學 "wikilink")。在[概率论和](https://zh.wikipedia.org/wiki/概率论 "wikilink")[统计学中](../Page/统计学.md "wikilink")，一个[随机变量的](../Page/随机变量.md "wikilink")**方差**描述的是它的离散程度，也就是该变量离其[期望值的距离](../Page/期望值.md "wikilink")。一个实随机变量的方差也称为它的[二阶矩或二階中心動差](https://zh.wikipedia.org/wiki/矩_\(数学\) "wikilink")，恰巧也是它的二阶累积量。這裡把複雜說白了，就是將各個誤差將之平方（而非取絕對值，使之肯定為正數），相加之後再除以總數，透過這樣的方式來算出各個數據分佈、零散（相對中心點）的程度。繼續延伸的話，方差的正[平方根称为该随机变量的](../Page/平方根.md "wikilink")**[标准差](https://zh.wikipedia.org/wiki/标准差 "wikilink")**（此為相對各個數據點間）。
...
这个定义涵盖了连续、离散、或兩者都有的隨機變數。方差亦可當作是隨機變數與自己本身的[共變異數](https://zh.wikipedia.org/wiki/共變異數 "wikilink")(或[协方差](../Page/协方差.md "wikilink"))：
...
\[\mu = \int x \, f(x) \, dx\,\]
且此處的積分為以*X*為範圍的x[定積分](https://zh.wikipedia.org/wiki/定積分 "wikilink")（definite
integral）
如果一個連續分佈不存在期望值，如[柯西分佈](https://zh.wikipedia.org/wiki/柯西分佈 "wikilink")（Cauchy
distribution），也就不會有方差（不予定义）。
...
此數Cov(., .)代表[共變異數](https://zh.wikipedia.org/wiki/共變異數 "wikilink")。
...
在样本空间Ω上存在有限期望和方差的随机变量构成一个[希尔伯特空间](../Page/希尔伯特空间.md "wikilink")：
L<sup>2</sup>（Ω, dP），不过这裡的内积和长度跟协方差，标准差还是不大一样。
所以，我们得把这个空间“除”常变量构成的子空间，也就是说把相差一个常数的
所有原来那个空间的随机变量做成一个等价类。这还是一个新的无穷维线性空间， 并且有一个从旧空间内积诱导出来的新内积，而这个内积就是协方差。

## 一般化

如果*X*是一个[向量其取值范围在實數空间](../Page/向量空间.md "wikilink")*R*<sup>*n*</sup>，并且其每个元素都是一个一维随机变量，我们就把*X*称为[随机向量](https://zh.wikipedia.org/wiki/随机向量 "wikilink")。随机向量的方差是一维随机变量方差的自然推广，其定义为E\[(*X*
− μ)(*X* − μ)<sup>T</sup>\]，其中μ =
E(*X*)，*X*<sup>T</sup>是*X*的转置。这个方差是一个非[负定的](https://zh.wikipedia.org/wiki/正定矩陣 "wikilink")[方阵](../Page/方块矩阵.md "wikilink")，通常称为[协方差矩阵](../Page/协方差矩阵.md "wikilink")。

如果*X*是一个複數随机变量的向量（向量中每個元素均為複數的隨機變數），那么其方差定义则为E\[(*X* − μ)(*X* −
μ)<sup>\*</sup>\]，其中*X*<sup>\*</sup>是*X*的[共轭转置向量或稱為](../Page/共轭转置.md "wikilink")[埃尔米特向量](../Page/埃尔米特矩阵.md "wikilink")。根据这个定义，[變異數为实数](https://zh.wikipedia.org/wiki/變異數 "wikilink")。

## 历史

「**方差**」（variance）这个名词率先由[羅納德·費雪](https://zh.wikipedia.org/wiki/羅納德·費雪 "wikilink")（）在论文《*The
...
1.  [Ronald
    Fisher](https://zh.wikipedia.org/wiki/羅納德·費雪 "wikilink")（1918）[The
    correlation between relatives on the supposition of Mendelian
    Inheritance](http://digital.library.adelaide.edu.au/dspace/bitstream/2440/15097/1/9.pdf)
...
方波和[鋸齒波不同](https://zh.wikipedia.org/wiki/鋸齒波 "wikilink")。鋸齒波包含所有整數谐波成分（integer
...
phenomenon）。非理想方波中的[振铃被证明与此现象有关](https://zh.wikipedia.org/wiki/振铃 "wikilink")。吉布斯现象可使用[σ近似](https://zh.wikipedia.org/wiki/σ近似 "wikilink")（σ-approximation）来阻止，而σ近似使用[Lanczos
σ因子来使序列更理想地收敛](https://zh.wikipedia.org/wiki/Lanczos_σ因子 "wikilink")。
...
level）间的任意值。这是[脉宽调制](https://zh.wikipedia.org/wiki/脉宽调制 "wikilink")（pulse-width
modulation）的基础。

## 非理想方波的特性

正如已经提到的，理想方波在高和低两个值之间是瞬时变化的。实际上，由于波形产生系统的物理局限性，这永远不可能实现。信号从低值上升到高值然后再下降所花费的时间分别称为**[脉冲上升时间](https://zh.wikipedia.org/wiki/脉冲上升时间 "wikilink")**（rise
time）和**[脉冲衰减时间](https://zh.wikipedia.org/wiki/下降时间 "wikilink")**（fall
time）。

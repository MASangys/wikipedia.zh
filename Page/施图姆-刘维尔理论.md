...
在一个正则的**施图姆-刘维尔（S-L）本征值问题**中，在有界闭区间\[*a*,*b*\]上，三个系数函数\(p(x),w(x),q(x)\)应满足以下性质：
  ...
  - \(p(x)>0,w(x)>0\)；
  - \(p(x),p'(x),w(x),q(x)\) 均连续；
  - \(y(x)\) 满足边界条件 \(\alpha_1 y(a)+\alpha_2 y'(a)=0\) 及
    \(\beta_1 y(b)+\beta_2 y'(b)=0\)（\(\alpha_1^2+\alpha_2^2>0,\beta_1^2+\beta_2^2>0\)）。

只有一些恰当的\(\lambda\)能够使得方程拥有满足上述条件的[非平凡解](https://zh.wikipedia.org/wiki/非平凡解 "wikilink")（非零解）。这些\(\lambda\)称为方程的[特徵值](https://zh.wikipedia.org/wiki/特徵值 "wikilink")，对应的非平凡解称为[特徵函数](https://zh.wikipedia.org/wiki/特徵函数 "wikilink")，而特徵函数的集合则称为特徵函数族。史、刘二人在一些由边界条件确定的[函数空间中](../Page/函数空间.md "wikilink")，引入[埃尔米特算子](https://zh.wikipedia.org/wiki/埃尔米特算子 "wikilink")，形成了**施图姆-刘维尔理论**。这个理论提出了特徵值的存在性和渐近性，以及特徵函数族的正交完备性。这个理论在应用数学中十分重要，尤其是在使用分离变量法求解[偏微分方程的时候](../Page/偏微分方程.md "wikilink")。

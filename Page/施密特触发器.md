...
在[电子学中](../Page/电子学.md "wikilink")，**施密特触发器**（）是包含[正反馈的](https://zh.wikipedia.org/wiki/正反馈 "wikilink")[比较器](../Page/比较器.md "wikilink")[电路](../Page/电路.md "wikilink")。
...
施密特触发器可作为波形整形电路，能将[模拟信号波形整形为数字电路能够处理的](https://zh.wikipedia.org/wiki/模拟信号 "wikilink")[方波波形](../Page/方波.md "wikilink")，而且由於施密特触发器具有滞回特性，所以可用於抗干扰，其应用包括在[開迴路配置中用於抗扰](https://zh.wikipedia.org/wiki/開迴路 "wikilink")，以及在[閉迴路](https://zh.wikipedia.org/wiki/閉迴路 "wikilink")[正回授/負回授配置中用於实现](https://zh.wikipedia.org/wiki/正回授/負回授 "wikilink")[多谐振荡器](https://zh.wikipedia.org/wiki/多谐振荡器 "wikilink")。

## 发明

施密特触发器是由美国科学家[奥托·赫伯特·施密特](https://zh.wikipedia.org/wiki/奥托·赫伯特·施密特 "wikilink")（Otto
Herbert Schmitt）於1934年发明，当时他只是一个研究生，\[1\]
...
施密特触发器常用接入正反馈的[比较器来实现](../Page/比较器.md "wikilink")\[3\]，而不像[运算放大器电路常接入负反馈](../Page/运算放大器.md "wikilink")。对於这一电路，翻转发生在接近地的位置，[迟滞量由R](https://zh.wikipedia.org/wiki/迟滞 "wikilink")<sub>1</sub>和R<sub>2</sub>的[阻值控制](https://zh.wikipedia.org/wiki/電阻 "wikilink")：

[Opampschmitt_xcircuit.svg](https://zh.wikipedia.org/wiki/File:Opampschmitt_xcircuit.svg "fig:Opampschmitt_xcircuit.svg")

比较器提取了两个输入之[差的](https://zh.wikipedia.org/wiki/差 "wikilink")[符号](../Page/符号函数.md "wikilink")。当非反相（+）输入的电压高於反相（−）输入的电压时，比较器输出翻转到高工作电压+*V*<sub>S</sub>；当非反相（+）输入的电压低於反相（−）输入的电压时，比较器输出翻转到低工作电压−*V*<sub>S</sub>。这里的反相（−）输入是[接地的](../Page/接地.md "wikilink")，因此这里的比较器实现了[符号函数](../Page/符号函数.md "wikilink")，具有二态输出的特性，只有高和低两种状态，当非反相（+）端连续输入时总有相同的符号。
...
这样，电路就形成了一段围绕原点的翻转电压带，而触发电平是\(\pm\frac{R_1}{R_2}{V_s}\)。只有当输入电压上升到电压带的上限，输出才会翻转到高电平；只有当输入电压下降到电压带的下限，输出才会翻转回低电平。若*R*<sub>1</sub>为0，*R*<sub>2</sub>为无穷大（即[开路](https://zh.wikipedia.org/wiki/开路 "wikilink")），电压带的宽度会压缩到0，此时电路就变成一个标准比较器
...
下图是一个反相施密特触发器的例子，右图是其滞回曲线，其中U<sub>e</sub>是输入电压，U<sub>r</sub>[是参考电压](https://zh.wikipedia.org/wiki/是参考电压 "wikilink")：
...
例如，在[仙童半导体公司的QSE](https://zh.wikipedia.org/wiki/仙童半导体公司 "wikilink")15x红外光电传感器家族\[4\]
中，[放大式](https://zh.wikipedia.org/wiki/线性放大器 "wikilink")[红外](https://zh.wikipedia.org/wiki/红外 "wikilink")[光电二极管能产生电信号使频率在绝对最高值和绝对最低值间翻转](../Page/光电二极管.md "wikilink")。这种电信号经过[低通滤波後能产生平滑信号](../Page/低通滤波器.md "wikilink")，而这种平滑信号的上升和下降与翻转信号为开启或关闭所需时间的相对量一致。滤波後的输出传递到施密特触发器的输入。实际结果是施密特触发器的输出只从低电平过渡到高电平，而这一过程在接收到的红外信号以长於某个已知时延的时间激励光电二极管之後，一旦施密特触发器的输出变为高电平，其输出只会在红外信号不再以长於类似已知时延的时间激励光电二极管之後才会变为低电平。鉴於光电二极管容易因为环境中的噪声发生伪翻转，由滤波器和施密特触发器实现的时延能确保输出只在输入确实激励元-{}-件时才会翻转。

#### 内建施密特触发器的元-{}-件

如上例所述，飞兆半导体公司QSE15x光电传感器家族内建施密特触发器用於抗扰，而在很多开关电路中，内建施密特触发器也是处於相同的原因，例如开关[去抖动](https://zh.wikipedia.org/wiki/去抖动 "wikilink")（switch
debouncing）。

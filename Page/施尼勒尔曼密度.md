設[集](../Page/集.md "wikilink")\(A \subseteq \mathbb{Z}\)，\(A(n) = |A \cap \{1, 2, \ldots , n \} |\)，\(A\)中不大於\(n\)的元素的數目。**施尼勒尔曼密度**函數\(\sigma : \mathcal{P}( \mathbb{Z}) \to [0,1]\)，或\(A\)的施尼勒尔曼密度定義為：
...
設\(\mathfrak{G}^2 = \{k^2\}_{k=1}^{\infty}\)，[拉格朗日四平方和定理可以寫成](../Page/拉格朗日四平方和定理.md "wikilink")\(\sigma(\mathfrak{G}^2 \oplus \mathfrak{G}^2 \oplus \mathfrak{G}^2 \oplus \mathfrak{G}^2) = 1\)，其中\(A\oplus B\)表示\(A\)和\(B\)的[和集](../Page/和集.md "wikilink")。

...
\[f(\zeta) = \int^\zeta \frac{K}{(w-a)^{1-(\alpha/\pi)}(w-b)^{1-(\beta/\pi)}(w-c)^{1-(\gamma/\pi)} \cdots} \,\mbox{d}w\]， 其中\(K\)是[常数](https://zh.wikipedia.org/wiki/常数 "wikilink")，\(a < b < c < ...\)是\(\zeta\)平面的实轴上的点的值，对应\(z\)平面上的多边形的顶点。这形式的变换称为施瓦茨—克里斯托费尔映射。
...
考虑\(z\)平面中的半无穷带。这可以视作顶点是\(P=0\), \(Q=\pi i\)和\(R\)的[三角形](../Page/三角形.md "wikilink")，当\(R\)趋向无穷大的极限情形。极限时有\(\alpha=0\)和\(\beta=\gamma=\pi/2\)。假设我们要找映射*f*，有*f*(−1) = *Q*，*f*(1) = *P*，和*f*(∞) = *R*，那么*f*是

...
**旋度**（Curl）或稱**回轉度**（Rotation\[1\]），是[向量分析中的一个](../Page/向量分析.md "wikilink")[向量](https://zh.wikipedia.org/wiki/向量场 "wikilink")[算子](../Page/算子.md "wikilink")，可以表示三维向量场对某一点附近的微元造成的旋转程度。向量场每一点的旋度是一个向量，称为旋度向量。它的方向表示向量场在这一点附近旋度最大环量的旋转轴，它和向量场旋转的方向满足[右手定则](https://zh.wikipedia.org/wiki/右手定则 "wikilink")。旋度向量的大小则是这一点附近向量场旋转度的一个量化体现，定义为当绕着这个旋转轴旋转的环量与旋转路径围成的面元面积之比趋近于零时的极限。举例来说，假设一台滚筒洗衣机运行的时候，从前方看来，内部的水流是逆时针旋转，那么中心水流[速度向量场的旋度就是朝前方向外的向量](../Page/速度.md "wikilink")。

## 定义

定义向量场的旋度，首先要引入**[环量](https://zh.wikipedia.org/wiki/环量 "wikilink")**（或称为**旋涡量**）的概念。给定一个三维空间中的向量场\(\mathbf{A}\)以及一个简单闭合有向曲线\(\Gamma\)，\(\mathbf{A}\)沿着曲线\(\Gamma\)的环量就是\(\mathbf{A}\)沿着路径\(\Gamma\)的[闭合曲线积分](https://zh.wikipedia.org/wiki/路径积分 "wikilink")：
...
[圆柱坐标系中](https://zh.wikipedia.org/wiki/圆柱坐标系 "wikilink")，假设物体的位置为\((r, \theta, z)\)，定义其径向单位矢量、横向单位矢量和纵向单位矢量为\(\boldsymbol{e}_{r}, \boldsymbol{e}_{\theta}, \boldsymbol{e}_{z}\)，那么向量场\(\mathbf A\)可以表示成：
...
要注意的是：以上的行列式中元素并不是[可交换的](https://zh.wikipedia.org/wiki/交换律 "wikilink")。实际计算时，展开式其中的每一项应该是第一-{zh-cn:行;zh-hk:列;zh-tw:列;}-的元素乘以第二-{zh-cn:行;zh-hk:列;zh-tw:列;}-的元素再作用于第三-{zh-cn:行;zh-hk:列;zh-tw:列;}-的元素。例如应该是\(\frac{1}{r}\mathbf{e}_{z} \cdot \frac{\partial}{\partial r} ( r A_{\theta})\)而不是\(\frac{\partial}{\partial r} ( r A_{\theta} \cdot \frac{1}{r}\mathbf{e}_{z}).\)
后文中的球面坐标系表达式也是如此。

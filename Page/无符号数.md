...
大多数[架构的](https://zh.wikipedia.org/wiki/架构 "wikilink")[机器语言不区分有符号数及无符号数](../Page/机器语言.md "wikilink")。然而算术指令通常设定进位标志等[CPU标志](https://zh.wikipedia.org/wiki/CPU "wikilink")，为无符号算术及[溢出标志设定](https://zh.wikipedia.org/wiki/溢出 "wikilink")。这些标志能够被带入随后的分支及算术指令中。
...
编译器在遇到有符号数与无符号数间的比较、算术等操作时常会发出警告，因为可能因其范围不同而导致溢出。C/C++语言规定无符号整数运算不存在溢出，如果结果超出了无符号类型能表示的最大数，则做模运算取余数。\[1\]例如，对于uint32的
2-3， 其结果对0x10000模运算取余数，最终结果为0xFFFF。
...
[Category:程式設計小作品](https://zh.wikipedia.org/wiki/Category:程式設計小作品 "wikilink")
[Category:数据类型](https://zh.wikipedia.org/wiki/Category:数据类型 "wikilink")

1.  ISO/IEC 9899:1999 (E) §6.2.5/9：\[...\] A computation involving

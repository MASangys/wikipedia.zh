...
`# 1.  `[`ISO``   ``3166``   ``2-character``   ``country``
 ``code`](https://zh.wikipedia.org/wiki/ISO_3166-1二位字母代码 "wikilink")``.  See the file `iso3166.tab'.``
...
`# 文件中的表格包含以下几列：`
`# 1.  `[`ISO``   ``3166``   ``2-character``   ``country``
 ``code`](https://zh.wikipedia.org/wiki/ISO_3166-1二位字母代码 "wikilink")`.  详见文件“iso3166.tab”。`
`# 2.  时区代表位置的经纬度。`
`#     `[`ISO``

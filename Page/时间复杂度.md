...
常见的非合次线性时间演算法都采用了诸如[平行处理](https://zh.wikipedia.org/wiki/平行演算法 "wikilink")（就像NC<sub>1</sub> matrix行列式计算那样）、（如同葛羅佛搜尋那樣），又或者选择性地对有保证的输入结构作出假设（如幂对数时间的[二分搜尋](https://zh.wikipedia.org/wiki/二分搜尋演算法 "wikilink")）。不过，一些情况，例如在头 log(n) 位元中每个字串有一个位元作为索引的字串组就可能依赖于输入的每个位元，但又符合次线性时间的条件。

...
另外對一個訊號的[相位](https://zh.wikipedia.org/wiki/相位 "wikilink") \(\phi(t)\) 作[微分](../Page/微分.md "wikilink")，我們可以得到該訊號的瞬時頻率 \(\omega\)。現在我們定義一個訊號的[共變異數](https://zh.wikipedia.org/wiki/共變異數 "wikilink") \(COV = \langle t\phi'(t)\rangle - \langle t\rangle \langle\omega\rangle\) ，在此討論的共變異數是看時間與頻率有怎樣的關係，若明顯兩者無關，則 \(COV = 0\) 。

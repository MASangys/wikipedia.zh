在訊號分析中，訊號的[時間分布](../Page/時間.md "wikilink") \(x(t)\)
與[頻率分布](../Page/頻率.md "wikilink")
\(X(\omega)\)之間是有關連的，如果其中一個是寬的，另一個必定是窄的，這是[傅立葉轉換的基本觀念](../Page/傅立葉轉換.md "wikilink")，同時也是[物理學中](../Page/物理學.md "wikilink")[測不準原理的精神](../Page/測不準原理.md "wikilink")。不論是物理學或是訊號分析，測不準原理必須討論兩個變量之間的關係，且這兩個變量在[希爾伯特空間中必須是不可交換的運算子](../Page/希爾伯特空間.md "wikilink")，而在訊號分析當中，經常討論的兩個變量是時間與頻率。
...
其中 \(\langle t\rangle\) 與 \(\langle\omega\rangle\) 代表平均值，\(x(t)\) 與
\(X(\omega)\)
互為訊號的傅立葉轉換與反轉換，兩者的[標準差相乘就是測不準原理](../Page/標準差.md "wikilink"):
...
另外對一個訊號的[相位](../Page/相位.md "wikilink") \(\phi(t)\)
作[微分](../Page/微分.md "wikilink")，我們可以得到該訊號的瞬時頻率
\(\omega\)。現在我們定義一個訊號的[共變異數](../Page/共變異數.md "wikilink")
\(COV = \langle t\phi'(t)\rangle - \langle t\rangle \langle\omega\rangle\)
，在此討論的共變異數是看時間與頻率有怎樣的關係，若明顯兩者無關，則 \(COV = 0\) 。
...
假設訊號的形式為 \(x(t) = A(t)e^{j\phi(t)}\)
且能夠被歸一化。現在考慮一個新訊號\(x_{new}(t)\)與一個舊訊號\(x_{old}(t)\)，其中新訊號的平均時間與平均頻率皆為零，而舊訊號與新訊號的形狀一樣，但有特定的平均時間與平均頻率，即:
...
若取 \(f = tx(t)\) 且 \(g = x'(t)\) 並搭配柯西-施瓦茨不等式就能把
\(\sigma_t^2\sigma_{\omega}^2 = \int  |x'(t)|^2 dt \times \int  |tx(t)|^2 dt\)
整理成:
...
上式中的被積分函數
\(tx^*(t)x'(t) = tA'A + jt\phi'(t)A^2 = \frac{1}{2} \frac{d}{dt}tA^2 - \frac{1}{2}A^2 + jt\phi'(t)\)

上式第一項是[全微分](../Page/全微分.md "wikilink")，[積分為零](../Page/積分.md "wikilink")。第二項積分後是1/2，第三項積分後是j乘上一個訊號的共變異數。所以:
...
我們得到了測不準原理的形式，接下來的問題是:甚麼樣的訊號能得到乘積的最小值1/2?我們在推導過程中可以發現，在柯西-施瓦茨不等式當中的兩個[函數成正比時等號會成立](../Page/函數.md "wikilink")，我們取
\(g(x)=-cf(x)\) ，其中c是任意常數，所以得到
...
上式就是[高斯函數](../Page/高斯函數.md "wikilink")，高斯函數的時間寬度與頻寬的乘積能達到最小，高斯函數也作為[加伯轉換的](../Page/加伯轉換.md "wikilink")[窗函數](../Page/窗函數.md "wikilink")，在[時頻分析中具有重要的地位](../Page/時頻分析.md "wikilink")。

### 正弦波調變

現在對高斯函數進行正弦波[調變](../Page/調變.md "wikilink")，訊號為
\(x(t)=(a/\pi)^{\frac{1}{4}}e^{-at^2/2+jm\sin\omega_mt+j\omega_0t}\)
，其中有:
...
上式\(F_t(\omega)\)代表在時間t的[頻譜含量](../Page/頻譜.md "wikilink")，它可以拿來定義更多相關的量，例如平均時間、持續時間、頻寬。

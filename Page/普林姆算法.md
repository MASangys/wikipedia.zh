...
3.  重复下列操作，直到V<sub>new</sub> = V：
    1.  在集合E中选取权值最小的边（u, v），其中u为集合V<sub>new</sub>中的元素，而v则是V中没有加入V<sub>new</sub>的顶点（如果存在有多条满足前述条件即具有相同权值的边，则可任意选取其中之一）；
...
同时存在第一个属于 T<sub>min</sub> 但不属于 T<sub>0</sub> 且以v<sub>s</sub>为顶点的边，记为 e<sub>d<sub>2</sub></sub>, 连接该边的两个顶点为 (v<sub>s</sub>, v<sub>e<sub>2</sub></sub>)。

[Zig-zag.png](https://zh.wikipedia.org/wiki/File:Zig-zag.png "fig:Zig-zag.png")
曲折掃描（）是壓縮技術的核心演算法之一，輸入三個係數、、後，輸出三個曲折掃描矩陣、、，目的是將出現頻率低的係數組合在一起。

## 介紹

[JPEG影像壓縮技術之所以使用曲折掃描的原因是](../Page/JPEG.md "wikilink")，它能合理的期待在一個區塊中的像素相較於在一條直線上的像素的出現頻率會比較高，而出現頻率高就代表能壓縮的比例高。
...
例如，在影像A中某個區塊已經被儲存，而在影像B中該區塊位移了(5,
5)的距離，則在影像B的儲存上只需要紀錄該區塊的編號以及新的位置即可，如此就能達到壓縮的目的。

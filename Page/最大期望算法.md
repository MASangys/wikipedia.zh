...
在[统计](../Page/统计.md "wikilink")[计算中](../Page/计算.md "wikilink")，**最大期望（EM）算法**是在[概率模型中寻找](../Page/概率模型.md "wikilink")[参数](../Page/参数.md "wikilink")[最大似然估计或者](../Page/最大似然估计.md "wikilink")[最大后验估计的](../Page/最大后验概率.md "wikilink")[算法](../Page/算法.md "wikilink")，其中概率模型依赖于无法观测的[隐变量](../Page/隐变量.md "wikilink")。最大期望算法经常用在[机器学习和](../Page/机器学习.md "wikilink")[计算机视觉的](../Page/计算机视觉.md "wikilink")[数据聚类](../Page/数据聚类.md "wikilink")（Data
...
我们用\(\textbf{y}\)表示能够观察到的不完整的变量值，用\(\textbf{x}\)表示无法观察到的变量值，这样\(\textbf{x}\)和\(\textbf{y}\)一起组成了完整的数据。\(\textbf{x}\)可能是实际测量丢失的数据，也可能是能够简化问题的隐藏变量，如果它的值能够知道的话。例如，在[混合模型中](../Page/混合模型.md "wikilink")，如果“产生”样本的混合元素成分已知的话最大似然公式将变得更加便利（参见下面的例子）。

### 估计无法观测的数据

让\(p\,\)代表矢量\(\theta\):
\(p( \mathbf y, \mathbf x | \theta)\)定义的参数的全部数据的[機率密度函數](../Page/機率密度函數.md "wikilink")（连续情况下）或者[機率質量函數](../Page/機率質量函數.md "wikilink")（离散情况下），那么从这个函数就可以得到全部数据的[最大似然值](../Page/最大似然估计.md "wikilink")，另外，在给定的观察到的数据条件下未知数据的[条件分布可以表示为](../Page/条件分布.md "wikilink")：
...
## 参考文献
  ...
  - Radford Neal, [Geoffrey
    Hinton](../Page/Geoffrey_Hinton.md "wikilink"). "A view of the EM
    algorithm that justifies incremental, sparse, and other variants".
    In Michael I. Jordan (editor), *Learning in Graphical Models* pp
    355-368. Cambridge, MA: MIT Press, 1999.
  ...
  - [A Gentle Tutorial of the EM Algorithm and its Application to
    ...
    Bilmes](../Page/J._Bilmes.md "wikilink") includes a simplified
    derivation of the EM equations for Gaussian Mixtures and Gaussian
  ...
  - [Information Geometry of the EM and em Algorithms for Neural
    ...
    [Shun-Ichi Amari](../Page/Shun-Ichi_Amari.md "wikilink") give a view
    of EM algorithm from geometry view point.

...
最小费用最大流建立在[最大流和](https://zh.wikipedia.org/wiki/最大流 "wikilink")[网络流问题的基础之上](../Page/网络流.md "wikilink")。

带权[有向图](https://zh.wikipedia.org/wiki/有向图 "wikilink") \(G=(V,E)\)
是一个特殊的[容量网络](https://zh.wikipedia.org/wiki/网络流#网络流图 "wikilink"),所有边
\((u,v)\in E\) 包含 \(c(u,v)\in\mathbb{R}^+\), 称为这条弧的容量； 以及
\(w(u,v)\in\mathbb{R}\) 称为这条边的**费用**。

容量网络中一个[可行流的总费用为](https://zh.wikipedia.org/wiki/网络流#可行流 "wikilink")
\(\sum\left (f(u,v)\times w(u,v)\right )\).
所有[最大流中总费用最少的称为这个容量网络的](https://zh.wikipedia.org/wiki/最大流 "wikilink")**最小费用最大流**。
...
在[最短路问题中](../Page/最短路问题.md "wikilink")，我们利用[队列优化的Bellman-Ford算法](https://zh.wikipedia.org/wiki/SPFA "wikilink")（以下简称
SPFA)
求[单源最短路](https://zh.wikipedia.org/wiki/单源最短路 "wikilink")，进而得到两个结点之间的最短路径
\(dis_{u\to v}\). 使用类似的思想，将两点之间的距离转换为两点之间的费用，然后运行 SPFA
算法，同时维护可以从源点到达每个点的最大流量，得到从源点到汇点一条费用最小的[增广路](https://zh.wikipedia.org/wiki/网络流#增广路 "wikilink")，使用这条路径进行增广，然后重复这个过程。直到找不到增广路，此时的总流量和总费用即为所求答案。
...
求出[单源最短路后](https://zh.wikipedia.org/wiki/单源最短路 "wikilink")，就等同于找到了一条增广路，花费
\(f(t)\times d(t)\) 将流量增大 \(f(t)\).
增广结束后，我们需要更新这条增广路上[弧和反向弧的流量](https://zh.wikipedia.org/wiki/网络流#弧 "wikilink")。\[1\]
...
[二分图的最佳带权匹配问题在经过变形之后](https://zh.wikipedia.org/wiki/二分图的带权匹配 "wikilink")，可以使用最小费用最大流相关算法进行求解。首先对于[二分图中的每一条边](../Page/二分图.md "wikilink")，视其容量为1，它的权值也就是费用，由于最佳带权匹配需要所有匹配边权值之和最大，所以视其费用为权值的相反数。正确地求得最小费用
\(C\) 之后，最佳带权匹配的总权值之和 \(T\) 就是最小费用的相反数 \(T=-C\).

需要注意的是，二分图匹配问题中有许多个源点和许多个汇点，一条[可行流可以从其中任何一个源点出发到达任何一个汇点结束](https://zh.wikipedia.org/wiki/网络流#可行流 "wikilink")，对于这种情况，我们可以建立一个额外的源点何一个额外的汇点，将额外源点与所有源点连容量为

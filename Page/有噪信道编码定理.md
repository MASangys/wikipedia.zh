...
下面这个可实现性的证明是使用[渐近等同分割特性](https://zh.wikipedia.org/wiki/渐近等同分割特性 "wikilink")（[Asymptotic equipartition property](https://zh.wikipedia.org/wiki/:en:Asymptotic_equipartition_property "wikilink") - AEP）方法。另一种信息论常用证明方法是错误列举法（[Error Exponent](https://zh.wikipedia.org/wiki/:en:Error_Exponent "wikilink")）。

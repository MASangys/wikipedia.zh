...
符号(*a*,
*b*)也表示在[实数轴上的](https://zh.wikipedia.org/wiki/实数轴 "wikilink")[开区间](https://zh.wikipedia.org/wiki/区间 "wikilink")；在有歧义的场合可使用符号\(\langle a,b\rangle\)。
...
有序对的概念对于定义[笛卡尔积和](https://zh.wikipedia.org/wiki/笛卡尔积 "wikilink")[关系是至关重要的](../Page/关系_\(数学\).md "wikilink")。

## 有序对的集合论定义

[诺伯特·维纳在](https://zh.wikipedia.org/wiki/诺伯特·维纳 "wikilink")1914年提议了有序对的第一个集合论定义：
...
在[公理化集合论中](../Page/公理化集合论.md "wikilink")，有序对(*a*,*b*)通常定义为[库拉托夫斯基对](https://zh.wikipedia.org/wiki/库拉托夫斯基 "wikilink")：
...
“逆”（reverse）对基本不使用，因为它比通用的Kuratowski对没有明显的优点（或缺点）。“短”（short）对有一個缺點，它的特征性质的证明會比Kuratowski对的证明更加复杂（要使用[正规公理](https://zh.wikipedia.org/wiki/正规公理 "wikilink")）；此外，因为在集合论中数2有时定义为集合{
...
[Rosser](https://zh.wikipedia.org/wiki/J._Barkley_Rosser "wikilink")（1953年）\[1\]扩展了[蒯因的有序对定义](https://zh.wikipedia.org/wiki/蒯因 "wikilink")。Quine-Rosser的定义要求[自然数的先决定义](../Page/自然数.md "wikilink")。设\(\N\)是自然数的集合，\(x \setminus \N\)是\(\N\)在\(x\)內的相對差集，並定義：
...
有序对的这个定义有个显著的优点。在[类型论和从类型论派生出的集合论如](../Page/类型论.md "wikilink")[新基础中](https://zh.wikipedia.org/wiki/新基础 "wikilink")，这个对与它的投影有相同的类型（所以术语叫做“类型齐平”有序对
）。因此一個函数（定义为有序对的集合），有只比序對的投影的类型高1的类型。对蒯因集合论中有序对的广泛的讨论请参见Holmes
(1998)。\[2\]

### Morse定义

Morse（1965年）\[3\]提出的[Morse-Kelley集合论可以自由的使用](https://zh.wikipedia.org/wiki/Morse-Kelley集合论 "wikilink")[真类](https://zh.wikipedia.org/wiki/真类 "wikilink")。Morse定义有序对的方法，使得它的投影可以是真类或者集合。（Kuratowski定义不允许这样）。它首先像Kuratowski的方式那樣，定义投影为集合的有序对。接着，他*重定义*对
(*x*,*y*)为
...
1.  [J. Barkley
    Rosser](https://zh.wikipedia.org/wiki/J._Barkley_Rosser "wikilink"),

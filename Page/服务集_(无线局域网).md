**服務設定**（Service set）是[无线局域网](../Page/无线局域网.md "wikilink")中的一个术语，用以描述[802.11无线网络的构成单位](https://zh.wikipedia.org/wiki/802.11 "wikilink")（一组互相有联系的无线设备），使用**服務設定識別碼**（SSID）作为识别。可以分为**独立基本服务設定**（IBSS）、**基本服务設定**（BSS）和**扩展服务設定**（ESS）三类。其中IBSS属于对等拓扑模式（又称[Ad-Hoc模式、無線隨意網路](../Page/無線隨意網路.md "wikilink")），而BSS和ESS属于基础架构模式。这些拓扑是原始的802.11规范中定义的，其他的如[网桥](https://zh.wikipedia.org/wiki/网桥 "wikilink")、[中继器等则是属于特定厂商的扩展或者](https://zh.wikipedia.org/wiki/中继器 "wikilink")[WDS的拓扑模式](https://zh.wikipedia.org/wiki/WDS "wikilink")。\[1\]

一些新的访问点支持伪装成多个独立的访问点，各自拥有独立的SSID、[安全规则等等](https://zh.wikipedia.org/wiki/无线局域网#无线局域网安全 "wikilink")，但这也不属于原始的802.11规范。\[2\]\[3\]

## 服務設定識別碼

**服務設定識別碼**（英语：Specific Service Set Identifier，**SSID**）是一个或一组基础架构模式[无线网络](../Page/无线网络.md "wikilink")的标识，依照标识方式又可细分为两种：

  - 基本服務設定識別碼（BSSID），表示的是[AP的](https://zh.wikipedia.org/wiki/无线接取器 "wikilink")[数据链路层](../Page/数据链路层.md "wikilink")的[MAC地址](../Page/MAC地址.md "wikilink")
  - 扩展服務設定識別碼（ESSID），一个最长32[字节](../Page/字节.md "wikilink")区分大小写的[字符串](../Page/字符串.md "wikilink")，表示无线网络的名称

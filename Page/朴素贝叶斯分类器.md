在[机器学习](../Page/机器学习.md "wikilink")中，**單純貝氏分类器**是一系列以假设特征之间强（朴素）[独立下运用](https://zh.wikipedia.org/wiki/統計獨立性 "wikilink")[贝叶斯定理](../Page/贝叶斯定理.md "wikilink")为基础的简单。

單純貝氏自20世纪50年代已广泛研究。在20世纪60年代初就以另外一个名称引入到[文本信息检索](../Page/文本信息检索.md "wikilink")界中，\[1\] 并仍然是[文本分类的一种热门](../Page/文档分类.md "wikilink")（基准）方法，文本分类是以[词频为特征判断文件所属类别或其他](https://zh.wikipedia.org/wiki/词袋 "wikilink")（如[垃圾邮件](../Page/反垃圾邮件技术.md "wikilink")、合法性、体育或政治等等）的问题。通过适当的预处理，它可以与这个领域更先进的方法（包括[支持向量机](../Page/支持向量机.md "wikilink")）相竞争。\[2\] 它在自动[医疗诊断中也有应用](../Page/诊断.md "wikilink")。\[3\]

單純貝氏分类器是高度可扩展的，因此需要数量与学习问题中的变量（特征/预测器）成线性关系的参数。[最大似然训练可以通过评估一个](../Page/最大似然估计.md "wikilink")[封闭形式的表达式来完成](https://zh.wikipedia.org/wiki/解析解 "wikilink")，\[4\] 只需花费[线性时间](../Page/时间复杂度.md "wikilink")，而不需要其他很多类型的分类器所使用的费时的[迭代逼近](https://zh.wikipedia.org/wiki/迭代法 "wikilink")。

在[统计学](../Page/统计学.md "wikilink")和[计算机科学](../Page/计算机科学.md "wikilink")文献中，單純貝氏模型有各种名称，包括**简单贝叶斯**和**独立贝叶斯**。\[5\] 所有这些名称都参考了贝叶斯定理在该分类器的决策规则中的使用，但單純貝氏不（一定）用到[贝叶斯方法](../Page/贝叶斯概率.md "wikilink")；\[6\] 《》提到“『單純貝氏』有时被称为**贝叶斯分类器**，这个马虎的使用促使真正的贝叶斯论者称之为**傻瓜贝叶斯**模型。”\[7\]
...
对于某些类型的概率模型，在[监督式学习的样本集中能获取得非常好的分类效果](../Page/監督式學習.md "wikilink")。在许多实际应用中，單純貝氏模型参数估计使用[最大似然估计](../Page/最大似然估计.md "wikilink")方法；换而言之，在不用到[贝叶斯概率](../Page/贝叶斯概率.md "wikilink")或者任何贝叶斯模型的情况下，單純貝氏模型也能奏效。

尽管是带着这些朴素思想和过于简单化的假设，但單純貝氏分类器在很多复杂的现实情形中仍能够取得相当好的效果。2004年，一篇分析贝叶斯分类器问题的文章揭示了單純貝氏分类器取得看上去不可思议的分类效果的若干理论上的原因。\[8\] 尽管如此，2006年有一篇文章详细比较了各种分类方法，发现更新的方法（如和[随机森林](../Page/随机森林.md "wikilink")）的性能超过了贝叶斯分类器。\[9\]
...
\[p(C \vert F_1,\dots,F_n)\,\] 独立的类别变量\(C\)有若干类别，条件依赖于若干特征变量 \(F_1\),\(F_2\),...,\(F_n\)。但问题在于如果特征数量\(n\)较大或者每个特征能取大量值时，基于概率模型列出概率表变得不现实。所以我们修改这个模型使之变得可行。 [贝叶斯定理](../Page/贝叶斯定理.md "wikilink")有以下式子：

\[p(C \vert F_1,\dots,F_n) = \frac{p(C) \ p(F_1,\dots,F_n\vert C)}{p(F_1,\dots,F_n)}. \,\] 用朴素的语言可以表达为：

\[\mbox{posterior} = \frac{\mbox{prior} \times \mbox{likelihood}}{\mbox{evidence}}.
\,\]

实际中，我们只关心分式中的分子部分，因为分母不依赖于\(C\)而且特征\(F_i\)的值是给定的，于是分母可以认为是一个常数。这样分子就等价于[联合分布](../Page/联合分布.md "wikilink")模型。

\[p(C, F_1, \dots, F_n)\,\] 重复使用[链式法则](../Page/链式法则.md "wikilink")，可将该式写成[条件概率](../Page/条件概率.md "wikilink")的形式，如下所示：
...
\[\varpropto p(C) \ p(F_1\vert C) \ p(F_2\vert C, F_1) \ p(F_3\vert C, F_1, F_2) \ \dots p(F_n\vert C, F_1, F_2, F_3,\dots,F_{n-1}).\] 现在“朴素”的[条件独立](../Page/条件独立.md "wikilink")假设开始发挥作用:假设每个特征\(F_i\)对于其他特征\(F_j\),\(j\neq i\)是条件独立的。这就意味着

\[p(F_i \vert C, F_j) = p(F_i \vert C)\,\] 对于\(i\ne j\)，所以联合分布模型可以表达为

\[\begin{align}
p(C \vert F_1, \dots, F_n) & \varpropto p(C, F_1, \dots, F_n)\\ & \varpropto p(C) \ p(F_1\vert C) \ p(F_2\vert C) \ p(F_3\vert C) \ \cdots\, \\& \varpropto p(C) \prod_{i=1}^n p(F_i \vert C).\,\end{align}\] 这意味着上述假设下，类变量\(C\)的条件分布可以表达为：

\[p(C \vert F_1,\dots,F_n) = \frac{1}{Z}  p(C) \prod_{i=1}^n p(F_i \vert C)\] 其中\(Z\)(证据因子)是一个只依赖与\(F_1,\dots,F_n\)等的缩放因子，当特征变量的值已知时是一个常数。 由于分解成所谓的类先验概率\(p(C)\)和独立概率分布\(p(F_i\vert C)\)，上述概率模型的可掌控性得到很大的提高。如果这是一个\(k\)分类问题，且每个\(p(F_i\vert C=c)\)可以表达为\(r\)个参数，于是相应的單純貝氏模型有(*k* − 1) + *n* *r* *k*个参数。实际应用中，通常取\(k=2\)（二分类问题），\(r=1\) ([伯努利分布](../Page/伯努利分布.md "wikilink")作为特征），因此模型的参数个数为\(2n+1\)，其中\(n\)是二值分类特征的个数。
...
所有的模型参数都可以通过训练集的相关频率来估计。常用方法是概率的[最大似然估计](../Page/最大似然估计.md "wikilink")。类的先验概率可以通过假设各类等概率来计算（先验概率 = 1 / (类的数量)），或者通过训练集的各类样本出现的次数来估计（A类先验概率=（A类样本的数量）/(样本总数)）。为了估计特征的分布参数，我们要先假设训练集数据满足某种分布或者非参数模型。\[10\]

### 高斯單純貝氏

如果要处理的是连续数据一种通常的假设是这些连续数值为高斯分布。 例如，假设训练集中有一个连续属性，\(x\)。我们首先对数据根据类别分类，然后计算每个类别中\(x\)的均值和方差。令\(\mu_c\) 表示为\(x\)在*c*类上的均值，令\(\sigma^2_c\)为 \(x\)在*c*类上的方差。在给定类中某个值的概率，\(P(x=v|c)\)，可以通过将\(v\)表示为均值为\(\mu_c\)方差为\(\sigma^2_c\)正态分布计算出来。如下， \(P(x=v|c)=\tfrac{1}{\sqrt{2\pi\sigma^2_c}}\,e^{ -\frac{(v-\mu_c)^2}{2\sigma^2_c} }\) 处理连续数值问题的另一种常用的技术是通过离散化连续数值的方法。通常，当训练样本数量较少或者是精确的分布已知时，通过概率分布的方法是一种更好的选择。在大量样本的情形下离散化的方法表现更优，因为大量的样本可以学习到数据的分布。由于單純貝氏是一种典型的用到大量样本的方法（越大计算量的模型可以产生越高的分类精确度），所以單純貝氏方法都用到离散化方法，而不是概率分布估计的方法。
...
\[posterior numerator (female) = 5.3778e^{-04}\] 由于女性后验概率的分子比较大，所以我们预计这个样本是女性。
...
## 参见
  ...
  - [Class membership probabilities](https://zh.wikipedia.org/wiki/Class_membership_probabilities "wikilink")
  ...
  - [boosted trees](https://zh.wikipedia.org/wiki/boosted_trees "wikilink")
  - [随机森林](../Page/随机森林.md "wikilink")
...
<!-- end list -->
  ...
  - Naive Bayes classifiers are available in many general-purpose machine learning and NLP packages, including [Apache Mahout](https://zh.wikipedia.org/wiki/Apache_Mahout "wikilink"), [Mallet](http://mallet.cs.umass.edu/), [NLTK](https://zh.wikipedia.org/wiki/Weka "wikilink"), [Orange](https://zh.wikipedia.org/wiki/Orange_\(software\) "wikilink"), [scikit-learn](https://zh.wikipedia.org/wiki/scikit-learn "wikilink") and [Weka](https://zh.wikipedia.org/wiki/Weka "wikilink").
  - [IMSL Numerical Libraries](https://zh.wikipedia.org/wiki/国际数学及统计程序库数字库 "wikilink") Collections of math and statistical algorithms available in C/C++, Fortran, Java and C\#/.NET. Data mining routines in the IMSL Libraries include a Naive Bayes classifier.
  - [Winnow content recommendation](https://web.archive.org/web/20101031173559/http://doc.winnowtag.org/open-source) Open source Naive Bayes text classifier works with very small training and unbalanced training sets. High performance, C, any Unix.
  - An interactive [Microsoft Excel](../Page/Microsoft_Excel.md "wikilink") spreadsheet [Naive Bayes implementation](http://downloads.sourceforge.net/naivebayesclass/NaiveBayesDemo.xls?use_mirror=osdn) using [VBA](../Page/Visual_Basic_for_Applications.md "wikilink") (requires enabled macros) with viewable source code.
...
[Category:分類演算法](https://zh.wikipedia.org/wiki/Category:分類演算法 "wikilink") [Category:统计分类](https://zh.wikipedia.org/wiki/Category:统计分类 "wikilink") [Category:概率论](https://zh.wikipedia.org/wiki/Category:概率论 "wikilink")
...
8.  Harry Zhang "The Optimality of Naive Bayes". FLAIRS2004 conference. *(available online: [PDF](http://www.cs.unb.ca/profs/hzhang/publications/FLAIRS04ZhangH.pdf))*

9.  Caruana, R. and Niculescu-Mizil, A.: "An empirical comparison of supervised learning algorithms". Proceedings of the 23rd international conference on Machine learning, 2006. *(available online [1](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.122.5901&rep=rep1&type=pdfPDF))*

10. George H. John and Pat Langley (1995). Estimating Continuous Distributions in Bayesian Classifiers. Proceedings of the Eleventh Conference on Uncertainty in Artificial Intelligence. pp. 338-345. Morgan Kaufmann, San Mateo.
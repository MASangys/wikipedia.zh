...
1999年，由於複雜的问题，3GPP委員會正式提交了ODMA標準。這項技術由持有關鍵專利的IWICS持續開發和改進。
...
與蜂巢狀网络一样，ODMA網路的站點運作在相同的寬頻帶中，但是以较低的資料傳輸率、跳频、引入次頻帶的方式運作。由於傳輸是基于封包、無連接式的，所以站点中继的封包来自於相邻站点。 对于每个封包，站点可以藉由大幅度调整路由、功率、資料速率、封包长度、频率、時間視窗和資料品質来优化传输。 每个站點都有责任和很大的自主权對当前环境進行路由和增强对適應性的服务。 为了安全起见，站点要接受网络管理员的授权。

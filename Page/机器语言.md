...
机器语言是用[二进制代码表示的](https://zh.wikipedia.org/wiki/二进制 "wikilink")、[计算机能直接识别和执行的一种机器指令的集合](https://zh.wikipedia.org/wiki/计算机 "wikilink")。它是计算机的设计者通过计算机的[硬件](../Page/硬件.md "wikilink")结构赋予计算机的操作功能。机器语言具有灵活、直接执行和速度快等特点。不同种类的计算机其机器语言是不相容的，按某种计算机的机器指令编制的[程序不能在另一种计算机上执行](https://zh.wikipedia.org/wiki/程序 "wikilink")。

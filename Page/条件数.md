...
在任何自洽的[矩阵范数中](https://zh.wikipedia.org/wiki/矩阵范数 "wikilink")。这个数字经常在数值[线性代数中出现](../Page/线性代数.md "wikilink")，因而单独有个名字，称为**矩阵条件数**：
...
当然，这个定义依赖于范数的选取。
  ...
  - 若\(\|\cdot\|\) 是 \(l_2\)
    [矩阵范数则](https://zh.wikipedia.org/wiki/矩阵范数 "wikilink")

\[\kappa(A) = \frac{\sigma_{max}(A)}{\sigma_{min}(A)}\]
其中\(\sigma_{max}(A)\)和\(\sigma_{min}(A)\)分别是\(A\)的极大和极小[奇异值](https://zh.wikipedia.org/wiki/奇异值 "wikilink")。因此

:\* 若\(A\)是[正规矩阵则](../Page/正规矩阵.md "wikilink")

\[\kappa(A) = \left|\frac{\lambda_{max}(A)}{\lambda_{min}(A)}\right|\]
(\(\lambda_{max}(A),\ \lambda_{min}(A)\)分别是\(A\)的极大和极小（根据模数）[特征值](https://zh.wikipedia.org/wiki/特征值 "wikilink")）
...
\[\kappa(A) = 1\]
  ...
  - 若 \(\|\cdot\|\)是\(l_{\infty}\)
    [矩阵范数而](https://zh.wikipedia.org/wiki/矩阵范数 "wikilink")\(A\)是[下三角矩阵](../Page/三角矩阵.md "wikilink")，非奇异（也即\(a_{ii} \ne 0 \; \forall i\)）则：\(\kappa(A) \geq \frac{\max_i(|a_{ii}|)}{\min_i(|a_{ii}|)}\)

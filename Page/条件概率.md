...
**条件概率**（）就是[事件](../Page/事件.md "wikilink")*A*在事件*B*发生的条件下发生的[概率](../Page/概率.md "wikilink")。条件概率表示为*P*（*A*|*B*），读作“*A*在*B*发生的条件下发生的概率”。

**联合概率**表示两个事件共同发生的概率。*A*与*B*的联合概率表示为\(P(A \cap B)\)或者\(P(A, B)\)或者\(P(AB)\)。

**边缘概率**是某个事件发生的概率。边缘概率是這樣得到的：在聯合概率中，把最終結果中不需要的那些事件合并成其事件的[全概率而消失](../Page/全概率.md "wikilink")（對离散隨机變量用求和得全概率，對連續隨机變量用積分得全概率）。這稱為**邊緣化**（**marginalization**）。*A*的边缘概率表示为*P*（*A*），*B*的边缘概率表示为*P*（*B*）。

需要注意的是，在这些定义中*A*与*B*之间不一定有[因果或者](../Page/因果.md "wikilink")[时间序列关系](../Page/时间序列.md "wikilink")。*A*可能会先于*B*发生，也可能相反，也可能二者同时发生。*A*可能会导致*B*的发生，也可能相反，也可能二者之间根本就没有因果关系。
...
的时候，它们才是[统计独立的](../Page/統計獨立性.md "wikilink")，这样联合概率可以表示为各自概率的简单乘积。
...
的时候，*A*与*B*是[互斥的](../Page/互斥.md "wikilink")。
...
## 其它
  ...
  - 如果事件\(B\)的概率\(P(B) > 0\)，那么\(Q(A) = P(A|B)\)在所有事件\(A\)上所定义的函数\(Q\)就是[概率测度](../Page/概率空间.md "wikilink")。
  - 如果\(P(B)=0\)，\(P(A|B)\)没有定义。
...
考虑概率空间Ω(S,
σ(S))，其中σ(S)是集S上的[σ代数](../Page/σ代数.md "wikilink")，Ω上对应于随机变量X的[概率测度](../Page/概率测度.md "wikilink")（可以理解为概率分布）为P<sub>X</sub>；又A∈σ(S)，P<sub>X</sub>(A)≥0（这里可以理解为事件A，A不是[零测集](../Page/零测集.md "wikilink")）。则∀E∈σ(S)，可以定义集函数P<sub>X|A</sub>如下：
...
条件概率的[谬论是假设](../Page/谬论.md "wikilink")*P*（*A*|*B*）大致等于*P*（*B*|*A*）。数学家[John
Allen
Paulos在他的](../Page/John_Allen_Paulos.md "wikilink")[《数学盲》一书中指出医生](../Page/《数学盲》.md "wikilink")、律师以及其他受过很好教育的非[统计学家经常会犯这样的错误](../Page/统计学家.md "wikilink")。这种错误可以通过用实数而不是概率来描述数据的方法来避免。
...
## 参见
  ...
  - [蒙提霍尔问题](../Page/蒙提霍尔问题.md "wikilink")
  - [后验概率](../Page/后验概率.md "wikilink")

在许多[程序设计语言中](https://zh.wikipedia.org/wiki/程序设计语言 "wikilink")，**条件运算符**（**conditional
operator**,
**`?:`**）是一个基本的[条件语句](https://zh.wikipedia.org/wiki/条件语句 "wikilink")。条件运算符最初起源于[BCPL程序设计语言](../Page/BCPL.md "wikilink")。
...
在C++中，由于C++明确区分[初始化和](../Page/初始化.md "wikilink")[赋值](https://zh.wikipedia.org/wiki/赋值 "wikilink")，有时使用*if-else*的条件赋值语句并不可行。在这种情形下总是可能使用函数调用，但是这样做会显得很笨重而且不简洁。例如，如果你想要根据条件传递不同的值作为构造函数的参数，单纯使用*if-else*语句根本不可能;在这种情况下，可以使用条件赋值表达式，或函数调用。注意有些类型允许初始化，但不允许赋值，或者赋值运算与构造函数不全相同。对于引用类型后者为真，例如：
...
[Category:三目运算符](https://zh.wikipedia.org/wiki/Category:三目运算符 "wikilink")
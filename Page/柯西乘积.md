...
形式上，
\(\exp(a) = \sum x\)，\(\exp(b) = \sum y\)，我们已表明\(\exp(a+b) = \sum C(x,y)\)。由于该两个[绝对收敛](../Page/绝对收敛.md "wikilink")数列的柯西乘积等于两个数列极限的乘积，（见[下面的证明](https://zh.wikipedia.org/wiki/柯西乘积#收敛和梅尔滕斯定理 "wikilink")），因此我们就可证明这个表达式对于
...
Mertens）提出，如果级数\(\sum y\)[收敛到](https://zh.wikipedia.org/wiki/收敛 "wikilink")*Y*，且级数\(\sum x\)[绝对收敛](../Page/绝对收敛.md "wikilink")到*X*，则他们的柯西乘积
\(\sum C(x,y)\)收敛到*XY*。

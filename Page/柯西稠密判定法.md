在数学分析领域中、
'''柯西稠密测试（得名于法国数学家[柯西](../Page/奧古斯丁·路易·柯西.md "wikilink")），是一个应对无穷级数的[收敛测试](../Page/审敛法.md "wikilink")。

一般而言，一个单调递减、非负的实数序列 \(f(n)\)所对应的级数\(\displaystyle\sum\limits_{n=1}^{\infty}f(n)\)收敛当且仅当其“凝结”级数（英语：Condensed
Series）\(\displaystyle\sum\limits_{n=0}^{\infty} 2^{n}f(2^{n})\) 收敛。
且此极限（如果存在）满足以下不等式：
...
## 註釋
  ...
  - Bonar, Khoury (2006). *Real Infinite Series*. Mathematical
    Association of America. .

## 外部連結

  - [Cauchy condensation test
    proof](http://www.mathcs.org/analysis/reals/numser/t_conden.html)

[BITDemo.gif](https://zh.wikipedia.org/wiki/File:BITDemo.gif "fig:BITDemo.gif") **树状数组**或**二叉索引树**（），又以其发明者命名为Fenwick树，最早由Peter M. Fenwick于1994年以A New Data Structure for Cumulative Frequency Tables\[1\]为题发表在SOFTWARE PRACTICE AND EXPERIENCE。其初衷是解决数据压缩里的累积频率（Cumulative Frequency）的计算问题，现多用于高效计算数列的前缀和， 区间和。它可以以\(O(\log n)\)的时间得到任意前缀和\(\sum_{i=1}^j A[i], 1 <= j <= N\)，并同时支持在\(O(\log n)\)时间内支持动态单点值的修改。空间复杂度\(O(n)\)。

在[数值分析这个数学分支中](../Page/数值分析.md "wikilink")，**样条插值**是使用一种名為[样条的特殊](../Page/样条.md "wikilink")[分段](../Page/分段.md "wikilink")[多项式进行插值的形式](../Page/多项式.md "wikilink")。由于样条插值可以使用低阶多项式样条实现较小的插值误差，这样就避免了使用高阶多项式所出现的[龙格现象](../Page/龙格现象.md "wikilink")，所以样条插值得到了流行。

## 样条插值

使用多项式插值，对给定数据集进行插值的*n*阶多项式就被给定数据点所唯一地定义出来。但是，对同样的数据进行插值的*n*阶样条并不是唯一的，为了构建一个唯一的样条插值式它还必须满足另外*n*-1个[自由度](../Page/自由度.md "wikilink")。

## 线性样条插值

线性样条插值是最简单的样条插值。数据点使用[直线进行连接](../Page/直线.md "wikilink")，结果样条是一个[多边形](../Page/多边形.md "wikilink")。

从代数的角度来看，每个*S*<sub>*i*</sub>都是一个如下

\[S_i(x) = y_i + \frac{y_{i+1}-y_i}{x_{i+1}-x_i}(x-x_i)\]
的[线性函数](../Page/线性函数.md "wikilink")。 样条在每个数据点都必须连续，即
...
通过选择\(z_0\)，然后用[递推关系就可以得到](../Page/遞迴關係式.md "wikilink")[系数](../Page/系数.md "wikilink")：

\[z_{i+1} = -z_i + 2 \frac{y_{i+1}-y_i}{x_{i+1}-x_i}\]

## 三次样条插值

对于*n*+1个给定点的数据集{*x*<sub>i</sub>}，我们可以用*n*段三次[多项式在数据点之间构建一个三次样条](../Page/多项式.md "wikilink")。如果
...
表示对函数*f*进行插值的样条函数，那么需要：
  ...
  - 样条相互连接，*S*<sub>*i*-1</sub>(*x*<sub>*i*</sub>) =
    *S*<sub>*i*</sub>(*x*<sub>*i*</sub>), *i*=1,...,*n*-1
  - 两次连续可导，*S*'<sub>*i*-1</sub>(*x*<sub>*i*</sub>) =
    *S*'<sub>*i*</sub>(*x*<sub>*i*</sub>)以及*S*''<sub>*i*-1</sub>(*x*<sub>*i*</sub>)
    = *S*''<sub>*i*</sub>(*x*<sub>*i*</sub>), *i*=1,...,*n*-1.

由于每个三次多项式需要四个条件才能确定曲线形状，所以对于组成*S*的*n*个三次多项式来说，这就意味着需要4*n*个条件才能确定这些多项式。但是，插值特性只给出了*n*
+ 1个条件，内部数据点给出*n* + 1 − 2 = *n* − 1个条件，总计是4*n* −
2个条件。我们还需要另外两个条件，根据不同的因素我们可以使用不同的条件。
...
\[S''(x_0) = S''(x_n) = 0 \,\!\].
这样就得到*自然*三次样条。自然三次样条几乎等同于[样条设备生成的曲线](../Page/样条设备.md "wikilink")。
...
## 参见
  ...
  - [三次埃尔米特样条](../Page/三次埃尔米特样条.md "wikilink")

在[数值分析这个数学分支中](https://zh.wikipedia.org/wiki/数值分析 "wikilink")，**样条插值**是使用一种名為[样条的特殊](https://zh.wikipedia.org/wiki/样条 "wikilink")[分段](https://zh.wikipedia.org/wiki/分段 "wikilink")[多项式进行插值的形式](https://zh.wikipedia.org/wiki/多项式 "wikilink")。由于样条插值可以使用低阶多项式样条实现较小的插值误差，这样就避免了使用高阶多项式所出现的[龙格现象](../Page/龙格现象.md "wikilink")，所以样条插值得到了流行。
...
的[线性函数](https://zh.wikipedia.org/wiki/线性函数 "wikilink")。
样条在每个数据点都必须连续，即
...
\[S''(x_0) = S''(x_n) = 0 \,\!\].
这样就得到*自然*三次样条。自然三次样条几乎等同于[样条设备生成的曲线](https://zh.wikipedia.org/wiki/样条设备 "wikilink")。

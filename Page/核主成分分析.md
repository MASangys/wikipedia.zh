**核主成分分析**（，简称）是[多变量统计领域中的一种分析方法](https://zh.wikipedia.org/wiki/多变量统计 "wikilink")，是使用对[主成分分析的非线性扩展](../Page/主成分分析.md "wikilink")，即将原数据通过核映射到后再使用原本线性的主成分分析。\[1\]
...
一般而言，*N*个数据点在\(d < N\)维空间中是线性不可分的，但它们在\(d \geq N\)维空间中则是[几乎必然线性可分的](../Page/几乎必然.md "wikilink")。这也意味着，如果我们能将*N*个数据点\(\mathbf{x}_i\)映射到一个*N*维空间

\[\Phi(\mathbf{x}_i)\] 其中 \(\Phi : \mathbb{R}^d \to \mathbb{R}^N\)
中，就能很容易地构建一个[超平面将数据点作任意聚类](https://zh.wikipedia.org/wiki/超平面 "wikilink")。不过由于经\(\Phi\)映射后的向量是线性无关的，我们无法再像在线性PCA中那样显式地对协方差进行特征分解。
...
[Kernel_pca_input.png](https://zh.wikipedia.org/wiki/File:Kernel_pca_input.png "fig:Kernel_pca_input.png")
[Kernel_pca_output.png](https://zh.wikipedia.org/wiki/File:Kernel_pca_output.png "fig:Kernel_pca_output.png")
[Kernel_pca_output_gaussian.png](https://zh.wikipedia.org/wiki/File:Kernel_pca_output_gaussian.png "fig:Kernel_pca_output_gaussian.png")
...
[Category:多变量统计](https://zh.wikipedia.org/wiki/Category:多变量统计 "wikilink")
...
2.  [Nonlinear Component Analysis as a Kernel Eigenvalue Problem
    (Technical
    Report)](http://www.face-rec.org/algorithms/Kernel/kernelPCA_scholkopf.pdf)

3.
4.

5.  [Kernel PCA and De-Noising in Feature Spaces.
    NIPS, 1999](http://citeseer.ist.psu.edu/old/mika99kernel.html)
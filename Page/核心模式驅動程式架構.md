> 本文内容由[核心模式驅動程式架構](https://zh.wikipedia.org/wiki/核心模式驅動程式架構)转换而来。


**核心模式驅動程式架構**（**Kernel-Mode Driver Framework**，缩写**KMDF**）是[微軟公司推出的](https://zh.wikipedia.org/wiki/微軟公司 "wikilink")[Windows](https://zh.wikipedia.org/wiki/Windows "wikilink")[驅動程式基礎](https://zh.wikipedia.org/wiki/驅動程式 "wikilink")（[Windows Driver Foundation](https://zh.wikipedia.org/wiki/Windows_Driver_Foundation "wikilink")）之一，建構[Windows XP與](../Page/Windows_XP.md "wikilink")[Windows Server 2003的核心模式](../Page/Windows_Server_2003.md "wikilink")（Kernel-Mode）驅動程式所需的基本功能，包括對隨插即用（PNP）、電源管理（Power Manager）、I/O佇列、直接記憶體存取（DMA）、Windows Management Instrumentation（WMI）和同步處理等的完整支援。KMDF的設計並不能用來取代WDM，它提供“Skeletal WDM”建置來替代WDM；目前，KMDF並不支援匯流排篩選驅動程式（Bus Filter Driver）。
...
自Windows 2000開始，開發驅動程式必以WDM為基礎的，但開發難度太大，無法像使用者模式應用程式開發那樣容易。KMDF支援驅動程式在[Windows Driver Model環境中撰寫驅動程式](../Page/Windows_Driver_Model.md "wikilink")，簡化其中的過程，但是KMDF的設計並不能用來取代WDM，它提供「Skeletal WDM」建置來替代WDM。早期的WDM可支援[Windows 98](../Page/Windows_98.md "wikilink")、[Windows Me](../Page/Windows_Me.md "wikilink")、[Windows 2000和](../Page/Windows_2000.md "wikilink")[Windows XP](../Page/Windows_XP.md "wikilink")；至於WDF計劃支援Windows XP，以及更新的版本。
...
## 外部連結
  ...
  - [Architecture of the Kernel-Mode Driver Framework白皮書](http://www.microsoft.com/taiwan/whdc/driver/wdf/kmdf-arch.mspx)

**桶排序（Bucket sort）**或所謂的**箱排序**，是一個[排序演算法](https://zh.wikipedia.org/wiki/排序演算法 "wikilink")，工作的原理是將[陣列分到有限數量的桶裡](https://zh.wikipedia.org/wiki/陣列 "wikilink")。每個桶再個別排序（有可能再使用別的[排序演算法或是以遞迴方式繼續使用桶排序進行排序](https://zh.wikipedia.org/wiki/排序演算法 "wikilink")）。桶排序是[鴿巢排序的一種歸納結果](https://zh.wikipedia.org/wiki/鴿巢排序 "wikilink")。當要被排序的陣列內的數值是均勻分配的時候，桶排序使用線性時間（\(\Theta(n)\)（[大O符號](https://zh.wikipedia.org/wiki/大O符號 "wikilink")））。但桶排序並不是[比较排序](../Page/比较排序.md "wikilink")，他不受到\(O(n\log n)\)下限的影響。

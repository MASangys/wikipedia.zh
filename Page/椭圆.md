...
\[\overline{CP} = r' = \frac{ab}{\sqrt{a^2 \sin^2 \psi + b^2 \cos^2 \psi}}=\frac{b}{\sqrt{1-\varepsilon^2 \cos^2 \psi}}\]
这里的 \(\varepsilon\) 是椭圆的离心率；\(\psi\) 是 \(\overline{CB}\) 与
\(\overline{CP}\) 的夹角
...
\[\overline{F_1P} = r = \frac{ a\cdot(1-\varepsilon^{2})}{1 - \varepsilon\cdot\cos\theta}\]
这里的 \(\theta\) 是 \(\overline{F_1B}\) 与 \(\overline{F_1P}\) 的夹角

#### 半正焦弦和极坐标

椭圆的半[正焦弦](https://zh.wikipedia.org/wiki/正焦弦 "wikilink")(通常指示为
\(\ell\,\!\))，是从椭圆的一个焦点到椭圆自身，沿着垂直主轴的直线测量的距离。它有关于 \(a\,\!\) 和
\(b\,\!\)（椭圆的半轴），通过公式 \(a\ell=b^2\,\!\) 或者如果使用离心率的话
\(\ell=a\cdot(1-\varepsilon^2)\,\!\)。

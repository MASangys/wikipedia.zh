...
`   這與假設`\(d = |a - b|\)`不符，`
`   故`\(\{x_n\}\)`不可能以兩個不相等的數為極限。`

**定理2**（有界性）若數列\(\{x_n\}\)有極限，則\(\{x_n\}\)有界，即\(\exists M > 0\)，使得\(\forall n \in \mathbb{N}\)，有\(|x_n| \le M\).
...
**定理3**（保序性）若\(\lim _{n \to \infty} x_n=a\)，\(\lim _{n \to \infty} y_n=b\)，且\(a>b\)，則\(\exists N \in \mathbb{N}\)，使得\(\forall n > N\)，有\(x_n > y_n\).

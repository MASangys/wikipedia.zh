**機率公理**（）是[概率論的公理](https://zh.wikipedia.org/wiki/概率論 "wikilink")，任何[事件發生的](https://zh.wikipedia.org/wiki/事件_\(概率論\) "wikilink")[概率的定義均滿足概率公理](../Page/概率.md "wikilink")。因其发明者为[安德烈·柯尔莫果洛夫](https://zh.wikipedia.org/wiki/柯尔莫果洛夫 "wikilink")，也被人们熟知为**柯尔莫果洛夫公理**（）。

某个事件\(E\)的概率\(P(E)\)是定义在“全体”（universe）或者所有可能基础事件的[样本空间](https://zh.wikipedia.org/wiki/样本空间 "wikilink")\(\Omega\)时，概率\(P\)必须满足以下柯尔莫果洛夫公理。

也可以说，概率可以被解释为定义在样本空间的子集的[σ代数上的一个](https://zh.wikipedia.org/wiki/σ代数 "wikilink")[测度](../Page/测度.md "wikilink")，那些子集为事件，使得所有集的测度为\(1\)。这个性质很重要，因为这裡提出[条件概率的自然概念](../Page/条件概率.md "wikilink")。对于每一个非零概率A都可以在空间上定义另外一个概率：

\[P(B \vert A) = {P(B \cap A) \over P(A)}\]
这通常被读作“给定A时B的概率”。如果给定A时B的条件概率与B的概率相同，则A与B被称为是**[独立的](https://zh.wikipedia.org/wiki/统计独立 "wikilink")**。
...
如想通过[代数了解柯尔莫果洛夫的方法](../Page/代数.md "wikilink")，请参照[随机变量代数](https://zh.wikipedia.org/wiki/随机变量代数 "wikilink")。
...
## 相关条目
  ...
  - [人位概率](https://zh.wikipedia.org/wiki/人位概率 "wikilink")（personal
    probability）
  - [主观概率](https://zh.wikipedia.org/wiki/主观概率 "wikilink")（subjective
    probability）
  - [折衷概率](https://zh.wikipedia.org/wiki/折衷概率 "wikilink")（eclectic
    probability）
  - [统计恒性](https://zh.wikipedia.org/wiki/统计恒性 "wikilink")（statistical
    regularity）

> 本文内容由[模块:Adjacent stations](https://zh.wikipedia.org/wiki/模块:Adjacent_stations)转换而来。
...
`       if not inline then -- `[`Template:Legend`](https://zh.wikipedia.org/wiki/Template:Legend "wikilink")
`           result = '`
...
`           local link = args.link or mw.ustring.match(line, '%[%[([^%[:|%]]+)[|%]]')`
`           if link then`
`               result = '`[<span style="color:#' .. color .. ';line-height:initial">`■`</span>](https://zh.wikipedia.org/wiki/'_.._link_.._' "wikilink")`'`
`           else`
...
`               end`
`           else -- `[`Template:Legend`](https://zh.wikipedia.org/wiki/Template:Legend "wikilink")` (fallback; duplication to simplify logic)`
`               result = '`
...
`   local result = {}`
`   local data = args.data`
`   local default = 'background-color:#efefef' -- Default background color for `
`   if system or data then`
`       data = data or getData(system, true)`
`   end`
`   if data then`

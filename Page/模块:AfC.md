> 本文内容由[模块:AfC](https://zh.wikipedia.org/wiki/模块:AfC)转换而来。
...
<td data-sort-type="number" data-sort-value="%d">
...
`       cols[5] = string.format('`

<td data-sort-value="%s">

%s

</td>

', special_id, string.format(diffString, special_id, special_time))
...
', old_id, string.format(diffString, old_id, modified_at))

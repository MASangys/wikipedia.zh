...
local Collection -- a table to hold items Collection = {
...
`   local categories = {`
`       error = '',`
`       warning = '',  -- same as error until determine whether 'Age warning' would be worthwhile`
`   }`
`   local a, b, category`
`   if id == 'warning' then`
`       a = '`<sup>`[`<i>`'`
...
`       -- `` → 1 year, 12 days or 2 years, 11 days`
`       local opt = {`

> 本文内容由[模块:Category see also](https://zh.wikipedia.org/wiki/模块:Category_see_also)转换而来。
...
`   else`
`       if options.showPrefix then`
`           formatString = '`[`:Category:%s`](https://zh.wikipedia.org/wiki/Category:%s "wikilink")`'`
`       else`
`           formatString = '`[`%s`](https://zh.wikipedia.org/wiki/Category:%s "wikilink")`'`
`       end`
`   end`

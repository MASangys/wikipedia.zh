...
`     elseif t == T_CHARGE then formula = formula .. "`<sup>`";``
 ``if``   ``string.match(x,``   ``"%d+")``   ``then``   ``formula``
 ``=``   ``formula``   ``..``   ``string.match(x,``   ``"%d+");``
 ``end``   ``formula``   ``=``   ``formula``   ``..``
 ``string.gsub(string.match(x,``   ``"[%+-]"),``   ``"-",``   ``"−")``
 ``..``
 ``"`</sup>`";  -- can not concatenat a nil value from string.match(x, "%d+");`
...
`            if (auto == '') then formula = formula .. "η`<sup>`"``
 ``..``   ``string.match(x,``   ``'%d+')``   ``..``   ``"`</sup>`−" `
`              else`
`            formula = formula .. "`[`η`<sup>`"``   ``..``
 ``string.match(x,``   ``'%d+')``   ``..``
 ``"`</sup>](https://zh.wikipedia.org/wiki/哈普托數 "wikilink")`−" `
`            end`

\-- elseif parameter == "i" then formula = formula .. su(string.match(x,
'%d+'), "") -- [isotope](../Page/同位素.md "wikilink")

`         elseif parameter == "m" then formula = formula .. "μ`<sup>`"``
 ``..``   ``string.match(x,``   ``'%d+')``   ``..``
 ``"`</sup>`−"   -- mu (`[`bridging``
 ``ligand`](../Page/橋接配體.md "wikilink")`)`
`         end`
...
-- "[H<sub>2</sub>O](../Page/结晶水.md "wikilink")"; -- else -- formula =
formula .. -- DotIt .. "H<sub>2</sub>O"; ---- xxx brug af sub til tal --
end
...
\-- Removed: Gives false positive for wikilinks, like . -- if sumO \> 0
then formula = formula .. "\<span
style=\\"display:none;font-size:100%\\" class=\\"error
citation-comment\\"\> Too many (</span>;" -- elseif sumO \< 0 then
formula = formula .. "'\<span style=\\"display:none;font-size:100%\\"
class=\\"error citation-comment\\"\> Too many )</span>;" -- end

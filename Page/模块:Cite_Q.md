...
get_name_list -- adapted from getAuthors code taken from [Module:RexxS](https://zh.wikipedia.org/wiki/Module:RexxS "wikilink") arguments:
...
`                   out[position] = "[[:d:Q"_.._v.mainsnak.datavalue.value["numeric-id"]_.._"|" .. label .. "]] `<span title='" .. i18n["errors"]["local-article-not-found"] .. "'>[`Wikidata-logo.svg`](https://zh.wikipedia.org/wiki/File:Wikidata-logo.svg "fig:Wikidata-logo.svg")</span>`"`
`               else`
...
Takes standard cs1|2 template parameters and passes all to . If neither of |author= and |author1= are set, calls get_authors() to try to get an author name-list from wikidata. The result is passed to  for rendering.

\]\]

function citeq.cite_q (frame) local citeq_args = {}; local qid; local wdl;

local pframe = frame:getParent() local args = pframe.args; -- first get parent frame arguments - these from the template call

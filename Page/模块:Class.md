...
`       ret = string.format('`<span style="%s">[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")</span>`', style, class[idx].icon, class[idx].name)`
`   end`
`   `
`   return ret`
...
`       '<td class="assess assess-%s %s" style="text-align:center; white-space:nowrap; font-weight:%s; background:%s; %s" %s> %s`[`%s`](https://zh.wikipedia.org/wiki/Category:%s "wikilink")` `

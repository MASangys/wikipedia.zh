...
local function buildList( args, listOnly )

`   -- Get the list items.`
`   local listItems = {}`
`   local argNums = getArgNums( args )`
`   for i, num in ipairs( argNums ) do`
`       table.insert( listItems, fmtListItem( args[ num ] ) )`
`   end`
`   if #listItems == 0 then`
`       return ''`
`   end`
...
`       return '<ul' .. ulclass .. ulstyle .. '>' .. listItems .. '`
...
function p.main( frame, listOnly )
...
`       end`
`   end`
`   return buildList( args, listOnly )`

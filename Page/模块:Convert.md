...
`       -- Floating point limits may need this, as in ``.`
`       remainder = 0`
`   end`
...
`               return (parms.frame):preprocess('``') end)`
...
`       end`
`       if parts.n == 0 then`
`           -- No link like "`[`...`](https://zh.wikipedia.org/wiki/... "wikilink")`" was found in the original name.`
...
`           -- `` → 1,000,000,000 metres (1,000,000,000 m)`
`           local significand`
`           if #show > 1 then`
...
`                   -- May have something like `` (empty input) if source is an infobox`
...
`               if sep:find('%d') then`
`                   return success, result  -- to reject `` with a decent message (en only)`
`               end`
`               parms[i] = rhs`
...
`           -- Some infoboxes have examples like `` (3 or more "N").`
...
`   local fudge = 1e-14  -- `` adds this, so we do too`
...
`           -- Conversion like "``" (where subunit is "in")`
...
`   -- Return wikilink "`[`id`](https://zh.wikipedia.org/wiki/link "wikilink")`", possibly abbreviated as in examples:`
`   --   `[`mile`](https://zh.wikipedia.org/wiki/Mile "wikilink")`  --> `[`mile`](https://zh.wikipedia.org/wiki/mile "wikilink")
`   --   `[`miles`](https://zh.wikipedia.org/wiki/Mile "wikilink")` --> `[`miles`](https://zh.wikipedia.org/wiki/mile "wikilink")
...
`   --   link = true if result should be `[`linked`](https://zh.wikipedia.org/wiki/linked "wikilink")

> 本文内容由[模块:DGBUS](https://zh.wikipedia.org/wiki/模块:DGBUS)转换而来。
...
`   local d = _loadSystemData(loc)`
`   if d[no] == nil then`
`       a = `[`align=center`](https://zh.wikipedia.org/wiki/ "wikilink")`..no..`[](https://zh.wikipedia.org/wiki/''' "wikilink")`..'`[`Module:DGBUS中未包含这条`](https://zh.wikipedia.org/wiki/Module:DGBUS "wikilink")[`"..loc.."的线路`](https://zh.wikipedia.org/wiki/'..loc.."巴士路线列表 "wikilink")`）''"`
`   elseif d[no].note == "已停办" or d[no].note == "已停辦" then`
...
`   local ss = frame.args`
`   local loc = ss.loc`
`   if loc == "" or loc == nil or loc == "東莞" then loc = "东莞" end`

...
\-- Only works for separator is a single unicode character function special_find(text, separator, start)
...
\-- Split a string into tokens. (Adapted from Gavin Kistner's split on -- <http://lua-users.org/wiki/SplitJoin>. -- -- @param text A string to be split. -- @param separator \[optional\] the separator pattern (defaults to any -- white space - %s+). -- @return A list of tokens.
...
\-- Formats an inline diff as HTML, with <ins> and <del> tags. -- -- @param tokens a table of {token, status} pairs. -- @return an HTML string.

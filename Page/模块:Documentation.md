...
`   if display then`
`       return mw.ustring.format('`[`%s`](https://zh.wikipedia.org/wiki/%s "wikilink")`', page, display)`
`   else`
`       return mw.ustring.format('`[`%s`](https://zh.wikipedia.org/wiki/%s "wikilink")`', page)`
`   end`
...
`       -- Diff link between the sandbox and the main template using `[`Special:ComparePages`](https://zh.wikipedia.org/wiki/Special:ComparePages "wikilink")`.`
`       local templateTitle = env.templateTitle`
...
`   -- "This is the template sandbox for `[`Template:Foo`](https://zh.wikipedia.org/wiki/Template:Foo "wikilink")` (diff)."`
`   local text = ''`
`   local pagetype`
...
`           text = text .. '`
<small>`' .. message('sandbox-notice-testcases-run-blurb', {pagetypee, testcasesLink, testcasesRunLink}) .. '`</small>`'`
`       else`
`           local testcasesLinkDisplay = message('sandbox-notice-testcases-link-display')`
`           local testcasesLink = makeWikilink(testcasesTitle.prefixedText, testcasesLinkDisplay)`
`           text = text .. '`
<small>`' .. message('sandbox-notice-testcases-blurb', {pagetypee, testcasesLink}) .. '`</small>`'`
`       end`
`   end`
...
`   --`
`   -- Messages:`
`   -- 'documentation-icon-wikitext' --> '`[`Test_Template_Info-Icon_-_Version_(2).svg`](https://zh.wikipedia.org/wiki/File:Test_Template_Info-Icon_-_Version_\(2\).svg "fig:Test_Template_Info-Icon_-_Version_(2).svg")`'`
...
`       text = text .. linkBox`
`   else`
`       text = text .. (p.makeDocPageBlurb(args, env) or '') -- "This documentation is transcluded from `[`Foo`](https://zh.wikipedia.org/wiki/Foo "wikilink")`." `
...
`   --[=[`
`   -- Makes the blurb "This documentation is transcluded from `[`Template:Foo`](https://zh.wikipedia.org/wiki/Template:Foo "wikilink")` (edit, history)".`
`   -- @args - a table of arguments passed by the user`
`   -- @env - environment table containing title objects, etc., generated with p.getEnvironment`
`   -- `
`   -- Messages:`
...
`   -- 'The above `[`documentation`](https://zh.wikipedia.org/wiki/Wikipedia:Template_documentation "wikilink")` `
`   -- is `[`transcluded`](https://zh.wikipedia.org/wiki/Wikipedia:Transclusion "wikilink")` from $1.'`
`   -- 'module-preload' --> 'Template:Documentation/preload-module-doc'`
`   -- 'create-link-display' --> 'create'`

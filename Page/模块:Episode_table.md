...
`           ((not args.viewers_type or args.viewers_type ~= '') and '`
`（' .. (args.viewers_type or '百万') .. '）' or '')`
`       `
`       local cellNames = {`
...
`   end`
`   `
`   return (args.dontclose and mw.ustring.gsub(tostring(root), "`

</table>

", "") or tostring(root)) .. categories end
...
[Category:使用Template:Episode_table含有無效顏色組合的條目](https://zh.wikipedia.org/wiki/Category:使用Template:Episode_table含有無效顏色組合的條目 "wikilink")
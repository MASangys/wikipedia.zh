> 本文内容由[模块:Excerpt](https://zh.wikipedia.org/wiki/模块:Excerpt)转换而来。
...
`   if image then`
`       image = mw.ustring.match(image, "%b[]%s*") -- matching `[`...`](https://zh.wikipedia.org/wiki/... "wikilink")` to handle wikilinks nested in caption`
`   end`
`   return image`
...
`       local token = parseimage(image, true) -- look for image=`[`File:...`](https://zh.wikipedia.org/wiki/File:... "fig:File:...")` etc.`
`       if not token then`
...
`   local filetext = "" -- output text with concatenated `[`...`](https://zh.wikipedia.org/wiki/File:Foo "fig:...")`\n entries`
...
`               if not images then`
`                   local image = parseimage(token, false) -- look for embedded `[`File:...`](https://zh.wikipedia.org/wiki/File:... "fig:File:...")`, |image=, etc.`
...
`           pagename = mw.ustring.match(pagename, "%[%[%s*(.-)[%]|]") or pagename -- "`[`Bar`](https://zh.wikipedia.org/wiki/Foo "wikilink")`" → "Foo"`

...
[template:Autotranslate](https://zh.wikipedia.org/wiki/template:Autotranslate "wikilink")

`   local args = frame.args`
`   if not args.lang or args.lang == '' then`
`       args.lang = frame:preprocess( "``" )           -- get user's chosen language `
`   end`
...
`   -- Transclude `` with template arguments the same as the ones passed to `` template.`
...
[Category:LangSwitch_template_without_default_version](https://zh.wikipedia.org/wiki/Category:LangSwitch_template_without_default_version "wikilink")
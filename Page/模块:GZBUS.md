...
`   local state`
`   if loc == nil or loc == "" or loc == "广州" or loc == "廣州" then`
...
`   elseif loc == "佛山" then`
`       state, system = pcall(mw.loadData, "Module:GZBUS/foshan")`
`   elseif loc == "东莞" or loc == "東莞" then`
`       state, system = pcall(mw.loadData, "Module:GZBUS/dongguan")`
`   else`
`       state, system = pcall(mw.loadData, "Module:GZBUS/data")`
`   end`
`   return system`
...
`   if d[no] == nil then`
`       a = `[`width="1%"`](https://zh.wikipedia.org/wiki/ "wikilink")`..no..`[](https://zh.wikipedia.org/wiki/''' "wikilink")`..'`[`Module:GZBUS中未包含这条`](https://zh.wikipedia.org/wiki/Module:GZBUS "wikilink")[`"..loc.."的线路`](https://zh.wikipedia.org/wiki/'..loc.."巴士路线列表 "wikilink")`）''"`
...
`       -- BRT样式：`
`       -- 注：不显示公司栏（noco）已不适用于BRT样式`
`       if ((style == "BRT" or style == "brt") and (d[no].code == "B28")) then          -- BRT样式：B28（四列）`
...
`       elseif ((style == "BRT" or style == "brt") and (d[no].brt_east == nil)) then        -- BRT样式：早高峰单向线路（一列）`
...
`       elseif ((style == "BRT" or style == "brt") and (d[no].brt_east ~= nil)) then        -- BRT样式：除B28外的双向线路（两列）`

...
\-- internal one-byte lookups hex._d2hbyte = function(val)
...
\-- internal multi-byte lookups hex._d2h = function(val, integer)
...
`               bytes = 6`
`           else`
`               error ("`[`Module:Hex`](https://zh.wikipedia.org/wiki/Module:Hex "wikilink")` received invalid UTF8")`
`           end`
`           repeat`
...
\-- template front doors hex.d2h = function(frame)
...
`   end`
`   if nowiki then `
`       text = frame:preprocess("<pre>" .. text .. "</pre>")`
`   end`
`   text = "" .. text .. ""`
`   return text`

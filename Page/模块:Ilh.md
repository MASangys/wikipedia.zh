...
`   ,link_body='-{`[<span lang="%s" dir="auto">`%s`</span>](https://zh.wikipedia.org/wiki/:%s:%s "wikilink")`}-'`
`   ,link_tail=COMMON_TAIL`

} local clazz_pageExist_framehead='ilh-blue' local TRA_CAT=''
...
`   context["foreignPage"]=args[2] or args[1] --如果``不传入的话`
...
`   return ( context["isExist"] and mw.ustring.format( '`[`%s`](https://zh.wikipedia.org/wiki/%s "wikilink")`', context["localPage"], context["displayName"] ) ) or context["displayName"]`
...
`           table.insert(output_context,`
`                           mw.ustring.format('`[`%s`](https://zh.wikipedia.org/wiki/:%s "wikilink")`' ,`
...
end --真实方法 function ilh._isExist(pageName)
...
[Category:有蓝链却未移除内部链接助手模板的页面](https://zh.wikipedia.org/wiki/Category:有蓝链却未移除内部链接助手模板的页面 "wikilink")
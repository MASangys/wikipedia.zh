...
`   table_head_isotope = "`[`同位素`](../Page/同位素.md "wikilink")`",`
`   table_head_decay = "`[`衰變`](https://zh.wikipedia.org/wiki/放射性#衰变 "wikilink")`",`
`   table_head_natural_abundance = "`[`丰度`](../Page/丰度.md "wikilink")`",`
`   table_head_decay_halflife = "`[`半衰期`](https://zh.wikipedia.org/wiki/半衰期 "wikilink")` `<small><span style=\"font-weight:normal;\">`(\'\'t\'\'`<sub>`1/2`</sub>`)`</span></small>`",`
`   table_head_decay_mode = "`[`方式`](https://zh.wikipedia.org/wiki/衰變方式 "wikilink")`",`
`   table_head_decay_energy = "`[`能量`](https://zh.wikipedia.org/wiki/衰變能量 "wikilink")<small>`（`[`MeV`](../Page/電子伏特.md "wikilink")`）`</small>`",`
`   table_head_decay_product = "`[`產物`](../Page/衰变产物.md "wikilink")`",`
...
end function static_funcs.decay_data_and_newline(mode, ref)

`   return "\n| |" .. (mode or ' ') .. (ref or ' ') .. "\n| style=\"text-align:right;\" | "`

end function static_funcs.check_natural_abundance(str)
...
\--產生表格頭部 function p.create_table_head(frame)
...
\--穩定同位素資料列 function p.stable_table(frame)
...
\--產生適合有有衰敗能量資訊表格的穩定同位素資料列 function p.stable_table_with_energy(frame)
...
\--產生有衰敗能量資訊的資料列 function p.table_with_energy(frame)
...
\--不適合模板包含的呼叫方式 function p.gen_table(frame)
...
\--產生同位素資料列主函數 function p._gen_table(args, is_stable, with_energy)
...
`   end`
`   --檢查是否有錯誤的元素符號`
`   if (not check_flag) and mw.title.getCurrentTitle().namespace == 0 then body = body .. "" end`
`   `
...
\--找出衰變資訊 function p.get_decay_list(args)
...
`                       decay_list.err_msg = decay_list.err_msg .. err_msg.error{ "參數編號“" .. deacy_number .. '”重複使用。' } .. " `
` "`
`                   end`
`               end`

> 本文内容由[模块:JMBUS](https://zh.wikipedia.org/wiki/模块:JMBUS)转换而来。


\-- 声明模块本体 local p = {} -- 导入数据 local c = mw.loadData('Module:JMBUS/color') local function _loadSystemData(loc)

`   local state`
`   if loc == nil or loc == "" or loc == "江门" or loc == "江門" then`
`       state, system = pcall(mw.loadData, "Module:JMBUS/data")`
`   --elseif loc == "佛山" then`
`   --  state, system = pcall(mw.loadData, "Module:JMBUS/foshan")`
`   --elseif loc == "中山" then`
`   --  state, system = pcall(mw.loadData, "Module:JMBUS/zhongshan")`
`   --elseif loc == "珠海" then`
`   --  state, system = pcall(mw.loadData, "Module:JMBUS/zhuhai")`
`   else`
`       state, system = pcall(mw.loadData, "Module:JMBUS/data")`
`   end`
`   return system`
...
`   if d[no] == nil then`
`       a = `[`width="1%"`](https://zh.wikipedia.org/wiki/ "wikilink")`..no..`[](https://zh.wikipedia.org/wiki/''' "wikilink")`..'`[`Module:JMBUS中未包含这条`](https://zh.wikipedia.org/wiki/Module:JMBUS "wikilink")[`"..loc.."的线路`](https://zh.wikipedia.org/wiki/'..loc.."巴士路线列表 "wikilink")`）''"`
...
`   local bb = frame.args`
`   local loc = bb.loc`
`   if loc == "" or loc == nil then loc = "江门" end`

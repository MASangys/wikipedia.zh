...
`       obj.link = mw.ustring.format( '`[`%s`](https://zh.wikipedia.org/wiki/%s "wikilink")`', obj.article, obj.label )`
`   elseif obj.article then`
`       obj.link = mw.ustring.format( '`[`%s`](https://zh.wikipedia.org/wiki/%s "wikilink")`', obj.article )`
`   end`
`   `

...
`               --如果數字很多建立章節 (用`[`#數字可跳到的章節`](https://zh.wikipedia.org/wiki/#數字 "wikilink")`)`
...
`                   --質因數分解只有一個元素的情況`
`                   if number == 0 then`
`                       body = body .. '*`[`中性數`](../Page/0.md "wikilink")`。\n' `
`                   elseif number == 1 then`
`                       body = body .. '*`[`單位數`](https://zh.wikipedia.org/wiki/单位數字 "wikilink")`。\n'`
`                   elseif number == -1 then`
`                       body = body .. '*`[`負數`](https://zh.wikipedia.org/wiki/負數 "wikilink")[`單位`](https://zh.wikipedia.org/wiki/单位數字 "wikilink")`。\n' `
`                   else`
`                       --是個質數`
`                       body = body .. '*'`
...
`                       end`
`                       body = body .. '`[`質數`](https://zh.wikipedia.org/wiki/質數 "wikilink")`。\n' `
`                       `
`                       if p.PrimeTable.table_max == nil then p.PrimeTable = require('Module:Factorization') end`
`                       --查質數表，找前一個和下一個質數`
...
`                               body = body .. "`[`孪生素数`](../Page/孪生素数.md "wikilink")`，" `
...
`                   local div_string = ''`
`   `
`                   body = print_information(body, number, "合数", "合數", nil, true, '*', (args["因數說明h"] or "`[`正因數有`](../Page/因數.md "wikilink")`") .. `
...
`                               body = body .. '`[`亏数`](https://zh.wikipedia.org/wiki/亏数 "wikilink")`'`
...
`                               body = body .. '`[`过剩数`](../Page/过剩数.md "wikilink")`'`
...
`                           end`
`                           body = body .. '，`[`真因數和為`](../Page/除數函數.md "wikilink")`' .. tostring(div_sum) .. tail_str .. '\n'`
`                       else`
...
`       end`
`   end `
`   if high_time_complexity == true then body = body .. '' end`
`   return mw.text.trim(body)`

end

\--用於少量數字(十幾個) 性質輸出 (支援客製化) function p.manyNumberInformation(frame)
...
`   if _flag_use_user_define_string == true then body = body .. "" end`
`   return body`
...
`   local body = ''`
`   body = '*' .. tostring(number) .. '是一個`[`实数`](../Page/实数.md "wikilink")`。\n'`
`   if number < 0 then`
`       body = body .. '*' .. '`[`负数`](../Page/负数.md "wikilink")`。\n'`
`   else`
`       body = body .. '*' .. '`[`正數`](https://zh.wikipedia.org/wiki/正數 "wikilink")`。\n'`
`   end`
`   if math.abs(math.floor(1 / number) - 1 / number)  <= 1e-15 then -- 要求超高精度`
`       body = body .. '*' .. tostring(1 / number) .. '的`[`倒数`](../Page/倒数.md "wikilink")`。\n'`
`   end`
...
`       `
`   body = '*' .. complex_str .. '是一個`[`複數`](../Page/复数_\(数学\).md "wikilink")`。\n'`
`   if numdata._is_integer(real) and numdata._is_integer(imag) then`
`       body = body ..'*' .. '`[`高斯整數`](../Page/高斯整數.md "wikilink")`。\n'`
...
`       body = body ..'*' .. '`[`平方數`](https://zh.wikipedia.org/wiki/平方數 "wikilink")`，為' .. tostring(sq_str) .. '的平方。\n'`
`   end`

`   if numdata._is_integer(length) then`
`       body = body ..'*' .. '`[`絕對值為`](https://zh.wikipedia.org/wiki/絕對值 "wikilink")`' .. tostring(length) .. '。\n'`
`   else`
`       body = body ..'*' .. '`[`絕對值約為`](https://zh.wikipedia.org/wiki/絕對值 "wikilink")`' .. string.format("%.4f",length) .. '。\n'`
`   end`
...
\--用於單一數字性質輸出 (支援客製化) function p.singleNumberInformation(frame)
...
`       (not (_flag_mass_output == true)) then body = body .. "" end`
`   return body`

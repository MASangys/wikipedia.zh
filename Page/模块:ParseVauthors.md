> 本文内容由[模块:ParseVauthors](https://zh.wikipedia.org/wiki/模块:ParseVauthors)转换而来。
...
`   local args = frame:getParent().args -- This gets the arguments passed to the template that called #invoke, so we don't need to pass them all through to the module. See also `[`Module:Arguments`](https://zh.wikipedia.org/wiki/Module:Arguments "wikilink")`.`

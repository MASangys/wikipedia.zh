...
` 4. 在標題中使用`<ref>
` 5. 標題沒有文字`
` 6. 标题中使用了`[`{{^}}这样的变态链接`](https://zh.wikipedia.org/wiki/User:Example "wikilink")
...
`   -- 展開連結`
`   r = string.gsub(r, '%[%`[`^%`](https://zh.wikipedia.org/wiki/^% "wikilink")`-%|(.-)%]%]', '%1')`
`   r = string.gsub(r, '%[%[:?(.-)%]%]', '%1')`
`   r = string.gsub(r, '%`[`^%`](https://zh.wikipedia.org/wiki/^% "wikilink")`-% (.-)%]', '%1')`
...
end function z.encodetitle(text)

`   return encodetitle(mw.getCurrentFrame(),text)`

end --便于其他模块借用此函数 function z.list( frame )

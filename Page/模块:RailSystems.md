> 本文内容由[模块:RailSystems](https://zh.wikipedia.org/wiki/模块:RailSystems)转换而来。
...
`   `
`   -- stationLink test`
`   if fp.stationLink{args={name='宁波站', system='UseCase'}} ~= '`[`宁波站`](../Page/宁波站.md "wikilink")`' then`
...
`   `
`   -- lineTitle test`
`   if p.lineTitle{args={name='1', system='UseCase'}} ~= "`**[`1号线`](../Page/宁波轨道交通1号线.md "wikilink")**`" then`
`       error('lineTitle test failed: name=1.')`
`   end`
`   if p.lineTitle{args={name='WRL', system='UseCase'}} ~= "`**[`西鐵綫`](../Page/西鐵綫.md "wikilink")**`" then`
...
[Category:需要去除时间判断模板的页面](https://zh.wikipedia.org/wiki/Category:需要去除时间判断模板的页面 "wikilink")
> 本文内容由[模块:Release timeline](https://zh.wikipedia.org/wiki/模块:Release_timeline)转换而来。
...
`   return frame:extensionTag{ name = 'templatestyles', args = { src = 'Release timeline/styles.css'} } .. tostring(p._main(args))`
...
`       :addClass('rt_caption')`
`       :addClass('nowrap')`
`       :wikitext((args.title or '发行时间轴')..(args.subtitle and ('`

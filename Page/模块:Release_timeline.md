...
`   builder =`
`       builder:tag("th"):attr("scope", "row"):css("border-right", "1.4em solid " .. color(args, year, itemNum)):wikitext(`
`       year`
`   )`
`   if itemNum > 1 then`
...
`   if itemNum == 1 then`
`       builder:tag("td"):wikitext(itemList[1])`
`       return`
`   end`

`   -- if itemNum >= 2`
`   builder:tag("td"):addClass("rt_first"):wikitext(itemList[1])`

`   for key = 2, itemNum - 1 do`
`       builder = builder:tag("tr"):tag("td"):addClass("rt_next"):wikitext(itemList[key])`
`   end`

`   builder = builder:tag("tr"):tag("td"):addClass("rt_last"):wikitext(itemList[itemNum])`
...
`   ret =`
`       mw.html.create("table"):addClass(`
`       args.align == "left" and "wikitable release_timeline rt_left" or "wikitable release_timeline"`
`   )`

`   ret:tag("caption"):addClass("rt_caption"):addClass("nowrap"):wikitext(`
`       (args.title or "发行时间轴") ..`
`           (args.subtitle and ('`

<div class="rt_subtitle">

...
`   --`[`This``   ``function``   ``is``   ``passed``   ``the``
 ...
 ``the``   ``status``   ``of``   ``the``
 ``route.`](https://zh.wikipedia.org/wiki/This_function_is_passed_the_dates_given_for_the_established_and_decommissioned_fields_to_the_template._It_then_returns_the_entry_in_the_routeStates_table_corresponding_to_the_status_of_the_route. "wikilink")`--`
...
`   -- This function is passed the length fields from the `` invocation, and calculates the missing length from the other.`
...
`   local km = args["length_km"] -- The kilometer length from the `` call.`
`   local mi = args["length_mi"] -- The length in miles as passed to ``.`
...
`   -- A limited replacement for ``. This is passed a date and derives a sort key from it. It returns a string with the hidden sort key, along with the year of the original date.`
...
`       return dtsSpan .. "`<abbr title=\"circa\">`c.`</abbr><span style=\"white-space:nowrap;\">` " .. year .. "`</span>`" -- Add the circa abbreviation to the display. Derived from `
`   else -- Otherwise,`
...
`       if routeNum then`
`           routeKey = string.format('%04d', route) -- This invocation is equivalent to the `` template. It zero-pads the given route number up to 4 digits.`
`       else`
...
`           banner = format("`[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")`", bannerFile)`
`       elseif widthCode == 'expand' then`
`           local route = args.route`
`           if #route >= 3 then`
`               banner = format("`[`No_image.svg`](https://zh.wikipedia.org/wiki/File:No_image.svg "fig:No_image.svg")[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")[`No_image.svg`](https://zh.wikipedia.org/wiki/File:No_image.svg "fig:No_image.svg")`", bannerFile)`
`           else`
`               banner = format("`[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")`", bannerFile)`
`           end`
...
`           banner = format("`[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")[`No_image.svg`](https://zh.wikipedia.org/wiki/File:No_image.svg "fig:No_image.svg")`", bannerFile)`
...
`           local shield1 = format("`[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")`", shieldFile)`
`           local shield2 = format("`[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")`", second)`
`           shield = shield1 .. shield2`
`       else`
`           shield = shieldFile and format("`[`%s`](https://zh.wikipedia.org/wiki/File:%s "fig:%s")`", shieldFile) or ''`
`       end`
`   else`
`       shield = ''`
`   end`

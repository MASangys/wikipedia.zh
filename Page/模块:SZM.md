...
\-- 模板原数据 p.displayLineNameFormat = {

`   l = [=`[`%d号线`](https://zh.wikipedia.org/wiki/[深圳地铁%d号线 "wikilink")`]=], --仅链接`
`   lc = [=`[<span style="color:#%s;">`%d号线`</span>](https://zh.wikipedia.org/wiki/[深圳地铁%d号线 "wikilink")`]=], --带色彩链接`
`   tc = [=[`<span style="color:#%s;">`%d号线`</span>`]=], --带色彩文本`
`   c = [=[`<span style="color:#%s;">`■ `</span>[`%d号线`](https://zh.wikipedia.org/wiki/深圳地铁%d号线 "wikilink")`]=], --带色块链接`
`   t = [=[%d号线]=], --纯文本`
`   i = [=`[<span title="可-{zh-hans:换乘; zh-hant:轉乘}-%d号线" style="color:#%s">`■`</span>](https://zh.wikipedia.org/wiki/[深圳地铁%d号线 "wikilink")`]=], --换乘站`
...
\-- 替换原版模板函数，参数1=线路名称，参数2=控制符，返回=输出结果 function p._szmTemplate(args)
...
\-- 替换原版模板函数，参数1=线路名称，参数2=控制输出'\#'前缀，返回=色彩代码 function p._getcolor(args)
...
\-- 替换原版模板函数，参数1=线路名称，返回=线路数字 function p._parsing(args)

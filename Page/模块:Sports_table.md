> 本文内容由[模块:Sports table](https://zh.wikipedia.org/wiki/模块:Sports_table)转换而来。
...
\-- Helper functions local function isnotblank(s)
...
`       status_string = '`
`'..mw.ustring.sub(status_string,1,mw.ustring.len(status_string)-2)..'.'`

...
\-- Set constants local DATA_MODULE = 'Module:Template test case/data'
...
`   if display then`
`       return string.format('`[`%s`](https://zh.wikipedia.org/wiki/:%s "wikilink")`', self:getFullPage(), display)`
`   else`
`       return string.format('`[`:%s`](https://zh.wikipedia.org/wiki/:%s "wikilink")`', self:getFullPage())`
`   end`

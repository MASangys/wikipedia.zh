> 本文内容由[模块:UnitTests](https://zh.wikipedia.org/wiki/模块:UnitTests)转换而来。
...
`   end`
`   local nowiki_open = (options and options.nowiki) and '' or ''`
`    local nowiki_close = (options and options.nowiki) and '' or ''`
`   local differs_at = self.differs_at and (' || ' .. first_difference(expected, actual)) or ''`
`   result_table = result_table .. ' || ' .. text:gsub('%|', '|') .. ' || ' .. nowiki_open .. expected .. nowiki_close .. ' || ' .. nowiki_open .. actual .. nowiki_close .. differs_at .. "\n|-\n"`
...
`    local nowiki_close = (options and options.nowiki) and '' or ''`
`   local differs_at = self.differs_at and (' || ' .. first_difference(expected, actual)) or ''`
`   result_table = result_table .. ' || ' .. text1:gsub('%|', '|') .. ' || ' .. nowiki_open .. expected .. nowiki_close .. ' || ' .. nowiki_open .. actual .. nowiki_close .. differs_at .. "\n|-\n"`

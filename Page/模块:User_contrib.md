...
`       args = getArgs(frame)`
`   end`
`   -- in the current template there is an {{#iferror: {{#expr: `` }}. So I need to do a check to make sure the argument supplied is actually a number.`
...
`   `
`   local language = ''`
`   local project = args['project'] or '维基百科'`
`   local project_site = args['projsite'] or 'zh.wikipedia.org'`
`   local user = '用户'`
...
`   if args['deleted'] then deleted     = '，其中已删除编辑超过`<b>`' ..formatNumber(args['deleted'])..'`</b>`次' end`
`   if args['articles'] then articles   = '，其中条目编辑超过`<b>`' ..formatNumber(args['articles'])..'`</b>`次' end`
`   if args['automated'] then automated = '，其中自动编辑超过`<b>`' ..formatNumber(args['automated'])..'`</b>`次' end`
`   if args['distinct'] then distinct   = '，涵盖至少`<b>`' ..formatNumber(args['distinct'])..'`</b>`个不同页面' end`
`   if args['unique'] then unique       = '，涵盖至少`<b>`' ..formatNumber(args['distinct'])..'`</b>`个不同页面' end`
`   if args['images'] then images       = '，其中上传至少`<b>`' ..formatNumber(args['images'])..'`</b>`张上传图像' end`
`   if args['insane'] then insane       = '，似乎是名编辑狂' end`
`   if args['bot']    then user     = '机器人' end`
`   if args['link']    then url     = args['link'] end`
`   `
...
`   user_args['id'] = (args['id'] or formated_count) .. '+'`
`   user_args['info-s']    = 9`
`   user_args['id-s']      = mw.ustring.len(user_args['id']) <= 4 and 12 or (mw.ustring.len(user_args['id']) <= 6 and 10 or 9)`
`   `
`   if args['log'] == 'yes' then`
`       url = '`<https://zh.wikipedia.org/w/index.php?title=Special:Log&user=>`'..urlencode(username)`
`       user_args['info'] = '`<span class="plainlinks neverexpand">`这位用户在' .. language.. project .. '上，有['..url..' 至少'.. '`<b>`'..formated_count..'`</b>`'..`
`       ' 笔移动或其他日志]'`
`       .. automated`
`       .. deleted`
`       .. articles`
...
`       .. '.`</span>`'`
`   else`
`       user_args['info'] = '`<span class="plainlinks neverexpand">`这位'`
`       .. user`
`       .. '在' .. language .. project`
`       .. '['`
`       .. url`

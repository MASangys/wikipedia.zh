...
`           '{{`[`val`](https://zh.wikipedia.org/wiki/Template:val "wikilink")`}}错误：' ..`
`           msg ..`
`           '`</strong>`'`
`   end`
...
`       local description`
`       if which == 'e' then`
`           description = '指数参数（`<b>`e`</b>`）'`
`       else`
`           description = '参数' .. which`
`       end`
`       return description .. (msg or '非有效数字') .. '。'`
`   end`
`   local result = {}`
...
`       return '在最后一个参数后需要一个数字，因为它是一个范围。'`
`   end`
...
`   else`
`       if want_link then`
`           result.text = '`[`'``   ``..``   ``result.symbol``   ``..``   ``'`](https://zh.wikipedia.org/wiki/'_.._result.link_.._' "wikilink")`'`
`       else`
...
`                   text = unit.symbol`
`               else`
`                   text = '`[`'``   ``..``   ``unit.display``   ``..``   ``'`](https://zh.wikipedia.org/wiki/'_.._unit.link_.._' "wikilink")`'`
`               end`
...
local delimit_groups = require('Module:Gapnum').groups local function delimit(sign, numstr, fmt)
...
`       end`
`       return valerror('无效动作"' .. action .. '"。', options.nocat)`
`   end`
...
`       return valerror('同时输入了单位（`<b>`u`</b>`）和有链接单位（`<b>`ul`</b>`），只允许输入其中一个。', nocat)`
`   end`
`   if args.up and args.upl then`
`       return valerror('同时输入了每单位（`<b>`up`</b>`）和有链接每单位（`<b>`upl`</b>`），只允许输入其中一个。', nocat)`
`   end`
`   local values`
...
[Category:带有错误格式模板的页面](https://zh.wikipedia.org/wiki/Category:带有错误格式模板的页面 "wikilink")
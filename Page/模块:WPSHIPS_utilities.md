...
`       used in `` to provide a value for `` and to provide a value for |infobox caption=`
...
`   |sclass=2 – for ship classes only; displays class name without italics (parameter name is loosely similar to `` which does the same thing); use when |infobox caption=class`
...
`   local err_msg = '%s`<span style="font-size:100%%" class="error">`列表错误：%s（`[`帮助`](https://zh.wikipedia.org/wiki/Category:WPSHIPS:Infobox_list_errors "wikilink")`）`</span>`%s%s%s';`
`   local category = '';`
...
`           return list_error ('', 'mixed text and list', '`
`', frame.args[1], showerrs);   -- return an error message with maintenance category`
`       end`
...
`   local level = 1;                                                            -- used to indicate when a new `
...
`   `
`   table.insert (html_table, '`

') -- this for first

tag; sets no bullets and no indent
...
`           table.insert (html_table, List_item_otag .. item .. '`
...
until level counted down to zero
...
[Category:WPSHIPS:Infobox_list_errors](https://zh.wikipedia.org/wiki/Category:WPSHIPS:Infobox_list_errors "wikilink")
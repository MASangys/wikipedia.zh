...
`   if args.width then root:wikitext('') end`
`   `
...
`           root:wikitext("`**`《"``   ``..``   ``args.title``   ``..``
 ``"》每集"``   ``..``   ``((countryDisplayUS``   ``or``
 ``countryDisplayUK``   ``or``   ``countryDisplayOther)``   ``or``
 ``"")``   ``..``   ``"收視人數（"``   ``..``   ``multiple``   ``..``
 ``"）`**`")`
`       else`
...
`       span:css( 'color', 'red' )`
`           :wikitext(frame:expandTemplate{ title='dummy reference', args={ '`[`来源请求`](https://zh.wikipedia.org/wiki/Wikipedia:来源请求 "wikilink")`', txtital = 'y' } })`
`        `
`   if countryDisplayUS then`
`       root:wikitext("`<small>`收視觀眾人數數據來源於美國`[`尼爾森收視率調查`](https://zh.wikipedia.org/wiki/尼爾森收視率 "wikilink")`。`</small>`" .. (args.refs or tostring(span)))`
`   elseif countryDisplayUK then`
`       root:wikitext("`<small>`收視觀眾人數數據來源於英國`[`廣播受眾研究會`](https://zh.wikipedia.org/wiki/廣播受眾研究會 "wikilink")`。`</small>`" .. (args.refs or tostring(span)))`
`   else`
...
[Category:包含寬度參數電視收視率圖的頁面](https://zh.wikipedia.org/wiki/Category:包含寬度參數電視收視率圖的頁面 "wikilink")
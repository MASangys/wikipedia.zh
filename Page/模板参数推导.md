**模板参数推导**（template argument deduction），是在调用[C++](../Page/C++.md "wikilink")的[模板函数时](../Page/模板_\(C++\).md "wikilink")，由[编译器根据使用上下文来推断所调用的模板函数的模板参数](https://zh.wikipedia.org/wiki/编译器 "wikilink")。这一概念也适用于类的模板成员函数。
...
模板函数在定义时，用`template<>`声明模板参数（或称模板形参）。调用模板函数时，可以在函数名字后用`< >`显式指出模板参数（这时称作模板实参）。例如：
...
说明：
  ...
  - TT表示[模板的模板参数](../Page/模板的模板参数.md "wikilink")（template template argument）\[1\]
...
另外，C++11标准明确规定不能由模板参数推导出对应实参为[std::initializer_list的类型](https://zh.wikipedia.org/wiki/initializer_list "wikilink")。<ref>§ 14.8.2.5/5 of the C++11 standard explicitly states that this is a non-deduced context for a template argument:
...
C++11增加了[右值引用](../Page/右值引用.md "wikilink")这一新的数据类型。如：`template`<class T>`  void foo(T&& arg); `“T&&”并不意味着形参arg的数据类型一定是[右值引用](../Page/右值引用.md "wikilink")。其数据类型既可能是左值引用，也可能是右值引用。依据“引用塌缩规则”，有：
...
1.  模板的模板参数，例如：Stack\<int,std::vector<int> \> vStack; // integer stack that uses a vector as internal container
...
deduction），是在调用[C++的](../Page/C++.md "wikilink")[模板函数时](../Page/模板_\(C++\).md "wikilink")，由[编译器根据使用上下文来推断所调用的模板函数的模板参数](https://zh.wikipedia.org/wiki/编译器 "wikilink")。这一概念也适用于类的模板成员函数。
...
C++11增加了[右值引用这一新的数据类型](../Page/右值引用.md "wikilink")。如：`template`<class T>`
void foo(T&&

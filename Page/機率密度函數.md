...
.\]\]
在[数学中](../Page/数学.md "wikilink")，[连续型随机变量的](https://zh.wikipedia.org/wiki/连续随机变量 "wikilink")**概率密度函數**（Probability
density
function）（在不至于混淆时可以简称为**密度函数**）是一个描述这个随机变量的输出值，在某个确定的取值点附近的可能性的[函数](../Page/函数.md "wikilink")。圖中，橫軸為隨機變量的取值，縱軸為概率密度函數的值，而随机变量的取值落在某个区域内的概率為概率密度函数在这个区域上的[积分](../Page/积分.md "wikilink")。当概率密度函数存在的时候，[累積分佈函數是概率密度函数的积分](https://zh.wikipedia.org/wiki/累積分佈函數 "wikilink")。概率密度函数一般以大写“”（）標记\[1\]。

...
zh-hant:
互質}-](https://zh.wikipedia.org/wiki/互素 "wikilink")（即\(\gcd(a,n)=1\)），则
...
小的正整数中所有与\(n\)
互素的数对应的[同余类组成的集合](https://zh.wikipedia.org/wiki/同余 "wikilink")（这个集合也称为模*n*
的[简化剩余系](https://zh.wikipedia.org/wiki/简化剩余系 "wikilink")）。这些同余类构成一个群，称为**[整数模n乘法群](https://zh.wikipedia.org/wiki/整数模n乘法群 "wikilink")**。因为此群阶为\(\varphi(n)\)，所以\(a^{\varphi(n)} \equiv 1 \pmod n\)。

**歐拉-拉格朗日方程**（）為[變分法中的一條重要方程](https://zh.wikipedia.org/wiki/變分法 "wikilink")。它提供了求[泛函的臨界值](../Page/泛函.md "wikilink")（平穩值）函數，換句話說也就是求此泛函在其定義域的[臨界點的一個方法](https://zh.wikipedia.org/wiki/臨界點 "wikilink")，與微積分差異的地方在於，泛函的定義域為函數空間而不是
\(\mathbb{R}^n\)。

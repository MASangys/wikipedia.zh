...
在數學裏，存在有各種各樣的正交座標系。應用二維[直角座標系](https://zh.wikipedia.org/wiki/直角座標系 "wikilink")\((x,\ y)\)的[共形映射方法](../Page/共形映射.md "wikilink")，可以簡易的生成這些正交座標系。一個複數\(z=x+iy\)的任何[全純函數](https://zh.wikipedia.org/wiki/全純函數 "wikilink")\(w=f(z)\)，其複值的導數，如果不等於零，則會造成一個[共形映射](../Page/共形映射.md "wikilink")。如果答案可以表達為\(w=u+iv\)，則\(u\)與\(v\)的等值曲線以直角相交，就如同原本的\(x\)與\(y\)的等值曲線以直角相交。
...
[Nabla_spherical2.svg](https://zh.wikipedia.org/wiki/File:Nabla_spherical2.svg "fig:Nabla_spherical2.svg")

在[哈密頓力學裏](https://zh.wikipedia.org/wiki/哈密頓力學 "wikilink")，當計算[正則變換時](../Page/正則變換.md "wikilink")，**生成函數**扮演的角色，好似在兩組[正則坐標](https://zh.wikipedia.org/wiki/正則坐標 "wikilink")
\((\mathbf{q},\ \mathbf{p})\) ，\((\mathbf{Q},\ \mathbf{P})\)
之間的一座橋。為了要保證正則變換的正確性
，採取一種間接的方法，稱為**生成函數方法**。這兩組變數必須符合方程式
...
其中，\(\mathbf{q}=(q_1,\ q_2,\ \dots,\ q_N)\)
是舊[廣義坐標](https://zh.wikipedia.org/wiki/廣義坐標 "wikilink")，\(\mathbf{p}=(p_1,\ p_2,\ \dots,\ p_N)\)
是舊[廣義動量](../Page/廣義動量.md "wikilink")，\(\mathbf{Q}=(Q_1,\ Q_2,\ \dots,\ Q_N)\)
是新廣義坐標，\(\mathbf{P}=(P_1,\ P_2,\ \dots,\ P_N)\)
是新廣義動量，\(\mathcal{H}(\mathbf{q},\ \mathbf{p},\ t),\ \mathcal{K}(\mathbf{Q},\ \mathbf{P},\ t)\)
分別為舊[哈密頓量與新哈密頓量](https://zh.wikipedia.org/wiki/哈密頓量 "wikilink")，\(G(-,\ -,\ t)\)
是**生成函數**，\(t\) 是時間。
...
第一型生成函數 \(G_{1}\)
只跟舊[廣義坐標](https://zh.wikipedia.org/wiki/廣義坐標 "wikilink")、新廣義坐標有關，

\[G=G_{1}(\mathbf{q},\ \mathbf{Q},\ t)\] 。

代入方程式 (1)
。展開生成函數對於時間的[全導數](https://zh.wikipedia.org/wiki/全導數 "wikilink")，
...
新廣義坐標 \(\mathbf{Q}\) 和舊廣義坐標 \(\mathbf{q}\) 都是自變量，其對於時間的全導數
\(\dot{\mathbf{Q}}\) 和 \(\dot{\mathbf{q}}\) 互相無關，所以，以下 \(2N+1\)
個方程式都必須成立：
...
在理想情況下，這些方程式可以逆算出 \(\mathbf{Q}\) 的 \(N\)
個函數方程式
...
第二型生成函數 \(G_{2}\) 只跟舊廣義坐標 \(\mathbf{q}\)
、新[廣義動量](../Page/廣義動量.md "wikilink") \(\mathbf{P}\)
有關 ：
...
第四型生成函數 \(G_{4}(\mathbf{p}, \mathbf{P}, t)\) 只跟舊廣義動量 \(\mathbf{p}\)
、新廣義動量 \(\mathbf{P}\) 有關：
...
有時候，可以將一個給定的哈密頓量，變成一個很像[諧振子的哈密頓量](../Page/諧振子.md "wikilink")，

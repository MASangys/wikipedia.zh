...
error）是將[方均根差化正規化後所得的統計數值](../Page/方均根差.md "wikilink")，其正規化的方法有很多種。
例如:在[信號處理中](../Page/信號處理.md "wikilink")，我們會以其訊號強度來正規化方均差。

`    在`[`影像處理中`](../Page/影像處理.md "wikilink")`，給定兩個影像矩陣 `\(x[m,n]\)`和`\(y[m,n]\)`，其正規化方均根差可被定義如下：`

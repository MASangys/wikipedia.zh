在[量子力學裏](https://zh.wikipedia.org/wiki/量子力學 "wikilink")，表達[粒子的](https://zh.wikipedia.org/wiki/粒子 "wikilink")[量子態的](../Page/量子態.md "wikilink")[波函數必須滿足](https://zh.wikipedia.org/wiki/波函數 "wikilink")**歸一條件**（**歸一化**，），也就是說，在空間內，找到粒子的[機率必須等於](https://zh.wikipedia.org/wiki/機率 "wikilink")
\(1\)
。這性質稱為**歸一性**。用[數學公式表達](https://zh.wikipedia.org/wiki/數學 "wikilink")，
...
一般而言，波函數 \(\psi\)
是一個[複函數](https://zh.wikipedia.org/wiki/複數 "wikilink")。可是，\(\psi^* \psi =  \mid \psi  \mid ^2\)
是一個[實函數](https://zh.wikipedia.org/wiki/實數 "wikilink")，大於或等於 \(0\)
，稱為「機率密度函數」。所以，在區域 \([x,\ x+\Delta x]\) 內，找到粒子的機率
\(\Delta P\) 是
...
\[P= \int_{ - \infty}^{\infty} \mid \psi  \mid ^2 dx = 1\]
。<span style="position:absolute;right:15%">(2)</span>

假若，從解析[薛丁格方程而得到的波函數](https://zh.wikipedia.org/wiki/薛丁格方程 "wikilink")
\(\psi\) ，其機率 \(P\) 是有限的，但不等於 \(1\) ，則可以將波函數 \(\psi\) 乘以一個常數，使機率 \(P\)
等於 \(1\) 。或者，假若波函數內，已經有一個任意常數，可以設定這任意常數的值，使機率 \(P\) 等於 \(1\)。
...
其中，\(k\) 是[波數](../Page/波數.md "wikilink")，\(\omega\)
是[角頻率](https://zh.wikipedia.org/wiki/角頻率 "wikilink")，\(A\)
是任意常數。
...
其中，\(\hbar\)
是[約化普朗克常數](https://zh.wikipedia.org/wiki/約化普朗克常數 "wikilink")，\(V(x)\)
是[位勢](https://zh.wikipedia.org/wiki/位勢 "wikilink")，\(E\)
是[能量](../Page/能量.md "wikilink")。
...
一個表達粒子量子態的波函數，必須滿足粒子的薛丁格方程。既然 \(\psi\) 和 \(\psi\,'\)
都能夠滿足同樣的薛丁格方程，它們必定都表達同樣的量子態。假若不使用歸一化的波函數，則只能知道機率的相對大小；否則，使用歸一化的波函數，可以知道絕對的機率。這對於量子問題的解析，會提供許多便利。

## 歸一化恆定性

給予一個歸一化的波函數．隨著時間的變化，波函數也會改變．假若，隨著時間改變的波函數不再滿足歸一條件，則勢必要重新將波函數歸一化．這樣，歸一常數
\(A\) 變得含時間．很幸運地，滿足薛丁格方程的波函數的歸一性是恆定的．設定波函數 \(\psi(x,\ t)\) 滿足薛丁格方程與歸一條件：
...
## 外部連結
  ...
  - [Middlebury
    大學講義：歸一化](https://web.archive.org/web/20120207042727/http://cat.middlebury.edu/~chem/chemistry/class/physical/quantum/help/normalize/normalize.html)

[en:Normalizable wave
function](https://zh.wikipedia.org/wiki/en:Normalizable_wave_function "wikilink")

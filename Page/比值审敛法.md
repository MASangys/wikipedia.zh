> 本文内容由[比值审敛法](https://zh.wikipedia.org/wiki/比值审敛法)转换而来。


**比值审敛法**是判别[级数](../Page/级数.md "wikilink")敛散性的一种方法，又称为**[达朗贝尔判别法](https://zh.wikipedia.org/wiki/达朗贝尔 "wikilink")**（）。

## 定理

设\(\sum_{n=1}^\infty u_n\)为正项级数，其中每一項皆為非 0 的實數或複數，如果

\(\lim_{n \to \infty}\left|\frac{u_{n+1}}{u_n}\right|=\rho\)，

  - 当ρ\<1时级数收敛
  - 当ρ\>1时级数发散
  - 当ρ=1时级数可能收敛也可能发散。

## 例子

### 收敛

考虑级数

\[\sum_{n=1}^\infty\frac{n}{e^n}\]

\[\begin{align}
   \lim_{n\to\infty} \left| \frac{a_{n+1}}                  {a_n}                   \right|
&= \lim_{n\to\infty} \left| \frac{\frac{n+1}{e^{n+1}}}      {\frac{n}{e^n}}         \right|\\
&= \lim_{n\to\infty} \left|       \frac{n+1}{e^{n+1}}  \cdot \frac{e^n}{n}          \right|\\
&= \lim_{n\to\infty} \left|       \frac{n+1}{n}        \cdot \frac{e^n}{e^n\cdot e} \right|\\
&= \lim_{n\to\infty} \left| \left(1+\frac{1}{n}\right) \cdot \frac{1}{e}            \right|\\
&= 1\cdot\frac{1}{e} = \frac{1}{e} < 1.
\end{align}\]

因此该级数收敛。

### 发散

考虑级数

\[\sum_{n=1}^\infty\frac{e^n}{n}\]

  -
    {| border="0" cellpadding="0" cellspacing="0"

|\(\lim_{n\rightarrow\infty}\left|\frac{a_{n+1}}{a_n}\right|\) |=\(\lim_{n\rightarrow\infty}\left|\frac{\frac{e^{n+1}}{n+1}}{\frac{e^n}{n}}\right|\) |- | |=\(\lim_{n\rightarrow\infty}\left|\frac{e^{n+1}}{n+1}\cdot\frac{n}{e^n}\right|\) |- | |=\(\lim_{n\rightarrow\infty}\left|\frac{n}{n+1}\cdot\frac{e^n\cdot e}{e^n}\right|\) |- | |=\(\lim_{n\rightarrow\infty}\left|(1-\frac{1}{n+1})\cdot e\right|\) |- | |=\(1\cdot e\) |- | |=\(\!\, e (>1)\) |}

因此该级数发散。

### 不能确定

级数

\[\sum_{n=1}^\infty 1\]

发散，但

\[\lim_{n\rightarrow\infty}\left|\frac{1}{1}\right| = 1.\]

而级数

\[\sum_{n=1}^\infty \frac{1}{n^2}\]

收敛，但

\[\lim_{n\rightarrow\infty}\left|\frac{\frac{1}{(n+1)^2}}{\frac{1}{n^2}}\right| = 1.\]

## 参见

  - [根值审敛法](../Page/根值审敛法.md "wikilink")
  - [比较审敛法](../Page/比较审敛法.md "wikilink")

[it:Criteri di convergenza\#Criterio del rapporto (o di d'Alembert)](https://zh.wikipedia.org/wiki/it:Criteri_di_convergenza#Criterio_del_rapporto_\(o_di_d'Alembert\) "wikilink")

[Category:级数](https://zh.wikipedia.org/wiki/Category:级数 "wikilink") [Category:审敛法](https://zh.wikipedia.org/wiki/Category:审敛法 "wikilink")
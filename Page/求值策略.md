...
“传值调用”求值是最常见的求值策略，[C和](https://zh.wikipedia.org/wiki/C语言 "wikilink")[Scheme这样差异巨大的语言都在使用](../Page/Scheme.md "wikilink")。在传值调用中实际参数被求值，其值被绑定到函数中对应的变量上（通常是把值复制到新内存区域）。如果函数或过程能把值赋给它的形式参数，则被赋值的只是局部拷贝——就是说，在函数返回后调用者作用域里的曾传给函数的任何东西都不会变。
...
Basic
.NET](../Page/Visual_Basic_.NET.md "wikilink")、[C\#和](https://zh.wikipedia.org/wiki/C_Sharp_\(programming_language\) "wikilink")[REALbasic默认使用传值调用](../Page/REALbasic.md "wikilink")，但是提供一种传引用的特别语法。
...
## 引用
  ...
  - [Henry G. Baker,
    Jr](https://zh.wikipedia.org/wiki/Henry_Baker_\(computer_scientist\) "wikilink").
    "[The Incremental Garbage Collection of

...
\[16 = 1 + 3 + 5 + 7\]
擴展為[數學的一般式](https://zh.wikipedia.org/wiki/數學 "wikilink")：若有\(n\)個數值\(x_1, x_2, \cdots, x_n\)，則此\(n\)個數值的總和為：

\[\Sigma = x_1 + x_2 + \cdots + x_n\]
上式的[等號右段在數學上常簡潔地寫為](https://zh.wikipedia.org/wiki/等號 "wikilink")：
...
\(\sum_{k=1}^n H_k p(k)=(\sum_{j=0}^m C_{n+1}^{j+1} \Delta^j p(0))H_n-\sum_{j=0}^m\frac{C_n^{j+1}}{j+1}\Delta^j p(0)\)，其中\(H_n\)為[調和數或](../Page/調和數.md "wikilink")[調和級數](https://zh.wikipedia.org/wiki/調和級數 "wikilink")
...
當\(f(x)\)在\[a,b\][單調遞減時](https://zh.wikipedia.org/wiki/單調遞減 "wikilink")：

**求和符号**（**Σ**，sigma），是[欧拉于](../Page/欧拉.md "wikilink")1755年首先使用的。这个符号是源于[希腊文](../Page/希腊文.md "wikilink")（增加）的字头，Σ正是σ的大写。求和的结果是給定的數值相[加後的總值](../Page/加.md "wikilink")，又稱**加總**。

舉例而言，若有4個數值：1、3、5、7，則這4個數值的總和為：

\[16 = 1 + 3 + 5 + 7\]
擴展為[數學的一般式](../Page/數學.md "wikilink")：若有\(n\)個數值\(x_1, x_2, \cdots, x_n\)，則此\(n\)個數值的總和為：

\[\Sigma = x_1 + x_2 + \cdots + x_n\]
上式的[等號右段在數學上常簡潔地寫為](../Page/等號.md "wikilink")：
...
1.  [裂項法](../Page/裂項法.md "wikilink")：利用\(a_k=b_{k+1}-b_k\)求出\(\sum_{k=m}^n a_k\)。
2.  錯位相減法：透過兩個求和式的相減化簡求和數列的求和方法。
3.  倒序求和：對於有[對稱中心的函數](../Page/對稱中心.md "wikilink")\(f(x)+f(2a-x)=2b\)首尾求和\[1\]\[2\]
4.  逐項求導：可從\(\displaystyle \sum_{k=0}^n x^k=\frac{x^{n+1}-1}{x-1}\)推導出\(\displaystyle \sum_{k=0}^n k^m x^k\)\[3\]
5.  [阿貝爾變換](../Page/阿貝爾變換.md "wikilink")：
...
\(\sum p(k)\)是對一個多項式求和，自然數方冪和、[等幂求和](../Page/等幂求和.md "wikilink")、等差數列求和都屬于對多項式求和。
  ...
  - [帕斯卡矩陣形式](../Page/帕斯卡矩陣.md "wikilink")
    :<math>\\sum_{k=1}^n p(k)=

\\begin{pmatrix}C_n^1 & C_n^2 & \\cdots & C_n^{m+1}\\end{pmatrix}
\\begin{pmatrix} C_0^0 & 0 & \\cdots & 0\\\\ -C_1^0 & C_1^1 & \\cdots
& 0\\\\ \\vdots & \\vdots & \\ddots & \\vdots\\\\ (-1)^mC_m^0 &
(-1)^{m-1}C_m^1 & \\cdots & C_m^m\\\\ \\end{pmatrix}
...
\(\sum_{k=1}^n H_k p(k)=(\sum_{j=0}^m C_{n+1}^{j+1} \Delta^j p(0))H_n-\sum_{j=0}^m\frac{C_n^{j+1}}{j+1}\Delta^j p(0)\)，其中\(H_n\)為[調和數或](../Page/調和數.md "wikilink")[調和級數](../Page/調和級數.md "wikilink")
...
范德蒙恒等式與[超幾何函數有關係](../Page/超幾何函數.md "wikilink")：

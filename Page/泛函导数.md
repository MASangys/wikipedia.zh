在[数学和理论](../Page/数学.md "wikilink")[物理中](https://zh.wikipedia.org/wiki/物理 "wikilink")，**泛函导数**是[方向导数的推广](../Page/方向导数.md "wikilink")。后者对一个有限维[向量求微分](../Page/向量.md "wikilink")，而前者则对一个连续函数（可视为无穷维向量）求微分。它们都可以认为是简单的一元[微积分中](https://zh.wikipedia.org/wiki/微积分 "wikilink")[导数的扩展](../Page/导数.md "wikilink")。数学里专门研究泛函导数的分支是[泛函分析](../Page/泛函分析.md "wikilink")。

## 定义

设有[流形](../Page/流形.md "wikilink") *M*
代表（[连续](../Page/連續函數_\(拓撲學\).md "wikilink")/[光滑](../Page/光滑函数.md "wikilink")/有某些[边界条件等的](https://zh.wikipedia.org/wiki/边界条件 "wikilink")）函数
φ 以及[泛函](../Page/泛函.md "wikilink") *F*：
...
用 \(\varphi\) 的[一次变分](https://zh.wikipedia.org/wiki/一次变分 "wikilink")
\(\delta\varphi\) 代替 \(f\) 就得到 \(F\) 的一次变分 \(\delta F\)；

在物理学中，通常用[狄拉克δ函数](../Page/狄拉克δ函数.md "wikilink")
\(\delta(x-y)\)，而不是一般的测试函数 \(f(x)\),
来求出点\(y\)处的泛函导数(这是整个泛函变分的关键点，就像[偏导数是](../Page/偏导数.md "wikilink")[梯度的一个分量](../Page/梯度.md "wikilink")):

  -
    <math>\\frac{\\delta F\[\\varphi(x)\]}{\\delta
    \\varphi(y)}=\\lim_{\\varepsilon\\to
    0}\\frac{F\[\\varphi(x)+\\varepsilon\\delta(x-y)\]-F\[\\varphi(x)\]}{\\varepsilon}.

</math>

这适用于\(F[\varphi(x)+\varepsilon f(x)]\) 可以展开成\(\varepsilon\)的级数時
(或者至少能展为1阶). 但是这一表达在数学上并不严格,因为
\(F[\varphi(x)+\varepsilon\delta(x-y)]\)一般而言并未定义。

## 正式表述

通过更仔细地定义[函数空间](../Page/拓撲向量空間.md "wikilink")，泛函导数的定义可以更准确、正式。例如，当函数空间是一个[巴拿赫空间时](../Page/巴拿赫空间.md "wikilink"),
泛函导数就是著名的[Fréchet导数](https://zh.wikipedia.org/wiki/Fréchet导数 "wikilink"),
而这在更一般的[局部凸空间上使用](https://zh.wikipedia.org/wiki/局部凸空间 "wikilink")[加托導數](../Page/加托導數.md "wikilink")。注意，著名的[希尔伯特空间是](../Page/希尔伯特空间.md "wikilink")[巴拿赫空间的特例](../Page/巴拿赫空间.md "wikilink")。更正式的处理允许将普通[微积分和](https://zh.wikipedia.org/wiki/微积分 "wikilink")[数学分析的定理推广为](../Page/数学分析.md "wikilink")[泛函分析中对应的定理](../Page/泛函分析.md "wikilink")，以及大量的新定理。
...
上面给出的定义是基于一种对所有[测量函数](https://zh.wikipedia.org/wiki/测量函数 "wikilink")
*f*都成立的关系，因此有人可能会想，它在
*f*是一个指定的函数（比如说[狄拉克δ函数](../Page/狄拉克δ函数.md "wikilink")）时也应该成立。但是，δ函数不是一个合理的测量函数。
...
其中第二行用到了  的[全微分](../Page/全微分.md "wikilink")， *ρ*
為標量對向量的導數。<ref group="Note">在三維[笛卡尔坐标系中](../Page/笛卡尔坐标系.md "wikilink")，
...
\end{align}\]</ref>
第三行則用到了[散度的積法則](https://zh.wikipedia.org/wiki/散度#性质 "wikilink")。第四行由[高斯散度定理及邊界上](../Page/高斯散度定理.md "wikilink")
0}} 的條件得到。由於
可以是任意的函數，由[變分法基本引理可知](../Page/變分法基本引理.md "wikilink")，所求泛函導數為
...
\\frac{\\delta F}{\\delta\\rho(\\boldsymbol{r})} = \\frac{\\partial
...
其中 *ρ* = *ρ*(****) 且  *f* (****}}, *ρ*, ∇*ρ*)。只要 \[*ρ*\]
具有本節首段的形式，上述公式就適用。對於其他的泛函形式，可由定義出發，求出其泛函導數。（見[库仑势能泛函](https://zh.wikipedia.org/wiki/泛函导数#库仑势能泛函 "wikilink")。)
...
其中向量 ，而  為一個張量，其  個分量分別為  階微分算子

\[\left [ \nabla^{(i)} \right ]_{\alpha_1 \alpha_2 \cdots \alpha_i} = \frac {\partial^{\, i}} {\partial r_{\alpha_1}  \partial r_{\alpha_2} \cdots \partial r_{\alpha_i} } \qquad \qquad \text{where} \quad  \alpha_1, \alpha_2, \cdots, \alpha_i = 1, 2, \cdots , n \ .\]<ref group = 'Note'>例如，對於三維
( 3}}) 和二階 ( 2}}) 導數，張量  的分量為
...
<ref group = 'Note'>例如，當  3}} 及  2}}時，張量的純量積為
...
\(T_\mathrm{TF}[\rho]\) 只与电子密度有关 \(\rho(\mathbf{r})\) 并且不依赖于其梯度,
[Laplacian](../Page/拉普拉斯算子.md "wikilink"), 或者其他更高阶的微分 (像这样的泛函被称为是“局部的”).
因此，
...
1935
年，[魏茨泽克提出](../Page/卡尔·冯·魏茨泽克.md "wikilink")，在托馬斯－費米動能泛函中添加一項梯度修正，使之能更準確描述分子的電子雲：
...
这个泛函只依赖于\(\rho\)，像上面两个例子一样(就是说，它们都是“局部的”)。因此
...
离散[随机变量的](../Page/随机变量.md "wikilink")[熵是](https://zh.wikipedia.org/wiki/信息熵 "wikilink")[概率质量函数的一个泛函](../Page/概率质量函数.md "wikilink")
...
## 参考来源
  ...
  - B. A. Frigyik, S. Srivastava and M. R. Gupta, *Introduction to
    Functional Derivatives*, UWEE Tech Report 2008-0001.
    <https://web.archive.org/web/20120207192424/http://www.ee.washington.edu/research/guptalab/publications/functionalDerivativesIntroduction.pdf>

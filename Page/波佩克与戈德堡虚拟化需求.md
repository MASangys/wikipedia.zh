...
系统虚拟机（system virtual
machine）是指能够虚拟包括单或多[处理器](https://zh.wikipedia.org/wiki/处理器 "wikilink")、[-{zh-hans:内存;zh-hant:記憶體;}-](https://zh.wikipedia.org/wiki/内存 "wikilink")、外存及周边设备在内之全体硬件资源的虚拟机。虚拟机监视器（virtual
machine monitor，VMM）是一个软件，用于提供虚拟机的抽象（）。当分析一个VMM所创造的环境时，有三点性质值得特别关注：
...
为导出虚拟化需求，波佩克与戈德堡将ISA的全部指令分为三类。
  ...
  - 优先级指令 :
    当处理器处于用户态时[自陷](https://zh.wikipedia.org/wiki/自陷 "wikilink")，处于内核态时不自陷的指令。

> 本文内容由[波斯特-图灵机](https://zh.wikipedia.org/wiki/波斯特-图灵机)转换而来。


**Post-图灵机**是一种特别简单类型的[图灵机](../Page/图灵机.md "wikilink")的"程序公式化"，由下面描述的[Emil Post的](https://zh.wikipedia.org/wiki/Emil_Post "wikilink")[图灵等价的](https://zh.wikipedia.org/wiki/图灵完全 "wikilink")[计算模型构成](https://zh.wikipedia.org/wiki/计算模型 "wikilink")。（Post的模型和图灵的模型，尽管相互之间非常类似，但却是独立开发的。图灵的论文在1936年五月出版，Post的论文在十月出版。）Post-图灵机使用二元字母表，无限序列的二元存储位置，和带有在存储位置上双向移动和一次一个更改其内容的指令的原始[编程语言](../Page/编程语言.md "wikilink")。"Post-图灵程序"和"Post-图灵机"的名字由[Martin Davis在](https://zh.wikipedia.org/wiki/Martin_Davis "wikilink")1973年-1974年使用（Davis 1973, p.69ff）。后来Davis在1980年使用名字"Turing-Post程序"（Davis, in Steen p. 241）。

## Post模型

在他1936年的论文"Finite combinatory processes—formulation 1"（可以在*The Undecidable*的289页找到它），[Emil Post描述了非常简单的一个模型](https://zh.wikipedia.org/wiki/Emil_Post "wikilink")，它被猜测为"逻辑上等价于[递归函数](../Page/递归函数.md "wikilink")"，并且后来被证明确实如此。

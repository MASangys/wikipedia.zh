[Regedit_on_Vista.png](https://zh.wikipedia.org/wiki/File:Regedit_on_Vista.png "fig:Regedit_on_Vista.png")中的登錄檔編輯程式\]\] **注册表**（Registry，中国大陆译作**-{zh-hans:注册表;zh-hant:註冊表;}-**，台湾、港、澳译作**-{zh-hant:登錄檔;zh-hans:登录档;}-**）是[Microsoft Windows中的一个重要的](https://zh.wikipedia.org/wiki/Microsoft_Windows "wikilink")[数据库](../Page/数据库.md "wikilink")，用于存储系统和应用程序的设置信息。早在[Windows](https://zh.wikipedia.org/wiki/Windows "wikilink") 3.0推出[OLE技术的时候](https://zh.wikipedia.org/wiki/OLE "wikilink")，注册表就已经出现。随后推出的[Windows NT是第一个从系统级别广泛使用注册表的操作系统](../Page/Windows_NT.md "wikilink")。但是，从[Windows 95开始](../Page/Windows_95.md "wikilink")，注册表才真正成为Windows用户经常接触的内容，并在其后的操作系统中继续沿用至今。

**流式接口**（fluent
interface）是[软件工程中](../Page/软件工程.md "wikilink")[面向对象API的一种实现方式](../Page/面向对象设计.md "wikilink")，以提供更为可读的源代码。最早由与[Martin
Fowler于](../Page/Martin_Fowler.md "wikilink")2005年提出。
...
[C++的](../Page/C++.md "wikilink")[iostream流式调用就是一个典型的例子](../Page/iostream.md "wikilink")。Smalltalk在1970年代就实现了。

## 例子

### JavaScript

用于数据库查询的[jQuery](../Page/jQuery.md "wikilink")，例如https://github.com/Medium/dynamite
:
...
[jOOQ库模拟了SQL](../Page/jOOQ.md "wikilink")
...
C\#在[LINQ中大量使用](../Page/LINQ.md "wikilink") *standard query
operators*与[扩展方法](../Page/扩展方法.md "wikilink")。
...
[Ruby语言允许修改核心类](../Page/Ruby.md "wikilink")，这使得流式接口成为原生易于实现。
...
Scala supports a fluent syntax for both method calls and class mixins,
using **trait**s and the **with** keyword. For example:
...
but one of the simplest is to declare attributes as read/write and use
the `given` keyword. The type annotations are optional, but the native
[gradual typing](../Page/gradual_typing.md "wikilink") makes it much
safer to write directly to public attributes.
...
## 外部链接
  ...
  - [Martin Fowler's original bliki entry coining the
    term](http://martinfowler.com/bliki/FluentInterface.html)
  - [A Delphi example of writing XML with a fluent
    interface](http://17slon.com/blogs/gabr/2009/04/fluent-xml-1.html)
  - [A .NET fluent validation library written in
    C\#](http://tnvalidate.codeplex.com/)
  - [A tutorial for creating formal Java fluent APIs from a BNF
    notation](http://blog.jooq.org/2012/01/05/the-java-fluent-api-designer-crash-course/)

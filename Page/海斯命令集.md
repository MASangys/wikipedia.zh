**海斯命令集**（Hayes command set），又称**AT命令集**（AT command
set），原本是為了[海斯智慧](https://zh.wikipedia.org/wiki/海斯 "wikilink")300數據機所開發的一種命令語言。這些命令集是由許多短的字串組成長的命令，用於代表撥號、掛號以及改變通訊參數的動作。大部分的[數據機都跟隨海斯命令集所制定的規則](https://zh.wikipedia.org/wiki/數據機 "wikilink")。
...
4.  暫存器指令集：Sr＝n 其中 r
    代表是[暫存器的編號](https://zh.wikipedia.org/wiki/暫存器 "wikilink")，n代表是要指定給暫存器的數值
...
:\*^M - 傳送一個結束字元（Carriage
Return）給數據機，這是一個控制字元（當傳送此字元其實是傳送一個位元組，內容為[ASCII的CR](../Page/ASCII.md "wikilink")）
...
## 外部連結
  ...
  - [A list of Hayes AT
    commands.](http://home.intekom.com/option/hayesat.htm)
  - [Extended AT Command
    Sets](http://www.modem.com/general/extendat.html)

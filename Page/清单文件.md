...
file）是[JAR档案](../Page/JAR_\(文件格式\).md "wikilink")\[1\]\[2\]中包含的特殊文件。**Manifest**文件被用来定义扩展或档案打包相关数据，是一个[元数据文件](../Page/元数据.md "wikilink")，它包含了不同部分中的数据。如果一个JAR文件被用来作为可执行文件，那么其中的Manifest文件需要指出该程序的主类文件。通常Manifest文件的文件名为`MANIFEST.MF`。

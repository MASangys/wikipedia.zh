**渐近分析**（asymptotic
analysis、asymptotics），在[数学分析中是一种描述函数在](../Page/数学分析.md "wikilink")[极限附近的行为的方法](https://zh.wikipedia.org/wiki/极限 "wikilink")。有多个科学领域应用此方法。例子如下：

  - 在[计算机科学中](../Page/计算机科学.md "wikilink")，[算法分析考虑给定算法在输入非常大的数据集时候的性能](https://zh.wikipedia.org/wiki/算法分析 "wikilink")。
...
函数\(f(x)\)的渐近展开是它的一种[级数展开](../Page/级数.md "wikilink")。这种展开的部分和未必收敛，但每一个部分和都表示\(f(x)\)的一个渐近表示式。例子：[斯特灵公式](https://zh.wikipedia.org/wiki/斯特灵公式 "wikilink")。
...
## 外部連結
  ...
  - J. P. Boyd, "The Devil's Invention: asymptotic, superasymptotic and
    hyperasymptotic series", *Acta Applicandae Mathematicae*, 56: 1-98
    (1999).
    [Preprint](http://www-personal.umich.edu/~jpboyd/boydactaapplicreview.pdf).

...
有些交換式電源供應器驅動的電機就是利用滑動模式控制\[2\]。這類轉換器本身就有不連續的特性，和不連續輸出的滑動模式控制器在本質上是一致的。使用連續性的控制器，還需要透過[脈衝寬度調變或是其他類似的技術](../Page/脈衝寬度調變.md "wikilink")\[3\]來將連續性的輸出轉換為可用離散狀態表示的輸出。滑動模式控制在[機器人學有許多的應用](https://zh.wikipedia.org/wiki/機器人學 "wikilink")。此控制演算法特別已用在[無人水面載具模擬在有大風大浪海面下的運作](https://zh.wikipedia.org/wiki/無人水面載具 "wikilink")，其成功率相當高\[4\]\[5\]。
...
為[非奇异的](https://zh.wikipedia.org/wiki/非奇异方阵 "wikilink")，也就是說，系統有某種[可控制性使得永遠存在一種控制方式可以讓軌跡更靠近滑動模式](https://zh.wikipedia.org/wiki/可控制性 "wikilink")。只要達到了滑動模式\(\sigma(\mathbf{x}) = \mathbf{0}\)，系統會維持在滑動模式上。沿著滑動模式軌跡，\(\sigma(\mathbf{x})\)為定值，因此滑動模式軌跡可以用以下的微分方程來描述
...
## 相關條目
  ...
  - [積分滑動模式控制](https://zh.wikipedia.org/wiki/積分滑動模式控制 "wikilink")

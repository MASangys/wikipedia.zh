**满射**或**蓋射**（），或稱**满射函数**或**映成函數**，一个函数\(f:X\rightarrow Y\)为满射，則对于任意的[陪域](https://zh.wikipedia.org/wiki/陪域 "wikilink")
\(Y\) 中的元素 \(y\)，在函数的定义域 \(X\) 中存在一點 \(x\) 使得
\(f(x)=y\)。换句话说，\(f\)是满射時，它的值域\(f(X)\)与陪域\(Y\)相等，或者，等价地，如果每一个陪域中的元素
\(y\in Y\) 其[原像](https://zh.wikipedia.org/wiki/原像 "wikilink")
\(f^{-1}(y)\subseteq X\) 不等於空集合。

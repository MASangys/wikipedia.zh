...
設 \(\mathbb{F}\)
為域（例如[實數或](https://zh.wikipedia.org/wiki/實數 "wikilink")[複數域](https://zh.wikipedia.org/wiki/複數 "wikilink")），對佈於
\(\mathbb{F}\) 上的 \(n \times n\) 矩陣 \(A\)，定義其**特徵多項式**為
...
此外：
  ...
  - 特徵多項式在[基變更下不變](https://zh.wikipedia.org/wiki/基變更 "wikilink")：若存在可逆方陣
    \(C\) 使得 \(B = C^{-1}AC\)，則 \(p_A(t) = p_B(t)\)。

**状态空间**是[控制工程中的一個名詞](../Page/控制工程.md "wikilink")。[状态是指在](https://zh.wikipedia.org/wiki/状态 "wikilink")[系统中可决定系统状态](https://zh.wikipedia.org/wiki/系统 "wikilink")、最小数目变量的有序[集合](../Page/集合_\(数学\).md "wikilink")\[1\]。而所谓状态空间则是指该系统全部可能状态的集合\[2\]。簡單來說，状态空间可以視為一個以狀態變數為座標軸的空間，因此系統的狀態可以表示為此空間中的一個向量。
...
通式中所有的矩陣均允許隨著時間而變化，此時所表示的就是線性時變系統。若表示的是[线性非时变系统](../Page/线性时不变系统理论.md "wikilink")，则通式的矩陣都不會隨著時間變化。時間變數\(t\)可以是連續的（例如\(t \in \mathbb{R}\)）或是離散的（例如\(t \in \mathbb{Z}\)）。若是後者，其時間變數一般會標示為\(k\)。[混合系統允許同時使用連續及離散的時域](https://zh.wikipedia.org/wiki/混合系統 "wikilink")。依不同的假設，状态空间表示法可以是以下的這些形式：
...
其多項式的根即為矩陣**A**的[特徵值](https://zh.wikipedia.org/wiki/特徵值 "wikilink")，也是傳遞函數的[极点](https://zh.wikipedia.org/wiki/极点_\(代数\) "wikilink")。极点可以用來分析系統是否為[指數穩定](https://zh.wikipedia.org/wiki/指數穩定 "wikilink")、[臨界穩定或者根本不穩定](https://zh.wikipedia.org/wiki/臨界穩定 "wikilink")。[李雅普諾夫穩定性是另一種不需計算特徵值](https://zh.wikipedia.org/wiki/李雅普諾夫穩定性 "wikilink")，也可以判斷穩定性的方式。

由傳遞函數的分子\(\textbf{G}(s)\)可以找到多項式的零點，可用來判斷系統是否是[最小相位](https://zh.wikipedia.org/wiki/最小相位 "wikilink")。
...
（[秩就是矩陣中線性獨立橫行的數目](https://zh.wikipedia.org/wiki/秩_\(線性代數\) "wikilink")）
...
針對每一個輸入，有\(q\)個传递函数，每個對應一個輸出。因此在多重輸入多重輸出（MIMO）的系統，會比較傾向用状态空间表示法來表達。[羅森布羅克系統矩陣提供了状态空间表示法和其](https://zh.wikipedia.org/wiki/羅森布羅克系統矩陣 "wikilink")[传递函数之間的橋樑](../Page/传递函数.md "wikilink")。
...
[framed](https://zh.wikipedia.org/wiki/Image:Typical_State_Space_model_with_feedback.png "wikilink")
...
此作法的好處是整個系統的特性不止是由矩陣*A*的特徵值決定，也可以由*K*的適當設定及\(\left(A + B K \left(I - D K\right)^{-1} C \right)\)的特徵值分解來調整。不過此作法的前提是開迴路系統本身是[可控制的](https://zh.wikipedia.org/wiki/可控制性 "wikilink")，或者矩陣*A*的不穩定特徵值可以由適當的*K*調整為穩定。
...
回授信號也可以再加上[目標值輸入](https://zh.wikipedia.org/wiki/目標值 "wikilink")\(r(t)\)，此時的回授值信號為\(\mathbf{u}(t) = -K \mathbf{y}(t) + \mathbf{r}(t)\).
...
依[可控制性測試](https://zh.wikipedia.org/wiki/可控制性 "wikilink")，結果為

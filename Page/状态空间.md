**状态空间**是[控制工程中的一個名詞](../Page/控制工程.md "wikilink")。[状态是指在](../Page/状态.md "wikilink")[系统中可决定系统状态](../Page/系统.md "wikilink")、最小数目变量的有序[集合](../Page/集合_\(数学\).md "wikilink")\[1\]。而所谓状态空间则是指该系统全部可能状态的集合\[2\]。簡單來說，状态空间可以視為一個以狀態變數為座標軸的空間，因此系統的狀態可以表示為此空間中的一個向量。

状态空间表示法即為一種將物理系統表示為一組輸入、輸出及狀態的數學模式，而輸入、輸出及狀態之間的關係可用許多一階[微分方程來描述](../Page/微分方程.md "wikilink")。

為了使數學模式不受輸入、輸出及狀態的個數所影響，輸入、輸出及狀態都會以[向量的形式表示](../Page/向量.md "wikilink")，而微分方程（若是線性非時變系統，可將微分方程轉變為代數方程）則會以矩陣的形式來表示。

状态空间表示法提供一種方便簡捷的方法來針對多輸入、多輸出的系統進行分析並建立模型。一般[頻域的系統處理方式需限制在常係數](../Page/頻域.md "wikilink")，啟始條件為0的系統。而状态空间表示法對系統的係數及啟始條件沒有限制。

## 状态变量

[framed](../Page/Image:Typical_State_Space_model.png.md "wikilink")

系統的[狀態變數是指系統變數中](../Page/狀態變數.md "wikilink")，可以表示任一時間系統完整狀態的最小子集合。要表示一系統需要的狀態變數最小值n，通常也是該系統微分方程式的階數。若系統是以[傳遞函數來表示](../Page/傳遞函數.md "wikilink")，狀態變數的最小個數等於傳遞函數分母多項式的階數。在電路中狀態變數的個數常常就是電路中儲能元件（如[電容器及](../Page/電容器.md "wikilink")[電感器](../Page/電感器.md "wikilink")）的個數。
...
\[D(\cdot)\]稱為前饋矩陣（若系統沒有直接從輸入到輸出的路徑，此矩陣為零矩陣）， 
\(\operatorname{dim}[D(\cdot)] = q \times p\),

  -
    \(\dot{\mathbf{x}}(t) := \frac{\operatorname{d}}{\operatorname{d}t} \mathbf{x}(t)\).

通式中所有的矩陣均允許隨著時間而變化，此時所表示的就是線性時變系統。若表示的是[线性非时变系统](../Page/线性时不变系统理论.md "wikilink")，则通式的矩陣都不會隨著時間變化。時間變數\(t\)可以是連續的（例如\(t \in \mathbb{R}\)）或是離散的（例如\(t \in \mathbb{Z}\)）。若是後者，其時間變數一般會標示為\(k\)。[混合系統允許同時使用連續及離散的時域](../Page/混合系統.md "wikilink")。依不同的假設，状态空间表示法可以是以下的這些形式：
...
連續[线性非时变系统的穩定性及響應特性可以由矩陣](../Page/线性时不变系统理论.md "wikilink")**A**的[特徵值得到](../Page/特徵值.md "wikilink")，也可以由系統對應的乘積型[傳遞函數中得到](../Page/傳遞函數.md "wikilink")。其型式如下所示：

\[\textbf{G}(s) = k \frac{ (s - z_{1})(s - z_{2})(s - z_{3})
                              }{ (s - p_{1})(s - p_{2})(s - p_{3})(s - p_{4})
                                }\]

傳遞函數的分母等於\(sI - A\)的[行列式所得的](../Page/行列式.md "wikilink")[特徵多項式](../Page/特徵多項式.md "wikilink")：

  -
    \(\mathbf{\lambda}(s) = |sI - A|\).

其多項式的根即為矩陣**A**的[特徵值](../Page/特徵值.md "wikilink")，也是傳遞函數的[极点](../Page/极点_\(代数\).md "wikilink")。极点可以用來分析系統是否為[指數穩定](../Page/指數穩定.md "wikilink")、[臨界穩定或者根本不穩定](../Page/臨界穩定.md "wikilink")。[李雅普諾夫穩定性是另一種不需計算特徵值](../Page/李雅普諾夫穩定性.md "wikilink")，也可以判斷穩定性的方式。

由傳遞函數的分子\(\textbf{G}(s)\)可以找到多項式的零點，可用來判斷系統是否是[最小相位](../Page/最小相位.md "wikilink")。

即使系統沒有內部穩定性，仍有可能是[輸入輸出穩定系統](../Page/輸入輸出穩定.md "wikilink")。此情形時的不穩定极点和零點相消，為一個[可去奇點](../Page/可去奇點.md "wikilink")。

### 傳遞函數

[傳遞函數可用來描述線性非時變系統的特性](../Page/傳遞函數.md "wikilink")。一個連續時間的線性非時變系統，可以用以下的方式，將其狀態空間表示式轉換為傳遞函數：

首先，對下式進行[拉氏轉換](../Page/拉氏轉換.md "wikilink")

\[\dot{\mathbf{x}}(t) = A \mathbf{x}(t) + B \mathbf{u}(t)\] 可得到

\[s\mathbf{X}(s) = A \mathbf{X}(s) + B \mathbf{U}(s)\]
再針對\(\mathbf{X}(s)\)化簡，可得到
...
[傳遞函數](../Page/傳遞函數.md "wikilink")\(\mathbf{G}(s)\)定義為系統輸出相對於輸入的比值
...
一系統的[狀態可控制性表示對於系統的一組初始狀態及一組終止狀態](../Page/狀態可控制性.md "wikilink")，存在一組輸入，使得此系統在有限時間內，可以由初始狀態轉移到終止狀態。一個連續线性时不变系统以状态空间表示時，系统[若且唯若在以下](../Page/若且唯若.md "wikilink")[秩的等式成立時才有可控制性](../Page/秩_\(線性代數\).md "wikilink")

\[\operatorname{rank}\begin{bmatrix}B& AB& A^{2}B& ...& A^{n-1}B\end{bmatrix} = n\]
（[秩就是矩陣中線性獨立橫行的數目](../Page/秩_\(線性代數\).md "wikilink")）

### 可觀察性

[可觀察性是可以依一系統的輸出得知其系統內部的初始狀態](../Page/可觀察性.md "wikilink")。一系統的可觀察性及可控制性有數學上的對偶關係，可控制性是指可以利用輸入將系統由初始狀態轉換成任意的最終狀態，而可觀察性是指系統的輸出軌跡預測其初始狀態。
...
\(\mathbf{G}(s)\)定義為在初始條件為0（\(\mathbf{x}(0) =\mathbf{0}\)）時，系統輸出和輸入的比值。不過因為向量和向量的比例不存在，因此考慮讓传递函数可以滿足下式

\[\mathbf{G}(s)\times\mathbf{U}(s) = \mathbf{Y}(s)\]
比較\(\mathbf{Y}(s)\)的方程，可得

\[\mathbf{G}(s) = C(s\mathbf{I} - A)^{-1}B + D.\]
可以看出\(\mathbf{G}(s)\)的維度需要是\(q\)乘\(p\)，共有\(qp\) 個元素。

針對每一個輸入，有\(q\)個传递函数，每個對應一個輸出。因此在多重輸入多重輸出（MIMO）的系統，會比較傾向用状态空间表示法來表達。[羅森布羅克系統矩陣提供了状态空间表示法和其](../Page/羅森布羅克系統矩陣.md "wikilink")[传递函数之間的橋樑](../Page/传递函数.md "wikilink")。

### 正則实现

任一個[嚴格真分的傳遞函數](../Page/嚴格真分傳遞函數.md "wikilink")，可以依以下的方式轉換為狀態空間的表示法（以下以一個4階、單一輸入及輸出的系統為例）：
...
這種狀態空間的實現方式稱為“可控制正則型”（controllable canonical
form），方便判斷系統的可控制性（控制輸入經過一連串的積分器後，即可影響每一個狀態），因此。
...
這種狀態空間的實現方式稱為“可觀察正則型”（observable canonical
form），方便判斷系統的可觀察性（輸出是經過狀態變數一連串積分後的輸出，因此可經由狀態變數影響系統的輸出）。
...
利用上一段的作法可以將嚴格真分傳遞函數轉換為正則型的狀態空間表示式。而常數部份可以轉換為\(\textbf{y}(t) = \textbf{G}(\infty)\textbf{u}(t)\)。因此狀態空間表示式的矩陣*A*、*B*及*C*可利用傳遞函數的嚴格真分部份求得，而矩陣*D*可利用常數部份求得。
...
[framed](../Page/Image:Typical_State_Space_model_with_feedback.png.md "wikilink")
要為系統增加反饋，可以將輸出乘以一矩陣*K*，當作系統的輸入：\(\mathbf{u}(t) = K \mathbf{y}(t)\). 因此系統
...
[framed](../Page/Image:Typical_State_Space_model_with_feedback_and_input.png.md "wikilink")

回授信號也可以再加上[目標值輸入](../Page/目標值.md "wikilink")\(r(t)\)，此時的回授值信號為\(\mathbf{u}(t) = -K \mathbf{y}(t) + \mathbf{r}(t)\).
...
以下以物體的一維移動來作為範例。
考慮一物體在一平面上水平移動，物體和牆壁之間有彈簧相連接，依[牛頓第二運動定律](../Page/牛頓第二運動定律.md "wikilink")，其受力如下
...
依[可控制性測試](../Page/可控制性.md "wikilink")，結果為
...
依[可觀察性測試](../Page/可觀察性.md "wikilink")，結果為
...
一系統的[機械平衡點或](../Page/機械平衡點.md "wikilink")[駐點為](../Page/駐點.md "wikilink")\(\dot{x} = 0\)的位置，因此單擺系統的平衡點如下
...
## 相关条目
  ...
  - [离散化](../Page/离散化.md "wikilink")
  - [相空間](../Page/相空間.md "wikilink")：物理學和數學中有關相空間（例如連續的狀態空間）的資訊。
  - [狀態空間
    (計算機科學)](../Page/狀態空間_\(計算機科學\).md "wikilink")：計算機科學中有關狀態空間的資訊。
  - [機率空間](../Page/機率空間.md "wikilink")：機率中關於狀態空間的資訊。
...
## 延伸閱讀
  ...
  - Antsaklis, P.J. and Michel, A.N. 2007. *A Linear Systems Primer*,
    Birkhauser. (ISBN 978-0-8176-4434-50)
  - Chen, Chi-Tsong 1999. *Linear System Theory and Design*, 3rd. ed.,
    Oxford University Press (ISBN 978-0-19-511777-6)
  ...
  - Nise, Norman S. 2004. *Control Systems Engineering*, 4th ed., John
    Wiley & Sons, Inc. (ISBN 978-0-471-44577-7)
  - Hinrichsen, Diederich and Pritchard, Anthony J. 2005. *Mathematical
    Systems Theory I, Modelling, State Space Analysis, Stability and
    Robustness*. Springer. (ISBN 978-3-540-44125-0)
  - Sontag, Eduardo D. 1999. *Mathematical Control Theory: Deterministic
    Finite Dimensional Systems. Second Edition*. Springer. (ISBN
    978-0-387-98489-6) ([available free
    online](http://www.math.rutgers.edu/~sontag/FTP_DIR/sontag_mathematical_control_theory_springer98.pdf))
  - Friedland, Bernard. 2005. *Control System Design: An Introduction to
    State Space Methods*. Dover. (ISBN 978-0-486-44278-5).
  - Zadeh, Lofti A. and Desoer, Charles A. 1979. *Linear System Theory*,
    Krieger Pub Co. (ISBN 978-0-88275-809-1)

**狀態轉移矩陣**（state-transition matrix）是[控制理論中的](https://zh.wikipedia.org/wiki/控制理論 "wikilink")[矩陣](https://zh.wikipedia.org/wiki/矩陣 "wikilink")，是時間\(t\)和初始時間\(t_0\)的函數，可以將時間\(t_0\)的狀態向量\(x\)和此矩陣相乘，得到時間\(t\)時的狀態向量\(x\)。狀態轉移矩陣可以用來找線性動態系統的通解。

## 線性系統的解

狀態轉移矩陣用來找以下形式[線性系統](../Page/線性系統.md "wikilink")在[状态空间](../Page/状态空间.md "wikilink")下的解：
...
|- |1.||\(\mathbf{\Phi}(t_2, t_1)\mathbf{\Phi}(t_1, t_0) = \mathbf{\Phi}(t_2, t_0)\) |- |2.||\(\mathbf{\Phi}^{-1}(t, \tau) = \mathbf{ \Phi}(\tau, t)\) |- |3.||\(\mathbf{\Phi}^{-1}(t, \tau)\mathbf{\Phi}(t, \tau) = I\) |- |4.||\(\frac{d\mathbf{\Phi}(t, t_0)}{dt} = \mathbf{A}(t)\mathbf{\Phi}(t, t_0)\) |}

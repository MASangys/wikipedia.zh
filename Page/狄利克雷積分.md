在数学中，有不只一个[积分称作](../Page/积分.md "wikilink")**狄利克雷積分**，都由德國數學家[約翰·彼得·古斯塔夫·勒熱納·狄利克雷提出](../Page/約翰·彼得·古斯塔夫·勒熱納·狄利克雷.md "wikilink")。
其中一个內容如下\[1\]：

  -
    \(\int_0^\infty \frac{\sin \omega}{\omega}\,d\omega = \frac{\pi}{2}\)

这个积分不是[绝对收敛的](../Page/绝对收敛.md "wikilink")，因此[勒貝格積分](../Page/勒貝格積分.md "wikilink")*甚至不能定义*这个积分，但它在[黎曼积分或](../Page/黎曼积分.md "wikilink")[Henstock–Kurzweil积分是有定义的](../Page/Henstock–Kurzweil积分.md "wikilink")。
\[2\]
可以通过多种方式导出这个（黎曼或Henstock）积分的值。例如，该值可以通过计算双[反常积分确定](https://zh.wikipedia.org/wiki/反常积分 "wikilink")，也可以通过在[积分符号内取微分来确定](../Page/积分符号内取微分.md "wikilink")。
...
其中\(\mathcal{L}\{\sin t\}(s)\)是函数\(\sin t\)的拉普拉斯变换。运用[欧拉公式](../Page/欧拉公式.md "wikilink")
，然后积分，使得分母为实数，并取虚部，我们发现该拉普拉斯变换是拉普拉斯变量 *s*
的函数\(\tfrac{1}{s^{2}+1}\)。这相当于尝试用两种不同方式求同一个二重定积分，通过颠倒积分的顺序
，*即*，
...
左边界：
\(\int_0^{\infty} \frac{\sin x}{x}dx = \sum_{n=0}^{n = \infty} \int_{2\pi n}^{2\pi (n+1)} \frac{\sin x}{x}dx = \sum_{n=0}^{n = \infty} \int_{0}^{2\pi} \frac{\sin x}{2\pi n + x}dx > \sum_{n=0}^{n = \infty} \int_{0}^{2\pi} \frac{\sin x}{2\pi (n+1)}dx = 0\)

右边界：
\(\int_0^{\infty} \frac{\sin x}{x}dx = \int_0^{\pi} \frac{\sin x}{x}dx + \int_{\pi}^{\infty} \frac{\sin x}{x}dx\)
...
极点已被移离实轴，所以 g(z) 的可沿半径为 R，中心在 z = 0
且与实轴围成的封闭半圆积分，然后取极限\(\epsilon \rightarrow 0\)。

由[留数定理知复积分为零](../Page/留数定理.md "wikilink")，因为积分路径内不存在极点

  -
    \(0 = \int_\gamma g(z) dz = \int_{-R}^R \frac{e^{ix}}{x +i\epsilon} dx + \int_{0}^{\pi} \frac{e^{i(Re^{i\theta} + \theta)}}{Re^{i\theta} +i\epsilon} iR d\theta\)

随着 R
趋向无穷大，第二项消失；对任意小的\(\epsilon\)，对第一项运用[索霍茨基－魏尔斯特拉斯定理得](../Page/索霍茨基－魏尔斯特拉斯定理.md "wikilink")
...
因為\(\lim _{a \to \infty} \int_0^1  \frac{2 \sin^2 \frac{x}{2}}{x^2}\,dx + \int_1^a  \frac{2 \sin^2 \frac{x}{2}}{x^2}\,dx\)
遞增並且有上界\(\int_0^1  \frac{2 \sin^2 \frac{x}{2}}{x^2}\,dx + 2\)，故由[單調收斂定理知](https://zh.wikipedia.org/wiki/單調收斂定理 "wikilink")，\(\int_0^\infty \frac{\sin x}{x}\,dx\)有極限值\(I\)。
...
因為\(\lim _{\lambda \to \infty} \int_0^\pi \frac{\sin \lambda z}{2 \sin \frac{z}{2}}\,dz\)
存在收斂值
*I*，故\(\lim _{n \to \infty} \int_0^\pi \frac{\sin (n + \frac{1}{2}) z}{2 \sin \frac{z}{2}}\,dz ,n \in \mathbb{N}\)亦收斂至
*I*。
...
2.  Robert G. Bartle, [Return to the Riemann
    Integral](http://mathdl.maa.org/mathDL/22/?pa=content&sa=viewDocument&nodeId=2900),
    *The American Mathematical Monthly*, vol. 103, 1996, pp. 625-632.
3.  Appel, Walter. *Mathematics for Physics and Physicists*. Princeton

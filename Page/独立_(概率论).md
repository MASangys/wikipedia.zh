...
標準的定義為：
  ...
  -
    兩個事件*A*和*B*是**獨立的**[若且唯若Pr](https://zh.wikipedia.org/wiki/若且唯若 "wikilink")(*A*
    ∩ *B*) = Pr(*A*)Pr(*B*)。

這裡，*A* ∩
*B*是*A*和*B*的[交集](../Page/交集.md "wikilink")，即為*A*和*B*兩個事件都會發生的事件。

更一般地，任意個事件都是**互相獨立的**若且唯若對其任一有限子集*A*<sub>1</sub>, ...,
*A*<sub>*n*</sub>，會有
...
兩個隨機變數*X*和*Y*是獨立的[若且唯若對任何數字](https://zh.wikipedia.org/wiki/若且唯若 "wikilink")*a*和*b*，事件\[*X*
≤ *a*\](*X*小於或等於*a*的事件)和\[*Y* ≤
*b*\]為如上面所定義的獨立事件。類似地，隨意數量的隨機變數是明確地獨立的，若對任一有限子集*X*<sub>1</sub>,
..., *X*<sub>*n*</sub>和任一數字的有限子集*a*<sub>1</sub>, ...,
*a*<sub>*n*</sub>，其事件\[*X*<sub>1</sub> ≤ *a*<sub>1</sub>\], ...,
\[*X*<sub>*n*</sub> ≤ *a*<sub>*n*</sub>\]會是如上面所定義的獨立事件。

其量測可以由事件\[*X* ∈ *A*\]來取代上面所定義的事件\[*X* ≤
*a*\]，其中*A*為任一[包絡集合](https://zh.wikipedia.org/wiki/包絡代數 "wikilink")。此一定義完全和上述其隨機變數的值為[實數的定義等價](https://zh.wikipedia.org/wiki/實數 "wikilink")。且他有著可以作用於複值隨機變數和在任一[拓撲空間中取值之隨機變數上的優點](https://zh.wikipedia.org/wiki/拓撲空間 "wikilink")。

即使任意數目中的任二個隨機變數都是獨立的，但它們可能仍舊會無法互相獨立；這種的獨立被稱為[兩兩獨立](https://zh.wikipedia.org/wiki/兩兩獨立 "wikilink")。

若*X*和*Y*是獨立的，則其[期望值](../Page/期望值.md "wikilink")*E*會有下列的好性質： E\[*X* *Y*\]
= E\[*X*\] E\[*Y*\], （假定都存在）且其[方差](../Page/方差.md "wikilink")（若存在）满足

  -
    var(*X* + *Y*) = var(*X*) + var(*Y*),

因为其[協方差](https://zh.wikipedia.org/wiki/協方差 "wikilink") cov(*X*,*Y*)
為零。(其逆命题不成立，即若兩個隨機變數的協方差為0，它們不一定独立。)

此外，具有[分佈函數](https://zh.wikipedia.org/wiki/累積分佈函數 "wikilink")*F*<sub>*X*</sub>(*x*)
及
*F*<sub>*Y*</sub>(*y*)和[機率密度](https://zh.wikipedia.org/wiki/機率函數 "wikilink")*f*<sub>*X*</sub>(*x*)
及 *f*<sub>*Y*</sub>(*y*)的隨機變數*X*和*Y*為獨立的，若且唯若其相結合的隨機變數(*X*,*Y*)有一共同分佈
...
## 書籍
  ...
  - Kirby Faciane (2006). *Statistics for Empirical and Quantitative
    Finance*. H.C. Baird: Philadelphia. ISBN 0-9788208-9-4.

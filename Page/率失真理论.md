...
theory）是[信息论的主要分支](../Page/信息论.md "wikilink")，其的基本问题可以归结如下：对于一个给定的信源（source,
input
signal）分布与[失真度量](../Page/失真.md "wikilink")，在特定的[码率下能达到的最小期望失真](../Page/码率.md "wikilink")；或者为了满足一定的失真限制，可允許的最大码率為何，D
定義為失真的符號。
...
對漢明失真函數取[期望值即為傳輸錯誤率](../Page/期望值.md "wikilink")。
...
係一種[條件機率之](../Page/條件機率.md "wikilink")[機率密度函數](../Page/機率密度函數.md "wikilink")
...
signal）和信宿（output
signal）相關的[條件熵](../Page/條件熵.md "wikilink")（conditional
entropy）, 分別為:
...
是[常態分配並取](../Page/常態分配.md "wikilink")[方差](../Page/方差.md "wikilink")
σ<sup>2</sup>, 並且假設訊號 *X* 是連續 [統計獨立性](../Page/統計獨立性.md "wikilink")
(或等同於來源無記憶或訊號不相關), 我們可以發現下列的率失真公式之「公式解」（analytical
expression）:
...
algorithm)，我們可以找出一閾值\(\lambda\)，只有[方差大於](../Page/方差.md "wikilink")\(\lambda\)的信號源才有必要配置位元來描述，其他信號源則可直接傳送與接收，不會超過最大可容許的失真範圍。

在[环论或](../Page/环论.md "wikilink")[抽象代数中](../Page/抽象代数.md "wikilink")，**环[同态](../Page/同态.md "wikilink")**是指两个环R與S之间的映射f保持两个环的加法与乘法运算。
...
直接从这些定义，我们可以推出：
  ...
  - *f*(0) = 0
  - *f*(−*a*) = −*f*(*a*)
  - 如果*a*在*R*内具有乘法逆元，则*f*(*a*)在*S*内具有乘法逆元，且有*f*(*a*<sup>−1</sup>) =
    (*f*(*a*))<sup>−1</sup>。
  - *f*的[核](../Page/核_\(代数\).md "wikilink")，定义为ker(*f*) = {*a* in *R* :
    *f*(*a*) =
    0}，是*R*内的一个[理想](https://zh.wikipedia.org/wiki/环理想 "wikilink")。每一个交换环*R*内的理想都可以从某个环同态用这种方法得出。对于具有单位元的环，环同态的核是一个没有单位元的子环。
  ...
  - 如果存在一个环同态*f* : *R* →
    *S*，那么*S*的[特征](https://zh.wikipedia.org/wiki/特征_\(代数\) "wikilink")[整除](https://zh.wikipedia.org/wiki/整除 "wikilink")*R*的特征。这有时候可以用来证明在一定的环*R*和*S*之间，不存在环同态*R*
    → *S*。
  ...
  - 对于每一个环*R*，都存在一个唯一的环同态**Z** →
    *R*。这就是说，整数环是环[范畴中的](https://zh.wikipedia.org/wiki/范畴 "wikilink")[始对象](https://zh.wikipedia.org/wiki/始对象 "wikilink")。

## 例子

  - 函数*f* : **Z** → **Z**<sub>*n*</sub>，由*f*(*a*) =
    \[*a*\]<sub>*n*</sub> = *a* **mod**
    *n*定义，是一个[满射的环同态](../Page/满射.md "wikilink")，它的核为*n***Z**。
  - 当*n* \> 1时，不存在环同态**Z**<sub>*n*</sub> → **Z**。
  - 如果**R**\[*X*\]表示变量为*X*的所有实系数[多项式的环](https://zh.wikipedia.org/wiki/多项式 "wikilink")，**C**表示[复数](https://zh.wikipedia.org/wiki/复数 "wikilink")，则函数*f*
    : **R**\[*X*\] → **C**，由*f*(*p*) =
    *p*(*i*)定义（在多项式*p*中用虚数单位*i*来代替变量*X*），是一个满射的环同态。*f*的核由**R**\[*X*\]内所有能被*X*<sup>2</sup>
    + 1整除的多项式组成。.

## 环同态的种类

  - [双射的环同态称为](../Page/双射.md "wikilink")*环同构*。
  - 定义域与值域相同的环同态称为*环自同态*。
...
然而，在环范畴中，满射的环同态与[满同态是非常不同的](https://zh.wikipedia.org/wiki/满同态 "wikilink")。例如，**Z**
⊆ **Q**是满同态，但不是满射。

...
其中\(E\)是量子態能量（隨著個別量子態有所不同），\(kT\)（對於一個波茲曼分布來說是常數）是[波茲曼常數與](../Page/波茲曼常數.md "wikilink")[热力学温度的乘積](../Page/热力学温度.md "wikilink")。

而機率分布則可表達為 \[2\]
\(p_i={\frac{e^{- {\varepsilon}_i / k T}}{\sum_{j=1}^{M}{e^{- {\varepsilon}_j / k T}}}}\)

其中\(p_i\)是量子態i的機率，\(\epsilon_i\)是量子態i的能量，\(k\)是波茲曼常數，\(T\)是系統溫度且\(M\)為系統具有的量子態數目。\[3\]
...
波茲曼分布是狀態能量與[系統溫度的函數](../Page/系統.md "wikilink")，給出了粒子處於特定狀態下的機率。其具有以下形式：\[6\]

\[p_i={\frac{e^{- {\varepsilon}_i / k T}}{\sum_{j=1}^{M}{e^{- {\varepsilon}_j / k T}}}}\]

其中\(p_i\)為量子態i的機率，\(\epsilon_i\)為量子態i之能量，
\(k\)為波茲曼常數，\(T\)為系統溫度，\(M\)為系統可具有的量子態數目。\[7\]\[8\]
分母的部分是對系統所有量子態進行總和，而此部分又被稱為[配分函數](../Page/配分函数.md "wikilink")，通常以Q（在某些書中為Z）表示：
...
若是知道系統中各狀態的能量，可以直接計算此系統的配分函數。各種原子的配分函數可以在NIST Atomic Spectra
Database找到。\[9\]
...
這個表達式在[光譜學中有重要的應用](../Page/光谱学.md "wikilink")。光譜中的[譜線位置代表粒子量子態轉移的能量](../Page/譜線.md "wikilink")。\[11\]\[12\]為了使譜線強度足夠，必須有足量粒子處於高量子態，對此可以透過上述表達式確定粒子分布與系統溫度、能階差的關係，得到恰當的系統參數。\[13\]
...
2.  McQuarrie, A. (2000) Statistical Mechanics, University Science
    Books, California
...
7.
8.
9.  [NIST Atomic Spectra Database Levels
    Form](http://physics.nist.gov/PhysRefData/ASD/levels_form.html) at
    nist.gov

10.
11.
12. Atkins, P. W.; de Paula J. (2009) Physical Chemistry, 9th edition,
    Oxford University Press, Oxford, UK

13. Skoog, D. A.; Holler, F. J.; Crouch, S. R. (2006) Principles of

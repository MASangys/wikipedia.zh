...
波尔兹曼方程并不对流体中每个粒子的[位置和](../Page/位置向量.md "wikilink")[动量](../Page/动量.md "wikilink")做统计分析，而只考虑一群同时占据着空间中[任意小](../Page/無窮小量.md "wikilink")（在数学上写作 \(d^3\mathbf{r}\) ）区域，且以位置矢量 \(\mathbf{r}\) 末端为中心的粒子。这群粒子的动量在一段极短的时间内，相对于动量矢量 \(\mathbf{p}\) 只有几乎同样小的变化（因此这些粒子在[动量空间中也占据着任意小区域](../Page/位置空间与动量空间.md "wikilink") \(d^3\mathbf{p}\) ）。
...
假设在时间 \(t\)，一定数量的粒子都有位置 \(\mathbf{r}\)（于微元 \(d^3\mathbf{r}\) 内），和动量 \(\mathbf{p}\)（于微元 \(d^3\mathbf{p}\) 内）。如果此时有一个力\(\mathbf{F}\)在这一瞬作用在每个颗粒上，那么在时间 \(t + \Delta\,t\)，它们的位置将会是\(\mathbf{r} + \Delta\,\mathbf{r} = \mathbf{r} + \mathbf{p} \Delta\,t/m\)，势能将变成 \(\mathbf{p} + \Delta\,\mathbf{p} = \mathbf{p} + \mathbf{F}\Delta\,t\)。在没有碰撞的情况下，\(f\)必须满足

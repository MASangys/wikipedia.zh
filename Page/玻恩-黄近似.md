...
可以证明，在无其他近似情况下，玻恩-黄近似给出基态能量的上限\[2\]；于此相对，玻恩-奥本海默近似则给出了基态能量的下限\[3\]。因此，当不确定体系是否经历非绝热过程时，可以采用两种近似分别进行计算，如果兩個结果相近则验证了近似的合理性。否则，两个近似的结果都不可取，需要考虑不同电子态之间的[非绝热耦合才能得到正确结果](../Page/非绝热耦合.md "wikilink")。

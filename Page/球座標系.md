...
与方位角 ([phi](../Page/phi.md "wikilink"))。\]\]

在[數學裏](../Page/數學.md "wikilink")，**球座標系**（）是一種利用**球座標**\((r,\ \theta,\ \varphi)\)表示一個點P在三維空間的位置的三維[正交座標系](../Page/正交座標系.md "wikilink")。右圖顯示了球座標的幾何意義：原點與點P之間的（）\(r\)，原點到點P的連線與正z-軸之間的（）\(\theta\)，以及原點到點P的連線在xy-平面的[投影線](../Page/投影.md "wikilink")，與正x-軸之間的（）\(\varphi\)。它可以被视为[极坐标系的三维版本](../Page/极坐标系.md "wikilink")。

在學術界內，關於球座標系的標記有好幾個不同的約定。按照[國際標準化組織建立的約定](../Page/國際標準化組織.md "wikilink")（ISO
31-11），徑向距離、天頂角、方位角，分別標記為\((r,\ \theta,\ \varphi)\)。這種標記在世界各地有許多使用者。通常，物理界的學者也採用這種標記。而在數學界，天頂角與方位角的標記正好相反：\(\varphi\)被用來代表天頂角，\(\theta\)被用來代表方位角。數學界的球座標標記是\((\rho,\ \varphi,\ \theta)\)。這種標記的優點是較廣的相容性：在二維[極座標系與三維](../Page/極座標系.md "wikilink")[圓柱座標系裏](../Page/極座標系#三维空间#圓柱座標系.md "wikilink")，\(\rho\)都同樣地代表徑向距離，\(\theta\)也都同樣地代表方位角。本條目採用的是物理標記約定。

## 定義

[Spherical_coordinate_surfaces.png](https://zh.wikipedia.org/wiki/File:Spherical_coordinate_surfaces.png "fig:Spherical_coordinate_surfaces.png")。紅色圓球面的\(r=2\)。藍色圓錐面的\(\theta=45^{\circ}\)。黃色半平面的\(\phi= - 60^{\circ}\)（黃色半平面與xz-半平面之間的[二面角角度是](../Page/二面角.md "wikilink")\(\left|\phi\right|\)）。z-軸是垂直的，以白色表示。x-軸以綠色表示。三個座標曲面相交於點P（以黑色的圓球表示）。[直角座標大約為](../Page/直角座標.md "wikilink")\((0.707, -1.225, 1.414)\)。\]\]
...
地理座標系是球座標系的第二個版本。地理座標標记为\((\rho,\ \lambda,\ \delta)\)，其中\(\rho\,\)表示径向距离，\(\lambda\)表示方位角，\(\delta\)表示高度角。它主要是用在[地理學](../Page/地理學.md "wikilink")。通常在地理學裏，\(\rho\,\)會被用來表示高度，或者完全不被使用。

[緯度的定義域是](../Page/緯度.md "wikilink")\(0^\circ \le \delta \le 90^\circ\)，南緯或北緯。使用以下方程式，可從緯度\(\delta\)變換為天頂角：

1.  \(\theta \le 90^\circ\)：北緯，\(\delta=90^\circ - \theta\)，
2.  \(\theta \ge 90^\circ\)：南緯，\(\delta=\theta - 90^\circ\)。

[經度](../Page/經度.md "wikilink")\(\lambda\)的定義域是\(- 180^\circ \le \lambda \le 180^\circ\)。設定經過[倫敦](../Page/倫敦.md "wikilink")[格林維治天文台的](../Page/格林維治天文台.md "wikilink")[子午線為經度](../Page/子午線.md "wikilink")\(0^\circ\)，往東或往西\(\lambda\)度。使用以下方程式，可從經度變換為方位角
...
圓柱座標系是[極座標系在三維空間往z](../Page/極座標系.md "wikilink")-軸的延伸。\(z\)座標用來表示高度。使用以下方程式，可以從球座標變換為圓柱座標\((\rho,\ \phi,\ z)\)：
...
其中的\(\boldsymbol{\hat r},\boldsymbol{\hat\theta },\boldsymbol{\hat \phi}\)是在\(r,\theta,\phi\)的各自的增加的方向上的[单位矢量](../Page/单位矢量.md "wikilink")。
...
[地理座標系用兩個角值](../Page/經緯度.md "wikilink")，緯度與經度，來表示地球表面的地點。正如二維直角座標系專精在平面上，二維球座標系可以很簡易的設定圓球表面上的點的位置。在這裏，我們認定這圓球是個單位圓球；其半徑是1。通常我們可以忽略這圓球的半徑。在解析[旋轉矩陣問題上](../Page/旋轉矩陣.md "wikilink")，這方法是非常有用的。

球座標系適用於分析一個對稱於點的系統。舉例而言，一個圓球，其直角座標方程式為\(x^2+y^2+z^2=c^2\)，可以簡易的用球座標系\(\rho =c\)來表示。

當求解[三重積分時](../Page/三重積分.md "wikilink")，如果定義域為圓球，則面積元素是

\[dS=r^2\sin\theta\,d\theta\,d\phi\]； 體積元素是

\[dV=r^2\sin\theta\,dr\,d\theta\,d\phi\]。

用來描述與分析擁有球狀對稱性質的物理問題，最自然的座標系，莫非是球座標系。例如，一個具有質量或電荷的圓球形[位勢場](../Page/位勢.md "wikilink")。兩種重要的[偏微分方程式](../Page/偏微分方程式.md "wikilink")，[拉普拉斯方程與](../Page/拉普拉斯方程.md "wikilink")[亥姆霍茲方程](../Page/亥姆霍茲方程.md "wikilink")，在球座標裏，都可以成功的使用[分離變數法求得解答](../Page/分離變數法.md "wikilink")。這種方程式在角部分的解答，皆呈[球諧函數的形式](../Page/球諧函數.md "wikilink")。

球座標的概念，延伸至高維空間，則稱為[超球座標](../Page/N维球面#超球坐标系.md "wikilink")。

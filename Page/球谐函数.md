**球谐函数**是[拉普拉斯方程的](../Page/拉普拉斯方程.md "wikilink")[球坐标系形式解的角度部分](https://zh.wikipedia.org/wiki/球坐标系 "wikilink")。在[古典場論](https://zh.wikipedia.org/wiki/古典場論 "wikilink")、[量子力学等领域广泛应用](../Page/量子力学.md "wikilink")。
...
[Rotating_spherical_harmonics.gif](https://zh.wikipedia.org/wiki/File:Rotating_spherical_harmonics.gif "fig:Rotating_spherical_harmonics.gif")
利用[分离变量法](https://zh.wikipedia.org/wiki/分离变量法 "wikilink")，设定
\(f(r,\ \theta,\ \varphi)=R(r)Y(\theta,\ \varphi)=R(r)\Theta(\theta)\Phi(\varphi)\)
。其中\(Y(\theta,\ \varphi)\)代表角度部分的解，也就是**球谐函数**。
...
而对于\(\Theta\)的方程，进行变量替换
\(t=\cos\theta\)，\(dt=-\sin\theta d\theta\)，\(|t|\leqslant 1\)，得到关于\(t\)的伴随勒让德方程。方程的解应满足在\([-1,1]\)区间上取有限值，此时必须有\(\lambda=l(l+1)\)，其中\(l\)为自然数，且\(l\geqslant |m|\)。对应方程的解为\(P_\ell^m(t)\)。即可以解出：

\[\Theta = P_\ell^m(\cos\theta)\]，\(l\in\mathbb{N},l\geqslant |m|\)

故球谐函数可以表达为：

\[Y_\ell^m (\theta, \varphi ) = N \Phi(\varphi)\Theta(\theta) = N \, e^{i m \varphi } \, P_\ell^m (\cos{\theta} )\,\!\]
，\(l\in\mathbb{N},m=0,\pm1,\pm2,\ldots\pm l\)；
...
这里的 \(Y_\ell^m\,\!\) 称为 \(\ell\,\!\) 和 \(m\,\!\) 的球谐函数。以上推导过程中，\(i\,\!\)
是[虛數單位](../Page/虛數單位.md "wikilink")， \(P_\ell^m\,\!\)
是[伴随勒让德多项式](../Page/伴随勒让德多项式.md "wikilink") 。
...
## 参见
  ...
  - [向量球諧函數](https://zh.wikipedia.org/wiki/向量球諧函數 "wikilink")

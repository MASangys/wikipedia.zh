[右](https://zh.wikipedia.org/wiki/File:Use_case_restaurant_model.svg "fig:右")用例图反映了顾客（角色，）在餐馆（系统，）中的交互\]\] **用例图**（）是用户与系统交互的最简表示形式，展现了用户和与他相关的[用例](../Page/用例.md "wikilink")之间的关系。通过用例图，人们可以获知系统不同种类的用户和用例。用例图也经常和其他图表配合使用。

## 作用与意义

尽管[用例](../Page/用例.md "wikilink")本身会涉及大量细节和各种可能性，用例图却能提纲挈领地让人了解系统概况。它为“系统做什么”提供了简化了的图形表示，因此被譽为“搭建系统的蓝图”。\[1\]
...
[Category:軟體需求](https://zh.wikipedia.org/wiki/Category:軟體需求 "wikilink") [Category:統一塑模語言](https://zh.wikipedia.org/wiki/Category:統一塑模語言 "wikilink") [Category:统一建模语言图表](https://zh.wikipedia.org/wiki/Category:统一建模语言图表 "wikilink")

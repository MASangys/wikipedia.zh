**用户数据-{zh-hans:报;zh-hant:包}-协议**（，縮寫：**UDP**；又稱**用户数据-{zh-hans:包;zh-hant:报}-协议**）是一个简单的面向[数据报的](https://zh.wikipedia.org/wiki/数据报 "wikilink")[通信协议](https://zh.wikipedia.org/wiki/通信协议 "wikilink")，位于[OSI模型的](../Page/OSI模型.md "wikilink")[传输层](../Page/传输层.md "wikilink")。该协议由在1980年设计且在-{}-RFC
768-{}-中被规范。典型网络上的众多使用UDP协议的关键应用在一定程度上是相似的。
...
UDP适用于不需要或在[程序中执行](../Page/计算机程序.md "wikilink")[错误检查和纠正的](../Page/错误检测与纠正.md "wikilink")[应用](../Page/应用程序.md "wikilink")，它避免了[协议栈中此类处理的](../Page/协议栈.md "wikilink")。对时间有较高要求的应用程序通常使用UDP，因为丢弃数据包比等待或重传导致延迟更可取。

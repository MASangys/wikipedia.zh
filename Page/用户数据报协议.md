**用户数据-{zh-hans:报;zh-hant:包}-协议**（，縮寫：**UDP**；又稱**用户数据-{zh-hans:包;zh-hant:报}-协议**）是一个简单的面向[数据报的](https://zh.wikipedia.org/wiki/数据报 "wikilink")[通信协议](https://zh.wikipedia.org/wiki/通信协议 "wikilink")，位于[OSI模型](../Page/OSI模型.md "wikilink")的[传输层](../Page/传输层.md "wikilink")。该协议由在1980年设计且在-{}-RFC 768-{}-中被规范。典型网络上的众多使用UDP协议的关键应用在一定程度上是相似的。

在[TCP/IP模型中](https://zh.wikipedia.org/wiki/TCP/IP "wikilink")，UDP为[网络层](../Page/网络层.md "wikilink")以上和[应用层以下提供了一个简单的接口](https://zh.wikipedia.org/wiki/应用层 "wikilink")。UDP只提供[数据的不可靠传递](https://zh.wikipedia.org/wiki/数据 "wikilink")，它一旦把应用程序发给网络层的数据发送出去，就不保留数据备份（所以UDP有时候也被认为是不可靠的数据报协议）。UDP在IP数据报的头部仅仅加入了复用和数据校验字段。

**監督式學習**（），是[機器學習的一種方法](https://zh.wikipedia.org/wiki/機器學習 "wikilink")，可以由訓練資料中學到或建立一個模式（函數
/ learning
model），並依此模式推測新的实例。\[1\][訓練資料是由輸入物件](https://zh.wikipedia.org/wiki/訓練資料 "wikilink")（通常是向量）和預期輸出所組成。函數的輸出可以是一個連續的值（稱為[迴歸分析](../Page/迴歸分析.md "wikilink")），或是預測一個分類標籤（稱作[分類](https://zh.wikipedia.org/wiki/分類 "wikilink")）。

一個監督式學習者的任務在觀察完一些事先標記過的訓練範例（輸入和預期輸出）後，去預測這個函數對任何可能出現的輸入的输出。要達到此目的，學習者必須以"合理"（見[歸納偏向](https://zh.wikipedia.org/wiki/歸納偏向 "wikilink")）的方式從現有的資料中一般化到非觀察到的情況。在人類和動物感知中，則通常被稱為[概念學習](https://zh.wikipedia.org/wiki/概念學習 "wikilink")（concept
learning）。
...
假設符合*g*行為的樣本集合是從某個更大甚至是無限的母體中，根據某種未知的[概率分布](../Page/概率分布.md "wikilink")*p*，以[独立同分布随机变量方式來取樣](https://zh.wikipedia.org/wiki/独立同分布随机变量 "wikilink")。則可以假設存在某個跟任務相關的[损失函数](https://zh.wikipedia.org/wiki/损失函数 "wikilink")
*L*

\[L: Y\times Y \to \mathbb{R}^+\]

其中，*Y*是*g*的[陪域](https://zh.wikipedia.org/wiki/陪域 "wikilink")，且*L*會對應到非負[實數](https://zh.wikipedia.org/wiki/實數 "wikilink")（*L*可能有其它限制）。如果預測出來*g*的值是*z*，但實際值是*y*，而*L*(*z*,
*y*)這個量是其間的*損失*。
...
## 應用
  ...
  - [化學資訊學](https://zh.wikipedia.org/wiki/化学信息学 "wikilink")
...
1.  Stuart J. Russell, Peter Norvig (2010) *[Artificial Intelligence: A
    Modern
    Approach](https://zh.wikipedia.org/wiki/Artificial_Intelligence:_A_Modern_Approach "wikilink"),

...
当网站违背用户期望时，网站之间模糊的边际会导致其他问题。有些时候，直接链接也可用于恶意目的。
  ...
  - 内链对象可能在客户端上执行[路过式下载](https://zh.wikipedia.org/wiki/路过式下载 "wikilink")， exploiting faults in the code that interprets the objects. When an object is stored on an external server, the referring site has no control over if and when an originally beneficial object's content is replaced by malicious content.

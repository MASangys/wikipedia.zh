**相对熵**（relative entropy）又称为**KL散度**（**Kullback–Leibler
divergence**，簡稱**KLD**）\[1\]，信息散度（information
divergence），信息增益（information gain）。
...
對於[离散隨機变量](https://zh.wikipedia.org/wiki/离散隨機变量 "wikilink")，其概率分布*P* 和
*Q*的KL散度可按下式定義為
...
對於[連續隨機變量](https://zh.wikipedia.org/wiki/連續隨機變量 "wikilink")，其概率分佈*P*和*Q*可按積分方式定義為
\[2\]
...
由[吉布斯不等式可知](../Page/吉布斯不等式.md "wikilink")，當且僅當*P* = *Q*時*D*<sub>KL</sub>(*P*||*Q*)為零。

尽管从直觉上KL散度是个[度量或距离函数](../Page/度量.md "wikilink"),
但是它实际上并不是一个真正的度量或距離。因為KL散度不具有对称性：从分布*P*到*Q*的距离通常并不等于从*Q*到*P*的距离。

\[D_{\mathrm{KL}}(P\|Q) \neq D_{\mathrm{KL}}(Q\|P)\]

## KL散度和其它量的关系

[自信息和KL散度](../Page/自信息.md "wikilink")

\[I(m) = D_{\mathrm{KL}}(\delta_{im} \| \{ p_i \}),\]
[互信息和KL散度](../Page/互信息.md "wikilink")
...
[交叉熵和KL散度](https://zh.wikipedia.org/wiki/交叉熵 "wikilink")

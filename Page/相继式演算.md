...
Gentzen](https://zh.wikipedia.org/wiki/Gerhard_Gentzen "wikilink")）引入，作为研究[自然演绎的工具](../Page/自然演绎.md "wikilink")；它的名字得来自德语的“Logischer
...
\[B\]，
这个\(B\)是[一阶逻辑的任何公式](../Page/一阶逻辑.md "wikilink")（或演绎系统适用的任何逻辑，比如[命题演算或](https://zh.wikipedia.org/wiki/命题演算 "wikilink")[高阶逻辑或](../Page/高阶逻辑.md "wikilink")[模态逻辑](../Page/模态逻辑.md "wikilink")）。定理出现为有效证明中结论判断。希尔伯特演绎系统不需要区分公式和判断；提及它只是为了和下面的情况做比较。
...
这里的\(A_i\)和\(B\)是公式并且\(n\geq 0\)。用语言表述，判断组成自[十字转门符号](https://zh.wikipedia.org/wiki/十字转门 "wikilink")“\(\vdash\)”左手端的公式（可能为空）列表与右手端的一个单一公式。定理是那些公式\(B\)使得\(\vdash B\)（带有空左手端）是一个有效证明的结论。（在某些自然演绎的介绍中，\(A_i\)和十字转门是不明显写出来的，转而使用二维表示法）。
...
作为一个例子，下面是叫做*[排中律](https://zh.wikipedia.org/wiki/排中律 "wikilink")*（拉丁语*tertium
non datur*）的\(\vdash(A\vee \neg A)\)的序列推导。
...
结构规则需要某些额外的讨论。规则的名字是**弱化**（\(W\)）、**紧缩**（\(C\)）和**排列**（\(P\)）。紧缩和排列确保序列元素的次序（\(P\)）和多次出现（\(C\)）都不重要。就是说，你可以不把它当作[序列而作为](../Page/序列.md "wikilink")[集合](https://zh.wikipedia.org/wiki/集合 "wikilink")。

但是使用序列的额外努力是正当的，因为部分或全部的结构规则可以被忽略。这么做了就得到了所谓的[亚结构逻辑](../Page/亚结构逻辑.md "wikilink")。

### LK系统的性质

这个规则系统可以被证明关于[一阶逻辑是](../Page/一阶逻辑.md "wikilink")[可靠的和](https://zh.wikipedia.org/wiki/可靠性 "wikilink")[完备的](../Page/完备性.md "wikilink")，就是说，从前提的集合Γ语义上得到的一个陈述\((\Gamma \vDash A)\)，[当且仅当相继式](../Page/当且仅当.md "wikilink")\(\Gamma \vdash A\)可以用上述规则推导出来。

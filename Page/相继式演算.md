在[证明论和](../Page/证明论.md "wikilink")[数理逻辑中](../Page/数理逻辑.md "wikilink")，**相继式演算**（又译矢列演算、矢列式演算、序贯演算）是[一阶逻辑](../Page/一阶逻辑.md "wikilink")（和作为它的特殊情况的[命题逻辑](../Page/命题逻辑.md "wikilink")）、[模态逻辑等逻辑的一类](../Page/模态逻辑.md "wikilink")。第一个相继式演算\(LK\)由格哈德·根岑（[Gerhard
Gentzen](../Page/Gerhard_Gentzen.md "wikilink")）引入，作为研究[自然演绎的工具](../Page/自然演绎.md "wikilink")；它的名字得来自德语的“Logischer
Kalkül”，意思是“逻辑演算”。相继式演算系统有时被称为**Gentzen系统**，但使用时应避免与同为Gentzen发明的证明演算[自然演绎混淆](../Page/自然演绎.md "wikilink")。自然演绎、[公理系统和相继式演算是常见的证明演算](../Page/公理系统.md "wikilink")。

相继式演算是逻辑研究的重要工具。许多逻辑学者会为其所研究的逻辑构造出一个或多个相继式演算，并研究其性质（如[切消定理](../Page/切消定理.md "wikilink")）。

## 介绍

分类不同风格的演绎系统的一种方式是查看在系统中“判断”的形式，就是说，什么事物可以作为（子）证明的结论出现。最简单的判断形式是用在[希尔伯特演绎系统中的](../Page/希尔伯特演绎系统.md "wikilink")，这里的判断有形式

\[B\]，
这个\(B\)是[一阶逻辑的任何公式](../Page/一阶逻辑.md "wikilink")（或演绎系统适用的任何逻辑，比如[命题演算或](../Page/命题演算.md "wikilink")[高阶逻辑或](../Page/高阶逻辑.md "wikilink")[模态逻辑](../Page/模态逻辑.md "wikilink")）。定理出现为有效证明中结论判断。希尔伯特演绎系统不需要区分公式和判断；提及它只是为了和下面的情况做比较。
...
这里的\(A_i\)和\(B\)是公式并且\(n\geq 0\)。用语言表述，判断组成自[十字转门符号](../Page/十字转门.md "wikilink")“\(\vdash\)”左手端的公式（可能为空）列表与右手端的一个单一公式。定理是那些公式\(B\)使得\(\vdash B\)（带有空左手端）是一个有效证明的结论。（在某些自然演绎的介绍中，\(A_i\)和十字转门是不明显写出来的，转而使用二维表示法）。

在自然演绎中判断的标准语义是断言只要\[1\] \(A_1\), \(A_2\)等都是真的，\(B\)就也是真的。判断
...
在第一印象上，这种判断的扩展可能带来奇怪的复杂性—它不是由于自然演绎的有明显缺陷而激发来的，人们最初被逗号在十字转门的两侧完全意味着不同的事物搞糊涂了。但是观察到相继式的语义还可以（通过命题[重言式](../Page/重言式.md "wikilink")）被表达为

  -
    \(\neg A_1 \lor \neg A_2 \lor \cdots \neg A_n \lor B_1 \lor B_2 \lor\cdots\lor B_k\)

在这种公式中，在十字转门两侧的公式间的唯一不同是在左侧的公式被否定了。因为对换相继式左右侧的公式对应于否定所有构成公式。这意味着对称性，如逻辑否定的[德·摩根定律在语义层次上所显现的](../Page/德·摩根定律.md "wikilink")，直接转换到了相继式的左右对称—实际上，在相继式中处理合取（\(\land\)）、的推理规则处理析取（\(\lor\)）的推理规则的镜像。

很多逻辑学家觉得这种对应的对称表述允许提供比其他证明系统在逻辑结构上的深刻洞察，这里的否定的对偶性不出现在规则中。

## LK系统

在这个演算中的（形式）证明是一序列的[相继式](../Page/相继式.md "wikilink")，这个的每个相继式使用下面的[推理规则之一而推导自更早出现的相继式](../Page/推理规则.md "wikilink")。
...
在查找某个证明的时候，多数规则提供或多或少的如何做的处方。切规则是不同的：它声称，在公式\(A\)可以被推导出来，并且这个公式也可以充当推导其他公式的前提的时候，则公式\(A\)可以被"切掉"并把各自的推导连接起来。在自底向上构造一个证明的时候，这产生了猜测\(A\)的问题（因为它在下面根本就没有出现）。这个问题在[切消定理中处理](../Page/切消定理.md "wikilink")。

第二个规则有某种特殊性，它就是同一性公理\((I)\)。明显的直觉读为：\(A\)证明\(A\)。

### 例子推导

作为一个例子，下面是叫做*[排中律](../Page/排中律.md "wikilink")*（拉丁语*tertium non
datur*）的\(\vdash(A\vee \neg A)\)的序列推导。
...
结构规则需要某些额外的讨论。规则的名字是**弱化**（\(W\)）、**紧缩**（\(C\)）和**排列**（\(P\)）。紧缩和排列确保序列元素的次序（\(P\)）和多次出现（\(C\)）都不重要。就是说，你可以不把它当作[序列而作为](../Page/序列.md "wikilink")[集合](../Page/集合.md "wikilink")。

但是使用序列的额外努力是正当的，因为部分或全部的结构规则可以被忽略。这么做了就得到了所谓的[亚结构逻辑](../Page/亚结构逻辑.md "wikilink")。

### LK系统的性质

这个规则系统可以被证明关于[一阶逻辑是](../Page/一阶逻辑.md "wikilink")[可靠的和](../Page/可靠性.md "wikilink")[完备的](../Page/完备性.md "wikilink")，就是说，从前提的集合Γ语义上得到的一个陈述\((\Gamma \vDash A)\)，[当且仅当相继式](../Page/当且仅当.md "wikilink")\(\Gamma \vdash A\)可以用上述规则推导出来。
...
首先，如上面提及的，相继式可以被看做由集合或[多重集组成](../Page/多重集.md "wikilink")。在这种情况下，排列和（在使用集合的时候）紧缩公式的规则就可以废弃了。
...
人们可以限制或禁用某个结构规则。这产生了[亚结构逻辑系统变体](../Page/亚结构逻辑.md "wikilink")。它们一般要弱于\(LK\)（就是说有更少的定理），因为关于标准的一阶逻辑语义是不完备的。但是它们的其他有趣性质导致其在理论[计算机科学和](../Page/计算机科学.md "wikilink")[人工智能中的应用](../Page/人工智能.md "wikilink")。

### LJ系统

令人惊讶的，\(LK\)的规则的一些细小的变更就足以把它变成[直觉逻辑的证明系统](../Page/直觉逻辑.md "wikilink")。你必须限制使用[直觉相继式](../Page/相继式.md "wikilink")（就是说，右上下文被消去）并修改规则\((\vee L)\)为如下：
...
1.  这里的“只要”是如下的非正式略写“对值到判断中的自由变量的所有指派”
...
使用着色器在图形硬件上计算[渲染](../Page/渲染.md "wikilink")效果有很高的自由度。尽管不是硬性要求，但目前大多数着色器是针对[GPU开发的](https://zh.wikipedia.org/wiki/GPU "wikilink")。GPU的可编程[绘图管线已经全面取代传统的固定管线](https://zh.wikipedia.org/wiki/绘图管线 "wikilink")，可以使用着色器语言对其编程。构成最终图像的[像素](../Page/像素.md "wikilink")、[顶点](https://zh.wikipedia.org/wiki/顶点 "wikilink")、[纹理](https://zh.wikipedia.org/wiki/纹理贴图 "wikilink")，它们的位置、色相、饱和度、亮度、对比度也都可以利用着色器中定义的[算法](../Page/算法.md "wikilink")进行动态调整。调用着色器的外部程序，也可以利用它向着色器提供的外部[变量](https://zh.wikipedia.org/wiki/变量 "wikilink")、纹理来修改这些着色器中的参数。
...
随着[图形处理器的进步](https://zh.wikipedia.org/wiki/图形处理器 "wikilink")，[OpenGL](../Page/OpenGL.md "wikilink")和[Direct3D](../Page/Direct3D.md "wikilink")等主要的图形软件库都开始支持着色器。第一批支持着色器的
GPU
...
三维着色器处理的是[三维模型](../Page/三维模型.md "wikilink")或者其它几何体，可以访问用来绘制模型的颜色和纹理。顶点着色器是最早的三维着色器；几何着色器可以在着色器中生成新的顶点；细分曲面着色器（）则可以向一组顶点中添加细节。
...
着色器被用来同时处理大量的数据，比如屏幕上的一整块像素群，或者一个模型结构的所有顶点。[并行计算](../Page/并行计算.md "wikilink")适用于这样的情况，而且当今的GPU也设计有多核结构来极大的提高处理效率。

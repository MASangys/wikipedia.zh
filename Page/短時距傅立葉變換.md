**短時距傅立葉變換**是[傅立葉變換的一種變形](../Page/傅立葉變換.md "wikilink")，也稱作windowed
...
## 與[傅立葉轉換在概念上的區別](../Page/傅立葉轉換.md "wikilink")

將訊號做[傅立葉變換後得到的結果](../Page/傅立葉變換.md "wikilink")，並不能給予關於信號頻率隨時間改變的任何資訊。以下的例子作為說明：
...
[傅立葉變換後的頻譜和短時距傅立葉轉換後的結果如下](../Page/傅立葉變換.md "wikilink")：
...
簡單來說，在連續時間的例子，一個函數可以先乘上僅在一段時間不為零的[窗函數再進行一維的](../Page/窗函數.md "wikilink")[傅立葉變換](../Page/傅立葉變換.md "wikilink")。再將這個窗函數沿著時間軸挪移，所得到一系列的傅立葉變換結果排開則成為二維表象。數學上，這樣的操作可寫為：

\[X(t, f) = \int_{-\infty}^{\infty} w(t-\tau)x(\tau)  e^{-j 2 \pi f \tau} \, d\tau\]
另外也可用[角頻率來表示](../Page/角頻率.md "wikilink")：

\[X(t, \omega) = \int_{-\infty}^{\infty} w(t-\tau)x(\tau)  e^{-j \omega \tau} \, d\tau\]
其中\(w(t)\)是[窗函數](../Page/窗函數.md "wikilink")，窗函數種類有很多種，會在稍後再做仔細討論。\(x(t)\)是待變換的訊號。\(X(t,\omega)\)是\(w(t-\tau)x(\tau)\)的傅立葉變換。
...
相同地，其中\(w[n]\)是[窗函數](../Page/窗函數.md "wikilink")，\(x[n]\)是待變換的訊號。在這個例子裡，m是離散的且ω是連續的，但大部分實際的應用當中，短時距傅立葉轉換在電腦中都是以快速傅立葉轉換進行計算(見實現方法的快速傅立葉變換)，而此時這兩個參數都是離散且被量化的。
...
反短時距傅立葉轉換，其數學類似[傅立葉轉換](../Page/傅立葉轉換.md "wikilink")，但須消除窗函數的作用，首先必須先將窗函數的總面積規模化使得
...
[窗函數通常滿足下列特性](../Page/窗函數.md "wikilink"):
...
常見的[窗函數有](../Page/窗函數.md "wikilink")：方形、三角形、[高斯函數等](../Page/高斯函數.md "wikilink")，而短時距傅立葉轉換也因窗函數的不同而有不同的名稱。而[加伯轉換](../Page/加伯轉換.md "wikilink")，即為窗函數是高斯函數的短時距傅立葉轉換，通常沒有特別說明的短時距傅立葉轉換，即為[加伯轉換](../Page/加伯轉換.md "wikilink")。
...
其大部分的特性都與[傅立葉轉換的特性相對應](../Page/傅立葉轉換.md "wikilink")
...
[DifferentB.JPG](https://zh.wikipedia.org/wiki/File:DifferentB.JPG "fig:DifferentB.JPG")
  ...
  - 接著我們來分析\(x(t) = 1\)，信號因為沒有改變，應該為DC。若短時距傅立葉轉換是理想的話，\(X(t,f)\)應該只有在\(f=0\)的時候有能量。但由上面的特性可發現，能量會沿著頻率軸呈現[sinc函數](../Page/sinc函數.md "wikilink")。若我們取較大的\(B\)，可使[sinc函數沿著頻率軸變窄](../Page/sinc函數.md "wikilink")，使得結果趨近理想。

...
磁盘分区可做看作是[逻辑卷管理](https://zh.wikipedia.org/wiki/邏輯捲軸管理 "wikilink")、[软件RAID前身的一项简单技术](https://zh.wikipedia.org/wiki/软件RAID "wikilink")。
...
zh-hant:分割;}-允许在一个磁盘上有多个[文件系统](../Page/文件系统.md "wikilink")。有许多理由需要这么做：
  ...
  - 碍于技术限制（例如旧版的微软[FAT文件系统不能访问超过一定的磁盘空间](https://zh.wikipedia.org/wiki/File_Allocation_Table "wikilink")；旧的[PC
    BIOS不允许从超过硬盘](../Page/BIOS.md "wikilink")1024个柱面的位置[启动操作系统](https://zh.wikipedia.org/wiki/启动 "wikilink")）
  ...
  - 在运行[Unix的多用户系统上](https://zh.wikipedia.org/wiki/Unix "wikilink")，有可能需要防止用户的硬连结攻击。为了达到这个目的，/home和/tmp路径必须与如/var和/etc下的系统文件分开。

## 分区机制

### 微软视窗

[微软视窗的标准分区机制是创建一个分区](https://zh.wikipedia.org/wiki/Microsoft_Windows "wikilink")
C:，其中操作系统、数据和程序都在这个分区上。如果可能的话，在不包含操作系统的硬盘上，为交换文件建立一个单独的分区，尽管这并不意味着两个硬盘都不会断电。在进行一点儿预分区工作之后，很容易就可实现操作系统不存储在
C 分区上甚至是 C
分区根本就不存在。这样做有一些益处，一些设计拙劣的病毒或者特洛伊木马将不能覆盖关键的系统文件或者控制系统。“我的文档”文件夹、“特殊文件夹”[主目录可以加载到一个独立分区上以利用所有空闲空间](https://zh.wikipedia.org/wiki/主目录 "wikilink")。
...
\]\]
对于基于[UNIX或者如](https://zh.wikipedia.org/wiki/Unix "wikilink")[Linux这样类似于Unix的操作系统来说](../Page/Linux.md "wikilink")，分区系统创建了
/、/boot、/home、/tmp、/usr、/var、/opt和[交换分区](https://zh.wikipedia.org/wiki/交换分区 "wikilink")。这就保证了如果其中一个[文件系统损坏](../Page/文件系统.md "wikilink")，其它的数据（其它的文件系统）不受影响，这样就减少了数据丢失。这样做的一个缺点是将整个驱动器划分成固定大小的小分区，例如，一个用户可能会填满
/home
分区并且用完可用硬盘空间，即使其它分区上还有充足的空闲空间。良好的实现方法要求用户预测每个分区可能需要的空间；有时这是一项困难的工作。典型的桌面系统使用另外一种约定；“/”（[根目录](https://zh.wikipedia.org/wiki/根目录 "wikilink")）分区包含整个文件系统和独立的交换分区。
/home

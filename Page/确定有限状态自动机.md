在[计算理论中](../Page/计算理论.md "wikilink")，**确定有限状态自动机**或**确定有限自动机**（）是一个能实现状态转移的[自动机](../Page/自动机.md "wikilink")。对于一个给定的属于该自动机的状态和一个属于该自动机字母表\(\Sigma\)的字符，它都能根据事先给定的转移函数转移到下一个状态（这个状态可以是先前那个状态）。
...
确定有限状态自动机\(\mathcal{A}\)是由
  ...
  - 一个非空有限的[状态集合](../Page/状态.md "wikilink")\(Q\)
  - 一个输入[字母表](../Page/字母表_\(计算机科学\).md "wikilink")\(\Sigma\)（非空有限的字符集合）
...
确定有限状态自动机从起始状态开始，一个字符接一个字符地读入一个字符串\(w \in \Sigma^*\)（这里的\({}^*\)指示[Kleene星号算子](../Page/Kleene星号.md "wikilink")。），并根据给定的转移函数一步一步地转移至下一个状态。在读完该字符串后，如果该自动机停在一个属于F的接受状态，那么它就接受该字符串，反之则拒绝该字符串。
...
DFA是一種实际的计算模型，因为有平凡的线性时间、恒定空间的[在线算法模拟在输入流上的DFA](../Page/在线算法.md "wikilink")。给定两个DFA有有效算法找到识别它们所识别语言的并集、交集和补集的DFA。还有有效算法确定一个DFA是否接受任何给定字符串，一个DFA是否接受所有字符串，两个DFA是否识别同样的语言，和对特定[正则语言找到状态数目最小的DFA](../Page/正则语言.md "wikilink")（最小DFA）。

在另一方面，DFA在可识别的语言上有严格的限制—很多简单的语言，包括需要多于恒定空间来解决的任何问题，不能被DFA识别。经典的DFA不能识别的简单语言的例子是括号语言，就是由正确配对的括号组成的语言，比如
(()())。由形如<em>a<sup>n</sup>b<sup>n</sup></em>的字符串组成的语言，就是有限数目个a，随后是相等数目个b。可以证明没有DFA有足够状态来识别这种语言（通俗地说，因为需要至少2n个状态，而n是不恒定的）。

### 其它

1.  能被确定有限状态自动机识别的语言是[正则语言](../Page/正则语言.md "wikilink")。
2.  确定有限状态自动机是[非确定有限状态自动机的一种极限形式](../Page/非确定有限状态自动机.md "wikilink")。
3.  确定有限状态自动机在计算能力上等价于非确定有限状态自动机。
4.  没有接受状态列表并没有指定开始状态的确定有限状态机叫做[转移系统或](../Page/转移系统.md "wikilink")[半自动机](../Page/半自动机.md "wikilink")。
...
确定有限状态自动机\(\mathcal{A} = \left( Q,\Sigma,\delta,s,F \right)\)
  ...
  - \(s=S_1\)
  - \(F=\{S_1\}\)
  - \(\delta\)由下面的[状态转移表定义](../Page/状态转移表.md "wikilink")：
...
\(\mathcal{A}\)能识别的语言是\(\mathcal{L} ( \mathcal{A} ) = \{ w | \#_0(w) \equiv 0~(mod~2) \}\)。用[正则表达式表示为](../Page/正则表达式.md "wikilink")：\((1^*(01^*0)^*)^*\)。
...
\(\mathcal{A}=(Q,\Sigma,\delta,s,F)\)是一个DFA，那么由补运算产生的新DFA定义为：\(\bar{\mathcal{A}}=(Q,\Sigma,\delta,s,Q-F)\)。显然只要将\(\mathcal{A}\)中接受的状态设为不接受的状态，同时把不接受的状态设为接受的状态就得到\(\bar{\mathcal{A}}\)。补运算的[复杂度是](../Page/计算复杂性理论.md "wikilink")：\(O(\left| Q \right|)\)。
...
1.  当\(M = F_1 \times F_2\)时，由上述方法得到的\(\mathcal{B}\)就是DFA
    \(\mathcal{A}_1\)和\(\mathcal{A}_2\)的交运算，记作：\(\mathcal{B} = \mathcal{A}_1 \cap \mathcal{A}_2\)。也就是说对于读入的字符串w，当且仅当\(\mathcal{A}_1\)和\(\mathcal{A}_2\)同时接受w的时候\(\mathcal{B}\)接受w。
2.  当\(M = Q_1 \times F_2 \bigcup F_1 \times Q_2\)时，由上述方法得到的\(\mathcal{B}\)就是DFA
    \(\mathcal{A}_1\)和\(\mathcal{A}_2\)的并运算，记作：\(\mathcal{B} = \mathcal{A}_1 \cup \mathcal{A}_2\)。也就是说对于读入的字符串w，只要\(\mathcal{A}_1\)或\(\mathcal{A}_2\)中至少有一个接受w，\(\mathcal{B}\)就接受w。
...
对于一个正则语言，接受该语言的[等价类自动机是一个](../Page/等价类.md "wikilink")\(~(Q,\Sigma,\delta,s,F)\)的5-元组。其定义如下：
...
\~<sub>L</sub>被称为Nerode关系，是[Myhill-Nerode定理的基础](../Page/Myhill-Nerode定理.md "wikilink")。简单的来说就是对于任意\(~x,y,z \in \Sigma^*\)，如果\(xz \in L \Leftrightarrow yz \in L\)，那么x\~<sub>L</sub>y。
...
## 引用
  ...
  - Michael Sipser, *Introduction to the Theory of Computation*. PWS,
    Boston. 1997. ISBN 0-534-94728-X. Section 1.1: Finite Automata,
    pp.31–47. Subsection "Decidable Problems Concerning Regular
    Languages" of section 4.1: Decidable Languages, pp.152–155.4.4 DFA
    can accept only regular language

## 参见

  - [无环确定有限状态自动机](../Page/无环确定有限状态自动机.md "wikilink")
  - [非确定有限状态自动机](../Page/非确定有限状态自动机.md "wikilink")
  - [图灵机](../Page/图灵机.md "wikilink")
  - [读只右移图灵机](../Page/读只右移图灵机.md "wikilink")

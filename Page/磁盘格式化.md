...
Format），对于部分硬盘制造厂商，它也被称为初始化（initialization）。最早，伴随着应用[CHS](../Page/CHS.md "wikilink")编址方法、[频率调制](../Page/频率调制.md "wikilink")（FM）、（MFM）等编码方案的磁盘的出现，**低级格式化**被用于指代对磁碟进行划分柱面、磁道、扇区的操作。现今，随着软盘的逐渐退出日常应用，应用新的编址方法和接口的磁盘的出现，这个词已经失去了原本的含义，大多数的硬盘制造商将低级格式化（Low-Level
...
通常的操作步骤是：先打开「电脑」，接著在希望被执行格式化的碟符图标上右击，然后选择“格式化”，再按照提示操作即可。需要注意的是：对硬碟执行格式化操作时，用户需要拥有[系统管理员](../Page/系统管理员.md "wikilink")权限（仅限于Windows
Vista以及此後推出的作業系統。）

在Windows环境中，除了可以使用[图形化的操作界面执行格式化操作之外](../Page/图形用户界面.md "wikilink")，也可以在[命令提示字元](../Page/命令提示字元.md "wikilink")中使用指令(僅限於[Windows

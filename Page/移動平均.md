...
**簡單移動平均**（，**SMA**）是某變數之前*n*個數值的未作加權[算術平均](https://zh.wikipedia.org/wiki/算術平均 "wikilink")。例如，收市價的10日簡單移動平均指之前10日收市價的平均數。若設收市價為\(p_1\)至\(p_n\)，則方程式為：
...
**指數移動平均**（，**EMA**或**EXMA**）是以[指數式遞減加權的移動平均](https://zh.wikipedia.org/wiki/指數 "wikilink")。各數值的加權影響力隨時間而指數式遞減，越近期的數據加權影響力越重，但較舊的數據也給予一定的加權值。右圖是一例子。

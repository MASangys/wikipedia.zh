...
[Eikonal_equation_traveling_wave_plot_1.gif](https://zh.wikipedia.org/wiki/File:Eikonal_equation_traveling_wave_plot_1.gif "fig:Eikonal_equation_traveling_wave_plot_1.gif")
\(g[1] := {u(x, t) = C4-(2*(C1+C2*x+C3*t))/\sqrt{C3^2+C2^2}}\)
\(g[2] := {u(x, t) = C4+(2*(C1+C2*x+C3*t))/\sqrt{C3^2+C2^2}}\)
...
1.  Inna Shingareva, Carlos Lizárraga-Celaya,Solving Nonlinear Partial
    Differential Equations with Maple p11 Springer
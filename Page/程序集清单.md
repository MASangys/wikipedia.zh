**程序集清单**（assembly manifest）是[.Net Framework中的文本文件](https://zh.wikipedia.org/wiki/.Net_Framework "wikilink")，包含了[CLI 程序集的](https://zh.wikipedia.org/wiki/程序集_\(CLI\) "wikilink")[元数据](../Page/元数据.md "wikilink")，包括程序集的组建间的依赖关系、解决资源与类的引用、版本信息、作用域信息、安全许可等。\[1\]存储在[PE文件](https://zh.wikipedia.org/wiki/Portable_Executable "wikilink")(.exe或.dll)用[通用中间语言](../Page/通用中间语言.md "wikilink") (MSIL) 代码表示。

...
由于地球是一个椭球体，因此在空间数据库之中，所有的几何体都是带有[地图投影](../Page/地图投影.md "wikilink")与空间参照系的。空间数量量算都是在一定的投影与参照系下进行。对于若干几何体，由于不同参照系的椭球参数不同，如果把他们放在不同的空间参照系下进行空间数量量算，会得出不同的结果。
...
[Dimensionally Extended Nine-Intersection Model (DE-9IM) by Christian Strobl](http://gis.hsr.ch/wiki/images/3/3d/9dem_springer.pdf)

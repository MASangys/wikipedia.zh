**端口扫描工具**（Port
Scanner）指用于探测[服务器或](../Page/服务器.md "wikilink")[主机开放](https://zh.wikipedia.org/wiki/主机 "wikilink")[端口情况的工具](https://zh.wikipedia.org/wiki/端口 "wikilink")。常被计算机[管理员用于确认安全策略](../Page/管理员.md "wikilink")，同时被攻击者用于识别目标主机上的可运作的网络服务。
...
扫描多个主机以获取特定的某个端口被称为**端口清扫**（Portsweep），以此获取特定的服务。例如，基于[SQL服务的](../Page/SQL.md "wikilink")[计算机蠕虫就会清扫大量主机的同一端口以在](https://zh.wikipedia.org/wiki/计算机蠕虫 "wikilink")
1433 端口上建立[TCP连接](https://zh.wikipedia.org/wiki/TCP "wikilink")。\[2\]

## 类型

### TCP扫描

最简单的端口扫描工具使用[操作系统原生的网络功能](../Page/操作系统.md "wikilink")，且通常作为[SYN扫描的替代选项](https://zh.wikipedia.org/wiki/TCP#建立通路 "wikilink")。[Nmap将这种模式称为连接扫描](../Page/Nmap.md "wikilink")，因为使用了类似[Unix系统的](https://zh.wikipedia.org/wiki/Unix "wikilink")`connect()`命令。如果该端口是开放的，操作系统就能完成[TCP三次握手](https://zh.wikipedia.org/wiki/TCP "wikilink")，然后端口扫描工具会立即关闭刚建立的该连接，防止[拒绝服务攻击](https://zh.wikipedia.org/wiki/拒绝服务攻击 "wikilink")。\[3\]这种扫描模式的优势是用户无需特殊权限。但使用操作系统原生网络功能不能实现底层控制，因此这种扫描方式并不流行。并且TCP扫描很容易被发现，尤其作为端口清扫的手段：这些服务会记录发送者的[IP地址](https://zh.wikipedia.org/wiki/IP "wikilink")，[入侵检测系统可能触发警报](../Page/入侵检测系统.md "wikilink")。
...
UDP扫描也是可能的,尽管存在一些技术挑战。 UDP是无连接协议，因此没有等同于TCP
SYN的数据包。但是，如果将UDP数据包发送到未打开的端口，目标系统将响应ICMP端口不可达的消息。大多数UDP端口扫描器都使用这种扫描方法，并使用缺少响应来推断端口是否打开。但是，如果端口被防火墙阻止，则此方法将错误地报告端口已打开。如果端口不可达消息被阻塞，则所有端口将显示为打开。这种方法也受到ICMP速率限制的影响。
...
3.  Erikson, Jon (1977). HACKING the art of exploitation (2nd ed.). San
    Francisco: NoStarch Press. p. 264. ISBN 1-59327-144-1.

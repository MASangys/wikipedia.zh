在[计算机科学中](../Page/计算机科学.md "wikilink")，**符号表**是一种用于语言翻译器（例如[编译器和](https://zh.wikipedia.org/wiki/编译器 "wikilink")[解释器](https://zh.wikipedia.org/wiki/解释器 "wikilink")）中的数据结构。在符号表中，程序[源代码中的每个标识符都和它的声明或使用信息绑定在一起](../Page/源代码.md "wikilink")，比如其[数据类型](https://zh.wikipedia.org/wiki/数据类型 "wikilink")、[作用域以及](../Page/作用域.md "wikilink")[内存地址](https://zh.wikipedia.org/wiki/物理地址 "wikilink")。

## 实现

[散列表是用来实现符号表的一种常用技术](https://zh.wikipedia.org/wiki/散列表 "wikilink")。编译器可能会使用一个很大的符号表来包含所有的符号，或是针对不同的[作用域使用层次结构的多个独立的符号表](../Page/作用域.md "wikilink")。

## 使用

[目标文件中通常会有一个包含了所有外部可见标识符的符号表](https://zh.wikipedia.org/wiki/目标文件 "wikilink")。在链接不同的目标文件时，[链接器会使用这些文件中的符号表来解析所有未解析的符号引用](../Page/链接器.md "wikilink")。
...
下图是一个小程序的符号表。该表是用[GNU
Binutils中的](../Page/GNU_Binutils.md "wikilink")[nm工具生成的](https://zh.wikipedia.org/wiki/nm_\(Unix\) "wikilink")。其中有一个数据类型符号（由类型
"D"
表明）和许多函数（自己定义的以及标准库中的）。第一列是每个符号在内存中的位置，第二列是"[符号类型](http://sourceware.org/binutils/docs-2.17/binutils/nm.html#nm)"，而第三列则是符号名称。通过传递适当的参数，可以按照地址顺序生成该符号表。
...
## 参见
  ...
  - [Debug
    symbol](https://zh.wikipedia.org/wiki/Debug_symbol "wikilink")

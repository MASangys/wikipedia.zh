**符号链接**（***软链接***）是一类特殊的[文件](https://zh.wikipedia.org/wiki/计算机文件 "wikilink")，
其包含有一条以[绝对路径或者](https://zh.wikipedia.org/wiki/绝对路径 "wikilink")[相对路径的形式指向其它文件或者目录的引用](https://zh.wikipedia.org/wiki/相对路径 "wikilink")。\[1\]
符号链接最早在4.2[BSD版本中出现](../Page/BSD.md "wikilink")（1983年）。今天[POSIX操作系统标准](https://zh.wikipedia.org/wiki/POSIX "wikilink")、大多数[类Unix系统](../Page/类Unix系统.md "wikilink")、[Windows
Vista](../Page/Windows_Vista.md "wikilink")、[Windows
7都支持符号链接](https://zh.wikipedia.org/wiki/Windows_7 "wikilink")。[Windows
...
OS还能使用](https://zh.wikipedia.org/wiki/Mac_OS "wikilink")[别名](https://zh.wikipedia.org/wiki/别名 "wikilink")（aliases）。当目标文件移动到同一磁盘上的其它地方时，别名仍然够继续有效。某些版本的[Linux系统也有与别名类似的功能](../Page/Linux.md "wikilink")。
...
[文件系统的](../Page/文件系统.md "wikilink")[重解析点功能](https://zh.wikipedia.org/wiki/重解析点 "wikilink")，具体请参考
NTFS 的相关介绍。

Windows Vista 使用 mklink
命令（注意这是[内部命令](https://zh.wikipedia.org/wiki/内部命令 "wikilink")，而非[可执行程序](https://zh.wikipedia.org/wiki/可执行程序 "wikilink")）创建符号连接，命令行如下：
...
[快捷方式](https://zh.wikipedia.org/wiki/快捷方式 "wikilink")（shortcut）是一种功能上类似符号链接的[文件对象](https://zh.wikipedia.org/wiki/文件 "wikilink")，但与符号链接有本质的不同。快捷方式是普通的文件（拥有扩展名
.lnk），而非符号，因此，快捷方式可以被复制、移动、更改（某些特殊的快捷方式无法更改所有信息）或删除。快捷方式可以指向文件、文件夹或其他任何系统中合法的位置（包括[控制面板](../Page/控制面板.md "wikilink")、[桌面等](https://zh.wikipedia.org/wiki/桌面 "wikilink")）。

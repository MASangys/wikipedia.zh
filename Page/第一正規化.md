> 本文内容由[第一正規化](https://zh.wikipedia.org/wiki/第一正規化)转换而来。


**第一正規化**（1NF）是[資料庫正規化所使用的](https://zh.wikipedia.org/wiki/資料庫正規化 "wikilink")[正規形式](https://zh.wikipedia.org/wiki/正規形式 "wikilink")。第一正規化是為了要排除 *重複群* 的出現，所採用的方法是要求資料庫的每个列的值域都是由原子值组成；每個欄位的值都只能是單一值。1971年[埃德加·科德](../Page/埃德加·科德.md "wikilink")提出了第一正規化。
...
[de:Normalisierung (Datenbank)\#Erste Normalform .281NF.29](https://zh.wikipedia.org/wiki/de:Normalisierung_\(Datenbank\)#Erste_Normalform_.281NF.29 "wikilink")

**第一正規化**（1NF，-{zh-tw:中國大陸及香港譯作**第一範式**;zh-hk:臺灣譯作**第一正規化**}-）是[資料庫正規化所使用的](https://zh.wikipedia.org/wiki/資料庫正規化 "wikilink")[正規形式](https://zh.wikipedia.org/wiki/正規形式 "wikilink")。第一正規化是為了要排除 *重複群* 的出現，所採用的方法是要求資料庫的每个列的值域都是由原子值组成；每個欄位的值都只能是單一值。1971年[埃德加·科德](../Page/埃德加·科德.md "wikilink")提出了第一正規化。

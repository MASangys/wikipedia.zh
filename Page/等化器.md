> 本文内容由[等化器](https://zh.wikipedia.org/wiki/等化器)转换而来。


[Com_DigidesignProfile.jpg](https://zh.wikipedia.org/wiki/File:Com_DigidesignProfile.jpg "fig:Com_DigidesignProfile.jpg") **等化器**（Equalizer）在[通訊系統中是很重要的一部分](https://zh.wikipedia.org/wiki/通訊系統 "wikilink")，由於傳送[信號在傳送路徑到接收器接收的過程中會受到](https://zh.wikipedia.org/wiki/信號 "wikilink")[多路徑干擾](https://zh.wikipedia.org/wiki/多路徑干擾 "wikilink")（multipath）、路徑中遮蔽物阻擋造成[遮蔽效應](https://zh.wikipedia.org/wiki/遮蔽效應 "wikilink")（shadow effect），這些現象都會造成接收訊號錯誤率上升。因此為了降低通訊系統傳輸的錯誤率要作通道估測，經由估測的結果對通道響應做補償進而降低傳送錯誤率。现亦有软件可利用等化器处理声音，将声音专门为某类优化，如摇滚、流行、舞曲、古典、柔和等。
...
\(d(n)=h(n)+e_{opt}(n)\) \(\Rightarrow E(d(n)h(n))=E(h(n)^{2})+E(h(n)e_{opt}(n))\) \(= >J_{MIN}=\sigma _{d}^{2}+\sigma _{h}^{2}-2\sigma _{h}^{2}=\sigma _{d}^{2}-\sigma _{h}^{2}\)
...
\(\text{d}_{n}=-\sum\limits_{m--k}^{0}{e_{m}f_{n-m}}\) for n=1,...,\(K_b\)（feedback filter的tap數）

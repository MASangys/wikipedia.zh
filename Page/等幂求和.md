...
伯努利数也通用於[等差数列的等幂和](../Page/等差数列.md "wikilink")。\[3\]

\(\sum_{i=1}^n (a_1+(i-1)d)^m = \frac{1}{m+1} \sum_{i=0}^m B_i d^{i-1} {m+1\choose i}(a_{n+1}^{m+1-i}-a_1^{m+1-i})\)

也可以利用[帕斯卡矩阵](https://zh.wikipedia.org/wiki/帕斯卡矩阵 "wikilink")，把多项式的和写成矩阵相乘。
...
也可以将数列表达成组合数然后利用[朱世杰恒等式求和](https://zh.wikipedia.org/wiki/朱世杰恒等式 "wikilink")。
...
[求和](https://zh.wikipedia.org/wiki/分类:求和法 "wikilink")

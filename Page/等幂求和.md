**等幂求和**，求幂数相同的变数之和\(\sum_{i=1}^{n} x_i^m\)。

## 常见公式

  - [三角形數](../Page/三角形數.md "wikilink")：\(\sum^{n}_{i=1} i = \frac{n(n+1)}{2}\)
  - [正方形數](https://zh.wikipedia.org/wiki/正方形數 "wikilink")：\(\sum_{i=1}^{n} (2i - 1) = n^2\)
  - [調和級數](https://zh.wikipedia.org/wiki/調和級數 "wikilink")：\(\sum_{n=1}^k\,\frac{1}{n} \;=\; \ln k + \gamma + \varepsilon_k\)
...
\(\sum_{i=0}^{n} i^m = {1\over{m+1}}\sum_{i=0}^m{m+1\choose{i}} B_i (n+1)^{m+1-i}\)\[1\]，其中B<sub>*i*</sub>是[伯努利数](../Page/伯努利数.md "wikilink")。

\(\displaystyle \sum_{i=1}^n i^{m+1} = \sum_{k=0}^m  L_k^m \binom{n+k+1}{m+2},\left(L_k^m = \sum_{r=0}^k (-1)^r \binom{m+2}{r} (k+1-r)^{m+1}\right)\)\[2\]

### 多项式求和

伯努利数也通用於[等差数列](../Page/等差数列.md "wikilink")的等幂和。\[3\]

\(\sum_{i=1}^n (a_1+(i-1)d)^m = \frac{1}{m+1} \sum_{i=0}^m B_i d^{i-1} {m+1\choose i}(a_{n+1}^{m+1-i}-a_1^{m+1-i})\)

也可以利用[帕斯卡矩阵](../Page/帕斯卡矩阵.md "wikilink")，把多项式的和写成矩阵相乘。
...
=\sum_{j=1}^{m+1} C_{n}^{j}\Delta^{j-1}p(1)\) \[4\] \[5\] \[6\]

  -
    其中\(\Delta p(n)=p(n+1)-p(n)\)

也可以将数列表达成组合数然后利用[朱世杰恒等式](../Page/朱世杰恒等式.md "wikilink")求和。

\[\sum_{i=1}^n \left(i^2-i\right)=2\sum_{i=1}^n C_i^2=2C_{n+1}^3\]\[7\]
...
\(s_m+a_1s_{m-1}+a_2s_{m-2}+...+a_{m-1}s_1+ma_m=0\)\[8\]
...
## 参见
  ...
  - [斯特靈數](https://zh.wikipedia.org/wiki/斯特灵数 "wikilink")
  - [求和](https://zh.wikipedia.org/wiki/求和 "wikilink")

## 参考资料

[求和](https://zh.wikipedia.org/wiki/分类:求和法 "wikilink") [分类:初等数学](https://zh.wikipedia.org/wiki/分类:初等数学 "wikilink")

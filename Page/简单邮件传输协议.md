...
由于这个协议开始是基于纯[ASCII](../Page/ASCII.md "wikilink")文本的，它在[二进制文件上处理得并不好](https://zh.wikipedia.org/wiki/二进制 "wikilink")。诸如[MIME的标准被开发来编码二进制文件以使其通过SMTP来传输](https://zh.wikipedia.org/wiki/MIME "wikilink")。今天，大多数SMTP服务器都支持8位MIME扩展，它使二进制文件的传输变得几乎和纯文本一样简单。

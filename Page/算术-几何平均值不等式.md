...
令 \(b_i  = \frac{{a_i }} {{{\mathbf{G}}_n }}(i=1,2,3,...,n)\)，于是有 \(b_1 b_2 \cdots b_n=1\)，再作代换 \(b_1  = \frac{{c_1 }}{{c_2 }},b_2  = \frac{{c_2 }}{{c_3 }},\cdots,b_n  = \frac{{c_n }}{{c_1 }}\)，运用[排序不等式得到](https://zh.wikipedia.org/wiki/排序不等式 "wikilink")：

在[计算机科学中](../Page/计算机科学.md "wikilink")，**算法分析**（）是分析执行一个给定[算法需要消耗的](../Page/算法.md "wikilink")[计算资源数量](https://zh.wikipedia.org/wiki/计算资源 "wikilink")（例如计算时间，存储器使用等）的过程。算法的效率或复杂度在理论上表示为一个[函数](../Page/函数.md "wikilink")。其定义域是输入数据的长度（通常考虑任意大的输入，没有上界），值域通常是执行步骤数量（[时间复杂度](../Page/时间复杂度.md "wikilink")）或者存储器位置数量（空间复杂度）。算法分析是[计算复杂度理论的重要组成部分](https://zh.wikipedia.org/wiki/计算复杂度理论 "wikilink")。

在[電腦科學中](https://zh.wikipedia.org/wiki/電腦科學 "wikilink")，**算法分析**（）是分析执行一个给定[算法需要消耗的](../Page/算法.md "wikilink")[计算资源数量](https://zh.wikipedia.org/wiki/计算资源 "wikilink")（例如计算时间，存储器使用等）的过程。算法的效率或复杂度在理论上表示为一个[函数](../Page/函数.md "wikilink")。其定义域是输入数据的长度（通常考虑任意大的输入，没有上界），值域通常是执行步骤数量（[时间复杂度](../Page/时间复杂度.md "wikilink")）或者存储器位置数量（空间复杂度）。算法分析是[计算复杂度理论的重要组成部分](https://zh.wikipedia.org/wiki/计算复杂度理论 "wikilink")。

理论分析常常利用[渐近分析估计一个算法的复杂度](../Page/渐近分析.md "wikilink")，并使用[大O符号](../Page/大O符号.md "wikilink")、[大Ω符号和](https://zh.wikipedia.org/wiki/大Ω符号 "wikilink")[大Θ符号作为标记](https://zh.wikipedia.org/wiki/大Θ符号 "wikilink")。举例，[二分查找所需的执行步骤数量与查找列表的长度之对数成正比](https://zh.wikipedia.org/wiki/二分查找 "wikilink")，记为
\(O(\log n)\)，简称为「对数时间」。通常使用[渐近分析的原因是](../Page/渐近分析.md "wikilink")，同一算法的不同具体[实现的效率可能有差别](https://zh.wikipedia.org/wiki/实现 "wikilink")。但是，对于任何给定的算法，所有符合其设计者意图的实现，它们之间的性能差异应当仅仅是一个系数。
...
[时间复杂度分析和如何定义](../Page/时间复杂度.md "wikilink")「一步操作」有紧密联系。作为算法分析成立的一项基本要求，单步操作必须能够在确定的常量时间内完成。
...
后者更难以应用，所以只在必要时使用。一个例子是对接受任意精度数据的算法（比如[密码学中用到的一些算法](../Page/密码学.md "wikilink")）的分析。

人们常常忽略一点：算法的效率的理论界限，通常建立在比实际情况更加严格的假定之上。因此在实际中，算法效率是有可能突破理论的界限的。\[6\]

### 经验分析的缺陷

算法是[平台无关的](https://zh.wikipedia.org/wiki/平台无关 "wikilink")，也即一个算法可以在任意[计算机](https://zh.wikipedia.org/wiki/计算机 "wikilink")、任意[操作系统上](../Page/操作系统.md "wikilink")、用任意[编程语言实现](../Page/编程语言.md "wikilink")。因此，算法性能的相对好坏，不能仅仅通过基于运行记录的经验来判断。
...
\(c\times f(n)\)。常用[大O符号对此进行描述](../Page/大O符号.md "wikilink")。比如，[插入排序的运行时间随数据大小二次增长](../Page/插入排序.md "wikilink")，那么插入排序具有
\(O(n^2)\)
的时间复杂度。大O符号通常用于表示某个算法在最差情况下的运行时间，但也可以用来表述平均情况的运行时间。比如，[快速排序的最坏运行时间是](../Page/快速排序.md "wikilink")
...
[迭代到](../Page/迭代.md "wikilink") n。 第一次执行外层循环时，j 从 1 迭代到
...
8.  但这对[量子计算机不成立](../Page/量子计算机.md "wikilink")。
9.  可用[数学归纳法证明](../Page/数学归纳法.md "wikilink")

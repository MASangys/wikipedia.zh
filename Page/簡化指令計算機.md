...
**簡化指令計算機**（），在李蘭·貝克所寫的《系統軟體》一書內，將它當成一個虛擬的[計算機](../Page/電子計算機.md "wikilink")。大多數的真實[微處理器有很多複雜架構是以嵌入的方式來增加它的效能](https://zh.wikipedia.org/wiki/微處理器 "wikilink")，學生們對於這種必修的複雜架構會感到很難理解，但是以微處理器的觀點來說卻是很常見的設計。基本上，它有三個位元組的整數結構，五個暫存器以及可以存取

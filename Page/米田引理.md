...
由上述推論，範疇中的對象\(X\)由它所表示的函子\(h_X\)或\(k_X\)唯一確定（至多差一個同調），這是[可表函子理論的根基所在](../Page/可表函子.md "wikilink")。例如在[代數幾何中](https://zh.wikipedia.org/wiki/代數幾何 "wikilink")，一個常見的技術是將概形等同於它所代表的函子，後者往往具有直觀的幾何詮釋，技術上亦較容易處理；另一方面，我們也往往從函子的觀點研究空間的商、極限或者是模空間問題，第一步是定義適當的「函子解」，其次再研究它可表與否。[代數拓撲中的](https://zh.wikipedia.org/wiki/代數拓撲 "wikilink")[分類空間也是可表函子概念的體現](https://zh.wikipedia.org/wiki/分類空間 "wikilink")。

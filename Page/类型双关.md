**类型双关**是[计算机科学](../Page/计算机科学.md "wikilink")的术语，指任何编程技术能颠覆或者绕过一门[程序设计语言的](https://zh.wikipedia.org/wiki/程序设计语言 "wikilink")[類型系統](../Page/類型系統.md "wikilink")，以达成在形式语言内部难以甚至不可能实现的效果。

[C语言与](https://zh.wikipedia.org/wiki/C语言 "wikilink")[C++语言](https://zh.wikipedia.org/wiki/C++语言 "wikilink")，语法结构如[类型转换](../Page/类型转换.md "wikilink")与[`union`](https://zh.wikipedia.org/wiki/联合体 "wikilink")，以及C++增加的`reinterpret_cast`运算符，用于实现类型双关。
...
[Berkeley sockets使用类型双关来处理](../Page/Berkeley套接字.md "wikilink")[IP地址](../Page/IP地址.md "wikilink")。函数`bind`绑定一个位初始化的套接字到一个IP地址，其声明如下：
...
假定浮点比较的代价高昂，并假定浮点数用[IEEE 754标准](../Page/IEEE_754.md "wikilink")，就可以用类型双关获取浮点数的符号位（sign bit）做整型比较：
...
这样的实现适合于实时计算而又不能被优化实现的情形。注意把所有假定均写为注释记录下来，并写入静态断言（static assertions）验证可移植期望是否满足。[雷神之锤III竞技场](../Page/雷神之锤III竞技场.md "wikilink")游戏用此方法实现[平方根倒数速算法](../Page/平方根倒数速算法.md "wikilink")。

## 使用`union`

为了遵循C99/C++的[严格别名规则](https://zh.wikipedia.org/wiki/严格别名 "wikilink")，可以使用`union`:\[1\] ISO/IEC 9899:1999 s6.5/7</ref>
...
[GCC](../Page/GCC.md "wikilink")编译器支持这样的语言扩展。\[2\]
...
In [C\#](https://zh.wikipedia.org/wiki/C_Sharp_\(programming_language\) "wikilink") (and other .NET languages), this is a bit harder to achieve because of the type system, but can be done nonetheless, using pointers or struct unions.
...
Raw [CIL](https://zh.wikipedia.org/wiki/Common_Intermediate_Language "wikilink") can be used instead of C\#, because it doesn't have most of the type limitations. This allows one to, for example, combine two enum values of a generic type:
...
## 外部链接
  ...
  - [Section](https://gcc.gnu.org/onlinedocs/gcc-4.1.1/gcc/Optimize-Options.html#index-fstrict_002daliasing-542) of the [GCC](https://zh.wikipedia.org/wiki/GNU_Compiler_Collection "wikilink") manual on `-fstrict-aliasing`, which defeats some *type punning*
  - [Defect Report 257](http://www.open-std.org/jtc1/sc22/wg14/www/docs/dr_257.htm) to the [C99](https://zh.wikipedia.org/wiki/C99 "wikilink") standard, incidentally defining "*type punning*" in terms of `union`, and discussing the issues surrounding the implementation-defined behavior of the last example above
  - [Defect Report 283](http://www.open-std.org/jtc1/sc22/wg14/www/docs/dr_283.htm) on the use of unions for type punning

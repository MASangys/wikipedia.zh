...
[操作语义不需要程序伴随着类型](https://zh.wikipedia.org/wiki/操作语义 "wikilink")，这称作“类型擦除语义”（type-erasure
semantics）。 类型擦除语义的一种可能是通过，确保程序在运行时执行不依赖类型信息。

与之相对的是类型传递语义（type-passing
semantics）。如通过[具体化](https://zh.wikipedia.org/wiki/具体化 "wikilink")。\[1\]。类型擦除的逆操作是[类型推断](https://zh.wikipedia.org/wiki/类型推断 "wikilink")。

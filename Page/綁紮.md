在[電腦科學中](https://zh.wikipedia.org/wiki/電腦科學 "wikilink")，所謂的**綁紮**（[Binding](../Page/綁紮.md "wikilink")）是指創造一個簡單的[參考](https://zh.wikipedia.org/wiki/參考 "wikilink")（reference），並將它指向一個更複雜更大的結構以利後用。
...
這些名詞在[程式語言表示利用先前指定的變數符號來指涉數值](https://zh.wikipedia.org/wiki/程式語言 "wikilink")。在這樣的情境下，綁紮是一種將機器記憶空間的位置與變數或符號產生關連的行為。此種關連要不是在[編譯時間產生](https://zh.wikipedia.org/wiki/編譯時間 "wikilink")，此種關連稱為靜態連結（Static
linking）；或者是在執行期機動產生，則稱為動態連結（Dynamic linking）。
...
[動態連結函式庫](https://zh.wikipedia.org/wiki/動態連結函式庫 "wikilink")(例如Windows的.dll檔案，Linux上的動態函式庫)是典型的[執行期綁紮](../Page/執行期.md "wikilink")[函式庫](../Page/函式庫.md "wikilink")。它們稱做**後期綁紮**（對比於**早期綁紮**，或稱編譯時期綁紮）。由於後期綁紮只會在程式第一次需要相關函式支援時才會匯入（若此程式從未呼叫相關函式，則此綁紮行為就不會啟動），因此此類綁紮策略也稱**懶惰綁紮**。
...
許多命令綁紮都非常有名，例如[Mac
OS的儲存功能可以用Command](https://zh.wikipedia.org/wiki/Mac_OS "wikilink")-S按鍵組合來達到，因此每當使用者要儲存文件時，可直接按Command-S達到功能，而非每次都從選單選取。[Emacs文書處理軟體提供了Control](../Page/Emacs.md "wikilink")-X再按Control-C達到儲存再關閉本軟體的功能。另外，最為惡名昭彰的按鍵巨集應該是Windows當機時的Control-Alt-Delete按鍵了。
...
[en:Binding (computer
science)](https://zh.wikipedia.org/wiki/en:Binding_\(computer_science\) "wikilink")

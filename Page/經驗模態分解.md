> 本文内容由[經驗模態分解](https://zh.wikipedia.org/wiki/經驗模態分解)转换而来。


'''經驗模態分解 '''（Empirical Mode Decomposition，缩寫EMD）是由[黃鍔](../Page/黃鍔.md "wikilink")（N. E. Huang）與其他人在[美國國家宇航局於](https://zh.wikipedia.org/wiki/美國國家宇航局 "wikilink")1998年創造性地提出的一種新型自適應信號時頻處理方法，特別適用於非線性非平穩信號的分析處理。
...
對數據信號進行EMD分解就是為了獲得本征模函數，因此，在介紹EMD分析方法的具體過程之前可先介紹EMD分解過程中所涉及的基本概念的定義：**[本征模函數](https://zh.wikipedia.org/wiki/本征模函數 "wikilink")**，這是掌握EMD方法的基礎。在物理上，如果瞬時頻率有意義，那麼函數必須是對稱的，局部均值為零，並且具有相同的過零點和極值點數目。在此基礎上，有人提出了本征模函數（Intrinsic Mode Function，簡稱IMF）的概念。本征模函數任意一點的瞬時頻率都是有意義的。任何信號都是由若干本征模函數組成，在任何時候，一個信號都可以包含若干個本征模函數，如果本征模函數之間相互重疊，便形成復合信號。EMD分解的目的就是為了獲取本征模函數，再對各個本征模函數進行希爾伯特變換，得到希爾伯特譜。

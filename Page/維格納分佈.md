**維格納分布**(又名**韋格納分佈**，英文: **Wigner Distribution Function**,縮寫為**WDF**)
是由1963年的[諾貝爾物理學獎得主](https://zh.wikipedia.org/wiki/諾貝爾物理學獎 "wikilink")[尤金·维格纳](../Page/尤金·维格纳.md "wikilink")，于1932年首次引用的一個新的[方程式](https://zh.wikipedia.org/wiki/方程式 "wikilink")。

眾所皆知，[傅立葉變換對於研究穩態](https://zh.wikipedia.org/wiki/傅立葉變換 "wikilink")(時間獨立)的訊號(波形)是一項非常有用的工具，然而，訊號(波形)一般來說在時間上並非是獨立的，這樣的訊號或是波形傅立葉變換並無法有效地完全分析其特性，因此對於一個非穩態的訊號完全分析需要測量出時間以及頻率上的表現。本頁面介紹的數學函數是[時頻分析中的基礎方法](../Page/時頻分析.md "wikilink")，在1980年，Claasen，Mecklenbrauker對WDF做了更進一步的研究。除此之外，線性時頻分析中，STFT、Gabor
transform和WDF扮演了相當重要的角色，其中WDF對於分析很多非穩態的隨機訊號都有很好的表現，例如：[量子力學](https://zh.wikipedia.org/wiki/量子力學 "wikilink")、[光學](https://zh.wikipedia.org/wiki/光學 "wikilink")、[聲學](https://zh.wikipedia.org/wiki/聲學 "wikilink")、[通訊](https://zh.wikipedia.org/wiki/通訊 "wikilink")、[生物工程](https://zh.wikipedia.org/wiki/生物工程 "wikilink")、[訊號處理和](https://zh.wikipedia.org/wiki/訊號處理 "wikilink")[影像處理](https://zh.wikipedia.org/wiki/影像處理 "wikilink")。有時也被用在分析地震的資料，以及處理聲音的相位失真。
...
在[聲納和](../Page/聲納.md "wikilink")[雷達系統中](https://zh.wikipedia.org/wiki/雷達 "wikilink")，傳送出去的聲波的反射波可以用來偵測目標物的位置跟速度，在很多情形下，收到的訊號因為[都普勒位移](https://zh.wikipedia.org/wiki/都普勒位移 "wikilink")，所以跟原本的訊號並不一樣。Woodward(1953)
改寫了原本的公式
...
WDF不是一個線性的轉換，由於\(x(t)\)的signal auto-correlation function
\(x(t+\frac{\tau}{2})x^*( t-\frac{\tau}{2})\)，如果有兩種以上不同性質的訊號疊加，會產生相交項。然而相交項卻有重要且有用的物理意義，像是可以用來分析期望值，相反的，短時距傅立葉轉換就沒有此特性，詳見維格納分佈與隨機程序的關係。以下數學方程式對於WDF後會產生相交項。
...
\== WDF與隨機程序的關係 == 對於一個隨機程序x(t)，我們無法得知其確切的值，因此會將其值表示為一個機率函數，通常E\[x(t)\]
= 0 for any t
...
而在訊號處理中常見的[白雜訊](../Page/白雜訊.md "wikilink")，其譜密度\(S_x(f)=\sigma\)，其中\(\sigma\)為一個常數。白雜訊的維格納分布如下圖，可看出此雜訊在所有時間及頻率都存在著。
[200x200像素](https://zh.wikipedia.org/wiki/File:White_noise_wdf.png "fig:200x200像素")
...
2.  使用[離散傅立葉變換](https://zh.wikipedia.org/wiki/離散傅立葉變換 "wikilink")
    複雜度：\(TN{\log _2}N\)
3.  使用[Chirp-Z 轉換](https://zh.wikipedia.org/wiki/Chirp-Z_轉換 "wikilink")
    複雜度 :
    \(TN{\log _2}N\)，通常為使用[離散傅立葉變換的](https://zh.wikipedia.org/wiki/離散傅立葉變換 "wikilink")2\~3倍，但限制比使用[離散傅立葉變換少](https://zh.wikipedia.org/wiki/離散傅立葉變換 "wikilink")
...
即 \(\max ({n_1} - n,n - {n_2}) \le p \le \min ({n_2} - n,n - {n_1})\)-–
(1)
...
且可以發現 \(({n_2} - n){\Delta _t},(n - {n_1}){\Delta _t}\) 代表 \(n\Delta_t\)
離兩個邊界的距離
...
其中
\(Q = \min ({n_2} - n,n - {n_1})\,,p \in [ - Q,Q],n \in [{n_1},{n_2}]\)
...
STEP 1 :
\({x_1}\left( {n,p} \right) = x\left( {(n + p){\Delta _t}} \right){x^ * }\left( {(n - p){\Delta _t}} \right){e^{ - j2\pi \,{p^2}{\Delta _t}{\Delta _f}}}\)

STEP 2 :
\({X_2}\left[ {n,m} \right] = \sum\limits_{p = n - Q}^{n + Q} {{x_1}\left[ p \right]\,c\left[ {m - p} \right]}\)
, 其中\(c\left[ m \right] = {e^{j2\pi \,{m^2}{\Delta _t}{\Delta _f}}}\)

STEP 3 :
\(X\left( {n{\Delta _t},m{\Delta _f}} \right) = 2{\Delta _t}\,{e^{ - j2\pi \;{m^2}{\Delta _t}{\Delta _f}}}{X_2}\left[ {n,m} \right]\)

## 延伸變化

### 視窗型韋格納分佈

**視窗型韋格納分佈**（），在[韋格納分佈中](https://zh.wikipedia.org/wiki/韋格納分佈 "wikilink")，當x(t)為無限長訊號時，WDF很難去實現它。所以在積分中加入一個新的函數
，目的是擷取x(t)中的片段來計算，不需從負無限積分到正無限。
...
原始[韋格納分佈定義](https://zh.wikipedia.org/wiki/韋格納分佈 "wikilink")
\({W_x}\left( {t,f} \right) = \int_{ - \infty }^\infty {x\left( {t + \tau /2} \right) \cdot } {x^*}\left( {t - \tau /2} \right)\,{e^{ - j2\pi \,\tau \,f}} \cdot d\tau\)
...
2.  可能不符合[譜密度](https://zh.wikipedia.org/wiki/譜密度 "wikilink")(Power
    spectral density)的定義。
3.  一些好用的數學運算性質會消失。
...
再令\(t = n\Delta_t, f = m\Delta_f,\tau'= p\Delta_t\)
...
假設w(t) = 0 for |t| \> B 即
\(w\left( {2p{\Delta _t}} \right) = 0{\rm{ }} \ for{\rm{ }} \ p < - Q{\rm{ }}\ \land \ p > Q\)
其中\(Q = \frac{B}{{2{\Delta _t}}}\) 如此一來，p範圍便可縮小。
...
假設\(x(t){\rm{ = }}\delta (t - {t_1}) + \delta (t - {t_2})\)的情況下，比較有無mask
function所產生的不同結果
[X(t).jpg](https://zh.wikipedia.org/wiki/File:X\(t\).jpg "fig:X(t).jpg")
...
\end{array}\)
[Wx(t,f).jpg](https://zh.wikipedia.org/wiki/File:Wx\(t,f\).jpg "fig:Wx(t,f).jpg")
...
由於\(w(\tau )\)只在-B到B有值，故乘上\(w(\tau )\)就能去除相交項(Cross
term)，只保留下圖中兩條紅線中間的區域，也就是Auto terms。
[With_mask.jpg](https://zh.wikipedia.org/wiki/File:With_mask.jpg "fig:With_mask.jpg")
但上述其實是理想的情況，x(t)為窄頻信號Delta function

如果X(τ)寬度太寬或是有ripple的話，Cross term仍會有殘留，示意圖如下
[With_mask_nonideal.jpg](https://zh.wikipedia.org/wiki/File:With_mask_nonideal.jpg "fig:With_mask_nonideal.jpg")
藍色線為X(τ)的訊號，若X(τ)的寬度太寬或是有ripple產生，就可能會跑進\(w(\tau )\)的範圍裡面，進而導致無法完全濾除Cross
term。

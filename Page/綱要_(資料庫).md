...
[Oracle数据库](../Page/Oracle数据库.md "wikilink")中，**schema object**是一类逻辑。\[4\]关联一个schema到每个数据库**user**。\[5\] schema是数据或模式对象的逻辑结构的集合，由数据库用户拥有，并且与该用户具有相同的名称，也就是说每个用户拥有一个独立的schema。
...
在Oracle中，CREATE SCHEMA语句实际上并不创建一个模式，因为已经在CREATE USER时就已经创建了一个schema。CREATE SCHEMA语句用于将schema同表和视图关联起来，并在这些对象上授权。

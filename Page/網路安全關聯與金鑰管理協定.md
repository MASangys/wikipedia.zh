**網際網路安全關聯鑰匙管理協定**（，縮寫為 ISAKM或 ISAKMP），網際網路協定之一，用於在網際網路上建立[安全關聯與加密金鑰](https://zh.wikipedia.org/wiki/安全關聯 "wikilink")。這個協定在 RFC 2408 中定義，它提供了一個架構來進行授權與金鑰交換，主要被設計來作為金鑰交換之用。[網際網路金鑰交換](../Page/網際網路金鑰交換.md "wikilink")與Kerberized Internet Negotiation of Key等協定，提供了授權金鑰的資料，可以在ISAKMP中使用。ISAKMP只提供了一个身份鉴权和钥匙交换的框架，其被设计为不受约束的钥匙交换。如IKE（因特网钥匙交换协议）和KINK（Kerberized 因特网钥匙协议）等协议都使用了ISAKMP所提供的授权钥匙的方案。例如：因特网钥匙交换协议(IKE)使用了Oakley和SKEME协议的一部分,通过ISAKMP连接获取授权了的钥匙信息。其同样对于其他的安全关联，如AH和ESP。

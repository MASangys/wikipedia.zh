...
regression）**是利用称为线性回归方程的[最小平方函數对一个或多个](../Page/最小二乘法.md "wikilink")[自变量和](https://zh.wikipedia.org/wiki/自变量 "wikilink")[因变量之间关系进行建模的一种](https://zh.wikipedia.org/wiki/因变量 "wikilink")[回归分析](https://zh.wikipedia.org/wiki/回归分析 "wikilink")。这种函数是一个或多个称为回归系数的模型参数的线性组合。只有一个自变量的情况称为简单回归，大于一个自变量情况的叫做多元回归。
...
這表示估計項是因變量的線性組合。進一步地說，如果所觀察的誤差服從[正態分佈](https://zh.wikipedia.org/wiki/正態分佈 "wikilink")。參數的估計值將服從聯合正態分佈。在當前的假設之下，估計的參數向量是精確分佈的。

\[\hat\beta \sim N ( \beta, \sigma^2 (X^TX)^{-1} )\]
其中\(N(\cdot)\)表示[多變量正態分佈](https://zh.wikipedia.org/wiki/多變量正態分佈 "wikilink")。
...
有关[吸烟对](https://zh.wikipedia.org/wiki/吸烟 "wikilink")[死亡率和](../Page/死亡率.md "wikilink")[发病率影响的早期证据来自采用了回归分析的](https://zh.wikipedia.org/wiki/发病率 "wikilink")[观察性研究](../Page/观察性研究.md "wikilink")。为了在分析观测数据时减少[伪相关](../Page/偽關係.md "wikilink")，除最感兴趣的变量之外,通常研究人员还会在他们的回归模型里包括一些额外变量。例如，假设我们有一个回归模型，在这个回归模型中吸烟行为是我们最感兴趣的独立变量，其相关变量是经数年观察得到的吸烟者寿命。研究人员可能将社会经济地位当成一个额外的独立变量，已确保任何经观察所得的吸烟对寿命的影响不是由于教育或收入差异引起的。然而，我们不可能把所有可能混淆结果的变量都加入到实证分析中。例如，某种不存在的基因可能会增加人死亡的几率，还会让人的吸烟量增加。因此，比起采用观察数据的回归分析得出的结论，[随机对照试验常能产生更令人信服的因果关系证据](../Page/随机对照试验.md "wikilink")。当[可控实验不可行时](https://zh.wikipedia.org/wiki/可控实验 "wikilink")，回归分析的衍生，如[工具变量回归](../Page/工具变量.md "wikilink")，可尝试用来估计观测数据的因果关系。
...
## 参见
  ...
  - [Lack-of-fit sum of
    squares](https://zh.wikipedia.org/wiki/Lack-of-fit_sum_of_squares "wikilink")
...
## 外部連結
  ...
  - [Least-Squares
    ...
    [PhET](https://zh.wikipedia.org/wiki/PhET "wikilink") Interactive
    simulations, University of Colorado at Boulder

在[统计学中](../Page/统计学.md "wikilink")，**线性回归（Linear
regression）**是利用称为线性回归方程的[最小平方函數对一个或多个](../Page/最小二乘法.md "wikilink")[自变量和](../Page/自变量.md "wikilink")[因变量之间关系进行建模的一种](../Page/因变量.md "wikilink")[回归分析](../Page/回归分析.md "wikilink")。这种函数是一个或多个称为回归系数的模型参数的线性组合。只有一个自变量的情况称为简单回归，大于一个自变量情况的叫做多元回归。
...
其他的模型可能被認定成非線性模型。一個線性回歸模型不需要是自變量的線性函數。線性在這裡表示\(Y_i\)的條件均值在參數\(\beta\)裡是線性的。例如：模型\(Y_i = \beta_1 X_i + \beta_2 X_i^2 + \varepsilon_i\)在\(\beta_1\)和\(\beta_2\)裡是線性的，但在\(X_i^2\)裡是非線性的，它是\(X_i\)的非線性函數。

### 數據和估計

區分随機變量和這些變量的觀測值是很重要的。通常來說，觀測值或數據（以小寫字母表記）包括了*n*個值
\((y_i, x_{i1}, \ldots, x_{ip}), \, i = 1, \ldots, n\).

我們有\(p + 1\)個參數\(\beta_0, \ldots, \beta_p\)需要決定，為了估計這些參數，使用[矩陣表記是很有用的](../Page/矩陣.md "wikilink")。
...
如果*X*列之間存在[線性相關](../Page/線性相關.md "wikilink")，那麽參數向量\(\beta\)就不能以最小二乘法估計除非\(\beta\)被限制，比如要求它的一些元素之和為0。

### 古典假設

  - **樣本**是在**母體**之中**随機抽取**出來的。
  - 因變量Y在[實直線上是](../Page/實直線.md "wikilink")**連續的**，
...
其中\(\chi_{n-p}^2\)服從[卡方分佈](../Page/卡方分佈.md "wikilink")，自由度是\(n-p\)

對普通方程的解可以冩為：

\[\hat{\boldsymbol\beta}=(\mathbf{X^TX)^{-1}X^Ty}.\]
這表示估計項是因變量的線性組合。進一步地說，如果所觀察的誤差服從[正態分佈](../Page/正態分佈.md "wikilink")。參數的估計值將服從聯合正態分佈。在當前的假設之下，估計的參數向量是精確分佈的。

\[\hat\beta \sim N ( \beta, \sigma^2 (X^TX)^{-1} )\]
其中\(N(\cdot)\)表示[多變量正態分佈](../Page/多變量正態分佈.md "wikilink")。

參數估計值的[標準差是](../Page/標準差.md "wikilink")：

\[\hat\sigma_j=\sqrt{ \frac{S}{n-p}\left[\mathbf{(X^TX)}^{-1}\right]_{jj}}.\]

參數\(\beta_j\)的\(100(1-\alpha)\%\)[置信區間可以用以下式子來計算](../Page/置信區間.md "wikilink")：
...
此二元一次線性方程組可用[克萊姆法則求解](../Page/克萊姆法則.md "wikilink")，得解\(\hat\alpha,\  \hat\beta\)：
...
**回歸平方和**SSReg (sum of squares for regression。也可寫做**模型平方和**，SSM，sum of
squares for model) 是：
...
[廣義最小二乘法可以用在當觀測誤差具有](../Page/廣義最小二乘法.md "wikilink")[異方差或者](../Page/異方差.md "wikilink")[自相關的情況下](../Page/自相關.md "wikilink")。

### 總體最小二乘法

[總體最小二乘法用於當自變量有誤時](../Page/總體最小二乘法.md "wikilink")。

### 廣義線性模式

[廣義線性模式應用在當誤差分佈函數不是正態分佈時](../Page/廣義線性模式.md "wikilink")。比如[指數分佈](../Page/指數分佈.md "wikilink")，[伽瑪分佈](../Page/伽瑪分佈.md "wikilink")，[逆高斯分佈](../Page/逆高斯分佈.md "wikilink")，[泊松分佈](../Page/泊松分佈.md "wikilink")，[二項式分佈等](../Page/二項式分佈.md "wikilink")。

### 穩健回歸

將平均[絕對誤差最小化](../Page/絕對誤差.md "wikilink")，不同於在線性回歸中是將均方誤差最小化。

## 線性回歸的應用

### 趨勢線

一條趨勢線代表著[時間序列數據的長期走勢](../Page/時間序列.md "wikilink")。它告訴我們一組特定數據（如GDP、石油價格和股票價格）是否在一段時期内增長或下降。雖然我們可以用肉眼觀察數據點在坐標系的位置大體畫出趨勢線，更恰當的方法是利用線性回歸計算出趨勢線的位置和斜率。

### 流行病学

有关[吸烟对](../Page/吸烟.md "wikilink")[死亡率和](../Page/死亡率.md "wikilink")[发病率影响的早期证据来自采用了回归分析的](../Page/发病率.md "wikilink")[观察性研究](../Page/观察性研究.md "wikilink")。为了在分析观测数据时减少[伪相关](../Page/偽關係.md "wikilink")，除最感兴趣的变量之外,通常研究人员还会在他们的回归模型里包括一些额外变量。例如，假设我们有一个回归模型，在这个回归模型中吸烟行为是我们最感兴趣的独立变量，其相关变量是经数年观察得到的吸烟者寿命。研究人员可能将社会经济地位当成一个额外的独立变量，已确保任何经观察所得的吸烟对寿命的影响不是由于教育或收入差异引起的。然而，我们不可能把所有可能混淆结果的变量都加入到实证分析中。例如，某种不存在的基因可能会增加人死亡的几率，还会让人的吸烟量增加。因此，比起采用观察数据的回归分析得出的结论，[随机对照试验常能产生更令人信服的因果关系证据](../Page/随机对照试验.md "wikilink")。当[可控实验不可行时](../Page/可控实验.md "wikilink")，回归分析的衍生，如[工具变量回归](../Page/工具变量.md "wikilink")，可尝试用来估计观测数据的因果关系。

### 金融

[資本資產定價模型利用線性回歸以及](../Page/資本資產定價模型.md "wikilink")[Beta係數的概念分析和計算投資的系統風險](../Page/Beta係數.md "wikilink")。這是從聯繫投資回報和所有風險性資產回報的模型Beta係數直接得出的。
...
<!-- end list -->
  ...
  -
  -
  - [Charles Darwin](../Page/查尔斯·达尔文.md "wikilink"). *The Variation of
    Animals and Plants under Domestication*. (1868) *(Chapter XIII
    describes what was known about reversion in Galton's time. Darwin
    uses the term "reversion".)*
...
## 参见
  ...
  - [横截面回归](../Page/横截面回归.md "wikilink")
  - [曲线拟合](../Page/曲线拟合.md "wikilink")
  - [经验贝叶斯方法](../Page/经验贝叶斯方法.md "wikilink")
  - [逻辑斯蒂回归](../Page/逻辑斯蒂回归.md "wikilink")
  - [M估计](../Page/M估计.md "wikilink")
  - [非线性回归](../Page/非线性回归.md "wikilink")
  - [非参数回归](../Page/非参数回归.md "wikilink")
  - [多元自适应回归样条](../Page/多元自适应回归样条.md "wikilink")
  - [Lack-of-fit sum of
    squares](../Page/Lack-of-fit_sum_of_squares.md "wikilink")
  - [截断回归模型](../Page/截断回归模型.md "wikilink")
  - [删失回归模型](../Page/删失回归模型.md "wikilink")
  - [简单线性回归](../Page/简单线性回归.md "wikilink")
  - [分段线性回归](../Page/分段回归.md "wikilink")

## 外部連結

  - [Least-Squares
    Regression](https://phet.colorado.edu/en/simulation/least-squares-regression),
    [PhET](../Page/PhET.md "wikilink") Interactive simulations,
    University of Colorado at Boulder

{{-}}

[Category:迴归分析](https://zh.wikipedia.org/wiki/Category:迴归分析 "wikilink")

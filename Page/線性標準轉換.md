...
transform](../Page/:en:linear_canonical_transformation.md "wikilink"),
LCT)也稱作"ABCD轉換。在漢米爾頓力學中，線性標準轉換是積分變換的一個代表家族，並且能夠將許多經典的轉換進行廣義化，例如[傅立葉變換](../Page/傅立葉變換.md "wikilink")、[分數傅立葉變換](../Page/分數傅立葉變換.md "wikilink")、[拉普拉斯變換](../Page/拉普拉斯變換.md "wikilink")、[菲涅爾轉換](../Page/菲涅爾轉換.md "wikilink")([Fresnel
transform](../Page/:en:Fresnel_transform.md "wikilink")，電磁波在空氣中傳播)、[高斯-魏爾斯特拉斯轉換](../Page/高斯-魏爾斯特拉斯轉換.md "wikilink")、[包格曼轉換等等等](../Page/包格曼轉換等.md "wikilink")。此轉換提供了這些最常使用的線性轉換一個統一框架，並且在光學、信號轉換以及系統響應領域中都提供一般化的概念。尤其從系統工程的角度看來，線性標準轉換提供一個強大的光學系統設計和分析的工具。
...
\[\begin{bmatrix} a & c \\ b & d \end{bmatrix} = \begin{bmatrix} \cos\phi & \sin\phi \\ -\sin\phi & \cos\phi \end{bmatrix}\]
  ...
  - [傅立葉變換](../Page/傅立葉變換.md "wikilink")，傅立葉變換的定義:
...
\[\begin{bmatrix} a & b \\ c & d \end{bmatrix} = \begin{bmatrix} 0 & -1 \\ 1 & 0 \end{bmatrix}\]
  ...
  - 菲涅爾轉換([Fresnel
    transform](../Page/:en:Fresnel_transform.md "wikilink"))，用來描述電磁波在空氣中傳播的情形，代表將時頻域上的面積做時間方向上的切變，若將其寫成矩陣的形式為：

\[\begin{bmatrix} a & b \\ c & d \end{bmatrix} = \begin{bmatrix} 1 & \lambda z/2\pi \\ 0 & 1 \end{bmatrix}\]

  - 拉普拉斯轉換([Laplace
    transform](../Page/:en:Laplace_transform.md "wikilink"))，代表將時頻域上的面積對其複數平面做+90度的旋轉，其矩陣如下:
...
\[W_{X_{(a,b,c,d)}}(au+bv,cu+dv)=W_{x}(u,v)\]
  ...
  -
    此兩式互為相同含意，因此可知反向LCT的就是由四個參數代表矩陣的[反矩陣](../Page/反矩陣.md "wikilink")。
...
<!-- end list -->
  ...
  -
    ...
    transform](../Page/:en:Fresnel_transform.md "wikilink"))來描述，假設一光學系統滿足菲涅爾近似而電磁波由\(x_i, y_i\)平面傳播到\(x, y\)平面，則式子如下:

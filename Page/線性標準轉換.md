在數學的文獻中，線性標準轉換([linear canonical
transform](https://zh.wikipedia.org/wiki/:en:linear_canonical_transformation "wikilink"),
LCT)也稱作"ABCD轉換。在漢米爾頓力學中，線性標準轉換是積分變換的一個代表家族，並且能夠將許多經典的轉換進行廣義化，例如[傅立葉變換](https://zh.wikipedia.org/wiki/傅立葉變換 "wikilink")、[分數傅立葉變換](../Page/分數傅立葉變換.md "wikilink")、[拉普拉斯變換](https://zh.wikipedia.org/wiki/拉普拉斯變換 "wikilink")、[菲涅爾轉換](https://zh.wikipedia.org/wiki/菲涅爾轉換 "wikilink")([Fresnel
transform](https://zh.wikipedia.org/wiki/:en:Fresnel_transform "wikilink")，電磁波在空氣中傳播)、[高斯-魏爾斯特拉斯轉換](https://zh.wikipedia.org/wiki/高斯-魏爾斯特拉斯轉換 "wikilink")、[包格曼轉換等等等](https://zh.wikipedia.org/wiki/包格曼轉換等 "wikilink")。此轉換提供了這些最常使用的線性轉換一個統一框架，並且在光學、信號轉換以及系統響應領域中都提供一般化的概念。尤其從系統工程的角度看來，線性標準轉換提供一個強大的光學系統設計和分析的工具。
...
積分變換是將輸入的訊號f(t)經由核心(Kernel)K(t,
u)的作用後對應後的輸出結果即是另一個函數\((TF)(u)\)，此時\((TF)(u)\)稱作是\(f(t)\)的積分變換。
...
|- |
\(X_{(a,b,c,d)}(u) = \sqrt{-i} \cdot e^{i  \pi \frac{d}{b} u^{2}} \int_{-\infty}^\infty e^{-i 2 \pi \frac{1}{b} ut}e^{i  \pi \frac{a}{b} t^2} x(t) \; dt \, ,\)
 || 此時 *b* ≠ 0, |- |
\(X_{(a,0,c,d)}(u) = \sqrt{d} \cdot e^{i  \pi cdu^{2}} x(du) \, ,\) ||
此時 *b* = 0. |} 並且需要滿足此條件:

\[ad-bc=1\]

此時的\(X_{(a,b,c,d)}(u)\)表示的是訊號\(x(t)\)經過線性標準變換過後的結果
而我們通常使用\(O_{F}^{(a,b,c,d)}\)作為線性標準轉換的操作函數，也就是:

\[O_{F}^{(a,b,c,d)}(x(t) = X_{(a,b,c,d)}(u))\]

另外，線性標準轉換(LCT)也可用簡單的2x2的矩陣以及一個[行列式限制條件來表示](../Page/行列式.md "wikilink")，如同矩陣\(\begin{bmatrix}
a & b \\
c & d
...
\[F(u) = FT{(f(t))} = \int_{-\infty}^{\infty} f(t)\ e^{- j u t}\,dx\],  
對任意實數 u
...
\[f(t) = IFT{(F(u))} = \int_{-\infty}^{\infty} F(u)\ e^{- j u t}\,dx\],
  對任意實數 u
...
\[\begin{bmatrix} a & b \\ c & d \end{bmatrix} = \begin{bmatrix} 0 & -1 \\ 1 & 0 \end{bmatrix}\]
  ...
  - 菲涅爾轉換([Fresnel
    transform](https://zh.wikipedia.org/wiki/:en:Fresnel_transform "wikilink"))，用來描述電磁波在空氣中傳播的情形，代表將時頻域上的面積做時間方向上的切變，若將其寫成矩陣的形式為：

\[\begin{bmatrix} a & b \\ c & d \end{bmatrix} = \begin{bmatrix} 1 & \lambda z/2\pi \\ 0 & 1 \end{bmatrix}\]

  - 拉普拉斯轉換([Laplace
    transform](https://zh.wikipedia.org/wiki/:en:Laplace_transform "wikilink"))，代表將時頻域上的面積對其複數平面做+90度的旋轉，其矩陣如下:
...
<!-- end list -->
  ...
  -
    當我們將兩個參數矩陣不同的LCT組合起來，也就是對同一個訊號做兩次線性標準轉換。這樣的結果相當於將兩矩陣的乘積後的矩陣作為參數的線性標準轉換，並以此新的LCT對原始訊號作用後得到的結果。最常見是[維格納分佈](../Page/維格納分佈.md "wikilink")(wigner
    distribution function, WDF)中的加法特性。
...
<!-- end list -->
  ...
  -
    電磁波在空氣中傳播的行為是透過菲涅爾轉換([Fresnel
    transform](https://zh.wikipedia.org/wiki/:en:Fresnel_transform "wikilink"))來描述，假設一光學系統滿足菲涅爾近似而電磁波由\(x_i, y_i\)平面傳播到\(x, y\)平面，則式子如下:
...
A. Stern, “Why is the linear canonical transform so little known?” in
Proceedings of 5th International Workshop on Information Optics, G.
Cristóbal, B. Javidi, and S. Vallmitjana, eds (Springer, 2006), pp.
225–234.

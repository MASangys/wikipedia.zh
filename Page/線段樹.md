**線段樹**（）是一種[二元樹形資料結構](https://zh.wikipedia.org/wiki/二元樹 "wikilink")，1977年由Jon
Louis
Bentley發明\[1\]，用以儲存[區間或](../Page/區間.md "wikilink")[線段](https://zh.wikipedia.org/wiki/線段 "wikilink")，並且允許快速查詢結構內包含某一點的所有區間。
...
本處以一維的線段樹為例。
[Segment_tree.svg](https://zh.wikipedia.org/wiki/File:Segment_tree.svg "fig:Segment_tree.svg")
令S是一維線段的集合。將這些線段的端點坐標由小到大排序，令其為\(x_1, x_2, \cdots , x_m\)。我們將被這些端點切分的每一個區間稱為「單位區間」（每個端點所在的位置會單獨成為一個單位區間），從左到右包含：

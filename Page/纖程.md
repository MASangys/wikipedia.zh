在[計算機科學中](https://zh.wikipedia.org/wiki/計算機科學 "wikilink")，**纖程**（）是一種最輕量化的[線程](https://zh.wikipedia.org/wiki/線程 "wikilink")（lightweight threads）。它是一種用户态線程（user thread），讓[應用程式可以獨立決定自己的線程要如何運作](https://zh.wikipedia.org/wiki/應用程式 "wikilink")。作業系統[內核不能看見它](https://zh.wikipedia.org/wiki/內核 "wikilink")，也不會為它進行[排程](../Page/排程.md "wikilink")。

就像一般的綫程，纖程有自己的[定址空間](https://zh.wikipedia.org/wiki/定址空間 "wikilink")。但是纖程採取合作式多工（Cooperative multitasking），而線程採取先佔式多工（Pre-emptive multitasking）。應用程式可以在一個線程環境中建立多個纖程，然後手動執行它。纖程不會被自動執行，必須要由[應用程式自己指定讓它執行](https://zh.wikipedia.org/wiki/應用程式 "wikilink")，或換到下一個纖程。
...
[Category:操作系统技术](https://zh.wikipedia.org/wiki/Category:操作系统技术 "wikilink") [Category:執行緒](https://zh.wikipedia.org/wiki/Category:執行緒 "wikilink")
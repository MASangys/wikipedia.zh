...
**纹素**（****，即或的[合成字](https://zh.wikipedia.org/wiki/合成字 "wikilink")）是**纹理元素**的简称，它是[计算机图形纹理空间中的基本单元](../Page/计算机图形学.md "wikilink")\[1\]。如同图像是由[像素排列而成](../Page/像素.md "wikilink")，纹理是由纹素排列表示的。
...
纹理工序起始于空间中的某一位置。这个位置可以是在世界坐标系中，但是一般情况下会设定在物体坐标系中。这样纹理会随着物体运动。然后通过投射的方式将其位置（坐标）从三维矢量值转化为0到1范围的二维矢量值（即[uv](https://zh.wikipedia.org/wiki/:en:UV_mapping "wikilink")）。再将这个二维矢量值与纹理的分辨率相乘从而获得纹素的位置。当所需纹素的位置不是[整数的时候](../Page/整数.md "wikilink")，需要使用[纹理滤镜进行处理](https://zh.wikipedia.org/wiki/:en:Texture_filtering "wikilink")。

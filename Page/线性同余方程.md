在[数论中](../Page/数论.md "wikilink")，**线性同余方程**是最基本的同余方程，“线性”表示方程的[未知数次数是一次](https://zh.wikipedia.org/wiki/未知数 "wikilink")，即形如：

\[ax \equiv b \ \pmod{n} \ \ \ \ (1)\]

的方程。此方程有解[当且仅当](../Page/当且仅当.md "wikilink") *b* 能够被 *a* 与 *n*
的[最大公约数](https://zh.wikipedia.org/wiki/最大公约数 "wikilink")[整除](https://zh.wikipedia.org/wiki/整除 "wikilink")（记作
gcd(*a*,*n*) | *b*）。这时，如果 *x<sub>0</sub>* 是方程的一个解，那么所有的解可以表示为：

\[\{x_0+k\frac{n}{d}\mid k\in\mathbb{Z}\}.\]

其中 *d* 是*a* 与 *n*
的[最大公约数](https://zh.wikipedia.org/wiki/最大公约数 "wikilink")。在模
n 的[完全剩余系](https://zh.wikipedia.org/wiki/完全剩余系 "wikilink") {0,1,…,n-1}
中，恰有 *d* 个解。
...
若 *d* = gcd(*a*, *n*) 整除 *b*
，那么\({b \over d}\)为整数。由[裴蜀定理](https://zh.wikipedia.org/wiki/裴蜀定理 "wikilink")，存在整数对
(r,s) （可用[辗转相除法求得](https://zh.wikipedia.org/wiki/辗转相除法 "wikilink")）使得
*ar*+*sn*=*d*，因此 \(x={rb \over d}\)是方程 (1) 的一个解。其他的解都关于\({n \over d}\)与
*x* 同余。
...
中 d = gcd(12,28) = 4 。注意到 \(4 = 12 \times (-2)+28 \times 1\)，因此
\(x \equiv 5 \times (-2) \equiv -10 \equiv 4 \ \pmod{7}\)是一个解。对模 28
来说，所有的解就是 {4,11,18,25} 。
...
解出：*l* ≡ 0 (**mod** 4)，即 *l* = 4*m*。代入原来的表达式就有 *x* = 21(4*m*) + 10 =
84*m* + 10，即解为：

...
其中方程左侧的[微分算子](../Page/微分算子.md "wikilink")\(\mathcal{L}\)是[线性算子](https://zh.wikipedia.org/wiki/线性算子 "wikilink")，是要解的未知函数，方程的右侧是一个已知函数。如果()
}}
0，那么方程(\*)的解的线性组合仍然是解，所有的解构成一个[向量空间](../Page/向量空间.md "wikilink")，称为**解空间**。这样的方程称为**齐次线性微分方程**。当不是零函数时，所有的解构成一个[仿射空间](../Page/仿射空间.md "wikilink")，由对应的齐次方程的解空间加上一个特解得到。这样的方程称为**非齐次线性微分方程**。线性微分方程可以是[常微分方程](../Page/常微分方程.md "wikilink")，也可以是[偏微分方程](../Page/偏微分方程.md "wikilink")。
...
换成*z*<sup>*k*</sup>，便可得到特征方程。这个方程有*n*个解：*z*<sub>1</sub>, ..., *z*<sub>*n*</sub>。把任何一个解代入*e*<sup> *zx*</sup>，便可以得到微分方程的一个解：*e*<sup> *z*<sub>*i*</sub>*x*</sup>。由于齐次线性微分方程满足[叠加原理](../Page/叠加原理.md "wikilink")，因此这些函数的任意[线性组合仍然满足微分方程](https://zh.wikipedia.org/wiki/线性组合 "wikilink")。

如果特征方程的根都不重复，我们便得到了微分方程的*n*个解。可以证明，这些解是[线性独立的](https://zh.wikipedia.org/wiki/线性独立 "wikilink")。于是，微分方程的通解就是*y*
= *C*<sub>1</sub>*e*<sup> *z*<sub>*1*</sub>*x*</sup> +
*C*<sub>2</sub>*e*<sup> *z*<sub>*2*</sub>*x*</sup> + …… +
*C*<sub>n</sub>*e*<sup> *z*<sub>*n*</sub>*x*</sup>，其中*C*<sub>1</sub>、*C*<sub>2</sub>、……、*C*<sub>n</sub>是常数。

以上讨论了*n*个根全不相同的情形。如果这*n*个根中有两个（或多个）相同，用上面的方法就无法得出*n*个线性独立的解。但是，可以验证，如果*z*是特征方程的
*m*<sub>*z*</sub> 重根，那么，对于
\(k\in\{0,1,\dots,m_{z}-1\} \,\)，\(y=x^ke^{zx} \,\) 就是微分方程的一个解。对每个特征根
*z*，都能得到 *m*<sub>*z*</sub> 个解，所有这些解的线性组合就是方程的通解。

一般地，如果微分方程的系数*A<sub>i</sub>*都是实数，那么它的解也应该表示成实数的形式。假如特征方程有复数根，那么它一定是成对的，也就是说，如果*a*
+ *bi*是特征方程的根，那么*a* - *bi*也是一个根。于是，*y* = *e*<sup> (*a* +
*bi*)*x*</sup>和*y* = *e*<sup> (*a* -
*bi*)*x*</sup>都是微分方程的解。但这两个解都是复数的形式。考虑到这两个解的任意线性组合也仍然是微分方程的解，我们可以把这两个解相加，再[除以2](../Page/除以二.md "wikilink")，利用[欧拉公式](../Page/欧拉公式.md "wikilink")，便得到一个实数形式的解：*y*
= *e*<sup> *ax*</sup>cos*bx*。如果把两个解相减，再除以2i，便得到另一个实数形式的解：*y* =
*e*<sup> *ax*</sup>sin*bx*。于是，*y* =
*C*<sub>1</sub>*e*<sup> *ax*</sup>cos*bx* +
*C*<sub>2</sub>*e*<sup> *ax*</sup>sin*bx*就是微分方程的通解。
...
这个方程可以用[积分因子求解](../Page/积分因子.md "wikilink")，方法是把两边乘以\(e^{\int f(x)\,dx}\)：
...
## 参考文献
  ...
  - Stanley J. Farlow(1994). *An introduction to differential equations
    and their applications*. McGraw-Hill, Inc. ISBN 0-07-020030-0.
    p.131-139, p.158-162.

...
[Intersecting_Lines.svg](https://zh.wikipedia.org/wiki/File:Intersecting_Lines.svg "fig:Intersecting_Lines.svg") 如果有一组数**x**<sub>1</sub>、**x**<sub>2</sub>、……**x**<sub>n</sub>使得方程组两边的等号都成立，那么这组数就叫做方程组的解。一个线性方程组的所有的解的[集合会被简称为](https://zh.wikipedia.org/wiki/集合 "wikilink")**解集**。根据解的存在情况，线性方程组可以分为三类：
...
当未知数只有两个（*x*和*y*）的时候，方程组里面的每一个方程可以看成**Oxy**[平面](../Page/平面.md "wikilink")（正交直角坐标系）上的一条[直线](../Page/直线.md "wikilink")的方程。直线上的[点](../Page/点.md "wikilink")的坐标就是满足这个方程的一组数。从这个角度看来，方程组的解就是所有这种直线的[公共点](https://zh.wikipedia.org/wiki/交点 "wikilink")。而若干条直线的公共部分要么是一条直线，要么是一个点，要么是[空集](../Page/空集.md "wikilink")，因此对应的，线性方程组的解要么有无穷个，要么恰好有一个，要么不存在。

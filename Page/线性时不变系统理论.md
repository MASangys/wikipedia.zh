**线性非时变系统理论**俗称**系统理论**，源自[应用数学](../Page/应用数学.md "wikilink")，直接在[核磁共振頻譜學](../Page/核磁共振頻譜學.md "wikilink")、[地震学](../Page/地震学.md "wikilink")、[电路](../Page/电路.md "wikilink")、[信号处理和](../Page/信号处理.md "wikilink")[控制理论等技术领域运用](../Page/控制理论.md "wikilink")。它研究的是**[线性](../Page/线性系统.md "wikilink")、[非时变系统](../Page/非时变系统.md "wikilink")**对任意输入信号的响应。虽然这些系统的轨迹通常会随时间变化（例如声学波形）来测量和跟踪，但是应用到[图像处理和](../Page/图像处理.md "wikilink")[场论时](../Page/场论.md "wikilink")，系统在空间维度上也有轨迹。因此，这些系统也被称为*线性非時變平移*，在最一般的范围理论给出此理论。在离散（即采样）系统中对应的术语是*线性非時變平移系统*。由电阻、电容、电感组成的电路是LTI系统的一个很好的例子。\[1\]
...
LTI系统的理论的基本结论是任何LTI系统都可以完全用一个单一方程来表示，称为系统的[冲激响应](../Page/冲激响应.md "wikilink")。系统的输出可以简单表示为输入信号与系统的冲激响应的[卷积](../Page/卷积.md "wikilink")。这种分析方法通常称为*[時域](../Page/時域.md "wikilink")*观点。相同的结果对于离散时间线性移位不变系统也成立，其中信号为离散时间取样信号，并且卷积对序列定义。

[LTI.png](https://zh.wikipedia.org/wiki/File:LTI.png "fig:LTI.png")

同理，任何LTI系统的特征可由*频域*的系统[传递函数刻画](../Page/传递函数.md "wikilink")，它是系统冲激响应的[拉普拉斯变换](../Page/拉普拉斯变换.md "wikilink")（在离散时间系统的情况下为Z变换）。由于这些变换的性质，该系统在频域的输出是传递函数与输入的变换的乘积。换句话说，时域中的卷积相当于频域中的乘法。

对于所有的LTI系统中，[本征函数和所用变换的基函数](../Page/本征函数.md "wikilink")，是[复](../Page/复数_\(数学\).md "wikilink")[指数函数](../Page/指数函数.md "wikilink")。这即是说，如果一个系统的输入是复波形\(A e^{st}\)，复振幅为\(A\)，复频率为\(s\)，输出将是输入的复常数倍，表示为新复振幅\(B\)的式子\(B e^{st}\)。比值\(B/A\)是频率\(s\)的传递函数。

因为是[正弦的复指数与复共轭频率的总和](../Page/正弦曲線.md "wikilink")，如果输入到该系统是一个正弦波，则系统的输出也将是一个正弦波，或许具有不同[振幅和不同](../Page/振幅.md "wikilink")[相位的](../Page/相位.md "wikilink")，但总是与相同的频率在达到稳定状态。LTI系统不能产生频率成分中没有的输入。

LTI系统理论善于描述了许多重要的系统。至少相对于时间变化的和/或非线性的情况下最LTI系统被认为是“容易”来分析。任何可以被模拟为常系数线性齐次[微分方程系统是LTI系统](../Page/微分方程.md "wikilink")。这类系统的实例是[电路由](../Page/电路.md "wikilink")[电阻器S](../Page/電阻器.md "wikilink")，[电感s和](../Page/电感元件.md "wikilink")[电容器S](../Page/电容器.md "wikilink")（RLC电路）的。理想的弹簧
...
其中*f*是本征函数而\(\lambda\)是[特征值](../Page/特征向量.md "wikilink")（一个常数）。
...
\(A, s \in \mathbb{C}\)，系统输出为输入\(A e^{st}\)和常量\(H(s)\)的乘积。因此，\(A e^{s t}\)是LTI系统的[本徵函數](../Page/本徵函數.md "wikilink")，对应的[特征向量为](../Page/特征向量.md "wikilink")\(H(s)\)。
...
本征函数的指数函数性质对分析和了解LTI系统都是很有用处的。[拉普拉斯变换](../Page/拉普拉斯变换.md "wikilink")
...
拉普拉斯变换通常用于单边信号的背景下，即*t*小于某个值时信号的所有值为零。通常，“起始时间”设置为零，为方便起见，不失一般性，变换都从零到无穷积分（上述变换的下限为负无穷的积分称作[双边拉普拉斯变换](../Page/双边拉普拉斯变换.md "wikilink")）。
...
因果性和稳定性是描述系统的两个重要性质。如果独立变量是时间，那么因果性是必须的，但并不是所有系统的独立变量都是时间。例如，一个处理静止图像的系统不需要具备因果性。非因果系统可以建立，并可以在许多情况下发挥作用。即使是[非实数系统也可以构建](../Page/示性函数.md "wikilink")，并且在很多场合也是非常有用的。
...
正式场合下所研究的离散时间信号几乎总是连续时间信号的均匀采样。如果\(x(t)\)是一个连续时间信号，那么[模数转换器将把它转换成离散时间信号](../Page/模数转换器.md "wikilink")\(x[n]\)，

\[x[n] = x(nT)\],
其中*T*是[采样周期](../Page/采样周期.md "wikilink")。为了保证离散信号能够忠实地表示输入信号，非常重要的一点就是需要限制输入信号的频率范围。根据[采样定理](../Page/采样定理.md "wikilink")，离散时间信号所包括的最大频率范围是\(1/(2T)\)。其它频率都成为这个范围的[混叠信号](../Page/混叠.md "wikilink")。

### 时不变和线性变换

我们从一个冲激响应是二维函数的时变系统开始来看看时不变这个条件是如何将系统降到一维的。例如，假设输入信号是
\(x[n]\)，其中n是整数，即\(n \in \mathbb{Z}\)。[线性算子](../Page/线性算子.md "wikilink")\(\mathcal{H}\)表示系统在输入信号上的操作，对于这个index
set来说合适的算子是一个二维函数

\[h[n_1, n_2] \mbox{ where } n_1, n_2 \in \mathbb{Z}\]。

由于\(\mathcal{H}\)是一个线性算子，系统在输入信号\(x[n]\)上的作用就是下面[累加和所表示的](../Page/累加.md "wikilink")[线性变换](../Page/线性变换.md "wikilink")

\[y[n_1] = \sum_{n_2=-\infty}^{\infty} h[n_1, n_2] \, x[n_2],\]

如果[线性算子](../Page/线性算子.md "wikilink")\(\mathcal{H}\)也是[时不变的](../Page/时不变.md "wikilink")，那么
...
这样，[卷积和表示一个](../Page/卷积.md "wikilink")[线性](../Page/线性.md "wikilink")[时不变系统在任意输入函数上所起的作用](../Page/时不变系统.md "wikilink")，对于类似的有限维参数，参见[轮换矩阵](../Page/轮换矩阵.md "wikilink")

### 冲激响应

如果我们给系统输入一个离散[δ函数](../Page/δ函数.md "wikilink")
，由于δ函数是一个理想的脉冲，所以系统的线性时不变变换就是[冲激响应](../Page/冲激响应.md "wikilink")。我们用下式表示：

\[(h * \delta) [n] = \sum_{m=-\infty}^{\infty} h[n - m] \, \delta [m] = h[n],\]

（通过[δ函数的](../Page/δ函数.md "wikilink") sifting特性）。
...
and Chen,
1995）。我们也可以构建不稳定的系统，这种系统在很多场合都很有用，甚至也可以构建在很多情况下非常有用的[non-real系统](../Page/Quadrature_filter.md "wikilink")。
...
其中\(h[n]\)是冲激响应。由于逆变换不是唯一的，所以通常很难从Z变换确定系统的因果性。如果[收敛域确定](../Page/收敛域.md "wikilink")，系统的因果性也就随之确定。
...
（也就是说\(x[n]\)和\(y[n]\)的[最大绝对值都是有限的](../Page/最大绝对值.md "wikilink")），那么系统就是稳定的。必要且充分条件就是冲激响应\(h[n]\)满足

\[||h[n]||_1 = \sum_{n = -\infty}^\infty |h[n]| < \infty\]。

在频域中，[收敛域必须包含单位圆](../Page/收敛域.md "wikilink")\(|z|=1\)。

## 参见

  - [轮换矩阵](../Page/轮换矩阵.md "wikilink")

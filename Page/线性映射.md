在[数学中](../Page/数学.md "wikilink")，**线性映射**（有的书上将“**线性变换**”作为其同义词\[1\]，有的则不然\[2\]）是在两个[向量空间](../Page/向量空间.md "wikilink")（包括由函数构成的抽象的向量空间）之间的一种保持向量加法和[标量乘法的特殊](../Page/标量.md "wikilink")[映射](https://zh.wikipedia.org/wiki/映射 "wikilink")。线性映射从[抽象代数角度看是向量空间的](../Page/抽象代数.md "wikilink")[同态](../Page/同态.md "wikilink")\[3\]，从[范畴论角度看是在给定的](https://zh.wikipedia.org/wiki/范畴论_\(数学\) "wikilink")[域上的向量空间所构成的范畴中的](https://zh.wikipedia.org/wiki/域_\(数学\) "wikilink")[态射](../Page/态射.md "wikilink")。
...
从向量空间*\(V\)*到数域*K*的线性映射有一个特别的名字，叫做“[线性泛函](https://zh.wikipedia.org/wiki/线性泛函 "wikilink")”。线性泛函分析就是将空间维度增加到无穷维（包括[不可数无穷维](https://zh.wikipedia.org/wiki/不可数集 "wikilink")）的高等线性代数。线性泛函分析是[泛函分析最成熟的分支](../Page/泛函分析.md "wikilink")，但泛函分析最早研究的是有关向量空间*\(V\)*上的实值函数（它们一般是非线性映射）的[变分学问题](https://zh.wikipedia.org/wiki/变分学 "wikilink")。
...
<!-- end list -->
  ...
  - [积分生成从在某个](../Page/积分.md "wikilink")[区间上所有可积分实函数的空间到](https://zh.wikipedia.org/wiki/区间 "wikilink")\(R\)的线性映射。这只是把积分的基本性质（“积分的可加性”和“可从积分号内提出常数倍数”）用另一种说法表述出来。\[15\]
...
2.  遇到\(y=x+3\)这样的加上了1个常量的非线性映射可以通过增加1个维度的方法，把变换映射写成2×2维的方形矩阵形式，从而在形式上把这一类特殊的非线性映射转化为线性映射。这个办法也适用于处理在高维线性变换上多加了一个常向量的情形。这在[计算机图形学和刚体理论](../Page/计算机图形学.md "wikilink")（及其相关[机械制造和](https://zh.wikipedia.org/wiki/机械制造 "wikilink")[机器人学](../Page/机器人学.md "wikilink")）中都有大量应用。
3.  对角化的矩阵具有诸多优点。线性映射在写成矩阵后可以进行[对角化](https://zh.wikipedia.org/wiki/对角化 "wikilink")（不能对角化的矩阵可以化简成接近对角矩阵的[准对角矩阵](https://zh.wikipedia.org/wiki/准对角矩阵 "wikilink")），从而可以获得对角化矩阵拥有的独特优势（极大地简化乘法运算，易于分块，容易看出与基的选取无关的[不变量](https://zh.wikipedia.org/wiki/不变量 "wikilink")）。比如，对于作用于同一个空间的可对角化的方形矩阵*\(A\)*，要求出*\(A\)*自乘\(n\)次后的结果\(A^n\)，一个一个慢慢地乘是很麻烦的事情。而知道对角化技巧的人会发现，在将这矩阵对角化后，其乘法运算会变得格外简单。实际应用中有很多有意思的问题或解题方法都会涉及到矩阵自乘n次的计算，如1阶非齐次线性[递推数列](https://zh.wikipedia.org/wiki/递推关系式 "wikilink")[通项公式的线性代数求解法和](https://zh.wikipedia.org/wiki/通项公式 "wikilink")[马尔可夫链的极限状态](../Page/马尔可夫链.md "wikilink")（极限分布）的求解。线性代数及矩阵论的一个主要问题就是寻找可使矩阵对角化的条件或者可使矩阵化简到含很多个0的条件\[22\]，以便简化计算（这是主要原因之一）。
...
二维空间\(R^2\)的线性变换的一些特殊情况有：
  ...
  - [垂直错切](https://zh.wikipedia.org/wiki/错切 "wikilink")：
...
若*\(V\)*的自同态也剛好是[同构則稱之為](../Page/同构.md "wikilink")[自同构](../Page/自同构.md "wikilink")。两个自同构的[复合再次是自同构](https://zh.wikipedia.org/wiki/複合函數 "wikilink")，所以*\(V\)*的所有的自同构的集合形成一个[群](../Page/群.md "wikilink")，*\(V\)*的[自同构群可表为](https://zh.wikipedia.org/wiki/自同构群 "wikilink")\(\mathrm{Aut}(V)\)或\(\mathrm{GL}(V)\)。因为自同构正好是那些在复合運算下擁有逆元的自同态，所以\(\mathrm{Aut}(V)\)也就是在环\(\mathrm{End}(V)\)中的[可逆元群](../Page/可逆元.md "wikilink")。

如果*\(V\)*之维度\(n\)有限\(\mathrm{End}(V)\)[同构於带有在](../Page/同构.md "wikilink")*\(K\)*中元素的所有\(n\times n\)矩阵構成的[结合代数](https://zh.wikipedia.org/wiki/结合代数 "wikilink")，且*\(V\)*的自同态群[同构于带有在](https://zh.wikipedia.org/wiki/群同构 "wikilink")*\(K\)*中元素的所有\(n\times n\)可逆矩阵構成的[一般线性群](../Page/一般线性群.md "wikilink")\(\mathrm{GL}(n,K)\)。
...
[多重线性映射是线性映射最重要的推广](../Page/多重线性映射.md "wikilink")，它也是[格拉斯曼代数和](https://zh.wikipedia.org/wiki/格拉斯曼代数 "wikilink")[张量分析的数学基础](https://zh.wikipedia.org/wiki/张量分析 "wikilink")。其特例为[双线性映射](../Page/双线性映射.md "wikilink")。
...
### 其它参考资料
  ...
  - [Halmos, Paul
    R.](https://zh.wikipedia.org/wiki/Paul_Halmos "wikilink"),

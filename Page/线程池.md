**线程池**（）：一种[线程](../Page/线程.md "wikilink")使用模式。线程过多会带来调度开销，进而影响缓存局部性和整体性能。而线程池维护着多个线程，等待着监督管理者分配可并发执行的任务。这避免了在处理短时间任务时创建与销毁线程的代价。线程池不仅能够保证内核的充分利用，还能防止过分调度。可用线程数量应该取决于可用的并发处理器、处理器内核、内存、网络sockets等的数量。 例如，线程数一般取cpu数量+2比较合适，线程数过多会导致额外的线程切换开销。
...
线程池模式一般分为两种：HS/HA半同步/半异步模式、L/F领导者与跟随者模式。
  ...
  - 领导者跟随者模式，在线程池中的线程可处在3种状态之一：领导者leader、追随者follower或工作者processor。任何时刻线程池只有一个领导者线程。事件到达时，领导者线程负责消息分离，并从处于追随者线程中选出一个来当继任领导者，然后将自身设置为工作者状态去处置该事件。处理完毕后工作者线程将自身的状态置为追随者。这一模式实现复杂，但避免了线程间交换任务数据，提高了CPU cache相似性。在[ACE](../Page/ACE自适配通信环境.md "wikilink")(Adaptive Communication Environment)中，提供了领导者跟随者模式实现。
...
线程池的[伸缩性对性能有较大的影响](https://zh.wikipedia.org/wiki/伸缩性 "wikilink")。
  ...
  - 銷毀執行緒太慢，导致其它執行緒[资源](../Page/资源.md "wikilink")饥饿。

## Windows API的线程池函数

[Windows操作系统的](https://zh.wikipedia.org/wiki/Windows操作系统 "wikilink")[API提供了一套线程池的实现接口](../Page/Windows_API.md "wikilink")。\[1\]可以方便地创建、使用线程池。Windows线程池API被设计为一组协同对象， 其中有些对象表示工作单位、计时器、异步I/O 等等。使用下述用户模式的对象来管理线程池及相关的数据：

  - 线程池对象（pool object）包含了一组工作线程（worker threads）。每个[进程可以创建多个线程池对象](https://zh.wikipedia.org/wiki/进程 "wikilink")，也可以直接使用进程缺省的线程池。
  ...
  - 线程池工作对象（work object） 由一个函数指针和一个被称为上下文的 void 指针组成，线程池每次在执行时将 void 指针送入函数。可异步多次投寄给线程池去执行它的回调函数。实际上就是以异步方式调用回调函数。通过`SubmitThreadpoolWork`投寄工作对象到任务队列。如果不创建新的工作对象，则不能更改函数和上下文。
...
[Category:软件设计模式](https://zh.wikipedia.org/wiki/Category:软件设计模式 "wikilink") [Category:线程](https://zh.wikipedia.org/wiki/Category:线程 "wikilink") [Category:Windows并发API](https://zh.wikipedia.org/wiki/Category:Windows并发API "wikilink")

1.  \[<https://msdn.microsoft.com/EN-US/library/windows/desktop/ms686980(v=vs.85>).aspx MSDN:Using the Thread Pool Functions\]
2.  \[<https://msdn.microsoft.com/EN-US/library/windows/desktop/ms686766(v=vs.85>).aspx MSDN:Thread Pool API\]
3.  [MSDN: ThreadPool 类](https://msdn.microsoft.com/zh-cn/library/system.threading.threadpool.aspx)
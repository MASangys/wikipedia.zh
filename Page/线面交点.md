...
2\. 有且只有一个交点；
3\.
有无数个交点。](https://zh.wikipedia.org/wiki/File:Plane-line_intersection.svg "fig:线面交点的三种情况： 1. 没有交点； 2. 有且只有一个交点； 3. 有无数个交点。")

在[解析几何中](../Page/解析几何.md "wikilink"),
一条[直线与一个](../Page/直线.md "wikilink")[平面的交点可能是](../Page/平面.md "wikilink")[空集](../Page/空集.md "wikilink")、一个[点或一条直线](../Page/点.md "wikilink")。在计算机图形学、运动规划和碰撞检测中，经常需要分析相交类型，以及计算出点坐标或线的方程。
...
其中 \(\mathbf{n}\)
是该平面的[法线](../Page/法线.md "wikilink")，\(\mathbf{p_0}\)是平面上任意一点。（\(\mathbf{a}\cdot\mathbf{b}\)表示[向量](../Page/向量.md "wikilink")
\(\mathbf{a}\) 和 \(\mathbf{b}\)
的[数量积](https://zh.wikipedia.org/wiki/数量积 "wikilink")）
...
其中
\(\mathbf{l}\)是该直线的方向向量，\(\mathbf{l_0}\)是直线上任意一点，\(d\)是[实数范围内的标量](../Page/实数.md "wikilink")。将直线方程代入平面方程得
...
若
\(\mathbf{l}\cdot\mathbf{n} = 0\)，则直线与平面平行。此时，如果(\(\mathbf{p_0}-\mathbf{l_0})\cdot\mathbf{n} =0\)，则该直线在平面内，即直线上所有的点都是交点。否则，直线与平面没有交点。
...
[Line_plane.svg](https://zh.wikipedia.org/wiki/File:Line_plane.svg "fig:Line_plane.svg")
空间中一条直线可以用一个点和一个给定的方向来描述。则一条直线可以表示为如下点的集合

\[\mathbf{l}_a + (\mathbf{l}_b - \mathbf{l}_a)t, \quad t\in \mathbb{R}\]

其中 \(\mathbf{l}_a=(x_a, y_a, z_a)\) 和 \(\mathbf{l}_b=(x_b, y_b, z_b)\)
是直线上两个不同的点。
...
若直线与平面平行或在平面内，那么向量
\(\mathbf{l}_b - \mathbf{l}_a\)，\(\mathbf{p}_1-\mathbf{p}_0\) 及
\(\mathbf{p}_2-\mathbf{p}_0\)是[线性独立的](https://zh.wikipedia.org/wiki/线性无关 "wikilink")，且矩阵为奇异矩阵。

若满足 \(t \in [0,1]\)，则交点在直线上 \(\mathbf{l}_a\) 与 \(\mathbf{l}_b\) 之间。

若满足

\[u,v \in [0,1], \;\;\; (u+v) \leq 1,\] 则交点位于平面上
\(\mathbf{p}_0\)，\(\mathbf{p}_1\) 及 \(\mathbf{p}_2\)所构成的三角形中。
...
在[计算机图形学中的](../Page/计算机图形学.md "wikilink")[光线追踪算法中](https://zh.wikipedia.org/wiki/光线追踪 "wikilink")，一个面可以被表示为几个平面的集合。一个面的图像可以用光线与每个面的交点表达。在基于视觉的三维重建中（计算机视觉的一个子场），深度通常是由“三角测量法”测算的。

## 外部链接

  - [Intersections of Lines, Segments and Planes (2D & 3D) from
    GeomAlgorithms.com](http://geomalgorithms.com/a05-_intersect-1.html)

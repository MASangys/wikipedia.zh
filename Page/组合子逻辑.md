**组合子逻辑**是[Moses
Schönfinkel和](../Page/Moses_Schönfinkel.md "wikilink")[哈斯凱爾·加里介入的一种符号系统](../Page/哈斯凱爾·加里.md "wikilink")，用来消除[数理逻辑中对](../Page/数理逻辑.md "wikilink")[变量的需要](../Page/变量.md "wikilink")。它最近在计算机科学中被用做计算的理论模型和设计函数式编程语言的基础。它所基于的**组合子**是只使用函数应用或早先定义的组合子来定义从它们的参数得出的结果的[高阶函数](../Page/高阶函数.md "wikilink")。

## 数学中的组合子逻辑

组合子逻辑意图作为简单的元逻辑，它能澄清在逻辑符号中的量化变量的意义，并真正的消除对它们的需要。消除量化变量的另一种方式是[蒯因的谓词函子](../Page/蒯因.md "wikilink")。尽管多数组合子逻辑系统超出了一阶逻辑的表达能力，蒯因的谓词函子的表达能力等价于一阶逻辑（\[\[\#Quine_1960_1966|Quine
\[1960\] 1966\]\]）。

组合子逻辑的最初发明者Schönfinkel，在1929年之后基本停止了发表著作。Curry在1927年\[1\]晚期于[哥廷根大学读博士的时候重新发现了组合子](../Page/哥廷根大学.md "wikilink")。在1930年代后期，[邱奇和他的学生在普林斯顿发明了一个竞争的函数抽象的形式化](../Page/邱奇.md "wikilink")，就是[lambda演算](../Page/lambda演算.md "wikilink")，它比组合子逻辑更加流行。这些历史上的意外事件的结果是直到在1960年代和1970年代理论计算机科学开始感兴趣于组合子逻辑，关于这个主题的几乎所有工作都是[Haskell
Curry和他的学生们](../Page/Haskell_Curry.md "wikilink")，或比利时的Robert
Feys做的。Curry和Feys (1958)和Curry等人（1972）
纵览了组合子逻辑的早期历史。组合子逻辑和lambda演算的更现代的平行处置参见Barendregt（1984），他还评论了[Dana
Scott在](../Page/Dana_Scott.md "wikilink")1960年代和1970年代为组合子逻辑设计的[模型](../Page/域理论.md "wikilink")。

## 计算中的组合子逻辑

在计算机科学中，组合子逻辑被用做[可计算性理论中](../Page/可计算性理论.md "wikilink")[计算和](../Page/计算.md "wikilink")[证明论的简化模型](../Page/证明论.md "wikilink")。这个理论尽管简单，但捕获了计算本质的很多根本特征。

组合子逻辑可以被看作是[lambda演算的变体](../Page/lambda演算.md "wikilink")，它把lambda表达式（用来允许函数抽象）替代为**组合子**的有限集合，它们是不包含[自由变量的原始函数](../Page/自由变量.md "wikilink")。很容易把lambda表达式变换成组合子表达式，并且因为组合子归约比lambda归约要简单，它已经被用做用软件中的某些[非严格的](../Page/非严格编程语言.md "wikilink")[函数式编程语言和硬件的](../Page/函数式编程.md "wikilink")[图归约机的实现基础](../Page/图归约机.md "wikilink")。
...
这里的v是变量名字，它取自预定义变量名字的无限集合，而E1和E2是lambda-项。形如λv.E1的项叫做**抽象**。变量v叫做抽象的[形式参数](../Page/形式参数.md "wikilink")，而E1是抽象的“主体”。

项λv.E1表示一个函数，它应用于一个参数，绑定形式参数v到这个参数，接着计算E1的结果值---就是说，它返回E1，带有v的所有出现被参数所替代。

形如 (E1
E2)的项叫做**应用**。应用建模函数调用或执行：调用E1所代表的函数，带有E2作为它的[参数](../Page/参数.md "wikilink")，并计算结果。如果E1（有时叫做applicand）是一个抽象，则这个项可以被归约：参数E2可以被代换如E1的主体中E1的形式参数的位置上，而结果是一个新的lambda项，它等价于旧的。如果一个lambda项不包含形如
...
z)。（就是说，应用是[左结合的](../Page/结合性.md "wikilink")）。
...
（使用"\*"来指示乘法）。这里的x是这个函数的[形式参数](../Page/形式参数.md "wikilink")。要计算一个特定参数的平方，比如3，我们把它插入到这个定义中形式参数的位置上：
...
已知lambda演算在计算性上等价于关于计算的任何其他似乎为真的模型（包括[图灵机](../Page/图灵机.md "wikilink")）的能力；就是说，可以在任何这些模型中完成的计算都可以用lambda演算表达，反之亦然。根据[邱奇-图灵论题](../Page/邱奇-图灵论题.md "wikilink")，这些模型都可以表达任何可能的计算。
...
这里的v是一个变量，P是基本函数之一，而E1和E2是组合子项。基本函数自身是**组合子**，或不包含[自由变量的函数](../Page/自由变量.md "wikilink")。
...
**K**)自身不等于**I**。我们称这种项是[外延相等的](../Page/外延相等.md "wikilink")。**外延相等**捕获了函数的等式的数学概念：两个函数是相等的，如果它们对于相同的参数总是生成相同的结果。相反的，项自身捕获了函数的内涵相等的概念：两个函数是相等的，当且仅当它们有相同的实现。有很多实现恒等函数的方式；(**S**
**K** **K**)和**I**是其中的方式，(**S** **K**
...
\]构造可以把长度为n的lambda项展开为长度为[Θ](../Page/大O表示法.md "wikilink")（3<sup>n</sup>）的组合子项。
...
除了组合子**S**和**K**之外，[Schönfinkel的著作中包含了现在叫做](../Page/Moses_Schönfinkel.md "wikilink")**B**和**C**的两个组合子，带有如下归约：
...
这些表达式在把谓词逻辑或lambda演算转换成组合子表达式的时候非常有用。它们也被[Curry和更后来的](../Page/Haskell_Curry.md "wikilink")[David
Turner所使用](../Page/David_Turner.md "wikilink")，他们的名字已经关联到了它们的应用上了。使用这些组合子，我们可以扩展变换规则为如下：
...
这些组合子的现代名字源于[哈斯凱爾·加里在](../Page/哈斯凱爾·加里.md "wikilink")1930年的博士论文（参见[B,C,K,W系统](../Page/B,C,K,W系统.md "wikilink")）。在[Schönfinkel的最初著作中](../Page/Moses_Schönfinkel.md "wikilink")，把现在的S,
...
[函数式编程语言经常基于lambda演算的简单而普遍的语义](../Page/函数式编程语言.md "wikilink")。

David Turner使用它的组合子实现了[SASL编程语言](../Page/SASL.md "wikilink")。

### 逻辑

[Curry-Howard同构蕴涵了在逻辑和编程之间的联系](../Page/Curry-Howard同构.md "wikilink")：每个[直觉逻辑的有效的定理证明都直接对应于一个有类型的lambda项的归约](../Page/直觉逻辑.md "wikilink")，反之亦然。定理自身也通过函数类型标志（signature）来识别。特别是，有类型的组合子逻辑对应于[证明论中的](../Page/证明论.md "wikilink")[希尔伯特系统](../Page/希尔伯特系统.md "wikilink")。
...
## 参见
  ...
  - [图归约机](../Page/图归约机.md "wikilink")
  - [超组合子](../Page/超组合子.md "wikilink")

## 注釋

## 引用

  - [Moses Schönfinkel](../Page/Moses_Schönfinkel.md "wikilink"), 1924,
    "Über die Bausteine der mathematischen Logik," translated as "On the
    Building Blocks of Mathematical Logic" in From Frege to Gödel: a
    source book in mathematical logic, 1879-1931, [Jean van
    Heijenoort](../Page/Jean_van_Heijenoort.md "wikilink"), ed. Harvard
    University Press, 1967. ISBN 978-0-674-32449-7 The article that
    founded combinatory logic.
  ...
  - [Hendrik Pieter
    Barendregt](../Page/Hendrik_Pieter_Barendregt.md "wikilink"), 1984.
    The Lambda Calculus, Its Syntax and Semantics. Studies in Logic and
    the Foundations of Mathematics, Volume 103, North-Holland. ISBN
    978-0-444-87508-2

  - <span id="Quine 1960 1966">[Quine, W.
    V.](../Page/Willard_Van_Orman_Quine.md "wikilink"), \[1960\] 1966.
    "Variables explained away." Chapter 23 in W. V. Quine, Selected
    Logic Papers, 227–235. New York: Random House. Originally read by
    invitation to the American Philosophical Association in April 1960
    and published in their Proceedings.</span>

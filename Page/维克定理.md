**维克定理**（）由[吉安·卡罗·威克提出](../Page/吉安·卡罗·威克.md "wikilink")，在[量子场论中广泛用于将](../Page/量子场论.md "wikilink")[創生及湮滅算符的连乘积转化为该连乘积的](../Page/創生及湮滅算符.md "wikilink")[正规序与相应的收缩之和](../Page/正规序.md "wikilink")\[1\]，在和[费曼图的相关问题中有重要应用](../Page/费曼图.md "wikilink")。
...
下面具体检视 \(\hat{A}\) and \(\hat{B}\) 分别是产生算符和湮灭算符的四种情形。 \(N\)
粒子体系的产生和湮灭算符分别用 \(\hat{a}_i^\dagger\)
和 \(\hat{a}_i\) 来表示。它们满足对易（玻色子）或反对易（费米子）关系 \([\hat{a}_i,\hat{a}_j^\dagger]_\pm=\delta_{ij}\)，其中
\(\delta_{ij}\) 是[克罗内克函数](../Page/克罗内克函数.md "wikilink")。
...
\[\hat{a}_i^\bullet \,\hat{a}_j^{\dagger\bullet}= \hat{a}_i\, \hat{a}_j^\dagger \,- \mathopen{:}\,\hat{a}_i\,\hat{a}_j^\dagger \,\mathclose{:}\, = \delta_{ij}\]
其中 \(i,j = 1,\ldots,N\).
...
其中 \(i,j = 1,\ldots,N\),
\(\left[ \hat{A}, \hat{B} \right] \equiv \hat{A} \hat{B} - \hat{B} \hat{A}\) 是对易子， \(\delta_{ij}\)
是克罗内克函数。
...
对比上式的最左边和最右边可见， \(\hat{a}_i \,\hat{a}_j^\dagger\) 的顺序并未发生改变，只是换了一种表达方式。
...
一组产生和湮灭算符的乘积 \(\hat{A} \hat{B} \hat{C} \hat{D} \hat{E} \hat{F}\ldots\)
可以用正规乘积和收缩表示为：
...
维克定理在计算场算符的[真空态期望值的时候很有用](../Page/真空态期望值.md "wikilink")。因为所有正规乘积的真空态期望值为零，而任意两个算符的收缩根据上面的定义是一个很容易计算的数值，故任意产生算符与湮灭算符的连乘积的真空态期望值可以很容易计算出来，例如，上面最后一个费米子的例子，式子右边取真空态期望值后，根据正规乘积的性质，前面五项都是零，而後两项则可用克罗内克函数计算出来（分别为
-1 和 0），故式子左边的算符的真空态期望值为 -1。

## 参考文献

<references />

  - [G.C. Wick](../Page/吉安·卡罗·威克.md "wikilink"), [The Evaluation of the
    Collision Matrix](http://link.aps.org/abstract/PR/v80/p268), Phys.
    Rev. 80, 268 - 272 (1950)

  - Silvan S. Schweber, An Introduction to Relativistic Quantum Field
    Theory, Harper and Row, New York (1962). (Chapter 13, Sec c)

  - M. E. Peskin and D. V. Schroeder, An Introduction to Quantum Field
    Theory, Perseus Books (1995). (§4.3)

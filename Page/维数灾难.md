...
在[机器学习](../Page/机器学习.md "wikilink")问题中，需要在高维[特征空间](https://zh.wikipedia.org/wiki/特征空间 "wikilink")（每个特征都能够取一系列可能值）的有限数据样本中学习一种“自然状态”（可能是无穷分布），要求有相当数量的训练数据含有一些样本组合。给定固定数量的训练样本，其预测能力随着维度的增加而减小，这就是所谓的*Hughes影响*\[4\]或*Hughes现象*（以Gordon
F. Hughes命名）。\[5\]\[6\]

...
很多[多媒体数据流需要同时包含](../Page/多媒体.md "wikilink")[音频数据和](https://zh.wikipedia.org/wiki/音频 "wikilink")[视频数据](../Page/视频.md "wikilink")，这时通常会加入一些用于音频和视频数据同步的[元数据](../Page/元数据.md "wikilink")。这三种数据流可能会被不同的程序，进程或者硬件处理，但是当它们传输或者存储的时候，这三种数据通常是被封装在一起的。通常这种封装是通过[视频文件格式来实现的](../Page/视频文件格式.md "wikilink")，例如常见的[\*.mpg](../Page/MPEG.md "wikilink"),
...
[\*.rm](https://zh.wikipedia.org/wiki/RealMedia "wikilink") or
[\*.tta](../Page/TTA.md "wikilink").
这些格式中有些只能使用某些编解码器，而更多可以以[容器的方式使用各种编解码器](https://zh.wikipedia.org/wiki/容器 "wikilink")。
...
编译码器（endec=
**en**coder/**dec**oder）是一个相近但是不同的概念，它主要应用于硬件。在二十世纪中叶，“编解码器”是用于编码模拟信息到[PCM并能将它们解码回来的硬件设备](https://zh.wikipedia.org/wiki/PCM "wikilink")。该世纪后些时候，这个名字开始应用于一系列软件用于转换数字信号之间的格式，同时也包括了的功能。

**缺省逻辑**是[Ray
Reiter提出的用来形式化有缺省假定的推理的](https://zh.wikipedia.org/wiki/Ray_Reiter "wikilink")[非单调逻辑](../Page/非单调逻辑.md "wikilink")。
...
例如，计算机语言[Prolog在推导否定的时候](../Page/Prolog.md "wikilink")，使用了某种程度的缺省假定：如果否定的原子不能被证明为真，则它被假定为假。但是要注意，Prolog使用了所谓的[否定为失败](https://zh.wikipedia.org/wiki/否定为失败 "wikilink")：当解释器必须求值原子\(\neg F\)的时候，它尝试证明\(F\)是真，并如果这失败了则结论为\(\neg F\)是真。转而在缺省逻辑中，缺省有\(\neg F\)作为论据，它只能在\(\neg F\)与当前知识一致的时候应用。

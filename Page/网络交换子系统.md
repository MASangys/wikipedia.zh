...
switching）和[移动性管理](../Page/移动性管理.md "wikilink")（mobility
...
Register，简称：VLR）是一个数据库，用于存储漫游到一个MSC所提供服务的范围的签约用户的数据。网络中的每个主要基站都严格地仅由一个VLR来提供服务（在MSC池的情况下，一个[BTS可能由多个](../Page/基地收发机站.md "wikilink")[MSC提供服务](https://zh.wikipedia.org/wiki/#移动交换中心（MSC） "wikilink")），因此一个签约用户不会同时出现在超过1个VLR上。
存储在VLR中的数据要么接收自[HLR](https://zh.wikipedia.org/wiki/#归属位置寄存器（HLR） "wikilink")，要么从[MS](../Page/移动台.md "wikilink")（移动台，英文：Mobile

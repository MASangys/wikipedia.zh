...
网页抓取和网页索引极其相似，其中网页索引指的是大多数[搜索引擎采用使用的](../Page/搜索引擎.md "wikilink")[机器人或](../Page/机器人.md "wikilink")[网络爬虫等技术](https://zh.wikipedia.org/wiki/网络爬虫 "wikilink")。与此相反，网页抓取更侧重于转换网络上非结构化数据（常见的是HTML格式）成为能在一个中央[数据库和](../Page/数据库.md "wikilink")[电子表格中储存和分析的结构化数据](https://zh.wikipedia.org/wiki/电子表格 "wikilink")。网页抓取也涉及到网络自动化，它利用计算机软件模拟了人的浏览。网页抓取的用途包括在线的价格比较，联系人抓取，气象数据监测，网页变化检测，科研，[混搭和Web数据集成](../Page/混搭_\(互聯網\).md "wikilink")。

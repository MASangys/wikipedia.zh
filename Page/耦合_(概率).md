...
考察两个随机变量*X*,*Y*,分别具有连续[累积分布函数](../Page/累积分布函数.md "wikilink")*F*<sub>*X*</sub>和*F*<sub>*Y*</sub>。通过分别在两个随机变量上应用[概率积分转换](https://zh.wikipedia.org/wiki/概率积分转换 "wikilink")，得到*X’*
= *F*<sub>*X*</sub>（*X*） 和*Y’* =
*F*<sub>*Y*</sub>（*Y*）。因此*X’*和*Y’*都是具有连续[均匀分布的变量](https://zh.wikipedia.org/wiki/均匀分布 "wikilink")，相关性通常取决于*X*和*Y*是否是相关（自然，如果*X*和*Y*是不相关的，那么*X’*和*Y’*也是不相关的）。因为这个转换是可逆的，可以定义*X*和*Y*之间的相关性等于*X’*和*Y’*之间的相关性。因为*X’*和*Y’*是均匀分布的随机变量，所以问题被简化为定义一个在两个均匀分布之上的[二项分布](https://zh.wikipedia.org/wiki/二项分布 "wikilink")，这就是关联结构。所以，这一基本思想就是，通过把边缘变量转化为均匀分布变量而不再需要考察很多不同的边缘分布以简化问题，然后再把相关性定义为一个在均匀分布之上的联合分布。

## 定义

一个
**关联结构**是一个定义在*n*维[单位立方体](https://zh.wikipedia.org/wiki/单位立方体 "wikilink")\[0, 1\]<sup>*n*</sup>上的多元[联合分布](../Page/联合分布.md "wikilink")，其每个[边缘分布都是在](https://zh.wikipedia.org/wiki/边缘分布 "wikilink")\[0, 1\]区间上的[均匀分布](https://zh.wikipedia.org/wiki/均匀分布 "wikilink")。
...
<!-- end list -->
  ...
  -
    \(C\left(\mathbf u\right)\)是[n维递增的](https://zh.wikipedia.org/wiki/n维递增 "wikilink")，也即，有每个[hyperrectangle](https://zh.wikipedia.org/wiki/hyperrectangle "wikilink")
...
以[二项分布为例](https://zh.wikipedia.org/wiki/二项分布 "wikilink")，Sklar定理应用如下。对任一二项分布函数*H*（*x*, *y*），令*F*（*x*） = *H*（*x*, ∞）
...
在金融建模中常用到的一个关联结构是正态关联结构，正态关联结构是根据Sklar定理由二元[正态分布构成](../Page/正态分布.md "wikilink")。设\(\Phi_\rho\)是标准二元正态累积分布函数，[相关系数为](https://zh.wikipedia.org/wiki/相关系数 "wikilink")*ρ*，则正态关联结构函数为

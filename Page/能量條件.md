...
<!-- end list -->
  ...
  - **強能量條件(strong energy condition)**規定對於每個指向未來的「類時向量場」
    \(\vec{X}\)，由相應的觀察者所測量到的[潮汐張量](https://zh.wikipedia.org/wiki/潮汐張量 "wikilink")(tidal
    tensor)的[跡數](https://zh.wikipedia.org/wiki/跡數 "wikilink")(trace)永不為負值：
      -
...
[完美流體(Perfect
fluids)其物質張量具有如下形式](https://zh.wikipedia.org/wiki/流體解 "wikilink")

\[T^{ab} = \mu \, u^a \, u^b + p \, h^{ab}\]
其中\(\vec{u}\)為物質粒子的[4-速度](https://zh.wikipedia.org/wiki/4-速度 "wikilink")，而\(h^{ab}\)為在每個事件點，投影到垂直於4-速度的空間性超平面元素(spatial
hyperplane elements)[投影張量(projection
tensor)](https://zh.wikipedia.org/wiki/投影張量 "wikilink")。（注意到這些超平面元素**不會**構成空間性的超切片(hyperslice)，除非速度是無渦性(vorticity-free)的，也就是不旋轉的(irrotational)）。對於物質粒子運動相伴行的[參考系](https://zh.wikipedia.org/wiki/frame_fields_in_general_relativity "wikilink")，物質張量的分量呈現對角形式：

> 本文内容由[臨界區段](https://zh.wikipedia.org/wiki/臨界區段)转换而来。
...
當有執行緒進入臨界區段時，其他執行緒或是[行程](../Page/行程.md "wikilink")必須等待（例如：bounded waiting 等待法），有一些同步的機制必須在臨界區段的進入點與離開點實現，以確保這些共用資源是被[互斥或的使用](../Page/逻辑异或.md "wikilink")，例如：[semaphore](https://zh.wikipedia.org/wiki/semaphore "wikilink")。

只能被單一執行緒存取的裝置，例如：[印表機](https://zh.wikipedia.org/wiki/印表機 "wikilink")。

一個最簡單的實現方法就是當[執行緒](https://zh.wikipedia.org/wiki/執行緒 "wikilink")（Thread）進入臨界區段時，禁止改變處理器；在[uni-processor系統上](https://zh.wikipedia.org/wiki/uni-processor "wikilink")，可以用「禁止中斷（CLI）」來完成，避免发生[系统调用](https://zh.wikipedia.org/wiki/系统调用 "wikilink")（System Call）导致的[上下文交換](../Page/上下文交換.md "wikilink")（Context switching）；當離開臨界區段時，[處理器回復原先的狀態](https://zh.wikipedia.org/wiki/處理器 "wikilink")。
...
[MSDN Library -- Critical section](http://msdn2.microsoft.com/en-us/library/ms682530.aspx)

[Category:協同控制](https://zh.wikipedia.org/wiki/Category:協同控制 "wikilink") [Category:程序架構](https://zh.wikipedia.org/wiki/Category:程序架構 "wikilink")
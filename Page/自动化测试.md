...
测试自动化有许多途径，下面列出一些广泛应用的一般方法：
  ...
  - **（又称基于API的测试，API Based Testing）。**接口测试指的是通过调用接口（API）绕过GUI，，以应用到验证的行为进行测试。 通常API动绕过测试的应用程序的用户界面。 它也可以测试 公共(通常)的接口 ，以类、模块或图书馆都经过测试，有各种各样的输入参数来验证返回的结果是正确的。
...
## 外部链接
  ...
  - [Success Factors for Keyword Driven Testing](http://www.logigear.com/resources/articles-presentations-templates/389--key-success-factors-for-keyword-driven-testing.html) by Hans Buwalda

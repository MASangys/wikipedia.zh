...
**自动换行** （word wrap）是大多数[文字編輯器](https://zh.wikipedia.org/wiki/文字編輯器 "wikilink")、[文書處理器](https://zh.wikipedia.org/wiki/文書處理器 "wikilink")、和[网页浏览器](../Page/网页浏览器.md "wikilink")的一个附加功能。它用于在行间或一行里的单词间隔处分行，不考虑一个单词超过一行长度的情况。

它通常是在看文档或打印的时候[实时](../Page/实时.md "wikilink")完成的，所以没有储存或手动插入换行代码。如果用户改变了文档的边缘，编辑器就会自动重排换行符的位置，保证全部文本都处于可见状态，或者给打字员提供一些便捷的方式重排换行符。
...
在[中文](https://zh.wikipedia.org/wiki/中文 "wikilink")，[日文和](https://zh.wikipedia.org/wiki/日文 "wikilink")[韩文中](https://zh.wikipedia.org/wiki/韩语 "wikilink")，每个[汉字](../Page/汉字.md "wikilink")通常被认为是一个词，因此自动换行可以在汉字之前或之后发生。
...
大多数现有的文字处理和[排版](../Page/排版.md "wikilink")软件不能处理上述任意一种情况。

[中日韩标点符号可能不遵守上述特殊情况](https://zh.wikipedia.org/wiki/中日韩越统一表意文字 "wikilink")，这些规则通常称为（意思是“禁止规则处理”）。

有一个总是成立的特例：自动换行永远不能在中日韩越统一表意文字符集中的[破折号](../Page/破折号.md "wikilink")和[省略号](../Page/省略号.md "wikilink")之间发生。虽然由于现存字符编码的关系，这些符号需要用两个字符表示，但这些字符实际上是一个两字符宽的符号，而不是两个单字宽的符号对。
...
自动换行的一个简单的实现是使用[贪心算法](../Page/贪心算法.md "wikilink")：将尽可能多的单词放进一行里，直到所有的单词都已经放进去为止。这是很多现代文字处理软件的做法，如[Microsoft Word和](../Page/Microsoft_Word.md "wikilink")[Open Office](https://zh.wikipedia.org/wiki/Open_Office "wikilink") 。这个算法在追求最少行数的目标上能够达到最优化。下面是该算法的伪代码实现：
...
在[TeX](../Page/TeX.md "wikilink")中使用的，则是另一个算法，旨在将行尾空格数的平方和最小化，以产生一个更加美观的结果。以上的算法不能完成这一目标，例如：
...
最优解的代价可以用以下的[递归](../Page/递归.md "wikilink")式定义：
...
这可以利用[动态规划](../Page/动态规划.md "wikilink")来高效地实现，时间和空间复杂度均为\(O(j^2)\)
...
### Knuth's algorithm
  ...
  - [adjusting the Knuth algorithm](https://web.archive.org/web/20070930015603/http://www.nabble.com/Initial-soft-hyphen-support-t2970713.html) to recognize the ["soft hyphen"](https://zh.wikipedia.org/wiki/Hyphen#Hyphens_in_computing "wikilink").
  - [Knuth's breaking algorithm.](http://wiki.apache.org/xmlgraphics-fop/KnuthsModel) "The detailed description of the model and the algorithm can be found on the paper "Breaking Paragraphs into Lines" by Donald E. Knuth, published in the book "Digital Typography" (Stanford, California: Center for the Study of Language and Information, 1999), (CSLI Lecture Notes, no. 78.)" ; part of [Google Summer Of Code 2006](http://wiki.apache.org/xmlgraphics-fop/GoogleSummerOfCode2006/FloatsImplementationProgress)
...
### 其他自动换行有关的连接
  ...
  - [the reverse problem -- picking columns just wide enough to fit (wrapped) text](http://www.codecomments.com/message230162.html)
  ...
  - ["a Knuth-Plass-like linebreaking algorithm](http://www.techwr-l.com/archives/0504/techwhirl-0504-00203.html) ... The \*really\* interesting thing is how Adobe's algorithm differs from the Knuth-Plass algorithm. It must differ, since Adobe has managed to patent its algorithm (6,510,441)."[](http://www.techwr-l.com/archives/0504/techwhirl-0504-00206.html)
  - ["Murray Sargent: Math in Office"](http://blogs.msdn.com/murrays/archive/2006/11/15/lineservices.aspx)

[Category:动态规划](https://zh.wikipedia.org/wiki/Category:动态规划 "wikilink") [Category:字體排印](https://zh.wikipedia.org/wiki/Category:字體排印 "wikilink") [Category:文本编辑器功能](https://zh.wikipedia.org/wiki/Category:文本编辑器功能 "wikilink")
...
自动机编程中處理問題的思考方式很類似在利用[圖靈機](https://zh.wikipedia.org/wiki/圖靈機 "wikilink")、[马尔可夫算法處理問題時的思考方式](../Page/马尔可夫算法.md "wikilink")。

## 歷史

自动机编程的技術常用在以自动机原理為基礎的演算法中，例如[形式語言分析](https://zh.wikipedia.org/wiki/形式語言 "wikilink")\[1\]。

约翰逊等在1968年發表的《Automatic generation of efficient lexical processors using
finite state techniques》論文是早期提到自动机编程的論文\[2\]。 Peter
Naur在1963年的論文將自动机编程當成一種通用的軟體技術\[3\]。作者將此技術稱為「圖靈機的方法」，不過此論文是以自動機的狀態及步驟為基礎，沒有提到[圖靈機](https://zh.wikipedia.org/wiki/圖靈機 "wikilink")。
...
以下是傳統[指令式編程的C語言程式](../Page/指令式編程.md "wikilink")：
...
[Automata_that_prints_the_first_word_of_each_line.png](https://zh.wikipedia.org/wiki/File:Automata_that_prints_the_first_word_of_each_line.png "fig:Automata_that_prints_the_first_word_of_each_line.png")
...
自動化領域有些設計方式類似[图形用户界面的程式設計](../Page/图形用户界面.md "wikilink")，機器狀態的改變可以視為由事件而造成，由於事件使機器由一個狀態變為下一個狀態，直到到達最後的狀態為止。可能出現狀態的組合可以產生許多的事件，因此可以定義較複雜的製造週期，其產生的製造週期一般會比線性循序流程複雜許多。一般常常會有一些同時執行的平行路徑，以及依不同事件決定執行方式的路徑，如下圖：
...
若程式語言支援[物件導向程式設計](https://zh.wikipedia.org/wiki/物件導向程式設計 "wikilink")，就可以將自動機[封裝為一個物件](../Page/封裝_\(物件導向程式設計\).md "wikilink")，隱藏自動機實現的細節。一種稱為「」的[設計模式即包括了此作法](https://zh.wikipedia.org/wiki/設計模式 "wikilink")。上述的程式可以改為為以下的物件導向程式，利用[C++來實現](../Page/C++.md "wikilink")：
...
自动机编程常用在[詞法分析及](https://zh.wikipedia.org/wiki/詞法分析 "wikilink")[語法分析器中](../Page/語法分析器.md "wikilink")\[4\]。

此外，用自動機的方式處理問題（將執行的程式分為自動機的步驟，以及各步驟間只透過顯式的狀態傳遞資訊）是[事件驅動程式設計中必要的一部份](../Page/事件驅動程式設計.md "wikilink")，否則就要使用平行程序或是多[线程的作法](../Page/线程.md "wikilink")。

狀態及狀態機的表示法常用在的領域。例如以[統一塑模語言為基礎的軟體架構開發](https://zh.wikipedia.org/wiki/統一塑模語言 "wikilink")，會使用[狀態圖表示程式的行為](https://zh.wikipedia.org/wiki/狀態圖 "wikilink")，許多[通訊協定也利用顯式的狀態來加以定義](https://zh.wikipedia.org/wiki/通訊協定 "wikilink")，例如RFC
793\[5\]。
...
[Scheme程式語言不是一個和狀態機有關的程式語言](../Page/Scheme.md "wikilink")（Scheme為遞迴式的），但其中的[延續性](https://zh.wikipedia.org/wiki/延續性 "wikilink")（Continuation）需要以自動機的步驟及狀態的方式來思考。若要使的機能有效，需要可以記錄整個執行程式的狀態，只有在所有狀態都是顯式，不存在隱式狀態的情形下才可能達到。此處的「記錄完整狀態」即為延續性，可以視為一個較複雜自動機的狀態，自動機的步驟是由以前的延續性資料推斷下一個的延續性資料，而所執行的程序就是這些步驟的循環。

亞歷山大·奥隆格罗（Alexander Ollongren）在其著作\[6\]中解釋了一種稱為「維也納方法」（Vienna
method）的程式語言語義描述，完全以形式自動機為基礎。
...
一般來說，狀態可視為所有在執行時會更改的資訊的結合，任何[计算机程序執行時都有其對應的狀態](../Page/计算机程序.md "wikilink")。一傳統[指令式編程程式的狀態包括](../Page/指令式編程.md "wikilink")：
...
4.  [程式計數器中的內容](../Page/程式計數器.md "wikilink")。
...
## 外部連結
  ...
  - [J. V. Noble. «Finite State Machines in
    Forth»](https://web.archive.org/web/20080318180229/http://dec.bournemouth.ac.uk/forth/jfar/vol7/paper1/paper.html)
    — automata-based programming in [Forth](../Page/Forth.md "wikilink")

  -
  -
  - <cite id="CITEREFPolikarpova2009">Polikarpova N. I., Shalyto A. A.
    [Automata-based programming](http://is.ifmo.ru/books/_book.pdf)
    SPb.: Piter. 2009 (rus)</cite>

**自动重传请求**（，）是[OSI模型中](../Page/OSI模型.md "wikilink")[数据链路层和](../Page/数据链路层.md "wikilink")[传输层的错误纠正协议之一](../Page/传输层.md "wikilink")。它通过使用确认和超时这两个机制，在不可靠服务的基础上实现可靠的信息传输。如果发送方在发送后一段时间之内没有收到确认帧，它通常会重新发送。ARQ可能包括停止等待协议和连续协议，错误检测（Error
Detection）、正面确认（Positive Acknowledgment）、超时重传（Retransmission after
Timeout）和 负面确认及重传（Negative Acknowledgment and Retransmission）等机制。
...
UMTS的ARQ机制是在[基地台控制站](../Page/基地台控制站.md "wikilink")（Radio Network
Controller，[RNC](../Page/RNC.md "wikilink")），使用安置在协议数据单元（Protocol Data
Unit，[PDU](../Page/PDU.md "wikilink")）前的序号来作为是否有封包丢失的依据，有不少的延迟时间。

## 优点和缺点

ARQ协议的优点是它非常的简单。因而被广泛的应用在[分组交换网络中](../Page/分组交换.md "wikilink")。

ARQ协议的缺点是需要接收方发送ACK，这样增加了网络的负担也影响了传输速度。重复发送数据包来纠正错误的方法也严重的影响了它的传输速度。

## 参见

  - [HARQ](../Page/HARQ.md "wikilink")
  - [选择重传ARQ](../Page/选择重传ARQ.md "wikilink")

## 參考文獻

  - Peterson and Davie, *Computer Networks: A Systems Approach*, Third
    Edition, 2003
  ...
  - [Negative acknowledgement
    ARQ](http://www.techex.co.uk/other/arq-video-packet-resend) for live
    video

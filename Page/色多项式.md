在[代数图论中](https://zh.wikipedia.org/wiki/代数图论 "wikilink")，**色多项式**是[乔治·戴维·伯克霍夫为了尝试证明](https://zh.wikipedia.org/wiki/乔治·戴维·伯克霍夫 "wikilink")[四色定理而定义的一种](../Page/四色定理.md "wikilink")[多项式](https://zh.wikipedia.org/wiki/多项式 "wikilink")。

**色多项式**\(P(G,t)\)的值是在[图](https://zh.wikipedia.org/wiki/图 "wikilink")\(G\)中[顶点的不同着色方法数目](https://zh.wikipedia.org/wiki/顶点 "wikilink")，是关于不同颜色数目\(t\)的函数。
...
在图\(G\)的一边\(e\)上添加点\(v\)所得图记为\(G+v_e\)，两端点着同色时有\((t-1)P(G\cdot e)\)种着色法，两端点着不同色是有\((t-2)P(G)\)种着色法。

\[P(G+v_e)=(t-2)P(G)+(t-1)P(G\cdot e)\]\[3\]

### 补图

[L(G).png](https://zh.wikipedia.org/wiki/File:L\(G\).png "fig:L(G).png")的[线图的补图](https://zh.wikipedia.org/wiki/线图 "wikilink")。\]\]
若\(G\)为有\(n\)个顶点的图，且它的[独立数](https://zh.wikipedia.org/wiki/独立数 "wikilink")\<3，

\[P(G,t)=(t)_n+a_1(t)_{n-1}+a_2(t)_{n-2}+...+a_{[\frac{n}{2}]}(t)_{[\frac{n}{2}]}\]\[4\]
其中\((t)_n\)表示[阶乘幂](https://zh.wikipedia.org/wiki/阶乘幂 "wikilink")，\(a_i\)为图\(\overline{L(\overline{G})}\)中所含的完全子图\(K_i\)的个数。

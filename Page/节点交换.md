...
节点交换不能用在将一个新对等端介绍到一个swarm。要初始化与swarm的接触，每个对等端必须使用一个“.torrent”文件连接到tracker，或者使用名为[bootstrap
node的路由计算机来找到一个描述了对等端swarm列表的](https://zh.wikipedia.org/wiki/bootstrap_node "wikilink")[DHT](https://zh.wikipedia.org/wiki/DHT "wikilink")。对于大多数BitTorrent用户而言，DHT和PEX将在BitTorrent客户端启动并打开一个.torrent文件后开始工作。另一点值得注意的是“私有种子”，它不会公开可用，且通常将禁用DHT。
...
为创建一个PEX协议，提供均匀分布的对等端选择，其中一个可以对种子形成一个小的DHT本地网络。 For each desired new
peer one would look up a (uniformly) random key, and use the node
responsible for the key as a new peer.这在概念上很简单，但也需要不小的开销。
...
下列客户端的特定版本已实现对等端交换：
  ...
  - [aria2](https://zh.wikipedia.org/wiki/aria2 "wikilink")，支持µTorrent
    PEX\[3\]
  - [Vuze](../Page/Vuze.md "wikilink")，旧称Azureus，以及基于它的客户端（Vuze
    PEX仅与Transmission客户端兼容。其他客户端的PEX已在Vuze及Azureus
    3.0.4.3及以上版本中实现）\[4\]
  - [BitComet的旧版本中支持使用专有协议的PEX](../Page/BitComet.md "wikilink")。\[5\]从v.1.19开始，它也添加了µTorrent/Mainline实现的基于扩展协议的PEX。\[6\]
  - [Bitflu](https://zh.wikipedia.org/wiki/Bitflu "wikilink")\[7\]
  - [BitTorrent](../Page/BitTorrent_\(软件\).md "wikilink")\[8\]
  - [KTorrent在](../Page/KTorrent.md "wikilink")2.1 RC1中已实现完整的µTorrent
    PEX支持。\[9\]
  ...
  - [Opera](https://zh.wikipedia.org/wiki/Opera電腦瀏覽器 "wikilink")
    9.5，支持µTorrent PEX\[14\]
  - [qBittorrent](https://zh.wikipedia.org/wiki/qBittorrent "wikilink")，支持µTorrent
    PEX\[15\]
  ...
  - [XTorrent基于Transmission的源代码](https://zh.wikipedia.org/wiki/XTorrent "wikilink")，因此在其1.0
    (v40)版本中同样兼容Vuze和µTorrent的实现\[18\]

## 参考资料

## 外部链接

  - [Description on the official Azureus
    wiki](http://wiki.vuze.com/w/Peer_Exchange)
...
8.  [BitTorrent User Manual - Appendix A: The BitTorrent Interface -
    Miscellaneous - Torrent Properties - General - Other Settings - Peer
    Exchange](http://www.bittorrent.com/intl/es/help/manual/appendixa040401#Other_Settings.Peer_Exchange)

**莱维过程**（Lévy
process）源于[法国](https://zh.wikipedia.org/wiki/法国 "wikilink")[数学家](../Page/数学家.md "wikilink")[保羅·皮埃爾·萊維](https://zh.wikipedia.org/wiki/保羅·皮埃爾·萊維 "wikilink")，是连续时间上的一种拥有独立稳定增量的左极限右连续（Càdlàg）的随机过程。著名的例子有**维纳过程**和**泊松过程**。
...
2.  **独立增量**：对任何\(0 \leq t_1 < t_2<\cdots <t_n <\infty\),
    \(X_{t_2}-X_{t_1}, X_{t_3}-X_{t_2},\dots,X_{t_n}-X_{t_{n-1}}\)[相互独立](https://zh.wikipedia.org/wiki/统计独立性 "wikilink")。
3.  **稳定增量**：对任何\(s<t \,\),
    \(X_t-X_s \,\)与\(X_{t-s} \,\)有相同[分布](../Page/概率分布.md "wikilink")
4.  \(t \mapsto X_t\) is
    [几乎确定](https://zh.wikipedia.org/wiki/几乎处处 "wikilink")[右连左极](../Page/右连左极函数.md "wikilink").

## 性质

### 独立增量

设*X*<sub>*t*</sub>是一个连续时间上的随机过程。也就是说，对于任何固定的*t* ≥
0，*X*<sub>*t*</sub>是一个随机变量。过程的**增量**为差值*X*<sub>*s*</sub> −
*X*<sub>*t*</sub>（任意的时间*t* \< *s*）。 **独立增量**意味着对于任何时间*s* \> *t* \> *u*
\> *v*，*X*<sub>*s*</sub> − *X*<sub>*t*</sub>和*X*<sub>*u*</sub> −
*X*<sub>*v*</sub>相独立。
...
1.  对任何\(\scriptstyle t\geq 0\)，
    随机变量\(X_t\)服从[正态分布](../Page/正态分布.md "wikilink")\(\scriptstyle \mathcal N(0,t)\),
2.  它的轨迹是几乎处处连续的；即，
    对于几乎所有的事件\(\scriptstyle \omega\)，关于*t*的函数\(\scriptstyle \omega \mapsto X_t(\omega)\)是连续的。
...
当且仅当它的[傅立叶变换为](https://zh.wikipedia.org/wiki/傅立叶变换 "wikilink")：
...
Ken-iti Sato. Lévy Processes and Inﬁnitely Divisible

...
萨维奇定理的证明是构造性的。证明过程为设计一个针对有向图连通性问题的算法（其它问题可以通过图灵机的格局图归约到此问题）。有向图连通问题可以简述为对于一个[有向图和给定的两个顶点](https://zh.wikipedia.org/wiki/有向图 "wikilink")*s*和*t*，是否存在从*s*到*t*的有向路径。对于*n*个顶点，存在一个算法在\(\mbox{O}\left((\log{n})^2\right)\)空间内解决这一问题。这一算法的基本思路是利用[递归](../Page/递归.md "wikilink")解决一个更一般化的问题：检查是否存在从*s*到*t*的一条至多包含*k*条边的有向路径，*k*是递归的输入参数。原始的有向图连通问题当\(k=n\)时与此问题等价。为了测试是否存在一条从*s*到*t*的长度为*k*的有向边，可以测试是否存在一条从*s*到*t*的以*u*为中点的有向边。如果存在，那么对从*s*到*u*和从*u*到*t*递归此算法。

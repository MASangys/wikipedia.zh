**行程**（），是指電腦中已執行的[程式](https://zh.wikipedia.org/wiki/程式 "wikilink")。行程為曾经是[分時系統的基本運作單位](https://zh.wikipedia.org/wiki/分時系統 "wikilink")。在面向进程设计的系统（如早期的[UNIX](../Page/UNIX.md "wikilink")，[Linux](../Page/Linux.md "wikilink")
...
## 名稱
  ...
  - [分時系統環境](https://zh.wikipedia.org/wiki/分時系統 "wikilink")，行程稱為**使用者程式**（*user
    progams*）或**任務**（*tasks*）。
...
操作系统使用[进程ID来唯一标识每个进程](https://zh.wikipedia.org/wiki/进程ID#Microsoft_Windows "wikilink")。在一个进程内部，使用进程[句柄来标识关注的每个进程](../Page/句柄.md "wikilink")。使用Windows
API从进程ID获取进程句柄：
...
创建的子进程可以继承父进程的：
  ...
  - CreateFile返回的打开句柄，包括[文件](https://zh.wikipedia.org/wiki/文件 "wikilink")、控制台输入缓冲区、控制台屏幕缓冲区,
    [命名管道](https://zh.wikipedia.org/wiki/命名管道 "wikilink"), 串口通信设备,
    [邮槽](https://zh.wikipedia.org/wiki/邮槽 "wikilink").

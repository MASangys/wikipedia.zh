**行程**（），是指電腦中已執行的[程式](../Page/程式.md "wikilink")。行程為曾经是[分時系統的基本運作單位](../Page/分時系統.md "wikilink")。在面向进程设计的系统（如早期的[UNIX](../Page/UNIX.md "wikilink")，[Linux](../Page/Linux.md "wikilink")
2.4及更早的版本）中，进程是程序的基本执行实体；在面向线程设计的系统（如当代多数操作系统、[Linux](../Page/Linux.md "wikilink")
2.6及更新的版本）中，行程本身不是基本執行单位，而是[執行緒的容器](../Page/執行緒.md "wikilink")。程式本身只是指令、数据及其组织形式的描述，行程才是程式（那些指令和数据）的真正執行实例。若干行程有可能與同一個程式相關聯，且每個行程皆可以同步（循序）或异步（[平行](../Page/平行計算.md "wikilink")）的方式獨立執行。現代[電腦系統可在同一段時間內以进程的形式将多個程式載入到記憶體中](../Page/電腦系統.md "wikilink")，並藉由時間共享（或稱[时分复用](../Page/时分复用.md "wikilink")），以在一個[處理器上表現出同時](../Page/處理器.md "wikilink")（[平行性](../Page/平行性.md "wikilink")）執行的感覺。同樣的，使用多執行緒技術（多執行緒即每一個執行緒都代表一個行程内的一个独立执行上下文）的作業系統或電腦架構，同樣程式的**平行**线程，可在多CPU主機或網絡上真正**同時**執行（在不同的CPU上）。

## 名稱

  - [整批系統環境](../Page/整批系統.md "wikilink")，行程稱為**工作**（*jobs*）。
  - [分時系統環境](../Page/分時系統.md "wikilink")，行程稱為**使用者程式**（*user
    progams*）或**任務**（*tasks*）。
...
行程需要一些資源才能完成工作，如[CPU使用時間](../Page/CPU.md "wikilink")、[記憶體](../Page/記憶體.md "wikilink")、檔案以及[I/O裝置](../Page/I/O.md "wikilink")，且為依序逐一進行，也就是每个CPU核心任何時間內僅能執行一項行程。
...
一個電腦系統行程包括（或者說「擁有」）下列資料：
  ...
  - 分配給該行程的資源的作業系統描述符，諸如[檔案描述符](../Page/檔案描述符.md "wikilink")（Unix術語）或[檔案控制代碼](../Page/句柄.md "wikilink")（Windows）、資料源和資料終端。
...
操作系统使用[进程ID来唯一标识每个进程](../Page/进程ID#Microsoft_Windows.md "wikilink")。在一个进程内部，使用进程[句柄来标识关注的每个进程](../Page/句柄.md "wikilink")。使用Windows
API从进程ID获取进程句柄：
...
创建的子进程可以继承父进程的：
  ...
  - CreateFile返回的打开句柄，包括[文件](../Page/文件.md "wikilink")、控制台输入缓冲区、控制台屏幕缓冲区,
    [命名管道](../Page/命名管道.md "wikilink"), 串口通信设备,
    [邮槽](../Page/邮槽.md "wikilink").
...
下述API的函数用于获取进程相关信息：
  ...
  - GetProcessWorkingSetSize：获取进程的[工作集内存被允许的下限与上限](../Page/工作集.md "wikilink")
  - SetProcessWorkingSetSize：设置进程的[工作集内存的下限与上限](../Page/工作集.md "wikilink")

进程终止时，所有打开的句柄被关闭，进程对象被触发（signaled）。进程的退出码（exit
code）或者在ExitProcess、TerminateProcess函数中指出，或者是main、WinMain函数返回值。如果进程由于一个致命异常（fatal
exception）而终止，退出码是这个异常值，同时进程的所有执行中的线程的退出码也是这个异常值。
...
## 相關
  ...
  - [時間片](../Page/時間片.md "wikilink")
  - [进程树](../Page/进程树.md "wikilink")

[Category:操作系统技術](https://zh.wikipedia.org/wiki/Category:操作系统技術 "wikilink")
[Category:进程](https://zh.wikipedia.org/wiki/Category:进程 "wikilink")

1.  \[<https://msdn.microsoft.com/en-us/library/windows/desktop/ms686722(v=vs.85>).aspx

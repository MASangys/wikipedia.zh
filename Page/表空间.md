...
一个表空间是一个存储位置，其中底层的基础[数据库](../Page/数据库.md "wikilink")的对象可以保留。它在物理和逻辑数据之间提供了一个抽象层\[1\] 并用于所有[DBMS管理的段分配储存](../Page/数据库.md "wikilink")。 (数据库段是占物理空间的数据库对象，例如[表和](https://zh.wikipedia.org/wiki/数据库表 "wikilink")[索引](https://zh.wikipedia.org/wiki/数据库索引 "wikilink")。)创建后，可以在创建数据库段时按名称引用表空间。
...
虽然表空间通常将其数据储存在文件系统的文件中，但是数据文件必须是表空间的一部分 ，一些[数据库](../Page/数据库.md "wikilink")管理系统允许直接在操作系统（被称为[原始设备](https://zh.wikipedia.org/wiki/裸设备 "wikilink")）上配置表空间，从而通过避免操作系统文件的时间开销来提供更好的性能。

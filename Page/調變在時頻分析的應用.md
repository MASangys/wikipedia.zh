...
用於了解一訊號的頻譜隨著時間的變化情形，常用的方法有[短時距傅立葉變換](../Page/短時距傅立葉變換.md "wikilink")（STFT）、[韋格納分布](https://zh.wikipedia.org/wiki/韋格納分布 "wikilink")（WDF）、[加伯轉換等](../Page/加伯轉換.md "wikilink")。例如有一訊號x(t)在0\~10秒等於cos(2\*pi\*t)，在10\~20秒等於cos(6\*pi\*t)，在20\~30秒等於cos(4\*pi\*t)，則時頻分析的結果如圖。（此為方波短時距傅立葉轉換（rec-STFT）的結果）
...
\[\frac{1}{\sqrt{|a|}}x(\frac{t}{a}) \longrightarrow S_x(\frac{t}{a},af)\]
韋格納分布：
...
\[x(t) = e^{j\pi at^2}y(t) \longrightarrow S_x(t,f) \approx  S_y(t,f-at)\]
韋格納分布：
...
\[x(t) = e^{j\pi at^2}*y(t) \longrightarrow S_x(t,f) \approx  S_y(t-af,f)\]
韋格納分布：
...
若\(\phi=0.5\pi\)則此分數傅立葉轉換會與傅立葉轉換相等，即一信號做傅立葉轉換可順時鐘旋轉\(0.5\pi\)，且會有以下特性：
...
## 參考文獻
  ...
  - Jian-Jiun Ding, Time frequency analysis and wavelet transform class
    ...
    University (NTU), Taipei, Taiwan, 2015.
  - SG Mallat, Z Zhang, Matching pursuits with time-frequency
    dictionaries, Signal Processing, IEEE Transactions on, 1993 -
    ieeexplore.ieee.org

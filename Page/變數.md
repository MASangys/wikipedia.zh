在[初等數學裡](https://zh.wikipedia.org/wiki/初等數學 "wikilink")，**變數**或**變元**、**元**是一個用來表示值的[符號](https://zh.wikipedia.org/wiki/符號 "wikilink")，該值可以是隨意的，也可能是未指定或未定的。在代數運算時，將變數當作明確的數值代入運算中，可以於單次運算時解出多個問題。一個典型的例子為[一元二次公式](https://zh.wikipedia.org/wiki/一元二次方程#公式解 "wikilink")，該公式可以解出每個[一元二次方程的值](../Page/一元二次方程.md "wikilink")，只需要將[方程的系數代入公式中的變數即可](../Page/方程.md "wikilink")。
...
使用[C和其相關語言](https://zh.wikipedia.org/wiki/C语言 "wikilink")，[變數名稱在语法上称为](../Page/变量_\(程序设计\).md "wikilink")[标识符](https://zh.wikipedia.org/wiki/标识符 "wikilink")，必須是由英文字母、數字和底線組成，且必須由字母起頭。有時還不可以使用某些[保留字命名](../Page/保留字.md "wikilink")。

使用某些語言，變數的名字同時告訴了這個變數帶有甚麼種類的值。例如[FORTRAN的程式裏](https://zh.wikipedia.org/wiki/FORTRAN "wikilink")，變數的首個字母顯示了它是[整數還是](https://zh.wikipedia.org/wiki/整數 "wikilink")[浮点数](../Page/浮点数.md "wikilink")。變數名字首個字符是$的話，在[BASIC的程式裏表示其值是字串](../Page/BASIC.md "wikilink")。[Perl透過字首如](../Page/Perl.md "wikilink")$,@,%和&來分辨哪是純量、陣列、雜湊或副程式。

每個編程組織都有非正式的命名規矩——單打獨鬥的程式員亦是如此。有人喜歡所有變數都用簡單的英文字母取名，認為能增加輸入程式碼的速度，但只要變數一多，就會容易混淆，甚至以後自己看回程式碼也不懂在寫甚麼。

[迴圈控制變數这样的](https://zh.wikipedia.org/wiki/程式迴圈 "wikilink")[哑变量和数学上的习惯类似](https://zh.wikipedia.org/wiki/哑变量 "wikilink")，通常以`i`,
`j`, `k`命名。

## 统计学上

变量是[统计学研究中对象的特征](../Page/统计学.md "wikilink")。它可以是定性的也可以是定量的，一个定量变量要么是[离散的](https://zh.wikipedia.org/wiki/离散 "wikilink")，要么是[连续的](https://zh.wikipedia.org/wiki/连续 "wikilink")。社会科学中研究变量的关系，通常採用數學中[對應的觀念](https://zh.wikipedia.org/wiki/映射 "wikilink")，把一个变量称为[自变量](https://zh.wikipedia.org/wiki/自变量 "wikilink")（独立变量），另一个变量称之为[因变量](https://zh.wikipedia.org/wiki/因变量 "wikilink")（依赖变量）\[4\]。

> 本文内容由[计数排序](https://zh.wikipedia.org/wiki/计数排序)转换而来。


**计数排序（Counting sort）**是一种稳定的[线性时间](https://zh.wikipedia.org/wiki/线性时间 "wikilink")[排序算法](../Page/排序算法.md "wikilink")。该算法于1954年由 Harold H. Seward 提出。计数排序使用一个额外的数组\(C\)，其中第i个元素是待排序数组\(A\)中值等于\(i\)的元素的个数。然后根据数组\(C\)来将\(A\)中的元素排到正确的位置。

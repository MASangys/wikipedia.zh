...
很多情况下，对大分子体系的处理可以完全避免使用量子化学计算。[分子力学模拟使用古典力学模型](../Page/分子力学.md "wikilink")（例如[谐振子](https://zh.wikipedia.org/wiki/谐振子 "wikilink")）描述化合物的能量。分子力学模型的所有常数均通过实验数据或[第一原理计算结果得到](https://zh.wikipedia.org/wiki/第一原理 "wikilink")。参数和方程的优化结果称为[分子力场](../Page/分子力场.md "wikilink")。

**计算流体力学**（，簡稱**CFD**）是21世纪[流体力学领域的重要技術之一](../Page/流体力学.md "wikilink")，使用[数值方法在计算机中对流体力学的控制方程进行求解](https://zh.wikipedia.org/wiki/数值方法 "wikilink")，从而可预测[流场的流动](https://zh.wikipedia.org/wiki/流场 "wikilink")。目前有多种商业CFD软件问世，比如FLUENT、CFD-ACE+（CFDRC）、Phoenics、CFX、Star-cd等。
...
CFD最基本的考虑是如何把连续流体在计算机上用离散的方式处理。一个方法是把空间区域离散化成小胞腔，以形成一个立体网格或者[格点](https://zh.wikipedia.org/wiki/格点 "wikilink")，然后应用合适的[算法来解运动方程](../Page/算法.md "wikilink")（对于不粘滞流体用[欧拉方程](../Page/欧拉方程_\(流体动力学\).md "wikilink")，对于粘滞流体用[纳维-斯托克斯方程](../Page/纳维-斯托克斯方程.md "wikilink"))。另外，这样的一个网格可以是不规则的（例如在二维由三角形组成，在三维由四面体组成）或者是规则的；前者的特征是每个胞腔必须单独存储在内存中。最后，如果问题是高度动态的并且在尺度上跨越很大的范围，网格本身应该可以动态随时间调整，譬如在[自适应网格细化方法中](https://zh.wikipedia.org/wiki/自适应网格细化 "wikilink")。

如果选择不使用基于网格的方法，也有一些可选的替代，比较突出的有：

  - [光滑粒子流体动力学](https://zh.wikipedia.org/wiki/光滑粒子流体动力学 "wikilink")，求解流体问题的[拉格朗日方法](https://zh.wikipedia.org/wiki/拉格朗日方法 "wikilink")，
  - 谱方法，把方程映射到像[球谐函数和](../Page/球谐函数.md "wikilink")[切比雪夫多项式等正交函数上的技术](../Page/切比雪夫多项式.md "wikilink")。
  - [格子波尔兹曼方法](https://zh.wikipedia.org/wiki/格子波尔兹曼方法 "wikilink")（Lattice
    Boltzmann Methods），它在直角正交格点上模拟一个等价的中尺度系统，而不是求解宏观系统（也不是真正的微观物理）。

对于[层流情况和对于所有相关的长度尺度都可以包含在格点中的](https://zh.wikipedia.org/wiki/层流 "wikilink")[湍流的情形](../Page/湍流.md "wikilink")，直接求解[纳维-斯托克斯方程是可能的](../Page/纳维-斯托克斯方程.md "wikilink")（通过直接数值[模拟](https://zh.wikipedia.org/wiki/模拟 "wikilink")）。但一般情况下，适合于问题的尺度的范围甚至大于今天的大型[并行计算机可以建模的范围](https://zh.wikipedia.org/wiki/并行计算机 "wikilink")。

在这些情况下，[湍流模拟需要引入湍流模型](../Page/湍流.md "wikilink")。[大涡流模拟和RANS表述](https://zh.wikipedia.org/wiki/大涡流模拟 "wikilink")（[雷诺平均纳维－斯托克斯方程](../Page/雷诺平均纳维－斯托克斯方程.md "wikilink")）和*k-ε*模型或者雷诺应力模型一起，是处理这些尺度的两种技术。

很多实例中，其他方程和[纳维-斯托克斯方程要同时被求解](../Page/纳维-斯托克斯方程.md "wikilink")。这些其他的方程可能包括描述种类[浓度](https://zh.wikipedia.org/wiki/浓度 "wikilink")，[化学反应](../Page/化学反应.md "wikilink")，[热传导](../Page/热传导.md "wikilink")，等等。很多高级的代码允许更复杂的情形的模拟，涉及到多相流（例如，液/气，固/气，液/固）或者非[牛顿流体](../Page/牛顿流体.md "wikilink")（例如[血液](../Page/血液.md "wikilink")）。
...
在使用中的一些离散化方法包括：
  ...
  - [有限体积法](https://zh.wikipedia.org/wiki/有限体积法 "wikilink")（Finite
    Volume
...
[分离涡流模拟](https://zh.wikipedia.org/wiki/分离涡流模拟 "wikilink")（Detached eddy
...
离散化完成后要进行的方程系统的基本求解由很多数值线性代数的为人熟知的许多算法完成。可以使用静态的迭代方法，譬如对称[高斯-赛得尔](https://zh.wikipedia.org/wiki/高斯-赛得尔 "wikilink")（Gauss-Seidel）或者[渐进松弛](https://zh.wikipedia.org/wiki/渐进松弛 "wikilink")（successive
overrelaxation），或者[克雷洛夫子空间法](https://zh.wikipedia.org/wiki/克雷洛夫子空间 "wikilink")（Krylov
...
AVL/FIRE、[CFX](https://zh.wikipedia.org/wiki/CFX "wikilink")、[Fluent](https://zh.wikipedia.org/wiki/Fluent "wikilink")、KIVA、NUMECA、Phoenics、和STAR-CD。其它软件包作为附件或CFD工具的补充产品使用。这些包括用于后置处理的FieldView和求解细节化学动力学的KINetics。
...
## 软件
  ...
  - [Fire Dynamics
    Simulator消防工程应用软件](https://zh.wikipedia.org/wiki/Fire_Dynamics_Simulator "wikilink")
  ...
  - [COMSOL
    Multiphysics](https://zh.wikipedia.org/wiki/COMSOL_Multiphysics "wikilink")（也称FEMLAB）商用有限元包，用于[多物理包括CFD](https://zh.wikipedia.org/wiki/多物理 "wikilink")

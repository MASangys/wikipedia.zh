[机器学习](../Page/机器学习.md "wikilink")的普遍任务就是从[数据中学习和构建](https://zh.wikipedia.org/wiki/数据 "wikilink")[模型](../Page/数学模型.md "wikilink")（该过程称之为[训练](https://zh.wikipedia.org/wiki/训练 "wikilink")），并且能够在将来遇到的数据上进行[预测](../Page/推理.md "wikilink")。\[1\]用于构建最终模型的[数据集通常有多个](https://zh.wikipedia.org/wiki/训练集、验证集和测试集 "wikilink")；在构建模型的不同阶段，通常有三种数据集：**训练集、验证集和测试集**。

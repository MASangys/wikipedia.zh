...
當程式[當機時](https://zh.wikipedia.org/wiki/當機 "wikilink")，如果除錯器是屬於來源階段除錯器（source-level debugger）或象徵階段除錯器（symbolic debugger），除錯器即可以顯示出錯誤所在位置的原始碼，並使其於整合開發環境裡也能看見。要是屬於[低階除錯器](https://zh.wikipedia.org/wiki/低階除錯器 "wikilink")（low-level debugger）或機器語言除錯器（machine-language debugger）的話，它將能顯示的是一行一行的[反組譯碼](https://zh.wikipedia.org/wiki/反組譯 "wikilink")（這裡指的[當機情況是指](https://zh.wikipedia.org/wiki/當機 "wikilink")，當發生原因是因為程式設計師在設計上的疏失，使得程式無法繼續正常執行的情況。例如程式嘗試去調用某個對在該版本的CPU上而言是不合理的操作，或者是對保護或無法存取的記憶體位置進行寫入）。

...
## 相关概念
  ...
  - 大多数“频率”图实际上仅仅表示了谱密度。有时完整的频率要用两部分来表示，一部分是对应于频率的“幅度”（它就是谱密度），另外一部分是对应于频率的“[相位](https://zh.wikipedia.org/wiki/相位 "wikilink")”（它包含了频谱中剩余的其它信息）。信号 f(t) 可以从一个完整的[频谱](../Page/频谱.md "wikilink")进行恢复。需要注意的是 f(t) 不能仅仅从谱密度这一部分进行恢复——它丢失了“临时信息”。

...
Language，DDL）是[SQL](../Page/SQL.md "wikilink")語言集中負責資料結構定義與[資料庫物件定義的語言](https://zh.wikipedia.org/wiki/資料庫物件 "wikilink")，由`CREATE`、`ALTER`與`DROP`三個語法所組成，最早是由Codasyl（Conference
on Data Systems Languages）資料模型開始，現在被納入SQL指令中作為其中一個子集。

## 語法結構

目前大多數的[DBMS都支援對資料庫物件的DDL操作](https://zh.wikipedia.org/wiki/DBMS "wikilink")，部份資料庫
(如 [PostgreSQL](../Page/PostgreSQL.md "wikilink"))
可把DDL放在交易指令中，也就是它可以被撤回
(Rollback)。較新版本的[DBMS會加入DDL專用的觸發程序](https://zh.wikipedia.org/wiki/DBMS "wikilink")，讓資料庫管理員可以追蹤來自DDL的修改。

### CREATE

CREATE
是負責資料庫物件的建立，舉凡[資料庫](https://zh.wikipedia.org/wiki/資料庫 "wikilink")、[資料表](https://zh.wikipedia.org/wiki/資料庫表 "wikilink")、[資料庫索引](https://zh.wikipedia.org/wiki/資料庫索引 "wikilink")、[預存程序](https://zh.wikipedia.org/wiki/存儲程序 "wikilink")、[使用者函數](https://zh.wikipedia.org/wiki/使用者函數 "wikilink")、[觸發程序或是](https://zh.wikipedia.org/wiki/觸發器_\(資料庫\) "wikilink")[使用者自定型別等物件](https://zh.wikipedia.org/wiki/使用者自定型別 "wikilink")，都可以使用
...
其中，每個欄位的格式都有定義，並且若有需要建立[參考完整性的連結時](https://zh.wikipedia.org/wiki/參考完整性 "wikilink")，可以使用
...
[pl:Create
(SQL)](https://zh.wikipedia.org/wiki/pl:Create_\(SQL\) "wikilink")

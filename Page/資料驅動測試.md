...
在[軟體測試中](https://zh.wikipedia.org/wiki/軟體測試 "wikilink")，有許多不同的方法可以應用。這些方法並存的原因是它們在初始建立及維護時所需要的努力程度不同。数据驅動測試的優點是易於增加新的輸入，在產品或是[被測系統的輸入值發現新分組時格外方便](https://zh.wikipedia.org/wiki/被測系統 "wikilink")。以成本的觀點來看，数据驅動測試配合自動化測試的成本很低，但配合人工測試的成本則相當高昂。数据驅動測試和[關鍵字驅動測試看似相同](https://zh.wikipedia.org/wiki/關鍵字驅動測試 "wikilink")，後續會再說明二者的不同之處。
...
asset），可能是組態或是測試的数据集，而腳本中執行的邏輯則由数据內容來決定。[關鍵字驅動測試和数据驅動測試類似](https://zh.wikipedia.org/wiki/關鍵字驅動測試 "wikilink")，不過關鍵字驅動測試的測試用例是包含在数据的組合中，而不是嵌入在測試腳本中。

数据驅動測試所用的数据庫可以包括：

  -
  - [ODBC來源](../Page/ODBC.md "wikilink")
  ...
  -
  - [ADO物件](../Page/ADO.md "wikilink")

## 相關條目

  - [控制表](../Page/控制表.md "wikilink")

  - [關鍵字驅動測試](https://zh.wikipedia.org/wiki/關鍵字驅動測試 "wikilink")

...
数论中的不少[函数与正整数的质因子有关](../Page/函数.md "wikilink")，比如取值为的质因数个数的函数和取值为的质因数之和的函数。它们都是[加性函数](https://zh.wikipedia.org/wiki/加性函数 "wikilink")，但并非完全加性函数。
...
完全平方数是指等于某个正整数的[平方的数](../Page/平方.md "wikilink")。比如225 =
15<sup>2</sup>是完全平方数，而226不是。完全平方数的质因数分解中，每个质因数的幂次都是[偶数](https://zh.wikipedia.org/wiki/偶数 "wikilink")，这是因为假设完全平方数\(M = n^2\)，则它的质因数分解可以从的质因数分解推出\[1\]
。假设的质因数分解是：

\[n = p_1^{\alpha_1} \times p_2^{\alpha_2} \times \cdots \times p_r^{\alpha_r},\]
那么的质因数分解就是：

\[M = n^2 = p_1^{2\alpha_1} \times p_2^{2\alpha_2} \times \cdots \times p_r^{2\alpha_r},\]
所以每个质因子的幂次都是\(2\alpha_i\)的形式，是偶数。
...
\[420 = 2^2 \times 3 \times 5 \times 7,\] 所以(420) \(=\) 4，而(420) \(=\)
2×1 + 1 + 1 + 1 \(=\) 5. 因为420的质因数分解中2的幂次是2而其余质因子的幂次是1.

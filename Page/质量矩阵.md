在[分析力学中](../Page/分析力学.md "wikilink")，**质量矩阵**是[质量到](../Page/质量.md "wikilink")[廣義坐標概念上的推广](../Page/廣義坐標.md "wikilink")，它给出了系统[广义坐标](../Page/广义坐标.md "wikilink")*q*的变化率和系统[动能](../Page/动能.md "wikilink")*T*的关系，即

\[T = \frac{1}{2} \dot q^\mathrm{T} M \dot q\] 其中 \(\dot q^\mathrm{T}\)
是向量\(\dot q\)的 [转置](../Page/转置.md "wikilink") \[1\]。若粒子质量\(m\) ，速度*v*,
那么单个粒子的动能*T*为
...
假设粒子具有质量\(m_1\)和\(m_2\)，系统的[动能为](../Page/动能.md "wikilink")

\[E = \sum_{i=1}^{2} \frac{1}{2} m_i \dot{x_i}^2\] 将质量列写成矩阵

\[M=\begin{bmatrix}m_1&0\\0 & m_2\end{bmatrix}\]
那么总[动能由下列公式给出](../Page/动能.md "wikilink")：

\[E=\frac{1}{2} \dot{\mathbf{x}}^\mathrm{T} M \dot{\mathbf{x}}\]

在多维情况下，质量矩阵会变得更为复杂。例如，在二维情况下，一个给定的粒子具有两个自由度，因此，如果第*i* 个粒子对应自由度*j*
和*j*+1，那么

\[M_{j,j}=M_{j+1,j+1}=m_i\]

在如[刚体动力学之类质量是分布式的情况下的应用中](../Page/刚体动力学.md "wikilink")，非对角线元素非零的情况也是存在的。

一般地，质量矩阵*M*依赖于位置矢量*q*，且随时间变化。[拉格朗日力学中](../Page/拉格朗日力学.md "wikilink")，[常微分方程](../Page/常微分方程.md "wikilink")（组）描述了在系统中由粒子的位置所定义的广义坐标矢量随时间的变化。方程中的动能公式表示所有粒子的总动能。
...
\[q=[x_1\, x_2]^\mathrm{T}\]. 设粒子的质量分别为 *m*<sub>1</sub>,
*m*<sub>2</sub>, 系统的总动能是
...
更一般地，考虑一个N个粒子的系统，记标号分别为*i*=1, 2, ..., *N*，其中粒子 *i*
的位置是*N<sub>i</sub>*个自由直角坐标（其中*N<sub>i</sub>*是1，2，或3）。令*q*是由坐标形成的列向量。质量矩阵*M*是[对角块矩阵](../Page/对角块矩阵.md "wikilink")，每个块中的对角元素是对应的粒子的质量:\[2\]

\[M = \mathrm{diag}[ m_1 I_{n_1}, m_2 I_{n_2}, \cdots, m_N I_{n_N} ]\]

其中 **I**<sub>*n i*</sub> 是 *n<sub>i</sub>* × *n<sub>i</sub>*
[单位阵](../Page/单位阵.md "wikilink")，更具体地:
...
\[q=[ x, y, \alpha]\] 其中 *x*, *y* 是以棒中点为原点的直角坐标系，角度 *α*
是棒转动的角度。两个粒子的位置和速度是：
...
其中\(m  = m_1 + m_2\) ， \(d = m_1 - m_2\)。 这个公式写成矩阵形式为：
...
对於[连续介质力学的离散近似](../Page/连续介质力学.md "wikilink")，如在[有限元分析中](../Page/有限元分析.md "wikilink")，有多种方法可以构造质量方程，这取决於所期望的计算和精度性能。例如，利用忽略每一有限元变形的集中质量法可以构建一个对角质量矩阵并且不需要通过变形元来累积质量。
...
## 参见
  ...
  - [刚度矩阵](../Page/刚度矩阵.md "wikilink")
  - [惯性力矩](../Page/惯性力矩.md "wikilink")
  - [应力-能量张量](../Page/应力-能量张量.md "wikilink")

**费曼－卡茨公式**是一个[数学](../Page/数学.md "wikilink")[公式与](https://zh.wikipedia.org/wiki/公式 "wikilink")[定理](../Page/定理.md "wikilink")，得名于[理查德·费曼和](https://zh.wikipedia.org/wiki/理查德·费曼 "wikilink")[马克·卡茨](https://zh.wikipedia.org/wiki/马克·卡茨 "wikilink")，将[随机过程和抛物型](../Page/随机过程.md "wikilink")[偏微分方程结合在一起](../Page/偏微分方程.md "wikilink")。使用费曼－卡茨公式可以通过将某些抛物型偏微分方程的解写成随机过程的[条件期望的方式](https://zh.wikipedia.org/wiki/条件期望 "wikilink")，从而将求此类[微分方程的](../Page/微分方程.md "wikilink")[数值解转化为模拟随机过程的路径](https://zh.wikipedia.org/wiki/数值解 "wikilink")。反过来，此一类随机过程的期望可以通过确定性的计算（偏微分方程求解）得到。考虑偏微分方程：
...
其中的 \(\mu,\ \sigma,\ \psi, V\) 是已知的函数， \(\ T\)
是给定的参数，\(u:\mathbb{R}\times[0,T]\to\mathbb{R}\)
是所求的解函数。费曼－卡茨公式声明，这个偏微分方程的解函数可以写成某个随机过程的（条件）期望：

\[u(x,t) = E\left[ \int_t^T e^{-\int_t^s V(X_\tau)\ , d\tau}f(X_s,s)ds + e^{-  \int_t^T V(X_\tau)\, d\tau}\psi(X_T) | X_t=x \right]\]
其中\(\ X = \left( X_t; t \geqslant 0 \right)\) 是由以下的随机动力方程
...
决定的[伊藤随机过程](https://zh.wikipedia.org/wiki/伊藤过程 "wikilink")。其中的\(\ W_t\)是[维纳过程](../Page/维纳过程.md "wikilink")（Wiener过程，又称为布朗过程），\(\ X_t\)
满足初始条件\(\ X_0 = x\)。
...
1.  参数函数\(\mu,\ \sigma,\ \psi,\ V,\ f\)以及函数*u*
    都是[连续函数](../Page/连续函数.md "wikilink")。
2.  函数*u* 关于*x*
    变量保持[多项式增长](https://zh.wikipedia.org/wiki/多项式 "wikilink")，即存在正常数*M*和*c*，使得对所有的*x*，都有：
    \[\ u(x,t) \leqslant M(1+|x|^c)\]
3.  参数函数 \(\psi\) 和 \(f\) 都要么是正值函数，要么也满足类似以上的多项式增长条件。
4.  参数函数 \(V\) 有下界，并且
5.  参数函数 \(\mu,\ \sigma\) 满足关于*x*
    变量的[利普希茨条件](https://zh.wikipedia.org/wiki/利普希茨条件 "wikilink")，即存在常数*K*，使得对所有不相等的*x*
    和*y*，都有：
    \[| \sigma (x,t) -\sigma (y,t)| + | \mu (x,t) - \mu (y,t) | \leqslant K|x - y|\]

的时候，解函数可以用费曼－卡茨公式表达为条件期望的形式\[1\]。这些条件中并不保证解的存在性。要保证后者，需要更强的条件：

1.  参数函数 \(\mu,\ \sigma\) 有界，并且局部地满足关于*x* 变量和*t*
    变量的[利普希茨条件](https://zh.wikipedia.org/wiki/利普希茨条件 "wikilink")（即常数*K*可以和*x*相关）。
2.  对任意的*t*，参数函数 \(\sigma\)
    都满足[赫尔德连续条件](https://zh.wikipedia.org/wiki/赫尔德连续 "wikilink")，即存在与*t*无关的常数*H*和介于0与1之间的常数
    \(\alpha\)，使得
      -
        \(| \sigma (x,t) -\sigma (y,t)| \leqslant K|x - y|^{\alpha}\)
3.  参数函数 \(V\)
    有界，并且对任意的*t*，都局部地满足[赫尔德连续条件](https://zh.wikipedia.org/wiki/赫尔德连续 "wikilink")。
4.  对任意的*t*，参数函数 \(f\)
    都局部地满足[赫尔德连续条件](https://zh.wikipedia.org/wiki/赫尔德连续 "wikilink")，并关于*x*
    变量满足多项式增长条件。
5.  参数函数 \(\psi\) 关于*x* 变量满足多项式增长条件。

以上条件由弗里德曼在1975年给出。1980年克里洛夫提出用更简洁（同时更强）的条件代替，可以是：
在以上的条件下，偏微分方程的解唯一存在，并且满足费曼－卡茨公式的期望表达，同时也满足多项式增长条件\[2\]。

## 证明

为简化起见，以下只证明\(f(x, t)=0\)的情况。 设偏微分方程的解函数为
\(u(x, t)\)。对以下函数\(Y_s = e^{-  \int_t^s V(X_\tau)\, d\tau} u(X_s,s)\)
使用[伊藤公式](../Page/伊藤引理.md "wikilink")，可以得到：

\[dY_s = de^{-  \int_t^s V(X_\tau)\, d\tau} u(X_s,s) + e^{-  \int_t^s V(X_\tau)\, d\tau}\,du(X_s,s) +de^{-  \int_t^s V(X_\tau)\, d\tau}du(X_s,s)\]

由于
\(de^{-  \int_t^s V(X_\tau)\, d\tau} =-V(X_s) e^{-  \int_t^s V(X_\tau)\, d\tau} \,ds\)，等式右边第三项是高阶无穷小\(o(dt)\)，因此可以忽略。再一次对\(du(X_s,s)\)
使用伊藤公式，会得到
...
\[\;+e^{-  \int_t^s V(X_\tau)\, d\tau}\sigma(X_s,s)\frac{\partial u}{\partial x} (X_s,s) \,dW_s.\]
...
\[Y_T - Y_t = \int_t^T e^{-  \int_t^s V(X_\tau)\, d\tau}\sigma(X_s,s)\frac{\partial u}{\partial X} (X_s,s) \,dW_s.\]
两边取在已知\(X_t = x\)下的[条件期望](https://zh.wikipedia.org/wiki/条件期望 "wikilink")，并且注意到等式右边是一个[伊藤积分](../Page/伊藤积分.md "wikilink")，因此右边等于0。所以
\(E[Y(T)| X_t=x] =  E[Y(t)| X_t=x] = u(x,t)\)。注意到
...
也就是说 \(\gamma =\sigma\,\sigma^\prime\)，其中\(\sigma^\prime\)是矩阵
\(\sigma\)的[转置矩阵](../Page/转置矩阵.md "wikilink")\[4\]。
  ...
  - 此定理最早由卡茨于1949年发表\[5\]，最初的费曼－卡茨公式是作为一个解决某些维纳泛函的分布的公式提出的。假设\(\ x(\tau)\)
    是满足初始条件 \(\ x(0) = 0\)的某个扩散过程。现要求出以下函数的期望值

\[e^{-\int_0^t V(x(\tau))\, d\tau}\]
费曼－卡茨公式说明这个期望值等价于对某个扩散方程（抛物型偏微分方程）的解的积分。特别地，当条件\(\ u V(x) \geqslant 0\)满足时，若设\(\ w(x,0) = \delta(x)\)并满足\(\frac{\partial w}{\partial t} = \frac{1}{2} \frac{\partial^2 w}{\partial x^2} - u V(x) w\)，则有

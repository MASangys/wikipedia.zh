...
HTTP/1.1协议中共定义了八种方法（也叫“动作”）来以不同方式操作指定的资源：
  ...
  - GET:向指定的资源发出「顯示」请求。使用GET方法應該只用在讀取資料，而不应当被用于产生「[副作用](https://zh.wikipedia.org/wiki/#副作用 "wikilink")」的操作中，例如在[网络应用程序](../Page/网络应用程序.md "wikilink")中。其中一个原因是GET可能会被[网络蜘蛛等随意访问](https://zh.wikipedia.org/wiki/网络蜘蛛 "wikilink")。参见[安全方法](https://zh.wikipedia.org/wiki/#安全方法 "wikilink")
...
HTTP 1.1還使改進了HTTP 1.0的頻寬。 例如，HTTP 1.1引入了[分块传输编码](../Page/分块传输编码.md "wikilink")，以允許傳遞內容可以在持續連線上被串流傳輸而不必使用到[緩衝器](https://zh.wikipedia.org/wiki/緩衝器 "wikilink")。HTTP管道允許客戶端在收到每個回應之前發送多個請求，進一步減少[用戶感受到的滯後時間](../Page/使用者經驗.md "wikilink")。協議的另一個補充是字節服務，允許客戶端請求資源的某一部份，伺服器僅回應某資源的指明部分。

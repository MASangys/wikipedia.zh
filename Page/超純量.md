...
除了那些低功率CPU、嵌入式CPU、以及使用[電池電源的CPU](https://zh.wikipedia.org/wiki/電池 "wikilink")，1998年以後開發的CPU設計都已經是超純量體系结構。第一種採用了超標量技術的X86處理器是[Pentium](https://zh.wikipedia.org/wiki/Pentium "wikilink")。第一批採用了把CISC指令異步解碼为微指令序列技術的處理器是[Nx586](https://zh.wikipedia.org/wiki/Nx586 "wikilink"), [P6體系結構的](https://zh.wikipedia.org/wiki/P6 "wikilink")[Pentium Pro以及](https://zh.wikipedia.org/wiki/Pentium_Pro "wikilink")[AMD K5](../Page/AMD_K5.md "wikilink")。這使得被缓存的微指令可以動態調度（dynamic scheduling）執行，使得P6架構的並行性更優於P5架構的Pentium;這也使得[預測執行](https://zh.wikipedia.org/wiki/預測執行 "wikilink")（）更易實現，CPU的時脈也可以更高。

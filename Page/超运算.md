**超運算序列**是[数学中一种](../Page/数学.md "wikilink")[二元运算的](../Page/二元运算.md "wikilink")[序列](../Page/序列.md "wikilink")，前三[项分别为](../Page/表示式.md "wikilink")[加法](../Page/加法.md "wikilink")、[乘法](../Page/乘法.md "wikilink")、[幂](https://zh.wikipedia.org/wiki/幂 "wikilink")，一般來說，除了序列中第一項的加法運算之外，序列中每一項的運算都是重複的前一項的運算（例如乘法是重複的加法：\(a \cdot b = \underbrace{a + a + a + \cdots + a}_b\)，冪是重複的乘法：\(a^b = \underbrace{a \cdot a \cdot a \cdot \ldots \cdot a}_b\)）。这些运算通称为**超运算**（或稱為**hyper運算符**）。序列中的第*n*项称为**超-*n*运算**或**第*n*級的超運算**，其符號為**\[*n*\]**。英文則由命名，當*n*≥4時，由*n*的[希腊语](https://zh.wikipedia.org/wiki/希腊语 "wikilink")[前缀加上](https://zh.wikipedia.org/wiki/前缀 "wikilink")[后缀](https://zh.wikipedia.org/wiki/后缀 "wikilink")-ation组成（例如[超-4运算称为tetration](https://zh.wikipedia.org/wiki/超-4运算 "wikilink")，称为pentation）。\[1\]當*n*≥3
時，使用[高德纳箭号表示法可将超](https://zh.wikipedia.org/wiki/高德纳箭号表示法 "wikilink")-*n*运算的符號表示为(*n*-2)个箭头。

超运算可通过[递归进行定义](../Page/递归.md "wikilink")，對於所有[正整數](https://zh.wikipedia.org/wiki/正整數 "wikilink")*a*，正整數*b*和正整數*n*：
...
超运算序列是定义在[自然数集](../Page/自然数.md "wikilink")\(\mathbb{N}\)上的一个序列，记为\(H_n\)。前几项为[加法](../Page/加法.md "wikilink")（n=1）、[乘法](../Page/乘法.md "wikilink")（n=2）和[幂](https://zh.wikipedia.org/wiki/幂 "wikilink")（n=3）。高阶超运算的参数与幂运算相似，\[2\]即a称为底数，b称为指数（或称超指数\[3\]），而n则称为阶数。
...
1914年，阿尔伯特·贝内特（Albert
Bennett）最早提出了超运算，他发展出了一套交换超运算（[见下文](https://zh.wikipedia.org/wiki/#交换超运算 "wikilink")）的理论。\[4\]12年之后，[威廉·阿克曼定义了函数](https://zh.wikipedia.org/wiki/威廉·阿克曼 "wikilink")\(\phi(a, b, n)\)\[5\]，和超运算序列已经有了某种程度上的相似。最早的使用三个自变量的[阿克曼函数使用了同样的递归法则](https://zh.wikipedia.org/wiki/阿克曼函数 "wikilink")，但有两点与现在的超运算不同。一是它定义了\(n=0\)时为加法、\(n=1\)时为乘法、\(n=2\)时为幂运算，二是由其对\(\phi\)初始条件的定义能得到\(\phi(a, b, 3) = a [4] (b + 1)\)，最后的运算结果与超运算不同。\[6\]\[7\]\[8\]
...
| 名称                                                            | 符号表示                                                             | 注解                                                                              |
| ------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| [高德纳箭号表示法](https://zh.wikipedia.org/wiki/高德纳箭号表示法 "wikilink") | \(a \uparrow^{n-2} b\)                                           | [高德纳使用](../Page/高德纳.md "wikilink")（對於\(n \ge 3\)）<ref name=knuth>{{cite journal |
...
| 方括号表示法                                                        | \(a[n]b\)                                                        | 在一些在线论坛中使用，利于[ASCII表示](../Page/ASCII.md "wikilink")                             |
| [康威鏈式箭號表示法](../Page/康威鏈式箭號表示法.md "wikilink")                  | \(a \to b \to (n-2)\)                                            | [約翰·何頓·康威使用](../Page/約翰·何頓·康威.md "wikilink")（對於\(n \ge 3\)）                     |

## 一般化

[Hyperoperation_3_and_3_with_real_number.svg](https://zh.wikipedia.org/wiki/File:Hyperoperation_3_and_3_with_real_number.svg "fig:Hyperoperation_3_and_3_with_real_number.svg")的可能結果，當\(F_n(3, 3)\)的n為實數時。<sup>\[[來源請求](https://zh.wikipedia.org/wiki/Template:来源请求 "wikilink")\]</sup>\]\]
在取不同的初始条件或不同的递归法则时，就会产生不同的运算。一些数学家扩展出了超运算的许多变体。

通常，超运算等级（hyperoperation
hierarchy）\((S,\,I,\,F)\)是一个以集合\(I\)为[索引集](https://zh.wikipedia.org/wiki/索引集 "wikilink")、基于集合\(S\)的[二元运算](../Page/二元运算.md "wikilink")[族](https://zh.wikipedia.org/wiki/加标族 "wikilink")\((F_n)_{n \in I}\)。对于\(i, j, k \in I\)，有：
...
| 1   | \(F_1(a, b) = ab\)                             |                                                          |
| 2   | \(F_2(a, b) = a^b\)                            |                                                          |
| 3   | \(F_3(a, b) = a [4] (b + 1)\)                  | 类似超-4运算，但其[迭代函数比普通超](../Page/迭代函数.md "wikilink")-4运算更为复杂 |
...
1984年，C.W.克莱恩肖（C. W. Clenshaw）和F.W.J.奥立弗（F. W. J.
Olver）开始讨论如何使用超运算以防止计算机[浮点数溢出](../Page/浮点数.md "wikilink")。\[12\]此后，很多人\[13\]\[14\]\[15\]都开始对于超运算在浮点数表示中的应用产生兴趣。在探讨超-4运算时，克莱恩肖等人曾令\(F_n(a, 0) = 0\)作为初始条件，这就产生了又一个超运算等级。
...
| 4   | \(F_4(a, b) = a [4] (b - 1)\)                  | 类似超-4运算，但其[迭代函数比普通超](../Page/迭代函数.md "wikilink")-4运算更为复杂 |
...
均衡超运算于1991年首先由克莱门特·弗拉皮耶（Clément
Frappier）提出\[16\]，这种超运算是基于函数\(x^x\)的，因而与[斯坦豪斯-莫泽表示法](https://zh.wikipedia.org/wiki/斯坦豪斯-莫泽表示法 "wikilink")（Steinhaus-Moser
notation）有关。均衡超运算的递归法则是
...
令（通过°或下标）\(a_{(n+1)}b = (a_{(n+1)}(b-1))_{(n)}a\)，有初始条件\(a_{(1)}b = a+b, a _ {(2)} 0 = 0\)，且对所有\(n>2\)有
\(a _ {(n)} 0 = 1\)。

这样所产生的一个问题是，在4阶时它就与通常的定义不同：\(a_{(4)}b = a^{(a^{(b-1)})}\)。出现这一问题的原因在于加法和乘法运算有一种称为[结合律的对称性](../Page/结合律.md "wikilink")，但这在幂运算上并不成立。由于通过这种超运算所得到的结果在3阶以上都比普通的超运算更小，因而把这种超运算称为低级超运算。
...
[Category:超运算](https://zh.wikipedia.org/wiki/Category:超运算 "wikilink")

在[线性代数中](../Page/线性代数.md "wikilink")，一個\(n \times n\)的[矩陣](../Page/矩陣.md "wikilink")\(\mathbf{A}\)的**跡**（或**跡數**），是指\(\mathbf{A}\)的[主對角線](../Page/主對角線.md "wikilink")（從左上方至右下方的對角線）上各個元素的總和，一般記作\(\operatorname{tr}(\mathbf{A})\)或\(\operatorname{Sp}(\mathbf{A})\)：

\[\operatorname{tr}(\mathbf{A}) = \mathbf{A}_{1, 1} + \mathbf{A}_{2, 2} + \cdots + \mathbf{A}_{n, n}\]
其中\(\mathbf{A}_{i, j}\)代表矩陣的第**i**行**j**列上的元素的值\[1\]。一個矩陣的跡是其[特徵值的總和](../Page/特征向量.md "wikilink")（按代數重數計算）。

跡的[英文為](../Page/英文.md "wikilink")，是來自[德文中的](../Page/德文.md "wikilink")這個單字（與英文中的是同源詞），在數學中，通常簡寫為「Sp」或「tr」。
...
給定一個[環](../Page/環.md "wikilink")\(\mathbb{R}\)，跡是一個從係數在環中的\(n \times n\)矩陣的空間\(\mathcal{M}_n(\mathbb{R})\)射到環\(\mathbb{R}\)之上的[線性算子](../Page/線性算子.md "wikilink")。也就是說，對於任兩個\(n \times n\)的矩陣\(\mathbf{A}\)、\(\mathbf{B}\)和[純量](../Page/純量.md "wikilink")\(r\)，都有：

\[\mathrm{tr}(\mathbf{A} + \mathbf{B}) = \mathrm{tr}(\mathbf{A}) + \mathrm{tr}(\mathbf{B})\]

\[\mathrm{tr}(r \cdot \mathbf{A} ) = r \cdot \mathrm{tr}(\mathbf{A})\]\[2\]

更進一步來說，當\(\mathbb{R}\)是一個[域時](../Page/域.md "wikilink")，跡數函數\(\mathrm{tr}\)是\(n \times n\)矩陣的空間\(\mathcal{M}_n(\mathbb{R})\)上的一個[線性泛函](../Page/線性泛函.md "wikilink")。

由於一個矩陣\(\mathbf{A}\)的[轉置矩陣](../Page/轉置矩陣.md "wikilink")\(\mathbf{A}^T\)的主對角線元素和原來矩陣的主對角線元素是一樣的，所以任意一個矩陣和其轉置矩陣都會有相同的跡\[3\]：
...
如果\(\mathbf{A}\)和\(\mathbf{B}\)都是\(n \times n\)的[方形矩陣](../Page/方形矩陣.md "wikilink")，那麼它們的乘積\(\mathbf{AB}\)和\(\mathbf{BA}\)也會是方形矩陣。因此，利用這個結果，可以推導出：計算若干個同樣大小的方形矩陣的乘積的跡數時，可以**循環**改變乘積中方形矩陣相乘的順序，而最終的結果不變\[5\]。例如，有三個方形矩陣\(\mathbf{A}\)、\(\mathbf{B}\)和\(\mathbf{C}\)，則：
...
另外，如果\(\mathbf{A}\)、\(\mathbf{B}\)和\(\mathbf{C}\)是同樣大小的[方陣而且還是](../Page/方形矩陣.md "wikilink")**[對稱矩陣](../Page/對稱矩陣.md "wikilink")**的話，那麼其乘積的跡数不只在循環置換下不會改變，而且在所有的[置換下都不會改變](../Page/置換.md "wikilink")：
...
跡數擁有相似不變性。如果矩陣\(\mathbf{A}\)和\(\mathbf{B}\)[相似的話](../Page/相似矩陣.md "wikilink")，它們會有相同的跡。這一性質可使上面講過的循環性質來證明：
  ...
  -
    矩陣\(\mathbf{A}\)和\(\mathbf{B}\)[相似也就是說存在](../Page/相似矩陣.md "wikilink")[可逆矩陣](../Page/可逆矩陣.md "wikilink")\(\mathbf{P}\)，使得\(\mathbf{B} =\mathbf{P}\mathbf{A} \mathbf{P}^{-1}\)
...
一个\(n \times n\)的方形矩阵\(\mathbf{A}\)的[特征多项式](../Page/特征多项式.md "wikilink")\(P_{A}(\lambda)\)定义为\(\mathbf{A}\)减去\(\lambda\)倍的[单位矩阵后所得到的矩阵的](../Page/单位矩阵.md "wikilink")[行列式](../Page/行列式.md "wikilink")：
...
特征多项式是一个关于\(\lambda\)的**n**次[多项式](../Page/多项式.md "wikilink")，它的常数项是\(\mathbf{A}\)的行列式的值，最高次项是\((-1)^n \lambda^n\)，而接下来的**n-1**次项就是\((-1)^{n-1} \mathrm{tr}( \mathbf{A}) \lambda^{n-1}\)，也就是说：
...
当系数域是[代数闭域时](../Page/代数闭域.md "wikilink")（否则可以将系数域扩展到其代数闭包上来看），[特征多项式](../Page/特征多项式.md "wikilink")\(P_{A}(\lambda)\)有**n**个[根](../Page/根.md "wikilink")，它可以表达成：
...
其中的\(r_1,r_2 \cdots r_k\)是特征多项式的不同的根，而\(\alpha_1,\alpha_2 \cdots \alpha_k\)是这些根在特征多项式裡的重数，称为代数重数。显然，所有代数重数加起来等于**n**。一方面，特征多项式的根就是矩阵的[特征值](../Page/特征值.md "wikilink")，而另一方面，借由根与多项式系数的关系可以知道：特征多项式的所有的根加起来等于矩阵的迹数。所以矩阵的迹数是矩阵的所有特征值（按照代数重数计算）的和\[9\]。
...
## 參見
  ...
  - [对角矩阵](../Page/对角矩阵.md "wikilink")
  - [三角矩阵](../Page/三角矩阵.md "wikilink")
  - [特征多项式](../Page/特征多项式.md "wikilink")

...
式中 \(p:\{1,2,\dots ,N\} \to \{1,2,\dots, N\}\)
是一个对参数排序的[置换](https://zh.wikipedia.org/wiki/置换 "wikilink")，使得：
...
在[量子场论](../Page/量子场论.md "wikilink")中经常需要对算符进行时间排序，这一操作用原算符
\({\mathcal T}\) 表示。对于分别依赖于两个时空点 x 和 y 的算符 \(A(x)\) 和 \(B(y)\)
而言，\({\mathcal T}\) 的定义如下：
...
这里 \(\theta\) 表示[单位阶跃函数](../Page/单位阶跃函数.md "wikilink")，而 \(\pm\)
取决于算符是[玻色子](../Page/玻色子.md "wikilink")体系的还是[费米子](../Page/费米子.md "wikilink")体系的。对玻色子体系总是取正号。对于费米子体系取决于前述置换的奇偶性，对偶置换取正号，对奇置换取负号。

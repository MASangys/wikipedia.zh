**路由**（**routing**）就是通过互联的[网络把](https://zh.wikipedia.org/wiki/互聯網 "wikilink")[信息从源地址传输到目的地址的活动](../Page/信息.md "wikilink")。路由发生在[OSI网络参考模型中的第三层即](../Page/OSI模型.md "wikilink")[网络层](../Page/网络层.md "wikilink")。
...
在小型網路中，[距離向量路由協定十分簡單且有效率](https://zh.wikipedia.org/wiki/距離向量路由協定 "wikilink")，且只需要一些管理。然而，它們的規模性不好，且
[收斂性質也十分差](https://zh.wikipedia.org/wiki/收斂 "wikilink")，因此促進了較複雜但規模性較好的[連線狀態路由協定的開發](https://zh.wikipedia.org/wiki/連線狀態路由協定 "wikilink")，以使用在較大型的網路。距離向量路由協定也有[無限計數問題](https://zh.wikipedia.org/wiki/距離向量路由協定 "wikilink")（見[1](http://wiki.uni.lu/secan-lab/Count-To-Infinity+Problem.html)）。
...
當路由器從不同的路由協定裡發現有多個能抵達相同目的地的不同路徑時，它們使用稱為**[管理距離](https://zh.wikipedia.org/wiki/管理距離 "wikilink")**（administrative
distance）的特性來選擇最佳的路徑。管理距離定義了路由協定的可靠程度。每個路由協定按照管理距離值，由最可靠到最不可靠排列來區分優先順序。

依照路由器與其他[自治系统的關係](../Page/自治系统.md "wikilink")，有許多種類的路由協定：

  - **[Ad
    hoc-{}-網路路由協定](https://zh.wikipedia.org/wiki/Ad_hoc網路 "wikilink")**出現在沒有或一點點基礎的網路。參見[Ad
    hoc路由協定列表以獲得提議中的協定](https://zh.wikipedia.org/wiki/Ad_hoc路由協定列表 "wikilink")。

<!-- end list -->

  - **內部閘道協定**（**IGP**s）在單一的自治系統中交換路由資訊。常見的範例包括：
      - [IGRP](https://zh.wikipedia.org/wiki/IGRP "wikilink")（Interior
        Gateway Routing Protocol）
      - [EIGRP](https://zh.wikipedia.org/wiki/EIGRP "wikilink")（Enhanced
        Interior Gateway Routing Protocol）\[1\]
      - [OSPF](https://zh.wikipedia.org/wiki/OSPF "wikilink")（Open
        Shortest Path First）
      - [RIP](https://zh.wikipedia.org/wiki/路由信息協議 "wikilink")（Routing
        Information Protocol）
      - [IS-IS](https://zh.wikipedia.org/wiki/IS-IS "wikilink")（Intermediate
...
<!-- end list -->
  ...
  - **外部閘道協定**（**EGP**s）在不同的自治系統中交換路由資訊。EGP包括：
      ...
      - [BGP](https://zh.wikipedia.org/wiki/BGP "wikilink")（Border
        Gateway Protocol：大約從1995年開始使用目前的版本，BGPv4）
...
## 參見
  ...
  - [延展網路](https://zh.wikipedia.org/wiki/延展網路 "wikilink") 路由模式
      ...
      - [基於關鍵值的轉送方法](https://zh.wikipedia.org/wiki/基於關鍵值的轉送方法 "wikilink")
        (KBR)
      - [分散式物件定址及路由](https://zh.wikipedia.org/wiki/分散式物件定址及路由 "wikilink")
        (DOLR)
      - [群組任播及多播](https://zh.wikipedia.org/wiki/群組任播及多播 "wikilink")
        (CAST)
...
## 外部連結
  ...
  - [Routing and Routed
    Protocols](https://web.archive.org/web/20080720144853/http://www.networkdictionary.com/protocols/routing.php)
...
1.  在許多[思科的廣告文件中](https://zh.wikipedia.org/wiki/思科 "wikilink")，EIGRP

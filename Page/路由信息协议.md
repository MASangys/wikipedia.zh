**路由信息协议**（，缩写：**RIP**）是一种[内部网关协议](../Page/内部网关协议.md "wikilink")（IGP），為最早出現的[距離向量路由協定](https://zh.wikipedia.org/wiki/距離向量路由協定 "wikilink")。屬於[網路層](https://zh.wikipedia.org/wiki/網路層 "wikilink")，其主要应用于规模较小的、可靠性要求较低的网络，可以通过不断的交换信息让[路由器动态的适应网络连接的变化](../Page/路由器.md "wikilink")，这些信息包括每个路由器可以到达哪些网络，这些网络有多远等。
...
RIP所使用的路由[算法是](../Page/算法.md "wikilink")[Bellman-Ford算法](https://zh.wikipedia.org/wiki/Bellman-Ford "wikilink").这种算法最早被用于一个计算机网络是在1969年，当时是作为[ARPANET的初始路由算法](../Page/ARPANET.md "wikilink")。

RIP是由“网关信息协议”（[Xerox
Parc的用于互联网工作的](https://zh.wikipedia.org/wiki/Xerox_Parc "wikilink")[PARC通用数据包协议簇的一部分](https://zh.wikipedia.org/wiki/PARC通用数据包 "wikilink")）发展过来的，可以说网关信息协议是RIP的最早的版本。后来的一个版本才被命名为“路由信息协议”，是[Xerox网络服务协议簇的一部分](https://zh.wikipedia.org/wiki/Xerox网络服务 "wikilink")。
...
RIPv1使用[分类路由](../Page/分类网络.md "wikilink")，定義在《RFC 1058》中。在它的路由更新（Routing
Updates）中並不帶有[子网的資訊](https://zh.wikipedia.org/wiki/subnetwork "wikilink")，因此它無法支援[可变长度子网掩码](../Page/无类别域间路由.md "wikilink")。這個限制造成在RIPv1的網路中，在[同級網路下無法使用不同的子网掩码](https://zh.wikipedia.org/wiki/network_class "wikilink")。換句話說，在同一個網路下所有的子網路數目都是相同的。另外，RIPv1的协议报文中没有验证字段，所以RIPv1不支持验证。
...
RIPv2在RIPv1的基础上改进如下几点：
  ...
  - 支持外部路由标记（Route
    Tag），可以在路由策略中根据Tag对路由进行灵活的控制。实际上不同RIP进程间相互引入路由也可以使用Tag。 
...
RIPng（Routing Information Protocol next generation）在RFC
2080中被定义，主要是針對[IPv6做一些延伸的規範](../Page/IPv6.md "wikilink")。與RIPv2相比下其最主要的差異是：

  - RIPv2 支援RIP更新認證, RIPng
    則不支持，因为IPv6路由器理应会使用[IPsec来进行身份验证](../Page/IPsec.md "wikilink");
  - RIPv2 容許给路由器附上任何標籤， RIPng 則不容許；
  - RIPv2 在每个路由表项中都保存下一跳的信息，RIPng 是对一组路由表项指定下一跳信息；
  - RIPv2
    使用[UDP端口](https://zh.wikipedia.org/wiki/UDP "wikilink")520和多播地址224.0.0.9通信，RIPng
    则使用UDP端口521和多播地址FF02::9通信

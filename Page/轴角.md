旋转的轴角表示用两个值参数化了[旋转](../Page/旋转.md "wikilink"):
一个[轴或直线](https://zh.wikipedia.org/wiki/旋转轴 "wikilink")，和描述绕这个轴的旋转量的一个[角](https://zh.wikipedia.org/wiki/角 "wikilink")。它也叫做旋转的指数坐标。
...
假如你站在地面上，选取重力的方向为负 *z* 方向。如果你左转，你将绕 *z* 轴旋转 \(\tfrac{\pi}{2}\) 弧度 (或 90
度)。在轴角表示中，这将是
...
从旋转的轴角表示到[旋转矩阵的变换使用](../Page/旋转矩阵.md "wikilink")[指数映射](https://zh.wikipedia.org/wiki/指数映射 "wikilink")。

\[\exp\colon so(3) \to SO(3)\]

本质上说，通过使用[泰勒展开](https://zh.wikipedia.org/wiki/泰勒展开 "wikilink")，你可以得出在这两种表示之间的闭合形式的关系。给出一个轴
\(\omega \in \mathbb{R}^{3}\) 和角 \(\theta \in \mathbb{R}\)，等价的旋转矩阵给出为:
...
这里的 R 是 3x3
[旋转矩阵而](../Page/旋转矩阵.md "wikilink")**帽算子**给出与[叉积被乘数对应的](../Page/叉积.md "wikilink")[反对称矩阵算符](https://zh.wikipedia.org/wiki/反对称矩阵 "wikilink")。

### 从 SO(3) 到 so(3) 的对数映射

要获得[旋转矩阵的轴角表示](../Page/旋转矩阵.md "wikilink")，计算旋转的角

...
BGP是为了取代[外部网关协议](https://zh.wikipedia.org/wiki/外部网关协议 "wikilink")（EGP）协议而创建的，允许运行一个完全分散的路由系统，从[ARPANET](../Page/ARPANET.md "wikilink")模型的核心路由系统过渡到包括NSFNET骨干网及其相关区域网络的分散系统。这使得互联网成为一个真正的[分权制度](https://zh.wikipedia.org/wiki/分权制度 "wikilink")。自1994年以来，第四版本的BGP在互联网上使用，所有以前的版本现在已经过时不可用。在第4版主要的增强功能是通过支持[无类别域间路由](../Page/无类别域间路由.md "wikilink")和路由聚合来减少[路由表](../Page/路由表.md "wikilink")的大小。第4版是在早期的 RFC 1771 第4版的基础上编纂，通过20多个草案修改，最终在2006年1月通过形成 RFC 4271 。RFC 4271版本纠正了一些错误，澄清模糊之处，带来了更接近工业级应用标准的RFC行业惯例。

大多数[互联网服务提供商](https://zh.wikipedia.org/wiki/互联网服务提供商 "wikilink")（ISP）必须使用BGP来与其他ISP建立路由连接（尤其是当它们采取多宿主连接时）。因此，即使大多数互联网用户不直接使用它，但是与[7号信令系统](https://zh.wikipedia.org/wiki/7号信令系统 "wikilink")（SS7）相比，即通过PSTN的跨供应商核心响应设置协议，BGP仍然是互联网最重要的协议之一。特大型的私有[IP网络也可以使用BGP](../Page/网际协议.md "wikilink")。例如当需要将若干个大型的[开放最短路径优先](https://zh.wikipedia.org/wiki/开放最短路径优先 "wikilink")（OSPF）网络进行合并，而开放最短路径优先协议本身又无法提供这种可扩展性时。使用BGP的另一个原因是其能为多宿主的单个ISP（RFC 1998）或多个ISP网络提供更好的冗余网络。

## BGP操作

BGP的邻居关系（或称通信对端/对等实体）是通过人工配置实现的，对等实体之间通过TCP（端口179）会话交互数据。BGP路由器会周期地发送19字节的保持存活keep-alive消息来维护连接（默认周期为60秒）。在路由协议中，只有BGP使用TCP作为[传输层](../Page/传输层.md "wikilink")协议。

同一个AS自治系统中的两个或多个对等实体之间运行的BGP被称为IBGP（Internal/Interior BGP）。归属不同的AS的对等实体之间运行的BGP称为EBGP（External/Exterior BGP）。在AS边界上与其他AS交换信息的路由器被称作边界路由器（border/edge router）。在[互联网操作系统](https://zh.wikipedia.org/wiki/互联网操作系统 "wikilink")（Cisco IOS）中，iBGP通告的路由的距离为200，优先级比eBGP和任何[内部网关协议](../Page/内部网关协议.md "wikilink")（IGP）通告的路由都低。其他的路由器实现中，优先级顺序也是eBGP高于IGP，而IGP又高于IBGP。

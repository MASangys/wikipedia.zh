在[实分析或](https://zh.wikipedia.org/wiki/实分析 "wikilink")[数学分析中](../Page/数学分析.md "wikilink")，**达布积分**是一种定义一个函数的[积分的方法](../Page/积分.md "wikilink")，它是通过**达布和**构造的。达布积分和黎曼积分是等价的，也就是说，一个实值函数是达布可积的当且仅当它是黎曼可积的，并且积分的值相等。达布积分的定义比黎曼积分简单，并且更具操作性。达布积分的名字来自于数学家[让·加斯东·达布](https://zh.wikipedia.org/wiki/达布 "wikilink")（Jean
Gaston Darboux）。

## 区间的分割

一个[闭区间](https://zh.wikipedia.org/wiki/闭区间 "wikilink")\([a,b]\)的一个**分割**是指在此区间中取一个有限的点列\(a = x_0 < x_1 < x_2 < \ldots < x_n = b\)。每个闭区间\([x_i, x_{i+1}]\)叫做一个**子区间**。定义\(\lambda\)
为这些子区间长度的最大值：\(\lambda = \max (x_{i+1}-x_i)\)，其中\(0 \le i \le n - 1\)。

再定义**取样分割**。一个[闭区间](https://zh.wikipedia.org/wiki/闭区间 "wikilink")\([a,b]\)的一个取样分割是指在进行分割\(a = x_0 < x_1 < x_2 < \ldots < x_n = b\)后，于每一个子区间中\([x_i, x_{i+1}]\)取出一点
...
那么*F*是[利普希茨连续的](https://zh.wikipedia.org/wiki/利普希茨连续 "wikilink")。当*F*是用达布积分定义的，一个相似的结论也成立。
...
f(x) &=
 ...
 \end{cases}
\end{align}\]
由于有理数和无理数都是**R**的[稠密子集](https://zh.wikipedia.org/wiki/稠密子集 "wikilink")，因而断定*f*在任何分割的任何子区间只能取0或1。所以对于任意分割*P*我们有

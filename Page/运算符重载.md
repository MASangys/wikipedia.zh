在计算机[程序设计中](../Page/程序设计.md "wikilink")，**运算符重载**（）是[多态的一种](https://zh.wikipedia.org/wiki/多态_\(计算机科学\) "wikilink")。这里，运算符（比如`+`，`=`或`==`）被当作多态函数，它们的行为随着其参数类型的不同而不同。运算符并不一定总是符号。
...
支持运算符重载的语言：
  ...
  - [Pascal](https://zh.wikipedia.org/wiki/Pascal编程语言 "wikilink")（仅Free
    Pascal Dialect）

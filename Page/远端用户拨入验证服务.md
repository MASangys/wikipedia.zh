**遠端用户撥入驗證服務**（**RADIUS**, **Remote Authentication Dial In User Service**）是一個协议，意思就是同時兼顧[驗證](https://zh.wikipedia.org/wiki/驗證 "wikilink")（authentication）、授权（authorization）及计帐（accounting）三種服務的一种[网络传输协议](../Page/网络传输协议.md "wikilink")（protocol），通常用於網絡存取、或流動IP服務，適用於[局域網及](https://zh.wikipedia.org/wiki/局域網 "wikilink")[漫遊](../Page/漫遊.md "wikilink")服務。
...
因特网连接服务中，拨号呼叫装置和宽带接入服务器（BAS、Broadband Access Server）等接收装置（NAS、Network Access Server）即为RADIUS客户端。（虽然名字含有“服务器”，但从RADIUS协议的角度来看是客户端）。无线LAN环境中的[無線接取器和VLAN中的交换机等都是](https://zh.wikipedia.org/wiki/無線接取器 "wikilink")。在内容提供的服务中，Web 服务器起到RADIUS客户端的作用。

## 协议的概要

客户端对服务器提出“RADIUS请求包”，服务器对客户端发送“RADIUS应答包”。双方通信均由IP的[UDP包进行](../Page/用户数据报协议.md "wikilink")。

双方的数据包，头部分由20个 [8位元](../Page/8位元.md "wikilink")和“属性”组成。头部分包括类别码（Code）1个[8位元](../Page/8位元.md "wikilink")、识别码（Identifier）1个[8位元](../Page/8位元.md "wikilink")、数据包整体长度2个[8位元](../Page/8位元.md "wikilink")、验证码（Authenticator）16个[8位元](../Page/8位元.md "wikilink")。识别码根据客户端决定的需求而设定，服务器直接照抄到应答包里，因为客户端需要在收到的应答包与过去曾接收到的请求包对照。实际客户端一般只是进行累计数值编号，但没有规定必须是[序列号](https://zh.wikipedia.org/wiki/序列号 "wikilink")。验证码是为了证明无发送者伪装和篡改。属性部分是将属性値对 (Attribute Value Pair)任意次重复。属性値对由属性编号1个[8位元](../Page/8位元.md "wikilink")、长度1个[8位元](../Page/8位元.md "wikilink")、属性値组成。对于値可以赋予4个[8位元](../Page/8位元.md "wikilink")的整数値、4个[8位元](../Page/8位元.md "wikilink")的[IP地址](../Page/IP地址.md "wikilink")、1 - 253个[8位元](../Page/8位元.md "wikilink")的文字串等等。
...
协议通过属性値对里含有各种个样信息进行验证和计费。为实现验证，用户名和密码各有属性编号。拨号上网使用[PPP时针对](https://zh.wikipedia.org/wiki/Point-to-Point_Protocol "wikilink") PPP 用的验证协议 [PAP](https://zh.wikipedia.org/wiki/密碼認證協議 "wikilink")、[CHAP](https://zh.wikipedia.org/wiki/CHAP "wikilink")、[EAP均备有各自的属性编号](https://zh.wikipedia.org/wiki/PPP_Extensible_Authentication_Protocol "wikilink")。为实现计费，备有使用秒数、收发数据量等的属性编号。由此根据属性编号可以判断、验证和计费两功能是只有一种才能使用还是二者可同时使用。

RADIUS分组的最大长度，在RADIUS验证协议中是4096个8位元、RADIUS计费协议中是4095个8位元，这个差异据说并没有特殊含义，而是 RFC 当初的错字将错就错地沿用下来的缘故。

## AAA

RADIUS协议是一种基于的协议，但由于RADIUS协议本身创建于 AAA 模式之前，因此，RADIUS协议中并没有区分“验证”和“授权”，而是合为“验证”处理， 因此RADIUS客户端在实际操作中无法知道被拒绝的原因是由于密码错误还是因为没有权限。

## 共有鍵

UDP与[TCP不同](https://zh.wikipedia.org/wiki/TCP "wikilink")，无法识别伪装送信者和数据篡改。因此仅靠通信对方的IP地址是不足以信任通信内容的。为了防止伪装和篡改，RADIUS客户端和服务器之间共享一个叫“共享密码” (Shared secret)的密钥字符串，将数据包的内容和共享密码得到的[摘要信息配给验证符号和属性值](https://zh.wikipedia.org/wiki/散列函数 "wikilink")。应该为每对 RADIUS客户端和服务器准备一个共享密码。针对一个RADIUS服务器只用一个密码而与所有RADIUS客户端都共用会造成安全上的重大隐患。另外，从安全的角度来说，共享密码的内容若被第三方泄露也是一大问题\[1\]。
...
EAP-TLS 对于基于[TLS数字证书进行相互验证](https://zh.wikipedia.org/wiki/TLS "wikilink")（为防止伪装服务提供者）这一点虽然很重要，但数字证书的管理和运用对于一般机构是一个很大的负担。PEAP 和 EAP-TTLS 的验证步骤是在创建以 TLS [加密的通讯线路后再进行密码信息的交换](https://zh.wikipedia.org/wiki/加密 "wikilink") 。EAP-TLS 与 PEAP・EAP-TTLS 的对比可以参照比较是通过[网页浏览器](../Page/网页浏览器.md "wikilink")的[TLS中利用数字证书进行相互验证还是](https://zh.wikipedia.org/wiki/TLS "wikilink") SSL 上的密码验证。

## 软件

RADIUS协议的开发者Livingston Enterprises 公司（后被 Lucent Technologies 公司收购）进行的RADIUS服务器的搭载以及之后派生出来的使用逐渐增多。近年出现里更多的[开源软件](../Page/开源软件.md "wikilink")、商用软件。

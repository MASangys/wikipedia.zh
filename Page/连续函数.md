...
举例来说，考虑描述一棵树的[高度随](https://zh.wikipedia.org/wiki/高度 "wikilink")[时间而变化的函数](../Page/时间.md "wikilink")\(h(t)\)，那么这个函数是连续的（除非树被砍断）。又例如，假设\(T(P)\)表示地球上某一点\(P\)的空气温度，则这个函数也是连续的。事实上，[古典物理学中有一句格言](https://zh.wikipedia.org/wiki/古典物理学 "wikilink")：“自然界中，一切都是连续的。”相比之下，如果\(M(t)\)表述在时间t的时候银行账户上的钱币金额，则这个函数无论在存钱或者取钱的时候都会有跳跃，因此函数\(M(t)\)是不连续的。

## 实值连续函数

最基本也是最常见的连续函数是[定义域为](../Page/定义域.md "wikilink")[实数集的某个子集](../Page/实数.md "wikilink")、取值也是[实数的连续函数](../Page/实数.md "wikilink")。例如前面提到的花的高度，就是属于这一类型。这类函数的连续性可以用[直角坐标系中的](../Page/笛卡尔坐标系.md "wikilink")[图像来表示](https://zh.wikipedia.org/wiki/函数图像 "wikilink")。一个这样的函数是连续的，如果粗略地说，它的图像为一个单一的不破的曲线，并且没有**间断**、**跳跃**或**无限逼近的振荡**。
...
### 例子
  ...
  - 所有[多项式函数都是连续的](https://zh.wikipedia.org/wiki/多项式 "wikilink")。各类[初等函数](../Page/初等函数.md "wikilink")，如[指数函数](../Page/指数函数.md "wikilink")、[对数函数](https://zh.wikipedia.org/wiki/对数函数 "wikilink")、[平方根函数与](https://zh.wikipedia.org/wiki/开平方 "wikilink")[三角函数在它们的定义域上也是连续的函数](../Page/三角函数.md "wikilink")。
...
如果一个函数在定义域中的某个点\(f ( c )\)[可微](https://zh.wikipedia.org/wiki/可微 "wikilink")，则它一定在点\(c\)连续。反过来不成立；连续的函数不一定可微。例如，[绝对值函数在点](../Page/绝对值.md "wikilink")\(c=0\)连续，但不可微。
...
这个定义可以用[序列与](../Page/序列.md "wikilink")[极限的语言重述](https://zh.wikipedia.org/wiki/极限 "wikilink")：
...
如上连续函数的定义可以自然地推广到一个[拓扑空间到另一拓扑空间的函数](../Page/拓扑空间.md "wikilink")：函数\(f: X\rightarrow Y\)，这里\(X\)与\(Y\)是拓扑空间是连续的[当且仅当任何](../Page/当且仅当.md "wikilink")[开集](../Page/开集.md "wikilink")\(V\subseteq Y\)的[逆像](https://zh.wikipedia.org/wiki/逆像 "wikilink")\(f^{-1}(V)\)是\(X\)中开集。
...
## 参考文献
  ...
  - [Visual Calculus](http://archives.math.utk.edu/visual.calculus/) by
    ...
    Tennessee](https://zh.wikipedia.org/wiki/University_of_Tennessee "wikilink")
    (2001)

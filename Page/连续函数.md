...
如上连续函数的定义可以自然地推广到一个[拓扑空间](../Page/拓扑空间.md "wikilink")到另一拓扑空间的函数：函数\(f: X\rightarrow Y\)，这里\(X\)与\(Y\)是拓扑空间是连续的[当且仅当](../Page/当且仅当.md "wikilink")任何[开集](../Page/开集.md "wikilink")\(V\subseteq Y\)的[逆像](https://zh.wikipedia.org/wiki/逆像 "wikilink")\(f^{-1}(V)\)是\(X\)中开集。

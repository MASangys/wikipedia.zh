**连通分量标记**（或者称**连通分量分析**,**连通区域标记**）是[图论应用中的一种算法](../Page/图论.md "wikilink")，给二值图像中的每个连通区域标上一个特定的标号。该算法可用来对图像的目标进行定位和计数。该算法不要和[图像分割相混淆](../Page/图像分割.md "wikilink")。

连通分量标记通常在计算机视觉领域中对二值图像的连通区域进行检测，也可以处理彩色图像和更高维的数据。\[1\]\[2\]当将其集成到[图像识别系统或者是](https://zh.wikipedia.org/wiki/图像识别 "wikilink")[人机交互系统是](../Page/人机交互.md "wikilink")，该算法也起到重要作用。\[3\]\[4\]

## 概述

[Square_4_connectivity.png](https://zh.wikipedia.org/wiki/File:Square_4_connectivity.png "fig:Square_4_connectivity.png")
[Square_8_connectivity.png](https://zh.wikipedia.org/wiki/File:Square_8_connectivity.png "fig:Square_8_connectivity.png")
...
以一个8领域的例子来说明两次扫描算法。 该算法的数据结构为[并查集](../Page/并查集.md "wikilink")。 1，第一次扫描：

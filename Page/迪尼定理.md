在数学中，**迪尼定理**叙述如下：设 *X*
是一个[紧致的](https://zh.wikipedia.org/wiki/紧致 "wikilink")[拓扑空间](../Page/拓扑空间.md "wikilink")，
*f*(*n*) 是 *X*
上的一个[单调递增的](https://zh.wikipedia.org/wiki/单调递增 "wikilink")[连续实值](https://zh.wikipedia.org/wiki/连续 "wikilink")[函数列](../Page/函数.md "wikilink")（即使得对任意
*n* 和 *X* 中的任意 *x*
都有\(\scriptstyle f_n(x) \leq f_{n+1}(x)\)）。如果这个函数列[逐点收敛到一个连续的函数](https://zh.wikipedia.org/wiki/逐点收敛 "wikilink")
*f* ，那么这个函数列[一致收敛到](https://zh.wikipedia.org/wiki/一致收敛 "wikilink") *f*
...
时*f*(*x*) 等于 0 ，*f*(*1*) 等于
1。但这个函数列不是[一致收敛的](https://zh.wikipedia.org/wiki/一致收敛 "wikilink")，因为
*f* 不连续。
...
[逐点收敛到](https://zh.wikipedia.org/wiki/逐点收敛 "wikilink") *f*
，所有（\(E_n\)） 的[并集是](../Page/并集.md "wikilink") *X*
的一个[开覆盖](https://zh.wikipedia.org/wiki/开覆盖 "wikilink")。但是
*X* 是一个[紧集于是存在正整数](https://zh.wikipedia.org/wiki/紧集 "wikilink") *N*
使得\(E_N = X\)。因此对所有 \(n > N\)，对所有的 \(x \in X\)，都有
\(0< g_n(x) = f(x) - f_n(x) < \varepsilon\)，于是{\(f_n\)}
[一致收敛于](https://zh.wikipedia.org/wiki/一致收敛 "wikilink") *f* 。

**迭代**是重复[反馈过程的活动](../Page/反馈.md "wikilink")，其目的通常是为了接近并到达所需的目标或结果。每一次对过程的重复被称为一次“迭代”，而每一次迭代得到的结果会被用来作为下一次迭代的初始值。
...
数学中的**迭代**可以指[函数迭代的过程](../Page/迭代函数.md "wikilink")，即反复地运用同一[函数计算](../Page/函数.md "wikilink")，前一次迭代得到的结果被用于作为下一次迭代的输入。即使是看上去很简单的函数，在经过迭代之后也可能产生复杂的行为，衍生出具有难度的问题。这样的例子可以参见[考拉兹猜想和](../Page/考拉兹猜想.md "wikilink")[杂耍者序列](https://zh.wikipedia.org/wiki/杂耍者序列 "wikilink")（Juggler
sequence）。又如一个简单的二次变换x→x(1-x)，它的迭代将形成一个具有[混沌性质的](../Page/混沌.md "wikilink")[动力系统](../Page/动力系统.md "wikilink")。

迭代在数学中的另一应用是[迭代法](https://zh.wikipedia.org/wiki/迭代法 "wikilink")，用来对特定数学问题作数值解估计。[牛顿法就是迭代法的一个例子](../Page/牛顿法.md "wikilink")。
...
在第一种意义下，[递归是](../Page/递归.md "wikilink")*迭代*的一个例子，但是通常使用一种*递归式的表达*。比如用0\!=1，n\!=n\*(n-1)\!来表示[阶乘](https://zh.wikipedia.org/wiki/阶乘 "wikilink")。而*迭代*通常不是这样写的。
...
下述例子用[Scheme语言写成](../Page/Scheme.md "wikilink")。注意它是一个递归（迭代的特例），因为函数iter在解决问题时调用了自身。特别地，它使用了[尾部递归](https://zh.wikipedia.org/wiki/尾部递归 "wikilink")，一种能被Scheme这样的编程语言完备支持的技巧，因此程序不会占用大量[堆栈](../Page/堆栈.md "wikilink")。

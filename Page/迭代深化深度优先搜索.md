**迭代深化深度优先搜索** （iterative deepening depth-first search (IDS or
IDDFS))）是对状态空间的搜索策略。它重复地运行一个有深度限制的[深度优先搜索](../Page/深度优先搜索.md "wikilink")，每次运行结束后，它增加深度并迭代，直到找到目标状态。

IDDFS 与[广度优先搜索有同样的时间复杂度](../Page/广度优先搜索.md "wikilink")，而空间复杂度远优。

[Normdist_regression.png](https://zh.wikipedia.org/wiki/File:Normdist_regression.png "fig:Normdist_regression.png") **迴歸分析**（）是一種[統計學上分析數據的方法](https://zh.wikipedia.org/wiki/統計學 "wikilink")，目的在於了解兩個或多個變數間是否相關、相關方向與強度，並建立數學模型以便觀察特定變數來預測研究者感興趣的變數。更具体的来说，回归分析可以帮助人们了解在只有一个自变量变化时因变量的变化量。一般来说，通过回归分析我们可以由给出的自变量估计應变量的条件期望。

...
设A为一个有n个[数字的](https://zh.wikipedia.org/wiki/数字 "wikilink")[有序集](https://zh.wikipedia.org/wiki/有序集 "wikilink")（n\>1），其中所有数字各不相同。

如果存在正整數i, j使得1 ≤ i ＜ j ≤ n而且A\[i\] ＞ A\[j\]，則\<A\[i\],
A\[j\]\>這一個[有序對稱為A的一個](https://zh.wikipedia.org/wiki/有序對 "wikilink")**逆序對**，也称作逆序。逆序對的數量称作逆序数。
...
設\(\pi\)為一個[排列](https://zh.wikipedia.org/wiki/排列 "wikilink")，如果\(i < j\)而且\(\pi(i) > \pi(j)\)，

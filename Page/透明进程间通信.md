...
Communication，缩写为TIPC）是一种用于[进程间通信的网络](https://zh.wikipedia.org/wiki/inter-process_communication_\(IPC\) "wikilink")[通信协议](https://zh.wikipedia.org/wiki/通信协议 "wikilink")，原本是为集群间通信特别设计的。它允许设计人员能够创建可以和其它应用快速可靠地通信应用，无须考虑在其它需要通信的应用在集群环境中的位置。
...
TIPC的一些特点：
  ...
  - [无链接传输](https://zh.wikipedia.org/wiki/Connectionless_mode_transmission "wikilink")，[面向链接传输以及](https://zh.wikipedia.org/wiki/面向链接传输 "wikilink")[多播消息机制](../Page/多播.md "wikilink")。
  - 网络事件订阅。
...
TIPC 协议从[Linux
kernel版本](https://zh.wikipedia.org/wiki/Linux_kernel "wikilink")2.6.16开始已经在内核支持，也可以作为模块置于之前版本的内核。其他[操作系统中也有支持](../Page/操作系统.md "wikilink")，包括Wind
...
一个TIPC节点在所有配置的接口上定期[广播](https://zh.wikipedia.org/wiki/Broadcasting_\(computing\) "wikilink")*Link
Request*消息，以发现相邻的集群节点。如果一个以前没有建立过链接的节点收到这样的消息，它会返回一个[单播的](https://zh.wikipedia.org/wiki/单播 "wikilink")*Link
...
TIPC协议最初由Jon Paul
Maloy在[Ericsson开发](https://zh.wikipedia.org/wiki/Ericsson "wikilink")，在逐步分发到[开源社区之前用于该公司的电信级](https://zh.wikipedia.org/wiki/开源 "wikilink")[集群应用已有多年](https://zh.wikipedia.org/wiki/计算机集群 "wikilink")。

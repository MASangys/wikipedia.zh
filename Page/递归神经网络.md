...
其中，计算隐藏状态的方程\(f(x, h)\)是一个[非线性方程](https://zh.wikipedia.org/wiki/非线性 "wikilink")，可以是简单的[Logistic方程](https://zh.wikipedia.org/wiki/逻辑函数 "wikilink")（tanh），也可以是复杂的[LSTM单元](https://zh.wikipedia.org/wiki/LSTM "wikilink")（Long
Short-Term Memory）。\[5\] \[6\] 而有了隐藏状态序列，就可以对下一个出现的词语进行预测：

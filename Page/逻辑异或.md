...
在-{zh-hant:數位;zh-hans:数字}-邏輯中，[逻辑算符](../Page/逻辑运算符.md "wikilink")**互斥或閘**（**exclusive or**）是对两个[运算元的一种](https://zh.wikipedia.org/wiki/运算元 "wikilink")[邏輯分析类型](../Page/逻辑或.md "wikilink")。与一般的[邏輯或OR不同](https://zh.wikipedia.org/wiki/邏輯或 "wikilink")，當兩兩數值相同為否，而數值不同時為真。

...
unions）都會被映射到[高阶函数](https://zh.wikipedia.org/wiki/高阶函数 "wikilink")。
在[無型別lambda演算](https://zh.wikipedia.org/wiki/lambda_演算 "wikilink")，函數是唯一的[原始型別](../Page/原始型別.md "wikilink")。

邱奇編碼本身並非用來實踐原始型別，而是透過它來展現我們不須額外原始型別即可表達計算。

很多学数学的学生熟悉[可计算函数集合的](../Page/可计算函数.md "wikilink")[哥德尔编号](https://zh.wikipedia.org/wiki/哥德尔编号 "wikilink")；邱奇编码是定义在[lambda抽象而不是自然数上的等价运算](../Page/Λ演算.md "wikilink")。

## 邱奇数

**邱奇数**為使用邱奇编码的[自然数表示法](../Page/自然数.md "wikilink")，而用以表示自然数\(n\)的[高阶函数是個任意函数](https://zh.wikipedia.org/wiki/高阶函数 "wikilink")\(f\)映射到它自身的*n*重[函数复合之函数](https://zh.wikipedia.org/wiki/函数复合 "wikilink")，簡言之，數的「值」即等價於參數被函數包裹的次數。
...
就是说，自然数\(n\)被表示为邱奇数**n**，它对于任何lambda-项`F`和`X`有着性质：
  ...
  -
    **n** `F X`
    [=<sub>β</sub>](https://zh.wikipedia.org/wiki/Lambda演算#β-归约 "wikilink")
    `F`<sup>*`n`*</sup>`  X `。
...
后继函数 \(\text{succ}(n)=n+1\)
[β-等价于](https://zh.wikipedia.org/wiki/beta归约 "wikilink")（**plus**
**1**）。

...
在[图论和](../Page/图论.md "wikilink")[计算机科学中](../Page/计算机科学.md "wikilink")，**邻接表**是表示了[图中与每一个顶点相邻的边集的](../Page/图_\(数学\).md "wikilink")[集合](../Page/集合_\(数学\).md "wikilink")，这里的集合指的是无序集。

如果是无向图，那么每条边由两个结点组成，分别代表边的两个端点；如果是有向图，那么每条边是一个[结点对](../Page/结点对.md "wikilink")，分别代表边的始点和终点。
...
在[计算机科学中](../Page/计算机科学.md "wikilink")，邻接表描述一种紧密相关的[数据结构](../Page/数据结构.md "wikilink")，用于表征图。在邻接表的表示中，对于图中的每个顶点，我们将保存所有其它与之相连的顶点（即“邻接表”）。例如，由[吉多·范罗苏姆提出的](../Page/吉多·范罗苏姆.md "wikilink")，使用[哈希表将每个顶点和该顶点的邻接点](../Page/哈希表.md "wikilink")[数组关连起来](../Page/数组.md "wikilink")，就可以看作是上述表示方法的一种实现。又如，在Cormenetal中，顶点数组的每个元素都指向一个邻接点[单链表](../Page/单链表.md "wikilink")。

邻接表结构的困难之一是无法明确在什么地方保存相关边的长度或花销。为了解决这个问题，一些算法，如 Goodrich and
Tamassia所提出的[面向对象邻接表](../Page/面向对象.md "wikilink")，有时也称**关联度**，它为每个顶点保存一个对象表，每个对象表示指向顶点的那条边的关联度。为了完善这个结构，每条边必须指向两个组成其端点的顶点。这个额外的边对象使得它比直接列出所有边的邻接表消耗更多的内存，但它不失为一种保存边相关信息的好方法。
...
可用于替代邻接表的主要有[邻接矩阵](../Page/邻接矩阵.md "wikilink")。用[稀疏邻接矩阵表示邻接表时](../Page/稀疏矩阵.md "wikilink")，将占用更少的空间。这是因为它能避免为*不存在*的边分配任何空间。在一台32位计算机上，如果使用原始的[数组结构实现邻接表](../Page/数组.md "wikilink")，那么对于一个无向图来说，它大约需要占用\(8e\)字节的存储空间，其中\(e\)表示边的个数。每条边都将会在两个邻接表中重复出现，并分别占用4字节空间。

相反地，由于邻接矩阵中的每个元素仅占用一位（bit），故可以以非常紧密的方式来存储，仅占用\(n^2/8\)个字节，其中\(n\)代表顶点个数。除了节省空间外，这种紧密存储也发扬了[locality
of reference](../Page/locality_of_reference.md "wikilink")。

注意到一个图至多能有\(n^2\)条边（允许循环）。令\(d=e/n^2\)表示图的*[致密度](../Page/图致密度.md "wikilink")*，则由\(8e > n^2/8\)，可知邻接表将占用更多的空间，准确地说，仅当\(d > 1/64\)'以说只有当图比较稀疏的时候，才有可能以较少的空间来存储邻接表。不过，以上分析只有当我们仅考虑边的[连接性](../Page/连接性.md "wikilink")，而不考虑关于边的任何数值信息时才有效。

除了空间方面的考虑外，不同的数据结构也使得不同的操作变得更容易。在一个邻接表中，给定一个顶点，我们能很容易地找出它的所有邻边，因为只需要读取它的邻接表就可以了。在一个邻接矩阵中，相同的操作则需要扫描一行，花费大约\(O(n)\)（[大O标记](../Page/大O标记.md "wikilink")）时间。而如果你想知道给定的两个顶点间是否存在有边，在邻接矩阵里可以立刻查到，在邻接表中则需要花费以边的最小关联度成比例的时间。
...
[Category:图数据结构](https://zh.wikipedia.org/wiki/Category:图数据结构 "wikilink")
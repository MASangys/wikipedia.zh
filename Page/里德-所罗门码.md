**里德-所罗门码**（，簡稱**里所码**或
）是一种[前向錯誤更正](../Page/前向錯誤更正.md "wikilink")的[信道编码](https://zh.wikipedia.org/wiki/信道编码 "wikilink")，对由校正[过采样数据所产生的有效](https://zh.wikipedia.org/wiki/过采样 "wikilink")[多项式](https://zh.wikipedia.org/wiki/多项式 "wikilink")。编码过程首先在多个点上对这些多项式求冗余，然后将其传输或者存储。对多项式的这种超出必要值的采样使得多项式超定（过限定）。当接收器正确地收到足够的点后，它就可以恢复原来的多项式，即使接收到的多项式上有很多点被噪声干扰失真。
...
这两种定义的等价性可以通过[有限域](../Page/有限域.md "wikilink")上的[离散傅立叶变换来证明](https://zh.wikipedia.org/wiki/离散傅立叶变换 "wikilink")。[离散傅立叶变换建立起了多项式的系数与值指间的对偶关系](https://zh.wikipedia.org/wiki/离散傅立叶变换 "wikilink")。这种对偶关系可以不严格的概述如下：令\(p(x)\)和\(q(x)\)为两个次数小于\(n\)的多项式。如果多项式

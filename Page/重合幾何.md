...
重合結構 包含一個其元素被稱為「點」的集合 P、一個其元素被稱為「線」的不相交集合 L，以及兩個集合間的「重合關係」 I，即 P × L 的子集（其元素被稱為「標記」）\[2\]。若 (A, l) 是一個標記，則稱 A 重合於 l 或 l 重合於 A（此一關係具對稱性），且寫作　A I l。直上，一個點與一條線在此一關係內，若且唯若該點位於該線上。給定一個點 B 與一條線 m，使其不組成一個標記，亦該點不位於該線上，則 (B, m) 被稱為「非標記」。

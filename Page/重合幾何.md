...
重合結構 包含一個其元素被稱為「點」的集合 P、一個其元素被稱為「線」的不相交集合 L，以及兩個集合間的「重合關係」 I，即 P × L
的子集（其元素被稱為「標記」）\[2\]。若 (A, l) 是一個標記，則稱 A 重合於 l 或 l 重合於
A（此一關係具對稱性），且寫作　A I
l。直上，一個點與一條線在此一關係內，若且唯若該點位於該線上。給定一個點
B 與一條線 m，使其不組成一個標記，亦該點不位於該線上，則 (B, m) 被稱為「非標記」。
...
有限部分線性空間若滿足正則條件，且 有限部分線性空間若滿足正則條件，且 k, r \> 1，則稱之為「策略配置」（tactical
configuration）\[4\]。一些作者會簡單稱之為[配置](https://zh.wikipedia.org/wiki/配置_\(幾何\) "wikilink")\[5\]，或「投影配置」\[6\]。若一策略配置有
n 個線與 m 條線，則透過重復計算標記，可建立 nr = mk 此一關係式，通常標記為  配置。在 n = m （因此　r = k）時，　
通常簡寫為 。
...
這被用來排除一些非常小的例子（主要是集合 P 或 L
內少於2個元素之情形），這些例子通常會成為與重合結構有關之一般陳述的例外。另一種附加公理的方式為，將不符合公理的重合結構稱為「平凡」的；符合的則稱為「非平凡」的。
...
在投影平面裡，點 P 與線 L 間存在著[對射](https://zh.wikipedia.org/wiki/對射 "wikilink")。若
P 為一有限集合，該投影平面稱之為「有限」投影平面。有限投影平面的階為 n = k -
1，即線上的點之數量減一。所有已知的投影平面均有[質數冪次的階](https://zh.wikipedia.org/wiki/質數冪次 "wikilink")。n
階投影平面為  配置。
...
此一著名的重合幾何係由義大利數學家[基諾·法諾研究而得](../Page/基諾·法諾.md "wikilink")\[9\]。在證明 n
維[投影空間公理之獨立性的過程中](https://zh.wikipedia.org/wiki/投影空間 "wikilink")\[10\]，法諾發現了一個具15個點、35條線及15個平面的有限三維空間，其中每條線上有三個點\[11\]。在這個空間裡的平面包含7個點與7條線，並被稱為[法諾平面](https://zh.wikipedia.org/wiki/法諾平面 "wikilink")。

法諾平面無法於[歐氏平面內只使用點與直線段來表示](https://zh.wikipedia.org/wiki/二維空間 "wikilink")。這是[西爾維斯特-加萊定理的結論](../Page/西爾維斯特-加萊定理.md "wikilink")。
...
「仿射平面」是一個線性空間，使得：
  ...
  - 對任一點 A 與不與該點重合的線 l（非標記）恰有一條與 A 重合（即　A I m），但不與 l 相交的線
    m（稱之為[普萊費爾公理](https://zh.wikipedia.org/wiki/普萊費爾公理 "wikilink")）。

並滿足非退化條件：

  - 存在一個三角形，即三個非[共線的點](../Page/共線_\(幾何\).md "wikilink")。

在普萊費爾公理裡所述之線　l 與 m 被稱為是平行的。每個仿射平面均可唯一地被擴展成投影平面。有限仿射平面的「階」為 k，即一條線上點的數量。n
階仿射平面為  配置。
[Hesse_configuration.svg](https://zh.wikipedia.org/wiki/File:Hesse_configuration.svg "fig:Hesse_configuration.svg")

#### 黑塞配置

3階仿射平面為
配置。當嵌入一些周圍的空間時，稱之為**[黑塞配置](https://zh.wikipedia.org/wiki/黑塞配置 "wikilink")**。黑塞配置不可能在歐氏平面裡實現，但可於[複投影平面裡實現](https://zh.wikipedia.org/wiki/複投影平面 "wikilink")，有9個[橢圓曲線的](https://zh.wikipedia.org/wiki/橢圓曲線 "wikilink")[反曲點及](https://zh.wikipedia.org/wiki/反曲點 "wikilink")12條線，且每條線各與3個點重合。

這12條線可以分成4類，每類3條線。在各類中，線互不相交。這些類被稱為線的「平行類」。加上4個新的點，於各個平行類中的所有線上（所以現在所有線都相交）；以及一條新的線，只包括這4個新的點，即可形成3階投影平面，具
 配置。相反地，從（唯一一種）3階投影平面開始，移除任意一條線，以及所有在該線上的點，即可形成一個（唯一一種）3階仿射平面。

移除一個點並通過該點的4條線（但不包括其他在這些線上的點）會形成
[莫比烏斯-坎特配置](https://zh.wikipedia.org/wiki/莫比烏斯-坎特配置 "wikilink")。

## 部分幾何

[GQ(2,2),_the_Doily.svg](https://zh.wikipedia.org/wiki/File:GQ\(2,2\),_the_Doily.svg "fig:GQ(2,2),_the_Doily.svg")

給定一整數 α ≥ 1，策略配置若滿足： 對每個非標記 (B, m)，存在 α 個標記，使得 B I l 與 A I m，
則稱之為「部分幾何」。若一條線上有 s+1 個點，且一個點上有 t+1 條線，則該部分幾何標記為
pg(s, t, α)。

若 α = 1，該部分幾何為[廣義四邊形](https://zh.wikipedia.org/wiki/廣義四邊形 "wikilink")。

若 α = s +
1，該部分幾何為[斯坦納系統](https://zh.wikipedia.org/wiki/斯坦納系統 "wikilink")。

## 廣義多邊形

對 n \> 2,\[12\]，廣義 n 邊形是一個部分線性空間，其重合圖 Γ 具下列性質： Γ
的[周長](https://zh.wikipedia.org/wiki/周長_\(圖論\) "wikilink")（最短[環的長度](https://zh.wikipedia.org/wiki/環_\(圖論\) "wikilink")）是　Γ　的[直徑](https://zh.wikipedia.org/wiki/直徑_\(圖論\) "wikilink")（兩個頂點間最長的距離，在此為　n）的兩倍。

「廣義2邊形」是一個重合結構，但不是部分線性空間，包括至少2個點與2條均與每個點重合的線。廣義2邊形的重合圖為一完整二分圖。

廣義 n 邊形不包含[一般 m 邊形](https://zh.wikipedia.org/wiki/多邊形 "wikilink")，其中 2 ≤
m \< n；且對每一對物件（兩個點、兩條線或一個點與一條線），總存在一包含這兩個物件的一般 n 邊形。

廣義3邊形為投影平面，廣義4邊形稱為[廣義四邊形](https://zh.wikipedia.org/wiki/廣義四邊形 "wikilink")。由范特-希格曼定理可知，具有每條線至少3個點與每個點至少3條線的有限廣義
n 邊形只有 n = 2、3、4、6 或 8 時的廣義多邊形。
...
近 0 邊形為一個點，近 2 邊形為一條線。近 2 邊形的共線圖為一[完全圖](../Page/完全圖.md "wikilink")。近 4
邊形為一（可能退化的）廣義四邊形。每個廣義多邊形都是個近多邊形。任何連通二分圖均是近多邊形，且任一每條線上恰有2個點的近多邊形也都是連通二分圖。此外，所有的[對偶極空間都是近多邊形](https://zh.wikipedia.org/wiki/極空間 "wikilink")。
...
具體來說，莫比烏斯平面是一個點與環的重合結結，使得：
  ...
  - 每三個不同點恰與一個環重合。
  - For any flag  and any point  not incident with  there is a unique
    cycle  with  and }. (The cycles are said to *touch* at .)
  - 對任一標記　(P, z) 與任一不重合於 z 的點 Q，存在唯一個環 ，使得 、，及 }（即這兩個環相交於　P）。
  - 每個環有至少3個點，且至少存在一個環。

對莫比烏斯平面上的任一點 P，取 P 以外的其他所有點為點，以及僅包括 P
的環（並移除　P）為線，所得出之重合結構為一仿射平面。此一結構在設計理論中稱之為在　P
的「剩餘」。

m 階有限莫比烏斯平面具一策略配置，使得每個為
[3-設計](https://zh.wikipedia.org/wiki/區塊設計 "wikilink")（具體來說，為　
區塊設計）的環有  個點，
...
西爾維斯特-加萊定理是[詹姆斯·約瑟夫·西爾維斯特提出一個與歐氏平面裡有限點集合之重合關係有關的問題](../Page/詹姆斯·約瑟夫·西爾維斯特.md "wikilink")，並由[蒂博爾·由加萊提出解答](https://zh.wikipedia.org/wiki/蒂博爾·由加萊 "wikilink")。
...
相關的結論為迪布恩-艾狄胥定理。[尼古拉斯·霍弗特·迪布恩與](https://zh.wikipedia.org/wiki/尼古拉斯·霍弗特·迪布恩 "wikilink")[保羅·艾狄胥於更一般設定的投影平面上證明出此一結論](https://zh.wikipedia.org/wiki/保羅·艾狄胥 "wikilink")，但於歐氏平面中亦仍然成立。該定理為\[13\]：
  ...
  -
      ...
      -
        於一[投影平面上](https://zh.wikipedia.org/wiki/投影平面 "wikilink")，每組非[共線的](../Page/共線_\(幾何\).md "wikilink")　n
        個點，可至少決定 n 條不同的線。
...
## 外部連結
  ...
  - [*incidence
    system*](http://www.encyclopediaofmath.org/index.php/Incidence_system)
    at the [Encyclopedia of
    Mathematics](https://zh.wikipedia.org/wiki/數學百科全書 "wikilink")
...
13. [Weisstein, Eric
    W.](http://mathworld.wolfram.com/about/author.html), ["de
    Bruijn–Erdős
    Theorem"](http://mathworld.wolfram.com/deBruijn-ErdosTheorem.html)

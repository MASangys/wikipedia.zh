在计算机领域，**重定向**是大多数命令行解释器所具有的功能，包括各种可以将标准流重定向用户规定地点的[Unix](../Page/Unix.md "wikilink")
shells。类[Unix操作系统的程序可以透過dup](../Page/Unix.md "wikilink")2(2)[系统调用完成重定向](../Page/系统调用.md "wikilink")，或者透過缺少一些灵活性但是更高一级层次的freopen(3)和popen(3)来完成。
...
一个使用命令管道的很好例子是使用[echo和另外一个命令达到在一个非交互式shell中达到一定的交互效果](../Page/echo.md "wikilink")：

``` bash
echo -e "user\npass" | ftp localhost
```

这个例子运行[ftp客户端](../Page/ftp.md "wikilink")，然后输入用户名，回车，然后再输入密码。

## 标准文件句柄的重定向

源自[Bourne shell的许多](../Page/Bourne_shell.md "wikilink")[Unix
shell](../Page/Unix_shell.md "wikilink")，可以将一个数字（文件描述符）放在重定向符号前，这样可以影响用于重定向的数据流。[Unix的标准输入输出流是](../Page/Unix.md "wikilink"):

| Handle | Name                                   | Description |
| ------ | -------------------------------------- | ----------- |
| 0      | [stdin](../Page/stdin.md "wikilink")   | 标准输入        |
| 1      | [stdout](../Page/stdout.md "wikilink") | 标准输出        |
| 2      | [stderr](../Page/stderr.md "wikilink") | 标准错误输出      |
...
上面这个命令将当前目录的内容列出来，将其作为[grep命令的输入内容](../Page/grep.md "wikilink")，[grep将以没有以](../Page/grep.md "wikilink")".sh"结尾的内容过滤掉，然后将内容输出给[sort命令作为输入参数](../Page/sort.md "wikilink")，[sort将输入内容按照字符的顺序排序以后](../Page/sort.md "wikilink")，将最终的输入内容写入shlis文件。在[Unix和](../Page/Unix.md "wikilink")[Linux操作系统中](../Page/Linux.md "wikilink")，这个样的组合命令非常的常见。

## 重定向到多个输出

命令[tee可以将一个命令的输出重定向到几个目标](../Page/tee.md "wikilink"):
...
## 参见
  ...
  - [Here-document](../Page/Here-document.md "wikilink"), a way of
    specifying text for input in command line shells

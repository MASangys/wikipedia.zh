...
在[数字信号处理领域](../Page/数字信号处理.md "wikilink")，**量化**指将信号的连续取值（或者大量可能的离散取值）近似为有限多个（或较少的）离散值的过程。量化主要应用于从[连续信号到](../Page/连续信号.md "wikilink")[数字信号的转换中](../Page/数字信号.md "wikilink")。连续信号经过[采样成为](../Page/采样.md "wikilink")[离散信号](../Page/离散信号.md "wikilink")，离散信号经过量化即成为数字信号。注意离散信号并不需要经过量化的过程。信号的采样和量化通常都是由ADC实现的。

例如[CD音频信号就是按照](../Page/CD.md "wikilink")44100[Hz的频率采样](../Page/Hz.md "wikilink")，按16[位元量化为有着](../Page/位元.md "wikilink")65536（=\(2^{16}\)）个可能取值的数字信号。

量化就是将模拟声音的波形转换为数字，表示采样值的二进制位数决定了量化的精度。量化的过程是先将整个幅度划分成有限个小幅度（量化阶距）的集合，把落入某个阶距内的样值归为一类，并赋予相同的量化值。

## 数学描述

最简单最易懂的量化是[标量](../Page/scalar.md "wikilink")（有别于多维[矢量](../Page/vector_\(spatial\).md "wikilink")）量化，开始标量量化之前先要给出输入数据。
...
在这个例子中\(f(x)\)和\(g(i)\)运算符都是乘以比例因子（其中一个是另外一个的逆），并且在*g*（*i*）中带有一个偏移量以使得每个量化表示都位于输入区域的中间位置。\(2^{-(M-1)}\)经常称为*量化步长*。按照这个量化定律，假定在整个量化步长上量化噪声大致是[均匀分布的](../Page/连续型均匀分布.md "wikilink")，并且假定量化的输入信号\(x\)在整个-1到1的区间大致均匀分布，量化的[信噪比](../Page/信噪比.md "wikilink")（SNR）可以用下面的公式计算，

\[\frac{S}{N_q} \approx 20 \log_{10}(2^M)
=
6.0206 M \ \operatorname{dB}\].

根据这个等式，人们常说SNR大约是每[位](../Page/位.md "wikilink")6
[dB](../Page/decibel.md "wikilink")。

在mid-tread一致量化中，偏移0.5将加在下取整函数内部而不是外部。

有时候，mid-rise量化使用时不加偏移0.5。这将信号与噪声比减小了大约6.02 dB，但是当步距小的时候为了简化这是可接受的。

在数字[电话系统中](../Page/电话.md "wikilink")，两个流行的量化机制是'[A-law](../Page/A-law_algorithm.md "wikilink")'（在[欧洲占据主导地位](../Page/欧洲.md "wikilink")）和'[μ-law](../Page/Mu-law_algorithm.md "wikilink")'（在[北美和](../Page/北美.md "wikilink")[日本占据主导地位](../Page/日本.md "wikilink")）。这些机制将离散的模拟数值映射到8位尺度，在小值的时候近似线性随着幅度增长按照对数增加。由于人耳对于[音量的感知近似对数曲线](../Page/音量.md "wikilink")，这就使用一定的位数在可听见的声音强度范围提供了更高的信噪比。
...
量化在[有损数据压缩中起着相当重要的作用](../Page/有损数据压缩.md "wikilink")。很多情况下，量化可以被当作将有损数据压缩同[无损数据压缩相区别的标志之一](../Page/无损数据压缩.md "wikilink")。量化的目的通常是为了减少数据量。一些压缩算法，例如[MP3和](../Page/MP3.md "wikilink")[Vorbis](../Page/Vorbis.md "wikilink")，以有选择地丢弃部分数据作为压缩的一种方法，这种手段可以被认为是量化的过程也可以被看作是一种有损压缩的形式。

[JPEG是一种利用了量化的图像有损压缩](../Page/JPEG.md "wikilink")。JPEG的编码过程对原始的图像数据作[离散余弦变换](../Page/离散余弦变换.md "wikilink")，然后对变换结果进行量化并作[熵编码](../Page/熵编码.md "wikilink")。通过量化可以降低变换值的精度，从而减少图像的数据量。当然，精度的损失意味着图像质量的下降。然而图像的质量可以通过量化位数的选择加以控制。例如，JPEG在每像素3[比特的精度下得到的图像质量还让人可以接受的](../Page/比特.md "wikilink")，相对于[PCM抽样得到的每个像素](../Page/PCM.md "wikilink")24比特的原始图像来说，数据量大大下降了。

现代压缩技术通常以量化输出的[信息熵](../Page/信息熵.md "wikilink")，而不是输出值集合的大小度量信息量的多少。
...
量化误差是指在量化过程引起的误差，表现为量化结果和被量化模拟量之间存在差值。这种差值在输出端体现为引入了[量化噪声](../Page/量化噪声.md "wikilink")。

## 相關條目

  - [模-数转换器](../Page/模-数转换器.md "wikilink")，[数-模转换器](../Page/数-模转换器.md "wikilink")
  - [量化误差](../Page/量化误差.md "wikilink"),
    [量化噪声](../Page/量化噪声.md "wikilink")
  - [离散信号](../Page/离散信号.md "wikilink")，[数字信号](../Page/数字信号.md "wikilink")
  - [抖动](../Page/抖动.md "wikilink")
  - [信息论](../Page/信息论.md "wikilink")
  - [率失真](../Page/率失真.md "wikilink")
  - [矢量量化](../Page/矢量量化.md "wikilink")
...
2.  Stuart P. Lloyd, "Least Squares Quantization in PCM", **, Vol.
    ...
    Statistics](../Page/Institute_of_Mathematical_Statistics.md "wikilink"),

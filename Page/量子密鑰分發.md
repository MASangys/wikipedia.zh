**量子密鑰分發**（）是利用[量子力学特性实现](../Page/量子力学.md "wikilink")[密码协议的](https://zh.wikipedia.org/wiki/密码协议 "wikilink")方法。它使通信的双方能够产生并分享一个随机的、安全的[密钥](https://zh.wikipedia.org/wiki/密钥 "wikilink")，来加密和解密[訊息](https://zh.wikipedia.org/wiki/訊息 "wikilink")。它常常被误称为[量子密碼學](../Page/量子密碼學.md "wikilink")，因为它是[量子密碼學任务中最著名的例子](../Page/量子密碼學.md "wikilink")。
...
量子密鑰分發的安全性基于量子力学的基本原理，而[传统密码学是基于某些数学算法的](../Page/密码学.md "wikilink")[计算复杂度](https://zh.wikipedia.org/wiki/计算复杂度 "wikilink")。传统密码学无法察觉窃听，也就无法保证密钥的安全性。QKD的安全性是是可以依据[信息论证明的](../Page/信息论.md "wikilink")，而且它还具有[前向安全性](https://zh.wikipedia.org/wiki/前向安全性 "wikilink")。

量子密鑰分發只用于产生和分发密钥，并没有传输任何实质的訊息。密钥可用于某些[加密算法来加密訊息](https://zh.wikipedia.org/wiki/加密算法 "wikilink")，加密过的訊息可以在标准[信道中传输](../Page/信道.md "wikilink")。跟量子密鑰分發最常見的相關演算法就是[一次性密碼本](../Page/一次性密碼本.md "wikilink")，如果使用保密而隨機的密鑰，這種演算法是\[1\]。在實際的運用上，量子密鑰分發常常被拿來與[對稱密鑰加密的加密方式](../Page/對稱密鑰加密.md "wikilink")，如[AES這類演算法一同使用](../Page/高级加密标准.md "wikilink")。

## 量子密钥交換

[量子通訊中](https://zh.wikipedia.org/wiki/量子通訊 "wikilink")，訊息编码为量子态，或称[量子比特](../Page/量子位元.md "wikilink")，与此相对，经典通信中，訊息编码为[比特](https://zh.wikipedia.org/wiki/位 "wikilink")。通常，[光子被用来制备量子态](../Page/光子.md "wikilink")。[量子密码学利用量子态的特性来确保安全性](https://zh.wikipedia.org/wiki/量子密码学 "wikilink")。量子密钥分发有不同的实现方法，但根据所利用量子态特性的不同，可以分为两大类。

  - 基于製備和测量的协议 :
    与[经典物理不同](https://zh.wikipedia.org/wiki/经典物理 "wikilink")，测量是量子力学不可分割的组成部分。一般来讲，测量一个未知的量子态会以某种形式改变该量子的状态。这被称为量子的不确定性，它的一些基本结论有[维尔纳·海森堡的](../Page/维尔纳·海森堡.md "wikilink")[不确定性原理](../Page/不确定性原理.md "wikilink")，[信息干扰理论和](https://zh.wikipedia.org/wiki/信息干扰理论 "wikilink")[不可克隆原理](../Page/不可克隆原理.md "wikilink")。这些性质可以被利用来检测通訊过程中的任何窃听（窃听必然需要测量），更重要的是，能够计算被截获訊息的数量。

<!-- end list -->

  - 基于纠缠态的协议 :
    两个或更多的量子态能够建立某种联系，使得他们无论距离多远依然要被看做是一个整体的量子态，而不是独立的个体。这被称为[量子纠缠](https://zh.wikipedia.org/wiki/量子纠缠 "wikilink")。他们之间的联系是，比如，对其中一个量子的测量会影响其他量子。如果纠缠的量子对被通訊的双方分别持有，任何对訊息的拦截会改变整个系统，使第三方的存在（以及他截获訊息的数量）被检测到。
...
[Artur
Ekert的方案使用纠缠的光子对](https://zh.wikipedia.org/wiki/Artur_Ekert "wikilink")。这些光子对可以由Alice、Bob或与他们两者都不在一起的某些源（包括窃听者Eve）产生。这些光子会被分发，Alice和Bob最终都会得到每对光子中的其中一个。
...
\(S\)，类似于[Bell测试实验中所用的相关系数](https://zh.wikipedia.org/wiki/貝爾定理的實驗驗證 "wikilink")。最大纠缠光子会导致
...
上述量子密钥分发协议为Alice和Bob提供了几乎相同的共享密钥，并且还对密钥之间的差异进行了估计。这些差异可能是由窃听引起的，也可能是由传输线和探测器的缺陷引起的。由于无法区分这两种类型的错误，因此保证安全性需要假设所有错误都是由于窃听造成的。如果密钥之间的错误率低于某一阈值（截至2007年4月为20％\[3\]），则可以执行两个步骤：首先移除错误的比特，然后将Eve对密钥的了解减少到任意小的值。这两个步骤分别称为**信息协调**和**隐私增强**，这两个步骤最早在1992年被提出来。\[4\]
  ...
  - **信息协调**（）是密钥纠错（）的一种方式，可保证Alice和Bob共同拥有的密钥的一致性。这个过程在公共信道中完成，由于可能被Eve窃听，所以要保证关于密钥本身的信息公布的越少越好。用于信息协调的通用协议是1994年提出的**级联协议**（）。\[5\]
    这可以在几轮中运行，每一轮中的两个密钥被分成块，并且比较这些块的[奇偶校验位](../Page/奇偶校验位.md "wikilink")。如果发现奇偶校验有差异，则执行[二分搜索来查找和纠正错误](https://zh.wikipedia.org/wiki/二分搜索 "wikilink")。如果在前一轮中奇偶校验正确的块中发现错误，则该块中必须包含另一个错误；需要找到此错误并像以前一样进行纠正。这个过程以递归方式重复，这是“级联”这个名称的来源。在比较了所有的块之后，Alice和Bob都以相同的随机方式对其键进行重新排序，并开始新一轮。在多轮结束后，很大概率Alice和Bob拥有相同密钥；但是，Eve从交换的奇偶校验信息中也获得了关于密钥的附加信息。不过，从编码理论的角度来看，信息协调本质上就是带有辅助信息的源编码，因此适用于该问题的任何编码方案都可以用于信息协调。近来，Turbo码\[6\]、LDPC码\[7\]和极性码\[8\]已用于此目的，提高了级联协议的效率。

...
如欲建構地證明一個理論可消去量詞，僅須處理一個存在量詞配上若干個[文字合取的情形](https://zh.wikipedia.org/wiki/文字_\(數理邏輯\) "wikilink")；換言之，即證明每個形如\(\exists x. \bigwedge_{i=1}^n L_i\)
（其中每個\(L_i\)都是文字）
的公式都在該理論中等價於一個無量詞的公式——誠然，假設已知如何對一串公式的合取消去量詞，則若\(F\)是個公式，可將其寫作[析取範式](https://zh.wikipedia.org/wiki/析取範式 "wikilink")：

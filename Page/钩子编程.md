...
table）。还有一种挂钩方法是采用，使得应用程序不需修改即可调用该包装库完成其功能，而在包装库中插入钩子然后再调用原有的库。
...
下述例子使用指令，修改[windows
API中的MessageBoxW函数的前](https://zh.wikipedia.org/wiki/windows_API "wikilink")6个字节执行其他代码。这段代码编译为DLL文件，采用技术让应用程序使用。\[1\]微软提供了封装好的Detours库用于此目的。

...
钩子编程也被用于恶意代码，如[rootkit是各种通过假扮系统API调用输出来隐藏自身进程的可见性的工具与技术](https://zh.wikipedia.org/wiki/rootkit "wikilink")；[游戏外挂是另一类例子](../Page/游戏外挂.md "wikilink")。

## 方法

### 物理修改

在应用程序执行之前，物理修改[可执行程序](https://zh.wikipedia.org/wiki/可执行程序 "wikilink")，这典型通过找到函数调用入口点，修改入口点使之在函数被执行前先执行其他的代码。另一种挂钩的方法是修改可执行程序的[输入表](../Page/可移植可执行.md "wikilink")（import
table）。还有一种挂钩方法是采用[包装库](https://zh.wikipedia.org/wiki/包装库 "wikilink")，使得应用程序不需修改即可调用该包装库完成其功能，而在包装库中插入钩子然后再调用原有的库。

### 运行时修改

操作系统与软件可提供方法，在[运行时插入事件钩子](https://zh.wikipedia.org/wiki/运行时 "wikilink")。Microsoft
Windows允许插入钩子以处理或修改对话框、滚动条、菜单等的系统事件、应用程序事件；插入、删除、处理或修改键盘鼠标事件。Linux允许类似的钩子通过[NetFilter以处理网络事件](https://zh.wikipedia.org/wiki/NetFilter "wikilink")。

如果没有上述机制或权限，也可拦截进程的库函数调用，在函数调用开始处注入代码。这可通过修改内存中的进程的或[输入表](../Page/可移植可执行.md "wikilink")（import
table）实现。
...
C++使用[虚函数](../Page/虚函数.md "wikilink")，因此可在运行时直接修改虚函数表的内容来挂钩。
Window上很多软件库以[COM方式提供的](../Page/组件对象模型.md "wikilink")（如[DirectX](../Page/DirectX.md "wikilink")），
所以有需求拦截COM调用的COM
Hook。COM里的接口是C++虚表的形式提供的，所以COM的Hook其实就是虚表（vtable）的Hook。[ATL就是用接口替代的方式来调试和记录COM接口引用计数的次数](https://zh.wikipedia.org/wiki/ATL "wikilink")
...
下述例子使用指令，修改[windows
API中的MessageBoxW函数的前](https://zh.wikipedia.org/wiki/windows_API "wikilink")6个字节执行其他代码。这段代码编译为DLL文件，采用[DLL注入技术让应用程序使用](../Page/DLL注入.md "wikilink")。\[1\]微软提供了封装好的Detours库用于此目的。
...
使用[Netfilter钩子修改Linux内核的网络交通](../Page/Netfilter.md "wikilink")。
...
### Windows API提供的挂钩函数
  ...
  - SetWindowsHookEx：64位编程情形，32位DLL没法直接注入到64位的应用程序里面，
    因为地址空间完全不一样。UAC打开的情况下低权限程序没法Hook高权限程序。对于64位问题，解决方法是提供2个DLL，分别可以Hook32和64位程序。对于权限问题，通过注册系统服务，由服务进程创建工作进程。因为Windows
    Vista开始有了Session隔离机制，服务进程运行在Session 0，用户程序运行在Session 1, Session
    2等，如果直接在服务程序里干活，我们就只能在Session
    0里工作。通过创建进程，可以在DuplicateTokenEx后将Token的SessionID设置成目标Session，并且在CreateProcessAsUser时指定目标WinStation和Desktop，
    这样就获得了System权限，并且也可以和当前桌面进程交互。
...
### 通论
  ...
  - [Various hooking methods explained for
    Linux/Windows/Macintosh.](http://ntvalk.blogspot.nl/2013/11/hooking-explained-detouring-library.html)

### Windows

  - [Information on Import Address Table function
    hooking.](http://sandsprite.com/CodeStuff/IAT_Hooking.html)
  - \[<http://msdn.microsoft.com/en-us/library/ms632589(VS.85>).aspx
    Information from Microsoft on hooking\]
  - [Information and various techniques regarding x86
    hooking.](http://jbremer.org/x86-api-hooking-demystified/)
  - [APISpy32](http://www.internals.com/) is an application used to hook
    win32 API.
  - [Detours](http://research.microsoft.com/sn/detours) is a general
    purpose function hooking library created by Microsoft Research which
    works in C / C++.
  - [winspy](http://www.codeproject.com/KB/threads/winspy.aspx) Three
    ways to inject code into another process.
  - [HookTool SDK (ACF
    SDK)](http://www.codeproject.com/KB/system/hooksys.aspx) Provides a
    comprehensive overview on API hooking and code injection. A
    commercial product available too.
  - [madCodeHook](http://help.madshi.net/ApiCodeHooking.htm) is a
    commercial x86 and x64 API hooking and DLL injection library for C++
    and Delphi.
  - [EasyHook](http://www.codeplex.com/easyhook) is an open source
    hooking engine supporting x86 and x64 in Windows in both user and
    kernel land.
  - [SpyStudio Application
    Trace](http://www.nektra.com/products/spystudio-api-monitor/)
    SpyStudio is an Application tracer which hook calls, displaying the
    results in a structured way.
  - [rohitab.com API Monitor](http://www.rohitab.com/apimonitor) is a
    freeware application that can hook and display 10,000+ Windows APIs
    and COM Interfaces in 32-bit and 64-bit applications and services.
  - [Deviare API
    Hook](http://www.nektra.com/products/deviare-api-hook-windows/)
    Deviare is a freeware inter-process hook framework that can be used
    to intercept other processes' API calls and show full-parameter
    information or create API monitors.
  - [WinAPIOverride](http://jacquelin.potier.free.fr/winapioverride32/)
    WinAPIOverride is a freeware for non commercial use. It can hook
    win32 API, COM, OLE, ActiveX, .NET in 32-bit and 64-bit processes.
    It includes monitoring post analysis tools.
  - [urmem](https://github.com/urShadow/urmem) C++11 cross-platform
    library (x86) for working with memory (hooks, patches, pointer's
    wrapper, signature scanner etc.)

### Linux

  - [1](https://web.archive.org/web/20070610083142/http://rtg.informatik.tu-chemnitz.de/docs/da-sa-txt/sa-dienelt.pdf)
    A student research project that utilizes hooking.
  - [2](https://web.archive.org/web/20090317112907/http://www.linuxmanpages.com/man2/ptrace.2.php)
    Functionality that allows a piece of software to observe and control
    the execution of another process.
  - [3](http://securityvulns.ru/articles/reveng/) Use of LD_PRELOAD to
...
### OS X 与 iOS
  ...
  - [Cydia Substrate](http://cydiasubstrate.com) is a framework for
    jailbroken iOS devices allowing developers to hook into any other
    framework or application.
  - [harpoon](https://github.com/jndok/harpoon) is an OS X library for
    runtime function hooking.

### Depth API Hooking

  - [x86 API Hooking
    Demystified](http://jbremer.org/x86-api-hooking-demystified/)
    Article on various API Hooking methods, for the x86 architecture.

[Category:控制流程](https://zh.wikipedia.org/wiki/Category:控制流程 "wikilink")
[Category:DLL注入](https://zh.wikipedia.org/wiki/Category:DLL注入 "wikilink")

1.  For more information, see
    <http://ntvalk.blogspot.nl/2013/11/hooking-explained-detouring-library.html>
...
消息的[校验和是固定字长](https://zh.wikipedia.org/wiki/校验和 "wikilink")（例如字节值）的字节码的[模算數](../Page/模算數.md "wikilink")和。和可能在传输前用[一補數以检测全零消息出现的错误](https://zh.wikipedia.org/wiki/一補數 "wikilink")。

校验和方案包括[奇偶校验位](../Page/奇偶校验位.md "wikilink")、[校验码和](https://zh.wikipedia.org/wiki/校验码 "wikilink")[纵向冗余校验](https://zh.wikipedia.org/wiki/纵向冗余校验 "wikilink")。部分校验和方案如、[Luhn算法](../Page/Luhn算法.md "wikilink")和在设计上是专门用于检测人类书写或记录数字时常出现的错误。

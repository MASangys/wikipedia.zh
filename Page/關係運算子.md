**關係運算子**在計算機科學的[編程語言中](https://zh.wikipedia.org/wiki/編程語言 "wikilink")，是測試或定義兩個實體之間某種[關係的構造或操作符](../Page/二元关系.md "wikilink")。這些包括數值[等式和](https://zh.wikipedia.org/wiki/等於 "wikilink")[不等式](https://zh.wikipedia.org/wiki/不等式 "wikilink")（例如
5 = 5 和
4≥3）。在具備[布爾型別的編程語言中](https://zh.wikipedia.org/wiki/布爾_\(數據類型\) "wikilink")（如
Pascal，Ada 或
Java），這些運算符通常根據兩個[操作變量之間的條件關係是否成立](../Page/運算數.md "wikilink")，判定為真或假。諸如
C 語言中關係運算子返回整數 0 或 1，其中 0 表示假，任何非零值表示真。使用關係運算子創建的表達式，形成所謂的關係表達式或條件。
關係運算子可以被視為[謂詞邏輯的特殊情況](../Page/逻辑.md "wikilink")。

## 相等性

### 用法

在許多編程語言構造和[資料型別中都使用到相等性](../Page/類型系統.md "wikilink")，它用於測試元素是否已存在於[集合中](../Page/集合_\(计算机科学\).md "wikilink")，或者藉由鍵來存取值。它在切換（switch）語句，以及編程的邏輯併聯過程中，用於將控制流調度到正確的分支。相等性的可能含義之一是“如果
a 等於 b，那麼我們可以在任何上下文中互換 a 或 b，而不會產生任何差異。但這樣的聲明不一定成立，尤其在可變性和內容等同性一起考慮時。
...
在許多現代編程語言中會藉由[參照來存取物件和資料結構](../Page/參照.md "wikilink")。在這些語言中，需要測試兩種相等性質：
  ...
  - 語義同等性：如果兩個參照物件或兩個值在某種意義上是等價的：
      ...
      - 其它特制的同等性，保留外部行為。例如將\(\frac{1}{~{}2~{}}\)和\(\frac{2}{~{}4~{}}\)視為有理數時，被判斷是相等的。除了[反射性](https://zh.wikipedia.org/wiki/自反關係 "wikilink")、[對稱性和](https://zh.wikipedia.org/wiki/對稱關係 "wikilink")[傳遞性之外](https://zh.wikipedia.org/wiki/遞移關係 "wikilink")，對
        A = B 特制的定義可能是“若且唯若對於物件A和物件B之上的所有操作，都將具有相同的結果時，則 A = B ”。

第一種同等性質通常蘊涵著第二種同等性質（除了[非數字類](../Page/NaN.md "wikilink")（not a number,
NaN），它們不等於自身），但反向的同等性質並不一定成立。例如兩個字串物件可以是不同物件（第一種意義不相等），但它們包含相同的字元序列（第二種意義上相等）。有關此問題的更多信息，請參閱識別（identity）。

[實數中包括許多簡](https://zh.wikipedia.org/wiki/實數 "wikilink")[分數](../Page/分數.md "wikilink")，無法以浮點算數精確地表示，所以需要在給定誤差範圍內來測試相等性。但這樣的誤差範圍將打破一些例如傳遞性、反身性的要求性質：[IEEE浮點標準是判斷](../Page/IEEE_754.md "wikilink")
Nan ≠ NaN 成立（NaN不等於自身）。
...
JavaScript，PHP
和一些其它動態型別的語言中，如果兩個值相等，等號運算符將計算為真，即使它們實際上為不同型別的物件，例如以數值4和字串"4"相比較，結果會是相等。在這類語言中通常也會提供型別相等運算子，僅對具有相同或等價型別的物件比較返回真（在PHP
5中 4 ==="4"為假，但 4 =="4"
為真）。而在將數值0也當作布爾值為假的編程語言中，該運算子可化簡為檢查物件是否為數值零（例如，對於數值0或字串"0"的x物件，使用型別相等運算子，則
x == 0 判斷傳回真值）。

## 次序比較

非數值資料的次序比較（大於或小於）運算是根據排序慣例（例如字串依照編程語言內定的[字典次序](../Page/字典序.md "wikilink")，和/或可由開發人員設定的）。當兩個資料項
a 和 b 之間的比較結果，要和數值關聯時，通常慣例是如果 a \< b 則結果賦值為 -1，如果 a = b 則為 0，如果 a \> b
則為 1。例如C語言的函數`strcmp`執行三方向比較，並根據此慣例返回 -1, 0 或
1，而`qsort`預期比較函數依此慣例返回值。在排序演算法中比較方法源碼的效率至為關鍵，因為它是排序性能的主要因素之一。
...
雖然一開始可能不那麼顯而易見，像布爾邏輯運算符 XOR，AND，OR 和
NOT，這些關係運算子可以設計為具有邏輯等同性，使得它們都可以相互定義。對於任何給定的
x 和 y 值，以下四個條件語句都有相同的邏輯等價性 E（全為真或全為假）：
...
其他較少見的：Common Lisp的不等關係運算子是 /=，Macsyma/Maxima 的不等關係運算子是
\#。舊的Lisp使用equal，greaterp 和 lessp; 而以not運算子作邏輯否定。

## 語法

關係運算子也用於技術文獻而不是單詞，如果編程語言支援通常以[中綴表示法](https://zh.wikipedia.org/wiki/中綴表示法 "wikilink")，亦即出現在其操作變量（兩個表達式是相關的）之間。
...
其他編程語言如 Lisp
使用[前綴表示法](https://zh.wikipedia.org/wiki/前綴表示法 "wikilink")，如下所示：
...
鏈接關係在數學中是普遍的寫法，例如 3 \< x \< y \< 20 表示 3 \< x 而且 x \< y 而且 y
\<20。語義是很清楚的，因為數學中這些關係運算是有傳遞性的。然而，許多最近的編程語言會把 3 \< x \<
y 的表達式，看作兩個左（或右）關係運算子的組合，而解譯為`(3 < x ) < y`。如果我們設 x = 4 則得到`(3 < 4 )<
y`，而運算式變成`true < y`，這是無意義的。但它卻可能通過 C/C++ 和一些其它語言的編譯（因為 true 會以數值1代表）。

有些編程語言如Python和Perl 6 能正確給出`x < y < z`表達式所代表的數學意義，其它種語言則不，
部份是因大多數運算符在C語言種類中，以中綴表示法的運作方式有所不同。D編程語言保持與C的一些兼容性，而“允許C語言表達式卻有微妙不同的語義（雖然可說是方向正確），與便利性比起來造成更多的混淆”。

有些語言如 Common Lisp，對此則使用多參數謂詞。當 *x* 在 1 和 10 之間時，評估比較運算式 `(<= 1
x 10)`結果為真。
...
Simula，SET（SETL），Pascal，Smalltalk，Modula-2，Ada，Standard
ML，OCaml，Eiffel，Object Pascal（Delphi），Oberon，Dylan，VHSIC（VHDL）等。
...
這兩種意義，因此[Ken
Thompson使用了特別的雙等號](../Page/肯·汤普逊.md "wikilink")`==`組合取代相等關係判斷。
...
C編程的賦值語句會有返回值，由於任何非零值在[條件運算式中被解譯為真](https://zh.wikipedia.org/wiki/條件運算式 "wikilink")，源碼`if(x
= y)`是合法的，
...
如果意外使用了`=`，因為 2 不是變量則源碼的編譯無效，編譯器會產生一個錯誤訊息，
指出在等號的位置應該以適當的運算子替換。這種編程寫法被稱為左手比較或[尤達條件式](../Page/尤達條件式.md "wikilink")。
...
2.  [ALGOL 68](../Page/ALGOL_68.md "wikilink"):
    *[stropping](https://zh.wikipedia.org/wiki/Stropping_\(syntax\) "wikilink")*
    regimes are used in code on platforms with limited character sets
    (*e.g.*, use `>=` or `GE` instead of `≥`), platforms with no
    **`bold`**
    [emphasis](https://zh.wikipedia.org/wiki/Emphasis_\(typography\) "wikilink")
    (use `'ge'`), or platforms with only
    [UPPERCASE](https://zh.wikipedia.org/wiki/UPPERCASE "wikilink") (use
    `.GE` *or* `'GE'`).
3.  Including [Visual Basic
    .NET](../Page/Visual_Basic_.NET.md "wikilink"),
    [OCaml](../Page/OCaml.md "wikilink"),
    [SQL](../Page/SQL.md "wikilink"), [Standard
    ML](https://zh.wikipedia.org/wiki/Standard_ML "wikilink"),
    [Excel](../Page/Microsoft_Excel.md "wikilink"), and others.
4.  Including [ALGOL](../Page/ALGOL.md "wikilink"),
    [Simula](../Page/Simula.md "wikilink"),
    [Modula-2](../Page/Modula-2.md "wikilink"), [Object
    Pascal](../Page/Object_Pascal.md "wikilink")
    ([Delphi](https://zh.wikipedia.org/wiki/Delphi_\(programming_language\) "wikilink")),
    [OCaml](../Page/OCaml.md "wikilink"), [Standard
    ML](https://zh.wikipedia.org/wiki/Standard_ML "wikilink"),
    [Eiffel](https://zh.wikipedia.org/wiki/Eiffel_\(programming_language\) "wikilink"),
    [APL](https://zh.wikipedia.org/wiki/APL_\(programming_language\) "wikilink"),
    and others.
5.  Including [C](https://zh.wikipedia.org/wiki/C语言 "wikilink"),
    [C++](../Page/C++.md "wikilink"),
    [C\#](https://zh.wikipedia.org/wiki/C_Sharp_\(programming_language\) "wikilink"),
    [Go](https://zh.wikipedia.org/wiki/Go_\(programming_language\) "wikilink"),
    [Java](../Page/Java.md "wikilink"),
    [JavaScript](../Page/JavaScript.md "wikilink"),
    [Perl](../Page/Perl.md "wikilink") (numerical comparison only),
...
6.  Including [Bourne shell](../Page/Bourne_shell.md "wikilink"),
    [Bash](https://zh.wikipedia.org/wiki/Bash_\(Unix_shell\) "wikilink"),
    [Korn shell](https://zh.wikipedia.org/wiki/Korn_shell "wikilink"),
    and [Windows PowerShell](../Page/Windows_PowerShell.md "wikilink").
    The symbols `<` and `>` are usually used in a shell for
    [redirection](https://zh.wikipedia.org/wiki/Redirection_\(computing\) "wikilink"),
    so other symbols must be used. Without the hyphen, is used in
    [Perl](../Page/Perl.md "wikilink") for string comparison.
7.  MATLAB, although in other respects using similar syntax as C, does
    not use `!=`, as `!` in MATLAB sends the following text as a command
    line to the [operating
    system](https://zh.wikipedia.org/wiki/operating_system "wikilink").
    The first form is also used in
    [Smalltalk](../Page/Smalltalk.md "wikilink"), with the exception of
    equality, which is `=`.
8.  Including FORTRAN 95, 2003, 2008 and 2015.
9.  [Relational and Logical
    Operators](http://reference.wolfram.com/mathematica/tutorial/RelationalAndLogicalOperators.html)
    of
    [Mathematica](https://zh.wikipedia.org/wiki/Mathematica "wikilink")
10. Patent application: On May 14, 2003,  "IS NOT OPERATOR" was filed
    for the `ISNOT` operator by employees of
    [Microsoft](https://zh.wikipedia.org/wiki/Microsoft "wikilink").
    This patent was granted on November 18, 2004.
...
[彼得·兰丁](../Page/彼得·兰丁.md "wikilink")在1964年将术语“闭包”定义为一种包含环境成分和控制成分的实体，用于在他的SECD机器上对表达式求值。\[1\]
[Joel
...
[Scheme](../Page/Scheme.md "wikilink")语言的时候予以采纳。\[4\] 并广为流传。
...
## 闭包的用途
  ...
  - 因为闭包只有在被调用时才执行操作（暂且不论用于生成这个闭包对象本身的开销，比如 C++
    中按值捕获意味着执行复制构造函数），即“[惰性求值](../Page/惰性求值.md "wikilink")”，所以它可以被用来定义控制结构。例如：在[Smalltalk](../Page/Smalltalk.md "wikilink")语言中，所有的控制结构，包括分歧条件（if/then/else）和循环（while和for），都是通过闭包实现的。用户也可以使用闭包定义自己的控制结构。
...
(使用[LLVM](../Page/LLVM.md "wikilink")编译器或[苹果修改版的GCC](https://zh.wikipedia.org/wiki/苹果公司 "wikilink"))支持[块](../Page/块_\(C语言扩展\).md "wikilink")。闭包变量用`__block`标记。同时，这个扩展也可以应用到[Objective-C](../Page/Objective-C.md "wikilink")与[C++](../Page/C++.md "wikilink")中。

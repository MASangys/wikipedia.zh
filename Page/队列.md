**-{zh-hans:队列;zh-hant:佇列}-**，又稱為**-{zh-hans:伫列;zh-hant:隊列}-**（queue），是[先进先出](../Page/先进先出.md "wikilink")（FIFO,
First-In-First-Out）的[线性表](../Page/线性表.md "wikilink")。在具体应用中通常用[链表或者](../Page/链表.md "wikilink")[数组来实现](../Page/数组.md "wikilink")。队列只允许在后端（称为*rear*）进行插入操作，在前端（称为*front*）进行删除操作。

...
[3noshadow.png](https://zh.wikipedia.org/wiki/File:3noshadow.png "fig:3noshadow.png")

**阴影贴图**（***Shadow
mapping***）是在[三维计算机图形中加入](../Page/三维计算机图形.md "wikilink")[阴影的过程](https://zh.wikipedia.org/wiki/阴影 "wikilink")。阴影贴图的概念最初是由
[Lance
Williams](https://zh.wikipedia.org/wiki/Lance_Williams "wikilink") 于
1978年在“在曲面上投射阴影”这篇论文中提出的。从那时开始，这种方法就已经用于场景预渲染、实时甚至是许多游戏设备以及高端电脑游戏中。在[Pixar](https://zh.wikipedia.org/wiki/Pixar "wikilink")
的 [RenderMan](https://zh.wikipedia.org/wiki/RenderMan "wikilink")
中就使用了阴影贴图技术，同样，在 *[玩具总动员](../Page/玩具总动员.md "wikilink")*
这样的游戏中也使用了这项技术。

像素与以[纹理形式保存的光照](https://zh.wikipedia.org/wiki/纹理 "wikilink")[深度缓冲区或者深度图像比较](../Page/深度缓冲.md "wikilink")，通过这种方式计算像素是否处于光源照射范围之内，从而生成阴影。
...
对于实时阴影来说，这项技术要比[阴影体的精度差一些](../Page/阴影体.md "wikilink")，但是根据在特定应用中每种技术所需填充时间的不同，阴影图有些情况下是一种速度比较快的选择。并且，阴影图不需要额外的[模板缓存](https://zh.wikipedia.org/wiki/模板缓存 "wikilink")，并且可以经过修改生成柔和边界的阴影。但是，与阴影体不同，阴影图的精度受到分辨率的限制。
...
[1light.png](https://zh.wikipedia.org/wiki/File:1light.png "fig:1light.png")
[2shadowmap.png](https://zh.wikipedia.org/wiki/File:2shadowmap.png "fig:2shadowmap.png")

第一步是从光源的视角渲染场景。对于点光源来说，视场是一个视角足够宽的[透视投影](https://zh.wikipedia.org/wiki/透视投影 "wikilink")。对于象[太阳光这样的定向光来说](../Page/太阳.md "wikilink")，需要使用[正投影](https://zh.wikipedia.org/wiki/正投影 "wikilink")。
...
将世界坐标系变换到光照空间坐标系的变换矩阵与第一步中用于渲染阴影图的变换矩阵相同，在[OpenGL中它是模型视图与投影矩阵之积](../Page/OpenGL.md "wikilink")。这样就生成一组[齐次坐标](../Page/齐次坐标.md "wikilink")，如果可见的话经过透视除法（perspective
division）转换成*归一化设备坐标*，每个分量（*x*、*y*、*z*）都落在 -1 到 1
之间。有许多的实现方法要进行另外的*缩放及偏移*矩阵乘法将
-1 到 1 之间的数值转换到深度图或纹理图中更常用的 0 到 1
之间的数值。这个缩放过程也可以在透视除法之前完成，并且可以通过那个矩阵与下面的矩阵相乘很容易地将前面的变换计算融合到一起：
\(\begin{bmatrix}
0.5 & 0 & 0 & 0.5 \\
0 & 0.5 & 0 & 0.5 \\
0 & 0 & 0.5 & 0.5 \\
...
如果 *z*
大于深度图中相应位置保存的数值，那么就认为物体被其它物体所遮挡，这个点就标记为*失败*，在绘制过程中将它描绘成阴影状态；否则的话，就将它绘制成照亮状态。

如果位置落在深度图之外，那么程序就需要确定表面要用缺省的照亮或者阴影方式绘制，通常是绘制成照亮的状态。

在 [shader](https://zh.wikipedia.org/wiki/shader "wikilink")
的实现中，这个过程是在片断层次完成的。另外，当选择硬件所用的纹理图存储类型的时候需要注意：如果无法进行插值，那么阴影将会出现明显的锯齿边界，可以通过使用较高的阴影图分辨率来减少这种现象。
...
带有阴影的场景的可以通过几种不同的方法进行绘制。如果有可用的可编程
[shader](https://zh.wikipedia.org/wiki/shader "wikilink")，深度图检验可以用碎片
shader(fragment/pixel program) 完成。在最初的生成阴影图之后，它一次就可以根据结果完成阴影或者被照亮物体的绘制。

如果没有可以使用的阴影工具，通常必须使用一些硬件扩展如
[*GL_ARB_shadow*](http://www.opengl.org/registry/specs/ARB/shadow.txt)
等进行阴影图检验，它们通常不允许在两个光照模型之间进行选择，并且需要更多的渲染过程：
...
这篇文章中的例图是使用 [OpenGL](../Page/OpenGL.md "wikilink") 的扩展
[*GL_ARB_shadow_ambient*](http://www.opengl.org/registry/specs/ARB/shadow_ambient.txt)
完成阴影贴图过程的。
...
## 参见
  ...
  - [Photon
    mapping](https://zh.wikipedia.org/wiki/Photon_mapping "wikilink")，一种速度较慢、能够实现非常真实的光照的技术
  - [辐射着色](https://zh.wikipedia.org/wiki/辐射着色 "wikilink")，另外一种速度很慢，但真实感非常好的技术

## 外部链接

  - [硬件阴影贴图](https://archive.is/20130104180749/http://developer.nvidia.com/attach/8456),
    [nVidia](https://zh.wikipedia.org/wiki/NVIDIA "wikilink")
  - [当今 OpenGL
    硬件的阴影贴图](https://web.archive.org/web/20040619120702/http://developer.nvidia.com/attach/6769),
    nVidia

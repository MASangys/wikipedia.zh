**阿貝爾定理**是[冪級數的一個重要結果](../Page/冪級數.md "wikilink")。

## 定理

設\(f(z)= \sum_{n \geq 0} a_n z^n\)為一冪級數，其[收斂半徑為](../Page/收斂半徑.md "wikilink")*R*。若对收敛圆（模长为
*R*
的复数的集合）上的某个复数\(z_0\)，[级数](../Page/级数.md "wikilink")\(\sum_{n\geq 0} a_n z_0^n\)收斂，則有:
\(\lim_{t\to 1^-} f(t z_0) =  \sum_{n \geq 0} a_n z_0^n\)。

若\(\sum_{n \geq 0} a_n R^n\)[收斂](../Page/收斂.md "wikilink")，則結果顯然成立，無須引用這定理。
...
令\(b_0^{\prime} = b_0 - \sum_{n \geq 0} b_n\)，则可化归到\(\sum_{n\geq 0} b_n =0\)，于是以下只需要考虑\(\sum_{n\geq 0} b_n =0\)
的情况。

设\(S_n = \sum_{k = 0}^n b_n\)，那么\(\lim_{n\to +\infty} S_n = 0\)。由幂级数性质可知\(\sum_{n\geq 0} S_n z^n\)
的收敛半径也是1。于是
...
阿贝尔定理的一个有用应用是计算已知收敛级数。方法是通过在级数每项后加上\(x^n\)项，将问题转换为幂级数求和，最后再计算 *x* 趋于 1
时幂级数的极限。由阿贝尔定理可知，这个极限就是原级数的和。

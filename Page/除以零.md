...
[婆什迦羅第二](../Page/婆什迦羅第二.md "wikilink")嘗試解決此問題，設\(\frac{n}{0}=\infty\)，雖然此定義有一定道理，但會導致一个悖論：\(0\times\infty\)的结果可以是任意一个数，那么就会得出所有的数都相同的错误结论。\[1\] [缩略图](https://zh.wikipedia.org/wiki/File:Division_by_zero_on_android_2.2.1_calculator.png "fig:缩略图")手機計算器除以0顯示無限大\]\]
...
\[\lim_{b \to 0^{+}} {a \over b} = {-}\infty\] 所以，對於正數\(a\)，\(\frac{a}{0}\)可被定義為\(\infty\)，而對於負數\(a\)則可定義為\(-\infty\)。不過，某數也可以由負數一方（左面）趨向零，這様，對於正數\(a\)，\(\frac{a}{0}\)定義為\(-\infty\)，對於負數\(a\)，\(\frac{a}{0}\)定義為\(+\infty\)。由此可得（假設實數的基本性質可應用在極限上）：

\[+\infty = \frac{1}{0} = \frac{1}{-0} = -\frac{1}{0} = -\infty\]

最終變成 \(+\infty = -\infty\)，與在[扩展的实数轴上對極限賦予的標準定義不相符](https://zh.wikipedia.org/wiki/扩展的实数轴 "wikilink")。唯一的辦法是用沒有正負號的無限，參見下面。

另外，利用極限的比無為\(\frac{0}{0}\)提供解釋：

\[\lim_{(a,b) \to (0,0)} {a \over b}\] 並不存在，而

\[\lim_{x \to 0} {f(x) \over g(x)}\] 若隨着\(x\)趨向\(0\)，\(f(x)\)與\(g(x)\)均趨向\(0\)，該極限可等於任何實數或無限，或者根本不存在，視乎\(f\)及\(g\)是何函數（參閱[洛必達法則](https://zh.wikipedia.org/wiki/洛必達法則 "wikilink")）。由此，\(\frac{0}{0}\)難以被定義為一極限。

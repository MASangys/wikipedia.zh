...
**随机存取存储器**（，缩写：），也叫**主存**，是与[CPU直接交换数据的内部存储器](https://zh.wikipedia.org/wiki/CPU "wikilink")。\[1\]它可以隨時读写（重新整理時除外，見下文），而且速度很快，通常作为[操作系统或其他正在运行中的程式的临时資料存储媒介](../Page/操作系统.md "wikilink")。

主記憶體（Main
memory）即電腦內部最主要的記憶體，用來載入各式各樣的程式與資料以供[CPU直接執行與運用](https://zh.wikipedia.org/wiki/CPU "wikilink")。由於[DRAM的](https://zh.wikipedia.org/wiki/DRAM "wikilink")[性價比很高](../Page/性價比.md "wikilink")，且擴展性也不錯，是現今一般電腦[主記憶體的最主要部分](https://zh.wikipedia.org/wiki/主記憶體 "wikilink")。2011年生產電腦所用的[主記憶體主要是](https://zh.wikipedia.org/wiki/主記憶體 "wikilink")[DDR3
...
現代的随机存取存储器依賴[電容器儲存資料](https://zh.wikipedia.org/wiki/電容器 "wikilink")。電容器充滿電后代表1（[二進制](https://zh.wikipedia.org/wiki/二進制 "wikilink")），未充電的代表0。由于電容器或多或少有漏電的情形，若不作特別處理，電荷會漸漸隨時間流失而使資料發生錯誤。**刷新**是指重新為電容器充電，彌補流失了的電荷。DRAM的讀取即有刷新的功效，但一般的定時刷新並不需要作完整的讀取，只需作該晶片的一個列（Row）選擇，整列的資料即可獲得刷新，而同一時間內，所有相關記憶晶片均可同時作同一列選擇，因此，在一段期間內逐一做完所有列的刷新，即可完成所有記憶體的刷新。需要刷新正好解釋了随机存取存储器的易失性。
...
电脑运行时，电脑的主内存按照被使用情况可分类为：\[2\]
  ...
  - 被使用（used）物理内存：已经被进程使用的内存
      - 用户进程使用的物理内存，即[工作集](https://zh.wikipedia.org/wiki/工作集 "wikilink")(Working

...
3.  真隨機性。其定義爲隨機樣本不可重現。實際上衹要給定邊界條件，真隨機數并不存在，可是如果產生一個真隨機數樣本的邊界條件十分複雜且難以捕捉（比如計算機當地的[本底輻射波動值](https://zh.wikipedia.org/wiki/本底輻射 "wikilink")），可以認爲用這個方法演算出來了真隨機數。但實際上，這也只是非常接近真隨機數的偽隨機數，一般認為，無論是本地輻射、物理噪音、拋硬幣……等都是可被觀察了解的，任何基於經典力學產生的隨機數，都只是偽隨機數。

相應的，隨機數也分爲三類：

1.  **僞隨機數**：滿足第一個條件的隨機數。
2.  **密碼學安全的僞隨機數**：同時滿足前兩個條件的隨機數。可以通过[密码学安全伪随机数生成器计算得出](../Page/密码学安全伪随机数生成器.md "wikilink")。
3.  **真隨機數**：同時滿足三個條件的隨機數。
...
在[统计学的不同技术中需要使用随机数](../Page/统计学.md "wikilink")，比如在从[统计总体中抽取有代表性的样本的时候](https://zh.wikipedia.org/wiki/统计总体 "wikilink")，或者在将实验动物分配到不同的试验组的过程中，或者在进行[蒙特卡罗模拟法计算的时候等等](https://zh.wikipedia.org/wiki/蒙特卡罗模拟法 "wikilink")。
...
在真正关键性的应用中，比如在[密码学中](../Page/密码学.md "wikilink")，人们一般使用真正的随机数。

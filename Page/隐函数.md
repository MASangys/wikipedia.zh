在[數學中](../Page/數學.md "wikilink")，**隱式方程**（）是形同\(f(x_1,x_2,\cdots,x_n)=0\)的[關係](../Page/关系_\(数学\).md "wikilink")，其中\(f\)是[多元函數](../Page/多元函數.md "wikilink")。比如[單位圓的隱式方程是](../Page/單位圓.md "wikilink")\(x^2+y^2-1=0\)。

**隱函数**（）是由隱式方程所隱含定義的[函數](../Page/函數.md "wikilink")，比如\(y=\sqrt{1-x^2}\)是由\(x^2+y^2-1=0\)確定的函數。而可以直接用含自变量的算式表示的函数称为显函数，也就是通常所说的[函数](../Page/函数.md "wikilink")，如\(y=\cos(x)\)。

[隱函數定理說明了隱式方程在什麼情況下會確定出隱函數](../Page/隱函數定理.md "wikilink")。

## 例子

### 反函数

隐函数的一个常见类型是[反函数](../Page/反函数.md "wikilink")。若*f*是一个函数，那么*f*的反函数记作*f*<sup>−1</sup>,
是给出下面方程解的函数
...
1.  [对数函数](../Page/对数函数.md "wikilink") ln(*x*) 给出方程*x* −
    *e*<sup>*y*</sup> = 0或等价的*x* = *e*<sup>*y*</sup>的解 *y* = ln(*x*)。 这里
    *f*(*y*) = *e*<sup>*y*</sup> 并且 *f*<sup>−1</sup>(*x*) = ln(*x*)。
2.  [朗伯W函數則可以解出](../Page/朗伯W函數.md "wikilink") *x* − *y*
    *e*<sup>*y*</sup> = 0的*y*值。
...
代數函數在[數學分析和](../Page/數學分析.md "wikilink")[代数几何中扮演重要角色](../Page/代数几何.md "wikilink")，我們再拿單位圓方程式來當作代數函數的範例：
...
對於*y*的二次、三次和四次方程，可以找到只包含有限次四則運算和開方運算的顯函數解,
但這并不适用于包括五次在内的更高次数的方程（參見[阿贝尔-鲁菲尼定理](../Page/阿贝尔-鲁菲尼定理.md "wikilink")），例如：
...
### 方法一
  ...
  - 把n元隐函数看作(n+1)元函数，通过[多元函数的](../Page/多元函数.md "wikilink")[偏导数的商求得n元隐函数的导数](../Page/偏导数.md "wikilink")。
...
1.兩邊皆取其相應的[導數](../Page/導數.md "wikilink")，得出

...
[SSH](https://zh.wikipedia.org/wiki/SSH "wikilink") 隧道可以通過[加密頻道將](https://zh.wikipedia.org/wiki/加密 "wikilink")[明文流量導入隧道中](https://zh.wikipedia.org/wiki/明文 "wikilink")。為了建立 SSH 隧道， SSH 客戶端要設定并轉交一個特定本地埠號到遠端機器上。一旦 SSH 隧道建立，使用者可以連到指定的本地埠號以存取網路服務。本地埠號不用與遠地埠號一樣。

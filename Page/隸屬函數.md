**隸屬函數**（**membership
function**）也稱為**歸屬函數**或**模糊元函數**，是[模糊集合中會用到的函數](https://zh.wikipedia.org/wiki/模糊集合 "wikilink")，是一般[集合中](https://zh.wikipedia.org/wiki/集合 "wikilink")[指示函數的一般化](https://zh.wikipedia.org/wiki/指示函數 "wikilink")\[1\]。指示函數可以說明一個集合中的元素是否屬於特定子集合。一元素的指示函數的值可能是0或是1，而一元素的隸屬函數會是0到1之間的數值，表示元素屬於某模糊集合的「真實程度」（degree
of truth）。
...
有時\[2\]會用一個更通用的定義，隸屬函數的值可以是任意的固定代數或是[数学结构中取值](https://zh.wikipedia.org/wiki/数学结构 "wikilink")\(L\)，一般會要求\(L\)至少是[偏序关系或是](../Page/偏序关系.md "wikilink")[格

在[向量分析](../Page/向量分析.md "wikilink")中，**雅可比矩阵**(也稱作**Jacobi矩陣**)是函數的一阶[偏导数](../Page/偏导数.md "wikilink")以一定方式排列成的矩阵，當其為方陣(square matrix)時，其[行列式](../Page/行列式.md "wikilink")称为**Jacobi行列式**。要注意的是，如果雅可比矩陣為方陣，那在英文中雅可比矩陣跟Jacobi行列式兩者都稱作 **Jacobian**。
...
此矩陣的第 \(i\)列是由函數 \(f_i\) 的梯度函数所表示的，\(1\le i\le m\)。

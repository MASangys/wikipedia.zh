在[幾何學中](https://zh.wikipedia.org/wiki/幾何學 "wikilink")，**雙四角錐**是指以[四邊形做為基底的雙錐體](../Page/四邊形.md "wikilink")，由於雙錐體是由二個錐體疊起來的，因此不存在底面，因此只能討論其基底之形狀。當基底的形狀為正方形時會成為雙正四角錐又稱為正四角雙錐。若基底的形狀為正方形且每個面皆為正三角形則為正八面體。所有四角柱都有8個面6個頂點和12個邊。對偶多面體是[四角柱](../Page/四角柱.md "wikilink")。

<table>
<tbody>
<tr class="odd">
<td><p><a href="https://zh.wikipedia.org/wiki/File:Quadrilateral_bipyramid.png" title="fig:Quadrilateral_bipyramid.png">Quadrilateral_bipyramid.png</a><br />
只要基底是四邊形皆稱為雙四角錐</p></td>
</tr>
</tbody>
</table>

## 正四角雙錐

基底為正方形和且每一個面皆為正三角形的雙錐體稱為正四角雙錐，即是正八面體，是[柏拉圖立體之一](https://zh.wikipedia.org/wiki/柏拉圖立體 "wikilink")。

<table>
<tbody>
<tr class="odd">
<td><p><a href="https://zh.wikipedia.org/wiki/File:Octahedron.png" title="fig:Octahedron.png">Octahedron.png</a><br />
正四角雙錐</p></td>
</tr>
</tbody>
</table>

## 長方雙錐

基底為長方形的雙四角錐稱為長方雙錐。

<table>
<tbody>
<tr class="odd">
<td><p><a href="https://zh.wikipedia.org/wiki/File:Rectangular_bipyramid.png" title="fig:Rectangular_bipyramid.png">Rectangular_bipyramid.png</a><br />
長方雙錐</p></td>
</tr>
</tbody>
</table>

## 梯形雙錐

基底是梯形的雙四角錐稱為梯形雙錐

<table>
<tbody>
<tr class="odd">
<td><p><a href="https://zh.wikipedia.org/wiki/File:Trapezoid_bipyramid.png" title="fig:Trapezoid_bipyramid.png">Trapezoid_bipyramid.png</a><br />
梯形雙錐</p></td>
</tr>
</tbody>
</table>

## 凹雙四角錐

[Concave_quadrilateral_bipyramid.png](https://zh.wikipedia.org/wiki/File:Concave_quadrilateral_bipyramid.png "fig:Concave_quadrilateral_bipyramid.png")
凹雙四角錐是指有一個角大於180度的雙四角錐，通常凹雙四角錐都是因為基底為凹四邊形才會構成

## 相關多面體與鑲嵌

<table>
<caption>半正方形二面體球面多面體</caption>
<thead>
<tr class="header">
<th><p>：<a href="../Page/二面體群.md" title="wikilink">[4,2</a>], (*422)</p></th>
<th><p>[4,2]<sup>+</sup>, (422)</p></th>
<th><p>[1<sup>+</sup>,4,2], (222)</p></th>
<th><p>[4,2<sup>+</sup>], (2*2)</p></th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td></td>
<td></td>
<td></td>
<td></td>
</tr>
<tr class="even">
<td><p><a href="https://zh.wikipedia.org/wiki/File:Tetragonal_dihedron.png" title="fig:Tetragonal_dihedron.png">Tetragonal_dihedron.png</a></p></td>
<td><p><a href="https://zh.wikipedia.org/wiki/File:Octagon.svg" title="fig:Octagon.svg">Octagon.svg</a></p></td>
<td><p><a href="https://zh.wikipedia.org/wiki/File:Tetragonal_dihedron.png" title="fig:Tetragonal_dihedron.png">Tetragonal_dihedron.png</a></p></td>
<td><p><a href="https://zh.wikipedia.org/wiki/File:Spherical_square_prism.png" title="fig:Spherical_square_prism.png">Spherical_square_prism.png</a></p></td>
</tr>
<tr class="odd">
<td></td>
<td></td>
<td></td>
<td><p><a href="https://zh.wikipedia.org/wiki/四角柱#正四角柱" title="wikilink">2t{4,2}=t{2,4}</a></p></td>
</tr>
<tr class="even">
<td><p>半正對偶</p></td>
<td></td>
<td></td>
<td></td>
</tr>
<tr class="odd">
<td></td>
<td></td>
<td></td>
<td></td>
</tr>
<tr class="even">
<td><p><a href="https://zh.wikipedia.org/wiki/File:Spherical_square_hosohedron.png" title="fig:Spherical_square_hosohedron.png">Spherical_square_hosohedron.png</a></p></td>
<td><p><a href="https://zh.wikipedia.org/wiki/File:Spherical_octagonal_hosohedron.png" title="fig:Spherical_octagonal_hosohedron.png">Spherical_octagonal_hosohedron.png</a></p></td>
<td><p><a href="https://zh.wikipedia.org/wiki/File:Spherical_square_hosohedron.png" title="fig:Spherical_square_hosohedron.png">Spherical_square_hosohedron.png</a></p></td>
<td><p><a href="https://zh.wikipedia.org/wiki/File:Spherical_square_bipyramid.png" title="fig:Spherical_square_bipyramid.png">Spherical_square_bipyramid.png</a></p></td>
</tr>
<tr class="odd">
<td></td>
<td></td>
<td></td>
<td><p><a href="https://zh.wikipedia.org/wiki/雙四角錐" title="wikilink">V4.4.4</a></p></td>
</tr>
</tbody>
</table>

## 參見

[Category:多面體](https://zh.wikipedia.org/wiki/Category:多面體 "wikilink")

[Category:雙錐體](https://zh.wikipedia.org/wiki/Category:雙錐體 "wikilink")
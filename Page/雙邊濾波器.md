...
Filter）為使影像[平滑化的非線性濾波器](../Page/平滑.md "wikilink")， 這個想法由C.
Tomasi在1998年提出。
和傳統的影像平滑化演算法不同，雙邊濾波器除了使用像素之間幾何上的靠近程度之外，還多考慮了[像素之間的](../Page/像素.md "wikilink")[光度](../Page/光度.md "wikilink")/[色彩差異](https://zh.wikipedia.org/wiki/色彩 "wikilink")，
使得雙邊濾波器能夠有效的將影像上的[雜訊去除](../Page/雜訊.md "wikilink")，同時保存影像上的邊緣資訊。
在實作上，雙邊濾波器和[非等向性擴散及](https://zh.wikipedia.org/wiki/非等向性擴散 "wikilink")[Adaptive
...
為了使影像[平滑化](../Page/平滑.md "wikilink")，考慮像素之間的空間幾何分布情形，我們可以把[低通濾波器寫成以下形式](https://zh.wikipedia.org/wiki/低通濾波器 "wikilink")：
...
*c*在選擇上一個很好的例子是[高斯函數](https://zh.wikipedia.org/wiki/高斯函數 "wikilink")（請見[高斯模糊](../Page/高斯模糊.md "wikilink")），它在影像[平滑上經常被使用](../Page/平滑.md "wikilink")。
...
和空間關係的濾波器不同的是，光度/色彩差異函數\(s(\vec{f}(\vec{\xi}),\vec{f}(\vec{x}))\)描述的是\(\vec{\xi}\)和\(\vec{x}\)之間的[光度差異](../Page/光度.md "wikilink")。
因此差異函數\(s(\vec{f}(\vec{\xi}),\vec{f}(\vec{x}))\)的輸入為影像在\(\vec{\xi}\)和\(\vec{x}\)兩點上面的內容，也就是這兩點對於影像函數\(\vec{f}\)的輸出。
對於灰階影像，差異函數*s*的內容很單純，舉例來說，我們可以直接對兩點上的灰階亮度值做相減；而對於彩色影像而言，我們可以使用[Lab色彩空间來求得兩點之間的光度差異](../Page/Lab色彩空间.md "wikilink")。
而歸一化函數可以寫為：
...
對光度/色彩進行低通濾波本身並不具有太大的意義，因為他只不過是將[影像直方圖中聚集的部份集合在一起](https://zh.wikipedia.org/wiki/影像直方圖 "wikilink")，也就是說讓影像直方圖中相似的色彩更向區域的平均靠近，色彩在空間中的分布情形並不會有任何的影響。
然而，如果同時考慮對空間幾何的濾波行為\(c(\vec{\xi},\vec{x})\)，
\(s(\vec{f}(\vec{\xi}),\vec{f}(\vec{x}))\)所代表的意義便成為，將影像平滑化的範圍限制在光度/色彩差異較小的部分，如此便能夠達到邊緣保存的目的。

### 離散形式

一般來說，在[图像处理上](../Page/图像处理.md "wikilink")，影像的[像素在空間上的分佈是離散的](../Page/像素.md "wikilink")，光度/色彩的儲存形式也是離散的。因此，雙邊濾波器常用的為其離散形式：

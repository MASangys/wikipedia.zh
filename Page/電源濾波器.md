...
電源濾波器的目的是在抑制電磁雜訊，雜訊的影響可分為以下二種：
  ...
  - 發射（Emissions）：是要將由設備產生，影響電源或其他設備的雜訊降到法規（例如FCC part
    15）允許值以下，例如由[開關電源產生的雜訊](https://zh.wikipedia.org/wiki/開關電源 "wikilink")。
...
若保護等級Class
I的設備中，接地保護的導體沒有導通，則設備的金屬外殼會有電流流到地。這是Y電容產生的[電容分壓的結果](https://zh.wikipedia.org/wiki/電容分壓器 "wikilink")，電源對地的電壓約為115伏。一般醫療用的設備其漏電流需小於0.5

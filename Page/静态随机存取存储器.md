...
SRAM用于：
  ...
  - \-{zh-hans:集成; zh-hant:整合}-于芯片内
      ...
      - 作为强大的微处理器的主caches，如[x86系列与许多其它CPU](https://zh.wikipedia.org/wiki/x86 "wikilink")（从8 [kiB到几百萬字节的量级](https://zh.wikipedia.org/wiki/kiB "wikilink")）
...
[非揮發性SRAM](https://zh.wikipedia.org/wiki/非揮發性SRAM "wikilink")（Non-volatile SRAM，nvSRAM）具有SRAM的标准功能，但在失去电源供电时可以保住其-{zh-hans:数据; zh-hant:資料}-。非揮發性SRAM用于网络、航天、医疗等需要关键场合\[10\]—保住-{zh-hans:数据; zh-hant:資料}-是关键的而且不可能用上电池。

**非均勻取樣**是取樣定理下的產物，藉由[拉格朗日插值法及均勻取樣理論的關係](../Page/拉格朗日插值法.md "wikilink")，來達成滿足[取樣定理的概括化型態](https://zh.wikipedia.org/wiki/取樣定理 "wikilink")。取樣定理限制了在對連續訊號取樣時的條件，如[奈奎斯特準則](https://zh.wikipedia.org/wiki/取樣定理#夏農的原始證明 "wikilink")，以避免取樣後的重構時產生訊號缺陷。而非均勻取樣則是在不同時間有不同的取樣間隔，但平均下來，整段取樣滿足取樣定理的限制，根據取樣定理的回推，這樣的作法在有限頻寬訊號重構時不會造成缺陷。因此，雖然均勻取樣在訊號重構時較簡單，完整的重構訊號卻不見得一定要用均勻取樣來的訊號才能達到。
...
## [取樣混疊](https://zh.wikipedia.org/wiki/取樣定理#混疊 "wikilink")
...
[居中](https://zh.wikipedia.org/wiki/File:Nonuniform_Sampling1.png "fig:居中")
...
[居中](https://zh.wikipedia.org/wiki/File:Nonuniform_Sampling2.png "fig:居中")
...
<!-- end list -->
  ...
  -
    \(\text{ where }G(t)=(t-t_0)\prod_{k=1}^\infty \left(1-\frac{t}{t_k}\right)\left(1-\frac{t}{t_{-k}}\right),\)
    \(B^2_\sigma.\)是Bernstein
    space，\(f(t)\)均勻[收斂於](https://zh.wikipedia.org/wiki/收斂 "wikilink")[緊緻集](https://zh.wikipedia.org/wiki/緊空間 "wikilink")。

以上稱為**培力-威納-萊文森定理**，為將WSK定理從均勻取樣時間拓展至非均勻取樣時間的定裡。



[Category:Digital_signal_processing](https://zh.wikipedia.org/wiki/Category:Digital_signal_processing "wikilink")

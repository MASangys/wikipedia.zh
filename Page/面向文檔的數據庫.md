**面向文档的数据库**（）或**文档存储**，是用于存储、检索和管理面向文档的信息的一种[计算机程序](../Page/计算机程序.md "wikilink")。这里称为文档的是[半结构化数据](https://zh.wikipedia.org/wiki/半结构化数据 "wikilink")\[1\]，它是不完全形式的[结构化数据](https://zh.wikipedia.org/wiki/数据模型 "wikilink")，不服从于与[关系数据库或其他形式的](../Page/关系数据库.md "wikilink")[数据表有关联的](https://zh.wikipedia.org/wiki/数据库表 "wikilink")[数据模型的形式结构](https://zh.wikipedia.org/wiki/数据模型 "wikilink")，然而却包含[标签或其他标记](https://zh.wikipedia.org/wiki/标签_\(元数据\) "wikilink")，用以在数据内分割语义元素和强制记录和字段的层级，因此它也叫做自我描述结构。

## 概述

面向文档的数据库是[NoSQL数据库的主要类别之一](../Page/NoSQL.md "wikilink")，且“面向文档的数据库”一词的普及程度也因术语[NoSQL的使用而有所增长](../Page/NoSQL.md "wikilink")\[2\]
。[XML数据库是针对](https://zh.wikipedia.org/wiki/XML数据库 "wikilink")[XML文档做了优化的面向文档的数据库的子类](../Page/XML.md "wikilink")。[图数据库与之相似](../Page/图数据库.md "wikilink")，但其添加了一个连接各文档的“联系”层，以便进行快速遍历。一些[搜索引擎](../Page/搜索引擎.md "wikilink")（也称为[信息检索](https://zh.wikipedia.org/wiki/信息检索 "wikilink")）系统如[Elasticsearch提供了足够的对文档的核心操作从而已经满足了面向文档数据库的定义](../Page/Elasticsearch.md "wikilink")。

[键-值数据库是NoSQL数据库的另一概念](https://zh.wikipedia.org/wiki/键-值数据库 "wikilink")，面向文档的数据库是[键-值数据库的子类](https://zh.wikipedia.org/wiki/键-值数据库 "wikilink")。它们的差别在于处理数据的方式：在键值数据库中，数据是对数据库不透明的；而面向文档的数据库系统依赖于“文档”的内部结构来获取[元数据](../Page/元数据.md "wikilink")，数据库引擎使用这些[元数据进行更深层次的优化](../Page/元数据.md "wikilink")。虽然由于系统中的工具使这一差别不甚明显，但在设计概念上，这种文档存储方式利用了现代程序技术来提供更丰富的体验。现代键-值存储经常包括处理元数据的特征，模糊了它与文档存储之间的界线。

文档数据库与传统的[关系数据库差异显著](../Page/关系数据库.md "wikilink")。关系数据库通常将数据存储在相互独立的表中，这些[表由程序开发者定义](https://zh.wikipedia.org/wiki/数据库表 "wikilink")，一个单一的对象可能散布在若干表中。
对于一个给定对象，文档数据库将其所有信息存储在数据库内某个单一实例中，并且存储的每一个对象可以不同于任一其它对象。这简化了将对象装载入数据库的过程，通常能消除对[对象关系映射等类似方案的需求](../Page/对象关系映射.md "wikilink")。文档数据库不同于关系数据库，关系数据库基于了[关系模型](../Page/关系模型.md "wikilink")，而文档数据库采用了[半结构化模型](https://zh.wikipedia.org/wiki/半结构化模型 "wikilink")，没有在数据和[模式](../Page/Schema_\(数据库\).md "wikilink")（schema）之间的分离，使用的结构的数量依赖于目标用途。在[半结构化数据中](https://zh.wikipedia.org/wiki/半结构化数据 "wikilink")，属于相同[类的](../Page/类_\(计算机科学\).md "wikilink")[实体可有不同的](https://zh.wikipedia.org/wiki/实体 "wikilink")[属性](../Page/属性.md "wikilink")，即使它们被分组在一起，并且属性的次序是不重要的。

## 文档

面向文档数据库的中心概念是“文档”这个概念。尽管每个面向文档数据库实现在这个定义的细节上都有所不同，一般而言，它们都假定文档以某种标准[格式或](https://zh.wikipedia.org/wiki/文档文件格式 "wikilink")[编码来封装和编码数据](https://zh.wikipedia.org/wiki/序列化#序列化格式 "wikilink")（或信息）。面向文档数据库使用的编码包括[XML](../Page/XML.md "wikilink")、[YAML](../Page/YAML.md "wikilink")、[JSON和](../Page/JSON.md "wikilink")[BSON](../Page/BSON.md "wikilink")，有的实现还可以存储二进制文档格式如[PDF和Microsoft](https://zh.wikipedia.org/wiki/PDF "wikilink")
Office文档（MS
[Word](https://zh.wikipedia.org/wiki/Word "wikilink")、[Excel之类](https://zh.wikipedia.org/wiki/Excel "wikilink")）。
...
文档存储和键-值存储有很大差异。在理论上，在键-值存储中值对于存储是不透明的，它们本质上是黑箱子。它们可以提供类似于文档存储的的查找系统，但是对内容的组织有着更少的理解。文档存储使用在文档中的元数据来分类内容，例如允许它们理解一个数字序列是电话号码，而另一个是邮政编码。这允许它们在这种类型的数据上进行查找，例如，包含555的所有电话号码，但忽略邮编
55555。
...
<td><p>分布式面向文档XML/JSON数据库平台，有着服从<a href="../Page/ACID.md" title="wikilink">ACID的</a><a href="../Page/事务处理.md" title="wikilink">事务</a>；<a href="../Page/高可用性.md" title="wikilink">高可用性</a><a href="https://zh.wikipedia.org/wiki/data_replication" title="wikilink">数据复制和</a><a href="https://zh.wikipedia.org/wiki/Shard_(database_architecture)" title="wikilink">分片</a>；带有<a href="https://zh.wikipedia.org/wiki/相关性" title="wikilink">相关性</a><a href="https://zh.wikipedia.org/wiki/排名" title="wikilink">排名的内建</a><a href="https://zh.wikipedia.org/wiki/全文查找" title="wikilink">全文查找引擎</a>；JS/SQL<a href="https://zh.wikipedia.org/wiki/查询语言" title="wikilink">查询语言</a>；<a href="../Page/地理信息系统.md" title="wikilink">GIS</a>；可获得为每使用付费<a href="https://zh.wikipedia.org/wiki/云数据库" title="wikilink">云数据库或作为一个on</a>-premise免费软件下载。</p></td>
<td></td>
</tr>
<tr class="odd">
...
<td><p>JSON于REST/HTTP上，有着<a href="../Page/多版本并发控制.md" title="wikilink">多版本并发控制和有限的</a><a href="../Page/ACID.md" title="wikilink">ACID特性</a>。对视图和查询使用<a href="https://zh.wikipedia.org/wiki/map_(高阶函数)" title="wikilink">map和</a><a href="https://zh.wikipedia.org/wiki/fold_(高阶函数)" title="wikilink">reduce</a>。[6]</p></td>
<td><p>[7]</p></td>
</tr>
<tr class="odd">
...
<td><p>分布式面向文档数据库，对于 JSON, XML, 和<a href="https://zh.wikipedia.org/wiki/资源描述框架" title="wikilink">RDF triples</a>。内建<a href="https://zh.wikipedia.org/wiki/全文查找" title="wikilink">全文查找</a>, <a href="../Page/ACID.md" title="wikilink">ACID事务</a>, <a href="../Page/高可用性.md" title="wikilink">高可用性和</a><a href="https://zh.wikipedia.org/wiki/灾难恢复" title="wikilink">灾难恢复</a>, 证书安全性。</p></td>
<td></td>
</tr>
<tr class="even">
...
<td><p>文档数据库带有复制和分片, <a href="../Page/BSON.md" title="wikilink">BSON存储</a>（二进制格式<a href="../Page/JSON.md" title="wikilink">JSON</a>）。</p></td>
<td><p>[15]</p></td>
</tr>
<tr class="odd">
...
<td><p><a href="../Page/中间件.md" title="wikilink">中间件和</a><a href="https://zh.wikipedia.org/wiki/数据库引擎" title="wikilink">数据库引擎混合</a></p></td>
<td></td>
</tr>
<tr class="even">
...
<td></td>
<td><p><a href="../Page/Java.md" title="wikilink">Java</a></p></td>
<td><p>JSON在HTTP之上, SQL支持, <a href="../Page/ACID.md" title="wikilink">ACID事务</a></p></td>
<td></td>
</tr>
<tr class="odd">
...
<td><p>分布式面向文档<a href="../Page/JSON.md" title="wikilink">JSON数据库带有复制和分片</a>。</p></td>
<td></td>
</tr>
<tr class="even">
...
<td></td>
<td><p>类<a href="../Page/SQL.md" title="wikilink">SQL语言</a></p></td>
<td><p><a href="../Page/ACID.md" title="wikilink">ACID事务支持</a>, 唯<a href="../Page/JSON.md" title="wikilink">JSON</a></p></td>
<td></td>
</tr>
<tr class="odd">
...
<td><p><a href="../Page/MongoDB.md" title="wikilink">MongoDB带有</a><a href="https://zh.wikipedia.org/wiki/分形树索引" title="wikilink">分形树索引</a></p></td>
<td></td>
</tr>
</tbody>
</table>
...
1.
2.  [DB-Engines Ranking per database model
    category](http://db-engines.com/en/ranking_categories)
3.
4.  [1](https://www.arangodb.com)
5.  [Documentation](http://www.couchbase.com/docs/) . Couchbase.
    Retrieved on 2013-09-18.
6.  [CouchDB Overview](http://couchdb.apache.org/docs/overview.html)
7.
8.
9.  [eXist-db Open Source Native XML Database](http://exist-db.org).
    Exist-db.org. Retrieved on 2013-09-18.
10.
11.
12.
13.
14.
15. [MongoDB REST
    Interfaces](http://www.mongodb.org/display/DOCS/Http+Interface#HttpInterface-RESTInterfaces)
16.

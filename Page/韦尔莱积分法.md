**韦尔莱算法**是一种用于求解牛顿运动方程的数值方法，被广泛应用于[分子动力学](../Page/分子动力学.md "wikilink")模拟以及[视频游戏中](https://zh.wikipedia.org/wiki/视频游戏 "wikilink")。韦尔莱算法的优点在于：数值稳定性比简单的欧拉方法高很多，并保持了物理系统中的时间可逆性与相空间体积元体积守恒的性质。
...
\[r(t+2\Delta t)=r(t+\Delta t)+v(t+\Delta t)\Delta t+\frac{f(t+\Delta t)}{2m}\Delta t^{2}\] 对两式相减可得

\[r(t+2\Delta t)+r(t)=2r(t+\Delta t)+\left[v(t+\Delta t)-v(t)\right]\Delta t+\frac{f(t+\Delta t)-f(t)}{2m}\Delta t^{2}\] 将最初的 Verlet 公式中的 \(t\) 换为 \(t+\Delta t\) ，

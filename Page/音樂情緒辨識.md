...
SVR與SVM的概念類似。不同之處在於SVM所找的平面將空間一分為二，但SVR要找的是能準確預測資料分布的平面。 機器學習後，通常使用[交叉驗證](../Page/交叉驗證.md "wikilink")來對準確度加以驗證。交叉驗證主要是將資料庫隨機分成數個不重疊的部分，每次提取一個部分做測試資料，剩餘的作為訓練資料。其作法是基於：

...
[編譯器](../Page/編譯器.md "wikilink")可能使用值的靜態型別以最佳化所需的儲存區，並選取對值運算時的較佳演算法。例如，在許多[C](../Page/C.md "wikilink")編譯器中，「浮點數」[資料型別是以](https://zh.wikipedia.org/wiki/資料型別 "wikilink")32 [位元](../Page/位元.md "wikilink")表示，與[IEEE 754規格一致的單精度浮點數](../Page/IEEE_754.md "wikilink")。因此，在數值運算上，C應用了浮點數規範（浮點數加法、乘法等等）。

在[计算机科学中](../Page/计算机科学.md "wikilink")，**-{A|zh:型別;zh-tw:型別;zh-cn:类型;}-系統**用于定義如何將[程式語言中的](https://zh.wikipedia.org/wiki/程式語言 "wikilink")[數值和](https://zh.wikipedia.org/wiki/數值 "wikilink")[-{A归類为许多不同的](https://zh.wikipedia.org/wiki/運算式 "wikilink")**[型別](https://zh.wikipedia.org/wiki/型別 "wikilink")**，如何操作这些型別，这些型別如何互相作用。型別可以确认一个值或者一组值具有特定的意义和目的（雖然某些型別，如抽象型別和-{A|zh:函式;zh-tw:函式;zh-cn:函数}-型別，在-{A|zh:程式;zh-tw:程式;zh-cn:程序}--{A|zh:執行;zh-tw:執行;zh-cn:运行}-中，可能不表示為值）。型別系統在各種語言之間有非常大的不同，也許，最主要的差異存在於編譯時期的語法，以及執行時期的操作实现方式。

[編譯器可能使用值的靜態型別以最佳化所需的儲存區](../Page/編譯器.md "wikilink")，並選取對值運算時的較佳演算法。例如，在許多[C編譯器中](../Page/C.md "wikilink")，「浮點數」[資料型別是以](https://zh.wikipedia.org/wiki/資料型別 "wikilink")32
[位元表示](../Page/位元.md "wikilink")，與[IEEE
754規格一致的單精度浮點數](../Page/IEEE_754.md "wikilink")。因此，在數值運算上，C應用了浮點數規範（浮點數加法、乘法等等）。
...
<!-- end list -->
  ...
  -
    使用型別可允許[編譯器偵測無意義的](../Page/編譯器.md "wikilink")，或者是可能無效的代碼。例如，可以識出一個無效的運算式`"Hello,
    World"
    + 3`，因為不能對（在平常的直覺中）[逐字字串加上一個](https://zh.wikipedia.org/wiki/逐字字串 "wikilink")[整數](https://zh.wikipedia.org/wiki/整數 "wikilink")。強型別提供更多的安全性，但它並不能保證絕對安全（詳情請見[型別安全](../Page/型別安全.md "wikilink")）。
...
**型別檢查**所進行的檢驗處理以及實行型別的約束，可發生在[編譯時期](https://zh.wikipedia.org/wiki/編譯時期 "wikilink")（靜態檢查）或[執行時期](https://zh.wikipedia.org/wiki/執行時期 "wikilink")（動態檢查）。靜態型別檢查是在[編譯器所進行](../Page/編譯器.md "wikilink")[語義分析中進行的](https://zh.wikipedia.org/wiki/語義分析 "wikilink")。如果一個語言強制實行型別規則（即通常只允許以不遺失資訊為前提的自動型別轉換）就稱此處理為**強型別**，反之稱為**弱型別**。
...
術語**推斷型別**（[鸭子类型](../Page/鸭子类型.md "wikilink")，duck
typing）指的是動態型別在語言中的應用方式，它會「推斷」一個數值的型別。
...
在這個範例中，（1）宣告x；（2）將[整數值](https://zh.wikipedia.org/wiki/整數 "wikilink")5代給x；（3）將[字串值](https://zh.wikipedia.org/wiki/字串 "wikilink")"hi"代給x。在主要的靜態系統中，這個代碼片斷將會違反規則，因為（2）和（3）對
x所約束的型別相矛盾。
...
在這個程式片斷中，（1）將數值5約束給x；（2）將數值"hi"約束給y；以及（3）嘗試將x加到y。在動態型別語言中，約束給x的值會是一對`（`[`整數`](https://zh.wikipedia.org/wiki/整數 "wikilink")`, 5）`，且約束給y的值會是一對`（`[`字串`](https://zh.wikipedia.org/wiki/字串 "wikilink")`,
"hi"）`。當這個程式嘗試執行第3行時，語言對型別標記`整數`和`字串`進行檢查，如果這兩個型別的`+`（加法）運算尚未定義，就會發出一個錯誤。
...
靜態型別通常可以編譯出速度較快的代碼。當編譯器清楚知道所要使用的資料型別，就可以產生最佳化過後的機器碼。更進一步，靜態型別語言中的編譯器，可以更輕易地發現較佳捷徑。某些動態語言（如[Common
Lisp](../Page/Common_Lisp.md "wikilink")）允許任意型別的宣告，以便於最佳化。以上理由使靜態型別更為普及。參閱[最佳化](https://zh.wikipedia.org/wiki/最佳化 "wikilink")。
...
動態型別允許建構一些靜態型別系統所做不出來的東西。例如，*[eval](https://zh.wikipedia.org/wiki/eval "wikilink")*函式，它使得執行任意資料作為代碼成為可能（不過其代碼的型別仍是靜態的）。此外，動態型別容納過渡代碼和原型設計，如允許使用字串代替資料結構。靜態型別語言最近的增強（如Haskell
[一般化代數資料型別](https://zh.wikipedia.org/wiki/GADT "wikilink")）允許eval函式以型別安全的方式撰寫。

動態型別使[元程式設計更為強大](https://zh.wikipedia.org/wiki/元程式設計 "wikilink")，且更易於使用。例如[C++模板的寫法](../Page/C++.md "wikilink")，比起等價的[Ruby或](../Page/Ruby.md "wikilink")[Python寫法要來的麻煩](../Page/Python.md "wikilink")。更高度的執行時期構成物，如[元類別](https://zh.wikipedia.org/wiki/元類別 "wikilink")（metaclass）和[內觀](https://zh.wikipedia.org/wiki/內觀 "wikilink")（Introspection），對靜態型別語言而言通常更為困難。
...
在弱型別語言中編寫上述代碼，並不清楚將會得到哪一種結果。某些語言如[Visual
Basic](../Page/Visual_Basic.md "wikilink")，將會產生可以運作的代碼，它將會給出的結果是42：系統將字串"37"轉換成數字37，以符合運算上的直覺；其它的語言，像[JavaScript將會產生的結果是](../Page/JavaScript.md "wikilink")"537"：系統將數字5轉換成字串"5"並把兩者串接起來。在Visual
Basic和JavaScript中，最終的型別是以那兩個[運算元為考量的規則所決定](https://zh.wikipedia.org/wiki/運算元 "wikilink")。在部分語言中，如[AppleScript](../Page/AppleScript.md "wikilink")，某個值最終的型別，只以最左邊的運算元的型別所決定。
...
在一個如[Visual
Basic的語言中](../Page/Visual_Basic.md "wikilink")，例子中的變數z得到的值為42。不管編寫者有沒有這個意圖，該語言定義了明確的結果，且程式不會就此崩潰，或將不明定義的值賦給z。就這方面而言，這樣的語言就是型別安全的。
...
推斷型別（[鸭子类型](../Page/鸭子类型.md "wikilink")，Duck typing）最初是由[Dave
Thomas在](https://zh.wikipedia.org/wiki/Dave_Thomas "wikilink")[Ruby社群中提出的](../Page/Ruby.md "wikilink")，推斷型別用了這個論證法「如果它像什麼，而且其它地方也像什麼，那麼它就是什麼。」

在某些程式設計環境中，兩個物件可以有相同的型別，即使它們沒有什麼交集。一個例子是[C++在](../Page/C++.md "wikilink")[迭代器和指標之間的雙重性](../Page/迭代器.md "wikilink")。兩者皆以不甚相同的機制實作並提供一個\*
運算。
...
*型別的型別*是一種**種類**。在[型別程式設計中有明確的種類](https://zh.wikipedia.org/wiki/型別程式設計 "wikilink")，如[Haskell程式語言的](../Page/Haskell.md "wikilink")*型別建構子*，在申請比較簡單的型別之後，其返回一個簡單的型別。例如，型別建構子*二選一*有這些種類*\*
-\> \* -\>
\**（*\**代表種類），而且它的申請*二選一字串整數*是一個簡單的型別。然而，大多數程式語言的型別，是由編寫者來暗示或*寫死*，這就並未將*種類*的概念用作為首選層。
...
<!-- end list -->
  ...
  -
    由基本型別組合成的型別，例如：[陣列或](https://zh.wikipedia.org/wiki/陣列 "wikilink")[記錄單元](https://zh.wikipedia.org/wiki/物件_\(電腦科學\) "wikilink")。[抽象資料型別具有複合型別和界面兩種屬性](../Page/抽象資料型別.md "wikilink")，這取決於你提及哪一個。
...
對於靜態型別語言的型別檢查器，必須檢驗所有[運算式的型別](https://zh.wikipedia.org/wiki/運算式 "wikilink")，是否與前後文所期望的型別一致。例如[指派語句](https://zh.wikipedia.org/wiki/指派 "wikilink")` x
:=
 `*`e`*，推斷運算式*e*的型別，必定與宣告或推斷的變數型別`x`一致。這個一致性的概念，就稱為*相容性*，是每一個程式語言所特有的。

很明顯，如果*e*和`x`的型別相同，就允許指派，然後這是一個有效的運算式。因此在最簡單的型別系統中，問題從兩個型別是否相容，簡化為兩個型別是否*相等*（或*等價*）。然而不同的語言對於兩個型別運算式是否理解為表示了相同型別，有著不同的標準。型別的*相等理論*的差異相當巨大，兩個極端的例子是*[結構型別系統](https://zh.wikipedia.org/wiki/結構型別系統 "wikilink")*（Structural
type
system），任兩個以相同結構所描述的值的型別都是等價的，且在*[標明型別系統](https://zh.wikipedia.org/wiki/標明型別系統 "wikilink")*（Nominative
type system）上，沒有兩個獨特的語法構成的型別運算式表示同一型別，（*即*型別若要相等，就必須具有相同的「名字」）。
...
## 參閱
  ...
  - [Type
    signature](https://zh.wikipedia.org/wiki/Type_signature "wikilink")
...
1.  Harper, Robert & Benjamin C. Pierce (2005), "Design Considerations
    for ML-Style Module Systems", in Pierce, Benjamin C., *Advanced
    Topics in Types and Programming Languages*, Cambridge, MA: MIT
    Press, ISBN
    [0262162288](http://en.wikipedia.org/w/index.php?title=Special:Booksources&isbn=0262162288)

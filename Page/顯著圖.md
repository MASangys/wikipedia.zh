在[電腦視覺中](https://zh.wikipedia.org/wiki/電腦視覺 "wikilink")，顯著性是一種圖像分割的模式，而**顯著圖**（）是顯示每個[像素獨特性的圖像](../Page/像素.md "wikilink")。顯著圖的目標在於將一般圖像的表示簡化或是改變為更容易分析的樣式。舉例來說，某個像素在一張[彩色圖中具有較高的](https://zh.wikipedia.org/wiki/彩色圖 "wikilink")[灰階](https://zh.wikipedia.org/wiki/灰階 "wikilink")，其會在顯著圖中以較為明顯的方式被顯示出來。

視覺刺激的觀點上，如果某些特徵特別能夠被捕捉到[注意力](https://zh.wikipedia.org/wiki/注意力 "wikilink")，這樣子的特性在[心理學上被稱為顯著性](https://zh.wikipedia.org/wiki/心理學 "wikilink")()。

## 圖像顯著性

如前述，顯著性是[圖像分割的一部分](../Page/图像分割.md "wikilink")，圖像分割通常用於定位圖像中的物體和邊界(例：線條，曲線等）。更精確來說，圖像分割是為圖像中的每個[像素](../Page/像素.md "wikilink")(pixel)分配一個標籤的過程，以便讓相同標籤的像素能夠共享某些特徵。

圖像顯著性是圖像中重要的視覺特徵，體現出人眼對圖像各區域的重視程度，自從[Laurent
Itti在](https://zh.wikipedia.org/wiki/Laurent_Itti "wikilink")1998年發表的論文\[1\]後，產生了大量的顯著性映射方法。圖像顯著性也廣泛運用在壓縮，編碼，圖像邊緣加強，顯著性目標分割和提取等方面。

## 視覺顯著性檢測\[2\]

視覺注意機制，即面對一個場景時，人類自動地對感興趣區域進行處理而選擇性地忽略不感興趣區域，這些人們感興趣區域被稱之為顯著性區域。

[視覺顯著性檢測(英語：Visual Saliency
Detection)](https://zh.wikipedia.org/wiki/視覺顯著性檢測 "wikilink")，指透過演算法模擬人的視覺，提取出圖像中的顯著性區域。
...
1.  良好的檢測率：要有較低丟失實際顯著區域的可能性以及將背景錯誤地標記為顯著區域的錯誤發生。
2.  高分辨率：產生的顯著圖需要具有高分辨率以準確定位顯著物體並保留原始圖像訊息。
3.  計算效率：作為其他復雜流程的起始點，這些模型應該要能夠快速檢測顯著區域。

### 認知注意模型

Itti於1998年提出基於顯著性的視覺注意模型，而後在2001年對該模型的理論作了更進一步的完善，現今，Itti的模型已經成為由下而上視覺注意模型的標準。

對於一幅輸入的圖像，該模型提取初級視覺特徵：顏色、亮度和方位、在多種尺度下使用中央環繞(Center-surround)操作產生體現顯著性量值的特徵圖，將這些特徵圖合併得到最終的顯著圖(Saliency
map)後,利用生物學中贏者取全(Winner-take-all)的競爭機制得到圖像中最顯著的空間位置,
用來引導注意，最後採用返回抑制(Inhibition of return)
的方法來完成注意焦點的轉移。
...
這是一種基於頻譜分析的顯著性模型，非常容易解釋和實現，由於其理論基於[快速傅里葉轉換實現](https://zh.wikipedia.org/wiki/快速傅利葉轉換 "wikilink")，能夠滿足實時(real-time)要求，此模型在注意焦點預測和顯著區域檢測方面取得了很大的成功，和iNVT類似的模型相比，運算速度可提高近10倍。
...
SALS(*I<sub>k</sub>*) = |*I<sub>k</sub>* - *I*<sub>1</sub>| +
|*I<sub>k</sub>* - *I*<sub>2</sub>| + ... + |*I<sub>k</sub>* -
*I<sub>N</sub>*|

其中N是當前幀中的像素總和，若更進一步的擴展公式，將同樣的值放在一起，得到結果如下：

SALS(*I<sub>k</sub>*) = ∑ *F<sub>n</sub>* × |*I<sub>m</sub>* -
*I<sub>n</sub>*|

F<sub>n</sub>代表I<sub>n</sub>的頻率，且n的範圍在\[0,255\]，頻率以[直方圖的形式表示](../Page/直方图.md "wikilink")，此演算法的時間複雜度為O(N)。

### 虛擬碼

以pseudo [matlab](../Page/MATLAB.md "wikilink") code為例，首先讀取數據。
...
在讀取數據之後，對每一幀進行超像素([super-pixel](https://zh.wikipedia.org/wiki/super-pixel "wikilink"))處理，spnum1和spnum2分別表示當前幀與前一個幀的像素總數。
...
之後計算每個像素之間的顏色距離，這流程稱為[契約函數](https://zh.wikipedia.org/wiki/契約函數 "wikilink")。

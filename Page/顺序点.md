**顺序点**，也称作**序列点**，是计算机程序中一些执行点，在该点处之前的求值的所有的[副作用已经发生](../Page/函数副作用.md "wikilink")，在它之后的求值的所有副作用仍未开始。在[C与](https://zh.wikipedia.org/wiki/C语言 "wikilink")[C++程序设计语言中](../Page/C++.md "wikilink")，表达式的值依赖于它的子表达式的求值顺序。增加更多的顺序点限制了可能的求值顺序，能保证有一个一致结果。

[C++11中](../Page/C++11.md "wikilink")，顺序点概念已经被[sequenced
...
1.  `&&`（[逻辑与](../Page/逻辑与.md "wikilink")）、`||`（[逻辑或](../Page/逻辑或.md "wikilink")）、的左操作数与右操作数求值之间（前两者是[短路求值的一部分](../Page/短路求值.md "wikilink")）。例如，表达式`*p++
...
6.  [初始化的结束](../Page/初始化.md "wikilink")。例如，声明`int a = 5;`中的对`5`求值之后。
7.  [初始化列表的以逗号分割的各个初始化值](https://zh.wikipedia.org/wiki/:b:Initializer_list "wikilink")，严格遵照从左至右求值。例如：`int
    a[3] =
    {i++,j--,foo(101)};`注意，此处不是逗号运算符。（从[C++11标准指出这一顺序点](../Page/C++11.md "wikilink")）

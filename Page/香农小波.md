...
\\begin{cases} 1, & \\mbox{if } {|x| \\le 1/2}, \\\\ 0 & \\mbox{if} \\mbox{ otherwise}. \\\\ \\end{cases} </math> 实香农小波的解析表达式可由逆傅里叶变换得到：
...
是出现在[香农采样定理中的常见](https://zh.wikipedia.org/wiki/香农采样定理 "wikilink")[正弦函数](https://zh.wikipedia.org/wiki/正弦函数 "wikilink")。 该小波有\(C^\infty\)级的[可微性](https://zh.wikipedia.org/wiki/可微性 "wikilink")，但是在无穷远处缓慢减小并且没有有界支撑，因为有频带限制的信号没有时间限制。

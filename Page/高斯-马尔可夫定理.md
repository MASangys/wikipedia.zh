...
Theorem)**陈述的是：在[线性回归模型中](../Page/线性回归.md "wikilink")，如果误差满足零[均值](../Page/均值.md "wikilink")、[同方差且](../Page/协方差.md "wikilink")[互不相关](../Page/Uncorrelated_random_variables.md "wikilink")，则回归系数的最佳线性[无偏](../Page/偏差.md "wikilink")[估计](../Page/估计量.md "wikilink")(**BLUE**,
...
**高斯-马尔可夫定理的假设条件**是：
  ...
  - \({\rm E}\left(\boldsymbol{\varepsilon}\mid\mathbf{X}\right)=0\)
    ，\(\forall \mathbf{X}\)（零均值），
  - \({\rm Var}\left(\boldsymbol{\varepsilon}\mid\mathbf{X}\right)={\rm E}\left(\boldsymbol{\varepsilon}\boldsymbol{\varepsilon}^T\mid\mathbf{X}\right)=\sigma^2_\varepsilon\mathbf{I_n}\)，（同方差且不相关），其中\(\mathbf{I_n}\)为n阶[单位矩阵](../Page/单位矩阵.md "wikilink")(Identity
    Matrix)。

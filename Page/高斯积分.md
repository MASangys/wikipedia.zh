...
**高斯积分**（），有时也被称为**概率积分**，是[高斯函数](https://zh.wikipedia.org/wiki/高斯函数 "wikilink")（*e*<sup>−*x*<sup>2</sup></sup>）在整个[實數線上的](../Page/實數線.md "wikilink")[积分](../Page/积分.md "wikilink")。它是依[德国](../Page/德国.md "wikilink")[数学家兼](../Page/数学家.md "wikilink")[物理学家](../Page/物理学家.md "wikilink")[卡爾·弗里德里希·高斯之姓氏所命名](../Page/卡爾·弗里德里希·高斯.md "wikilink")。

\[\int_{-\infty}^\infty e^{-x^2}dx = \sqrt{\pi}\]

这个积分用处很广。例如，在变量略有变化的情况下，它用于计算[正态分布的](../Page/正态分布.md "wikilink")。还是这个积分，在极限为有限值的时候，与[正态分布的](../Page/正态分布.md "wikilink")[误差函数和](../Page/误差函数.md "wikilink")[累积分布函数密切相关](../Page/累积分布函数.md "wikilink")。在物理学中，这种积分经常出现，例如在[量子力学中](../Page/量子力学.md "wikilink")，为了求谐振子基态的概率密度，以及在路径积分公式中，求谐振子的传播子，我们都要用到这个积分。

尽管误差函数不存在[初等函数](../Page/初等函数.md "wikilink")，但可以通过[Risch算法证明](https://zh.wikipedia.org/wiki/Risch算法 "wikilink")，高斯积分可以通过[多元微积分方法分析求解](../Page/多元微积分.md "wikilink")。下面这个[不定积分](../Page/不定积分.md "wikilink")

\[\int e^{-x^2}\,dx,\]
无法用初等函数表示，但可以计算[定积分](https://zh.wikipedia.org/wiki/定积分 "wikilink")

\[\int_{-\infty}^\infty e^{-x^2}\,dx\]
任意[高斯函数的定积分为](https://zh.wikipedia.org/wiki/高斯函数 "wikilink")

\[\int_{-\infty}^{\infty}  e^{-a(x+b)^2}\,dx= \sqrt{\frac{\pi}{a}}.\]

在物理学中，经常用到高斯积分；而在[量子场论中会用到许多该积分的推广形式](../Page/量子场论.md "wikilink")。
...
使用[富比尼定理以上双重积分可以被看作是](../Page/富比尼定理.md "wikilink")[直角坐标系上一个顶点为](https://zh.wikipedia.org/wiki/直角坐标系 "wikilink"){(−*a*,
*a*), (*a*, *a*), (*a*, −*a*), (−*a*,
−*a*)}的正方形的面积积分\(\int e^{-(x^2+y^2)}\,d(x,y)\)。

由于对任何实数来说指数函数均大于0，因此对于这个正方形内的[内切圆的积分必须小于](../Page/内切圆.md "wikilink")\(I(a)^2\)。类似地正方形的[外接圆积分必须大于](https://zh.wikipedia.org/wiki/外接圆 "wikilink")\(I(a)^2\)。通过从直角坐标系转化到[极坐标系](../Page/极坐标系.md "wikilink")\(x=r\,\cos \theta\),
\(y= r\,\sin\theta\), \(d(x,y) = r\, d(r,\theta)\)对这两个圆面的积分可以简单地计算出来：
...
这里\(~\Gamma\)是[Γ函数](../Page/Γ函数.md "wikilink")。这说明了为什么一个半整数的[階乘是](../Page/階乘.md "wikilink")\(\sqrt \pi\)的倍数。更广义地，
...
任一[高斯函数的积分都可以用以下的公式计算](https://zh.wikipedia.org/wiki/高斯函数 "wikilink")：
...
这一公式在计算有关[正态分布的一些连续](../Page/正态分布.md "wikilink")[概率分布的数学期望的时候特别有用](../Page/概率分布.md "wikilink")，例如[对数正态分布](../Page/对数正态分布.md "wikilink")。

### n维和泛函一般化

令\(A\)为一个对称的、[正定的](../Page/正定矩阵.md "wikilink")（因而[可逆](https://zh.wikipedia.org/wiki/可逆矩阵 "wikilink")）\(n \times n\)
（即[协方差矩阵的逆矩阵](../Page/协方差矩阵.md "wikilink")），则
...
这里的 σ 表示的是有序集 {1, ..., 2*N*}
的不同[排列](https://zh.wikipedia.org/wiki/排列 "wikilink")。等式右边的系数是对
\(N\) 个重复的 A<sup>-1</sup> 的 {1, ..., 2*N*} 中所有的组合的求和（the sum over all
combinatorial pairings of {1, ..., 2*N*} of *N* copies of
*A*<sup>−1</sup>）。
...
以上积分中的 \(f\)
是[解析函数](../Page/解析函数.md "wikilink")，且函数值的增长必须满足某些边界条件以及另一些特定要求。微分算子的幂可以理解为[幂级数](../Page/幂级数.md "wikilink")。

虽然没有严格的定义，但是我们仍然可以依照有限维的情况“定义”高斯泛函积分。 然而，\((2\pi)^\infty\)
无穷大的问题依然存在，且大部分的也是无穷大的。如果只考虑比例：
...
\[\int_0^\infty x^{n}e^{-ax^2}\,dx = \frac{\Gamma(\frac{n+1}{2})}{2a^{\frac{n+1}{2}}}\]
其中，n 为正整数，“\!\!”表示。 这类积分的一种简单的计算方式是应用对参数进行微分：

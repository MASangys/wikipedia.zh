...
**高斯符號**是一个数学符号，形式为方括号\[x\]，表示不大於（等于或小于）[数x的最大](../Page/数.md "wikilink")[整數](https://zh.wikipedia.org/wiki/整數 "wikilink")，即\(x-1<[x]\leq x\)。
...
Language》中把高斯符号称作**[取底符号](https://zh.wikipedia.org/wiki/取底符号 "wikilink")**（\(\lfloor x\rfloor\)，floor），并同时引进**取顶符号**（\(\lceil x\rceil\)，ceil）(用以表示不小於x的整數中最小的一個)。之后大家就普遍使用取底、取顶这种说法了。

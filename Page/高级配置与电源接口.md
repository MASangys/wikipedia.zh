...
[ACPI规范定义了一台兼容ACPI的](https://zh.wikipedia.org/wiki/ACPI "wikilink")[计算机系统可以有以下七个状态](https://zh.wikipedia.org/wiki/计算机 "wikilink")（所谓的全局状态）：
  ...
  - *'G0*(**S0**)*正常工作状态*：计算机的正常工作状态-操作系统和应用程序都在运行。[CPU](https://zh.wikipedia.org/wiki/CPU "wikilink")(s)执行指令。 在这个状态下(即没有进入G1*睡眠*)，[CPU和像硬盘](https://zh.wikipedia.org/wiki/CPU "wikilink")、DVD驱动器等等这些的设备可以一再的进入和从低能源状态回来，叫做[*C0*-*C*n和](https://zh.wikipedia.org/wiki/#处理器状态 "wikilink")[*D0*-*D3*](https://zh.wikipedia.org/wiki/#设备状态 "wikilink")。(例如[筆記型電腦](../Page/筆記型電腦.md "wikilink")，当使用电池运行的时候通常关掉所有当前未使用的设备；一些[桌上型電腦](../Page/桌上型電腦.md "wikilink")也这么做来减少噪声。)
  - **G1** *睡眠* 细分为从S1到S4等幾種状态。系统从这几种状态被唤醒到G0*运行*（*[唤醒等待](https://zh.wikipedia.org/wiki/唤醒等待 "wikilink")*时间）所需的时间最短的是S1，其次是S2和S3，最後是S4。Linux內核的/sys/power/state檔案中對應的關鍵字與狀態: standby(S1), mem(S3), disk(S4)。在Windows中可用`powercfg /a`命令列出平台支援的ACPI G1狀態。
      ...
      - **S1**：最耗电的睡眠模式。CPU的所有[寄存器](../Page/寄存器.md "wikilink")被刷新，并且[CPU停止执行指令](https://zh.wikipedia.org/wiki/CPU "wikilink")。[CPU和](https://zh.wikipedia.org/wiki/CPU "wikilink")[内存的电源被維持](https://zh.wikipedia.org/wiki/内存 "wikilink")。这种模式通常叫*Power on Suspend*或者叫做[POS](https://zh.wikipedia.org/wiki/POS "wikilink")。老式的电脑对S1支持可能要比S3好。
...
各个状态的定义如下所示：
  ...
  - C1（通常称为*Halt*），拥有最短的唤醒时间，这个延时必须短到操作系统软件使用CPU的时候不会考虑到唤醒时间方面的因素。一些处理器，比如说[奔腾4](../Page/奔腾4.md "wikilink")（[Pentium 4](https://zh.wikipedia.org/wiki/Pentium_4 "wikilink")），支持[C1E](https://zh.wikipedia.org/wiki/C1E "wikilink")（Enhanced C1 state）这样的低电能消耗技术。

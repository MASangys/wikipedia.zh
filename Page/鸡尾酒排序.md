**鸡尾酒排序**，也就是**定向冒泡排序**，**雞尾酒攪拌排序**，**攪拌排序**（也可以視作[選擇排序的一種變形](https://zh.wikipedia.org/wiki/選擇排序 "wikilink")），**漣漪排序**，**來回排序**或**快乐小時排序**，是[冒泡排序的一種变形](../Page/冒泡排序.md "wikilink")。此演算法与[冒泡排序的不同處在於排序時是以双向在序列中進行排序](../Page/冒泡排序.md "wikilink")。
...
鸡尾酒排序等於是[冒泡排序的輕微變形](../Page/冒泡排序.md "wikilink")。不同的地方在於從低到高然後從高到低，而[冒泡排序則僅從低到高去比較序列裡的每個元素](../Page/冒泡排序.md "wikilink")。他可以得到比[冒泡排序稍微好一點的效能](../Page/冒泡排序.md "wikilink")，原因是[冒泡排序只從一個方向進行比對](../Page/冒泡排序.md "wikilink")（由低到高），每次循環只移動一個項目。

以序列(2,3,4,5,1)為例，鸡尾酒排序只需要訪問一次序列就可以完成排序，但如果使用[冒泡排序則需要四次](../Page/冒泡排序.md "wikilink")。但是在亂數序列的狀態下，鸡尾酒排序與冒泡排序的效率都很差勁。
...
## 額外連結
  ...
  - [Java source code and an animated demo of cocktail sort (called
    bi-directional bubble sort) and several other
    algorithms](https://web.archive.org/web/20061008105719/http://www.cs.ubc.ca/~harrison/Java/sorting-demo.html)
  - [Implementations of cocktail sort in several different programming
    languages](https://zh.wikipedia.org/wiki/:wikisource:Cocktail_Sort "wikilink")

在
[实分析中](https://zh.wikipedia.org/wiki/实分析 "wikilink")，由[黎曼创立的](https://zh.wikipedia.org/wiki/黎曼 "wikilink")**黎曼积分**（）首次对[函数在给定](../Page/函数.md "wikilink")[区间上的](https://zh.wikipedia.org/wiki/区间 "wikilink")[积分给出了一个精确定义](../Page/积分.md "wikilink")。黎曼积分在技术上的某些不足之处可由后来的[黎曼－斯蒂尔杰斯积分和](../Page/黎曼－斯蒂尔杰斯积分.md "wikilink")[勒贝格积分得到修补](../Page/勒貝格積分.md "wikilink")。

## 概念

[Integral_as_region_under_curve.svg](https://zh.wikipedia.org/wiki/File:Integral_as_region_under_curve.svg "fig:Integral_as_region_under_curve.svg")与[坐标轴所夹](https://zh.wikipedia.org/wiki/坐标轴 "wikilink")[面积的黎曼积分](../Page/面积.md "wikilink")\]\]

对于一在[区间](https://zh.wikipedia.org/wiki/区间 "wikilink")\(\lbrack a, b \rbrack\)上之给定非负函数\(f(x)\)，我们想要确定\(f(x)\)所代表的[曲线与](../Page/曲线.md "wikilink")\(X\)[坐标轴所夹图形的](https://zh.wikipedia.org/wiki/坐标轴 "wikilink")[面积](../Page/面积.md "wikilink")，我们可以将此记为
...
和式中的每一项是子区间长度\(x_{i+1}-x_i\)与在\(t_i\)处的函数值\(f(t_i)\)的乘积。直观地说，就是以标记点\(t_i\)到X轴的[距离为高](../Page/距离.md "wikilink")，以分割的子区间为长的[矩形的面积](../Page/矩形.md "wikilink")。
...
**另一个定义**:
\(S\)是函数\(f\)在闭区间\([a,b]\)上的黎曼积分，当且仅当对于任意的\(\epsilon > 0\)，都存在一个取样分割\(x_0,\ldots,x_n\)、\(t_0,\ldots,t_{n-1}\)，使得对于任何比其“精细”的分割\(y_0,\ldots,y_m\)
and \(s_0,\ldots,s_{m-1}\)，都有：
...
其次证明满足第二个定义的\(S\)也满足第一个定义。首先引进**[达布积分](../Page/达布积分.md "wikilink")**的概念，第二个定义和达布积分的定义是等价的，具体见[达布积分](../Page/达布积分.md "wikilink")。其次我们证明[达布积分的定义满足第一个定义](../Page/达布积分.md "wikilink")。任选一个分割\(x_0,\ldots,x_n\)使得它的**上达布和**与**下达布和**都与\(S\)相差不超过\(\frac{\epsilon}{2}\)。令\(r\)等于\(\max_{0 \le i \le n-1} (M_i-m_i)\)，其中\(M_i\)和\(m_i\)是\(f\)在\([x_i,x_{i+1}]\)上的[上确界和](https://zh.wikipedia.org/wiki/上确界 "wikilink")[下确界](https://zh.wikipedia.org/wiki/下确界 "wikilink")。再令\(\delta\)是\(\frac{\epsilon}{2rn}\)和\(\min_{0 \le i \le n-1} (x_{i+1}-x_i)\)中的较小者。可以看出，当一个分割的子区间长度最大值小于\(\delta\)时，\(f\)关于它的黎曼和与**上达布和**或**下达布和**至多相差\(\frac{\epsilon}{2}\)，所以和\(S\)至多相差\(\epsilon\)。
...
由于一个函数的黎曼积分是一个实数，因此在固定了一个区间\([a,b]\)后，将一个黎曼可积的函数设到其黎曼积分的映射\(I: f \longrightarrow \int_{a}^{b} f dx\)是所有黎曼可积的函数空间上的一个[线性泛函](https://zh.wikipedia.org/wiki/线性泛函 "wikilink")。
  ...
  - 正定性：如果函数\(f\)在区间\([a,b]\)上[几乎处处](https://zh.wikipedia.org/wiki/几乎处处 "wikilink")（[勒贝格测度意义上](../Page/勒贝格测度.md "wikilink")）大于等于0，那么它在\([a,b]\)上的积分也大于等于零。如果\(f\)在区间\([a,b]\)上几乎处处大于等于0，并且它在\([a,b]\)上的积分等于0，那么\(f\)几乎处处为0。
...
无论*a*、*b*、*c*之间的大小关系如何，以上关系式都成立。
  ...
  - \([a,b]\)上的实函数\(f\)是黎曼可积的，当且仅当它是[有界和](../Page/有界函数.md "wikilink")[几乎处处](https://zh.wikipedia.org/wiki/几乎处处 "wikilink")[连续的](../Page/连续函数.md "wikilink")。
...
\[\int_{a}^{b} f\, dx = \int_a^b{\lim_{n \to \infty}{f_n}\, dx} = \lim_{n \to \infty} \int_{a}^{b} f_n\, dx.\]
  ...
  - 如果一个实函数在区间\([a,b],\)上是[单调的](../Page/单调函数.md "wikilink")，则它是黎曼可积的，因为其中不连续的点集是至多可列集。
...
黎曼积分只定义在有界区间上，扩展到无界区间并不方便。可能最简单的扩展是通过极限来定义积分，即如同[反常积分](https://zh.wikipedia.org/wiki/反常积分 "wikilink")（improper
integral）一样。我们可以令

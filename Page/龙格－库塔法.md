[数值分析中](../Page/数值分析.md "wikilink")，**龙格－库塔法**（Runge-Kutta
methods）是用于[非线性常微分方程的解的重要的一类隐式或显式迭代法](../Page/非线性常微分方程.md "wikilink")。这些技术由数学家[卡尔·龙格和](../Page/卡尔·龙格.md "wikilink")[马丁·威尔海姆·库塔于](../Page/马丁·威尔海姆·库塔.md "wikilink")1900年左右发明。

## 经典四阶龙格库塔法

在各種龙格－库塔法當中有一個方法十分常用，以至于经常被称为“RK4”或者就是“龙格－库塔法”。该方法主要是在已知方程导数和初值信息，利用计算机仿真时应用，省去求解微分方程的复杂过程。

令[初值问题表述如下](../Page/初值问题.md "wikilink")。
...
这样，下一个值（*y*<sub>*n*+1</sub>）由现在的值（*y*<sub>*n*</sub>）加上时间间隔（*h*）和一个估算的斜率的乘积所决定。该斜率是以下斜率的加权平均：
  ...
  - *k*<sub>2</sub>是时间段中点的斜率，通过[欧拉法采用斜率](../Page/欧拉法.md "wikilink")*k*<sub>1</sub>来决定*y*在点*t*<sub>*n*</sub>
    + *h*/2的值；
...
RK4法是四阶方法，也就是说每步的误差是*h*<sup>5</sup>[阶](../Page/大O记法.md "wikilink")，而总积累误差为*h*<sup>4</sup>阶。
...
tableau*（得名自[John C. Butcher](../Page/John_C._Butcher.md "wikilink")）：
...
在显式龙格库塔方法的框架里，定义参数\(a_{ij}\)的矩阵是一个[下三角矩阵](../Page/下三角矩阵.md "wikilink")，而隐式龙格库塔方法并没有这个性质，这是两个方法最直观的区别：
